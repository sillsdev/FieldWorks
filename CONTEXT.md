...
- **Hotlinks**: The always-visible strip of section-level commands under a DataTree section header — the layout `hotlinks=` attribute, `DetailField.HotlinksId`, the `-Hotlinks` menu-id suffix, legacy `SummaryCommandControl`, Avalonia `DataTree.CreateHotlinkStrip`. Say **hotlinks**, not "quick links": the latter appears nowhere in the repo. Legacy paints one clickable label per command with per-command enabled state.
...
- DataTree object commands split by test environment. Insert commands are always enabled, so they run headlessly end-to-end through `MockFwXWindow` -> `XCoreMenuBridge` -> mediator -> unit of work (see `DetailObjectCommandExecutionTests`). Delete, Move, Demote, and Merge only materialize and enable when xCore display handlers can read live slice-sequence context from a laid-out, VISIBLE legacy DataTree, which headless runs do not have; those tests are `[Explicit]` and desktop-environment only. Do not write a plan or a claim that implies headless coverage of the second group.
