...
.\build.ps1 -BuildInstaller -InstallerOnly -InstallerToolset Wix6
...
.\build.ps1 -BuildInstaller -InstallerToolset Wix6 -MsBuildArgs '/p:FastInstallerBuild=0'

# Use normal (slower) bundle embedding/compression in Debug
.\build.ps1 -BuildInstaller -InstallerToolset Wix6 -MsBuildArgs '/p:FastBundleBuild=0'
```

## Outputs

Build outputs go under `FLExInstaller/wix6/bin/<platform>/<configuration/>`.

- MSI and MSI symbols (culture-specific):
  - `FLExInstaller/wix6/bin/x64/<Config>/en-US/FieldWorks.msi`
  - `FLExInstaller/wix6/bin/x64/<Config>/en-US/FieldWorks.wixpdb`
- Bundle and bundle symbols:
  - `FLExInstaller/wix6/bin/x64/<Config>/FieldWorksBundle.exe`
  - `FLExInstaller/wix6/bin/x64/<Config>/FieldWorksBundle.wixpdb`
