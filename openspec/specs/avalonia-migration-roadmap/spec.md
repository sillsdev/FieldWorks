# avalonia-migration-roadmap Specification
...
view-definition IR path (`ViewDefinitionModel` → `DetailModel`), not as a standalone end
state.
...
> as the detail-model path (`ViewDefinitionModel`/`DetailModel`) inside
