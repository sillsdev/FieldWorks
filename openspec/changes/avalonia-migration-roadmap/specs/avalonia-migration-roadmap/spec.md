...
> this change's `design.md` for the full context. (`datatree-model-view-separation` was research-only,
> never implemented, and its change folder was removed 2026-07-02.) The scenario below is retained
> for historical traceability but no longer reflects the as-built architecture — see the as-built
> vocabulary diagram in `design.md` instead.
