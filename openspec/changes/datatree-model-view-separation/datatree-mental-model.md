...
This document explains the **current** runtime architecture around `DataTree` in FieldWorks.

> **Status note (2026-06-09).** This document originally also described how the proposed
> `DataTreeModel`/`SliceSpec`/`IDataTreeView` split would change the architecture below. That plan is
> superseded — see `proposal.md` and `hybrid-alignment.md`. The historical "target shape" section has
> been removed; only the current-state description (still accurate) is kept.
...
Practical impact expectation: **minimal/no direct `Src/views` code impact** for any future work in this area.
