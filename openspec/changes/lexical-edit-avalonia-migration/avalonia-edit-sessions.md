...
|---|---|---|
| Concrete session | [Src/LexText/AdvancedEntry.Avalonia/Services/AdvancedEntryEditSession.cs](Src/LexText/AdvancedEntry.Avalonia/Services/AdvancedEntryEditSession.cs) | Starts a fenced LCModel undo task for a selected entry, then exposes `Save()` and `Cancel()`. |
| View-model ownership | [Src/LexText/AdvancedEntry.Avalonia/ViewModels/MainWindowViewModel.cs](Src/LexText/AdvancedEntry.Avalonia/ViewModels/MainWindowViewModel.cs) | Loads one entry lifetime, disposes it on save/cancel, and requests window close through a callback. |
