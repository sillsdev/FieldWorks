...
**19f — Browse remainders:** **Rapid Data Entry** new row;
...
Right-click row context menu currently shows only Cell Copy/Paste — host commands (Delete
Entry, Show in Concordance, etc.) are NOT wired yet in this build; **do not file that as a new
bug**, it's tracked as tasks.md 19i.3. There is likewise no UI trigger for CSV export yet
(tracked as tasks.md 19i.11) — please don't spend time hunting for one.
...
- **Import/Export dialogs** (LIFT/SFM/CSV wizards). The browse "export visible rows to CSV"
  computation exists but has no menu/toolbar trigger yet in this build either (tasks.md 19i.11) —
  not tester-reachable, so don't file it as a separate bug.
