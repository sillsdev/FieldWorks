...
**Affordance-button selection-collapse bug (xcut-review-2026-06-21.json, tasks.md
19i.2/19i.9).** Every gesture in this table and in the char-style/ws-retag rows
below is driven by a focusable trigger button (style/WS picker triggers, the
link button, the ORC-delete button) that reads `box.SelectionStart/End` at
click/open time. Because the buttons were plain focusable `Button`s, a real
pointer click moved focus off the `TextBox` first, which collapses its
selection to the caret (Avalonia 11.3.17 behavior) — so every row marked
"§19c (ship)" above (link insert, link edit-URL) plus the char-style and
ws-retag pickers below were a no-op in production; only tests that skipped the
real pointer click (setting `SelectionStart/End` directly, or using
`RaiseEvent`/`ShowAt` instead of a click) stayed green. Fixed by setting
`Focusable=false` on all span-applying triggers (19i.2); guarded going forward
by `RichTextTriggerButtons_AreNonFocusable_...` (19i.9). Any NEW span-affordance
button added to this surface must carry `Focusable=false` or it silently
regresses to this bug.
