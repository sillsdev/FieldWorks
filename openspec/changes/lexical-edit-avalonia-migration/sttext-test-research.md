...
// PARITY (xcut-review-2026-06-21.json, tasks.md 19i.6): `RegionFieldControlFactory`
does not forward `context.MenuRequested` to `FwStructuredTextField` (the
multistring/`FwMultiWsTextField` case does), so the in-text **Lexicon Lookup** /
**Add to Lexicon** context-menu commands available on the legacy word-aware
`StTextView` right-click menu are not reachable on the Avalonia StText editor.
The row-level kebab menu is wired separately and is unaffected.
