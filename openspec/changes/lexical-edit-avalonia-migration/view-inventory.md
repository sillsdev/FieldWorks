...
> **Superseded (historical).** This section describes the pre-implementation prototype/spike and
> its gaps as they stood before production work started. It has since been superseded by the
> built `LexicalEditRegionModel`/`FullEntryRegionComposer` region model, described at length in
> `tasks.md` in this directory. Read this section as historical record of where the spike started,
