...
1. **WinForms modal dialogs launched from an active Avalonia surface** (choosers, message boxes,
   ...
   `PocWinFormsHostControl`/host), never `null` and never an Avalonia handle. This guarantees
...
3. **Avalonia flyouts/popups inside the WinForms-hosted surface**: prefer flyouts attached to the
   triggering control (the POC chooser pattern). Account for the known 11.x popup-DPI quirk on
...
- Covered now: focus-return contract at the seam level (`IHostSurface` focus API,
  `HostSurfaceContractTests`); popup focus return inside the surface (`PocLexEntrySliceTests`).
