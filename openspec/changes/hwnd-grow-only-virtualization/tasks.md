...
Decision: keep **Phase 0 (measurement baseline)** only and revert all behavioral virtualization work.

Rationale summary:

- Render benchmark profiling shows warm render time is dominated by duplicate Views layout passes (`Reconstruct` + `PerformOffscreenLayout`), not WinForms HWND creation.
- The native layout guard path (`PATH-L1`) is lower risk and materially higher impact for the current objective.
- Deferred HWND creation touches broad UI lifecycle and accessibility/focus/event timing surfaces; risk is high for current schedule.

Reversion plan (executed in this branch):

- Revert Phase 1-3 code paths (`DeferSliceHwndCreationEnabled`, `EnsureHwndCreated`-driven install/layout behavior, virtual-layout model hooks).
- Preserve Phase 0 instrumentation only:
	- `HwndDiagnostics` USER/GDI handle counters.
	- `DataTreeRenderHarness` handle telemetry.
	- `Slice.InstallCreationCount` baseline counters.
	- Baseline `DataTreeHwndCountTest` assertions.
...
- [x] 0.5 Record baseline numbers in `research/baseline-measurements.md` for: Lexicon Edit (253-slice entry), Grammar Edit (typical category), Lists Edit (typical item). [Documentation, ~1 hr]
...
## Phase 1+: Deferred (not in this change)

All Phase 1-4 items are intentionally deferred and out of scope for this branch. The previous exploratory implementation was reverted to reduce risk and keep focus on Views-engine layout optimizations.

- [ ] 1.1+ Deferred. Re-open only if Views-side optimizations fail to meet performance targets.
...
## Validation Checklist (Phase 0 scope)
