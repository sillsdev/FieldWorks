...
namespace FwBuildTasks
{
	...
	public class ClouseauTests
	{
		...
		public void ProperlyImplementedIDisposable_LogsNeitherErrorsNorWarnings()
		{
			...
			Assert.That(1, Is.LessThanOrEqualTo(_tbi.Messages.Count), string.Join(Environment.NewLine, _tbi.Messages));
		}
		...
		public void NoProtectedDisposeBool_LogsError()
		{
			...
			StringAssert.That(_tbi.Errors[0], Does.Contain(type.FullName));
		}
		...
		public void DisposeBoolDoesNotWriteWarning_LogsError()
		{
			...
			StringAssert.That(error, Does.Contain(type.FullName));
			StringAssert.That(error, Does.Contain("Missing Dispose() call"));
		}
