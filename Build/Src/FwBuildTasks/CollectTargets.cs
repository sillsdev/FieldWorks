		...
		private void WriteTargetFiles()
		{
			...
			try
			{
				...
				using (var writer = new StreamWriter(targetsFile))
				{
					...
					writer.WriteLine($"<Project xmlns=\"http://schemas.microsoft.com/developer/msbuild/2003\" ToolsVersion=\"{ToolsVersion}\">");
					writer.WriteLine();
					foreach (var project in m_mapProjFile.Keys)
					{
						...
						foreach (XmlNode node in ConfigNodes)
						{
							...
							if (condition.Contains("Debug") && !otherwiseAdded)
							{
								...
								otherwiseBldr.AppendLine(string.Format("\t\t\t\t<{0}Defines>{1} CODE_ANALYSIS</{0}Defines>", project.Replace(".", string.Empty),
