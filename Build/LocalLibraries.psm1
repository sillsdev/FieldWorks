...
function Get-FieldWorksLocalPackVersion {
	...
	# The g prefix is git describe's: SemVer rejects a numeric pre-release identifier
	# with a leading zero, so an all-digit sha would otherwise be unparseable.
	return "$core-$($SourceState.Label).g$($SourceState.ShortSha)"
}
