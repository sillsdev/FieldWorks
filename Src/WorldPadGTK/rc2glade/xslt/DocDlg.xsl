...
<xsl:template match="child[widget/@id = 'kctidOk']">
	<xsl:if test="not(ancestor::widget[@id = 'kridDocDlg'])">
		<xsl:copy-of select="." />
	</xsl:if>
</xsl:template>
...
<xsl:template match="widget[@id = 'kctidDocRtl']">
	<xsl:choose>
		<xsl:when test="ancestor::widget[@id = 'kridDocDlg']">
			<widget class="{@class}" id="{@id}">
				<xsl:apply-templates select="property" />
				<property name="group">
					<xsl:text>kctidDocLtr</xsl:text>
				</property>
			</widget>
		</xsl:when>
		<xsl:otherwise>
