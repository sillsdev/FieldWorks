	...
	for (int isbt = 0; isbt < csbt && ichLim > 0; ++isbt)
	{
		...
		if (qtms)
		{
			...
			if (cch > ichMin)
			{
				...
				int ichMinTss = max(0, ichMin);
				int ichLimTss = min(ichLim, cch);
	...
	VwPropertyStore ** ppzvps)
{
	...
	{
		m_vpst[i].qtms->get_Length(&cch);
		cchSum += cch;
	}
	StrUni msg;
	msg.Format(L"Argument is too large (Details: ich=%d, csbt=%d, last string length=%d, cumulated string length=%d)",
		ich, csbt, cch, cchSum);
	...
	IVwViewConstructor * pvc)
{
	...
	for (const OLECHAR * pch = prgch; pch < prgch + cch; pch++)
	{
		if (*pch == 0xfffc)
		{
			...
			if (*pchData == kodtGuidMoveableObjDisp)
			{	// picture
				ITsStrBldrPtr qtsb;
...
int VwConcTxtSrc::RenToLog(int ichlog)
{
	return SuperClass::LogToRen(ichlog + m_cchDiscardInitialRen) - m_cchDiscardInitial;
}
/*----------------------------------------------------------------------------------------------
...
void VwMappedTxtSrc::ReplaceContents(int itssMin, int itssLim, VwTxtSrc * pts)
{
	...
	if (pmts)
	{
		...
		if (itmiMin > 0)
		{
			...
			dichren += vtmi[itmi - 1].DichRenLog();
		}
...
void VwOverrideTxtSrc::Merge(LgCharRenderProps * pchrpOrig, LgCharRenderProps & pchrpOverride)
{
	...
	if (pchrpOverride.szFaceName != NULL && wcslen(pchrpOverride.szFaceName) != 0)
		wcscpy_s(pchrpOrig->szFaceName, pchrpOverride.szFaceName);
	if (pchrpOverride.szFontVar != NULL && wcslen(pchrpOverride.szFontVar) != 0)
		wcscpy_s(pchrpOrig->szFontVar, pchrpOverride.szFontVar);
}
...
template Vector<TextMapItem>; // TmiVec;
template Vector<DispPropOverride>; // PropOverrideVec;
