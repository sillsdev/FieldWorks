...
BoxAccessorMap VwAccessRoot::m_hmboxacc;
...
VwAccessRoot::VwAccessRoot(VwBox * pbox)
{
	...
	m_hmboxacc.Insert(pbox, (IAccessible *) this);
}
...
{
	IAccessiblePtr qacc;
	if (m_hmboxacc.Retrieve(pbox, qacc))
	{
		m_hmboxacc.Delete(pbox);
...
STDMETHODIMP VwAccessRoot::Next(unsigned long celt, VARIANT FAR * prgvar, unsigned long FAR* pceltFetched)
{
	...
	uint iaccLim = min(iaccMin + celt, (uint)m_pbox->ChildCount());
...
template ComHashMap<VwBox *, IAccessible>; // BoxAccessorMap; // Hungarian hmboxacc;
