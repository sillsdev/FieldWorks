	...
	Rect rcSrcRoot, Rect rcDstRoot, bool fIsLastParaOfSelection)
{
	...
	if (ichMin <= ichOurMin && (ichOurMin + ichLimSeg) <= ichLim)
	{
		...
		if (pboxNext && pboxNext->Baseline() == Baseline())
		{
			...
			// of. It doesn't matter for the start or end of a selection, but if it is in
		...
		Rect rcSrcRoot, Rect rcDstRoot, Rect rcSrc, Rect rcDst)
{
	...
	if (sbstr.Length() > 0)
	{
			...
			|| chFirst == kodtOwnNameGuidHot)
		{
			...
			CheckHr(qvvc->DoHotLinkAction(sbstr, hvoOwner, tag, qtssProp, ichObj));
		}
	}
}
...
OLECHAR * VwStringBox::DebugStr()
{
	if (!::_CrtIsValidPointer(this, isizeof(this), TRUE))
		return L"A bad string box pointer";
	if (rgchDebugStr[0] != 0xffff)
		return L"A corrupted string box";
	VwParagraphBox * pvpbox = dynamic_cast<VwParagraphBox *>(Container());
	if (!pvpbox)
		return L"Lost string box";
	if (!pvpbox->Source())
		return L"String box w/o source";
	if (!m_qlseg)
		return L"String box w/o segment";
	int dich;
	if (FAILED(m_qlseg->get_Lim(IchMin(), &dich)))
		return L"String box--get_Lim failed";
	int ichLimOrig = IchMin() + dich;

	int ichLim = min(ichLimOrig, IchMin() + MAX_DEBUG_STR_LEN);
...
//	int m_yBaseLine;		// baseline of line being laid out
	...
	bool InitDropCaps()
	{
		...
		bool fWantDropCapHere = tri.ichLim != 0; // don't want a drop cap here if para is empty.
		if (fWantDropCapHere)
		{
		...
		for (; pbox; pbox = pboxNext)
		{
			...
			else
			{
				...
				if (pbox->IsBoxFromTsString())
				{
					...
					{
						// eg, a picture box: discard it; as a layout extension nothing
	...
	virtual ~ParaBuilder()
	{
		...
		// Actually delete any discarded string boxes that have not been reused
		while (m_psboxReusable)
		{
			VwStringBox * psbox = static_cast<VwStringBox *>(m_psboxReusable->Next());
			delete m_psboxReusable;
			m_psboxReusable = psbox;
		}
		// Actually delete any leftover moveable piles (and their contents and notifiers).
		while (m_pmpboxReusable)
		{
			VwMoveablePileBox * pmpbox = static_cast<VwMoveablePileBox *>(m_pmpboxReusable->NextOrLazy());
...
//		(*ppsbox)->SetTrWsHandling(twsh);
	...
	void SetWeakDirections(int nTopDepth)
	{
		...
		for (ibox = 0; ibox < m_vbox.Size(); ++ibox)
		{
			...
			for (int ibox2 = ibox; --ibox2 >= 0; )
			{
				if (!vfWeak[ibox2])
				{
					nDepthPrev = min(nDepthPrev, vnDepth[ibox2]);
					break;
				}
			}
			...
			for (int ibox2 = ibox + 1; ibox2 < m_vbox.Size(); ++ibox2)
			{
				if (!vfWeak[ibox2])
				{
					nDepthNext = min(nDepthNext, vnDepth[ibox2]);
					break;
				}
			}
			...
			int nNewDepth = min(nDepthPrev, nDepthNext);
	...
	// Note that this does NOT give a ref count on the notifier.
...
//-		m_pts->CharAndPropsAt(m_pts->LogToRen(m_ichMinString), &ch, &qttp);
		...
		if (sbstr.Length() >= 1)
		{
			...
			byte * pbData = (byte *)(const_cast<OLECHAR *>(pchData + 1));
			switch(*pchData)
			{
			case kodtPictOdd:
				cbData--;
			...
			case kodtPictEven:
				{
					...
					else
					{
						...
						m_pboxpic = NewObj VwPictureBox(qzvpsReset, qpic);
...
//-							m_pts->CharAndPropsAt(m_pts->LogToRen(m_ichMinString), &ch, &qttp);
			...
			case kodtGuidMoveableObjDisp:
				...
				// Enhance JohnT: we could also display an anchor symbol as part of this line.
			...
			case kodtNameGuidHot:
				...
				// the ORC into). Otherwise, things to wrong wrapping paragraphs with links.
				return false;
			default:
				break;
			}
...
//-				m_pts->CharAndPropsAt(m_pts->LogToRen(m_ichMinString), &ch, &qttp);
	...
	bool Justify()
	{
		...
		if (m_dxWidthRemaining < 0)
		{
			...
			_itoa_s(m_clines + 1, rgch, 10);
			strTmp += rgch;
			strTmp += L" by ";
			_itoa_s(m_dxWidthRemaining * -1, rgch, 10);
			strTmp += rgch;
			strTmp += L" units\n";
	...
	void OutputLine()
	{
		...
		if (m_nMaxDirectionDepth > 0)
		{
			...
			for (i = 0; i < m_vbox.Size(); ++i)
			{
				...
				dyMaxDescent = max(dyMaxDescent, pbox->LineSepDescent(fAloneOnLine));
				dyMaxAscent = max(dyMaxAscent, pbox->Ascent());
			}
		...
		if (m_pxvo && !m_fSemiTagging && !m_fExactLineHeight)
		{
			...
			dyMaxDescent = max(dyMaxDescent, dydMinDescent);
		}
		...
		if (est != kestNoMore && m_ichMinString != -1)
		{
				...
				m_fDidOneExtraLine)
			{
				...
				{
					dyMaxDescent = max(dyMaxDescent, pbox->Descent());
					dyMaxAscent = max(dyMaxAscent, pbox->Ascent());
				}
		...
		if (psboxLast)
		{
			...
			if (ichLimBacktrack > m_ichMinString)
			{
				...
				if (nSegDirDepth == TopDepth())
					...
					m_twsh = ktwshNoWs;
				ILgSegmentPtr qlseg;
				int dxWidth, dichLimSeg; //dichContext;
				LgEndSegmentType est;
	...
	VwNoteProps * pvnp, int * pitssProp, ITsString ** pptssProp)
{
	...
	int itssProp;
	ITsStringPtr qtssProp;
	int ipropBest; // same index in current best notifier
	...
	BoxSet * pboxsetDeleted)
{
	...
	Root()->FixSelectionsForStringReplacement(Source(), itssMin, itssLim, pvpboxRep->Source());
	...
	VpsTssVec & vpst = pvpboxRep->Source()->Vpst();
	Root()->FixTextEdit(this, itssMin, itssLim, vpst); // adjust selection
...
int VwParagraphBox::MaxLines()
{
	...
	// is less.
	return min(nlinesMax, nlinesMaxCont);
}
	...
	int * pysTop, int * pysBottom)
{
	...
	if (Source()->Overlay())
	{
		...
		ysBottom = max(ysBottom, dysMinDescent + ysBaseline);
	}
	// Exit when we find a different baseline or run out of boxes.
	// In either case, pbox is the value to return.
	*pysTop = ysTop;
...
// copy of bullet array from FmtBulNumDlg.cpp
static char * s_rgszBulletOptions[] = {
  "\x9e", "\x9f", "l", "m", "\xa7", "n", "\xfa", "o", "q", "r", "s", "t", "u", "v", "z", "F",
  "U", "V", "\xb6", "\xd8", "\xdc", "\xe0", "\xe8", "\xf0", "\xfc"
};
...
StrUni VwParagraphBox::GetBulNumString(IVwGraphics * pvg, COLORREF * pclrUnder, int * punt)
{
	...
	int nVal;
	StrUni stuNum; // May hold number temporarily
	if (vbn == kvbnNone)
		return StrUni(L"");
	...
	*punt = chrp.m_unt;
	...
	while (pchProps < pchPropsLim)
	{
		...
		{
			wcscpy(chrp.szFaceName, pchProps);
	...
	default:
		if (vbn >= kvbnBulletBase &&
			vbn < kvbnBulletBase + isizeof(s_rgszBulletOptions) / isizeof(char *))
		{
			// bullet
			wcscpy(chrp.szFaceName, L"WingDings"); // only font that works for bullets
			CheckHr(pvg->SetupGraphics(&chrp));
			StrUni stuText;
			stuText.Format(L"%S", s_rgszBulletOptions[vbn - kvbnBulletBase]);
			return stuText;
		}
		...
		break;
	case kvbnNone:
		return StrUni(L""); // don't bother to setup graphics
	case kvbnArabic:
		nVal = ParaNumber();
LArabic:
		_itow(nVal,pch,10);
	...
	case kvbnRomanLower:
		...
		if (nVal > 14999)
			goto LArabic;
		wcscpy(pch, s_rgszDig1000[nVal / 1000]);
		pch += wcslen(pch);
		wcscpy(pch, s_rgszDig100[(nVal / 100) % 10]);
		pch += wcslen(pch);
		wcscpy(pch, s_rgszDig10[(nVal / 10) % 10]);
		pch += wcslen(pch);
		wcscpy(pch, s_rgszDig0[nVal % 10]);
	...
	wcscpy(chrp.szFaceName, stuFaceName.Chars());
	...
	CheckHr(pvg->SetupGraphics(&chrp));
	StrUni stuText;
	stuText.Format(L"%s%s%s", m_qzvps->NumTxtBefore().Chars(), rgchNum,
...
public:
		...
		for (VwBox * pboxLine = FirstBox(); pboxLine && pboxLine->Top() != knTruncated; )
		{
			...
			for (int ibox = 0; ibox < plineCurrent->vbox.Size() && !fAnyColoredBackground; ibox++)
			{
				...
				while (ich < ichLim)
				{
					...
					Source()->GetCharProps(ich, &chrp, &ichMinRun, &ichLimRun);
			...
			if (stuBulNum.Length())
			{
				...
				if (RightToLeft())
				{
					...
					xdNum = rcSrc.MapXTo(pvpbox->m_dxsRightEdge, rcDst) -
						max(0, MulDiv(Style()->FirstIndent(), rcSrc.Width(), kdzmpInch)) -
						dxdWidth;
				}
				else
				{
	...
	psbox->DebugDrawBorder(pvg, rcSrc, rcDst);
				...
				for (; ichMinRun < ichLim; ichMinRun = ichLimRun)
				{
					...
					ichLimRun = min(ichLimRun, ichLim);
	...
	int ydTopText, int ydBottomText, TagSelectInfo * ptsi, FobData & fobd)
{
	...
	CheckHr(pvg->GetTextExtent(3, L"...", &dxdMissing, &dydHeight));
	if (vti.Size() > 1)
	{
		CheckHr(pvg->GetTextExtent(1, L",", &dxdSpacer, &dydHeight));
	}
	int ctagsDisp;
	CheckHr(pvo->get_MaxShowTags(&ctagsDisp));
	ctagsDisp = min(ctagsDisp, vti.Size()); // Number we will display...assume all
...
LFixedSpace:
	...
	if (fOpening)
	{
		...
		else
		{
			...
			{
				int dxdThick = max(rcDst.Width() / 96, 1);
				int dydThick = max(rcDst.Height() / 96, 1);
	...
	for (iti = 0; iti < ctagsDisp; iti++)
	{
		if (iti)
		{
			...
			if (!ptsi)
			{
				...
				CheckHr(pvg->DrawText(xdPos, ydTopTag, 1, L",", 0));
			}
			xdPos += dxdSpacer;
		}
	...
	if (cchMissing && !ptsi)
	{
		...
		CheckHr(pvg->DrawText(xdPos, ydTopTag, cchMissing, L"...", 0));
	}
}
...
public:
	...
	void CheckWord(int ichMinWord1, int ichLimWord1)
	{
		...
		SmartBstr sbstrWsId;
		IWritingSystemPtr qwse;
		CheckHr(m_qwsf->get_EngineOrNull(ws, &qwse));
		if (!qwse)
			return;
		CheckHr(qwse->get_SpellCheckDictionary(&sbstrWsId));
	...
	IVwOverlay * pvo, VecTagInfo & vtiDiffs, TagSelectInfo * ptsi)
{
	...
	int cguidKeep = min (cguidMaxDiff, vtiDiffs.Size());
	...
	int & dyTagAbove, int & dyTagBelow)
{
	...
	if (pxvo && !m_fSemiTagging)
	{
		...
		if (vof & kgrfofTagAnywhere)
		{
			...
			wcsncpy_s(chrp.szFaceName, prgchSrc, min(32, BstrLen(sbstrFont) + 1));
...
{
	return m_qzvps->PadLeading() + max(-m_qzvps->FirstIndent(), 0);
}
...
VwBox * VwParagraphBox::FindNonPileChildAtOffset (int dysPosition, int dpiY, int * pdysOffsetIntoBox)
{
			...
			pbox = pbox->NextRealBox())
		{
			...
			ysBottomNoMargins = max(ysBottomNoMargins, ysBoxBottomNoMargins);
		}
...
template Vector<VwParagraphBox::TagInfo>;
template Vector<int *>;
