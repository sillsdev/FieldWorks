		...
		for (int ipbrec = 0; ipbrec < vpbrec.Size(); ipbrec++)
		{
...
#if 0
						VARIANT v;
						CheckHr(qsda->get_Prop(hvo, tag, &v));

						CheckHr(pvvc->DisplayVariant(qzvwenv, tag, v, frag, &qtssNew));
			...
			else
			{
				...
				case kvnpTimeProp:
					...
					break;
				case kvnpGenDateProp:
					int gdat;
					CheckHr(qsda->get_IntProp(hvo, tag, &gdat));
					VwEnv::GenDateToTsString(gdat, prootb->GetDataAccess(), &qtssNew);
					break;
	...
	for (VwBox * pbox = pboxFirst; pbox != pboxLimProp; pbox = pbox->NextOrLazy())
	{
		...
		if (plzb && plzb->Object() == Object()) // JT: is this enough to ensure it is the right flid?
		{
			...
			else
			{
				...
				int ihvoMinRep = max(ihvoMin, ihvoMinLazy);
				int ihvoLimRep = min(ihvoMin + chvoDel, ihvoMinLazy + chvoLazy);
				if (!plzbIns)
					plzbIns = plzb;
...
/*----------------------------------------------------------------------------------------------
	...
	and char range info.
----------------------------------------------------------------------------------------------*/
void VwNotifier::GetPropOccurrences(int tag, PropBoxList& vpbrec)
{
	int * ptag = Tags();
...
public:
	...
	void CheckForAltMatch(int iprop)
	{
		...
		for (VwBox * pbox = pnote->Boxes()[iprop]; pbox && pbox != pboxLim; pbox = pbox->NextOrLazy())
		{
	...
	VwBox * pboxNewFirst, VwBox * pboxNewLast, NotifierVec & vpanoteDel)
{
	if (pboxNewFirst != NULL)
	{
		if (pboxOldFirst == m_pboxKey)
		{
			// The superclass method in this case would switch this notifier to monitor the
			// new box. But that is wrong: we were monitoring this box because something might
			// change affecting stuff inside that box (e.g., a missing property getting some content).
			// 'this' therefore came into existence as part of creating the display of its keybox.
			// If the replacement box needs a notifier of this type, a new one will have been
			// created as a result of building that box. Keeping this one will at best mean that
			// we do a double regenerate of that box when the property this cares about changes.
			// At worst, it may do an unnecessary regenerate at a harmful moment (TE-8270).
			m_pboxKey->Root()->DeleteNotifier(this);
			vpanoteDel.Push(this);
			return;
		}
...
void VwRegenNotifier::Regenerate()
{
	...
	if (vnp != kvnpObjProp)
	{
		...
		for (inote = 0; inote < vpanote.Size(); inote++)
		{
			...
			ihvoMin = min(ihvoMin, ihvo);
			ihvoLim = max(ihvoLim, ihvo + 1);
		}
		if (ihvoMin == INT_MAX)
		{
