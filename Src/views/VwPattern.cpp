...
VwPattern::VwPattern()
{
	...
	m_sbstrDefaultCharStyle = _T("<!default chars!>");
}
...
/*----------------------------------------------------------------------------------------------
	...
	Returns null (but not a bad HR) for index out of range.
...
STDMETHODIMP VwPattern::get_Group(int iGroup, ITsString ** pptssGroup)
{
	...
	if (U_FAILURE(status))
		return S_OK; // don't treat as bad HR, that's too severe. Just return null string.
...
/*----------------------------------------------------------------------------------------------
	Test whether there is a match for your pattern starting at pch in up to cch characters.
	Assume the characters at pch have been normalized etc. as necessary.
	Return true if matched. Also give the index (relative to pch) of the end of the range
	that matches. (Later versions of this routine, with more complex pattern-matching, may
	make this a non-trivial return value.)
----------------------------------------------------------------------------------------------*/
bool VwPattern::Match(OLECHAR *pch, int cch, int * pichLim, IVwSearchKiller * pxserkl)
{
	if (pxserkl)
		CheckHr(pxserkl->FlushMessages());

	if (cch < m_stuCompiled.Length())
		return false;
	OLECHAR * pchLim = pch + m_stuCompiled.Length();
	...
	int nTestCounter = 0;
	for (const OLECHAR * pch2 = m_stuCompiled.Chars(); pch < pchLim; pch++, pch2++)
	{
		if (pxserkl)
		{
			nTestCounter++;
			// Test for search abort every 200 characters:
			if (nTestCounter > 200)
			{
				nTestCounter = 0;
				CheckHr(pxserkl->FlushMessages());
	...
	OLECHAR * pchBuf2, ILgCharacterPropertyEngine * pcpe, int cchBuf)
{
	...
	for ( ; ; )
	{
		...
		else if (ichOut < ich)
		{
			...
			ichOutLow = ichOut;
			ichGuess = min(ichGuess + ich - ichOut, ichGuessHigh - 1);
		}
		else
		{
			...
			ichOutHigh = ichOut;
			ichGuess = max (ichGuess + ich - ichOut, ichGuessLow + 1);
		}
...
public:
			...
			while (ichRen < ichLimRen)
			{
				...
				if (MatchProps(chrp, sbstrNamedStyle, sbstrTags, ws, ichRen))
				{
					if (m_ichMinFoundSearch < 0)
					{
						...
						int ichMinFoundRen = max(ichMinRunRen, ichMinRen);
						CheckHr(m_pts->RenToSearch(ichMinFoundRen, &m_ichMinFoundSearch));
					}
					int ichLimFoundRen = min(ichLimRunRen, ichLimRen);
	...
	bool Run()
	{
		...
		* (m_pchBuf + m_cchSrcSearch) = 0; // null termination required.
		m_ichLimSearch = min(m_cchSrcSearch, m_ichLimSearch); // Because of disregarded ORCs, we might get fewer charcaters in the buffer than we asked for.
	...
	bool CheckMatchAndProps()
	{
		...
		for (int ich = m_ichMinFoundSearch; ich < m_ichLimFoundSearch; )
		{
			...
			CheckHr(m_pts->SearchToRen(ich, false, &ichRen));
			m_pts->GetCharProps(ichRen, & chrp, & ichRenMin, & ichRenLim);
			...
			if (m_pat->m_fMatchWritingSystem)
			{
			...
			int ichLimCopy = min(ichLimRun, m_ichLimFoundSearch);
...
STDMETHODIMP VwPattern::MatchWhole(IVwSelection * psel, ComBool * pfMatch)
{
	...
	*pfMatch = false;
	...
	int ichMinTestLog = min(qselQuery->AnchorOffset(), qselQuery->EndOffset());
	int ichLimTestLog = max(qselQuery->AnchorOffset(), qselQuery->EndOffset());
...
void VwPattern::RemoveIgnorableRuns(ITsString * ptssIn, ITsString ** pptssOut)
{
	...
	for (int irun = 0; irun < crun; irun++)
	{
		...
		if (m_pcoll->compare(L"", 0, pchBuf + tri.ichMin, cchRun) != 0)
		{
			...
			qtpb->GetTextProps(&qttpReduced);
			qtsb->ReplaceRgch(cchBldr, cchBldr, pchBuf + tri.ichMin,
				cchRun, qttpReduced);
			cchBldr += cchRun;
		}
	}
...
void VwPattern::Compile()
{
	...
	if (m_pmatcher || m_pcoll)
	{
		IIcuCleanupManagerPtr qicln;
		qicln.CreateInstance(CLSID_IcuCleanupManager);
		m_qCleanupCallback.Attach(new VwPatternIcuCleanupCallback(this));
		CheckHr(qicln->RegisterCleanupCallback(m_qCleanupCallback));
	}
	m_fCompiled = true;
}
...
template Vector<VwSelLevInfo>; // VecSelLevInfo; // Hungarian vvsli
...
/*----------------------------------------------------------------------------------------------
	Cleanup the callback object and the associated regular expression matcher, if any.
----------------------------------------------------------------------------------------------*/
void VwPattern::CleanupRegexPattern()
{
	if (m_qCleanupCallback)
	{
		Assert(m_pmatcher != NULL || m_pcoll != NULL);
		if (m_pmatcher != NULL)
		{
			delete m_pmatcher;
			m_pmatcher = NULL;
		...
		IIcuCleanupManagerPtr qicln;
		qicln.CreateInstance(CLSID_IcuCleanupManager);
		CheckHr(qicln->UnregisterCleanupCallback(m_qCleanupCallback));
		// Typically the cleanup callback object gets deleted the moment we clear our
		// pointer to it. But in case something goes wrong, it is now defunct, and we don't
		// want any more notifications from it.
		m_qCleanupCallback->m_ppat = NULL;
		m_qCleanupCallback.Clear();
...
/*----------------------------------------------------------------------------------------------
	Constructor.
----------------------------------------------------------------------------------------------*/
VwPatternIcuCleanupCallback::VwPatternIcuCleanupCallback(VwPattern * ppat)
{
	m_cref = 1;
	ModuleEntry::ModuleAddRef();
	m_ppat = ppat;
...
/*----------------------------------------------------------------------------------------------
	Destructor.
----------------------------------------------------------------------------------------------*/
VwPatternIcuCleanupCallback::~VwPatternIcuCleanupCallback()
{
	ModuleEntry::ModuleRelease();
}
//:>--------------------------------------------------------------------------------------------
//:>	IUnknown Methods
//:>--------------------------------------------------------------------------------------------
STDMETHODIMP VwPatternIcuCleanupCallback::QueryInterface(REFIID riid, void **ppv)
{
	AssertPtr(ppv);
	if (!ppv)
		return WarnHr(E_POINTER);
...
STDMETHODIMP VwPatternIcuCleanupCallback::DoneCleanup()
{
	if (m_ppat)
		m_ppat->CleanupRegexPattern();
	return S_OK;
}
