...
public:
	...
	STDMETHOD(OnTyping)(IVwGraphics *pvg, BSTR bstrInput,
	...
	STDMETHOD(Print)(IVwPrintContext * pvpc, IAdvInd3 * padvi3);
	...
	STDMETHOD(GetTotalPrintPages)(IVwPrintContext * pvpc, IAdvInd3 * padvi3,
		int *pcPageTotal);
	STDMETHOD(PrintSinglePage)(IVwPrintContext * pvpc, int nPageNo);

	// Misc
	STDMETHOD(Close)();
	STDMETHOD(AddSelChngListener)(IEventListener * pel);
	STDMETHOD(DelSelChngListener)(IEventListener * pel);
	STDMETHOD(Reconstruct)();
	STDMETHOD(OnStylesheetChange)();
	STDMETHOD(DrawingErrors)();
	...
	STDMETHOD(RequestObjCharDeleteNotification)(IVwNotifyObjCharDeletion *pnocd);
	...
	STDMETHOD(SetKeyboardForWs)(IWritingSystem * pws, BSTR * pbstrActiveKeymanKbd,
	...
	IVwNotifyObjCharDeletion * GetNotifyObjCharDeletion()
	{
		return m_qnocd;
	}
	...
	void NotifySelChange(int nHow, bool fUpdateRootSite = true);
	...
	void GetDeleteRangeNotifications(ITsStrBldr * ptsb, int ichMin, int ichLim,
		Vector<GUID> & vpguidDeletedObjs);
	void DoNotificationsForDeletePara(int hvoPara);
...
protected:
	...
	typedef ComVector<IEventListener> VecListener;
	VecListener m_velSelChange;
	bool m_fDirty;
	...
	IVwNotifyObjCharDeletionPtr m_qnocd; // thing to notify of obj char deletions.
	...
	// After creating with CreaeteCom, must set everything up from Init
	VwRootBox();
...
public:
	void FixSelectionsForStringReplacement(VwTxtSrc * psrcModify, int itssMin, int itssLim,
			VwTxtSrc * psrcRep);
