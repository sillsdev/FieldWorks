...
#define kclidSection 2103; // could use the real one, but minimize the includes...
	...
	public:
		...
		void testInsertDiffParasEmptySecondPara()
		{
			...
			MakeStyleProps(L"Heading", &qttpHeading);
			...
			VerifyParaContents(0, L"end para1");
		}
		...
		void testInsertDiffParas()
		{
			...
			ITsTextPropsPtr qttpHeading;
			MakeStyleProps(L"Normal", &qttpNormal);
			MakeStyleProps(L"Heading", &qttpHeading);
			...
			VerifyParaContents(0, L"Insert startThis is the first test paragraph");
			...
			MakeStyleProps(L"", &qttpEng, g_wsEng);
			...
			VerifyParaContents(0, L"Insert startHeading Para");
			VerifyParaContents(1, L"This is the first test paragraph");
			...
			VerifyStringProps(0, L"Heading Para", qttpHeading);
			int len;
			...
			m_qidrs->m_case = 1;
			qselTemp->ReplaceWithTsString(qtssTwoParas);
			unitpp::assert_eq("expected two strings", 2, m_qidrs->m_strings.Size());
			VerifyStringProps(0, L"Second inserted para", qttpNormal);
			VerifyStringProps(1, L"Heading Para", qttpHeading);
			SmartBstr sbstr;
