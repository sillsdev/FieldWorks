	...
	OLECHAR * prgchName, int cchMaxName, int * pcchName)
{
	...
	else
	{
		...
		*pclrUnder = tds.m_clrUnder;
		*punt = tds.m_unt;
		int cchCopy = min(cchMaxAbbr, tds.m_stuAbbr.Length());
		MoveItems(tds.m_stuAbbr.Chars(), prgchAbbr, cchCopy);
		*pcchAbbr = cchCopy;
		cchCopy = min(cchMaxName, tds.m_stuName.Length());
...
bool TagDispSpec::operator < (const TagDispSpec & arg)
{
	...
	int nResult;
	CheckHr(s_qcoleng->Compare(stuMyKey.Bstr(), stuItsKey.Bstr(), fcoDefault, &nResult));
	return nResult < 0;
}
...
STDMETHODIMP VwOverlay::Sort(ComBool fByAbbr)
{
	...
	s_fByAbbr = fByAbbr;
	s_qcoleng.CreateInstance(CLSID_LgUnicodeCollater);
...
template Vector<TagDispSpec>; // VecTagDispSpec; // vtds

// Map from a 26-char representation of a Guid to an index.
template HashMap<TagSpecKey, int>; // MapGuidInt; // hmgi
