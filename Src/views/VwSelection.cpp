...
----------------------------------------------------------------------------------------------*/
bool VwTextSelection::AdjustForRep(int & ichLog, VwParagraphBox * pvpbox, int itssMin,
	int itssLim, VpsTssVec & vpst)
{
	...
	for (int itss = itssMin; itss < itssLimLoop; itss++)
	{
		...
		if (ichLog < ichMin + cchOldTotal + cchOld)
		{
			...
			// of previous strings.
			bool didChange = cchNewTotal != cchOldTotal;
	...
	VwTxtSrc * psrcRep)
{
	...
	if (m_pvpbox->Source() == psrcModify)
		fDidChange |= AdjustForRep(m_ichAnchor, m_pvpbox, itssMin, itssLim, psrcRep->Vpst());
	VwParagraphBox * pvpboxEnd = (m_pvpboxEnd ? m_pvpboxEnd : m_pvpbox);
	if (pvpboxEnd->Source() == psrcModify)
		fDidChange |= AdjustForRep(m_ichEnd, pvpboxEnd, itssMin, itssLim, psrcRep->Vpst());
	return fDidChange;
}
