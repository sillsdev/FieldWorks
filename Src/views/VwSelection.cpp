...
void GetCpeFromRootAndProps(VwRootBox * prootb, ITsTextProps * pttp, ILgCharacterPropertyEngine ** ppcpe)
{
	...
	{
		CheckHr(qwsf->get_UnicodeCharProps(ppcpe));
	}
}
/*----------------------------------------------------------------------------------------------
...
void VwSelection::DoPageUpDown(IVwGraphics * pvg, bool fIsPageUp, bool fIsExtendedSelection)
{
	...
	LONG lPageHeight = VisiblePageHeight(pvg, rcSrc, rcDest);
										 ...
										 bool fIsExtendedSelection)
{
	...
	if (!fRange)
	{
		...
		newX = rcSelection.left + rcSelection.Width() / 2;
	}
	else
	{
		...
		if (fPutSelAtEdge)
			newX = (lYAdjustment > 0 ? rcSelection.left : rcSelection.right);
		else
			newX = rcSelBottom.left;
	}

	int newY = originalY + lYAdjustment;
	if(newY <= rcDest.top || newY > rcDest.top + rootbHeight)
	{
		newY = max(newY, rcDest.top);
		newY = min(newY, rcDest.top + rootbHeight);
		newX = (fEndBeforeAnchor? rcSelection.left : rcSelection.right);
	}

	// OK, we need to move by dy (newY - originalY). But, we may have to expand a
	...
	if(fIsExtendedSelection)
		CheckHr(pRootBox->MouseDownExtended(newX, newY, rcSrc, rcDest));
	else
		CheckHr(pRootBox->MouseDown(newX, newY, rcSrc, rcDest));
...
/*----------------------------------------------------------------------------------------------
	...
	@param rcClientCoord
----------------------------------------------------------------------------------------------*/
LONG VwSelection::VisiblePageHeight(IVwGraphics * pvg, Rect rcDocumentCoord, Rect rcClientCoord)
{
	AssertPtr(pvg);
...
/*----------------------------------------------------------------------------------------------
	...
	@param pfOk
----------------------------------------------------------------------------------------------*/
STDMETHODIMP VwSelection::Commit(ComBool * pfOk)
{
	BEGIN_COM_METHOD;
	ChkComOutPtr(pfOk);
	*pfOk = true;
...
/*----------------------------------------------------------------------------------------------
	If the selection is an IP, arrange that subsequent typing in this location
	(as long as it occurs before the selection is moved) will create text with
	the specified properties.
	The default (non-text) selection does nothing.
----------------------------------------------------------------------------------------------*/
STDMETHODIMP VwSelection::SetIpTypingProps(ITsTextProps * pttp)
{
	return S_OK;
}

STDMETHODIMP VwTextSelection::SetIpTypingProps(ITsTextProps * pttp)
{
	BEGIN_COM_METHOD;
	ChkComArgPtr(pttp);
	if (m_ichAnchor == m_ichEnd && !m_pvpboxEnd) // insertion point.
	{
		m_qttp = pttp;
		m_pvpbox->Root()->NotifySelChange(1);
	}
	END_COM_METHOD(g_fact, IID_IVwSelection);
}
...
//		StartEditing(); // OPTIMIZE JohnT: should we do this? May produce unnecessary commit.
		m_qttp = *prgpttp;
		int ws = -1;
		ModifyEmptyString(&ws);
		// This was taken out because ModifyEmptyString() calls NotifySelChange().
		//m_pvpbox->Root()->NotifySelChange(1);
		return S_OK;
	}
	...
	*pcpropPrevious = pnoteInner->PropCount(tagEdit, pvpbox, itssProp);
	...
	if (ppttpIns)
	{
		if (IsInsertionPoint())
		{
			...
			int c;
			GetSelectionProps(1, ppttpIns, &qvps, &c);
		} else
		{
			int cttp;
			GetSelectionProps(0, NULL, NULL, &cttp);
			if (cttp == 0)
	...
	int xd, int yd, Rect rcSrcRoot, Rect rcDstRoot, Rect rcSrc, Rect rcDst)
{
	...
	if (psboxClick->Container() != m_pvpbox)
	{
		...
		// multi-para selection. The easiest fix is to commit, which will ensure a new
		// start-editing call if we start editing on the multi-para selection.
		if (m_qtsbProp)
		{
			ComBool fOk;
			Commit(&fOk);
	...
	int nHowChanged = m_pvpboxEnd ? 1 : 2; // change of para if used to be multiple
	Hide();
...
VwDelProbType VwTextSelection::IsProblemSelection()
{
	...
	else
	{
		...
		if (pnoteParent->Tags()[ipropPara] != kflidStText_Paragraphs)
			return kdptComplexRange;
...
void VwTextSelection::StartEditing()
{
	...
	if (m_pvpboxEnd)
	{
		// Mult-paragraph selection. Verify that there is a similarly editable string
		// in the first para.
		int ichFirst = m_ichAnchor;
		VwParagraphBox * pvpboxFirst = m_pvpbox;
		if (m_fEndBeforeAnchor)
	...
	ITsTextProps ** ppttp)
{
	...
	if (sbstrObjData.Length())
	{
		...
		if (pch[0] == kodtPictEven || pch[0] == kodtPictOdd)
		{
			ITsPropsBldrPtr qtpb;
			CheckHr(qttp->GetBldr(&qtpb));
			CheckHr(qtpb->SetStrPropValue(ktptObjData, NULL));
...
public:
		...
		int cchInput, int cchBackspace, int cchDelForward, wchar chFirst, int * pwsPending)
	{
		...
		m_cchBackspace = cchBackspace;
		m_cchDelForward = cchDelForward;
		if (cchInput && chFirst == pchInput[0])
		{
			Assert(m_cchInput);
			m_chFirst = m_pchInput[0];
		}
		else
		{
			m_chFirst = chFirst;
	...
	bool CallOnProblemDeleting(VwDelProbType dpt, int & cbspOrDel)
	{
		...
		try{
			...
			CheckHr(m_prootb->Site()->OnProblemDeletion(m_qsel, dpt, &dpr));
			if (fWasWorking)
				m_qsel->m_csCommitState = kcsWorking;
		}
		catch(Throwable& thr) {
			if (thr.Result() == E_NOTIMPL || thr.Result() == E_FAIL)
			{
		...
		if (m_cchBackspace || m_cchDelForward || m_cchInput)
		{
			...
			m_prootb->Selection()->OnTyping(m_pvg, m_pchInput, m_cchInput,
				m_cchBackspace, m_cchDelForward, m_chFirst, m_pwsPending);
		}
	...
	{
		if (m_cchBackspace != -1 && m_cchDelForward != -1)
...
// This behavior (deleting a complete base + diacritic combination) is not wanted.
			...
			if (ichMin < ichLim)
			{
				...
				m_prootb->GetDeleteRangeNotifications(m_qsel->m_qtsbProp, ichMin, ichLim,
					m_vpguidDeletedObjs);
			...
			if (m_cchBackspace)
			{
				if (m_qsel->m_ichMinEditProp)
				{
					...
					// Now force the selection to associate with the previous prop
				...
				else
				{
					...
					ITsStringPtr qtssT;
					m_qsel->m_qtsbProp->GetString(&qtssT);
					if (cchPrev)
					{
						// Delete this paragraph (want to keep props of previous one)
						HVO hvoDel;
						CheckHr(m_qsda->get_VecItem(m_qsel->m_hvoParaOwner,
							m_qsel->m_tagParaProp, m_qsel->m_ihvoFirstPara, &hvoDel));
						m_qsel->AboutToDeletePara(hvoDel, m_qsel->m_hvoParaOwner,
							m_qsel->m_tagParaProp, m_qsel->m_ihvoFirstPara, false);
						CheckHr(m_qsda->DeleteObjOwner(m_qsel->m_hvoParaOwner, hvoDel,
							m_qsel->m_tagParaProp, m_qsel->m_ihvoFirstPara));
						m_qsda->PropChanged(m_qsel->m_pvpbox->Root(), kpctNotifyMeThenAll,
							m_qsel->m_hvoParaOwner, m_qsel->m_tagParaProp,
							m_qsel->m_ihvoFirstPara, 0, 1);
					}
					else
					{
						...
						m_qsel->AboutToDeletePara(hvoDel, m_qsel->m_hvoParaOwner,
							m_qsel->m_tagParaProp, m_qsel->m_ihvoFirstPara - 1, true);
						CheckHr(m_qsda->DeleteObjOwner(m_qsel->m_hvoParaOwner, hvoDel,
							m_qsel->m_tagParaProp, m_qsel->m_ihvoFirstPara - 1));
						m_qsda->PropChanged(m_qsel->m_pvpbox->Root(), kpctNotifyMeThenAll,
							m_qsel->m_hvoParaOwner, m_qsel->m_tagParaProp,
							m_qsel->m_ihvoFirstPara - 1, 0, 1);
					}
					...
					// On top of that, we deleted the object which was our hvoEdit
					m_qsel->m_ihvoFirstPara--;
					CheckHr(m_qsda->get_VecItem(m_qsel->m_hvoParaOwner,
						m_qsel->m_tagParaProp, m_qsel->m_ihvoFirstPara,
						&m_qsel->m_hvoEdit));
					m_qsel->FixStringVars(cchPrev - m_qsel->m_ichMinEditProp);
					// ENHANCE JohnT (later version, if we implement views that need it):
					// detect case where object is more than one para, deal with.
					if (cchPrev)
						m_ichAnchor = m_qsel->m_pvpbox->Source()->Cch();
					else
						// we're at start of merged paragraph, followed by text of old
						m_ichAnchor = 0;
					m_ichEnd = m_ichAnchor;
					// make all vars consistent for editing at end of previous para
					m_qsel->m_qtsbProp.Clear();
					// Must have these right to call StartEditing
					m_qsel->m_ichEnd = m_ichAnchor;
					m_qsel->m_ichAnchor = m_ichAnchor;
					m_qsel->m_ichAnchor2 = -1;
					m_qsel->StartEditing();
					if (cchPrev)
					{
						// Now insert the body of the deleted previous paragraph into the
						// retained one
						int ichIns = cchPrev - m_qsel->m_ichMinEditProp;
						CheckHr(m_qsel->m_qtsbProp->ReplaceTsString(ichIns, ichIns, qtssT));
						// And the editable property extends to there...
						CheckHr(m_qsel->m_qtsbProp->get_Length(&cchProp));
					}
					m_cchBackspace--; // and loop to see if any more...
				}
			}
		}
		return kTypingContinue;
	...
	TypingAction HandleDelForward(ITsTextPropsPtr & qttp)
	{
		...
		while (m_cchDelForward)
		{
			...
			if (m_cchDelForward)
			{
				// We can't delete in current prop, is there another in para?
				...
				else
				{
					...
					if (cch)
					{
						...
						CheckHr(m_qsda->get_VecItem(m_qsel->m_hvoParaOwner,
							m_qsel->m_tagParaProp, m_qsel->m_ihvoFirstPara + 1, &hvoDel));
						m_qsel->AboutToDeletePara(hvoDel, m_qsel->m_hvoParaOwner,
							m_qsel->m_tagParaProp, m_qsel->m_ihvoFirstPara + 1, false);
						CheckHr(m_qsda->DeleteObjOwner(m_qsel->m_hvoParaOwner, hvoDel,
							m_qsel->m_tagParaProp, m_qsel->m_ihvoFirstPara + 1));
						m_qsda->PropChanged(m_qsel->m_pvpbox->Root(), kpctNotifyMeThenAll,
							m_qsel->m_hvoParaOwner, m_qsel->m_tagParaProp,
					...
					else
					{
						...
						CheckHr(m_qsda->get_VecItem(m_qsel->m_hvoParaOwner,
							m_qsel->m_tagParaProp, m_qsel->m_ihvoFirstPara, &hvoDel));
						m_qsel->AboutToDeletePara(hvoDel, m_qsel->m_hvoParaOwner,
							m_qsel->m_tagParaProp, m_qsel->m_ihvoFirstPara, true);
						CheckHr(m_qsda->DeleteObjOwner(m_qsel->m_hvoParaOwner, hvoDel,
							m_qsel->m_tagParaProp, m_qsel->m_ihvoFirstPara));
						m_qsda->PropChanged(m_qsel->m_pvpbox->Root(), kpctNotifyMeThenAll,
							m_qsel->m_hvoParaOwner, m_qsel->m_tagParaProp,
					...
					// This time we are keeping the same current paragraph and para object.
					m_qsel->FixStringVars(m_qsel->m_ichMinEditProp);
					if (cch)
					{
						// Insert the body of the deleted paragraph into the retained one
						int cchThis;
						CheckHr(m_qsel->m_qtsbProp->get_Length(&cchThis));
						CheckHr(m_qsel->m_qtsbProp->ReplaceTsString(cchThis, cchThis,
							qtssT));
					}
					else
					{
						// Load builder with contents of following (now current) paragraph
						ITsStringPtr qtssThis;
						m_qsel->m_pvpbox->Source()->StringAtIndex(0, &qtssThis);
						CheckHr(qtssThis->GetBldr(&m_qsel->m_qtsbProp));
					}
					// And the editable property extends to there...
					CheckHr(m_qsel->m_qtsbProp->get_Length(&cchProp));
					m_cchDelForward--; // and loop to see if any more...
				}
			}
	...
	TypingAction HandleOtherInput(ITsTextPropsPtr & qttp, int cchPropOrig)
	{
		...
		while (m_cchInput)
		{
			...
			if (cchIns && m_qsel->m_pnoteParaOwner)
			{
				// Insert this number of returns, i.e., new paragraphs
				m_nHowChanged = ksctDiffPara;
				...
				if (m_ichAnchor - m_qsel->m_ichMinEditProp < cchProp)
				{
					...
					if (m_ichAnchor == 0)
					{
						...
						if (tag == kflidStText_Paragraphs)
						{
							...
							IUnknownPtr qunkTtp;
							CheckHr(qsda->get_UnknownProp(hvoBase, kflidStPara_StyleRules, &qunkTtp));
							if (qunkTtp)
						...
						{
							ITsStringPtr qtssT;
							CheckHr(m_qsel->m_qtsbProp->GetString(&qtssT));
							ITsStrBldrPtr qtsbT;
							CheckHr(qtssT->GetBldr(&qtsbT));
							CheckHr(qtsbT->ReplaceTsString(0, cchProp, NULL));
							CheckHr(qtsbT->GetString(&qtssT));
							HVO hvoNew;
							CheckHr(qsda->MakeNewObject(kclidStTxtPara, hvoObj, tag,
								ihvo + i2, &hvoNew));
							if (tag == kflidStText_Paragraphs)
								CheckHr(qsda->SetUnknown(hvoNew, kflidStPara_StyleRules, qttp));
							CheckHr(m_qsda->SetString(hvoNew, m_qsel->m_tagEdit, qtssT));
						}
						ihvoIns--;
					}
					else
					{
						// Chop off the tail end of the current paragraph.
						// Don't alter m_qtsbProp as we will use it to produce the
						// tail end of the string for the new paragraph.
						ITsStringPtr qtssT;
						CheckHr(m_qsel->m_qtsbProp->GetString(&qtssT));
						ITsStrBldrPtr qtsbT;
						CheckHr(qtssT->GetBldr(&qtsbT));
						int ichStartDel = m_ichAnchor - m_qsel->m_ichMinEditProp;
						// nb we do NOT call m_prootb->DoNotificationsForDeleteRange here;
						// the deleted text will be re-inserted into another paragraph.
						CheckHr(qtsbT->ReplaceTsString(ichStartDel, cchProp, NULL));
						CheckHr(qtsbT->GetString(&qtssT));
						CheckHr(m_qsda->SetString(m_qsel->m_hvoEdit, m_qsel->m_tagEdit, qtssT));
						CheckHr(m_qsda->PropChanged(m_qsel->m_pvpbox->Root(), kpctNotifyMeThenAll,
							m_qsel->m_hvoEdit, m_qsel->m_tagEdit, ichStartDel, 0,
							cchPropOrig - ichStartDel));
				...
				else
				{
					...
					ComBool f;
					m_qsel->CommitAndContinue(&f);
					CheckHr(m_qsda->InsertNew(m_qsel->m_hvoParaOwner, m_qsel->m_tagParaProp,
						m_qsel->m_ihvoFirstPara, cchIns,
						m_prootb->Stylesheet()));
				}

				// It indicates that at a position AFTER m_ihvoFirstPara we are inserting
				// cchIns objects.
				m_qsda->PropChanged(m_prootb, kpctNotifyMeThenAll,
					m_qsel->m_hvoParaOwner, m_qsel->m_tagParaProp,
					ihvoIns, cchIns, 0);
				m_qsel->m_ihvoFirstPara += cchIns;
				// Get the object ID of the (last) new para object.
				CheckHr(m_qsda->get_VecItem(m_qsel->m_hvoParaOwner, m_qsel->m_tagParaProp,
					m_qsel->m_ihvoFirstPara, &m_qsel->m_hvoEdit));

				m_qsel->FixStringVars(0);
				// The string builder should now only contain what is after the
				// old IP
				// nb do Not notify deleted text...it has been re-inserted in another para.
				// NOTE: We must use ReplaceRgch() here instead of ReplaceTsString() in
				// order to get the correct properties assigned to the insertion point!
				CheckHr(m_qsel->m_qtsbProp->ReplaceRgch(0,
					m_ichAnchor - m_qsel->m_ichMinEditProp, NULL, 0, qttp));
				m_ichAnchor = m_ichEnd = 0;
				m_qsel->m_itssProp = 0; // selection is in first string in new para.
				m_qsel->m_ichMinEditProp = 0; // selection prop now certainly starts at 0.
			...
			m_pchInput += cchIns;
			CheckHr(m_qsel->m_qtsbProp->get_Length(&cchProp));
			m_qsel->m_ichLimEditProp = m_qsel->m_ichMinEditProp + cchProp;
	...
	{
		if (m_qsel->m_pvpbox->IsSelectionTruncated(m_qsel->m_ichAnchor))
		{
			ComBool fOk;
			m_qsel->UnprotectedCommit(&fOk);
		...
		else
		{
			...
			m_prootb->NotifySelChange(m_nHowChanged);
		...
		for(int i = 0; i < m_vpguidDeletedObjs.Size(); i++)
		{
			if (m_prootb->GetNotifyObjCharDeletion())
				CheckHr(m_prootb->GetNotifyObjCharDeletion()->ObjDeleted(&m_vpguidDeletedObjs[i]));
		}
	}
...
// ENHANCE JohnT: It may be helpful to reinstate a variant of this code if there are cases
// where typing can occur and no outer code has created an undo item.
//		SmartBstr sbstrUndo;
//		SmartBstr sbstrRedo;
//		// Note: if we implement this, get these strings from a resource.
//		if (m_cchDelForward > 0 && m_cchInput == 0 && m_cchBackspace == 0)
//		{
//			sbstrUndo = L"Undo Delete";
//			sbstrRedo = L"Redo Delete";
//		}
//		else
//		{
//			sbstrUndo = L"Undo Typing";
//			sbstrRedo = L"Redo Typing";
//		}
//		CheckHr(m_qsda->BeginUndoTask(sbstrUndo, sbstrRedo));

		m_qsel->BeginUndoTask(m_qsda.Ptr(), kstidUndoTyping);
		m_fUndoTaskBegun = true;
		...
		if (!m_qsel->DeleteRangeAndPrepareToInsert(true, m_vpguidDeletedObjs))
			return;
		...
		{
			m_prootb->Selection()->OnTyping(m_pvg, m_pchInput, m_cchInput, m_cchBackspace,
				m_cchDelForward, m_chFirst, m_pwsPending);
			return;
		}
		...
		m_qsel->ClearLinksFromEmptyString();
...
----------------------------------------------------------------------------------------------*/
void VwTextSelection::OnTyping(IVwGraphics * pvg, const wchar * pchInput, int cchInput,
	int cchBackspace, int cchDelForward, wchar chFirst, int * pwsPending)
{
	StartProtectMethod();
	try
	{
		...
		LockThis();
		OnTypingMethod otm(this, pvg, pchInput, cchInput, cchBackspace, cchDelForward,
			chFirst, pwsPending);
		otm.Run();
	}
	catch(Throwable& thr)
...
/*----------------------------------------------------------------------------------------------
	...
	@param pfOk set to false if the edited text needs parsing and that fails.
----------------------------------------------------------------------------------------------*/
STDMETHODIMP VwTextSelection::Commit(ComBool * pfOk)
{
	BEGIN_COM_METHOD;
	ChkComOutPtr(pfOk);
	Assert(m_qrootb);
	...
	// Only set true if we make it all the way (though failures throw exceptions)
	// It is just possible in complex cases that updating the property will cause
	// the selection to be discarded. Keep a ref count so *this won't go away.
	END_COM_METHOD(g_fact, IID_IVwSelection);
}
...
----------------------------------------------------------------------------------------------*/
void VwTextSelection::UnprotectedCommit(ComBool * pfOk)
{
...
/*----------------------------------------------------------------------------------------------
	Modify the empty string to contain the properties in the insertion properties m_qttp.
----------------------------------------------------------------------------------------------*/
void VwTextSelection::ModifyEmptyString(int * pwsPending)
{
	VwRootBox * prootb = m_pvpbox->Root();
	Assert(prootb);

	int cch;
	CheckHr(m_pvpbox->Source()->get_Length(&cch));
	if (cch > 0)
		return;  // not an empty string

	if (!m_qttp)
		return;  // no properties to apply
	...
	try
	{
		...
		// default: only position changed; set to 2 if para changes
		int nHowChanged = ksctSamePara;
		BeginUndoTask(qsda.Ptr(), kstidUndoFormatting);

		StartEditing(); // set up notifier, builder
		if (!m_qtsbProp)
			goto LDone; // nothing editable

		int cchProp;
		CheckHr(m_qtsbProp->get_Length(&cchProp)); // make sure it is current one more time
		Assert(cchProp == 0);
		...
		// where and also what to do if the user says "No" and the method returns.
		HRESULT hrDbCache = E_FAIL;
		ISilDataAccess * psdaTemp = m_pvpbox->Root()->GetDataAccess();
		hrDbCache = psdaTemp->QueryInterface(IID_IVwOleDbDa, (void **) &qodde);
		if ((hrDbCache == S_OK) && m_hvoParaOwner)
		{
			HRESULT hrTemp = E_FAIL;
			if ((hrTemp = qodde->CheckTimeStamp(m_hvoParaOwner)) != S_OK)
			{
		...
		// Store the new properties in the string builder.
		CheckHr(m_qtsbProp->SetProperties(0, 0, m_qttp));

		if (*pwsPending != -1)
		{
			int ws = (*pwsPending == -1) ? 0 : *pwsPending;
			CheckHr(m_qtsbProp->SetIntPropValues(0, 0, ktptWs, ktpvDefault, ws));
		}

		// At this point the string builder has been updated with the new properties.
		// Now we have to update the display.
		// Get the new string that results from the changes.
		CheckHr(m_qtsbProp->GetString(&qtssNew));

		m_pvpbox->Source()->StyleAtIndex(m_itssProp, &qzvps);

		pvpboxT = MakeDummyPara(qtssNew, qzvps);
		{
		...
		// update the member variables
		m_ichLimEditProp = m_ichMinEditProp;
		m_ichAnchor = m_ichEnd = 0;
		m_ichAnchor2 = -1;
//		m_qttp = qttp;
		...
		HRESULT hr;
		IgnoreHr(hr = prootb->get_Site(&qvrs));
		if (SUCCEEDED(hr))
		{
			// Ignore anything that goes wrong with either of these calls--they are
			// just an optimization, and very unlikely to fail anyway.
			qvrs->DoUpdates(prootb);
		}
LDone:
	...
	{
		// Turn the selection back on no matter what.
...
/*----------------------------------------------------------------------------------------------
	...
	@param pfOk
----------------------------------------------------------------------------------------------*/
void VwTextSelection::CommitAndContinue(ComBool * pfOk, VwChangeInfo * pci)
{
...
/*----------------------------------------------------------------------------------------------
	Cleanup code for following method, needs to be executed in catch block as well as normally.
----------------------------------------------------------------------------------------------*/
void CleanupRWTS(VwTextSelection * pThis, VwRootBox * prootb, VwParagraphBox * pvpbox)
{
	if (pThis == prootb->Selection())
	{
		// We are still the active selection, display
		VwRootBox * prootb = pvpbox->Root();
		Assert(prootb);
		prootb->HandleActivate(vssEnabled);
		pThis->Show();
	}
...
STDMETHODIMP VwTextSelection::ReplaceWithTsString(ITsString * ptss)
{
	...
	try
	{
		...
		Vector<GUID> vpguidDeletedObjs;
		bool fSuccess = DeleteRangeAndPrepareToInsert(true, vpguidDeletedObjs);
		if (fSuccess)
		{
			for (int i = 0; i < vpguidDeletedObjs.Size(); i++)
			{
				if (prootb->GetNotifyObjCharDeletion())
					IgnoreHr(prootb->GetNotifyObjCharDeletion()->ObjDeleted(&vpguidDeletedObjs[i]));
			}
		}
		else
		{
		...
		else if (m_qvvcEdit)
		{
			...
			// TODO ASAP: Something needs to be done to initialize underlying data structures.  Setting
			...
			if (pnotePara)
			{
				...
				if (pnotePara->Tags()[ipropPara] == kflidStText_Paragraphs)
		...
		// Enhance JohnT: possibly we should NOT remove newlines now that our code can handle
		// embedded newlines? Should we permit pasting multiple paragraphs from other applications?
		if (cTabs || cNewlines)
		{
			StrUni stuSpace(L" ");
			ITsStrBldrPtr qtsb;
			...
			for (ich = cch; ich-- > 0; )
			{
				...
				else if (cNewlines && CheckForParaBreak(pchw, ich, cch, &ichNext))
				{
					...
					if (ichNext == cch)
					{
						// A trailing newline being pasted where we can't have newline can just be ignored.
						CheckHr(qtsb->Replace(ich, ichNext, NULL, NULL));
						cch -= (ichNext - ich);
					}
					else
					{
						// Other newlines get changed to spaces.
						CheckHr(qtsb->Replace(ich, ichNext, stuSpace.Bstr(), NULL));
						cch -= (ichNext - ich - stuSpace.Length());
					}
				}
			}
		...
		int cchOffset = 0; // amount by which qtsbFix is shorter than qtss.
		...
		if (vpttpPara.Size())
		{
			...
			HVO hvoOriginalPara;
			CheckHr(psda->get_VecItem(m_hvoParaOwner, kflidStText_Paragraphs,
				m_ihvoFirstPara, &hvoOriginalPara));
			ITsTextPropsPtr qttpOrig;
			IUnknownPtr qunkTtp;
			CheckHr(psda->get_UnknownProp(hvoOriginalPara, kflidStPara_StyleRules, &qunkTtp));
			if (qunkTtp)
			...
			if (fNeedInsertDiffParas)
			{
				...
				prootb->Site()->OnInsertDiffParas(prootb, qttpOrig, vpttpPara.Size(),
			...
			int ihvoFirstModPara = m_ihvoFirstPara;
			for (iPara = 0, ichMin = 0; ichMin < cch; ichMin = ichLim)
			{
				...
				// If the insertion position is at the beginning of the paragraph...
				if (ichAnchorNew == 0 && m_ichMinEditProp == 0 && m_itssProp == 0 && fNewline)
				{
					...
					CheckHr(psda->SetString(hvoNewPara, m_tagEdit, qtssT));
					// Set the paragraph styles (if any) of the paragraph object.
					Assert(iPara < vpttpPara.Size());
					ITsTextPropsPtr qttpT = vpttpPara[iPara];
					if (qttpT)
						CheckHr(psda->SetUnknown(hvoNewPara, kflidStPara_StyleRules, qttpT));
					else
						CheckHr(psda->SetUnknown(hvoNewPara, kflidStPara_StyleRules, qttpOrig));
					++iPara;
					++m_ihvoFirstPara;
					continue;
				}
				else if (ichMin < ichLim)
				{
					...
					ITsStringPtr qtssT;
					MakeSubString(qtss, ichMin, ichLim, &qtssT);
					CheckHr(m_qtsbProp->ReplaceTsString(ichAnchorNew - m_ichMinEditProp,
						ichAnchorNew - m_ichMinEditProp, qtssT));
				...
				else
				{
					...
					if (fHaveParas)			// (m_pnoteParaOwner)
					{
						...
						nHowChanged = ksctDiffPara;
						int cNewParas = 1;
						CheckHr(m_qtsbProp->get_Length(&cchProp));
						// If the insertion position is before the end of the paragraph...
						if (ichAnchorNew - m_ichMinEditProp < cchProp)
						{
							// Chop off the tail end of the current paragraph. We could just
							// update the property, not the display, since we are going to edit
							// the paragraph-level property and so regenerate it completely,
							// except that some other view might conceivably be displaying
							// the string property and not the other. So it's safest to send
							// the change. This only happens for the first new paragraph.

							// Don't alter m_qtsbProp as we will use it to produce the
							// tail end of the string for the new paragraph at the end.
							ITsStringPtr qtssT;
							ITsStrBldrPtr qtsbT;
							CheckHr(m_qtsbProp->GetString(&qtssT));
							CheckHr(qtssT->GetBldr(&qtsbT));
							int ichStartDel = ichAnchorNew - m_ichMinEditProp;
							CheckHr(qtsbT->ReplaceTsString(ichStartDel, cchProp, NULL));
							CheckHr(qtsbT->GetString(&qtssT));
							CheckHr(psda->SetString(m_hvoEdit, m_tagEdit, qtssT));
							psda->PropChanged(prootb, kpctNotifyMeThenAll,
								m_hvoEdit, m_tagEdit, ichStartDel, 0, cchProp - ichStartDel);
							// There is text after the IP, new lines take same style
							CheckHr(psda->InsertNew(m_hvoParaOwner, m_tagParaProp,
								m_ihvoFirstPara, cNewParas, NULL));
						}
						else
						{
							// This used to be "CommitAndContinue" but that makes use of the
							// m_pvpbox variable to get the root and SDA, and does other
							// unnecessary stuff for this newly created paragraph.
							ITsStringPtr qtssNewSub;
							CheckHr(m_qtsbProp->GetString(&qtssNewSub));
							CheckHr(psda->SetString(m_hvoEdit, m_tagEdit, qtssNewSub));
							CheckHr(psda->PropChanged(prootb, kpctNotifyMeThenAll,
								m_hvoEdit, m_tagEdit, 0, 0, cchProp));

							CheckHr(psda->InsertNew(m_hvoParaOwner, m_tagParaProp,
								m_ihvoFirstPara, cNewParas, NULL));
						}

						// Set the time stamp of the paragraph owner and cache the new value to
						// block other users from making changes to the record.
						IVwOleDbDaPtr qodde;
						HRESULT hrDbCache = E_FAIL;
						hrDbCache = psda->QueryInterface(IID_IVwOleDbDa, (void **) &qodde);
						...
						if (qttpT)
						{
							...
							CheckHr(psda->SetUnknown(hvoPara, kflidStPara_StyleRules, qttpT));
						}
						m_ihvoFirstPara += cNewParas;
						// Get the object ID of the new para object.
						CheckHr(psda->get_VecItem(m_hvoParaOwner, m_tagParaProp,
							m_ihvoFirstPara, &m_hvoEdit));
						// We don't want to FixStringVars because it expands lazy boxes we may
						// not need in a big paste. Fortunately the loop does not need m_qanote
						// or m_pvpbox updated until the end, nor to have *this reinstated as
						// the selection. So we call it once at the end of the loop.  Here just
						// fix a couple of other things it does.
						//FixStringVars();
						m_pvpboxEnd = NULL; // no longer a multi-para selection
						m_rcBounds.Clear();
						m_ihvoLastPara = m_ihvoFirstPara;

						// The string builder should now only contain what is after the old IP.
						m_qtsbProp->ReplaceTsString(0, ichAnchorNew - m_ichMinEditProp, NULL);
						ichAnchorNew = 0;
					}

					// Now that we've inserted a new paragraph...only the first para in a
					// structured text sometimes has another property displayed at the start.
			...
			if (iPara)
			{
				// Paragraph level changes occurred...notify interested parties.
				// We modified the paragraph that was originally first, (messing with its style)
				// and inserted iPara more, so feign that we deleted one and inserted iPara + 1.
				psda->PropChanged(prootb, kpctNotifyMeThenAll, m_hvoParaOwner,
					m_tagParaProp, ihvoFirstModPara, iPara + 1, 1);
			}

			// Update the member variables.  We know exactly what these should be; defeat any
			// smart tricks Replace tries to do.
			// First, get the new string that results from the paste operation.
			CheckHr(m_qtsbProp->GetString(&qtssNewProp));
			// Do this ONLY when editing at the paragraph level.
			FixStringVars(m_ichMinEditProp);
		}
		else
		{
...
LDone:;
	...
	{
		CleanupRWTS(this, prootb, m_pvpbox);
		throw;
	}
	CleanupRWTS(this, prootb, m_pvpbox);
	// Funny things can happen if we do this while hidden; if the notify sel change changes
	// the selection the new one can stay hidden.
	prootb->NotifySelChange(nHowChanged);
...
protected:
	void GetParaLevelProps(VwParagraphBox * pvpbox, ITsTextProps ** ppttp)
	{
		...
		VwEditPropVal vepv = CallEditableSubstring(pvpbox, ichTarget, ichTarget, true,
			&hvoEdit, &tagEdit, &ichMinEditProp, &ichLimEditProp, &qvvcEdit, &fragEdit,
			&qanote, &iprop, &vnp, &itssProp, &qtssProp);
		if (vepv == kvepvEditable && tagEdit == kflidStTxtPara_Contents)
		{
			// We probably have paragraph level props.
			IUnknownPtr qunkTtp;
			CheckHr(m_psda->get_UnknownProp(hvoEdit, kflidStPara_StyleRules, &qunkTtp));
			if (qunkTtp)
	...
	{
		CheckHr(m_qtsb->Replace(m_ichSel, m_ichSel, m_bstrNonText, NULL));
	...
	int AppendLineFromParagraph(VwParagraphBox * pvpbox, int ilineTarget)
	{
		...
		for (int itss = 0; itss < pts->CStrings(); itss++)
		{
			...
			else
			{
				...
				// necessary, but that is is safe.  It fixes the immediate crash encountered
...
STDMETHODIMP VwTextSelection::EndPoint(ComBool fEndPoint, IVwSelection ** ppsel)
{
	...
	if (!IsInsertionPoint())
	{
		...
		fAssocPrev = fEndPoint ? (!fEndBeforeAnchor) : fEndBeforeAnchor;
	}
...
STDMETHODIMP VwPictureSelection::EndPoint(ComBool fEndPoint, IVwSelection ** ppsel)
{
	...
	int ich = fEndPoint ? m_ipicEnd : m_ipicAnchor;
...
void VwTextSelection::DoUpdateProp(VwRootBox * prootb, HVO hvo, PropTag tag, VwNoteProps vnp,
	IVwViewConstructor * pvvcEdit, int fragEdit, ITsStrBldr * ptsb, ComBool * pfOk,
	VwChangeInfo * pci)
{
	*pfOk = false; // only set true if we make it all the way (though failures throw exceptions)
	bool fChanged = false; // set true if we changed something.
	...
	int ivIns = 0; // Default values to pass to PropChanged.
	int cvIns = 0;
	int cvDel = 0;
		...
		vnp != kvnpStringProp && vnp != kvnpStringAltMember && vnp != kvnpUnicodeProp)
	{
		...
		if (!fEq)
		{
			BeginUndoTask(qsda.Ptr(), kstidUndoTyping);
...
*/
		...
		case kvnpStringProp:
			{ // BLOCK
				...
				{
					// In case this ever happens when we don't already have an Undo task open,
					// "typing" is the best available guess as to what the user was doing.
					BeginUndoTask(qsda.Ptr(), kstidUndoTyping);
					// Make the new string the value of the property
					if (ptxs->IsCompositionActive())
					{
						// Change it but don't save the changes to the DB. This allows the display to update but without
						// destroying the composition by normalization.
						IVwCacheDaPtr qcda;
						CheckHr(qsda->QueryInterface(IID_IVwCacheDa, (void **) & qcda));
						CheckHr(qcda->CacheStringProp(hvo, tag, qtssNewSub));
					}
					else
					{
						// Normal case, update the property.
						CheckHr(qsda->SetString(hvo, tag, qtssNewSub));
					}
					fChanged = true;
					ivIns = ichMinDiff;
					cvIns = ichLimDiff - ichMinDiff;
					int cchOld, cchNew;
					CheckHr(qtssOld->get_Length(&cchOld));
					CheckHr(qtssNewSub->get_Length(&cchNew));
					cvDel = cchOld - cchNew + cvIns;
				}
			}
			break;
		case kvnpUnicodeProp:
			{ // BLOCK
				...
				if (wcscmp(sbstrOld.Chars(), sbstrNew.Chars()) != 0)
				{
					...
					fChanged = true;
					// Feign that we deleted the whole old value and inserted the whole new one.
					// We may optimize this one day if it gets used enough to matter.
					ivIns = 0;
					cvIns = sbstrNew.Length();
					cvDel = sbstrOld.Length();
				}
			}
			break;
		case kvnpIntProp:
		...
		case kvnpStringAltSeq:
			...
			if (fragEdit)
			{
				...
				if (!fEqual || ptxs->IsDoingRecommit())
				{
					...
					if (ptxs->IsCompositionActive())
					{
						// Change it but don't commit the changes. This allows the display to update but without
						// destroying the composition.
						IVwCacheDaPtr qcda;
						CheckHr(qsda->QueryInterface(IID_IVwCacheDa, (void **) & qcda));
						CheckHr(qcda->CacheStringAlt(hvo, tag, fragEdit, qtssNewSub));
					}
					else
					{
						// Normal case, update the property.
						...
						fChanged = true;
					ivIns = fragEdit; // specs call for passing this
				}
			} else
			{
	...
	{
		// We want to do the PropChanged only if something really changed. Otherwise we'll end up
		// reconstructing (i.e. deleting) boxes while we are in the middle of a delete. This makes
		// the selection invalid and thus causes a crash when try to access the information from
		// the selection.

		// see comment above where we set pRootBox
		// Mainly here we want to call PropChanged to update the display.
		// We have to be careful in several ways here. Typically we are not changing the length of the
		// string, and PropChanged won't change the selection position. But if we normalized,
		// the string might get longer, and we've already adjusted the position for that, so it
		// must not happen again. On the other hand, just conceivably, the string might be displayed
		// more than once in the paragraph, and we'd want to get the benefit of the correction for
		// any earlier occurrences. Keeping it fixed relative to the beginning of the property works
		// both ways.
		int dichAnchor = m_ichAnchor - m_ichMinEditProp;
		int dichEnd = m_ichEnd - m_ichMinEditProp;
		qsda->PropChanged(prootb, kpctNotifyMeThenAll, hvo, tag, ivIns, cvIns, cvDel);

		// The prop changed might have reconstructed the boxes, so we better re-get the
		// selection to make sure it's valid
		FixStringVars(m_ichMinEditProp);

		if (m_qtsbProp) // But we can only make this restriction if we're editing, else no min and lim.
		{
			m_ichAnchor = min(m_ichMinEditProp + dichAnchor, m_ichLimEditProp);
			// After calling FixStringVars m_pvpboxEnd is always NULL
			//if (!m_pvpboxEnd) // just possibly, might the end be in another property??
			m_ichEnd = min(m_ichMinEditProp + dichEnd, m_ichLimEditProp);
		}
	}
	if (fChanged)
		qsda->EndUndoTask();
...
void VwTextSelection::LoseFocus(IVwSelection * pvwselNew, ComBool * pfOk)
{
	...
	*pfOk = true;
	...
	// If no pending edit, fine to lose focus
	if (!m_qtsbProp)
		return;
	...
	// same thing as calling StartEdit();
...
/*----------------------------------------------------------------------------------------------
	...
	insert. If successful, all member variables are set up for editing.
	...
	If fSaveFirstCharProps is true and a range is deleted, save the properties that should be
	applied to typed text in m_qttp (where they will be applied to the text typed).

	@param ppttp
----------------------------------------------------------------------------------------------*/
bool VwTextSelection::DeleteRangeAndPrepareToInsert(bool fSaveFirstCharProps,
	Vector<GUID> & vpguidDeletedObjs)
{
	VwRootBox * prootb = m_pvpbox->Root();
	...
	if (fSaveFirstCharProps && !IsInsertionPoint())
	{
	...
	if (dpt != kdptNone)
	{
		...
		try{
			...
			CheckHr(prootb->Site()->OnProblemDeletion(this, dpt, &dpr));
			if (fWasWorking)
				m_csCommitState = kcsWorking;
		}
		catch(Throwable& thr){
	...
	if (m_pvpboxEnd)
	{
		...
		// If ichLim == 0, all of the paragraph survives. Keep the current qtssEnd.
		if (ichLim > 0)
		{
			// Extract the substring from the end of the last para.
			ITsStrBldrPtr qtsbEnd;
			CheckHr(qtssEnd->GetBldr(&qtsbEnd));
			prootb->GetDeleteRangeNotifications(qtsbEnd, 0, ichLim, vpguidDeletedObjs);
			CheckHr(qtsbEnd->ReplaceTsString(0,ichLim, NULL));
			CheckHr(qtsbEnd->GetString(&qtssEnd));
		}
		...
		// There is (usually) one paragraph object that is deleted, but its contents are
		// not entirely deleted. By default this is the last paragraph, the tail end of
		// which is copied to the surviving first paragraph.
		int ihvoNotAllDeleted = ihvoLastDel;

		if (ichMin == 0 && ichMinLastStart == 0)
		{
			// Keep the last paragraph, delete the first one entirely.
			// There is nothing to keep from it.
			ihvoFirstDel--;
			ihvoLastDel--;
			// Leave ihvoNotAllDeleted, it's now out of range; all these paras will
			// be deleted.
			// Whatever survived from the last para becomes the whole contents of
			// the surviving first one.
			CheckHr(qtssEnd->GetBldr(&m_qtsbProp));
		}
		else
		{
			...
			// StartEditing has already made a builder from the whole first-paragraph string.
			// Replace the selected part of it with the unselected part of the last para.
			prootb->GetDeleteRangeNotifications(m_qtsbProp, ichMin, cchStart,
				vpguidDeletedObjs);
			CheckHr(m_qtsbProp->ReplaceTsString(ichMin, cchStart, qtssEnd));
		...
		for (int ihvo = ihvoLastDel; ihvoFirstDel <= ihvo; ihvo--)
		{
			...
			CheckHr(qsda->get_VecItem(m_hvoParaOwner, m_tagParaProp,
				ihvo, &hvoDel));
			if (ihvo != ihvoNotAllDeleted)
				prootb->DoNotificationsForDeletePara(hvoDel);
			AboutToDeletePara(hvoDel, m_hvoParaOwner, m_tagParaProp, ihvo, false);
			CheckHr(qsda->DeleteObjOwner(m_hvoParaOwner, hvoDel, m_tagParaProp, ihvo));
		}

		qsda->PropChanged(m_pvpbox->Root(), kpctNotifyMeThenAll, m_hvoParaOwner,
			m_tagParaProp, ihvoFirstDel, 0, ihvoLastDel - ihvoFirstDel + 1);
		// That really messed us up!  The whole property which contained both our paragraphs
		// was replaced, which means both our paragraphs no longer exist.  Moreover we may no
		// longer be the current selection.  On top of that, we deleted the object which was
		// our hvoEdit
		CheckHr(qsda->get_VecItem(m_hvoParaOwner, m_tagParaProp, m_ihvoFirstPara,
			&m_hvoEdit));

		// The IP is now after the ichMin characters from the original first para.
		// This also works (setting the both to zero) when we deleted the whole of the
		// first paragraph.
		m_ichAnchor = m_ichEnd = ichLim = ichMin + ichMinLastStart;
		...
		{
			// And the editing property is now the last one of the first paragraph.
			m_itssProp = itssLastStart;
			m_ichMinEditProp = ichMinLastStart;
		}
		FixStringVars(m_ichMinEditProp);
		int cchProp;
		m_qtsbProp->get_Length(&cchProp);
		m_ichLimEditProp = m_ichMinEditProp + cchProp;
	}
	else if (m_ichAnchor != m_ichEnd)
	{
		...
		prootb->GetDeleteRangeNotifications(m_qtsbProp, ichMin - m_ichMinEditProp,
			ichLim - m_ichMinEditProp, vpguidDeletedObjs);
		...
		CheckHr(m_qtsbProp->ReplaceRgch(ichMin - m_ichMinEditProp,
			ichLim - m_ichMinEditProp, NULL, 0, qttp));
		m_ichAnchor = m_ichEnd = ichMin;
	}
	return true;
}

// If m_qtsbProp is empty, make sure it has no external link data,
// and also that m_qttp doesn't, so anything subsequently typed doesn't get the old props.
void VwTextSelection::ClearLinksFromEmptyString()
{
	if (!m_qtsbProp)
		return;
	int cch;
	CheckHr(m_qtsbProp->get_Length(&cch));
	if (cch)
		return; // not empty
	// Deleting the whole string, make sure an empty string doesn't keep external link properties.
	if (!m_qttp)
	{
		CheckHr(m_qtsbProp->get_PropertiesAt(0, &m_qttp));
	}
	ITsPropsBldrPtr qtpb;
	CheckHr(m_qttp->GetBldr(&qtpb));
	SmartBstr sbstrEmpty(L"");
	CheckHr(qtpb->SetStrPropValue(ktptObjData, sbstrEmpty));
	SmartBstr sbstrStyle;
	CheckHr(m_qttp->GetStrPropValue(ktptNamedStyle, &sbstrStyle));
	if (wcscmp(sbstrStyle.Chars(), L"External Link") == 0 || // Still used in DN?
		wcscmp(sbstrStyle.Chars(), L"Hyperlink") == 0) // Now used in TE & FLEx
	{
		// Also remove the external link style
	...
	CheckHr(qtpb->GetTextProps(&m_qttp));
	CheckHr(m_qtsbProp->ReplaceRgch(0, 0, NULL, 0, m_qttp));
}
...
/*----------------------------------------------------------------------------------------------
	Send any wanted AboutToDelete notifications when deleting a paragraph.
----------------------------------------------------------------------------------------------*/
void VwTextSelection::AboutToDeletePara(HVO hvoPara, HVO hvoText, PropTag tag, int ihvo, bool fMergeNext)
{
	VwRootBox * prootb = m_pvpbox->Root();
	IVwRootSite * pvrs = prootb->Site();
	IVwObjDelNotificationPtr qodn;
	pvrs->QueryInterface(IID_IVwObjDelNotification, (void **)&qodn);
	if (qodn)
	{
		CheckHr(qodn->AboutToDelete(this, prootb, hvoPara, hvoText, tag, ihvo,
			fMergeNext));
	}
}
...
LNextEditable:
	...
	if (fResult)
	{
		...
		m_qttp = NULL;
		m_rcBounds.Clear();
		m_pvpbox->Root()->NotifySelChange(1);	// same paragraph
		return true;
	}
	...
	for (;;)
	{
		...
		m_qttp = NULL;
		m_rcBounds.Clear();
		m_pvpbox->Root()->NotifySelChange(1);	// same para
		return true;
	}
	...
	m_qttp = NULL;
	m_rcBounds.Clear();
	m_pvpbox->Root()->NotifySelChange(2);	// Different paragraph.
	return true;
}
...
bool VwTextSelection::BackwardArrow(IVwGraphics * pvg, bool fSuppressClumping)
{
	...
	int nHowChanged = ksctSamePara; // default same para
...
void VwTextSelection::PhysicalHomeOrEnd(IVwGraphics * pvg, bool fEnd)
{
	...
	fAssocPrev = (ichNewSel == psbox->IchMin()) ? false : fAssocPrev;
...
void VwTextSelection::ShiftPhysicalArrow(IVwGraphics * pvg, bool fRight, bool fSuppressClumping)
{
	...
	fAssocPrevNeeded = (IsInsertionPoint()) ? (fRight != fOldEndRtl) : fAssocPrev;
	...
	m_pvpbox->Root()->NotifySelChange(2);	// different para
}
...
bool VwTextSelection::SetEnd(VwParagraphBox * pvpboxNew, VwParagraphBox * pvpboxEnd, int ichEnd,
							 bool fEndBeforeAnchor, int & nHowChanged)
{
...
void VwTextSelection::ShiftEndKey(IVwGraphics * pvg, bool fLogical)
{
	...
	m_pvpbox->Root()->NotifySelChange(2);
}
	...
	IVwGraphics * pvg, VwParagraphBox ** ppvpbox, bool fSuppressClumping)
{
	...
	int ichMinPrevBox; // if fSamePara is true, holds ichBoxMin of previous string box.
...
/*----------------------------------------------------------------------------------------------
	Commit if editing and moving outside the edit range.  Return false is commit fails,
...
{
	m_plbox = ppicbox;
	m_ipicAnchor = ipicAnchor;
	m_ipicEnd = ipicEnd;
	m_fEndBeforeAnchor = m_ipicEnd < m_ipicAnchor;
	m_fAssocPrevious = fAssocPrevious;
	m_xdIP = -1;
...
int VwPictureSelection::GetCharPosition()
{
	...
	int cchPrev = 0;
	for (VwBox * pbox = pvpbox->FirstBox(); pbox != m_plbox; pbox = pbox->NextOrLazy())
	{
...
VwNotifier * VwPictureSelection::GetNotifier(int * piprop)
{
	...
	if (pvpbox)
	{
		...
		int ichPic = GetCharPosition();
		VwNotifier * pnoteBest = NULL;
		int tagBest = 0;
		int ipropBest = NULL; // index where we found picture in the best notifier.
		for (int i = 0; i < vpanote.Size(); i++)
		{
			...
			VwEditPropVal vepv = pnote->EditableSubstringAt(pvpbox, ichPic, ichPic + 1, true,
...
void VwTextSelection::AdjustForStringReplacement1(VwTxtSrc * psrcModify, int itssMin, int itssLim,
...
// appropriately.
void VwTextSelection::AdjustForStringReplacement(VwTxtSrc * psrcModify, int itssMin, int itssLim,
	VwTxtSrc * psrcRep)
{
	AdjustForStringReplacement1(psrcModify, itssMin, itssLim, psrcRep, m_ichAnchor, m_pvpbox);
	AdjustForStringReplacement1(psrcModify, itssMin, itssLim, psrcRep, m_ichEnd,
		(m_pvpboxEnd ? m_pvpboxEnd : m_pvpbox));
	if (m_qtsbProp)
	{
		AdjustForStringReplacement1(psrcModify, itssMin, itssLim, psrcRep, m_ichMinEditProp, m_pvpbox);
		AdjustForStringReplacement1(psrcModify, itssMin, itssLim, psrcRep, m_ichLimEditProp,
			(m_pvpboxEnd ? m_pvpboxEnd : m_pvpbox));
	}
}

// Default does nothing.
void VwSelection::AdjustForStringReplacement(VwTxtSrc * psrcModify, int itssMin, int itssLim,
	VwTxtSrc * psrcRep)
{
}
//:>********************************************************************************************
...
#include "Set_i.cpp"
template HashMap<HvoTagRec, UpdateInfo>;	// Hungarian: hmhtru
template Vector<OLECHAR>;
