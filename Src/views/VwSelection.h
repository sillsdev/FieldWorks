...
Hungarian: vwsel
----------------------------------------------------------------------------------------------*/
class VwSelection : public SilDispatchImpl<IVwSelection, &IID_IVwSelection, &LIBID_Views>
{
public:
	// Static methods
	...
	STDMETHOD(SetIpTypingProps)(ITsTextProps * pttp);
	...
	// Usually, the change is made by updating relevant attributes and assuming that
	// a consequent update message will propagate to the display. If fUpdateAttr is
	// false, the display is updated directly, and no attrs are changed. This is needed,
	// for example, in an embedded control that is displaying Cellar data but not linked
	// to the KB.
	virtual void OnTyping(IVwGraphics *pvg, const wchar * pchInput, int cchInput,
	...
	virtual LONG VisiblePageHeight(IVwGraphics * pvg, Rect rcDocumentCoord, Rect rcClientCoord);
	...
	virtual void FixTextEdit(VwParagraphBox * pvpbox, int itssMin, int itssLim,
		VpsTssVec & vpst);
	...
	virtual void AdjustForStringReplacement(VwTxtSrc * psrcModify, int itssMin, int itssLim,
		VwTxtSrc * psrcRep);
...
public:
	...
	void DoUpdateProp(VwRootBox * prootb, HVO hvo, PropTag tag, VwNoteProps vnp,
		...
		VwChangeInfo * pci = NULL);
	virtual bool RuinedByDeleting(VwBox * pbox, VwBox * pboxReplacement=0);
	void ModifyEmptyString(int * pwsPending);
...
public:
	void FixTextEdit(VwParagraphBox * pvpbox, int itssMin, int itssLim, VpsTssVec & vpst);
	...
	int AnchorOffset() {return m_ichAnchor;}
	int EndOffset() {return m_ichEnd;}
...
protected:
	...
	// A TsTextProps for the next character to be inserted. Meaningful only when the selection
	// is an IP.
	...
	void UnprotectedCommit(ComBool * pfOk);
	VwStringBox * GetStringBox(int ichLogIP, VwParagraphBox * pvpboxIP, bool fAssocPrev);
	void AdjustForRep(int & ichLog, VwParagraphBox * pvpbox, int itssMin, int itssLim,
		VpsTssVec & vpst);
	...
	void CommitAndContinue(ComBool * pfOk, VwChangeInfo * pci = NULL);
...
protected:
	...
	bool DeleteRangeAndPrepareToInsert(bool fSaveFirstCharProps,
		Vector<GUID> & vpguidDeletedObjs);
	void AboutToDeletePara(HVO hvoPara, HVO hvoText, PropTag tag, int ihvo, bool fMergeNext);
	...
	void AdjustForStringReplacement1(VwTxtSrc * psrcModify, int itssMin, int itssLim,
		VwTxtSrc * psrcRep, int & ich, VwParagraphBox * pvpbox);
	void ClearLinksFromEmptyString();
