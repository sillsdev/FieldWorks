...
STDMETHODIMP VwEnv::AddProp(int tag, IVwViewConstructor * pvvc, int frag)
{
	...
	CheckHr(m_qsda->get_Prop(m_hvoCurr, tag, &v));
	CheckHr(pvvc->DisplayVariant(this, tag, v, frag, &qtssNew));
...
/*----------------------------------------------------------------------------------------------
	Convert an GenDate to a TsString.
----------------------------------------------------------------------------------------------*/
void VwEnv::GenDateToTsString(int gdat, ISilDataAccess * psda, ITsString ** pptss)
{
	StrUni stuBuf;
	stuBuf.Format(L"%D", gdat);
	ITsStrFactoryPtr qtsf;
	qtsf.CreateInstance(CLSID_TsStrFactory);
	CheckHr(qtsf->MakeStringRgch(stuBuf.Chars(), stuBuf.Length(), GetUserWs(psda), pptss));
}
...
/*----------------------------------------------------------------------------------------------
		Add a "generalized date" property, as used in the FieldWorks conceptual model.
----------------------------------------------------------------------------------------------*/
STDMETHODIMP VwEnv::AddGenDateProp(int tag)
{
	BEGIN_COM_METHOD;
	int gdat;
	CheckHr(m_qsda->get_IntProp(m_hvoCurr, tag, &gdat));
	ITsStringPtr qtss;
	GenDateToTsString(gdat, m_qsda, &qtss);
	VwParagraphBox * pvpbox = dynamic_cast<VwParagraphBox *>(m_pgboxCurr);
	if (!pvpbox)
		OpenParagraph();
	//
	OpenProp(tag, NULL, 0, kvnpGenDateProp);
	CheckHr(AddString(qtss));
	CloseProp();
	if (!pvpbox)
		CloseParagraph();
...
----------------------------------------------------------------------------------------------*/
STDMETHODIMP VwEnv::AddSimpleRect(COLORREF rgb, int dmpWidth, int dmpHeight,
...
STDMETHODIMP VwEnv::get_OpenObject(HVO * phvoRet)
{
	...
	*phvoRet = (m_fObjectOpen ? m_hvoCurr : NULL);
	...
	int * pdys)
{
	...
	try
	{
		...
		prootb->putref_DataAccess(m_qsda);
		pvpboxCont->Container(prootb);
#endif
	...
	int chvoPrev)
{
	...
	// may override this. These two types are as yet never editable. Adding an appropriate
	// case to VwNotifier::DoUpdateProp would fix this.
	if (m_qzvps->Editable() && vnp != kvnpTimeProp && vnp != kvnpGenDateProp)
...
void VwEnv::IntToTsString(int nVal, ITsStrFactory ** pptsf, ISilDataAccess * psda, ITsString ** pptss)
{
	...
	_itow_s(nVal, buf, 10);		// ENHANCE: encoding-dependent conversion
	GetStringFactory(pptsf);
	CheckHr((*pptsf)->MakeStringRgch(buf, wcslen(buf), GetUserWs(psda), pptss));
}
