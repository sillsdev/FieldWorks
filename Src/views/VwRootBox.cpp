...
// Another generic factory (put here rather arbitrarily) to allow
// creating VwOleDbDa objects using CoCreateInstance.
// We don't want this in the VwCacheDa.cpp file because that gets included in multiple projects.
static GenericFactory g_factVwOleDbDa(
	_T("SIL.Views.VwOleDbDa"),
	&CLSID_VwOleDbDa,
	_T("SIL Database Cache"),
	_T("Apartment"),
	&VwOleDbDa::CreateCom);

// Another generic factory (put here rather arbitrarily) to allow
// creating VwDataSpec objects using CoCreateInstance.
// We don't want this in the VwCacheDa.cpp file because that gets included in multiple projects.
static GenericFactory g_factVwDataSpec(
	_T("SIL.Views.VwDataSpec"),
	&CLSID_VwDataSpec,
	_T("SIL Data Spec"),
	_T("Apartment"),
	&VwDataSpec::CreateCom);

// Another generic factory (put here rather arbitrarily) to allow
// creating DelObjUndoAction objects using CoCreateInstance.
// We don't want this in the DelObjUndoAction.cpp file because that gets included in multiple projects.
static GenericFactory g_factDelObjUndoAction(
	_T("SIL.Views.UndoDeleteObjectAction"),
	&CLSID_UndoDeleteObjectAction,
	_T("SIL Delete Object Undo Action"),
	_T("Apartment"),
	&DelObjUndoAction::CreateCom);
	...
	int cvDel)
{
	...
	for (int i = 0; i < vpanote.Size(); i++)
	{
		...
		// the obsolete one.
		if (vpanote[i]->KeyBox())
			CheckHr(vpanote[i]->PropChanged(hvo, tag, ivMin, cvIns, cvDel));
...
----------------------------------------------------------------------------------------------*/
STDMETHODIMP VwRootBox::OnTyping(IVwGraphics *pvg, BSTR bstrInput,
	int cchBackspace, int cchDelForward,
	wchar chFirst, int * pwsPending)
{
	BEGIN_COM_METHOD;
	ChkComArgPtrN(pvg);
	...
	if (m_qvwsel)
	{
		...
		//	throw;
		//}
		m_qvwsel->OnTyping(pvg, bstrInput, BstrLen(bstrInput),
			cchBackspace, cchDelForward, chFirst,
			pwsPending);
	}
...
STDMETHODIMP VwRootBox::OnChar(int chw)
{
	...
	m_qvwsel->OnTyping(pvg, &chw2, cchNormal, cchBackspace, cchDelForward, chw2,
		&encPending);
			...
			case kfssShift:
				{
					...
					int encPending = -1;	// bogus
					m_qvwsel->OnTyping(pvg, &chw, 1, 0, 0, chw, &encPending);
					break;
				}
			case kfssNone:
			case kfssControl:
...
STDMETHODIMP VwRootBox::MouseDblClk(int xd, int yd, RECT rcSrc1, RECT rcDst1)
{
	...
	if (!pselOld || !pselNew)
	{
		...
		// So just make the new selection--assuming we can commit the old one.
		ComBool fOk = true;
		if (pselOld)
			CheckHr(pselOld->Commit(&fOk));
		if (!fOk)
			return S_OK;
		SetSelection(qsel);
		if (pselNew)
...
/*----------------------------------------------------------------------------------------------
	...
	Notify(ksctDiffPara, 0) if the selection moved to a different paragraph,
	Notify(ksctSamePara, 0) if it moved within the same paragraph.
----------------------------------------------------------------------------------------------*/
STDMETHODIMP VwRootBox::AddSelChngListener(IEventListener * pel)
{
	BEGIN_COM_METHOD;
	ChkComArgPtrN(pel);

	m_velSelChange.Push(pel);
...
/*----------------------------------------------------------------------------------------------
----------------------------------------------------------------------------------------------*/
STDMETHODIMP VwRootBox::DelSelChngListener(IEventListener * pel)
{
	BEGIN_COM_METHOD;
	ChkComArgPtrN(pel);

	for (int iel = 0; iel < m_velSelChange.Size(); iel++)
	{
		if (m_velSelChange[iel] == pel)
		{
			m_velSelChange.Delete(iel);
			return S_OK;
		}
	}
...
----------------------------------------------------------------------------------------------*/
STDMETHODIMP VwRootBox::DrawingErrors()
{
	BEGIN_COM_METHOD;

	HRESULT hr;
	IgnoreHr(hr = Style()->DrawingErrors());
	if (FAILED(hr))
		return hr;
...
/*----------------------------------------------------------------------------------------------
	Set the active keyboard appropriately for the given writing system.  This method doesn't
	really belong necessarily on IVwRootBox, but it fits there as well as anywhere.

	@param pws - points to IWritingSystem object.
	@param pbstrActiveKeymanKbd - name of the current (active) Keyman keyboard, if any.
	@param pnActiveLangId - lang id of the current (active) keyboard.
	@param phklActive - HKL of the current (active) keyboard/language
	@param pfSelectLangPending - set true as appropriate.
----------------------------------------------------------------------------------------------*/
STDMETHODIMP VwRootBox::SetKeyboardForWs(IWritingSystem * pws, BSTR * pbstrActiveKeymanKbd,
	int * pnActiveLangId, int * phklActive, ComBool * pfSelectLangPending)
{
	...
	ChkComArgPtr(phklActive);
	ChkComArgPtr(pfSelectLangPending);

	int nLangId;
	int nLocale;
	CheckHr(pws->get_Locale(&nLocale));
	CheckHr(pws->get_CurrentInputLanguage(&nLangId));
	int ws;
	CheckHr(pws->get_WritingSystem(&ws));
#ifdef Tracing_KeybdSelection
	StrAnsi sta;
	sta.Format("VwRootBox::SetKeyboardForWs(%d) [new langid %d from old hkl %d]%n",
		ws, nLangId, *pnActiveLangId);
	::OutputDebugStringA(sta.Chars());
#endif
	...
	if (nLangId == LANGIDFROMLCID(nLocale))
		CheckHr(pws->get_KeymanKbdName(&sbstrKeymanKbd));
	// else let it seem we don't have one, even if we do.
#ifdef TRACING_KEYMAN
	StrAnsi sta;
	sta.Format("Adjusting keyboard for ws %d, keyman keyboard should be %S\n",
		ws, sbstrKeymanKbd.Chars());
	OutputDebugStringA(sta.Chars());
#endif
	...
	int nPageTotal)
{
	...
	try
	{
		...
		do
		{
			...
			{
				fMatch = true;
				_itow_s(nPageNo, buf, 10);		// ENHANCE JohnT: locale-dependent conversion
				cchSpecial = 7;
			}
			else
			{
				pchSpecial = wcsstr(pchString, L"&[date]");
				if (pchSpecial)
				{
					fMatch = true;
					...
					StrUni stuConv = buf2;
					wcscpy_s(buf, stuConv.Chars());
					cchSpecial = 7; // delete 7 chars for &[date]
				}
				else
				{
					pchSpecial = wcsstr(pchString, L"&[time]");
					if (pchSpecial)
					{
						fMatch = true;
					...
					{
						pchSpecial = wcsstr(pchString, L"&[pages]");
						if (pchSpecial)
						{
							fMatch = true;
							_itow_s(nPageTotal, buf, 10);		// ENHANCE JohnT: locale-dependent conversion
							cchSpecial = 7;
...
/*----------------------------------------------------------------------------------------------
	Get total number of pages for a printed view.
----------------------------------------------------------------------------------------------*/
STDMETHODIMP VwRootBox::GetTotalPrintPages(IVwPrintContext * pvpc, IAdvInd3 * padvi3,
	int *pcPageTotal)
{
	BEGIN_COM_METHOD;
...
/*----------------------------------------------------------------------------------------------
	Print your contents. Assume they have not yet been laid out.
----------------------------------------------------------------------------------------------*/
STDMETHODIMP VwRootBox::Print(IVwPrintContext * pvpc, IAdvInd3 * padvi3)
{
	BEGIN_COM_METHOD;
	ChkComArgPtr(pvpc);
	ChkComArgPtr(padvi3);
	...
	VwPrintInfo vpi;
	CreatePrintInfo(pvpc, vpi);
	IVwGraphicsPtr qvg = vpi.m_pvg;

	int dxsAvailWidth = vpi.m_rcDoc.Width();
	if (dxsAvailWidth <= 0)
		ThrowHr(WarnHr(E_FAIL));

	SmartBstr sbstrProgress;
	StrApp strProgress(kstidLayingOutPage);
	strProgress.GetBstr(&sbstrProgress);
	padvi3->put_Message(sbstrProgress);

	try
	{
		InitializePrinting(pvpc);
	}
	catch (...)
	{
		// We have something wrong and already notified user.
		return S_OK;
	}

	int cactDocCopies;
	int cactPageCopies = 1;
	int nPageFirst;
	ComBool fCollate;
	CheckHr(pvpc->get_FirstPageNumber(&nPageFirst));
	CheckHr(pvpc->get_Copies(&cactDocCopies));
	if (cactDocCopies)
	{
		CheckHr(pvpc->get_Collate(&fCollate));
		if (!fCollate)
		{
			cactPageCopies = cactDocCopies;
			cactDocCopies = 1;
		}
	}
	Rect rcSrc;
	...
	strProgress.Load(kstidCalculatingPages);
	strProgress.GetBstr(&sbstrProgress);
	padvi3->put_Message(sbstrProgress);

	GetTotalPrintPages(pvpc, padvi3, &vpi.m_nPageTotal);

	strProgress.Load(kstidPrintingPage);
	strProgress.GetBstr(&sbstrProgress);
	padvi3->put_Message(sbstrProgress);
	padvi3->put_Position(0);

	// Use this to make sure we only open a document if at least one page
	// in the requested range exists.
	bool fDocOpen = false;
	for (int iCopy = 0; iCopy < cactDocCopies; iCopy++)
	{
		int ysStartPage = ChooseSecondIfInverted(0, Bottom());
		int ysEndDoc = ChooseSecondIfInverted(Bottom(), 0);
		int ysEnd = 0;
		vpi.m_nPageNo = nPageFirst;
		for (; IsVerticallyAfter(ysEndDoc, ysStartPage); vpi.m_nPageNo++, ysStartPage = ysEnd)
		{
			padvi3->Step(1);
			ComBool fWanted;
			CheckHr(pvpc->get_IsPageWanted(vpi.m_nPageNo, &fWanted));
			if (!fWanted)
			{
				// See if we are done altogether...
				ComBool fMoreWanted;
				CheckHr(pvpc->get_AreMorePagesWanted(vpi.m_nPageNo, &fMoreWanted));
				if (!fMoreWanted)
					break;
			}
			rcSrc.top = ysStartPage;
			...
			if (fWanted)
			{
				for (int iPageCopy = 0; iPageCopy < cactPageCopies; iPageCopy++)
				{
					if (!fDocOpen)
					{
						fDocOpen = true;
						CheckHr(pvpc->OpenDoc());
					}

					pvpc->OpenPage();

					// When caching page start positions, the following four lines can be
					// replaced by a single call to PrintSinglePage
					PrintHeaders(pvpc, m_qsda, &vpi, vpi.m_nPageNo == nPageFirst);
					qvg->PushClipRect(vpi.m_rcDoc);
					PrintPage(&vpi, rcSrc, rcDst, ysStartPage, ysEnd);
					qvg->PopClipRect();

					pvpc->ClosePage();
					ComBool fAborted;
					pvpc->get_Aborted(&fAborted);
					if (fAborted)
					{
						pvpc->AbortDoc();
						return S_OK;
...
/*----------------------------------------------------------------------------------------------
	Requests the view to provide notification when editing operations delete text
	that includes an object replacement character (U+ff0f) marked as a hot link
	to an object. See ${IVwNotifyObjCharDeletion} for more details.
	NULL may be passed as an argument to disable notification.
----------------------------------------------------------------------------------------------*/
STDMETHODIMP VwRootBox::RequestObjCharDeleteNotification(IVwNotifyObjCharDeletion *pnocd)
{
	BEGIN_COM_METHOD;
	ChkComArgPtrN(pnocd);
...
----------------------------------------------------------------------------------------------*/
void VwRootBox::NotifySelChange(int nHow, bool fUpdateRootSite)
{
	VecListener vel;
	// Copy the vector in case notifications result in additions or deletions.
	int iel;
	for (iel = 0; iel < m_velSelChange.Size(); iel++)
		vel.Push(m_velSelChange[iel]);

	for (iel = 0; iel < vel.Size(); iel++)
	{
		vel[iel]->Notify(nHow, 0); // ignore errors!
	}

	if (m_qvwsel && fUpdateRootSite)
...
/*----------------------------------------------------------------------------------------------
	Gets a list of the objects that would be deleted as a result of deleting the specified range

	Currently the only important things are single-character runs where the character is
	0xfffc and the ktptObjData property is a kodtOwnNameGuidHot. We do not notify if it is
	a kodtNameGuidHot because we don't want to delete the object in this case.

	This is implemented by scanning runs; the advantage is that often runs are long, and
	we don't have to examine individual characters in such runs.

	Issue: should we also do this for kodtNameGuidHot, or only for kodtOwnNameGuidHot? Only the
	latter requires the destination object to be deleted, but the former might require
	adjusting some sort of backreference one day...
----------------------------------------------------------------------------------------------*/
void VwRootBox::GetDeleteRangeNotifications(ITsStrBldr * ptsb, int ichMin, int ichLim,
	Vector<GUID> & vpguidDeletedObjs)
{
	...
	TsRunInfo tri;
	ITsTextPropsPtr qttp;
	CheckHr(ptsb->FetchRunInfoAt(ichMin, &tri, &qttp));
	int crun;
	CheckHr(ptsb->get_RunCount(&crun));
	while ( tri.ichMin < ichLim)
	{
		if (tri.ichMin + 1 == tri.ichLim) // one-character run
		{
			OLECHAR ch;
			CheckHr(ptsb->FetchChars(tri.ichMin, tri.ichLim, &ch));
			if (ch == 0xfffc) // Right character
			{
				// See whether this run has owning guid hotlink properties
				SmartBstr sbstr;
				CheckHr(qttp->GetStrPropValue(ktptObjData, &sbstr));
				if (sbstr.Length() == 9 && sbstr.Chars()[0] == kodtOwnNameGuidHot)
				{
					// Match! Store the guid for the notification.
					vpguidDeletedObjs.Push(*((GUID *)(sbstr.Chars() + 1)));
				}
			}
		}
		...
		if (tri.irun + 1 >= crun)
			break;
		CheckHr(ptsb->FetchRunInfo(tri.irun + 1, &tri, &qttp));
	}
}

/*----------------------------------------------------------------------------------------------
	Notify anyone interested of anything important in the paragraphs about to be deleted.
	(Does not actually perform the deletion.)
	Currently the only important things are single-character runs where the character is
	0xfffc and the ktptObjData property is a kodt{Own}NameGuidHot.
----------------------------------------------------------------------------------------------*/
void VwRootBox::DoNotificationsForDeletePara(int hvoPara)
{
	if (!m_qnocd)
		return;
	ITsStringPtr qtss;
	CheckHr(m_qsda->get_StringProp(hvoPara, kflidStTxtPara_Contents, &qtss));
	int cch;
	CheckHr(qtss->get_Length(&cch));
	// We could also make a method like DoNotificationsForDeleteRange which takes an ITsString.
	// However, most deletions are within a single paragraph, where the above is more efficient.
	ITsStrBldrPtr qtsb;
	CheckHr(qtss->GetBldr(&qtsb));
	Vector<GUID> vpguidDeletedObjs;
	GetDeleteRangeNotifications(qtsb, 0, cch, vpguidDeletedObjs);
	for(int i = 0; i < vpguidDeletedObjs.Size(); i++)
	{
		if(m_qnocd)
			IgnoreHr(m_qnocd->ObjDeleted(&vpguidDeletedObjs[i]));
	}
}
...
/*----------------------------------------------------------------------------------------------
	...
	them valid.
----------------------------------------------------------------------------------------------*/
void VwRootBox::FixSelectionsForStringReplacement(VwTxtSrc * psrcModify, int itssMin, int itssLim,
		VwTxtSrc * psrcRep)
{
	for (int isel = m_vselInUse.Size(); --isel >= 0; )
		m_vselInUse[isel]->AdjustForStringReplacement(psrcModify, itssMin, itssLim, psrcRep);
}
...
/*----------------------------------------------------------------------------------------------
	...
	Note: there is not much reason for this to be on the root box; the caller could
	go straight to the selection. However, this makes it more parallel with FixSelections,
	and provides a common point if ever anything else needs to know about text edits.
----------------------------------------------------------------------------------------------*/
void VwRootBox::FixTextEdit(VwParagraphBox * pvpbox, int itssMin,
	int itssLim, VpsTssVec & vpst)
{
	if (m_qvwsel)
		m_qvwsel->FixTextEdit(pvpbox, itssMin, itssLim, vpst);
}
...
/*----------------------------------------------------------------------------------------------
	If the current keyboard and window is associated with an IME, then set the IME composition
	window's location to (approximately) follow the screen location of the selection.
----------------------------------------------------------------------------------------------*/
void VwRootBox::SetImeWindowLocation(HKL hkl)
{
	if (!::ImmIsIME(hkl))
		return;
	HWND hwnd = NULL;
	m_qvrs->get_Hwnd((DWORD *)&hwnd);
	if (hwnd == NULL)
		return;
	HIMC himc = ::ImmGetContext(hwnd);
	if (himc == NULL)
		return;
	if (m_qvwsel.Ptr() == NULL)
		return;

	COMPOSITIONFORM comp;
	if (::ImmGetCompositionWindow(himc, &comp))
	{
#ifdef Tracing_KeybdSelection
		StrAnsi staStyle;
		switch (comp.dwStyle)
		{
		case CFS_DEFAULT:			staStyle = "CFS_DEFAULT";			break;
		case CFS_FORCE_POSITION:	staStyle = "CFS_FORCE_POSITION";	break;
		case CFS_POINT:				staStyle = "CFS_POINT";				break;
		case CFS_RECT:				staStyle = "CFS_RECT";				break;
		default:			staStyle.Format("0x%x", (int)comp.dwStyle);	break;
		}
		StrAnsi staMsg;
		staMsg.Format("VwRootBox::SetImeWindowLocation(hkl = %d) - "
			"::ImmGetCompositionWindow() => %s, {X=%d,Y=%d}, {L=%d,T=%d,R=%d,B=%d}%n",
			(int)hkl, staStyle.Chars(), (int)comp.ptCurrentPos.x, (int)comp.ptCurrentPos.y,
			(int)comp.rcArea.left, (int)comp.rcArea.top, (int)comp.rcArea.right,
			(int)comp.rcArea.bottom);
		::OutputDebugStringA(staMsg.Chars());
#endif
		HoldGraphics hg(this);
		Rect rdPrimary, rdSec;
		ComBool fSplit, fEndBeforeAnchor;
		CheckHr(m_qvwsel->Location(hg.m_qvg, hg.m_rcSrcRoot, hg.m_rcDstRoot, &rdPrimary,
			&rdSec, &fSplit, &fEndBeforeAnchor));
#ifdef Tracing_KeybdSelection
		staMsg.Format("VwRootBox::SetImeWindowLocation(hkl = %d) - "
			"m_qvwsel->Location() => rdPrimary = {L=%d,T=%d,R=%d,B=%d}%n",
			(int)hkl, rdPrimary.left, rdPrimary.top, rdPrimary.right, rdPrimary.bottom);
		::OutputDebugStringA(staMsg.Chars());
#endif
		comp.dwStyle = CFS_POINT;
		comp.ptCurrentPos.x = rdPrimary.left;
		comp.ptCurrentPos.y = rdPrimary.top;
