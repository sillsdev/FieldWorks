	...
	int dyBottomPrevious, int & irow, bool & fNeedAdjust)
{
	...
	dyAlignedBaseline = max(dyAlignedBaseline, dyBottomPrevious + Ascent());
...
void VwGroupBox::GetNotifiers(VwBox * pboxChild, NotifierVec& vpanote)
{
	...
	for (VwBox * pbox = FirstBox(); pbox != pboxChild->NextOrLazy(); pbox = pbox->NextOrLazy())
	{
		if (!pbox)
		{
			Assert(false); // should reach the box after pboxChild, this *this is its parent;
...
int VwPileBox::ComputeTopOfBoxAfter(VwBox * pboxPrev, int dypInch)
{
	...
	if (pboxNext)
	{
		...
		int dympCorrectSep = max(dympBottom + dympMswTop, dympTop);
...
void VwPileBox::AdjustLeft(VwBox * pbox, int dxLeft, int dxpInnerWidth)
{
	...
	case ktalRight:
		...
		break;
	case ktalCenter:
		xPos += max(0, (dxpInnerWidth - pbox->Width())/2);
		break;
	}
	pbox->Left(xPos);
}
...
VwBox * VwPileBox::FindNonPileChildAtOffset (int dysPosition, int dpiY, int * pdysOffsetIntoBox)
{
	...
	for (VwBox * pboxChild = FirstBox(); pboxChild; )
	{
		if (dysPosition < pboxChild->Bottom() - MulDiv(pboxChild->MarginBottom(), dpiY, kdzmpInch))
		{
			...
			{
				// if we are looking more than 20 inches into the box we may expand alot of
				// stuff and this is bad (slow). This is designed to handle one or 2 boxes
				// that need to be expanded.
				Assert((dysPosition / dpiY) < 20);

				VwLazyBox * plzbox = dynamic_cast<VwLazyBox *>(pboxChild);
				plzbox->ExpandItems(0, 1, false);
...
void VwPileBox::DrawForeground(IVwGraphics * pvg, Rect rcSrc, Rect rcDst)
{
	...
	DrawForeground(pvg, rcSrc, rcDst, ChooseSecondIfInverted(-1, Height() + 1), ChooseSecondIfInverted(Height() + 1, -Height() - 1));
}
...
void VwSeparatorBox::DoLayout(IVwGraphics* pvg, int dxAvailWidth, int dxpAvailOnLine, bool fSyncTops)
{
	...
	CheckHr(pvg->GetTextExtent(1,L"A",&m_dxsWidth, &m_dysHeight));
...
----------------------------------------------------------------------------------------------*/
void VwIndepPictureBox::GetSelection(IVwGraphics * pvg, VwRootBox * prootb, int xd, int yd,
	Rect rcSrcRoot, Rect rcDstRoot, Rect rcSrc, Rect rcDst, VwSelection ** ppvwsel,
	SearchDirection sdir)
{
	*ppvwsel = NewObj VwPictureSelection(this, 1, 1, true);
}
	...
	int yd, Rect rcSrcRoot, Rect rcDstRoot, Rect rcSrc, Rect rcDst, bool forceReadOnly)
{
	...
	StrUni stuUndo;
	StrUni stuRedo;
	stuUndo.Format(stuFmtUndo.Chars(), sbstrFieldName);
	stuRedo.Format(stuFmtRedo.Chars(), sbstrFieldName);
	...
	CheckHr(psda->PropChanged(Root(), kpctNotifyMeThenAll, m_hvo, m_tag, 0, 1, 1));
