...
{
	m_stuFontFamily = FwStyledText::FontDefaultMarkup();
...
void VwPropertyStore::InitChrp()
{
	...
	for ( ; ; )
	{
		...
		if (pch == pchStartName)
		{
			...
			// find a useable font.
			wcscpy_s(m_chrp.szFaceName, L"<default>"); // TODO: should this use FwStyledText::FontDefaultMarkup()?
			break;
		}
		...
		if (cchCopy > 31)
			cchCopy = 31;
		wcsncpy_s(m_chrp.szFaceName, pchStartName, cchCopy);
...
STDMETHODIMP VwPropertyStore::get_FontFamily(BSTR * pbstr)
{
	...
	{
		*pbstr = SysAllocString(L"<serif>"); // last ditch default
		if (*pbstr)
			return S_OK;
		else
...
//		*pnValue = m_nNumStartAt;	WRONG! INT_MIN is not suitable for XML export!
	...
	case ktptSpellCheck:
		*pnValue = m_smSpellMode;
		break;
	default:
		Warn("invalid ID for reading int property");
		return E_NOTIMPL;
	}
...
STDMETHODIMP VwPropertyStore::put_StringProperty(int sp, BSTR bstrValue)
{
	...
	case ktptFontVariations:
		...
		wcscpy_s(m_chrp.szFontVar, stuTmp.Chars());
		break;
	case ktptTags:
		m_stuTags = bstrValue;
		break;
	case ktptNamedStyle:
		if (BstrLen(bstrValue) && m_qss)
		{
			...
			m_fDropCaps = (wcscmp(L"Chapter Number", bstrValue) == 0);
...
void VwPropertyStore::DoWsDefaultFontVar(int ws)
{
	...
	if (sbstrFontFamily == DefaultSans)
		CheckHr(qws->get_SansFontVariation(&sbstrFeatures));
	else if (sbstrFontFamily == DefaultBodyFont)
		CheckHr(qws->get_BodyFontFeatures(&sbstrFeatures));
	else
		CheckHr(qws->get_FontVariation(&sbstrFeatures));
	if (sbstrFeatures)
...
----------------------------------------------------------------------------------------------*/
HRESULT VwPropertyStore::DrawingErrors()
{
	HRESULT hr;
	...
	if (m_fInitChrp)
	{
		...
		if (qws)
		{
			...
			CheckHr(m_qwsf->get_RendererFromChrp(&m_chrp, &qreneng));
			if (qreneng)
			{
	...
	if (m_qzvpsReset)
	{
		IgnoreHr(hr = m_qzvpsReset->DrawingErrors());
		if (FAILED(hr))
			return hr;
	}
	...
	{
		VwPropertyStore * pzvps = ithmvprzvps.GetValue();
		IgnoreHr(hr = pzvps->DrawingErrors());
		if (FAILED(hr))
			// Found an error.
			return hr;
	}
