...
public:
	...
	STDMETHOD(CacheStringFields)(HVO obj, PropTag tag, const OLECHAR * prgchTxt, int cchTxt,
		const byte * prgbFmt, int cbFmt);
	...
	STDMETHOD(PropChanged)(IVwNotifyChange * pnchng, int pct, HVO hvo, int tag, int ivMin,
		int cvIns, int cvDel);
	STDMETHOD(SuppressPropChanges)();
	STDMETHOD(ResumePropChanges)();
