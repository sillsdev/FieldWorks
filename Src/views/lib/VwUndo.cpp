...
----------------------------------------------------------------------------------------------*/
STDMETHODIMP_(ULONG) VwUndoAction::AddRef(void)
{
	Assert(m_cref > 0);
	return ++m_cref;
}
...
----------------------------------------------------------------------------------------------*/
STDMETHODIMP_(ULONG) VwUndoAction::Release(void)
{
	Assert(m_cref > 0);
	if (--m_cref > 0)
		return m_cref;
...
----------------------------------------------------------------------------------------------*/
STDMETHODIMP VwUndoAction::Undo(ComBool fRefreshPending, ComBool * pfSuccess)
{
	BEGIN_COM_METHOD;
...
----------------------------------------------------------------------------------------------*/
STDMETHODIMP VwUndoAction::Redo(ComBool fRefreshPending, ComBool *  pfSuccess)
{
	BEGIN_COM_METHOD;
	ChkComOutPtr(pfSuccess);
...
----------------------------------------------------------------------------------------------*/
STDMETHODIMP VwUndoAction::IsDataChange(ComBool * pfRet)
{
	BEGIN_COM_METHOD;
	ChkComOutPtr(pfRet);
...
----------------------------------------------------------------------------------------------*/
STDMETHODIMP VwUndoAction::IsRedoable(ComBool * pfRet)
{
	BEGIN_COM_METHOD;
	ChkComOutPtr(pfRet);
...
/*----------------------------------------------------------------------------------------------
	Return true if this kind of action doesn't reload the cache and thus requires a refresh;
	false if cache is up-to-date after the Undo/Redo and we called all necessary PropChangeds.
	We answer true because all current subclasses issue PropChanged. If you change this, be
	SURE to override in current subclasses. Note that the current subclasses are used in
	WorldPad, which currently DOES NOT SUPPORT the full-refresh approach; all UndoActions used
	in WorldPad MUST return false from this method.
----------------------------------------------------------------------------------------------*/
STDMETHODIMP VwUndoAction::RequiresRefresh(ComBool * pfRet)
{
	BEGIN_COM_METHOD;
	ChkComOutPtr(pfRet);
...
/*----------------------------------------------------------------------------------------------
	...
	data; false if it is just cleaning up the user interface (eg, replacing the selection).
----------------------------------------------------------------------------------------------*/
STDMETHODIMP VwUndoSelectionAction::IsDataChange(ComBool * pfRet)
{
	BEGIN_COM_METHOD;
	ChkComOutPtr(pfRet);
