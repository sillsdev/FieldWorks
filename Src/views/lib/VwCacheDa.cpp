...
//:>********************************************************************************************
//:>    Implementation of IStructuredTextDataAccess
//:>********************************************************************************************

/*----------------------------------------------------------------------------------------------
 Get the Contents field ID from the StTxtPara tags.
----------------------------------------------------------------------------------------------*/
STDMETHODIMP VwCacheDa::get_ParaContentsFlid(PropTag * paraContentsFlid)
{
	BEGIN_COM_METHOD;

	*paraContentsFlid = kflidStTxtPara_Contents;

	END_COM_METHOD(g_fact, IID_IStructuredTextDataAccess);
}

/*----------------------------------------------------------------------------------------------
 Get the Properties field ID from the StPara tags.
----------------------------------------------------------------------------------------------*/
STDMETHODIMP VwCacheDa::get_ParaPropertiesFlid(PropTag * paraPropertiesFlid)
{
	BEGIN_COM_METHOD;

	*paraPropertiesFlid = kflidStPara_StyleRules;

	END_COM_METHOD(g_fact, IID_IStructuredTextDataAccess);
}

/*----------------------------------------------------------------------------------------------
 Get the Paragraphs field ID from the StText tags.
----------------------------------------------------------------------------------------------*/
STDMETHODIMP VwCacheDa::get_TextParagraphsFlid(PropTag * textParagraphsFlid)
{
	BEGIN_COM_METHOD;

	*textParagraphsFlid = kflidStText_Paragraphs;
...
----------------------------------------------------------------------------------------------*/
STDMETHODIMP VwCacheDa::MoveString(int hvoSrc, PropTag flidSrc, int wsSrc, int ichMin,
	int ichLim, HVO hvoDst, PropTag flidDst, int wsDst, int ichDst, ComBool fDstIsNew)
{
	BEGIN_COM_METHOD;
	if (hvoSrc == hvoDst || wsSrc != 0 || wsDst != 0 || flidSrc != kflidStTxtPara_Contents || flidDst != kflidStTxtPara_Contents)
	{
		ThrowHr(E_NOTIMPL);
	}
	ITsStringPtr qtssSrc;
	CheckHr(get_StringProp(hvoSrc, flidSrc, &qtssSrc));
	ITsStringPtr qtssDst;
	CheckHr(get_StringProp(hvoDst, flidDst, &qtssDst));
	int cchSrc, cchDst;
	CheckHr(qtssSrc->get_Length(&cchSrc));
	CheckHr(qtssDst->get_Length(&cchDst));
	if (ichLim > cchSrc || ichDst > cchDst)
		ThrowHr(E_INVALIDARG);

	ITsStrBldrPtr qtsbMove;
	CheckHr(qtssSrc->GetBldr(&qtsbMove));
	if (ichLim < cchSrc)
		CheckHr(qtsbMove->ReplaceTsString(ichLim, cchSrc, NULL));
	if (ichMin > 0)
		CheckHr(qtsbMove->ReplaceTsString(0, ichMin, NULL));
	ITsStringPtr qtssMove;
	CheckHr(qtsbMove->GetString(&qtssMove));

	ITsStringPtr qtssNewSrc;
	ITsStrBldrPtr qtsbNewSrc;
	CheckHr(qtssSrc->GetBldr(&qtsbNewSrc));
	CheckHr(qtsbNewSrc->ReplaceTsString(ichMin, ichLim, NULL));
	CheckHr(qtsbNewSrc->GetString(&qtssNewSrc));

	ITsStringPtr qtssNewDst;
	ITsStrBldrPtr qtsbNewDst;
	CheckHr(qtssDst->GetBldr(&qtsbNewDst));
	CheckHr(qtsbNewDst->ReplaceTsString(ichDst, ichDst, qtssMove));
	CheckHr(qtsbNewDst->GetString(&qtssNewDst));

	SetStringVal(hvoSrc, flidSrc, qtssNewSrc);
	SetStringVal(hvoDst, flidDst, qtssNewDst);
	...
	ComBool * pfCached)
{
	...
	default:
		ThrowInternalError(E_INVALIDARG, "Not a valid property type");
	}
	END_COM_METHOD(g_fact, IID_ISilDataAccess);
}
...
STDMETHODIMP VwCacheDa::BeginUndoTask(BSTR bstrUndo, BSTR bstrRedo)
{
	...
	// VwCacheDa is now mainly used in testing the Views code (and maybe for various sandboxes).
	// We get a useful simulation of the way the real cache does undo tasks by suppressing
	// PropChanged messages until the end of the undo task/UOW.
	SuppressPropChanges();
...
void VwCacheDa::DoInsert(HVO hvoOwner, HVO hvoNew, PropTag tag, int ord)
{
	...
	if (ord >= -1)
	{
		...
		if (m_nSuppressPropChangesLevel)
			m_shvoNewObjectsWhileSuppressed.Insert(hvoNew);
		CheckHr(PropChanged(NULL, kpctNotifyAll, hvoOwner, tag, ord, 1, 0));
	}
	...
	bool fClearIncomingRefs)
{
	...
	CheckHr(PropChanged(NULL, kpctNotifyAll, hvoOwner, tag, ihvo, 0, 1));
}
...
void VwCacheDa::SetMultiStringAltVal(HVO hvo, PropTag tag, int ws, ITsString * ptss)
{
	...
	int cchOld = 0;
	if (m_hmopertss.Retrieve(opreKey, qtssOld))
		CheckHr(qtssOld->get_Length(&cchOld));
	m_hmopertss.Insert(opreKey, ptss, true); // allow overwrites
	InformNowDirty();
	int cchNew;
	CheckHr(ptss->get_Length(&cchNew));
	// NOTE: some PropChange MultiStringAlt monitors interpret ivMin to be a ws.
	CheckHr(PropChanged(NULL, kpctNotifyAll, hvo, tag, ws, cchNew, cchOld));
}
...
void VwCacheDa::SetStringVal(HVO hvo, PropTag tag, ITsString * ptss)
{
	...
	ITsStringPtr qtssOld;
	int cchOld = 0;
	if (m_hmoprtss.Retrieve(oprKey, qtssOld))
		CheckHr(qtssOld->get_Length(&cchOld));
	m_hmoprtss.Insert(oprKey, ptss, true); // allow overwrites
	InformNowDirty();
	int cchNew;
	CheckHr(ptss->get_Length(&cchNew));
	CheckHr(PropChanged(NULL, kpctNotifyAll, hvo, tag, 0, cchNew, cchOld));
}
	...
	int ivMin, int cvIns, int cvDel)
{
	...
	{
		if (m_shvoNewObjectsWhileSuppressed.IsMember(hvo))
			return S_OK;
...
----------------------------------------------------------------------------------------------*/
void VwCacheDa::SuppressPropChanges()
{
...
----------------------------------------------------------------------------------------------*/
void VwCacheDa::ResumePropChanges()
{
	if (m_nSuppressPropChangesLevel > 0)
		m_nSuppressPropChangesLevel--;
	...
	if (!m_nSuppressPropChangesLevel)
	{
		m_shvoNewObjectsWhileSuppressed.Clear();
		for (int i = 0; i < m_vPropChangeds.Size(); i++)
		{
			PropChangedInfo pci = m_vPropChangeds[i];
...
/*----------------------------------------------------------------------------------------------
	...
	@param ihvo The index of the object in the property
	@param pihvoDisp Pointer to receive the index in the display.
	@return S_OK for normal completion. E_POINTER if pihvoDisp is invalid. E_INVALID arg if
		hvoOwn or flid are zero.
----------------------------------------------------------------------------------------------*/
STDMETHODIMP VwCacheDa::GetDisplayIndex(HVO hvoOwn, int flid, int ihvo, int * pihvoDisp)
{
	BEGIN_COM_METHOD;
	ChkComOutPtr(pihvoDisp);
	if (!hvoOwn || !flid)
		ThrowInternalError(E_INVALIDARG);
...
template class HashMap<ObjPropRec, ObjSeq>; // ObjPropSeqMap; // Hungarian hmoprsobj
template class ComHashMap<ObjPropEncRec, ITsString>; // ObjPropEncTssMap; // Hungarian hmopertss
template class ComHashMap<ObjPropRec, IUnknown>; // ObjPropUnkMap; // Hungarian hmoprunk
template class Set<ObjPropEncRec>; // ObjPropEncSet;
template class Set<ObjPropRec>; // ObjPropSet; // Hungarian sopr
template class HashMap<ObjPropRec, StrUni>; // ObjPropStrMap; // Hungarian hmoprstu
template class Set<HVO>; // HvoSet; // Hungarian shvo
template class HashMap<ObjPropRec, SeqExtra>; // ObjPropExtraMap; // Hungarian hmoprsx
template class ComHashMap<PropTag, IVwVirtualHandler>; // TagVhMap; // Hungarian hmtagvp
template class ComHashMapStrUni<IVwVirtualHandler>; // StrVhMap; // Hungarian hmstuvh
