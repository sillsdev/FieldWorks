	...
	int dypTopToTopNatural)
{
	...
	for (int irootb = 0; irootb < m_vrootb.Size(); irootb++)
	{
		...
		{
			dypTopToTopActual = max(dypTopToTopActual,
	...
	if (pboxContainer)
	{
		...
		if (!pnote)
		{
			...
			sta.Format("Didn't find notifier for box: %x (%S)", pbox,
				pbox->Root()->m_stuAccessibleName ? pbox->Root()->m_stuAccessibleName.Chars() : L"<not set>");
			Warn(sta.Chars());
			return;
		}
...
template ComVector<IVwRootBox>; // RootBoxVec;
template Vector<ExpandLazyItemsInfo>; // LazyItemsInfoVec;
