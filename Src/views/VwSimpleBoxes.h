...
public:
	...
	virtual Rect VwBox::GetInvalidateRect();

	// a full rect in the rootbox coordinate system that contains the box.
	virtual Rect VwBox::GetBoundsRect(IVwGraphics * pvg, Rect rcSrcRoot, Rect rcDstRoot);

	// square of distance from box to point (0 if inside)
	int VwBox::DsqToPoint(int x, int y, Rect rcSrc);
	...
	// objects, this method needs to be called directly on those boxes if desired. Currently,
	// only string boxes do anything interesting.
...
public:
	VwPictureBox(VwPropertyStore * pzvps, IPicture * ppic, int dxmpWidth = 0, int dympHeight = 0)
		:VwLeafBox(pzvps)
	{
		m_qpic = ppic;
