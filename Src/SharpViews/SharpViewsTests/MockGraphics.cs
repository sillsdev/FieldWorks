...
namespace SIL.FieldWorks.SharpViews.SharpViewsTests
{
	class MockGraphics : IVwGraphics
	{
		...
		{
			ClipRectangle = new Rectangle(Math.Min(rcClip.left, ClipRectangle.Left), Math.Min(rcClip.top, ClipRectangle.Top), Math.Min(rcClip.right, ClipRectangle.Right), Math.Min(rcClip.bottom, ClipRectangle.Bottom));
		}
