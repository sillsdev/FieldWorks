...
namespace SIL.FieldWorks.SharpViews.SharpViewsTests
{
	...
	public class TextSourceTests: SIL.FieldWorks.Test.TestUtils.BaseTest
	{
		...
		ClientRun MockInterpretOrc(TextClientRun run, int offset)
		{
		...
		void VerifyRun(int logical, ClientRun clientRun, int render, int offset, string runText, MapRun run, string label)
		{
			VerifyRun(logical, clientRun, render, offset, 0, runText, run, label);
		}

		void VerifyRun(int logical, ClientRun clientRun, int render, int offset, int irun, string runText, MapRun run, string label)
		{
		...
		public void RenderRuns()
		{
			...
			TextSource source = new TextSource(clientRuns, (run, offset) => (run == clientRun2 ? new StringClientRun(orcText, run.UniformRunStyles(0).WithWs(wsFrn)) : (ClientRun)box));

			List<RenderRun> renderRuns = source.RenderRuns;
		...
		void VerifyRenderRun(RenderRun run, int start, int length, string label)
		{
