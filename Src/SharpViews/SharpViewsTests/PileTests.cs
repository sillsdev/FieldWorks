...
namespace SIL.FieldWorks.SharpViews.SharpViewsTests
{
	...
	public class PileTests : BaseTest
	{
		...
		public void ClippedPaint()
		{
			...
			VerifyPaint(root, new Rectangle(-1000, -1000, 2000, 2000), 0, paintRects);
			// Clipping off the top part, but not the whole, of a box does not prevent drawing it.
			VerifyPaint(root, new Rectangle(-1000, 10, 2000, -10 + rect4.Bottom - 10), 0, paintRects);
			// Even clipping all but one pixel does not prevent drawing.
			VerifyPaint(root, new Rectangle(-1000, 47, 2000, -47 + rect3.Bottom + 1), 0, paintRects);
			// However if we clip a bit more we should draw less
			var middleTwo = new[] {rect2, rect3};
			VerifyPaint(root, new Rectangle(-1000, 48, 2000, -48 + rect3.Bottom - 2), 0, middleTwo);
			// If the clip covers just a bit of the first box we paint just that.
			var firstOne = new[] {rect1};
			VerifyPaint(root, new Rectangle(-1000, -1000, 2000, 1000 + 10), 0, firstOne);
			// If the clip covers just a bit of the last box we paint just that.
			var lastOne = new[] { rect4 };
			VerifyPaint(root, new Rectangle(-1000, rect4.Bottom - 2, 2000, 1000), 0, lastOne);
			// If the clip is entirely above the pile we draw nothing.
			VerifyPaint(root, new Rectangle(-1000, -1000, 2000, 990), 0, null);
			// Likewise if entirely below.
			VerifyPaint(root, new Rectangle(-1000,rect4.Bottom + 10, 2000, 10), 0, null);
			...
			VerifyPaint(root, new Rectangle(-1000, 0, 2000, 200), offset, lastTwoOffset);
		}


		private void VerifyPaint(RootBox root, Rectangle clipRect, int scrollOffset, Rectangle[] paintRects)
		{
			MockGraphics graphics = new MockGraphics();
			graphics.ClipRectangle = clipRect;
			PaintTransform ptrans = new PaintTransform(0, 0, 96, 96, 0, scrollOffset, 96, 96);
