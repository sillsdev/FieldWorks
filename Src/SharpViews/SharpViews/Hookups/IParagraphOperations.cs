...
namespace SIL.FieldWorks.SharpViews.Hookups
{
	...
	public interface IParagraphOperations
	{
		...
		bool InsertRTF(InsertionPoint ip, string insert, out Action makeSelection);

		bool InsertLines(MultiLineInsertData input, out Action makeSelection);
		...
		void ApplyParagraphStyle(int index, int numBoxes, string style);
	...
	/// </summary>
	public interface ISeqParagraphOperations<T> : IParagraphOperations
	{
		/// <summary>
	...
	public interface IObjParagraphOperations<T> : IParagraphOperations
	{
		/// <summary>
		/// When a paragraph operations is passed as the PO of a particular hookup, the hookup is
		/// passed to this method.
		/// </summary>
		ObjHookup<T> Hookup { get; set; }
	}
