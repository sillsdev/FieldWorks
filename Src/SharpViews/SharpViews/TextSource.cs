...
namespace SIL.FieldWorks.SharpViews
{
	...
	/// </summary>
	public delegate IClientRun OrcInterpreter(TextClientRun run, int offset);
	/// <summary>
	...
	{
		internal List<IClientRun> ClientRuns { get; set; }
		...
		/// <summary>
		/// Answer true if there is an embedded box at the specified character index.
		/// </summary>
		public bool IsThereABoxAt(int ich)
		{
			return GetRenderText(ich, 1)[0] == LeafBox.CharThatStandsForBox;
		}


		OrcInterpreter Interpreter { get; set; }

		public TextSource(List<IClientRun> runs)
			: this(runs, null)
		{}
		...
		private void ComputeRuns()
		{
			...
			int ichLog = 0;
			int ichRen = 0;
			foreach (IClientRun clientRun in ClientRuns)
			{
		...
		/// <summary>
		/// Return the index of the client run containing the indicated character (the one at position ichRenTarget if associatePrevious
		...
		int ClientRunContainingRender(int ichRenTarget, bool associatePrevious, out int offset)
		{
			...
			int clientRunIndex = ClientRuns.IndexOf(clientRun);
			int logicalStartOfClientRun = ClientRuns.Take(clientRunIndex).Sum(run => run.Length);
			if (mapRun.RenderLength == mapRun.LogLength)
			{
				...
				offset = ichRenTarget - mapRun.RenderStart + mapRun.LogStart - logicalStartOfClientRun;
			}
			else
			{
				...
				offset = mapRun.Offset + mapRun.LogStart - logicalStartOfClientRun;
			}
		...
		/// </summary>
		internal IClientRun NonEmptyStringClientRunEndingBefore(int clientRunIndex)
		{
			for (int index = clientRunIndex - 1; index >= 0; index--)
			{
				...
				if (possibleResult.Length == 0)
					continue;
				if (possibleResult is MlsClientRun)
					return possibleResult as MlsClientRun;
				if (possibleResult is TssClientRun)
					return possibleResult as TssClientRun;
		...
		/// <summary>
		/// If possible, find a non-empty client run which starts at the end position of the argument run.
		/// This is usually the run immediately after it but might be later if there are adjacent empty runs.
		/// Otherwise return null.
		/// </summary>
		internal IClientRun NonEmptyStringClientRunBeginningAfter(int clientRunIndex)
		{
			for (int index = clientRunIndex + 1; index < ClientRuns.Count; index++)
			{
				var possibleResult = ClientRuns[index];
				if (possibleResult.Length == 0)
					continue;
			...
			return null; // ran out of next runs
		}
		...
		/// </summary>
		public List<IRenderRun> RenderRuns
		{
			get
			{
				List<IRenderRun> result = new List<IRenderRun>();
				int start = 0;
		...
		class ClientRunChangedMethod
		{
			...
			private IClientRun m_oldRun; // the one we are replacing.
			...
			{
			   var newClientRuns = new List<IClientRun>(m_source.ClientRuns);
				...
				foreach (var mr in m_source.Runs)
				{
					...
					{
						if (!madeNewRuns && m_source.ClientRuns.IndexOf(mr.ClientRun) > m_clientRunIndex)
						{
							ichLog = MakeRunsForMissingOldClientRun(isourceRun, ichLog, ref ichRen);
							madeNewRuns = true;
						}
				...
				if (!madeNewRuns)
					ichLog = MakeRunsForMissingOldClientRun(isourceRun+1, ichLog, ref ichRen);
				ComputeDifferences();
			...
			// There is no map run for this client run; probably it is an empty string with
			// no prompt. We will insert the new material here.
			private int MakeRunsForMissingOldClientRun(int isourceRun, int ichLog, ref int ichRen)
			{
				m_replacedRunMin = m_replacedRunLim = isourceRun;
				m_source.AddMapRunsForClientRun(m_newRun, m_newRuns, ref ichLog, ref ichRen);
				m_newRunLim = m_newRuns.Count;
				return ichLog;
			}
			...
			void ComputeDifferences()
			{
				...
				int ichStartChange = m_replacedRunMin == 0 ? 0 : m_source.Runs[m_replacedRunMin - 1].RenderLim;
				// It can be zero if we are replacing an empty run that produced no map runs before the first map run.
				int ichLimChangeOld = m_replacedRunLim == 0 ? 0 : m_source.Runs[m_replacedRunLim - 1].RenderLim;
