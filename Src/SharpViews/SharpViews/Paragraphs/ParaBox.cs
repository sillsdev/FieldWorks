...
namespace SIL.FieldWorks.SharpViews.Paragraphs
{
	public class ParaBox : GroupBox, IStringParaNotification
	{
		...
		public ParaBox(AssembledStyles styles)
			: this(styles, new TextSource(new List<IClientRun>()))
		{
		}

		public ParaBox(AssembledStyles styles, TextSource source)
			: base(styles)
		{
			Source = source;
		}
		...
		public ParaBox NextParaBox
		{
			get
			{
				Box newBox = Next;
				while (!(newBox is ParaBox))
				{
					if (newBox == null)
						return null;
					newBox = newBox.Next;
				}
				return (ParaBox)newBox;
			}
		}
	}
}
