...
namespace SIL.FieldWorks.SharpViews.Builders
{
	...
	public abstract class Flow
	{
		...
		/// <summary>
		/// Fluent language: return this, but the contents of the flow have the specified RightToLeft value.
		/// </summary>
		public Flow RightToLeft(bool rtl)
		{
			AddSetter(AssembledStyles.RtlSetter(rtl));
			return this;
		}
		/// <summary>
	...
	{
		internal ISeqParagraphOperations<T> m_paragraphOps;
		public Flow<T> EditParagraphsUsing(ISeqParagraphOperations<T> paragraphOps)
		{
	...
	internal class AddObjFlow<T> : Flow<T> where T : class
	{
		internal Expression<Func<T>> FetchItem { get; set; }
		internal ItemBuilder<T> ItemBuilder { get; set; }

		public override void AddContent(ViewBuilder builder)
		{
			builder.AddObj(FetchItem, ItemBuilder.m_displayOneItem);
			base.AddContent(builder);
		}
	}
	/// <summary>
	...
	internal class AtomicFlow : Flow
	{
		private Flow Child { get; set; }

		public AtomicFlow(Flow child)
		{
			Child = child;
		}
