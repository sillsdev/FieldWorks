...
bool TssListBox::OnDrawThisItem(DRAWITEMSTRUCT * pdis)
{
	...
	else
	{
		...
		if (FAILED(hr))
			return false;
		HFONT hfontOld = AfGdi::SelectObjectFont(pdis->hDC, (HFONT)::GetStockObject(DEFAULT_GUI_FONT));
...
bool TssListBox::OnMeasureThisItem(MEASUREITEMSTRUCT * pmis)
{
	...
	SIZE size;
	HDC hdc = ::GetDC(NULL);
	HFONT hfontOld = AfGdi::SelectObjectFont(hdc, (HFONT)::GetStockObject(DEFAULT_GUI_FONT));
