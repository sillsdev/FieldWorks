...
public:
	...
	static HBRUSH SelectObjectBrush(HDC hdc, HGDIOBJ brush, SelType fNew=NEW);
	static BOOL DeleteObjectBrush(HGDIOBJ brush);
...
public:
	PenWrap(HPEN hpen, HDC hdc)
	{
		...
		Assert(hdc);
		m_hdc = hdc;
		m_hpenOld = (HPEN)::SelectObject(hdc, hpen);
		if (!m_hpenOld)
			ThrowHr(WarnHr(E_FAIL));
		m_hpen = hpen;
	}
...
public:
	RgnWrap(HRGN hrgn, HDC hdc)
	{
		...
		m_hrgnOld = (HRGN)::SelectObject(hdc, hrgn);
...
//		::DeleteObject(hrgnTemp);  This DeleteObject is not needed.
		...
		else if (!iResult)
		{
			...
			::DeleteObject(m_hrgnOld); // trash the temp one
