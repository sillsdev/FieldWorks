...
static const char * ColorName(int nColor)
{
	...
	sprintf_s(szColor, "%02x%02x%02x", nRed, nGreen, nBlue);
#else
	sprintf_s(szColor, "%08x", nColor);
#endif
	return szColor;
}
...
static const char * ToggleValueName(byte ttv)
{
	...
	default:
		sprintf_s(szName, "%u", ttv);		// REVIEW SteveMc: what if it's not a valid value?
		return szName;
	}
}
...
static const char * PropVarName(int tpv)
{
	...
	default:
		sprintf_s(szName, "%u", tpv);		// REVIEW SteveMc: what if it's not a valid value?
		return szName;
	}
}
...
static const char * SuperscriptValName(byte ssv)
{
	...
	default:
		sprintf_s(szName, "%u", ssv);		// REVIEW SteveMc: what if it's not a valid value?
		return szName;
	}
}
...
static const char * UnderlineTypeName(byte unt)
{
	...
	default:
		sprintf_s(szName, "%u", unt);		// REVIEW SteveMc: what if it's not a valid value?
		return szName;
	}
}
...
static const char * SpellingModeName(byte sm)
{
	...
	default:
		sprintf_s(szName, "%u", sm);		// REVIEW SteveMc: what if it's not a valid value?
		return szName;
	}
}
...
static const char * AlignmentTypeName(byte tal)
{
	...
	default:
		sprintf_s(szName, "%u", tal);		// REVIEW SteveMc: what if it's not a valid value?
		return szName;
	}
}
...
static const char * EditableName(int ted)
{
	...
	default:
		sprintf_s(szName, "%u", ted);		// REVIEW SteveMc: what if it's not a valid value?
		return szName;
	}
}
...
template Vector<char>;	// Needed for Release build.
