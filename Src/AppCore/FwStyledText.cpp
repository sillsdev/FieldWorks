...
static Pcsz g_pszDefaultSerif = _T("<default serif>");
static Pcsz g_pszDefaultSans = _T("<default sans serif>");
static Pcsz g_pszDefaultMono = _T("<default monospace>");
static Pcsz g_pszDefaultBodyFont = _T("<default body>");
		...
		while (m_ich < m_stu.Length())
		{
			...
			if (cchFF > 0)
			{
				...
				if (cchFF == sbstr.Length() &&
					wcsncmp(sbstr.Chars(), m_stu.Chars() + m_ich, cchFF) == 0)
				{
					m_stu.SetAt(m_ich - 1, '\0'); // change length to zero.
					m_stu.Replace(m_ich , m_ich + cchFF, (OLECHAR *)NULL, 0);
				}
	...
	for (; iesi < vesi.Size(); iesi++)
	{
		...
		*pch++ = (OLECHAR) (ws >> 16);
		...
		if (stuFont == "<default>")	// ENHANCE JohnL(?): remove when we support 3 default fonts
			stuFont = g_pszDefaultSerif;
		*pch++ = (OLECHAR) stuFont.Length();
		wcscpy(pch, stuFont.Chars());
		pch += stuFont.Length();
		...
		if (esi.m_stuFontVar.Length())
		{
			...
			*pch++ = (OLECHAR) esi.m_stuFontVar.Length();
			wcscpy(pch, esi.m_stuFontVar.Chars());
