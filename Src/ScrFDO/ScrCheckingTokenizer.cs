	...
	class ScrCheckingTokenizer : IEnumerator<ITextToken>
	{
		...
		private class TokenizableText
		{
			...
			private void InitPara()
			{
				m_iRun = -1;
				ScrTxtPara para =
					new ScrTxtPara(m_text.Cache, m_text.ParagraphsOS.HvoArray[m_iPara]);

				m_obj = para;
				m_paraTss = para.Contents.UnderlyingTsString;
		...
		private Dictionary<int, string> m_writingSystemLocales;
		...
		{
			if (ws == m_book.Cache.DefaultVernWs || ws <= 0)
				return null;

			string icuLocale;
			if (!m_writingSystemLocales.TryGetValue(ws, out icuLocale))
			{
				IWritingSystem writingSystem =
					m_book.Cache.LanguageWritingSystemFactoryAccessor.get_EngineOrNull(ws);
				icuLocale = writingSystem == null ? null : writingSystem.IcuLocale;
				m_writingSystemLocales[ws] = icuLocale;
			}
			return icuLocale;
		}
