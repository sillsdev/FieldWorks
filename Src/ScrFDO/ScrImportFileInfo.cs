...
namespace SIL.FieldWorks.FDO.Scripture
{
	...
	/// </summary>
	/// ----------------------------------------------------------------------------------------
	public class ScrImportFileInfo
	{
		#region Data members
		/// <summary></summary>
		...
		/// <summary>
		/// The ICU locale of the source to which this file belongs (null for Scripture source)
		/// </summary>
		private string m_icuLocale;
		/// <summary>
		/// The HVO of the CmAnnotationDefn of the source to which this file belongs
		/// (only used for Note sources)
		/// </summary>
		private int m_noteTypeHvo;
		...
		public ScrImportFileInfo(CmFile file, ScrMappingList mappingList, ImportDomain domain,
			...
			this(file, mappingList, domain, null, 0, scanInlineBackslashMarkers)
		{
		}
		...
		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Gets the HVO of the CmAnnotationDefn of the source to which this file belongs
		/// (only used for Note sources)
		/// </summary>
		/// ------------------------------------------------------------------------------------
		public int NoteTypeHvo
		{
			get { return m_noteTypeHvo; }
		}
		...
		{
			m_fileEncoding = FileUtils.DetermineSfFileEncoding(m_fileName);
		}
		...
		protected virtual TextReader GetReader()
		{
			...
			Encoding useEncoding = (m_fileEncoding == Encoding.ASCII) ?
				Encoding.GetEncoding(SCTextEnum.kMagicCodePage) : m_fileEncoding;
			return new StreamReader(m_fileName, useEncoding);
		}
		...
		{
			return VersificationTable.Get(Paratext.ScrVers.English).LastChapter(book) == 1;
		}
		#endregion
		...
		/// ------------------------------------------------------------------------------------
		public static bool CheckForOverlap(ScrImportFileInfo map1, ScrImportFileInfo map2)
		{
