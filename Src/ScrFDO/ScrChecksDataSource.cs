...
namespace SIL.FieldWorks.FDO.Scripture
{
	...
	{
		private ScrBook m_bookBeingChecked = null;
		...
		/// will be deleted.
		/// </summary>
		private Dictionary<int, Dictionary<Guid, Dictionary<string, List<ScrScriptureNote>>>> m_pendingCheckErrors;
		...
		private static StyleMarkupInfo m_styleMarkupInfo = null;
		...
		public CharacterCategorizer CharacterCategorizer
		{
			get
			{
				ILgCharacterPropertyEngine charPropEngine =
					(ILgCharacterPropertyEngine)m_cache.LanguageWritingSystemFactoryAccessor.get_CharPropEngine(
						m_cache.DefaultVernWs);
				return new FwCharacterCategorizer(ValidCharacters, charPropEngine) as CharacterCategorizer;
			}
		}
		...
		public StylePropsInfo StyleInfo
		{
			get
			{
				...
				{
					m_styleMarkupInfo = StyleMarkupInfo.Load(
						DirectoryFinder.FWCodeDirectory + @"\Translation Editor\Testyles.xml");
				}
				return m_styleMarkupInfo.StyleInfo;
			}
		}
		...
		/// ------------------------------------------------------------------------------------
		private string GetSentenceFinalPunctuation(IWritingSystem ws,
				...
				foreach (PuncPattern pattern in puncPatternsList)
				{
						...
						pattern.ContextPos == ContextPosition.WordFinal)
					{
						...
						foreach (char puncChar in pattern.Pattern)
						{
							...
							if (StringUtils.IsEndOfSentenceChar(puncChar,
