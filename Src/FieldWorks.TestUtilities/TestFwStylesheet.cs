...
namespace FieldWorks.TestUtilities
{
	...
	public class TestFwStylesheet : IVwStylesheet
	{
		...
		public string GetNextStyle(string sName)
		{
			...
			{
				var sNameNext = GetStyleName(style.NextStyle);
				return sNameNext ?? "Normal";
			}
			return "Normal";
		}
		...
		public string GetBasedOn(string sName)
		{
			...
			{
				var sNameBasedOn = GetStyleName(style.BasedOnStyle);
				return sNameBasedOn ?? string.Empty;
			}
			return string.Empty;
		}
		...
		public void OverrideFontsForWritingSystems(string styleName, List<FontOverride> fontOverrides)
		{
			...
			var propsAsString = string.Empty;
			foreach (var aFontOverride in fontOverrides)
			{
				...
				propsAsString += Convert.ToChar((short)FwTextPropType.ktptFontSize);
				propsAsString += Convert.ToChar((short)FwTextPropVar.ktpvMilliPoint);
