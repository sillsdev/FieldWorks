...
namespace SIL.FieldWorks.Discourse
{
	...
	public class ConstChartBody : RootSite
	{
		...
		protected override void OnMouseDown(MouseEventArgs e)
		{
			if (System.DateTime.Now.Ticks - m_ticksWhenContextMenuClosed > 50000) // 5ms!
			{
				...
				if (cell != null && cell.IsValidLocation && cell.ColIndex >= m_AllColumns.Length)
				{
		...
		private bool GetCellInfo(MouseEventArgs e, out ChartLocation clickedCell, out int irow)
		{
			...
			irow = -1;
			if (m_hvoChart == 0 || m_AllColumns == null || e.Y > RootBox.Height || e.X > RootBox.Width)
				return false;
			Point pt;
			Rectangle rcSrcRoot;
			Rectangle rcDstRoot;
			using (new HoldGraphics(this))
			{
				...
				Debug.Assert(irow >= 0 && irow < chart.RowsOS.Count);
	...
	internal class ConstChartVc : InterlinVc
	{
		...
		internal void InsertOpenBracket(IVwEnv vwenv, string key)
		{
			...
			var sbracket = m_cache.TsStrFactory.MakeString(
								bracket.Substring(0, 1), m_cache.DefaultAnalWs);
		...
		internal void InsertCloseBracket(IVwEnv vwenv, string key)
		{
			...
			if (!m_brackets.TryGetValue(key, out bracket))
				return;
			var sbracket = m_cache.TsStrFactory.MakeString(
								bracket.Substring(1, 1), m_cache.DefaultAnalWs);
			vwenv.AddString(sbracket);
		}
	}
	...
	class MakeCellsMethod
	{
		...
		private int m_hvoCurCellCol = 0; // column for which cell is currently open (initially not for any column)
		private int m_iLastColForWhichCellExists = -1; // index of last column for which we have made (at least opened) a cell.
		...
		private int m_cCellPartsInCurrentCell = 0; // number of cellparts output in current cell.
		private int m_icellpart = 0;
		// index of last column where automatic missing markers are put.
		private int m_icolLastAutoMissing = -1;
		private ITsString m_missMkr; // stores the TsString displayed for missing markers (auto or user)
		...
		public void Run()
		{
			...
			{
				while (icellPart2 < m_cellparts.Length && !GoesInsideClauseBrackets(m_cellparts[icellPart2]))
					icellPart2++;
				m_icellPartOpenClause = icellPart2 < m_cellparts.Length ? icellPart2 : 0;
			}
			if (m_row.EndDependentClauseGroup)
			{
				icellPart2 = m_cellparts.Length - 1;
				while (icellPart2 >= 0 && !GoesInsideClauseBrackets(m_cellparts[icellPart2]))
				{
					icellPart2--;
				}
				m_icellPartCloseClause = icellPart2 >= 0 ? icellPart2 : m_cellparts.Length - 1;
			...
			for (m_icellpart = 0; m_icellpart < m_cellparts.Length; m_icellpart++)
			{
				...
				var cellPartFlidArray = new[] { ConstituentChartCellPartTags.kflidColumn,
		...
		private void AddCellPartToCell(IConstituentChartCellPart cellPart)
		{
			...
			if (m_icellpart == m_icellPartOpenClause)
			{
				m_this.InsertOpenBracket(m_vwenv, ConstChartVc.GetRowStyleName(m_row));
			}
			...
			if (m_icellpart == m_icellPartCloseClause)
			{
				m_this.InsertCloseBracket(m_vwenv, ConstChartVc.GetRowStyleName(m_row));
			}
		}
		...
		private void MakeEmptyCells(int count)
		{
			...
			{
				var icol = i + m_iLastColForWhichCellExists + 1;
		...
		{
			if (m_icellPartOpenClause == m_icellpart)
			{
				m_this.InsertOpenBracket(m_vwenv, ConstChartVc.GetRowStyleName(m_row));
