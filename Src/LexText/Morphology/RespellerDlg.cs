...
namespace SIL.FieldWorks.XWorks.MorphologyEditor
{
	public partial class RespellerDlg : Form
	{
		...
		public RespellerDlg()
		{
			...
			m_btnClose.Text = MEStrings.ksClose;
			label2.Text = MEStrings.ksNewSpelling;
			m_btnPreviewClear.Text = MEStrings.ksPreview;
			m_btnHelp.Text = MEStrings.ksHelp;
			m_btnApply.Text = MEStrings.ksApply;
			m_rbKeepAnalyses.Text = MEStrings.ksKeepMultiMorphemicAnalyses;
			m_rbDiscardAnalyses.Text = MEStrings.ksDeleteMultiMorphemicAnalyses;
			m_btnMore.Text = MEStrings.ksMore;
			m_cbMaintainCase.Text = MEStrings.ksMaintainExistingCaseInBaseline;
			m_lblExplainDisabled.Text = MEStrings.ksSomeOptionsAreDisabledBecause_;
			m_cbCopyAnalyses.Text = MEStrings.ksCopyApprovedAnalysesToNewSpelling;
			m_optionsLabel.Text = MEStrings.ksOptions;
			...
			foreach (var text in InterestingTexts)
			{
				...
				foreach (IStTxtPara para in text.ParagraphsOS)
				{
					...
					for (var irun = 0; irun < crun; irun++)
					{
						...
						FwObjDataTypes odt;
						ITsTextProps props;
						var guid = StringUtils.GetGuidFromRun(contents, irun, out odt, out tri, out props, desiredType.ToArray());
						if (guid == Guid.Empty)
							continue;
