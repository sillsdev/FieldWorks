...
namespace SIL.FieldWorks.XWorks.LexEd
{
	...
	public class SwapLexemeWithAllomorphDlg : Form, IFWDisposable
	{
		...
		private System.Windows.Forms.ListView m_lvAlloOptions;
		...
		private void SetBottomMessage()
		{
			...
			string sLexVal = m_entry.LexemeFormOA.Form.VernacularDefaultWritingSystem;
			// Treat null value as empty string.  This fixes LT-5889, LT-5891, and LT-5914.
			if (sLexVal == null)
				sLexVal = "";
			ITsString tss;
			string sFmt = LexEdStrings.ksSwapXWithY;
			string sWithVal = m_allomorph.Form.VernacularDefaultWritingSystem;
			if (sWithVal == null)
				sWithVal = "";
			tss = m_tsf.MakeString(String.Format(sFmt, sLexVal, sWithVal, "\x2028"), userWs);
			//ITsStrBldr tsb = tss.GetBldr();
			//int ich = sFmt.IndexOf("{0}");
			//int cch = sLexVal.Length;
			//if (ich >= 0 && cch > 0)
			//{
