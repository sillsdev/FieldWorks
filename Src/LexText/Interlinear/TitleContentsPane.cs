...
namespace SIL.FieldWorks.IText
{
	...
	/// </summary>
	public class TitleContentsPane : RootSiteControl
	{
		int m_hvoRoot; // The Text.
		TitleContentsVc m_vc;
	...
	/// </summary>
	class TitleContentsVc : VwBaseVc
	{
		...
		int m_wsAnalysis;
		LgWritingSystem[] m_writingSystems;
		ITsString[] m_WsLabels;
		ITsTextProps m_ttpWsLabel;
		int m_editBackColor = (int)SIL.FieldWorks.Common.Utils.ColorUtil.ConvertColorToBGR(Color.FromKnownColor(KnownColor.Window));
		...
		/// </summary>
		bool m_fIsScripture = false;
		internal bool IsScripture
		{
			get { return m_fIsScripture; }
			set { m_fIsScripture = value; }
		}

		/// <summary>
		/// Indicates whether we should allow editing in the TitleContents fields.
		/// </summary>
		bool m_fIsEditable = true;
		...
		protected override void Dispose(bool disposing)
		{
			...
			Marshal.ReleaseComObject(m_tssTitle);
			m_tssTitle = null;
			Marshal.ReleaseComObject(m_ttpBold);
			m_ttpBold = null;
			Marshal.ReleaseComObject(m_ttpDataCellProps);
			m_ttpDataCellProps = null;
			Marshal.ReleaseComObject(m_ttpWsLabel);
			m_ttpWsLabel = null;
			m_writingSystems = null;
			foreach (ITsString label in m_WsLabels)
			...
			switch(frag)
			{
				case kfragRoot:
					...
					for (int i = 0; i < m_writingSystems.Length; i++)
					{
						...
						bool fRTL = m_writingSystems[i].RightToLeft;
						...
						if (IsScripture)
						{
							vwenv.AddStringAltMember(m_vtagStTextTitle,
							m_writingSystems[i].Hvo, this);
						}
						else
						{
							vwenv.AddStringAltMember((int)CmMajorObject.CmMajorObjectTags.kflidName,
								m_writingSystems[i].Hvo, this);
						}
