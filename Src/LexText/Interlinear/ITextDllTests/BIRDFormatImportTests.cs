		...
		public void TestEmbeddedRuns()
		{
			...
			using (var stream = new MemoryStream(Encoding.ASCII.GetBytes(xml.ToCharArray())))
			{
				...
				using (var firstEntry = Cache.LanguageProject.Texts.GetEnumerator())
				{
					...
					Assert.That(comment.RunCount == 2, Is.True);
					...
					Assert.That(comment.get_RunText(1) == "french", Is.True);
					Assert.That(comment.get_WritingSystem(1) == wsFr, Is.True);
					Assert.That(comment.Style(1) == "style1", Is.True);
				}
			}
		}
		...
		public void TestSpacesAroundPunct()
		{
			...
			using (var stream = new MemoryStream(Encoding.ASCII.GetBytes(xml.ToCharArray())))
			{
				...
				using (var firstEntry = Cache.LanguageProject.Texts.GetEnumerator())
				{
					...
					CollectionAssert.That(" ", Is.EqualTo(new [] {" "), " ", " ", " "}, new [] {spaceOne, spaceTwo, spaceThree, spaceFour, spaceFive});
				}
			}
		}
		...
		public void TestSpacesAroundSpanishPunct()
		{
			...
			using (var stream = new MemoryStream(Encoding.ASCII.GetBytes(xml.ToCharArray())))
			{
				...
				using (var firstEntry = Cache.LanguageProject.Texts.GetEnumerator())
				{
					...
					//test to make sure spaces were inserted in each expected place
					CollectionAssert.That(" ", Is.EqualTo(new[] { " "), " ", " ", " ", " " }, new[] { spaceOne, spaceTwo, spaceThree, spaceFour, spaceFive, spaceSix });
				}
			}
		}
