...
namespace SIL.FieldWorks.IText
{
	...
	public class InterlinLineChoices : ICloneable
	{
		...
		public InterlinLineChoices(int defaultVernacularWs, int defaultAnalysisWs, ILangProject proj)
			: this(defaultVernacularWs, defaultAnalysisWs, proj, InterlinMode.Analyze)
		{
		}
		...
		public InterlinLineChoices(int defaultVernacularWs, int defaultAnalysisWs)
			: this(defaultVernacularWs, defaultAnalysisWs, null)
		{
		}
		...
		public static InterlinLineChoices DefaultChoices(int vern, int analysis, ILangProject proj)
		{
			return DefaultChoices(vern, analysis, proj, InterlinMode.Analyze);
		}
		...
		public static InterlinLineChoices Restore(string data, ILgWritingSystemFactory wsf, int defVern, int defAnalysis)
		{
			return Restore(data, wsf, defVern, defAnalysis, null);
		}
		...
		public const int kflidWord = (int)WfiWordform.WfiWordformTags.kflidForm;
		/// <summary></summary>
		public const int kflidMorphemes = (int)WfiMorphBundle.WfiMorphBundleTags.kflidMorph;
		/// <summary>
		...
		public const int kflidLexGloss = (int)WfiMorphBundle.WfiMorphBundleTags.kflidSense;
		public const int kflidLexPos = (int)WfiMorphBundle.WfiMorphBundleTags.kflidMsa;
		public const int kflidWordGloss = (int)WfiGloss.WfiGlossTags.kflidForm;
		public const int kflidWordPos = (int)WfiAnalysis.WfiAnalysisTags.kflidCategory;
		...
		private bool MatchingWritingSystem(int wsConfig, int ws, bool fExact)
		{
			...
			switch (wsConfig)
			{
				case LangProject.kwsAnal:
					return ws == m_proj.DefaultAnalysisWritingSystem;

				case LangProject.kwsPronunciation:
					return ws == m_proj.DefaultPronunciationWritingSystem;

				case LangProject.kwsVern:
					return ws == m_proj.DefaultVernacularWritingSystem;

				case LangProject.kwsAnals:
				case LangProject.kwsFirstAnal:
					return m_proj.AnalysisWssRC.Contains(ws);
	...
	{
		public EditableInterlinLineChoices(int defaultVernacularWs, int defaultAnalysisWs, ILangProject proj)
			: base(defaultVernacularWs, defaultAnalysisWs, proj)
		{
		}
		public EditableInterlinLineChoices(int defaultVernacularWs, int defaultAnalysisWs)
			: base(defaultVernacularWs, defaultAnalysisWs)
		{
		}
