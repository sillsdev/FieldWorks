...
namespace SIL.FieldWorks.IText
{
	...
	internal class ChooseAnalysisHandler : IComboHandler, IFWDisposable
	{
		...
		void AddItem(int hvoAnalysis, ITsString text, bool fPossibleCurrent)
		{
			AddItem(hvoAnalysis, text, fPossibleCurrent, 0);
		}
		...
		/// </summary>
		/// <param name="hvoAnalysis"></param>
		void AddAnalysisItems(int hvoAnalysis)
		{
			AddItem(hvoAnalysis,
				MakeAnalysisStringRep(hvoAnalysis, m_cache, StyleSheet != null, (m_owner as SandboxBase).RawWordformWs), true);
			foreach (int hvoGloss in m_cache.GetVectorProperty(hvoAnalysis,
				(int)WfiAnalysis.WfiAnalysisTags.kflidMeanings, false))
			{
				AddItem(hvoGloss, MakeGlossStringRep(hvoGloss, m_cache, StyleSheet != null), true);
			}

			//add the "new word gloss" option

			AddItem(hvoAnalysis, MakeSimpleString(ITextStrings.ksNewWordGloss), false, WfiGloss.kclsidWfiGloss);
		}
