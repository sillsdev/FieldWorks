...
namespace SIL.FieldWorks.IText
{
	...
	public class InterlinVc : FwBaseVc, IDisposable
	{
		...
		protected virtual void AddFreeformComment(IVwEnv vwenv, int hvoSeg, int lineChoiceIndex)
		{
			...
			tsbLabel.ReplaceTsString(0, tsbLabel.Length, StringUtils.MakeTss(label, m_cache.DefaultUserWs));
			tsbLabel.SetIntPropValues(0, tsbLabel.Length, (int) FwTextPropType.ktptBold,
									  (int) FwTextPropVar.ktpvEnum, (int) FwTextToggleVal.kttvForceOn);
			tsbLabel.SetStrPropValue(0, tsbLabel.Length, (int)FwTextPropType.ktptNamedStyle, SimpleRootSite.UiElementStylename);
		...
		private void AddSegmentReference(IVwEnv vwenv, int hvo)
		{
			...
			tsbSegNum.ReplaceTsString(0, tsbSegNum.Length, StringUtils.MakeTss(sbSegNum.ToString(), m_cache.DefaultUserWs));
			...
			bool fWantMultipleSenseGloss, bool fShowMorphemes)
		{
			...
			foreach (InterlinLineSpec spec in m_lineChoices)
			{
				if (!spec.WordLevel)
					break;
				SetColor(vwenv, LabelRGBFor(spec));
				ITsString tss = SimpleRootSite.MakeUiElementString(m_lineChoices.LabelFor(spec.Flid), wsUI);
		...
		class DisplayWordBundleMethod
		{
			...
			private void DisplayWord(InterlinLineSpec spec, int choiceIndex)
			{
					...
					if (m_this.m_fShowMorphBundles)
					{
						...
						if (m_hvoDefault != m_hvoWordBundleAnalysis)
						{
							...
							m_this.SetGuessing(m_vwenv,
								m_this.Decorator.get_IntProp(m_hvoDefault, InterlinViewDataCache.OpinionAgentFlid) != (int)AnalysisGuessServices.OpinionAgent.Parser);
						}
			...
			private void DisplayWordPOS(int choiceIndex)
			{
				...
				case WfiAnalysisTags.kClassId:
					if (m_hvoDefault != m_hvoWordBundleAnalysis)
					{
						...
						m_this.SetGuessing(m_vwenv, m_this.Decorator.get_IntProp(m_hvoDefault, InterlinViewDataCache.OpinionAgentFlid) != (int)AnalysisGuessServices.OpinionAgent.Parser);
					}
		...
		public override ITsString DisplayVariant(IVwEnv vwenv, int tag, int frag)
		{
			...
			if (tag == SimpleRootSite.kTagUserPrompt)
			{
				...
				ITsStrBldr bldr = StringUtils.MakeTss(ITextStrings.ksEmptyFreeTransPrompt, m_cache.DefaultUserWs).GetBldr();
