...
namespace SIL.FieldWorks.IText
{
	...
	public partial class SandboxBase : RootSite
	{
		...
		{
			string locale = m_caches.MainCache.ServiceLocator.WritingSystemManager.Get(StringUtils.GetWsAtOffset(tss, 0)).IcuLocale;
			...
			if (analysis == null)
			{
				if (fLookForDefaults)
				{
					...
					if (!fHumanApproved)
					{
						...
						fHumanApproved = IsAnalysisHumanApproved(m_caches.MainCache, analysis);
					}
				...
				if (this.ShowMorphBundles)
				{
					...
					foreach (var mb in analysis.MorphBundlesOS)
					{
						...
						if (senseReal == null)
						{
							...
							GuessColor = InterlinVc.MachineGuessColor;
						}
			...
			ISilDataAccess sdaMain, int hvoMain, int flidMain)
		{
			foreach (int ws1 in currentWsList)
			{
				...
				if (hvoMain != 0)
					...
					tssMain = StringUtils.MakeTss("", ws1);
		...
		private int GetMostPromisingEntry()
		{
			...
			List<ILexEntry> homographs =
				Cache.ServiceLocator.GetInstance<ILexEntryRepository>().CollectHomographs(m_caches.MainCache, wordform.Text,
