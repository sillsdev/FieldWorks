...
namespace FwParserWatcher
{
	...
	public class Form1 : Form, IFWDisposable
	{
		...
		protected List<FdoCache> m_FDOCaches;
		...
		public Form1()
		{
			...
			//
			InitializeComponent();
			m_FDOCaches = new List<FdoCache>();
			m_parsers = new List<ParserConnection>();

			m_FDOCaches.Add(FdoCache.Create("TestLangProj"));
			m_selectedFdoCache = m_FDOCaches[0];
		...
		protected override void Dispose( bool disposing )
		{
			...
			if( disposing )
			{
				...
				foreach (FdoCache cache in m_FDOCaches)
					cache.Dispose();
				m_FDOCaches.Clear();
			}
			m_FDOCaches = null;
		...
		{
			int hvo = m_selectedFdoCache.LangProject.LexDbOA.EntriesOC.HvoArray[0];
