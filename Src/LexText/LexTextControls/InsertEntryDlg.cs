	...
	public class InsertEntryDlg : Form, IFWDisposable
	{
		...
		private ITsString TssForm
		{
			...
			set
			{
				...
				{
					int wsForm = StringUtils.GetWsAtOffset(value, 0);
...
catch(Exception e)
{
	...
	MessageBox.Show(e.StackTrace);
		...
		public void SetDlgInfo(FdoCache cache, ITsString tssForm, Mediator mediator)
		{
			...
			IWritingSystemContainer wsContainer = cache.ServiceLocator.WritingSystems;
			int wsForm = StringUtils.GetWsAtOffset(tssForm, 0);
			...
			if (fVern)
			{
				...
				TssGloss = StringUtils.MakeTss("", wsContainer.DefaultAnalysisWritingSystem.Handle);
			...
			{
				TssForm = StringUtils.MakeTss("", wsContainer.DefaultVernacularWritingSystem.Handle);
				TssGloss = tssForm;
		...
		protected virtual void UpdateMatches()
		{
			...
			int vernWs = StringUtils.GetWsAtOffset(tssForm, 0);
		...
		private LexEntryComponents BuildEntryComponentsDTO()
		{
			...
			entryComponents.GlossAlternatives.Add(StringUtils.MakeTss(Gloss, m_cache.DefaultAnalWs));
