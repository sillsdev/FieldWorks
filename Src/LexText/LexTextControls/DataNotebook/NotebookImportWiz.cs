...
namespace SIL.FieldWorks.LexText.Controls.DataNotebook
{
	...
	public partial class NotebookImportWiz : WizardDialog, IFwExtension
	{
		...
		/// </summary>
		private object ImportStdFmtFile(IProgress progressDlg, object[] parameters)
		{
			int lineNumber = 0;
		...
		internal static void InitializeWritingSystemCombo(string sWs, FdoCache cache, ComboBox cbWritingSystem)
		{
			if (String.IsNullOrEmpty(sWs))
				sWs = cache.WritingSystemFactory.GetStrFromWs(cache.DefaultAnalWs);
			cbWritingSystem.Items.Clear();
			cbWritingSystem.Sorted = true;
			cbWritingSystem.Items.AddRange(cache.ServiceLocator.WritingSystems.AllWritingSystems.ToArray());
