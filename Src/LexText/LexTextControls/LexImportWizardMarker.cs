...
using SIL.FieldWorks.LexText.Controls;	 // for ContentMapping
...
{
	/// <summary>
	/// Summary description for LexImportWizardMarker.
	/// </summary>
...
//				return cbLangDesc.SelectedItem.ToString();
		...
		public string FWDestID
		{
			get
			{
				...
				if (tvDestination.SelectedNode == null)
					return "";
				return (tvDestination.SelectedNode.Tag as Sfm2Xml.LexImportField).ID;
			}
		}
		...
		public string FWDestinationClass
		{
			get
			{
				...
				if (tvDestination.SelectedNode == null)
					return "";
				return tvDestination.SelectedNode.Parent.Text.Trim(new char[] {'(',')'});
			}
		}
		...
		public bool IsCustomField
		{
			get
			{
				...
				if (tvDestination.SelectedNode == null)
					return false;
				return (tvDestination.SelectedNode.Tag is Sfm2Xml.LexImportCustomField);//.IsCustomField;
			}
		}
...
//				tnode.Tag = className;
		...
		private void chkbxExclude_CheckedChanged(object sender, System.EventArgs e)
		{
		...
		private void btnAddLangDesc_Click(object sender, System.EventArgs e)
		{
		...
		private void tvDestination_AfterSelect(object sender, System.Windows.Forms.TreeViewEventArgs e)
		{
			UpdateOKButtonState();
		...
		private void FillLexicalRefTypesCombo(Sfm2Xml.LexImportField field)
		{
			...
			if (field.IsRef)
			{
					...
					foreach (ILexRefType lrt in m_cache.LangProject.LexDbOA.ReferencesOA.PossibilitiesOS)
					{
						switch (lrt.MappingType)
						{
							...
							case (int)LexRefType.MappingTypes.kmtEntryOrSenseSequence:
								//abbr = lrt.Abbreviation.AnalysisDefaultWritingSystem;
								//name = lrt.Name.AnalysisDefaultWritingSystem;
							...
							case (int)LexRefType.MappingTypes.kmtEntryOrSenseTree:
								//abbr = lrt.Abbreviation.AnalysisDefaultWritingSystem;
								//name = lrt.Name.AnalysisDefaultWritingSystem;
								//reverseAbbr = lrt.ReverseAbbreviation.AnalysisDefaultWritingSystem;
								//reverseName = lrt.ReverseName.AnalysisDefaultWritingSystem;
				...
				else if (field.ID == "sub")
				{
					...
					// fill the comboBox with the names of the Variant objects
					foreach (ILexEntryType let in m_cache.LangProject.LexDbOA.ComplexEntryTypesOA.ReallyReallyAllPossibilities)
					{
						AddAbbrAndNameInfo(let.Abbreviation, let.Name, null, null);
						//int wsActual;
						//ITsString tssAnal = let.Name.GetAlternativeOrBestTss(m_cache.DefaultAnalWs, out wsActual);
...
//					btnOK.StyleChanged
		...
		private void AddCustomFieldsToPossibleFields()
		{
			...
			foreach (TreeNode classNameNode in tvDestination.Nodes)
			{
				...
				TreeNodeCollection leafNodes =  classNameNode.Nodes;
