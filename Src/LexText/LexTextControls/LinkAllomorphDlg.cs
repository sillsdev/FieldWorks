...
{
	public class LinkAllomorphDlg : BaseEntryGoDlg
	{
		#region Data members

		private System.Windows.Forms.Label label3;
		private SIL.FieldWorks.Common.Widgets.FwComboBox m_fwcbAllomorphs;
		...
		protected override WindowParams DefaultWindowParams
		{
			get
			{
				...
				wp.m_title = LexText.Controls.LexTextControls.ksChooseAllomorph;
		...
		public override int SelectedID
		{
			...
			{
				CheckDisposed();
				return (m_fwcbAllomorphs.SelectedItem as LAllomorph).HVO;
			}
		}
		...
		/// <param name="e"></param>
		protected void Form_TextChanged(object sender, System.EventArgs e)
		{
			if (m_tbForm.Text.Length == 0)
			{
				this.m_fwcbAllomorphs.Text = "";		// clear the text box
				this.m_fwcbAllomorphs.Items.Clear();	// clear the drop down list box
			}
		}
		...
		public LinkAllomorphDlg()
		{
			...
			this.SetHelpTopic("hktpInsertAllomorphChooseAllomorph");

			m_tbForm.TextChanged += new EventHandler(Form_TextChanged);			// erase when needed
