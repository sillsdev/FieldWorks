...
// LexImportWizard.cs(1875,19): 'System.Xml.Xsl.XslTransform' is obsolete: 'This class has been deprecated. Please use System.Xml.Xsl.XslCompiledTransform instead.
// LexImportWizard.cs(1875,58):
...
namespace SIL.FieldWorks.LexText.Controls
{
	public class LexImportWizard : WizardDialog, IFwExtension
	{
		...
		private XCore.Mediator m_mediator = null;
		...
		private string GetFile(OFType fileType, string currentFile)
		{
			openFileDialog.Filter = ResourceHelper.BuildFileFilter(new FileFilterType[] {
				FileFilterType.ImportMapping, FileFilterType.XML,
...
//			m_languages = langs;	// store in this converter for future IFM use
			...
			foreach (DictionaryEntry languageEntry in langs)
			{
				...
				if (fwName == null)
				{
					if (xmlLang == "Vernacular" && m_cache.DefaultVernWs != 0)
					{
						ILgWritingSystem lws = LgWritingSystem.CreateFromDBObject(m_cache, m_cache.DefaultVernWs);
				...
				if (encodingconverter == null || encodingconverter.Length == 0)
		...
		private void SetDatabaseNameIntoLabel()
		{
			...
			lblMappingLanguagesInstructions.Text = string.Format(lblText, m_cache.DatabaseName);
		}
		...
		public bool AddInLineMarker(Sfm2Xml.ClsInFieldMarker marker, bool makeSelected)
		{
			...
			{
				System.Diagnostics.Debug.WriteLine("Found a matching IFM entry, not adding again.  (" + marker.ElementName + ")");
			}
			return true;
		}

		private void listViewMappingLanguages_SelectedIndexChanged(object sender, System.EventArgs e)
		{
		...
		private ListViewItem CreateLanguageMappingItem(string langDesc, string ws, string ec, string icu)
		{
		...
		public bool AddLanguage(string langDesc, string ws, string ec, string icu)
		{
			CheckDisposed();

			// now put the lang info into the language list view
			ListViewItem lvItem = CreateLanguageMappingItem(langDesc, ws, ec, icu);
			listViewMappingLanguages.Items.Add(lvItem);

			return true;
		}

		private void btnAddMappingLanguage_Click(object sender, System.EventArgs e)
		{
			// get list of current Language descriptor values
			Hashtable langDescs = GetUILanguages();

			LexImportWizardLanguage dlg = new LexImportWizardLanguage(m_cache, langDescs);
		...
		private void btnModifyMappingLanguage_Click(object sender, System.EventArgs e)
		{
			// get list of current Language descriptor values
			...
			if (dlg.ShowDialog(this) == DialogResult.OK)
			{
				...
				listViewMappingLanguages.Items[selectedIndex] = CreateLanguageMappingItem(langDesc, ws, ec, icu);
				...
				if (m_MappingMgr != null)
				{
					...
					foreach(DictionaryEntry markerEntry in markers)
					{
						...
						{
							info.UpdateLangaugeValues(ws,icu,langDesc);
							anyUpdated = true;
						}
					}
					if (anyUpdated)
		...
		private void listViewMappingLanguages_ColumnClick(object sender, System.Windows.Forms.ColumnClickEventArgs e)
		{

		}

		private string ConvertNameFromICUtoFW(string icuName)
		{
			...
			int wsID = wsf.GetWsFromStr(icuName);
			IWritingSystem ws = wsf.get_EngineOrNull(wsID);
			if (ws != null)
				return ws.get_UiName(wsUser);
...
//				int sfmCOUNTTEST = testReader.GetSFMCount(sfmKEY);
		...
		private void btnModifyContentMapping_Click(object sender, System.EventArgs e)
		{
			...
			LexImportWizardMarker dlg = new LexImportWizardMarker(m_LexFields);
			dlg.Init(contentMapping, langDescs, m_cache);
...
//				contentMapping = listViewContentMapping.Items[selIndex].Tag as MarkerPresenter.ContentMapping;
				...
				if (!contentMapping.Exclude)
				{
					...
					if (!contentMapping.AutoImport)	// auto import only allows lang so skip the following
					{
						...
						//contentMapping.RefField = dlg.FuncField;
						//contentMapping.RefFieldWS = dlg.FuncFieldWS;
						Sfm2Xml.ClsFieldDescription xyz = contentMapping.LexImportField.ClsFieldDescriptionWith(contentMapping.ClsFieldDescription);
					}
					else
					{
...
//					if (xmlLang == "Vernacular" && m_cache.DefaultVernWs != 0)
//					{
//						LgWritingSystem lws = LgWritingSystem.CreateFromDBObject(m_cache, m_cache.DefaultVernWs);
//						fwName = lws.ShortName;
//						xmlLang = lang.XmlLang = lws.ICULocale;
//						encodingconverter = lws.LegacyMapping;
//					}
...
//			data.RefFunc = info.RefField;
		...
		private void SaveNewMapFile()
		{
			...
			{
				m_SaveAsFileName.Text = m_sTempDir + m_cache.DatabaseName + ".map";
			}
		...
		private class FlexConverter : Sfm2Xml.Converter
		{
			...
			{
				int clsid;
				IMoMorphType mmt = MoMorphType.FindMorphType(m_cache, m_rgmmt, ref sForm,
					out clsid);
				sAlloClass = m_cache.GetClassName((uint)clsid);
				int ws;
				ITsString tss = mmt.Name.GetAlternativeOrBestTss(m_wsEn, out ws);
				if (ws == m_wsEn)
					sMorphTypeWs = "en";
				else
					sMorphTypeWs = m_cache.LanguageWritingSystemFactoryAccessor.GetStrFromWs(ws);
				return tss.Text;
		...
		private string GetTempDir()
		{
			...
			if (!sTempDir.EndsWith("\\"))
				sTempDir += "\\";
			sTempDir += "Language Explorer\\";
			return sTempDir;
		}
		...
		private void InitOutputFiles()
		{
			...
			if (!sRootDir.EndsWith("\\"))
				sRootDir += "\\";
			sTransformDir = sRootDir + "Language Explorer\\Import\\";
			// Use a FieldWorks specific temp directory.
		...
		private bool FindDBSettingsSaved(string dbToImport, out string settings, out string saveAs)
		{
			...
			RegistryKey key = Registry.CurrentUser.OpenSubKey(@"Software\SIL\FieldWorks\Language Explorer\ImportFile" + dbHash);
			if (key != null)
			{
				retVal = true;
				settings = key.GetValue("Settings","")as string;
				saveAs = key.GetValue("SaveAs", "")as string;
			}
			return retVal;
		}
		...
		private bool GetLastImportFile(out string fileName)
		{
			...
			RegistryKey key = Registry.CurrentUser.OpenSubKey(@"Software\SIL\FieldWorks\Language Explorer");
			if (key != null)
			{
		...
		/// <param name="dbToImport"></param>
		/// <returns></returns>
		private bool FindFilesForDatabaseFile(string dbToImport)
		{
			bool retVal = false;
			...
			if (key != null)
			{
				string settings = key.GetValue("Settings") as string;
				string saveAs = key.GetValue("SaveAs") as string;
				if (settings != null)
				{
					m_SettingsFileName.Text = settings;
					retVal = true;
		...
		private void SaveSettings()
		{
			...
			if (dbImportName != "")	// has value to save
			{
				RegistryKey key = Registry.CurrentUser.CreateSubKey(@"Software\SIL\FieldWorks\Language Explorer");
				if (key == null)
					return;
				...
				key = Registry.CurrentUser.CreateSubKey(@"Software\SIL\FieldWorks\Language Explorer\ImportDictFiles");
				...
				key = Registry.CurrentUser.CreateSubKey(@"Software\SIL\FieldWorks\Language Explorer\ImportFile" + dbHash);
				if (key != null)
				{
		...
		protected override void OnCancelButton()
		{
			...
			if (m_dirtySenseLastSave || (Control.ModifierKeys & Keys.Shift) == Keys.Shift)
			{
				...
				if (result == DialogResult.Yes)
				{
					...
					{
						bool continueToFinish = EnableNextButton();
						m_CurrentStepNumber++;
					}
					SaveSettings();
				}
...
//					m_dirtyMapFile = false;
			...
			case 3:
				string dbName = m_cache.DatabaseName;
				...
				if (m_dirtyInputFile || m_dirtyMapFile)
				{
					...
					m_MappingMgr = new MarkerPresenter( /*m_cache,*/SIL.FieldWorks.Common.Utils.DirectoryFinder.FWCodeDirectory,
			...
			else if (m_SaveAsFileName.Text != m_SettingsFileName.Text)
			{
				try
				{
					...
					if (!fi.Exists)
					{
						...
						System.IO.FileStream s2 = new System.IO.FileStream(
		...
		private void btnBackup_Click(object sender, System.EventArgs e)
		{
			...
			//			if (nBkResult == 2)
			//				// successful backup, continue with import
			//			else if (nBkResult == 1)
			//				// user cancelled the backup, continue with import
			//			else
			//				// backup failure or any other backup/restore result, quit the import
		}
		...
		private void btnQuickFinish_Click(object sender, System.EventArgs e)
		{
			...
			if (AllowQuickFinishButton())
			{
				...
				while (CurrentStepNumber <= 6)
				{
					...
					////if (!continueToFinish)
					////{
					////    tabSteps.SelectedIndex = m_CurrentStepNumber;
					////    NextButtonEnabled = false;// EnableNextButton();
					////    btnQuickFinish.Visible = false;
					////    UpdateStepLabel();
					////    btnQuickFinish.Visible = false;
		...
		protected override void OnSizeChanged(EventArgs e)
		{
			...
			else
			{
				if (m_origSet == false && Width != 0 && m_origWidth != 0)	// adjust sizes
				{
					...
					MoveButton(ref btnAddMappingLanguage, diffWidth, diffHeight);
					MoveButton(ref btnModifyMappingLanguage, diffWidth, diffHeight);
					MoveButton(ref btnModifyContentMapping, diffWidth, diffHeight);
					MoveButton(ref btnAddCharMapping, diffWidth, diffHeight);
					MoveButton(ref btnModifyCharMapping, diffWidth, diffHeight);
					MoveButton(ref btnDeleteCharMapping, diffWidth, diffHeight);
		...
		private void MoveButton(ref Button btn, int dw, int dh)
		{
		...
		{
			LexImportWizardCharMarkerDlg dlg = new LexImportWizardCharMarkerDlg();
		...
		internal void GetNewMediator()
		{
			...
			if (app == null)
				return;
			Form wndActive = app.ActiveMainWindow;
			if (wndActive == null)
				return;
			PropertyInfo pi = wndActive.GetType().GetProperty("Mediator");
			if (pi == null)
				return;
			m_mediator = pi.GetValue(wndActive, null) as XCore.Mediator;
		}
