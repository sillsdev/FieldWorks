...
{
	/// <summary>
	/// Which feature system a create-feature flow targets -- the inflection (morphosyntactic)
	/// feature system
	...
	/// <summary>
	/// The LCModel-aware launcher for the "Create a new feature" / "Add a value to a feature"
	/// flows -- the New-UI
	...
	/// (<see cref="CreateFeatureDialogViewModel"/>), and on OK creates the feature/value in the
	/// feature system in ONE
	...
	/// * Inflection feature: create an <c>IFsClosedFeature</c>, add it to
	/// <c>MsFeatureSystemOA.FeaturesOC</c>,
	/// ensure an "Infl" <c>IFsFeatStrucType</c> exists (labelled across the analysis writing
	/// systems), and add the
	/// feature to that type's <c>FeaturesRS</c> -- exactly
	/// <c>MasterInflectionFeatureListDlg.linkLabel1_LinkClicked</c>.
	///   * Phonological feature: create an <c>IFsClosedFeature</c>, add it to <c>PhFeatureSystemOA.FeaturesOC</c>, and
	/// create the two default symbolic values <c>SimpleInit("+","positive")</c> /
	/// <c>SimpleInit("-","negative")</c>
	...
	/// name -> new-feature/value + node round-trip is unit-testable against a real cache (via
	/// InternalsVisibleTo)
