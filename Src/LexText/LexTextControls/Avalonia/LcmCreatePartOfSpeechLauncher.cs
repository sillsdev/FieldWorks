...
{
	/// <summary>
	/// The LCModel-aware launcher for the "Create a new Part of Speech" flow -- the New-UI
	/// replacement for the
	...
	/// the chosen master category is added to <c>cache.LangProject.PartsOfSpeechOA</c> (or under
	/// its catalog parent)
	/// via <see cref="MasterCategory.AddToDatabase"/> -- which creates the <c>IPartOfSpeech</c>
	/// with the catalog's
	/// fixed guid + name/abbr/description + CatalogSourceId, in ONE undoable step -- and returns
	/// a fresh
	/// <see cref="FwPosNode"/> for it (guid id + name + abbr) for the requesting chooser to add + select.
	///
	/// Layering mirrors <see cref="LcmChooserDialogLauncher"/>: BuildState / Apply are internal
	/// so the chosen-id ->
		...
		: AvaloniaDialogLauncher<ChooserDialogInput, ChooserDialogViewModel, LcmCreatePartOfSpeechLauncher.CreatePosPayload>
	{
		...
		/// category (a create flow must choose something). Internal so the catalog -> candidates
		/// mapping is testable.
		/// </summary>
		...
		/// <see cref="MasterCategoryListDlg"/>'s OK logic: the chosen <see
		/// cref="MasterCategory"/> is added to the
		/// project's parts-of-speech list (under its catalog parent when the parent is itself a category) via
		/// <see cref="MasterCategory.AddToDatabase"/> -- which creates the POS with the catalog's
		/// fixed guid, populates
		...
		/// no-ops). Returns null for a missing/unparsable chosen id. Internal so the chosen-id ->
		/// IPartOfSpeech
		...
		/// <see cref="MasterCategoryListDlg.AddNodes"/>/<c>AddNode</c>: each <c>item</c> becomes
		/// a
		/// <see cref="CatalogCategory"/> wrapping a <see cref="MasterCategory"/> built against the project's existing
		/// POSes (so an already-installed category resolves to its existing POS), the top
		/// <c>PartOfSpeechValue</c>
		...
		internal static IReadOnlyList<CatalogCategory> BuildCatalog(LcmCache cache, XmlNode root)
		{
			...
			// already installed (and resolve it to its existing POS) -- exactly as the WinForms
			// dialog seeds it.
			...
			{
				// The top-level grouping node is not a real category -- descend through it
				// (legacy AddNode behavior).
