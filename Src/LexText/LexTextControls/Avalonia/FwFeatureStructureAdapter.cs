...
namespace SIL.FieldWorks.LexText.Controls
{
	...
	/// <c>IFsFeatStruc</c>, and -- on commit -- REBUILDS the nested <c>IFsFeatStruc</c> from the
	/// editor's flat
	...
	/// <c>IFsFeatStruc</c>) and the closed feature, then set the closed value's <c>ValueRA</c> --
	/// setting
	...
	internal static class FwFeatureStructureAdapter
	{
		...
		/// <summary>
		/// Builds the inflection-feature SYSTEM for an MSA's part of speech (and its owning
		/// parent POSes, the legacy
		/// <c>PopulateTreeFromPos</c> walk) as a flat, document-order, depth-tagged <see cref="FwFeatureNode"/> list:
		/// a closed feature -> a Closed node + its <c>ValuesSorted</c> Value children; a complex
		/// feature -> a Complex
		...
		// is the guid path from the root to (not including) this node -- the AlreadyInTree cycle
		// guard.
		...
		/// <paramref name="nodes"/> -- the SAME depth-tagged node list the editor was fed -- to
		/// recover each closed
			...
			IReadOnlyList<FwFeatureNode> nodes, IReadOnlyList<FwFeatureValueAssignment> assignments)
		{
			...
			foreach (var assignment in assignments)
			{
				...
				// Descend through the complex ancestors (outermost first), find-or-creating each
				// nested FS -- the
		...
		/// result is empty, deletes the FS (the inflection LT-13596 path) -- the phonological
		/// dialog passes false (it
		...
		/// <summary>
		/// Returns the part of speech whose inflectable features apply to a morpheme MSA -- the
		/// lift of
		...
		/// inflection <c>IFsFeatStruc</c> -- the create-side parity of
		/// <c>MsaInflectionFeatureListDlg_Closing</c>'s OK
