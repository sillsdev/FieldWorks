	...
	public class LiftExportTests : MemoryOnlyBackendProviderRestoredForEachTestTestBase
	{
		...
		private void VerifyExport(XmlDocument xdoc)
		{
			...
			foreach (XmlNode xentry in entries)
			{
				...
				Assert.That(delta.TotalSeconds, Is.GreaterThan(300));
				Assert.That(delta.TotalSeconds, Is.LessThanOrEqualTo(0));	// allow time for breakpoints in debugging...
				...
				Assert.That(delta.TotalSeconds, Is.GreaterThan(300));
				Assert.That(delta.TotalSeconds, Is.LessThanOrEqualTo(0));
