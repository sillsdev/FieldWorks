...
namespace LexTextControlsTests
{
	...
	public class AvaloniaOptionsDialogLauncherTests
	{
		...
		public void ShouldUseAvaloniaUI_OnlyTrueForNew(string mode, bool expected)
		{
			Assert.That(SIL.FieldWorks.Common.FwUtils.UIModeGates.ShouldUseAvaloniaUI(mode),
				Is.EqualTo(expected));
		}
		...
		public void LoadPluginManagerDoc_WithDtd_IsRejected_NotResolved()
		{
			...
			// throws XmlException internally (returned as null) -- never resolves an external
			// entity.
			var path = WriteTemp(
