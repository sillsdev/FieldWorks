...
namespace LexTextControlsTests
{
	...
	public partial class LiftMergerTests : MemoryOnlyBackendProviderRestoredForEachTestTestBase
	{
		...
		public void TestLiftImport_PronunciationLanguageAddedToPronunciationAndVernacularLists()
		{
			...
			var repoEntry = Cache.ServiceLocator.GetInstance<ILexEntryRepository>();
			Assert.That(repoEntry.Count, Is.EqualTo(0));
			Assert.That(0, Is.EqualTo(Cache.LangProject.CurrentPronunciationWritingSystems.Count));
			Assert.That(1, Is.EqualTo(Cache.LangProject.VernacularWritingSystems.Count));
			...
			File.Delete(logFile);
			Assert.That(repoEntry.Count, Is.EqualTo(1));
			Assert.That(1, Is.EqualTo(Cache.LangProject.CurrentPronunciationWritingSystems.Count), "IPA from pronunciation was not added to pronunciation writing systems");
			Assert.That(2, Is.EqualTo(Cache.LangProject.VernacularWritingSystems.Count), "IPA from pronunciation was not added to vernacular writing systems");
		}
