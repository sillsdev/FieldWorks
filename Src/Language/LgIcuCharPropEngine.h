...
/*----------------------------------------------------------------------------------------------
	...
	Note that we do not implement this interface as part of LgIcuCharPropEngine, because then the engine
...
----------------------------------------------------------------------------------------------*/
class CharPropEngineIcuCleanupCallback : public IIcuCleanupCallback
{
	friend class LgIcuCharPropEngine;
public:
	CharPropEngineIcuCleanupCallback(LgIcuCharPropEngine * pcpe);
	virtual ~CharPropEngineIcuCleanupCallback();
...
protected:
	long m_cref;
	LgIcuCharPropEngine * m_pcpe;
};

DEFINE_COM_PTR(CharPropEngineIcuCleanupCallback);

class LgIcuCharPropEngine :
	public ILgCharacterPropertyEngine,
	public ISimpleInit
{
	friend class CharPropEngineIcuCleanupCallback; // allowed to clean up our break iterator
public:
	//:> Static methods
	...
	void Initialize(BSTR bstrLanguage, BSTR bstrScript, BSTR bstrCountry, BSTR bstrVariant);
...
//	STDMETHOD(NormalizeKd)(BSTR bstr, BSTR * pbstr); //**
	...
	void InitCharOverrides(StrUni wsCharsList);
...
protected:
	...
	CharPropEngineIcuCleanupCallbackPtr m_qCleanupCallback;
