...
LgIcuCollator::LgIcuCollator()
{
	...
	// Ensure this is done at least once before anything else.
...
void LgIcuCollator::EnsureCollator()
{
	...
	Assert(!m_qcicc); // should be created and cleared in parallel.
	...
	m_qcicc.Attach(new CollatorIcuCleanupCallback(this));
	CheckHr(qicln->RegisterCleanupCallback(m_qcicc));
}
...
STDMETHODIMP LgIcuCollator::Close()
{
	...
	{
		DoneCleanup();
		// We have to cleanup to release ICU's memory map. Otherwise we lock all other FW
		// apps that might want to create/modify a writing system.
		IIcuCleanupManagerPtr qicln;
		qicln.CreateInstance(CLSID_IcuCleanupManager);
...
/*----------------------------------------------------------------------------------------------
	In case someone else has shut down ICU, our collator is no longer useable, so discard it.
----------------------------------------------------------------------------------------------*/
void LgIcuCollator::DoneCleanup()
{
	if (m_pCollator)
	{
		Assert(m_qcicc); // create and destroy in parallel.
		delete m_pCollator;
		m_pCollator = NULL;
		IIcuCleanupManagerPtr qicln;
		qicln.CreateInstance(CLSID_IcuCleanupManager);
		CheckHr(qicln->UnregisterCleanupCallback(m_qcicc));
		m_qcicc.Clear();
	}
}
...
/*----------------------------------------------------------------------------------------------
	Constructor.
----------------------------------------------------------------------------------------------*/
CollatorIcuCleanupCallback::CollatorIcuCleanupCallback(LgIcuCollator * pico)
{
	m_cref = 1;
	ModuleEntry::ModuleAddRef();
	m_pico = pico;
...
/*----------------------------------------------------------------------------------------------
	Destructor.
----------------------------------------------------------------------------------------------*/
CollatorIcuCleanupCallback::~CollatorIcuCleanupCallback()
{
	ModuleEntry::ModuleRelease();
}
//:>--------------------------------------------------------------------------------------------
//:>	IUnknown Methods
//:>--------------------------------------------------------------------------------------------
STDMETHODIMP CollatorIcuCleanupCallback::QueryInterface(REFIID riid, void **ppv)
{
	AssertPtr(ppv);
	if (!ppv)
		return WarnHr(E_POINTER);
	*ppv = NULL;
	...
	else if (riid == IID_IIcuCleanupCallback)
		*ppv = static_cast<IIcuCleanupCallback *>(this);
	else if (riid == IID_ISupportErrorInfo)
	{
		*ppv = NewObj CSupportErrorInfo(this, IID_IIcuCleanupCallback);
		return S_OK;
	}
	else
		return E_NOINTERFACE;
...
STDMETHODIMP CollatorIcuCleanupCallback::DoneCleanup()
{
	BEGIN_COM_METHOD;
	if (m_pico)
		m_pico->DoneCleanup();
	END_COM_METHOD(g_fact, IID_IIcuCleanupCallback);
}