...
Description:
	...
	Using ICU v.2.1, implements Unicode 3.1.
-------------------------------------------------------------------------------*//*:End Ignore*/
...
LgIcuCharPropEngine::LgIcuCharPropEngine()
{
	...
	Initialize(stuUserWs.Bstr(), NULL, NULL, NULL);
}
...
LgIcuCharPropEngine::LgIcuCharPropEngine(BSTR bstrLanguage, BSTR bstrScript, BSTR bstrCountry, BSTR bstrVariant)
{
	...
	m_pLocale = NULL;
	m_pBrkit = NULL;
	Initialize(bstrLanguage, bstrScript, bstrCountry, bstrVariant);
}
...
void LgIcuCharPropEngine::SetupBreakIterator()
{
	...
	m_qCleanupCallback.Attach(new CharPropEngineIcuCleanupCallback(this));
...
----------------------------------------------------------------------------------------------*/
void LgIcuCharPropEngine::Initialize(BSTR bstrLanguage, BSTR bstrScript, BSTR bstrCountry, BSTR bstrVariant)
{
	StrUtil::InitIcuDataDir();

	StrAnsi staLanguage(bstrLanguage);
	StrAnsi staScript(bstrScript);
	StrAnsi staCountry(bstrCountry);
	StrAnsi staVariant(bstrVariant);
	Assert(staLanguage.Length());
	m_pLocale = new Locale(staLanguage.Chars(), staScript.Chars(), staCountry.Chars(), staVariant.Chars());
...
static GenericFactory g_fact(
	_T("SIL.Language.LgIcuCharPropEngine"),
...
STDMETHODIMP LgIcuCharPropEngine::InitNew(const BYTE * prgb, int cb)
{
	...
	if (cb != 0)
		ThrowInternalError(E_INVALIDARG, L"Expected empty init string");
...
STDMETHODIMP LgIcuCharPropEngine::get_SoundAlikeKey(BSTR bstrValue, BSTR * pbstrKey)
{
	...
	*pbstrKey = NULL;
	...
	Assert(false);
	ThrowInternalError(E_NOTIMPL, L"SoundAlikeKey");
	...
	int ichMin, int ichLim, byte * prglbsOut, int * pichBreak)
{
	...
	*pichBreak = -1;	// This will be set to something else if we come to a "break/stop".
	...
	for (ich = 0; ich < cb; ++ich)
	{
		...
		*prglbp = (byte)g_rglbp[u_getIntPropertyValue(ch, UCHAR_LINE_BREAK)];
		if (GenCategory(ch) == kccZs)
			*prglbp |= 0x80;
		if (ch == kchTAB)
...
/*----------------------------------------------------------------------------------------------
	...
	// Now that the break iterator is cleared, if we call u_cleanup() it should release the
	// memory mapping grip on icudt28l_uprops.icu
----------------------------------------------------------------------------------------------*/
void LgIcuCharPropEngine::CleanupBreakIterator()
{
	if (m_qCleanupCallback)
	{
		...
		m_qCleanupCallback->m_pcpe = NULL;
...
/*----------------------------------------------------------------------------------------------
	Constructor.
----------------------------------------------------------------------------------------------*/
CharPropEngineIcuCleanupCallback::CharPropEngineIcuCleanupCallback(LgIcuCharPropEngine * pcpe)
{
	m_cref = 1;
	ModuleEntry::ModuleAddRef();
	m_pcpe = pcpe;
...
/*----------------------------------------------------------------------------------------------
	Destructor.
----------------------------------------------------------------------------------------------*/
CharPropEngineIcuCleanupCallback::~CharPropEngineIcuCleanupCallback()
{
	ModuleEntry::ModuleRelease();
}
//:>--------------------------------------------------------------------------------------------
//:>	IUnknown Methods
//:>--------------------------------------------------------------------------------------------
STDMETHODIMP CharPropEngineIcuCleanupCallback::QueryInterface(REFIID riid, void **ppv)
{
	AssertPtr(ppv);
	if (!ppv)
		return WarnHr(E_POINTER);
...
STDMETHODIMP CharPropEngineIcuCleanupCallback::DoneCleanup()
{
	if (m_pcpe)
		m_pcpe->CleanupBreakIterator();
	return S_OK;
}
...
void LgIcuCharPropEngine::InitCharOverrides(StrUni wsCharsList)
{
