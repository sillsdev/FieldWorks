		...
		int * pichLim)
	{
		...
		if (ich < 1000)
		{
			if (m_vws.Size())
				...
				pchrp->ws = 0;
			wcscpy_s(pchrp->szFaceName, L"Lucida Console");
			*pichMin = 0;
		...
		else if (ich < 2000)
		{
			if (m_vws.Size() > 1)
				...
				pchrp->ws = kwsTest;
			wcscpy_s(pchrp->szFaceName, L"Lucida Sans Unicode");
			*pichMin = 1000;
		...
		else
		{
			if (m_vws.Size() > 2)
				...
				pchrp->ws = kwsTest2;
			wcscpy_s(pchrp->szFaceName, L"Times New Roman");
			*pichMin = 2000;
...
//			unitpp::assert_eq("get_InitializationData(NULL); HRESULT", E_POINTER, hr);
		...
		virtual IRenderEnginePtr GetRenderer(LgCharRenderProps* pchrp)
		{
			IRenderEnginePtr qreneng;
			m_qwsf->get_RendererFromChrp(pchrp, &qreneng);
			return qreneng;
		}
		...
		int VerifyBreakPoints(TxtSrc* ts, uint dxMax, int dichLimSeg, IVwGraphics* pvg)
		{
			...
			for (ichMin = 0; ichMin < cch; ichMin += dichLimSeg)
			{
				...
				IRenderEnginePtr qreneng = GetRenderer(&chrp);
				if (!qreneng)
				{
					unitpp::assert_true("VerifyBreakPoints - GetRenderer", chrp.ws != kwsEng);
...
//						ichMin, dichLimSeg, dxWidth);
	...
	public:
		virtual void Setup()
		{
			unitpp::assert_true("Render engine not created", m_qre);
			// These are created in the subclass.
			//ILgWritingSystemFactoryPtr qwsf;	// needed as data for m_qre.
			//LgWritingSystemFactory::CreateCom(NULL, IID_ILgWritingSystemFactory,
			//	(void **)&m_qwsf);
			//m_qre->putref_WritingSystemFactory(m_qwsf);
			//// Serif;Sans Serif;Monospace
			//StrUni stu(L"Times New Roman;Century Gothic;Lucida Console");
			//ISimpleInit* pre = dynamic_cast<ISimpleInit*>(m_qre.Ptr());
			//unitpp::assert_true("Render engine doesn't implement ISimpleInit", pre);
			//pre->InitNew(reinterpret_cast<const BYTE *>(stu.Chars()),
			//	stu.Length() * isizeof(wchar));
		}
		virtual void Teardown()
		{
			//m_qwsf->Shutdown(); // This happens in the subclass.
			m_qwsf.Clear();
			m_qre.Clear();
		}
	};
