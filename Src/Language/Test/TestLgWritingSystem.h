...
#else
			unitpp::assert_true("set and get font var", u_strcmp(sbstr2.Chars(), StrUni(L"Dummy").Chars()) == 0);
#endif
...
#else
			unitpp::assert_true("set and get sans font var",
				u_strcmp(sbstr2.Chars(), StrUni(L"Dummy").Chars()) == 0);
#endif
		}
...
#else
			unitpp::assert_true("get def serif, not set",
				u_strcmp(sbstr.Chars(), StrUni(L"Times New Roman").Chars()) == 0);
#endif
...
#else
			unitpp::assert_true("set and get def serif",
				u_strcmp(sbstr2.Chars(), StrUni(L"TIMES NEW ROMAN").Chars()) == 0);
#endif
		}
...
#else
			unitpp::assert_true("get def sans serif, not set",
				u_strcmp(sbstr.Chars(), StrUni(L"Arial").Chars()) == 0);
#endif
			sbstr = L"ARIAL";
...
#else
			unitpp::assert_true("set and get def sans serif",
				u_strcmp(sbstr2.Chars(), StrUni(L"ARIAL").Chars()) == 0);
#endif
		}
...
#else
			unitpp::assert_true("get def body font, not set",
				u_strcmp(sbstr.Chars(), StrUni(L"Charis SIL").Chars()) == 0);
#endif
			sbstr = L"CHARIS";
...
#else
			unitpp::assert_true("set and get body font",
				u_strcmp(sbstr2.Chars(), StrUni(L"CHARIS").Chars()) == 0);
#endif
		}
...
#else
			unitpp::assert_true("get def Monospace, not set",
				u_strcmp(sbstr.Chars(), StrUni(L"Courier").Chars()) == 0);
#endif
			sbstr = L"COURIER";
...
#else
			unitpp::assert_true("set and get def COURIER",
				u_strcmp(sbstr2.Chars(), StrUni(L"COURIER").Chars()) == 0);
#endif
		}
...
#else
			unitpp::assert_true("get_UiName, return English",
				!u_strcmp(sbstrName.Chars(), StrUni(L"English").Chars()));
#endif
			sbstr = L"MyWs";
...
#else
			unitpp::assert_true("get_UiName", u_strcmp(sbstr.Chars(), StrUni(L"MyWs").Chars()) == 0);
#endif
...
//BAD		qcoll.CreateInstance(CLSID_Collation);
		...
		{
#ifdef WIN32
			// REVIEW: Lonely Test (xUnit Test Patterns, p 232): this test fails when run on its own
			// because we don't create kszTest ws. On Linux it fails always and seems to corrupt
			// some memory.
			...
			fstream testFile;
#endif
			stuFile.Format(L"%s/test.xml", stuLangDir.Chars());
#ifdef WIN32
...
#else
			testFile.open(StrAnsi(stuFile).Chars(), fstream::in|fstream::out);
			if (testFile.is_open())
			{
				testFile.close();
				fLDFExists = true;
...
#else
				::MoveFileW(m_vstuBackup[i].Chars(), m_vstuFile[i].Chars());
#endif
				m_vstuBackup.Clear();
				m_vstuFile.Clear();
			}
