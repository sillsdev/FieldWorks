...
//:>********************************************************************************************
static DummyFactory g_fact(_T("SIL.Language.RomRenderSeg"));
	...
	LgIpValidResult * pipvr)
{
	...
	*pipvr = kipvrOK;
	...
	OLECHAR rgch[2] = {0, 0};
	CheckHr(m_qts->Fetch(ichBase, ichBase + 1, rgch));
...
public:
	DrawIPBinder(int dydAscent, int ich, int ichMin)
	{
		m_dydAscent = dydAscent;
		m_ich = ich;
	...
	RECT rcSrc1, RECT rcDst1, int ich, ComBool fAssocPrev, ComBool fOn, LgIPDrawMode dm)
{
	...
	DrawIPBinder dipb(dydAscent, ich, ichBase);
...
public:
	DrawRangeBinder(int ichMin, int ichLim, int cchPrev)
	{
		m_ichMin = ichMin;
		m_ichLim = ichLim;
		m_cch = cchPrev;
	...
	ComBool fIsLastLineOfSelection, RECT * prsBounds, ComBool * pfAnythingToDraw)
{
	...
	*pfAnythingToDraw = true;
	...
	DrawRangeBinder drb(ichMin, ichLim, ichBase);
...
public:
	PointCharBinder(int xdClick, int ichBase)
	{
	...
	RECT rcSrc1, RECT rcDst, POINT ptdClickPosition, int * pich, ComBool * pfAssocPrev)
{
	...
	PointCharBinder pcb(ptdClickPosition.x, ichBase);
	...
	*pfResult = TRUE;
	if (fRight)
	{
		...
		else
		{
			...
			else
			{
				...
				// Moving right means we moved over the character to the left, or before, this.
				// We want to associate the selection with that character.
				*pfAssocPrev = true;
			}
		}
	}
	else // moving left
	{
		...
		else
		{
			...
			// Moving left means we moved over the character to the right, or after, this.
			// We want to associate the selection with that character.
			*pfAssocPrev = false;
		}
	}
	...
	int * prgxdLefts, int * prgxdRights, int * prgydUnderTops)
{
	...
	CPBinder cpb(cxdMax, prgxdLefts, prgxdRights, prgydUnderTops, ichMin - ichBase,
		ichLim - ichBase);
