...
//:>********************************************************************************************
static DummyFactory g_fact(_T("SIL.Language.UniscribeSeg"));
	...
	if (ich < ichBase + cch)
	{
		...
		else
		{
			...
			if (!m_qcpe)
			{
				if (m_qure)
				{
					// We can get a WritingSystemFactory so get the CharPropEngine from it
					ILgWritingSystemFactoryPtr qwsf;
					CheckHr(m_qure->get_WritingSystemFactory(&qwsf));
					CheckHr(qwsf->get_UnicodeCharProps(&m_qcpe));
				}
				else
				{
	...
	bool fRightToLeft)
{
	if (cch <= ich)
	{
		...
		CheckHr(pvg->GetTextExtent(1, L" ", &dxdWidth, &dydHeight));
			...
			if (uri.fLast)
			{
				...
				if (m_ichLimRun < m_ichLimWhiteSpace && !m_fParaRTL)
				{
					...
					else
					{
						...
						int dxdWidth, dydHeight;
						CheckHr(uri.pvg->GetTextExtent(1, L" ", &dxdWidth, &dydHeight));
...
template Vector<WORD>;
template Vector<SCRIPT_VISATTR>;
template Vector<int>;
template Vector<GOFFSET>;
template Vector<SCRIPT_ITEM>; // ScrItemVec; // Hungarian vscri;
template Vector<SCRIPT_LOGATTR>; // ScrLogAttrVec; // Hungarian vsla.

#include "HashMap_i.cpp"
template HashMap<LgCharRenderProps, SCRIPT_CACHE>;
