...
namespace Sfm2Xml
{
	...
	public class ClsLog
	{
		...
		public void FlushTo(ref System.Xml.XmlTextWriter xmlOutput)
		{
			xmlOutput.Flush();
			...
			OutputErrorElement(ref xmlOutput);					// errors
			OutputWarningElement(ref xmlOutput);				// warnings
			OutputOOOInfo(ref xmlOutput);						// out of order cautions
			OutputDuplicateSfmWarnErrElement(ref xmlOutput);	// duplicate SFM errors and warnings
			OutputSfmInfo(ref xmlOutput);						// general statistics on sfm usage
		...
		private void OutputErrorElement(ref System.Xml.XmlTextWriter xmlOutput)
		{
			// put out the errors
			int ttlErrors = m_FatalErrors.Count + m_Errors.Count;	// fatal and reg errors
...
//			foreach (string msg in m_FatalErrors)
		...
		private void OutputWarningElement(ref System.Xml.XmlTextWriter xmlOutput)
		{
			int ttlWarnings = m_Warnings.Count;// + m_sfmsWithOutData;	// warnings and sfm's w/o data
		...
		private void OutputDuplicateSfmWarnErrElement(ref System.Xml.XmlTextWriter xmlOutput)
		{
			/*
		...
		private void OutputSfmInfo(ref System.Xml.XmlTextWriter xmlOutput)
		{
		...
		//        </Caution>
		//    </OutOfOrder>
		private void OutputOOOInfo(ref System.Xml.XmlTextWriter xmlOutput)
		{
