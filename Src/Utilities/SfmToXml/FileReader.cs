...
namespace Sfm2Xml
{
	...
	public /*internal*/ class ByteReader
	{
		...
		private void Init(string filename, byte[] content)
		{
			...
			m_space = Converter.WideToMulti(" ", System.Text.Encoding.ASCII);
		...
		private byte[] GetBytesUptoNextSfmMarker()
		{
			...
			long returnBufSize = foundPos - m_position + 1;
			byte[] returnData = new byte[returnBufSize];

			long readPos = m_position;
			long writePos = 0;
			...
			byte[] sizeAdjustedReturnData = new byte[writePos];
