...
//				}
							...
							string vernWs, string regWs, string natWs)
		{
			...
			System.Xml.XmlTextWriter xmlOutput = new System.Xml.XmlTextWriter(m_OutputFileName, System.Text.Encoding.UTF8);
...
//			private IEncConverter m_iconverter;
			...
			System.Text.Encoding encodingToUse, out MultiToWideError err, out byte[] badBytes)
		{
			...
			try
			{
				...
				int charsDecodedCount = encoding.GetChars(multi, start, end - start + 1, chars, 0);
		...
		class ClsStringToOrFromBytes
		{
			...
			public string String()
			{
				if (m_String == null)
				{
					...
					Char[] chars = new Char[charCount];
					int charsDecodedCount = utf8Decoder.GetChars(m_Data, 0, m_ByteLength, chars, 0);
...
//					RemoveObjFromOpenList(child);
			...
			public bool AddNewEntry(ClsHierarchyEntry newHierarchy, out ImportObject addedNode)
			{
				...
				Tree ancestorTree = new Tree(leaf);
				ArrayList nodes = new ArrayList();
