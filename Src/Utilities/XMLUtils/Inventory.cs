...
namespace SIL.Utils
{
	...
	public class Inventory
	{
		...
		protected Dictionary<PairKey<XmlNode, XmlNode>, XmlNode> m_unifiedNodes =
				new Dictionary<PairKey<XmlNode, XmlNode>, XmlNode>();
		...
		public Inventory(string customInventoryPath, string filePattern, string xpath,
			Dictionary<string, string[]> keyAttrs)
		{
			...
			if (xmlInventorySwitch.TraceInfo)
			{
				...
				Debug.WriteLine("Initializing part inventory " + customInventoryPath + " took " + (msEnd - msStart) + " ms", xmlInventorySwitch.DisplayName);
			}
		}
		...
		public void LoadUserOverrides(int version, string sDatabase)
		{
			...
			if (Directory.Exists(path))
			{
				...
				if (String.IsNullOrEmpty(sDatabase))
					...
					sPattern = String.Format("db${0}${1}", sDatabase, m_filePattern);
				AddElementsFromFiles(Directory.GetFiles(path, sPattern), version);
			}
		}
		...
		public void DeleteUserOverrides(string sDatabase)
		{
			...
			if (Directory.Exists(path))
			{
				...
				if (String.IsNullOrEmpty(sDatabase))
					...
					sPattern = String.Format("db${0}${1}", sDatabase, m_filePattern);
				string[] rgsFiles = Directory.GetFiles(path, sPattern);
		...
		///
		/// </summary>
		public void LoadFlexUserOverrides(int version, string sDatabase)
		{
			Debug.Assert(m_version == version || m_version == 0);
			m_version = version; // remember for reloads; currently we only support one version.
			Debug.Assert(m_sDatabase == null || m_sDatabase == sDatabase);
			m_sDatabase = sDatabase;
			string path = FlexUserOverridePath();
			if (path == UserOverridePath())
				return;
			m_inventoryPaths.Add(path);
			if (Directory.Exists(path))
			{
				...
				if (String.IsNullOrEmpty(sDatabase))
					...
					sPattern = String.Format("db${0}${1}", sDatabase, m_filePattern);
				AddElementsFromFiles(Directory.GetFiles(path, sPattern), m_version);
			}
		}
		...
		///
		/// </summary>
		public void DeleteFlexUserOverrides(string sDatabase)
		{
			string path = FlexUserOverridePath();
			if (path == UserOverridePath())
				return;
		...
		public void PersistOverrideElement(XmlNode element)
		{
			...
			string sDatabase = String.IsNullOrEmpty(m_sDatabase) ? "default$$" : String.Format("db${0}$", m_sDatabase);
			string fileName = sDatabase + name + "_" + m_filePattern.Substring(1); // strip of leading *
			string path = UserOverridePath() + fileName;
			...
			XmlTextWriter xwriter = new XmlTextWriter(path, System.Text.Encoding.UTF8);
		...
		private string UserOverridePath()
		{
			string path = System.Environment.GetFolderPath(System.Environment.SpecialFolder.LocalApplicationData);
			return System.IO.Path.Combine(path,System.Windows.Forms.Application.CompanyName+"\\"+ System.Windows.Forms.Application.ProductName + "\\");
		}

		private string FlexUserOverridePath()
		{
			string path = System.Environment.GetFolderPath(System.Environment.SpecialFolder.LocalApplicationData);
			return System.IO.Path.Combine(path, System.Windows.Forms.Application.CompanyName + "\\Language Explorer\\");
		}
		...
		static public void SetInventory(string key, string sDatabase, Inventory val)
		{
			...
			if (val == null)
				throw new ArgumentNullException("'val' argument is null.");
			...
			Dictionary<string, string[]> keyAttrs)
		{
			...
			{
				int msEnd = Environment.TickCount;
				Debug.WriteLine("Initializing part inventory " + filePattern + " took " + (msEnd - msStart) + " ms", xmlInventorySwitch.DisplayName);
			}
		}
		...
		protected XmlNodeList LoadOneInventoryFile(string inventoryFilePath)
		{
			...
			{
				string x = string.Format(XmlUtilsStrings.ErrorReadingXMLFile0, inventoryFilePath);
		...
		public void AddCustomFiles(string[] filePaths)
		{
			for(int i = 0; i < filePaths.Length; ++i)
			{
				...
				{
					m_inventoryPaths.Add(path);
					AddElementsFromFiles(Directory.GetFiles(path, m_filePattern));
				}
			}
		}
		...
		protected void AddElementsFromFiles(string[] filePaths, int version)
		{
		...
		public void ExpandWsTaggedNodes(string sWsTag)
		{
			...
			foreach (XmlNode xn in m_wsTaggedNodes)
			{
				...
				XmlNode layout = GetElement("layout", new string[] { sClass, sType, sWsName });
				if (layout != null)
		...
		protected void LoadElements()
		{
			...
			foreach(string inventoryPath in m_inventoryPaths)
			{
				...
				if (Directory.Exists(p))
					AddElementsFromFiles(Directory.GetFiles(p, m_filePattern));
			}
		}
		...
		bool NoFilesChanged()
		{
			...
			foreach(string inventoryPath in m_inventoryPaths)
			{
				...
				if (Directory.Exists(p))
				{
					foreach (string path in Directory.GetFiles(p, m_filePattern))
					{
		...
		{
			XmlNode result = null;
			PairKey<XmlNode, XmlNode> key = new PairKey<XmlNode, XmlNode>(main, alteration);
		...
		internal class GetElementKey
		{
			...
			{
				GetElementKey other = obj as GetElementKey;
			...
			int HashZeroForNull(object obj)
			{
				if (obj == null)
					return 0;
		...
		{
			XmlNode original = path[0] as XmlNode;
			...
			{
				XmlNode node = path[i] as XmlNode;
