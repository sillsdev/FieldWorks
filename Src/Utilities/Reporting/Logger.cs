...
namespace SIL.Utils
{
	...
	public class Logger : IFWDisposable, IStream
	{
		...
		/// Initializes a new instance of the <see cref="Logger"/> class.
		/// </summary>
		/// <param name="fTruncate">if set to <c>true</c> truncate the existing log file.</param>
		/// ------------------------------------------------------------------------------------
		private Logger(bool fTruncate)
		{
			try
			{
				m_out = null;
		...
		{
			Exception dummy;
			Debug.WriteLine(ErrorReporter.GetHiearchicalExceptionInfo(e, out dummy));

			if (s_theOne != null)
			{
				s_theOne.WriteEventInternal(
					ErrorReporter.GetHiearchicalExceptionInfo(e, out dummy));
			}
		}
		#endregion
