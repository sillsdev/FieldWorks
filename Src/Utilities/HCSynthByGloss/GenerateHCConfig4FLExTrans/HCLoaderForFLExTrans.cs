...
namespace SIL.FieldWorks.WordWorks.Parser
{
	...
	{
		public static Machine.Morphology.HermitCrab.Language Load(LcmCache cache, IHCLoadErrorLogger logger)
		{
		...
		// A caret ^ is invalid in IDs.  Plain reduplication patterns have them and can be used in IDs.
		// We replace them with a period (which is allowed in IDs).
		private static readonly string kCaretInID = ".";
		...
		private Machine.Morphology.HermitCrab.Language m_language;
		...
		private HCLoaderForFLExTrans(LcmCache cache, IHCLoadErrorLogger logger)
		{
			m_cache = cache;
			m_logger = logger;
			...
			foreach (
				IPhNaturalClass nc in m_cache.LanguageProject.PhonologicalDataOA.NaturalClassesOS
			)
				m_naturalClassLookup[nc.Abbreviation.BestAnalysisAlternative.Text] = nc;

			XElement parserParamsElem = XElement.Parse(
			...
			m_noDefaultCompounding =
				hcElem != null && ((bool?)hcElem.Element("NoDefaultCompounding") ?? false);
			m_notOnClitics = hcElem == null || ((bool?)hcElem.Element("NotOnClitics") ?? true);
			m_acceptUnspecifiedGraphemes =
				hcElem != null && ((bool?)hcElem.Element("AcceptUnspecifiedGraphemes") ?? false);
		...
		{
			m_language = new Machine.Morphology.HermitCrab.Language { Name = m_cache.ProjectId.Name };

			var inflClassesGroup = new MprFeatureGroup
			{
				Name = "inflClasses",
				MatchType = MprFeatureGroupMatchType.Any
			};
			var posSymbols = new List<FeatureSymbol>();
			foreach (IPartOfSpeech pos in m_cache.LanguageProject.AllPartsOfSpeech)
			{
				posSymbols.Add(
					new FeatureSymbol("pos" + pos.Hvo)
					{
						Description = pos.Abbreviation.BestAnalysisAlternative.Text
					}
				);
			...
			var prodRestrictsGroup = new MprFeatureGroup
			{
				Name = "exceptionFeatures",
				MatchType = MprFeatureGroupMatchType.All
			};
			foreach (
				ICmPossibility prodRestrict in m_cache
					.LanguageProject
					.MorphologicalDataOA
					.ProdRestrictOA
			...
			var lexEntryInflTypesGroup = new MprFeatureGroup
			{
				Name = "lexEntryInflTypes",
				MatchType = MprFeatureGroupMatchType.All
			};
			foreach (
				ILexEntryInflType inflType in m_cache.ServiceLocator
			...
			LoadFeatureSystem(
				m_cache.LanguageProject.MsFeatureSystemOA,
				m_language.SyntacticFeatureSystem
			);

			LoadFeatureSystem(
				m_cache.LanguageProject.PhFeatureSystemOA,
				m_language.PhonologicalFeatureSystem
			);
			...
			LoadCharacterDefinitionTable(
				m_cache.LanguageProject.PhonologicalDataOA.PhonemeSetsOS[0]
			);

			foreach (
				IMoStemName stemName in m_cache.ServiceLocator
					.GetInstance<IMoStemNameRepository>()
					.AllInstances()
			)
			{
				...
				foreach (IFsFeatStruc fs in stemName.RegionsOC.Where(fs => !fs.IsEmpty))
				{
					...
					hcFS.AddValue(
						m_headFeature,
						LoadFeatureStruct(fs, m_language.SyntacticFeatureSystem)
					);
				...
				if (regions.Count > 0)
				{
					var hcStemName = new StemName(regions)
					{
						Name = stemName.Name.BestAnalysisAlternative.Text
					};
			...
			m_morphophonemic = new Stratum(m_table)
			{
				Name = "Morphophonemic",
			...
			if (
				m_cache.LanguageProject.MorphologicalDataOA.CompoundRulesOS.Count == 0
				&& !m_noDefaultCompounding
			)
			{
			...
			{
				foreach (
					IMoCompoundRule compoundRule in m_cache.LanguageProject.MorphologicalDataOA.CompoundRulesOS.Where(
						r => !r.Disabled
					)
				)
				{
					switch (compoundRule.ClassID)
					{
						case MoEndoCompoundTags.kClassId:
							m_morphophonemic.MorphologicalRules.Add(
								LoadEndoCompoundingRule((IMoEndoCompound)compoundRule)
							);
							break;

						case MoExoCompoundTags.kClassId:
							m_morphophonemic.MorphologicalRules.AddRange(
								LoadExoCompoundingRule((IMoExoCompound)compoundRule)
							);
							break;
					}
				}
			...
			foreach (
				IMoInflAffixTemplate template in m_cache.ServiceLocator
					.GetInstance<IMoInflAffixTemplateRepository>()
					.AllInstances()
					.Where(t => !t.Disabled)
			)
			{
				IMoInflAffixSlot[] slots = template.SuffixSlotsRS
					.Concat(template.PrefixSlotsRS.Reverse())
					.Where(s => s.Affixes.Any(msa => m_morphemes.ContainsKey(msa)))
					.ToArray();
				if (slots.Length > 0)
			...
			foreach (
				IPhSegmentRule prule in m_cache.LanguageProject.PhonologicalDataOA.PhonRulesOS
					.Where(r => !r.Disabled)
			...
			)
			{
				switch (prule.ClassID)
				{
					case PhRegularRuleTags.kClassId:
						...
						if (
							regRule.StrucDescOS.Count > 0
							|| regRule.RightHandSidesOS.Any(rhs => rhs.StrucChangeOS.Count > 0)
						)
						{
			...
			foreach (
				IMoAlloAdhocProhib alloAdhocProhib in m_cache.ServiceLocator
					.GetInstance<IMoAlloAdhocProhibRepository>()
					.AllInstances()
					.Where(
						a => !a.Disabled && a.FirstAllomorphRA != null && a.RestOfAllosRS.Count > 0
					)
			)
			{
			...
			foreach (
				IMoMorphAdhocProhib morphAdhocProhib in m_cache.ServiceLocator
					.GetInstance<IMoMorphAdhocProhibRepository>()
					.AllInstances()
					.Where(
						a => !a.Disabled && a.FirstMorphemeRA != null && a.RestOfMorphsRS.Count > 0
					)
			)
			{
		...
		private void LoadInflClassMprFeature(
			IMoInflClass inflClass,
			MprFeatureGroup inflClassesGroup
		)
		{
				...
				switch (form.MorphTypeRA.Guid.ToString())
				{
					...
					case MoMorphTypeTags.kMorphSuffixingInterfix:
						if (formStr.Contains("[") && !formStr.Contains("[...]"))
							return ((IMoAffixAllomorph)form).PhoneEnvRC.Any(
								env => IsValidEnvironment(env.StringRepresentation.Text)
							);
						return true;

					case MoMorphTypeTags.kMorphInfix:
					case MoMorphTypeTags.kMorphInfixingInterfix:
						return ((IMoAffixAllomorph)form).PositionRS.Any(
							env => IsValidEnvironment(env.StringRepresentation.Text)
						);
				}
			}
		...
		)
		{
			...
			hcEntry.Properties[MsaID] = msa.Hvo;
		...
		private RootAllomorph LoadRootAllomorph(IMoStemAllomorph allo, IMoMorphSynAnalysis msa)
		{
			...
			foreach (IPhEnvironment env in allo.PhoneEnvRC)
			{
				...
				if (IsValidEnvironment(env.StringRepresentation.Text, out error))
				{
					...
					hcAllo.Environments.Add(
						new AllomorphEnvironment(
							ConstraintType.Require,
							LoadEnvironmentPattern(contexts.Item1, true),
			...
			hcAllo.Properties[FormID] = allo.Hvo;
			return hcAllo;
		}
		...
		private AffixProcessRule LoadDerivAffixProcessRule(
			ILexEntry entry,
			IMoDerivAffMsa msa,
			IList<IMoForm> allos
		)
		{
			...
			if (msa.FromMsFeaturesOA != null && !msa.FromMsFeaturesOA.IsEmpty)
				requiredFS.AddValue(
					m_headFeature,
					LoadFeatureStruct(msa.FromMsFeaturesOA, m_language.SyntacticFeatureSystem)
				);
			requiredFS.Freeze();
			...
			if (msa.ToPartOfSpeechRA != null)
				outFS.AddValue(
					m_posFeature,
					m_posFeature.PossibleSymbols["pos" + msa.ToPartOfSpeechRA.Hvo]
				);
			if (msa.ToMsFeaturesOA != null && !msa.ToMsFeaturesOA.IsEmpty)
				outFS.AddValue(
					m_headFeature,
					LoadFeatureStruct(msa.ToMsFeaturesOA, m_language.SyntacticFeatureSystem)
				);
			outFS.Freeze();
			...
			StemName hcStemName;
			if (
				msa.FromStemNameRA != null
				&& m_stemNames.TryGetValue(msa.FromStemNameRA, out hcStemName)
			)
				mrule.RequiredStemName = hcStemName;

			mrule.Properties[MsaID] = msa.Hvo;
		...
		private AffixProcessRule LoadInflAffixProcessRule(
			ILexEntry entry,
			IMoInflAffMsa msa,
			IList<IMoForm> allos
		)
		{
			...
			if (msa.InflFeatsOA != null && !msa.InflFeatsOA.IsEmpty)
				requiredFS.AddValue(
					m_headFeature,
					LoadFeatureStruct(msa.InflFeatsOA, m_language.SyntacticFeatureSystem)
				);
			requiredFS.Freeze();
			mrule.RequiredSyntacticFeatureStruct = requiredFS;
		...
		private AffixProcessRule LoadUnclassifiedAffixProcessRule(
			ILexEntry entry,
			IMoUnclassifiedAffixMsa msa,
			IList<IMoForm> allos
		)
		{
			var mrule = new AffixProcessRule { Name = entry.ShortName, IsPartial = true };
		...
		private AffixProcessRule LoadCliticAffixProcessRule(
			ILexEntry entry,
			IMoStemMsa msa,
			IList<IMoForm> allos
		)
		...
		private IEnumerable<AffixProcessAllomorph> LoadAffixProcessAllomorphs(
			IMoMorphSynAnalysis msa,
			IList<IMoForm> allos
		)
		{
			...
			{
				foreach (
					IMoAffixAllomorph prefixAllo in allos
						.OfType<IMoAffixAllomorph>()
						.Where(a => a.MorphTypeRA.Guid == MoMorphTypeTags.kguidMorphPrefix)
				)
				{
					MprFeature[] requiredMprFeatures = null;
					if (msa is IMoInflAffMsa)
						requiredMprFeatures = LoadAllInflClasses(prefixAllo.InflectionClassesRC)
							.ToArray();
					foreach (
						IMoAffixAllomorph suffixAllo in allos
							.OfType<IMoAffixAllomorph>()
							.Where(a => a.MorphTypeRA.Guid == MoMorphTypeTags.kguidMorphSuffix)
					)
					{
						foreach (
							IPhEnvironment prefixEnv in GetAffixAllomorphEnvironments(
								prefixAllo,
								msa
							)
						)
						{
							foreach (
								IPhEnvironment suffixEnv in GetAffixAllomorphEnvironments(
									suffixAllo,
									msa
								)
							)
							{
								...
								try
								{
									hcAllo = LoadCircumfixAffixProcessAllomorph(
										prefixAllo,
										prefixEnv,
										suffixAllo,
										suffixEnv
					...
					switch (allo.ClassID)
					{
						case MoAffixProcessTags.kClassId:
							...
							try
							{
								...
								if (msa is IMoInflAffMsa)
									hcAffixProcessAllo.RequiredMprFeatures.AddRange(
										LoadAllInflClasses(affixProcess.InflectionClassesRC)
									);
								m_allomorphs
									.GetOrCreate(allo, () => new List<Allomorph>())
									.Add(hcAffixProcessAllo);
							}
							catch (InvalidShapeException ise)
							{
								m_logger.InvalidShape(ise.String, ise.Position, msa);
						...
						case MoAffixAllomorphTags.kClassId:
							...
							MprFeature[] requiredMprFeatures = null;
							if (msa is IMoInflAffMsa)
								requiredMprFeatures = LoadAllInflClasses(
										affixAllo.InflectionClassesRC
									)
									.ToArray();
							foreach (
								IPhEnvironment env in GetAffixAllomorphEnvironments(affixAllo, msa)
							)
							{
								...
								try
								{
									...
									if (requiredMprFeatures != null)
										hcAffixAllo.RequiredMprFeatures.AddRange(
											requiredMprFeatures
										);
									var requiredFS = new FeatureStruct();
									if (
										affixAllo.MsEnvFeaturesOA != null
										&& !affixAllo.MsEnvFeaturesOA.IsEmpty
									)
										requiredFS.AddValue(
											m_headFeature,
											LoadFeatureStruct(
												affixAllo.MsEnvFeaturesOA,
												m_language.SyntacticFeatureSystem
											)
										);
									requiredFS.Freeze();
						...
						case MoStemAllomorphTags.kClassId:
							...
							foreach (
								IPhEnvironment env in GetStemAllomorphEnvironments(stemAllo, msa)
							)
							{
		...
		private IEnumerable<IPhEnvironment> GetAffixAllomorphEnvironments(
			IMoAffixAllomorph allo,
			IMoMorphSynAnalysis msa
		)
		{
		...
		private IEnumerable<IPhEnvironment> GetStemAllomorphEnvironments(
		...
		private IEnumerable<IPhEnvironment> GetValidEnvironments(
			IEnumerable<IPhEnvironment> envs,
			IMoForm allo,
			IMoMorphSynAnalysis msa
		)
		{
			...
			if (m_envValidator.Recognize(env))
			{
				...
				do
				{
					...
					if (start != -1)
					{
						...
						{
							error = string.Format(
								"The natural class \"{0}\" contains an invalid phoneme.",
								ncAbbr
							);
							return false;
						}
		...
		private AffixProcessAllomorph LoadCircumfixAffixProcessAllomorph(
			IMoAffixAllomorph prefixAllo,
			IPhEnvironment prefixEnv,
			IMoAffixAllomorph suffixAllo,
			IPhEnvironment suffixEnv
		)
		{
			var hcAllo = new AffixProcessAllomorph();

			Pattern<Word, ShapeNode> leftEnvPattern = null,
				rightEnvPattern = null;
			...
			if (leftEnvPattern != null || rightEnvPattern != null)
			{
				...
				if (leftEnvPattern != null && rightEnvPattern == null)
					...
					name = string.Format(
						"{0}, {1}",
						prefixEnv.StringRepresentation.Text,
						suffixEnv.StringRepresentation.Text
					);
				hcAllo.Environments.Add(
					new AllomorphEnvironment(
						ConstraintType.Require,
			...
			hcAllo.Properties[FormID] = prefixAllo.Hvo;
			hcAllo.Properties[FormID2] = suffixAllo.Hvo;
			if (prefixEnv != null)
			...
			foreach (IMoRuleMapping mapping in allo.OutputOS)
			{
				switch (mapping.ClassID)
				{
					...
					case MoCopyFromInputTags.kClassId:
						...
						{
							string partName = (copyFromInput.ContentRA.IndexInOwner + 1).ToString(
					...
					case MoModifyFromInputTags.kClassId:
						...
						if (
							modifyFromInput.ContentRA != null
							&& modifyFromInput.ModificationRA != null
						)
						{
							SimpleContext ctxt;
							if (!TryLoadSimpleContext(modifyFromInput.ModificationRA, out ctxt))
								throw new InvalidAffixProcessException(allo, false);
							string partName = (modifyFromInput.ContentRA.IndexInOwner + 1).ToString(
								CultureInfo.InvariantCulture
							);
		...
		private AffixProcessAllomorph LoadFormAffixProcessAllomorph(
			IMoForm allo,
			IPhEnvironment env
		)
		{
				...
				else
				{
					...
					switch (allo.MorphTypeRA.Guid.ToString())
					{
						...
						case MoMorphTypeTags.kMorphEnclitic:
							...
							var suffixNull = new Pattern<Word, ShapeNode>(
								"suffixNull",
								SuffixNull()
							);
							suffixNull.Freeze();
						...
						case MoMorphTypeTags.kMorphPrefixingInterfix:
						case MoMorphTypeTags.kMorphProclitic:
							var prefixNull = new Pattern<Word, ShapeNode>(
								"prefixNull",
								PrefixNull()
							);
							prefixNull.Freeze();
		...
		private IEnumerable<Pattern<Word, ShapeNode>> LoadReduplicationPatterns(string patternStr)
		{
			foreach (string token in TokenizeContext(patternStr))
			{
				if (token.StartsWith("["))
				{
					...
					var pattern = new Pattern<Word, ShapeNode>(
						token.Substring(1, token.Length - 2).Trim().Replace("^", kCaretInID),
						new Constraint<Word, ShapeNode>(ctxt.FeatureStruct) { Tag = ctxt }
					);
					pattern.Freeze();
		...
		private IEnumerable<MorphologicalOutputAction> LoadReduplicationOutputActions(
			string patternStr
		)
		{
			foreach (string token in TokenizeContext(patternStr))
			{
				if (token.StartsWith("["))
				{
					yield return new CopyFromInput(
						token.Substring(1, token.Length - 2).Trim().Replace("^", kCaretInID)
					);
				}
				else
		...
		private AffixTemplate LoadAffixTemplate(
			IMoInflAffixTemplate template,
			IList<IMoInflAffixSlot> slots
		)
		{
			...
			requiredFS.AddValue(
				m_posFeature,
				LoadAllPartsOfSpeech(template.OwnerOfClass<IPartOfSpeech>())
			);
			requiredFS.Freeze();
			hcTemplate.RequiredSyntacticFeatureStruct = requiredFS;
			hcTemplate.Properties[TemplateID] = template.Hvo;

			foreach (IMoInflAffixSlot slot in slots)
			{
				ILexEntryInflType type = slot.ReferringObjects
					.OfType<ILexEntryInflType>()
					.FirstOrDefault();
				...
				hcTemplate.Slots.Add(
					new AffixTemplateSlot(rules)
					{
						Name = slot.Name.BestAnalysisAlternative.Text,
		...
		private AffixProcessRule LoadNullAffixProcessRule(
			ILexEntryInflType type,
			IMoInflAffixTemplate template,
		...
		)
		{
			...
			if (type.InflFeatsOA != null && !type.InflFeatsOA.IsEmpty)
				outFS.AddValue(
					m_headFeature,
					LoadFeatureStruct(type.InflFeatsOA, m_language.SyntacticFeatureSystem)
				);
			outFS.Freeze();
			mrule.OutSyntacticFeatureStruct = outFS;
		...
		private CompoundingRule LoadEndoCompoundingRule(IMoEndoCompound compoundRule)
		{
			...
			if (compoundRule.HeadLast)
			{
				if (compoundRule.RightMsaOA.PartOfSpeechRA != null)
					headRequiredFS.AddValue(
						m_posFeature,
						LoadAllPartsOfSpeech(compoundRule.RightMsaOA.PartOfSpeechRA)
					);
				if (compoundRule.LeftMsaOA.PartOfSpeechRA != null)
					nonheadRequiredFS.AddValue(
						m_posFeature,
						LoadAllPartsOfSpeech(compoundRule.LeftMsaOA.PartOfSpeechRA)
					);
			}
			else
			{
			...
			if (compoundRule.OverridingMsaOA.PartOfSpeechRA != null)
				outFS.AddValue(
					m_posFeature,
					m_posFeature.PossibleSymbols[
						"pos" + compoundRule.OverridingMsaOA.PartOfSpeechRA.Hvo
					]
				);
			outFS.Freeze();
			...
			var hcCompoundRule = new CompoundingRule
			{
				...
				Properties = { { CRuleID, compoundRule.Hvo } }
			};

			var subrule = new CompoundingSubrule();

			if (compoundRule.OverridingMsaOA.InflectionClassRA != null)
				subrule.OutMprFeatures.Add(
					m_mprFeatures[compoundRule.OverridingMsaOA.InflectionClassRA]
				);
		...
		private IEnumerable<CompoundingRule> LoadExoCompoundingRule(IMoExoCompound compoundRule)
		{
			...
			if (compoundRule.ToMsaOA.PartOfSpeechRA != null)
				outFS.AddValue(
					m_posFeature,
					m_posFeature.PossibleSymbols["pos" + compoundRule.ToMsaOA.PartOfSpeechRA.Hvo]
				);
			outFS.Freeze();
			...
			if (compoundRule.ToMsaOA.InflectionClassRA != null)
				rightSubrule.OutMprFeatures.Add(
					m_mprFeatures[compoundRule.ToMsaOA.InflectionClassRA]
				);
			...
			if (compoundRule.ToMsaOA.InflectionClassRA != null)
				leftSubrule.OutMprFeatures.Add(
					m_mprFeatures[compoundRule.ToMsaOA.InflectionClassRA]
				);
		...
		private RewriteRule LoadRewriteRule(IPhRegularRule prule)
		{
			...
			hcPrule.Properties[PRuleID] = prule.Hvo;
			...
			foreach (IPhSegRuleRHS rhs in prule.RightHandSidesOS)
			{
				...
				psubrule.RequiredMprFeatures.AddRange(
					rhs.ReqRuleFeatsRC.SelectMany(LoadMprFeatures)
				);
				psubrule.ExcludedMprFeatures.AddRange(
					rhs.ExclRuleFeatsRC.SelectMany(LoadMprFeatures)
				);
				...
				if (rhs.LeftContextOA != null)
				{
					...
					if (IsWordInitial(rhs.LeftContextOA.ToEnumerable()))
						leftPattern.Children.Add(
							new Constraint<Word, ShapeNode>(HCFeatureSystem.LeftSideAnchor)
						);
				...
				if (rhs.RightContextOA != null)
				{
					...
					if (IsWordFinal(rhs.RightContextOA.ToEnumerable()))
						rightPattern.Children.Add(
							new Constraint<Word, ShapeNode>(HCFeatureSystem.RightSideAnchor)
						);
					rightPattern.Freeze();
		...
		private MetathesisRule LoadMetathesisRule(IPhMetathesisRule prule)
		{
			...
			hcPrule.Properties[CRuleID] = prule.Hvo;
		...
		{
			return new Quantifier<Word, ShapeNode>(
				0,
				-1,
		...
		private PatternNode<Word, ShapeNode> SuffixNull()
		{
			return new Quantifier<Word, ShapeNode>(
				...
				-1,
				new Group<Word, ShapeNode>(
					new Constraint<Word, ShapeNode>(m_morphBdry.FeatureStruct)
					{
						Tag = m_morphBdry
					},
					new Constraint<Word, ShapeNode>(m_null.FeatureStruct) { Tag = m_null }
		...
		private IEnumerable<PatternNode<Word, ShapeNode>> AnyPlus()
		{
			...
			yield return new Quantifier<Word, ShapeNode>(
				1,
				-1,
				new Constraint<Word, ShapeNode>(m_any.FeatureStruct) { Tag = m_any }
			);
		...
		private bool LoadPatternNode(
			IPhPhonContext ctxt,
			Dictionary<IPhFeatureConstraint, string> variables,
			out PatternNode<Word, ShapeNode> node
		)
		{
			switch (ctxt.ClassID)
			{
				...
				case PhSimpleContextNCTags.kClassId:
					...
					{
						node = new Constraint<Word, ShapeNode>(hcCtxt.FeatureStruct)
						{
							Tag = hcCtxt
						};
						return true;
		...
		private IEnumerable<PatternNode<Word, ShapeNode>> LoadPatternNodes(string patternStr)
		{
			foreach (string token in TokenizeContext(patternStr))
			{
				switch (token[0])
				{
					...
					case '[':
						IPhNaturalClass nc = m_naturalClassLookup[
							token.Substring(1, token.Length - 2).Trim()
						];
						SimpleContext ctxt;
						TryLoadSimpleContext(nc, out ctxt);
						yield return new Constraint<Word, ShapeNode>(ctxt.FeatureStruct)
						{
							Tag = ctxt
						};
						break;
					...
					case '(':
						yield return new Quantifier<Word, ShapeNode>(
							...
							1,
							new Group<Word, ShapeNode>(
								LoadPatternNodes(token.Substring(1, token.Length - 2).Trim())
							)
						);
						break;
					...
					default:
						...
						yield return new Group<Word, ShapeNode>(
							segments.Shape.Select(
								n => new Constraint<Word, ShapeNode>(n.Annotation.FeatureStruct)
							)
						)
						{
							Tag = segments
		...
		private bool IsWordFinal(IEnumerable<IPhPhonContext> ctxts)
		{
			...
			{
				if (
					seqCtxt.MembersRS.Count > 0
					&& IsWordBoundary(seqCtxt.MembersRS[seqCtxt.MembersRS.Count - 1])
				)
					return true;
			}
			return false;
		...
		private FeatureStruct LoadFeatureStruct(IFsFeatStruc fs, FeatureSystem featSys)
		{
			...
			if (fs != null)
			{
				foreach (IFsFeatureSpecification value in fs.FeatureSpecsOC)
				{
					...
					if (closedValue != null)
					{
						var hcFeature = featSys.GetFeature<SymbolicFeature>(
							"feat" + closedValue.FeatureRA.Hvo
						);
						// TODO: should we display something to the user if a FS has an invalid value?
						if (
							hcFeature.PossibleSymbols.TryGet(
								"sym" + closedValue.ValueRA.Hvo,
								out var symbol
							)
						)
					...
					else
					{
						...
						var hcFeature = featSys.GetFeature<ComplexFeature>(
							"feat" + complexValue.FeatureRA.Hvo
						);
						hcFS.AddValue(
							hcFeature,
							LoadFeatureStruct((IFsFeatStruc)complexValue.ValueOA, featSys)
						);
					}
				}
			}
			...
			if (m_acceptUnspecifiedGraphemes && !IsLexicalPattern(str))
			{
				...
				do
				{
					...
					if (errorPos != -1)
					{
						...
						if (
							CharUnicodeInfo.GetUnicodeCategory(symbolStr[0])
						...
						)
						{
							...
							symbolStr = StringInfo.GetNextTextElement(
								str,
								baseCharPositions.Last(pos => pos < errorPos)
							);
						}
		...
		private IEnumerable<FeatureSymbol> LoadAllPartsOfSpeech(IEnumerable<IPartOfSpeech> poss)
		{
			foreach (IPartOfSpeech pos in poss)
			{
				...
				foreach (
					FeatureSymbol symbol in LoadAllPartsOfSpeech(
						pos.SubPossibilitiesOS.Cast<IPartOfSpeech>()
					)
				)
					yield return symbol;
			}
		...
		private IEnumerable<MprFeature> LoadMprFeatures(IPhPhonRuleFeat ruleFeat)
		{
			switch (ruleFeat.ItemRA.ClassID)
			{
				case MoInflClassTags.kClassId:
					foreach (
						MprFeature mprFeat in LoadAllInflClasses((IMoInflClass)ruleFeat.ItemRA)
					)
						yield return mprFeat;
					break;
		...
		private static void LoadFeatureSystem(IFsFeatureSystem featSys, FeatureSystem hcFeatSys)
		{
			foreach (IFsFeatDefn feature in featSys.FeaturesOC)
			{
				...
				if (closedFeature != null)
				{
					hcFeatSys.Add(
						new SymbolicFeature(
							"feat" + closedFeature.Hvo,
							closedFeature.ValuesOC.Select(
								sfv =>
									new FeatureSymbol("sym" + sfv.Hvo)
									{
										Description = sfv.Abbreviation.BestAnalysisAlternative.Text
									}
							)
						)
						{
				...
				{
					hcFeatSys.Add(
						new ComplexFeature("feat" + feature.Hvo)
						{
							Description = feature.Abbreviation.BestAnalysisAlternative.Text
						}
					);
		...
		{
			m_table = new CharacterDefinitionTable
			{
				Name = phonemeSet.Name.BestAnalysisAlternative.Text
			};
			...
			foreach (
				IPhBdryMarker bdry in phonemeSet.BoundaryMarkersOC.Where(
					bdry => bdry.Guid != LangProjectTags.kguidPhRuleWordBdry
				)
			)
			{
		...
		private bool TryLoadSimpleContext(
			IPhSimpleContextNC ctxt,
			Dictionary<IPhFeatureConstraint, string> variables,
			out SimpleContext hcCtxt
		)
		{
			...
			hcCtxt = new SimpleContext(
				hcNaturalClass,
				GetVariables(variables, ctxt.PlusConstrRS, true)
					.Concat(GetVariables(variables, ctxt.MinusConstrRS, false))
			);
			return true;
		}

		private IEnumerable<SymbolicFeatureValue> GetVariables(
			Dictionary<IPhFeatureConstraint, string> variables,
			IEnumerable<IPhFeatureConstraint> constraints,
			bool agree
		)
		{
			foreach (IPhFeatureConstraint constraint in constraints)
			{
				...
				var feat = m_language.PhonologicalFeatureSystem.GetFeature<SymbolicFeature>(
					"feat" + constraint.FeatureRA.Hvo
				);
		...
		private bool TryLoadNaturalClass(
			IPhNaturalClass naturalClass,
			out NaturalClass hcNaturalClass
		)
		{
			...
			else
			{
				...
				FeatureStruct fs = LoadFeatureStruct(
					featNC.FeaturesOA,
					m_language.PhonologicalFeatureSystem
				);
