...
namespace SIL.Utils
{
	...
	public class UtilsTest // can't derive from BaseTest because of dependencies
	{
		...
		{
			Assert.AreEqual(1, Utils.Surrogates.NextChar("ab\xD800\xDC00c", 0));
			Assert.AreEqual(2, Utils.Surrogates.NextChar("ab\xD800\xDC00c", 1));
			Assert.AreEqual(4, Utils.Surrogates.NextChar("ab\xD800\xDC00c", 2));
			Assert.AreEqual(5, Utils.Surrogates.NextChar("ab\xD800\xDC00c", 4));
			Assert.AreEqual(4, Utils.Surrogates.NextChar("ab\xD800\xDC00", 2));
			Assert.AreEqual(3, Utils.Surrogates.NextChar("ab\xD800", 2), "Badly formed pair at end...don't go too far");
			Assert.AreEqual(3, Utils.Surrogates.NextChar("ab\xD800c", 2), "Badly formed pair in middle...don't go too far");
			Assert.AreEqual(0, Utils.Surrogates.PrevChar("ab\xD800\xDC00c", 1));
			Assert.AreEqual(1, Utils.Surrogates.PrevChar("ab\xD800\xDC00c", 2));
			Assert.AreEqual(2, Utils.Surrogates.PrevChar("ab\xD800\xDC00c", 3), "initial ich at a bad position, move back normally to sync");
			Assert.AreEqual(2, Utils.Surrogates.PrevChar("ab\xD800\xDC00c", 4), "double move succeeds");
			Assert.AreEqual(4, Utils.Surrogates.PrevChar("ab\xD800\xDC00c", 5));
			Assert.AreEqual(3, Utils.Surrogates.PrevChar("ab\xD800c", 4), "no double move on bad pair");
			Assert.AreEqual(0, Utils.Surrogates.PrevChar("\xD800\xDC00c", 2), "double move succeeds at start (and end)");
			Assert.AreEqual(0, Utils.Surrogates.PrevChar("\xDC00c", 1), "no double move on bad trailer at start");
			Assert.AreEqual(0, Utils.Surrogates.PrevChar("\xD800c", 1), "no double move on bad leader at start");
		}
