...
namespace SIL.AllomorphGenerator
{
	...
	/// </summary>
	/// This is a copy of SimpleMatchDlg with a few changes to allow for access to inforrmation
	/// needed for the Allomorph Generator
	public class SimpleMatchDlgAlloGen : Form
	{
		...
		public MatcherType matcherType { get; set; } = MatcherType.Anywhere;
		...
		public void SetDlgValues(Matcher agMatcher, IVwStylesheet stylesheet)
		{
			CheckDisposed();

			MatcherType type = agMatcher.Type;
			// Figure out which kind to check
			if (type == MatcherType.Anywhere)
			{
				m_anywhereButton.Checked = true;
			}
			...
			else if (type == MatcherType.Begin)
			{
				m_atStartButton.Checked = true;
			}
			...
			m_MatchCasecheckBox.Checked = agMatcher.MatchCase;
			m_MatchDiacriticscheckBox.Checked = agMatcher.MatchDiacritics;
			m_textBox.Text = agMatcher.Pattern;
			m_textBox.AdjustForStyleSheet(this, null, stylesheet);
		}

		public Matcher GetMatcher()
		{
			MatcherType matcherType = MatcherType.Anywhere;
			if (m_anywhereButton.Checked)
				matcherType = MatcherType.Anywhere;
			else if (m_atEndButton.Checked)
				matcherType = MatcherType.End;
			else if (m_atStartButton.Checked)
				matcherType = MatcherType.Begin;
			else if (m_regExButton.Checked)
				matcherType = MatcherType.RegularExpression;
			else
				matcherType = MatcherType.Exact;
			Matcher matcher = new Matcher(matcherType);
			matcher.MatchCase = m_MatchCasecheckBox.Checked;
			matcher.MatchDiacritics = m_MatchDiacriticscheckBox.Checked;
			matcher.Pattern = m_textBox.Text;
			return matcher;
		}
		...
		public IMatcher ResultingMatcher
		{
			get
			{
				...
				m_ivwpattern.IcuLocale = m_textBox.WritingSystemFactory.GetStrFromWs(
					m_textBox.WritingSystemCode
				);
		...
		private void m_okButton_Click(object sender, System.EventArgs e)
		{
			...
			string errMsg = String.Format(
				XMLViewsStrings.ksFilterErrorMsg,
				testMatcher.ErrorMessage()
			);
			MessageBox.Show(
				this,
				errMsg,
				XMLViewsStrings.ksFilterError,
