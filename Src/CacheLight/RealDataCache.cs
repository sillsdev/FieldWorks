...
namespace SIL.FieldWorks.CacheLight
{
	...
	/// </summary>
	public sealed class RealDataCache : ISilDataAccess, IVwCacheDa, IFWDisposable
	{
		...
		/// </summary>
		private List<uint> m_clids = new List<uint>();
		...
		/// <summary></summary>
		private Dictionary<HvoFlidKey, string> m_basicStringCache = new Dictionary<HvoFlidKey, string>();
		/// <summary>
		...
		/// <summary>
		/// Get the next available Hvo. It automatically incremnts with each call.
		/// </summary>
		...
		private void CheckHvoTagMatch(int hvo, int tag)
		{
			...
			int clid = get_IntProp(hvo, (int)CmObjectFields.kflidCmObject_Class);
			// First find out how many there are.
			int countAllFlidsOut = MetaDataCache.GetFields((uint)clid, true,
				(int)CellarModuleDefns.kgrfcptAll, 0, null);
			// Now get them for real.
			using (ArrayPtr flids = MarshalEx.ArrayToNative(countAllFlidsOut, typeof(uint)))
			{
				bool foundFlid = false;
				countAllFlidsOut = MetaDataCache.GetFields((uint)clid, true,
					(int)CellarModuleDefns.kgrfcptAll, countAllFlidsOut, flids);
				uint[] uFlids = (uint[])MarshalEx.NativeToArray(flids, countAllFlidsOut, typeof(uint));
				for (int i = 0; i < uFlids.Length; ++i)
				{
					uint flid = uFlids[i];
					if (flid == (uint)tag)
					{
		...
		private void AddToIntCache(int obj, int tag, int val, bool isIntType)
		{
			...
			if (isIntType)
			{
				...
				switch (tag)
				{
					case (int)CmObjectFields.kflidCmObject_Class:
						...
						if (m_clids.Count == 0)
						{
							int countAllClasses = MetaDataCache.ClassCount;
						...
						bool isValidClid = m_clids.Contains((uint)val);
						if (!isValidClid)
					...
					default:
						...
						int flidType = MetaDataCache.GetFieldType((uint)tag);
						if (flidType != (int)CellarModuleDefns.kcptInteger)
			...
			else
			{
				...
				int clid = get_IntProp(val, (int)CmObjectFields.kflidCmObject_Class);
				if (!MetaDataCache.get_IsValidClass((uint)tag, (uint)clid))
					throw new ArgumentException(String.Format("Can't put the 'val' ({0}) into the 'tag' ({1}).", val, tag));
			}

			// Made it here, so set it.
			m_intCache[new HvoFlidKey(obj, (uint)tag)] = val;
		}
		...
		private void AddToLongCache(int obj, int tag, long val, int flidType)
		{
			CheckBasics(obj);
			...
			int tagType = MetaDataCache.GetFieldType((uint)tag);
			if (tagType != flidType)
				throw new ArgumentException(String.Format("Can only put long integers in the tag/flid '{0}'.", tag));
			m_longCache[new HvoFlidKey(obj, (uint)tag)] = val;
		}
		...
		public void CacheBooleanProp(int hvo, int tag, bool value)
		{
			...
			int flidType = MetaDataCache.GetFieldType((uint)tag);
			if (flidType != (int)CellarModuleDefns.kcptBoolean)
				throw new ArgumentException(String.Format("Can only put booleans in the tag/flid '{0}'.", tag));

			// Made it here, so set it.
			m_boolCache[new HvoFlidKey(hvo, (uint)tag)] = value;
		}
		...
		/// <remarks>IVwCacheDa method</remarks>
		public void CacheGuidProp(int obj, int tag, System.Guid uid)
		{
			CheckDisposed();
			...
			switch (tag)
			{
				...
				default:
					...
					int flidType = MetaDataCache.GetFieldType((uint)tag);
					if (flidType != (int)CellarModuleDefns.kcptGuid)
						throw new ArgumentException(String.Format("Can only put Guids in the tag/flid '{0}'.", tag));
					break;
			}
			// Made it here, so set it.
			m_guidCache[new HvoFlidKey(obj, (uint)tag)] = uid;
		}
		...
		public int get_CachedIntProp(int obj, int tag, out bool isInCache)
		{
			...
			{
				// Some other kind of problem, such as mis-match between obj and tag.
				// Just re-throw it, since the client was so ill-behaved.
				throw;
			}
			return val;
		}
		...
		public void CacheUnicodeProp(int obj, int tag, string val, int cch)
		{
			...
			int flidType = MetaDataCache.GetFieldType((uint)tag);
			if ((flidType != (int)CellarModuleDefns.kcptUnicode)
				&& (flidType != (int)CellarModuleDefns.kcptBigUnicode))
				throw new ArgumentException(String.Format("Can only put Unicode data in the tag/flid '{0}'.", tag));

			m_basicStringCache[new HvoFlidKey(obj, (uint)tag)] = val;
		}
		...
		public void UnicodePropRgch(int obj, int tag, [MarshalAs(UnmanagedType.CustomMarshaler, MarshalTypeRef = typeof(ArrayPtrMarshaler), SizeParamIndex = 3)] ArrayPtr/*OLECHAR[]*/ rgch, int cchMax, out int cch)
		{
			...
			cch = 0;
			string str = get_UnicodeProp(obj, tag);
			cch = str.Length;
			if (cchMax == 0)
				return;
			if (cch >= cchMax)
		...
		public void CacheTimeProp(int hvo, int tag, long val)
		{
			...
			AddToLongCache(hvo, tag, val, (int)CellarModuleDefns.kcptTime);
		}
		...
		public void CacheInt64Prop(int obj, int tag, long val)
		{
			...
			AddToLongCache(obj, tag, val, (int)CellarModuleDefns.kcptGenDate);
		}
		...
		/// <remarks>IVwCacheDa method</remarks>
		public void CacheBinaryProp(int obj, int tag, [MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 3)] System.Byte[] rgb, int cb)
		{
			CheckDisposed();
			...
			m_basicByteArrayCache[new HvoFlidKey(obj, (uint)tag)] = rgb;
		}
		...
		/// <param name='cb'>cb</param>
		/// <remarks>ISilDataAccess method</remarks>
		public void SetBinary(int hvo, int tag, [MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 3)] System.Byte[] rgb, int cb)
		{
			CheckDisposed();
		...
		public void BinaryPropRgb(int obj, int tag, [MarshalAs(UnmanagedType.CustomMarshaler, MarshalTypeRef = typeof(ArrayPtrMarshaler), SizeParamIndex = 3)] ArrayPtr/*byte[]*/ rgb, int cbMax, out int cb)
		{
			...
			if (!m_basicByteArrayCache.ContainsKey(key))
				removeFromCache = CheckForVirtual(obj, tag);
			byte[] array = m_basicByteArrayCache[key];
			if (removeFromCache)
		...
		public void CacheStringProp(int obj, int tag, ITsString tss)
		{
			...
			m_basicITsStringCache[new HvoFlidKey(obj, (uint)tag)] = tss;
		}

		/// <summary>Member CacheStringFields</summary>
		/// <param name='obj'>obj</param>
		/// <param name='tag'>tag</param>
		/// <param name='rgchTxt'>rgchTxt</param>
		/// <param name='cchTxt'>cchTxt</param>
		/// <param name='rgbFmt'>rgbFmt</param>
		/// <param name='cbFmt'>cbFmt</param>
		/// <remarks>IVwCacheDa method</remarks>
		public void CacheStringFields(int obj, int tag, string rgchTxt, int cchTxt, [MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 5)] System.Byte[] rgbFmt, int cbFmt)
		{
			CheckDisposed();

			if (cchTxt != rgchTxt.Length)
				throw new ArgumentException("Text counts don't match.");
			if (cbFmt != rgbFmt.Length)
				throw new ArgumentException("Buffer counts don't match.");

			ITsStrFactory tsf = TsStrFactoryClass.Create();
			ITsString tss = tsf.DeserializeStringRgb(rgchTxt, rgbFmt, cbFmt);
			CacheStringProp(obj, tag, tss);
		}
		...
		public ITsString get_StringProp(int hvo, int tag)
		{
			...
			if (!m_basicITsStringCache.ContainsKey(key))
				removeFromCache = CheckForVirtual(hvo, tag);
			ITsString val = m_basicITsStringCache[key];
			if (removeFromCache)
		...
		public void CacheStringAlt(int obj, int tag, int ws, ITsString tss)
		{
			...
			int flidType = MetaDataCache.GetFieldType((uint)tag);
			if ((flidType == (int)CellarModuleDefns.kcptMultiUnicode)
				|| (flidType == (int)CellarModuleDefns.kcptMultiBigUnicode)
				|| (flidType == (int)CellarModuleDefns.kcptMultiString)
				|| (flidType == (int)CellarModuleDefns.kcptMultiBigString))
			{
				m_extendedKeyCache[new HvoFlidWSKey(obj, (uint)tag, ws)] = tss;
			}
			else
			{
		...
		public ITsString get_MultiStringAlt(int hvo, int tag, int ws)
		{
			...
			HvoFlidWSKey key = new HvoFlidWSKey(hvo, (uint)tag, ws);
			bool removeFromCache = false;
			if (!m_extendedKeyCache.ContainsKey(key))
				removeFromCache = CheckForVirtual(hvo, tag);
			ITsString tss = m_extendedKeyCache[key];
			if (tss == null)
			{
		...
		public ITsMultiString get_MultiStringProp(int hvo, int tag)
		{
			...
			ITsMultiString tsms = TsMultiStringClass.Create();
			foreach (HvoFlidWSKey key in m_extendedKeyCache.Keys)
			{
				if (key.Hvo == hvo && key.Flid == tag)
					tsms.set_String(key.Ws, (ITsString)m_extendedKeyCache[key]);
			}
			return tsms;
		}
		...
		public void CacheVecProp(int obj, int tag, [MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 3)] int[] rghvo, int chvo)
		{
			...
			if (m_checkWithMDC)
			{
				...
				{
					int hvo = rghvo[i];
					uint clid = (uint)get_IntProp(hvo, (int)CmObjectFields.kflidCmObject_Class);
					if (!MetaDataCache.get_IsValidClass((uint)tag, clid))
					...
					if (tag == (int)CellarModuleDefns.kcptOwningCollection
						|| tag == (int)CellarModuleDefns.kcptOwningSequence)
					{
						int currentOwner = get_ObjectProp(hvo, (int)CmObjectFields.kflidCmObject_Owner);
						uint ownFlid = (uint)get_IntProp(hvo, (int)CmObjectFields.kflidCmObject_OwnFlid);
					}
					*/
				}
			}

			m_vectorCache[new HvoFlidKey(obj, (uint)tag)] = new List<int>(rghvo);
		}
		...
		public int MakeNewObject(int clid, int hvoOwner, int tag, int ord)
		{
			...
			throw new NotImplementedException("'MakeNewObject' not implemented yet.");
		}
		...
		public void AddNotification(IVwNotifyChange _nchng)
		{
			...
			throw new NotImplementedException("'AddNotification' not implemented yet.");
		}
		...
		#region PropChanged methods

		/// <summary>Member SuppressPropChanges</summary>
		public void SuppressPropChanges()
		{
			CheckDisposed();

			throw new NotImplementedException("'SuppressPropChanges' not implemented yet.");
		}

		/// <summary>Member ResumePropChanges</summary>
		public void ResumePropChanges()
		{
			CheckDisposed();

			throw new NotImplementedException("'ResumePropChanges' not implemented yet.");
		}

		#endregion PropChanged methods
		...
		internal struct HvoFlidKey
		{
			...
			public HvoFlidKey(int hvo, uint flid)
			{
				m_hvo = hvo;
				m_flid = flid;
			}
		...
		internal struct HvoFlidWSKey
		{
			...
			public HvoFlidWSKey(int hvo, uint flid, int ws)
			{
				m_hvo = hvo;
				m_flid = flid;
				m_ws = ws;
