...
{
	/// <summary />
	internal sealed class LexImportWizardCharMarkerDlg : Form
	{
		private Label lblBeginMarker;
		private Label lblEndMarker;
		private TextBox tbBeginMarker;
		private TextBox tbEndMarker;
		private ComboBox cbLangDesc;
		...
		public string NoChange => LanguageExplorerControls.ksNoChange;
		...
		internal void Init(ClsInFieldMarker ifm, Hashtable uiLangsHT, LcmCache cache, ILexImportWizard importWizard)
		{
			if (ifm == null)
			{
			...
			_propertyTable = _importWizard.PropertyTable;
		...
		{
			FwStylesDlg.RunStylesDialogForCombo(cbStyle, InitializeStylesComboBox, string.Empty, m_stylesheet as LcmStyleSheet, m_cache, this, _propertyTable.GetValue<IApp>(LanguageExplorerConstants.App), m_helpTopicProvider, null);
		}
