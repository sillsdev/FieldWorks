...
namespace LanguageExplorer.Impls
{
	...
	internal sealed class ReapRamp
	{
		...
		internal static bool Installed { get; }
		...
		internal bool ArchiveNow(Form owner, Font dialogFont, Icon localizationDialogIcon, IEnumerable<string> filesToArchive, IPropertyTable propertyTable, IFlexApp thisapp, LcmCache cache)
		{
			...
			var model = new RampArchivingDlgViewModel(Application.ProductName, title, projectId, string.Empty, SetFilesToArchive(filesToArchive), GetFileDescription)
			{
			...
			var descr = cache.LanguageProject.Description;
		...
		private static string GetFileDescription(string key, string file)
		{
			switch (Path.GetExtension(file))
			{
				// TODO: Extend to supply "relationship" also (source, presentation or supporting)
				case LcmFileHelper.ksFwBackupFileExtension:
					return "FieldWorks backup";
				case FwFileExtensions.ksLexiconInterchangeFormat:
		...
		/// the RAMP package. METS = Metadata Encoding and Transmission Standard
		...
		private void AddMetsPairs(RampArchivingDlgViewModel model, string fieldWorksVersion, LcmCache cache)
		{
			...
			var fWsUsesKeyman = false;
			softwareRequirements.Add($"FieldWorks Language Explorer, Version {fieldWorksVersion} or later");

			foreach (var ws in cache.ServiceLocator.WritingSystems.CurrentVernacularWritingSystems.Union(
				cache.ServiceLocator.WritingSystems.CurrentAnalysisWritingSystems).Union(cache.ServiceLocator.WritingSystems.CurrentPronunciationWritingSystems))
			{
			...
			var domains = SilDomain.Linguistics;
