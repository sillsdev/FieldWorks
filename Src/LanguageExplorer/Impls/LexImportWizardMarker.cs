...
{
	/// <summary />
	internal sealed class LexImportWizardMarker : Form
	{
		...
		private TreeNode m_StoredTreeNode;
		...
		private ILexImportWizard _importWizard;
		...
		internal void Init(ContentMapping currentMarker, Hashtable uiLangsHT, LcmCache cache, ILexImportWizard importWizard)
		{
			_importWizard = importWizard;
			m_uiLangs = uiLangsHT;
			m_cache = cache;
			var propertyTable = importWizard.PropertyTable;
			...
			_publisher = importWizard.Publisher;
			...
			m_lblInstances.Text = string.Format(LanguageExplorerControls.ksXInstances, currentMarker.Count);
		...
		private void FillLexicalRefTypesCombo(LexImportField field)
		{
			...
			if (field.IsRef)
			{
				lblFunction.Text = LanguageExplorerControls.ksLexicalRelationType;
				rbAbbrAbbr.Checked = true;
		...
		private void btnShowInfo_Click(object sender, EventArgs e)
		{
			...
			if (panelBottom.Visible)
			{
				...
				btnShowInfo.Text = LanguageExplorerControls.ksS_howInfo;
			...
			else
			{
				...
				btnShowInfo.Text = LanguageExplorerControls.ks_HideInfo;
