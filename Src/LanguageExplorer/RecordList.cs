	...
	internal class RecordList : IRecordList
	{
		...
		public virtual void ActivateUI(bool updateStatusBar = true)
		{
			if (_subservientRecordList != null)
			{
				...
				{
					RecordListServices.SetRecordList(PropertyTable.GetValue<Form>(FwUtilsConstants.window).Handle, this);
				}
			}
			if (IsActiveInGui)
			{
		...
		{
			using (new WaitCursor(PropertyTable.GetValue<Form>(FwUtilsConstants.window)))
			{
		...
		public bool DeleteRecord(string uowBaseText, StatusBarProgressPanel panel)
		{
			...
			using (var dlg = new ConfirmDeleteObjectDlg(PropertyTable.GetValue<IHelpTopicProvider>(LanguageExplorerConstants.HelpTopicProvider)))
			{
				...
				{
					dlg.SetDlgInfo(thingToDelete, m_cache, PropertyTable, TsStringUtils.MakeString(LanguageExplorerResources.ksCannotDeleteItem, m_cache.DefaultUserWs));
				}
		...
		public bool ShouldNotHandleDeletionMessage => Id != LanguageExplorerConstants.AllReversalEntries && (!Editable || !IsPrimaryRecordList || !_shouldHandleDeletion);
		...
		protected virtual bool TryRestoreFilter()
		{
			...
			if (persistFilter != null)
			{
				try
				{
					filter = PropertyTable.GetValue<IPersistAsXmlFactory>(LanguageExplorerConstants.PersistAsXmlFactory).Create<IRecordFilter>(XDocument.Parse(persistFilter).Root);
					if (filter != null)
					{
