		...
		private void MakeDivInParaChildNotVisible(LayoutTreeNode ltn, object sender)
		{
			...
			foreach (TreeNode n in ltn.Nodes) // iterate over child nodes
			{
				...
				if (tn != null)
				{
					...
					{
						MessageBox.Show(xWorksStrings.ksRootSenseOnSubentriesGoneDlgText, xWorksStrings.ksRootSenseOnSubentriesGoneDlgLabel, MessageBoxButtons.OK, MessageBoxIcon.Information);
						tn.Checked = false;
						break;
					}
					// meant for tn.Label.Contains("Referenced Complex Form") in stem dictionary
					if (tn.Checked && tn.FlowType == "span" && tn.Label.Contains(xWorksStrings.ksReferencedComplexForm) && tn.ShowComplexFormPara)
					{
