...
namespace LanguageExplorer.UtilityTools
{
	...
	/// </summary>
	internal sealed class UtilityDlg : Form
	{
		...
		/// <summary />
		internal UtilityDlg(IHelpTopicProvider helpTopicProvider, MajorFlexComponentParameters majorFlexComponentParameters)
		{
			InitializeComponent();
			...
			// Add builtin utilities.
			foreach (var utility in utilityRepository.GetOfficialUtilities(majorFlexComponentParameters, this, ProgressBar))
			{
				Utilities.Items.Add(utility);
			}
			// Add user supplied utilities (if any).
			foreach (var utility in utilityRepository.GetUserDefinedUtilities(this))
			{
				Utilities.Items.Add(utility);
			}
			ResumeLayout();
		...
		private void m_btnRunUtils_Click(object sender, EventArgs e)
		{
