	...
	internal sealed class ChartmarkEditTool : ITool
	{
			...
			StatusBar statusbar)
		{
			_collapsingSplitContainer = CollapsingSplitContainerFactory.Create(new FlexComponentParameterObject(PropertyTable, Publisher, Subscriber), mainCollapsingSplitContainer, true,
				XDocument.Parse(ListResources.ChartmarkEditParameters).Root, XDocument.Parse(ListResources.ListToolsSliceFilters),
				MachineName,
				new PossibilityListClerkParameterObject("DiscChartMarkerList", PropertyTable.GetValue<FdoCache>("cache").LanguageProject.DiscourseDataOA.ChartMarkersOA, false, true, false, "best analysis"));
		}
