...
namespace LanguageExplorer.Areas.Lists.Tools.StatusEdit
{
	...
	internal sealed class StatusEditTool : ITool
	{
		...
		public void Activate(MajorFlexComponentParameters majorFlexComponentParameters)
		{
			...
			{
				_recordClerk = majorFlexComponentParameters.RecordClerkRepositoryForTools.GetRecordClerk(StatusList, majorFlexComponentParameters.Statusbar, FactoryMethod);
			}
		...
		{
			Require.That(clerkId == StatusList, $"I don't know how to create a clerk with an ID of '{clerkId}', as I can only create on with an id of '{StatusList}'.");

			return new RecordClerk(clerkId,
				statusBar,
				new PossibilityRecordList(cache.ServiceLocator.GetInstance<ISilDataAccessManaged>(), cache.LanguageProject.StatusOA),
