	...
	internal sealed class ComplexEntryTypeEditTool : ITool
	{
			...
			StatusBar statusbar)
		{
			_collapsingSplitContainer = CollapsingSplitContainerFactory.Create(new FlexComponentParameterObject(PropertyTable, Publisher, Subscriber), mainCollapsingSplitContainer, true,
				...
				MachineName,
				new PossibilityListClerkParameterObject("ComplexEntryTypeList", PropertyTable.GetValue<FdoCache>("cache").LanguageProject.LexDbOA.ComplexEntryTypesOA, false, true, false, "best analysis"));
		}
		...
		{
			get { return "complexEntryTypeEdit"; }
		}
