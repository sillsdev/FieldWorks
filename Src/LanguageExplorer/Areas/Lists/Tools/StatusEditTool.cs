...
{
	/// <summary>
	/// ITool implementation for the "statusEdit" tool in the "lists" area.
	/// </summary>
	[Export(LanguageExplorerConstants.ListsAreaMachineName, typeof(ITool))]
	internal sealed class StatusEditTool : IListTool
	{
		private const string StatusList = "StatusList";
		/// <summary>
		...
		public void Activate(MajorFlexComponentParameters majorFlexComponentParameters)
		{
			...
			if (_recordList == null)
			{
				_recordList = majorFlexComponentParameters.FlexComponentParameters.PropertyTable.GetValue<IRecordListRepositoryForTools>(LanguageExplorerConstants.RecordListRepository).GetRecordList(StatusList, majorFlexComponentParameters.StatusBar, FactoryMethod);
			}
			...
			_toolMenuHelper = new StatusMenuHelper(majorFlexComponentParameters, this, MyList, _recordList, dataTree);
			_collapsingSplitContainer = CollapsingSplitContainerFactory.Create(majorFlexComponentParameters.FlexComponentParameters, majorFlexComponentParameters.MainCollapsingSplitContainer,
				true, XDocument.Parse(ListResources.StatusEditParameters).Root, XDocument.Parse(ListResources.ListToolsSliceFilters), MachineName,
		...
		public string MachineName => LanguageExplorerConstants.StatusEditMachineName;

		/// <summary>
		/// User-visible localized component name.
		/// </summary>
		public string UiName => StringTable.Table.LocalizeLiteralValue(LanguageExplorerConstants.StatusEditUiName);
		...
		/// <inheritdoc />
		public ICmPossibilityList MyList => _cache.LanguageProject.StatusOA;
		#endregion
		...
		{
			Require.That(recordListId == StatusList, $"I don't know how to create a record list with an ID of '{recordListId}', as I can only create one with an id of '{StatusList}'.");
			/*
            <clerk id="StatusList">
		...
		private sealed class StatusMenuHelper : IDisposable
		{
			...
			internal StatusMenuHelper(MajorFlexComponentParameters majorFlexComponentParameters, ITool tool, ICmPossibilityList list, IRecordList recordList, DataTree dataTree)
			{
