...
namespace LanguageExplorer.Areas.Lists.Tools.RecTypeEdit
{
	...
	internal sealed class RecTypeEditTool : IListTool
	{
		...
		private RecTypeMenuHelper _toolMenuHelper;
		...
		public string MachineName => LanguageExplorerConstants.RecTypeEditMachineName;

		/// <summary>
		/// User-visible localized component name.
		/// </summary>
		public string UiName => StringTable.Table.LocalizeLiteralValue(LanguageExplorerConstants.RecTypeEditUiName);
		...
		{
			Require.That(recordListId == RecTypeList, $"I don't know how to create a record list with an ID of '{recordListId}', as I can only create one with an id of '{RecTypeList}'.");
			/*
		...
		private sealed class RecTypeMenuHelper : IDisposable
		{
			...
			internal RecTypeMenuHelper(MajorFlexComponentParameters majorFlexComponentParameters, ITool tool, ICmPossibilityList list, IRecordList recordList, DataTree dataTree)
			{
