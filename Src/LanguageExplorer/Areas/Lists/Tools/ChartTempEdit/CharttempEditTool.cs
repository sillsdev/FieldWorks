...
namespace LanguageExplorer.Areas.Lists.Tools.ChartTempEdit
{
	/// <summary>
	...
	internal sealed class CharttempEditTool : ITool
	{
			...
			StatusBar statusbar)
		{
			_collapsingSplitContainer = CollapsingSplitContainerFactory.Create(new FlexComponentParameterObject(PropertyTable, Publisher, Subscriber), mainCollapsingSplitContainer, true,
				...
				MachineName,
				new PossibilityListClerkParameterObject("DiscChartTemplateList", PropertyTable.GetValue<FdoCache>("cache").LanguageProject.DiscourseDataOA.ConstChartTemplOA, true, true, false, "best analysis"));
		}
		...
		{
			get { return "Text Constituent Chart Templates"; }
		}
