	...
	internal sealed class VariantEntryTypeEditTool : ITool
	{
			...
			StatusBar statusbar)
		{
			_collapsingSplitContainer = CollapsingSplitContainerFactory.Create(new FlexComponentParameterObject(PropertyTable, Publisher, Subscriber), mainCollapsingSplitContainer, true,
				...
				MachineName,
				new PossibilityListClerkParameterObject("VariantEntryTypeList", PropertyTable.GetValue<FdoCache>("cache").LanguageProject.LexDbOA.VariantEntryTypesOA, false, true, false, "best analysis"));
		}
		...
		{
			get { return "variantEntryTypeEdit"; }
		}
