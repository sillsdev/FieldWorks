...
namespace LanguageExplorer.Areas.Lists.Tools.ConfidenceEdit
{
	...
	internal sealed class ConfidenceEditTool : ITool
	{
		...
		public void Activate(MajorFlexComponentParameters majorFlexComponentParameters)
		{
			...
			{
				_recordClerk = majorFlexComponentParameters.RecordClerkRepositoryForTools.GetRecordClerk(ConfidenceList, majorFlexComponentParameters.Statusbar, FactoryMethod);
			}
		...
		{
			Require.That(clerkId == ConfidenceList, $"I don't know how to create a clerk with an ID of '{clerkId}', as I can only create on with an id of '{ConfidenceList}'.");

			return new RecordClerk(clerkId,
				statusBar,
				new PossibilityRecordList(cache.ServiceLocator.GetInstance<ISilDataAccessManaged>(), cache.LanguageProject.ConfidenceLevelsOA),
