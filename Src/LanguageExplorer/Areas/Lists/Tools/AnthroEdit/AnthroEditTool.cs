...
namespace LanguageExplorer.Areas.Lists.Tools.AnthroEdit
{
	...
	internal sealed class AnthroEditTool : ITool
	{
		...
		public void Activate(MajorFlexComponentParameters majorFlexComponentParameters)
		{
			...
			{
				_recordClerk = majorFlexComponentParameters.RecordClerkRepositoryForTools.GetRecordClerk(AnthropologyList, FactoryMethod);
			}
			_collapsingSplitContainer = CollapsingSplitContainerFactory.Create(
				majorFlexComponentParameters.FlexComponentParameters,
				majorFlexComponentParameters.MainCollapsingSplitContainer,
				true,
				XDocument.Parse(ListResources.AnthroEditParameters).Root,
		...
		public string MachineName => "anthroEdit";
		...
		{
			Guard.AssertThat(clerkId == AnthropologyList, $"I don't know how to create a clerk with an ID of '{clerkId}', as I can only create on with an id of '{AnthropologyList}'.");

			return new RecordClerk(clerkId,
				new PossibilityRecordList(cache.ServiceLocator.GetInstance<ISilDataAccessManaged>(), cache.LanguageProject.AnthroListOA),
