...
{
	public static class TreeBarHandlerUtils
	{
		...
		private static ICmPossibility CreateDuplicate(ICmPossibility orig, int ownerHvo, int flid, int order, LcmCache cache)
		{
			...
			UndoableUnitOfWorkHelper.DoUsingNewOrCurrentUOW(string.Format(AreaResources.ksUndoDuplicate, "Item"),
				string.Format(AreaResources.ksRedoDuplicate, "Item"), cache.ActionHandlerAccessor, () =>
				{
