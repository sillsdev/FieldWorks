...
namespace LanguageExplorer.Areas.TextsAndWords.Tools
{
	internal partial class ConcordanceControl : ConcordanceControlBase
	{
		...
		public override string AccName => "LanguageExplorer.Areas.TextsAndWords.Tools.ConcordanceControl";
		#endregion
		...
		private void UpdateButtonState()
		{
			...
			m_searchContentLabel.Text = TextAndWordsResources.ConcordanceSearchTextLabel;
			switch (sel.Line)
			{
				...
				case ConcordanceLines.kTags:
					...
					m_searchContentLabel.Text = sel.Line != ConcordanceLines.kTags ? TextAndWordsResources.ConcordanceSearchCatLabel : TextAndWordsResources.ConcordanceSearchTagLabel;
					break;
		...
		protected override List<IParaFragment> SearchForMatches()
		{
			...
			using (new WaitCursor(this))
			{
				...
				if (sMatch.Length > 1000)
				{
					...
					MessageBox.Show(TextAndWordsResources.ksMatchStringTooLong, LanguageExplorerResources.ksWarning);
		...
		private void FillLineComboList()
		{
			...
			m_cbLine.Items.Add(new ConcordLine(TextAndWordsResources.ksBaseline, WritingSystemServices.kwsVerns, ConcordanceLines.kBaseline));
			...
			m_cbLine.Items.Add(new ConcordLine(TextAndWordsResources.ksLexEntry, WritingSystemServices.kwsVerns, ConcordanceLines.kLexEntry));
			...
			m_cbLine.Items.Add(new ConcordLine(TextAndWordsResources.ksTagging, WritingSystemServices.kwsAnals, ConcordanceLines.kTags));
		...
		private void FillWritingSystemCombo(int wsMagic)
		{
			...
			switch (wsMagic)
			{
				...
				case WritingSystemServices.kwsAnal:
					m_cbWritingSystem.Items.Add(m_cache.ServiceLocator.WritingSystems.DefaultAnalysisWritingSystem);
					wsSet = m_cache.DefaultAnalWs;
					break;
				case WritingSystemServices.kwsVern:
					m_cbWritingSystem.Items.Add(m_cache.ServiceLocator.WritingSystems.DefaultVernacularWritingSystem);
					wsSet = m_cache.DefaultVernWs;
					break;
			}
			...
			foreach (var para in ParagraphsToSearch)
			{
				...
				if (matcher.Matches(para.Contents))
				{
					...
					foreach (var range in results)
					{
						...
						{
							MessageBox.Show(string.Format(TextAndWordsResources.ksShowingOnlyTheFirstXXXMatches, occurrences.Count, cPara, ParagraphsToSearch.Count), TextAndWordsResources.ksNotice, MessageBoxButtons.OK, MessageBoxIcon.Information);
							return occurrences;
						}
					}
				}
		...
		internal static List<IParaFragment> GetOccurrencesInCustomField(int flid, HashSet<IStTxtPara> paragraphsToSearch, ISilDataAccess sda, SimpleStringMatcher matcher)
		{
			var occurrences = new List<IParaFragment>();
			if (!matcher.IsValid())
			{
						...
						if (results.Count > 0)
						{
							...
							if (occurrences.Count >= MaxConcordanceMatches())
							{
								MessageBox.Show(String.Format(
										TextAndWordsResources.ksShowingOnlyTheFirstXXXMatches,
										occurrences.Count, cPara, paragraphsToSearch.Count),
									TextAndWordsResources.ksNotice,
		...
		private List<IParaFragment> GetOccurrencesOfAnalyses(IEnumerable<IAnalysis> analyses)
		{
			...
			foreach (var analysis in analyses)
			{
				...
				foreach (var seg in analysis.Wordform.OccurrencesInTexts)
				{
					...
					segs.Add(seg);
					result.AddRange(seg.GetOccurrencesOfAnalysis(analysis, int.MaxValue, true).Cast<IParaFragment>());
				}
			}
			return result;
		}
		...
		private void SetDefaultVisibilityOfItems(bool fDefault, string sConcordedOn)
		{
			...
			if (fDefault)
			{
				m_lblTop.Text = TextAndWordsResources.ksToSpecifyAConcordance_;
				m_lnkSpecify.Text = TextAndWordsResources.ksBackToConcordedItem;
				m_lnkSpecify.Enabled = m_lnkSpecify.Visible = (m_hvoMatch != 0);
			}
			else
			{
				m_lblTop.Text = string.Format(TextAndWordsResources.ksConcordedOn0, sConcordedOn);
				m_lnkSpecify.Text = TextAndWordsResources.ksSpecifyConcordanceCriteria_;
