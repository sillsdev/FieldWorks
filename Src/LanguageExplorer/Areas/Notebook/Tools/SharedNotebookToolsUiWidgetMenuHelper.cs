...
namespace LanguageExplorer.Areas.Notebook.Tools
{
	...
	internal sealed class SharedNotebookToolsUiWidgetMenuHelper : IDisposable
	{
		...
		internal ContextMenuStrip CreateBrowseViewContextMenu()
		{
			...
			ToolStripMenuItemFactory.CreateToolStripMenuItemForContextMenuStrip(menuItems, contextMenuStrip, DeleteSelectedBrowseViewObject_Clicked, string.Format(LanguageExplorerResources.Delete_selected_0, StringTable.Table.GetString("RnGenericRec", StringTable.ClassNames)));
		...
		{
			_recordList.DeleteRecord(string.Format(LanguageExplorerResources.Delete_selected_0, StringTable.Table.GetString("RnGenericRec", StringTable.ClassNames)), StatusBarPanelServices.GetStatusBarProgressPanel(_majorFlexComponentParameters.StatusBar));
		}
