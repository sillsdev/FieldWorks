...
namespace LanguageExplorer.Areas.Lexicon
{
	...
	class CircularRefBreaker : IUtility
	{
		...
		public void Process(FdoCache cache)
		{
			...
			bldr.Insert(0, String.Format(LanguageExplorerResources.ksFoundNCircularReferences, Circular, Count));
		...
		private void RemoveEntryFromLexEntryRef(ILexEntryRef ler, ILexEntry entry, StringBuilder bldrLog)
		{
			...
			bldrLog.AppendFormat(LanguageExplorerResources.ksRemovingCircularComponentLexeme, entry.HeadWord.Text, owner.HeadWord.Text);
			// If the Components list is now empty, delete the LexEntryRef
			if (ler.ComponentLexemesRS.Count == 0)
			{
				owner.EntryRefsOS.Remove(ler);
				bldrLog.AppendLine();
				bldrLog.AppendFormat(LanguageExplorerResources.ksAlsoEmptyComplexFormInfo, owner.HeadWord.Text);
			}
		}
