	...
	internal sealed class ReversalIndexEntrySlice : ViewPropertySlice, IVwNotifyChange
	{
		...
		internal class ReversalIndexEntrySliceView : RootSiteControl
		{
			...
			private int ConvertDummiesToReal(int hvoDummy)
			{
				...
				if (fChanged)
				{
					m_fdoCache.DomainDataByFlid.BeginUndoTask(LanguageExplorerResources.ksUndoSetRevEntries,
				...
				FdoCache cache)
			{
				...
				if (hvo == 0)
				{
					cache.DomainDataByFlid.BeginUndoTask(LanguageExplorerResources.ksCreateReversal,
						LanguageExplorerResources.ksRecreateReversal);
