...
namespace LanguageExplorer.LcmUi
{
	...
	internal class CmVernObjectVc : FwBaseVc
	{
		...
		{
			var wsVern = m_cache.ServiceLocator.WritingSystems.DefaultVernacularWritingSystem.Handle;
			var co = m_cache.ServiceLocator.GetInstance<ICmObjectRepository>().GetObject(hvo);
			switch (frag)
			{
				case (int)VcFrags.kfragHeadWord:
					vwenv.AddString(co is ILexEntry lexEntry ? lexEntry.HeadWord : TsStringUtils.MakeString(co.ShortName, wsVern));
					break;
