...
>>>>>>> develop:Src/xWorks/LcmXhtmlGenerator.cs
			IThreadedProgress progress = null, int entriesPerPage = EntriesPerPage)
		{
			...
			try
			{
				SavePublishedHtmlWithStyles(entryHvos, publicationDecorator, entriesPerPage, configuration, propertyTable, cache, recordList, xhtmlPath, progress);
			}
...
>>>>>>> develop:Src/xWorks/LcmXhtmlGenerator.cs
		{
			...
			var wantLetterHeaders = (entryCount > 1 || !IsLexEditPreviewOnly(publicationDecorator)) && (recordList.IsSortingByHeadword);
...
>>>>>>> develop:Src/xWorks/LcmXhtmlGenerator.cs
		...
		private static Tuple<int, int> GetPageForCurrentEntry(GeneratorSettings settings, int[] entryHvos, int entriesPerPage)
		{
			...
			var activeRecordList = settings.ReadOnlyPropertyTable.GetValue<IRecordListRepository>(LanguageExplorerConstants.RecordListRepository).ActiveRecordList;
...
>>>>>>> develop:Src/xWorks/LcmXhtmlGenerator.cs
		...
		private static string GenerateXHTMLTemplateForConfigNode(ConfigurableDictionaryNode node, LcmCache cache, string pathToField = null, bool isReferenceNode = false)
		{
			...
			using (var xw = XmlWriter.Create(bldr, new XmlWriterSettings { ConformanceLevel = ConformanceLevel.Fragment }))
			{
				...
				if (node.DictionaryNodeOptions is DictionaryNodeWritingSystemOptions wsOpts)
				{
