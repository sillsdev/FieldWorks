...
namespace LanguageExplorer.DictionaryConfiguration
{
	...
	public class LcmJsonGenerator : ILcmContentGenerator
	{
		...
		public string GenerateWsPrefixWithString(GeneratorSettings settings, bool displayAbbreviation, int wsId, string content)
		{
			return content;
		}
		...
		{
			return elementContent.StartsWith("{") ? WriteProcessedContents(elementContent, className, string.Empty, ",") : WriteProcessedContents(elementContent.TrimEnd(','), className, "{", "},");
		}
		...
		private static string WriteProcessedContents(string elementContent, string className, string begin, string end)
		{
		...
		private static void BeginArray(IFragmentWriter writer)
		{
			((JsonFragmentWriter)writer).StartArray();
		}

		private static void EndArray(IFragmentWriter writer)
		{
			((JsonFragmentWriter)writer).EndArray();
		}

		public void BeginObjectProperty(IFragmentWriter writer, bool isBlockProperty, string className)
		{
			((JsonFragmentWriter)writer).InsertPropertyName(className);
		...
		public class JsonFragmentWriter : IFragmentWriter
		{
			...
			{
				_stringWriter = new StringWriter(bldr);
			...
			{
				_jsonWriter.WritePropertyName(propName);
				_jsonWriter.WriteValue(propValue);
			}
			...
			{
				_jsonWriter.WriteRaw(jsonContent);
			}
		}
		...
		public static List<JArray> SavePublishedJsonWithStyles(int[] entriesToSave, DictionaryPublicationDecorator publicationDecorator, int batchSize,
			DictionaryConfigurationModel configuration, IPropertyTable propertyTable, string jsonPath, IThreadedProgress progress)
		{
