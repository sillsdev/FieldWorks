...
namespace LanguageExplorer.DictionaryConfiguration
{
	...
	public class DictionaryDetailsController
	{
		...
		public void LoadNode(DictionaryConfigurationModel model, ConfigurableDictionaryNode node)
		{
			...
			if (node.ReferencedNode != null) //REVIEW: make sure ReferencedNodes always have no options
			{
				...
				if (node.IsMasterParent) // node is the Master Parent
				{
					...
					{
						sharingParentsStringBuilder.Append(Environment.NewLine).Append(DictionaryConfigurationServices.BuildPathStringFromNode(sharingParent, false));
					}
					if (sharingParentsStringBuilder.Length > 0)
					{
						optionsView = new LabelOverPanel
						{
							...
							LabelToolTip = string.Format(DictionaryConfigurationStrings.SharesWithTheseNodes, nodePath, sharingParentsStringBuilder)
						};
					}
				}
				else // node is a Subordinate Parent
				{
					...
					{
						PanelContents = optionsView,
						ButtonText = DictionaryConfigurationStrings.ksConfigureNow,
						ButtonToolTip = string.Format(DictionaryConfigurationStrings.ClickToJumpTo, masterParentPath)
					};
					goToView.ButtonClicked += (sender, args) =>
					{
						SelectedNodeChanged?.Invoke(masterParent, args);
					};
					optionsView = new LabelOverPanel
					{
						PanelContents = goToView,
						LabelText = DictionaryConfigurationStrings.ThisIsConfiguredElsewhere,
						LabelToolTip = string.Format(LanguageExplorerResources.ksUsesTheSameConfigurationAs, nodePath, masterParentPath)
					};
				}
			}
			else
		...
		internal List<ListViewItem> GetListItemsAndLabel(ListIds listId, out string listLabel)
		{
			...
			{
				case ListIds.Minor:
					listLabel = LanguageExplorerResources.ksMinorEntryTypes;
					return GetMinorEntryTypes();
				case ListIds.Complex:
					listLabel = LanguageExplorerResources.ksComplexFormTypes;
					return GetComplexFormTypes();
				case ListIds.Note:
					listLabel = DictionaryConfigurationStrings.ksExtendedNoteTypes;
					return GetNoteTypes();
				case ListIds.Variant:
					listLabel = LanguageExplorerResources.ksVariantTypes;
					return GetVariantTypes();
				case ListIds.Sense:
				case ListIds.Entry:
					listLabel = LanguageExplorerResources.ksLexicalRelationTypes;
