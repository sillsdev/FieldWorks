...
namespace LanguageExplorer.DictionaryConfiguration
{
	...
	internal class DictionaryConfigurationManagerController : IFlexComponent
	{
		...
		public DictionaryConfigurationManagerController(DictionaryConfigurationManagerDlg view, List<DictionaryConfigurationModel> configurations, List<string> publications,
			string projectConfigDir, string defaultConfigDir, DictionaryConfigurationModel currentConfig)
		...
		internal DictionaryConfigurationModel CopyConfiguration(DictionaryConfigurationModel config)
		{
			...
			// generate a unique name (starting i=2 mimics old behaviour)
		...
		private void OnDeleteConfiguration(object sender, EventArgs eventArgs) // REVIEW (Hasso) 2017.01: this should be two methods, since there are two buttons.
		{
			...
			using (var dlg = new ConfirmDeleteObjectDlg(PropertyTable.GetValue<IFlexApp>(LanguageExplorerConstants.App)))
			{
				...
				if (IsConfigurationACustomizedOriginal(configurationToDelete))
				{
					dlg.TopMessage = IsConfigurationAnOriginalReversal(configurationToDelete, _cache)
		...
		{
			Guard.AgainstNull(configurationToExport, nameof(configurationToExport));
			Guard.AgainstNullOrEmptyString(destinationZipPath, nameof(destinationZipPath));
