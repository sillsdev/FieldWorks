...
namespace LanguageExplorer.Controls
{
	...
	internal sealed class MessageBoxExForm : Form
	{
		...
		/// </summary>
		private List<MessageBoxExButton> Buttons { get; } = new List<MessageBoxExButton>();

		/// <summary>
		/// Get/Set if message box allows a save response.
		/// </summary>
		internal bool AllowSaveResponse { get; set; }

		/// <summary>
		/// Get value of save response check box.
		/// </summary>
		internal bool SaveResponse => chbSaveResponse.Checked;
		...
		/// </summary>
		private bool PlayAlertSound { get; } = true;

		/// <summary>
		/// Set/Set a timeout value.
		/// </summary>
		private int Timeout { get; } = 0;

		/// <summary>
		/// Get/Set a timeout result.
		/// </summary>
		private TimeoutResult MyTimeoutResult { get; } = TimeoutResult.Default;
		...
		private void AddOkButtonIfNoButtonsPresent()
		{
			...
			{
				Text = LanguageExplorerResources.OK,
				Value = LanguageExplorerResources.OK
			};
		...
		private void DisableCloseIfMultipleButtonsAndNoCancelButton()
		{
			if (Buttons.Count > 1)
			{
				...
				foreach (var button in Buttons.Where(button => button.Text == LanguageExplorerResources.Cancel && button.Value == LanguageExplorerResources.Cancel))
				{
		...
		/// </summary>
		private sealed class MessageBoxExButton
		{
			/// <summary>
