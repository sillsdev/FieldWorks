...
namespace LanguageExplorer.Controls
{
	...
	internal class ExportDialog : Form, IFlexComponent
	{
		...
		/// <summary />
		internal ExportDialog(MajorFlexComponentParameters majorFlexComponentParameters)
			: this()
		{
		...
		private Control EnsureViewInfo()
		{
			...
			if (m_areaOrig == LanguageExplorerConstants.NotebookAreaMachineName)
			{
				...
				mainControl = new XmlDocView(XElement.Parse(LanguageExplorerResources.NotebookDocumentParameters), m_cache, m_recordList, _majorFlexComponentParameters.UiWidgetController);
			}
			else
			{
				switch (ft)
		...
		private void btnExport_Click(object sender, EventArgs e)
		{
			...
			using (EnsureViewInfo())
			{
				...
				if (fLiftExport)
				{
					...
					{
						dlg.Tag = LanguageExplorerResources.ksChooseLIFTFolderTitle; // can't set title !!??
						dlg.Description = string.Format(LanguageExplorerResources.ksChooseLIFTExportFolder, m_exportItems[0].SubItems[1].Text);
					...
					{
						sMsg = LanguageExplorerResources.ksLIFTAlreadyExists;
					...
					else
					{
						...
						if (rgfiles.Length > 0)
						{
							sMsg = LanguageExplorerResources.ksLIFTFolderNotEmpty;
			...
			using (var progressDlg = new ProgressDialogWithTask(this))
			{
				...
				catch (WorkerThreadException e)
				{
					...
					else if (e.InnerException is LiftFormatException)
					{
						...
						ErrorReporter.ReportException(new Exception(LanguageExplorerResources.ksLiftExportBugReport, e.InnerException), app.SettingsKey, app.SupportEmailAddress, this, false);
					}
					else
					{
...
#endif
			progress.Message = string.Format(LanguageExplorerResources.ksValidatingOutputFile, Path.GetFileName(outPath));
		...
		private object ExportFxt(IThreadedProgress progressDialog, object[] parameters)
		{
			...
			if (fLiftOutput)
			{
				progressDialog.Message = string.Format(LanguageExplorerResources.ksValidatingOutputFile, Path.GetFileName(outPath));
				Validator.CheckLiftWithPossibleThrow(outPath);
			}
#if DEBUG
			var dtValidate = DateTime.Now;
		...
		protected object ExportConfiguredDocView(IThreadedProgress progressDlg, object[] parameters)
		{
			...
			using (TextWriter w = new StreamWriter(outPath))
			{
				...
				if (!string.IsNullOrEmpty(ft.m_sXsltFiles))
				{
					...
					progressDlg.Message = LanguageExplorerResources.ProcessingIntoFinalForm;
		...
		protected void AddFxts(string[] filePaths)
		{
			...
			foreach (var path in filePaths)
			{
				...
				{
					description = LanguageExplorerResources.NoDescriptionForItem;
				}
		...
		/// </summary>
		private sealed class FileStreamXmlResolver : XmlUrlResolver
		{
