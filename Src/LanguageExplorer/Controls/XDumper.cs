...
namespace LanguageExplorer.Controls
{
	...
	internal  class XDumper
	{
		...
		private string m_format; //"xml", "sf"
		private XmlNode m_templateRootNode;
		private ICmObject m_rootObject;
		private bool m_outputGuids;
		private bool m_cancelNow;
		private Dictionary<string, XmlNode> m_classNameToclassNode = new Dictionary<string, XmlNode>(100);
		private Normalizer2 m_normalizer;
		...
		/// </summary>
		private Stack<int> m_openForRefStack = new Stack<int>(4);
		///// <summary>
		...
		/// </summary>
		private Dictionary<string, bool> m_dictTestVars = new Dictionary<string, bool>();

		/// <summary />
		internal XDumper(LcmCache cache) : this()
		{
			m_cache = cache;
		...
		internal void Go(ICmObject rootObject, string templateFilePath)
		{
		...
		internal void Go(ICmObject rootObject, string templateFilePath, TextWriter writer)
		{
		...
		internal void Go(ICmObject rootObject, TextWriter writer)
		{
			try
			{
				m_rootObject = rootObject;
		...
		/// </summary>
		internal bool SkipAuxFileOutput { get; set; } = false;
		...
		/// </summary>
		internal ISilDataAccess VirtualDataAccess { get; set; } = null;
		...
		private void DoTemplateElement(TextWriter contentsStream, XmlNode node)
		{
		...
		private XmlNode FindClassTemplateNode(Type type, string sClassTag)
		{
		...
		private XmlNode GetClassTemplateNode(string className)
		{
		...
		private void DumpObject(TextWriter contentsStream, ICmObject currentObject, string sClassTag)
		{
		...
		private void CollectCallElementAttributes(List<string> rgsAttrs, ICmObject currentObject, XmlNode node)
		{
		...
		/// </summary>
		private void DoCallElement(TextWriter contentsStream, ICmObject currentObject, XmlNode node)
		{
		...
		/// </summary>
		private void DoIfElement(TextWriter contentsStream, ICmObject currentObject, XmlNode node, bool fExpected)
		{
		...
		private void DoCustomElements(TextWriter contentsStream, ICmObject currentObject, XmlNode node)
		{
		...
		private void DoMultilingualStringElement(TextWriter contentsStream, ICmObject currentObject, XmlNode node, string flags)
		{
		...
		/// </summary>
		private void doCustomMultilingualStringElementSFM(TextWriter contentsStream, ICmObject currentObject, XmlNode node, string flags)
		{
			var name = XmlUtils.GetMandatoryAttributeValue(node, "name");
		...
		/// </summary>
		private void writeCustomStringAlternativeToSFM(ICmObject currentObject, int flid, CoreWritingSystemDefinition ws, string name, TextWriter contentsStream)
		{
			if (m_mapFlids.ContainsKey(flid))
			{
				flid = m_mapFlids[flid];
		...
		/// </summary>
		private void DoCustomStringElement(TextWriter contentsStream, ICmObject currentObject, XmlNode node, string flags)
		{
			var name = XmlUtils.GetMandatoryAttributeValue(node, "name");
			var flid = GetCustomFlid(node, currentObject.ClassID);
			string s = null;
		...
		/// <summary />
		private void DoNumberElement(TextWriter contentsStream, ICmObject currentObject, XmlNode node)
		{
			var name = XmlUtils.GetMandatoryAttributeValue(node, "name");
		...
		private void DoBooleanElement(TextWriter contentsStream, ICmObject currentObject, XmlNode node)
		{
			var name = XmlUtils.GetMandatoryAttributeValue(node, "name");
			var propertyName = XmlUtils.GetMandatoryAttributeValue(node, "simpleProperty");
		...
		private void DoStringElement(TextWriter contentsStream, ICmObject currentObject, XmlNode node)
		{
			var name = XmlUtils.GetMandatoryAttributeValue(node, "name");
			string sVal = null;
		...
		private void CollectElementElementAttributes(List<string> rgsAttrs, ICmObject currentObject, XmlNode node, string flags)
		{
		...
		private void DoElementElement(TextWriter contentsStream, ICmObject currentObject, XmlNode node, string flags)
		{
			var hideFlag = XmlUtils.GetOptionalAttributeValue(node, "hideFlag");
			var name = XmlUtils.GetMandatoryAttributeValue(node, "name");
			// If no hideflag was specified, or it was specified but is not in the list of flags
		...
		/// </summary>
		private void DoLiteralElement(TextWriter contentsStream, ICmObject currentObject, XmlNode node)
		{
			if (m_format == "xml")
			{
				var name = node.Name;
		...
		/// </summary>
		private void DoRefAtomicElement(TextWriter contentsStream, ICmObject currentObject, XmlNode node, string flags)
		{
		...
		/// export directory).
		/// </summary>
		internal bool ExportPicturesAndMedia { get; set; } = false;

		/// <summary>
		/// Used to make an attribute which gives the GUID, hvo, or other simple property of a referenced or owned atomic object
		/// </summary>
		private void DoAttributeIndirectElement(List<string> rgsAttrs, ICmObject currentObject, XmlNode node)
		{
		...
		private void DoAttributeElement(List<string> rgsAttrs, ICmObject currentObject, XmlNode node)
		{
			Debug.Assert(m_format == "xml");
			var propertyName = XmlUtils.GetMandatoryAttributeValue(node, "simpleProperty");
		...
		/// </summary>
		private void DoStringOutput(TextWriter outputStream, ICmObject currentObject, XmlNode node)
		{
		...
		private string GetSimplePropertyString(XmlNode node, ICmObject currentObject)
		{
			var propertyName = XmlUtils.GetMandatoryAttributeValue(node, "simpleProperty");
		...
		/// </summary>
		private void DoCommentOutput(TextWriter outputStream, XmlNode node)
		{
		...
		/// Useful when just building an XHtml page.
		/// </summary>
		private void DoXmlStringOutput(TextWriter outputStream, ICmObject currentObject, XmlNode node)
		{
			var x = GetSimplePropertyString(node, currentObject);
			if (x != null)
			{
		...
		/// </summary>
		private void DoDateAttributeOutput(List<string> rgsAttrs, ICmObject currentObject, XmlNode node)
		{
			Debug.Assert(m_format == "xml");
		...
		private int[] LoadVirtualField(ICmObject currentObject, string field)
		{
		...
		/// </summary>
		private void DoReferenceObjVectorElement(TextWriter contentsStream, ICmObject currentObject, XmlNode node)
		{
			if (m_format != "sf")
			{
		...
		/// </summary>
		private void DoReferenceVectorElement(TextWriter contentsStream, ICmObject currentObject, XmlNode node)
		{
		...
		private void ProcessSingleLexReferences(int[] rghvo, ICmObject currentObject, TextWriter contentsStream, XmlNode classNode)
		{
		...
		/// </summary>
		private void ProcessVirtualClassVector(TextWriter contentsStream, ICmObject currentObject, string property, string virtClass)
		{
		...
		/// this element.
		/// </summary>
		private void DoObjectVectorElement(TextWriter contentsStream, ICmObject currentObject, XmlNode node)
		{
		...
		/// </summary>
		private void DoObjectAtomicElement(TextWriter contentsStream, ICmObject currentObject, XmlNode node)
		{
			var property = XmlUtils.GetMandatoryAttributeValue(node, "objProperty");
			var sClassTag = XmlUtils.GetOptionalAttributeValue(node, "classtag");
		...
		private void DoGroupElement(TextWriter contentsStream, ICmObject currentObject, XmlNode node)
		{
			var property = XmlUtils.GetMandatoryAttributeValue(node, "objProperty");
		...
		/// </summary>
		private void CollectAttributes(List<string> rgsAttrs, ICmObject currentObject, XmlNode parentNode, string flags)
		{
		...
		private void DoChildren(TextWriter contentsStream, ICmObject currentObject, XmlNode parentNode, string flags)
		{
			foreach (XmlNode node in parentNode)
			{
				if (m_cancelNow)
		...
		/// </summary>
		private int GetSingleWritingSystemDescriptor(XmlNode node)
		{
		...
		private object GetMethodResult(ICmObject target, string methodName, object[] args)
		{
			var type = target.GetType();
		...
		private string GetStringOfProperty(object propertyObject, XmlNode node)
		{
		...
		private CoreWritingSystemDefinition FindWritingSystem(int handle)
		{
			return m_wsManager.Get(handle);
		}

		private ICmObject GetObjectFromProperty(ICmObject target, string property)
		{
			return (ICmObject)GetProperty(target, property);
		}

		private IEnumerable GetEnumerableFromProperty(ICmObject target, string property)
		{
			return (IEnumerable)GetProperty(target, property);
		}

		internal XmlDocument FxtDocument
		{
			get => m_fxtDocument;
			set
			{
