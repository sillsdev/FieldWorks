...
namespace LanguageExplorer.Controls
{
	...
	/// </summary>
	internal sealed class XmlDocView : ViewBase, IFindAndReplaceContext, IPostLayoutInit
	{
		// the root HVO.
		private int m_hvoOwner;
		/// <summary>
		/// Object currently being edited.
		/// </summary>
		private ICmObject m_currentObject;
		private int m_currentIndex = -1;
		private XmlSeqView m_mainView;
		...
		internal XmlDocView(XElement configurationParametersElement, LcmCache cache, IRecordList recordList, UiWidgetController uiWidgetController, string configureText = null)
		...
		internal DictionaryPublicationDecorator GetPubDecorator()
		{
		...
		protected override void SetInfoBarText()
		{
			...
			if (match.Success)
			{
				...
				if (match.Groups[1].Value == LanguageExplorerConstants.SelectedPublication)
				{
					...
					{
						replacement = LanguageExplorerResources.ksAllEntries;
					}
				}
				else
				{
		...
		private string MakePublicationTitlePart(string titleStr)
		{
			...
			if (string.IsNullOrEmpty(m_currentPublication) || m_currentPublication == LanguageExplorerResources.AllEntriesPublication)
			{
				...
				titleStr = LanguageExplorerResources.ksAllEntries;
				// Limit length of Publication title to half of available width
				titleStr = TrimToMaxPixelWidth(maxPublicationTitleWidth, titleStr);
			}
			else
			{
				titleStr = string.Format(LanguageExplorerResources.ksPublicationEntries, GetPublicationName(), titleStr);
				titleStr = TrimToMaxPixelWidth(maxPublicationTitleWidth, titleStr);
			}
			return titleStr;
		}
		...
		internal bool OnRecordNavigation(object argument)
		{
		...
		protected override void OnMouseMove(MouseEventArgs e)
		{
			...
			else
			{
				if (m_tooltip == null)
				{
					...
					m_tooltip.SetToolTip(m_mainView, LanguageExplorerResources.ksCtrlClickJumpTooltip);
				}
				if (m_tooltip.Active)
				{
					...
					{
						m_tooltip.Show(LanguageExplorerResources.ksCtrlClickJumpTooltip, m_mainView, relLocation.X, relLocation.Y, 2000);
		...
		private static ICmObject GetTarget(ICmObject obj, int clsid)
		{
		...
		private bool IsRootBasedView => !string.IsNullOrEmpty(m_currentConfigView) && m_currentConfigView.Split(new[] { "#" }, StringSplitOptions.None)[0] == ksRootBasedPrefix;
		...
		/// </summary>
		internal void PrintFromDetail(PrintDocument pd, int recordHvo)
		{
		...
		/// </summary>
		internal void OnSelectionChanged(object sender, FwObjectSelectionEventArgs e)
		{
