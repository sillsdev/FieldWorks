...
namespace LanguageExplorer.Controls
{
	...
	/// systems.
	/// </summary>
	internal sealed class TranslatedListsExporter
	{
		...
		/// <summary />
		internal TranslatedListsExporter(List<ICmPossibilityList> lists, List<int> writingSystems, IProgress progress)
		{
			m_lists = lists;
			if (m_lists.Count > 0)
			{
		...
		/// </summary>
		internal void ExportLists(string outputFile)
		{
			if (m_wsEn == 0)
			{
				return;
