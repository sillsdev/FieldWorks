...
namespace LanguageExplorer.Controls
{
	...
	public class MSAGroupBox : UserControl
	{
				...
				try
				{
					...
					switch (value)
					{
						...
						case MsaType.kStem:
							{
								...
								m_lMainCat.Text = LanguageExplorerControls.ksCategor_y;
								...
								m_mainPOSPopupTreeManager.SetEmptyLabel(LanguageExplorerControls.ks_NotSure_);
						...
						case MsaType.kUnclassified:
							{
								m_groupBox.SuspendLayout();
								m_lMainCat.Text = LanguageExplorerControls.ksAttachesToCategor_y;
								// Hide the controls we don't want, show the ones we do.
								...
								m_mainPOSPopupTreeManager.SetEmptyLabel(LanguageExplorerControls.ksAny);
								m_groupBox.ResumeLayout();
								break;
							}
						case MsaType.kInfl:
							{
								m_groupBox.SuspendLayout();
								m_lMainCat.Text = LanguageExplorerControls.ksAttachesToCategor_y;
								m_lSLots.Text = LanguageExplorerControls.ks_FillsSlot;
								// Hide the controls we don't want, show the ones we do.
								m_lAfxType.Visible = true;
								m_fwcbAffixTypes.Visible = true;
						...
						case MsaType.kDeriv:
							{
								m_groupBox.SuspendLayout();
								m_lMainCat.Text = LanguageExplorerControls.ksAttachesToCategor_y;
								m_lSLots.Text = LanguageExplorerControls.ksC_hangesToCategory;
								// Hide the controls we don't want, show the ones we do.
								m_lAfxType.Visible = true;
								m_fwcbAffixTypes.Visible = true;
								m_lSLots.Visible = true;
		...
		public void Initialize(LcmCache cache, FlexComponentParameters flexComponentParameters, Form parentForm, SandboxGenericMSA sandboxMSA)
		{
			...
			m_fwcbAffixTypes.Items.Add(TsStringUtils.MakeString(LanguageExplorerControls.ksNotSure, defUserWs));
			m_fwcbAffixTypes.Items.Add(TsStringUtils.MakeString(LanguageExplorerControls.ksInflectional, defUserWs));
			m_fwcbAffixTypes.Items.Add(TsStringUtils.MakeString(LanguageExplorerControls.ksDerivational, defUserWs));
		...
		private void HandleComboMSATypesChange(object sender, EventArgs ea)
		{
			...
			if (label == LanguageExplorerControls.ksNotSure)
			{
				MSAType = MsaType.kUnclassified;
			}
			else if (label == LanguageExplorerControls.ksInflectional)
			{
				MSAType = MsaType.kInfl;
			}
			else if (label == LanguageExplorerControls.ksDerivational)
			{
