...
namespace LanguageExplorer.Controls
{
	...
	/// nicer to the users.
	/// </summary>
	internal sealed partial class XmlDocConfigureDlg : Form, ILayoutConverter
	{
		...
		/// </summary>
		internal bool MasterRefreshRequired { get; set; }
		...
		/// </summary>
		internal void SetConfigDlgInfo(XElement configurationParameters, LcmCache cache, LcmStyleSheet styleSheet, IFwMainWnd mainWindow, IPropertyTable propertyTable, IPublisher publisher, string sLayoutPropertyName)
		{
			...
			if (!string.IsNullOrEmpty(labelKey))
			{
				...
				switch (labelKey)
				{
					case "ksDictionaryView":
						sLabel = LanguageExplorerResources.ksDictionaryView;
						break;
				}
		...
		private void m_tvParts_AfterCheck(object sender, TreeViewEventArgs e)
		{
			if (e.Node == m_current)
			{
		...
		private void OnListItemsItemCheck(object sender, ItemCheckEventArgs e)
		{
			switch (m_listType)
			{
				case ListViewContent.WritingSystems:
		...
		private void DisplayCurrentNodeDetails()
		{
			...
			if (tnParent != null)
			{
				...
				sHeading = string.Format(LanguageExplorerResources.ksHierarchyLabel, tnParent.Text, sHeading);
			}
			...
			if (m_lblPanel.Location.X + m_lblPanel.PreferredWidth > panel1.ClientSize.Width - (panel1.Margin.Left + panel1.Margin.Right))
			{
				...
				m_tbPanelHeader.Text = tnParent != null ? string.Format(LanguageExplorerResources.ksHierarchyLabel2, tnParent.Text, m_current.Text) : sHeading;
		...
		private static void MakeDivInParaChildNotVisible(LayoutTreeNode ltn, object sender)
		{
			...
			if (!(sender is ConfigSenseLayout configSenseLayout))
			{
				return;
			}
			if (configSenseLayout.DisplaySenseInPara)
			{
				return;
			}
			...
			foreach (TreeNode n in ltn.Nodes) // iterate over child nodes
			{
				...
				{
					MessageBox.Show(LanguageExplorerResources.ksRootSenseOnSubentriesGoneDlgText, LanguageExplorerResources.ksRootSenseOnSubentriesGoneDlgLabel, MessageBoxButtons.OK, MessageBoxIcon.Information);
					tn.Checked = false;
					break;
				}
				// meant for tn.Label.Contains("Referenced Complex Form") in stem dictionary
				if (tn.Checked && tn.FlowType == "span" && tn.Label.Contains(LanguageExplorerResources.ksReferencedComplexForm) && tn.ShowComplexFormPara)
				{
		...
		private void StoreSenseConfigData(LayoutTreeNode ltn)
		{
			...
			else
			{
				...
				if (ltn.NumFont == LanguageExplorerResources.ksUnspecified)
				{
		...
		private void DisplayDetailsForAParentNode(bool fEnabled)
		{
			...
			var sMoreDetail = string.Format(LanguageExplorerResources.ksCanBeConfiguredInMoreDetail, m_current.Text);
		...
		private void ShowComplexFormRelatedControls(bool fChecked)
		{
			...
			if (fChecked)
			{
				m_lblCharStyle.Text = LanguageExplorerResources.ksParagraphStyle;
		...
		private void DisplayWritingSystemControls(bool fEnabled)
		{
			...
			m_lblItemsList.Text = LanguageExplorerResources.ksWritingSystems;
		...
		private void DisplayBeforeStyleControls(bool fEnabled)
		{
			if (m_current.AllowBeforeStyle)
			{
				...
				if (fParaStyles)
				{
					m_lblBeforeStyle.Text = LanguageExplorerResources.ksParagraphStyleForBefore;
				...
				{
					m_lblBeforeStyle.Text = LanguageExplorerResources.ksCharacterStyleForBefore;
		...
		private void DisplayContextControls(bool fEnabled)
		{
			...
			{
				m_lblBefore.Text = LanguageExplorerResources.ksBefore;
