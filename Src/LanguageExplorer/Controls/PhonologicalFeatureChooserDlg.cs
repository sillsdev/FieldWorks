...
namespace LanguageExplorer.Controls
{
	...
	public class PhonologicalFeatureChooserDlg : Form
	{
		...
		private void PhonologicalFeatureChooserDlg_Resize(object sender, EventArgs e)
		{
			PositionValuesCombo();
		}
		...
		private void UpdateFeatureConstraints()
		{
			...
			foreach (var hvoClosedFeature in featureHvos)
			{
				...
				else
				{
					...
					if (str == LanguageExplorerControls.ksFeatConstrAgree)
					{
					...
					else if (str == LanguageExplorerControls.ksFeatConstrDisagree)
					{
		...
		private static bool ContainsFeature(IEnumerable<IPhFeatureConstraint> vars, IFsFeatDefn feat)
		{
		...
		private void PopulateValuesCombo()
		{
			...
			{
				m_valuesCombo.Items.Add(LanguageExplorerControls.ksFeatConstrAgree);
				index++;
				m_valuesCombo.Items.Add(LanguageExplorerControls.ksFeatConstrDisagree);
				index++;
				...
				for (var i = feat.ValuesOC.Count; i < m_valuesCombo.Items.Count; i++)
				{
					...
					if (str == comboStr || (string.IsNullOrEmpty(str) && comboStr == LanguageExplorerControls.ks_DontCare_))
					{
			...
			m_valuesCombo.Items.Add(ShowIgnoreInsteadOfDontCare ? LanguageExplorerControls.ks_Ignore_ : LanguageExplorerControls.ks_DontCare_);
		...
		private void m_valuesCombo_SelectedIndexChanged(object sender, EventArgs e)
		{
				...
				if (fs != null)
				{
					...
					else
					{
						if (m_ctxt != null && ShowFeatureConstraintValues)
						{
							...
							{
								str = LanguageExplorerControls.ksFeatConstrAgree;
							}
							else if (ContainsFeature(m_ctxt.MinusConstrRS, feat))
							{
								str = LanguageExplorerControls.ksFeatConstrDisagree;
							}
							else
							{
								str = string.Empty;
		...
		private void PhonologicalFeatureChooserDlg_Closing(object sender, System.ComponentModel.CancelEventArgs e)
		{
			if (DialogResult == DialogResult.OK)
			{
				...
				{
					UndoableUnitOfWorkHelper.DoUsingNewOrCurrentUOW(LanguageExplorerControls.ksUndoSelectionOfPhonologicalFeatures,
						LanguageExplorerControls.ksRedoSelectionOfPhonologicalFeatures, m_cache.ActionHandlerAccessor, () =>
						{
							if (FS == null)
							{
		...
		private sealed class PhonologicalFeaturePublisher : ObjectListPublisher
		{
			...
			private Dictionary<int, int> m_objProps;
			private LcmCache m_cache;
