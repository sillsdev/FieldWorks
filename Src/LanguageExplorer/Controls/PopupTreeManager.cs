...
namespace LanguageExplorer.Controls
{
	...
	public abstract class PopupTreeManager : IDisposable
	{
		...
		private HvoTreeNode m_lastConfirmedNode;
		/// <summary>
		...
		public void SetEmptyLabel(string label)
		{
			...
			if (TreeCombo != null && TreeCombo.SelectedNode == m_kEmptyNode)
			{
				TreeCombo.Tss = m_kEmptyNode.Tss;
			}
		}
		...
		/// </summary>
		protected int WritingSystem { get; private set; }

		/// <summary>
		/// Typically the parent indicated to the constructor; see Init() for other possibilities.
		/// </summary>
		protected Form ParentForm { get; private set; }
		...
		private string TimberLine
		{
			get
			{
				...
				{
					var fnt = GetPopupTree().Font;
					using (var g = GetPopupTree().CreateGraphics())
					{
						var width = GetPopupTree().Width - 18;
						var sz = g.MeasureString(line + "-", fnt);
		...
		public void LoadPopupTree(int hvoSpecifiedTarget)
		{
			...
			try
			{
				...
				if (popupTree != null)
				{
					...
					var hvoTarget = hvoSpecifiedTarget;
					if (hvoSpecifiedTarget == 0 && popupTree.SelectedNode is HvoTreeNode)
					{
					...
					var match = MakeMenuItems(popupTree, hvoTarget);
		...
		/// </summary>
		protected TreeNode AddNodes(TreeNodeCollection nodes, int hvoOwner, int flid, int hvoTarget, int tagName)
		{
			return AddNodes(nodes, hvoOwner, flid, CmPossibilityTags.kflidSubPossibilities, hvoTarget, tagName);
		}

		/// <summary />
		protected TreeNode AddNodes(TreeNodeCollection nodes, int hvoOwner, int flid, int flidSub, int hvoTarget, int tagName)
		{
			TreeNode result = null;
			var chvo = Cache.MainCacheAccessor.get_VecSize(hvoOwner, flid);
			for (var i = 0; i < chvo; i++)
			{
				...
				var tssLabel = WritingSystemServices.GetMagicStringAlt(Cache, WritingSystemServices.kwsFirstAnalOrVern, hvoChild, tagName)
				               ?? TsStringUtils.MakeString(LanguageExplorerControls.ksStars, Cache.WritingSystemFactory.UserWs);
		...
		{
			popupTree.Nodes.Add(new HvoTreeNode(TsStringUtils.MakeString(LanguageExplorerControls.ksMore_, Cache.WritingSystemFactory.UserWs), kMore));
		}
		...
		{
			var empty = new HvoTreeNode(TsStringUtils.MakeString(LanguageExplorerControls.ks_NotSure_, Cache.WritingSystemFactory.UserWs), kEmpty);
		...
		protected void SetComboTextToLastConfirmedSelection()
		{
			...
			{
				TreeCombo.Tss = m_lastConfirmedNode.Tss;
			}
		}
		...
		protected virtual void m_treeCombo_AfterSelect(object sender, TreeViewEventArgs e)
		{
			...
			if (e.Action == TreeViewAction.ByMouse)
			{
				...
				if (TreeCombo != null)
				{
					if (TreeCombo.Text != m_lastConfirmedNode.Text)
					{
						TreeCombo.Tss = m_lastConfirmedNode.Tss;
					}
				}
				AfterSelect?.Invoke(this, new TreeViewEventArgs(m_lastConfirmedNode, e.Action));
