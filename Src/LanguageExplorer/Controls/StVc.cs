...
namespace LanguageExplorer.Controls
{
	...
	internal class StVc : FwBaseVc
	{
		...
		/// </summary>
		public virtual bool BaseDirectionOnParaContents => false;
		...
		protected ITsTextProps NormalStyle
		{
			get
			{
				if (m_ttpNormal == null)
				{
					...
					tsPropsBuilder.SetStrPropValue((int)FwTextPropType.ktptNamedStyle, StyleServices.NormalStyleName);
		...
		public override int DefaultWs
		{
			set
			{
				...
				{
					var defWs = m_cache.ServiceLocator.WritingSystemManager.Get(value);
		...
		/// </summary>
		public bool PrintLayout { get; set; } = false;
		...
		protected bool HandleEmptyText(IVwEnv vwenv, int hvo)
		{
			...
			var ws = vwenv.DataAccess.WritingSystemFactory.UserWs;
			var tssMissing = TsStringUtils.EmptyString(ws);
			vwenv.set_IntProperty((int)FwTextPropType.ktptParaColor, (int)FwTextPropVar.ktpvDefault, m_BackColor.ToArgb());
			...
			vwenv.set_IntProperty((int)FwTextPropType.ktptBaseWs, (int)FwTextPropVar.ktpvDefault, m_wsDefault);
		...
		protected static void ApplyParagraphStyleProps(IVwEnv vwenv, int paraHvo, StVc vc)
		{
			...
			if (vc.DefaultParaStyle.Length > 0)
			{
				...
				vwenv.set_IntProperty((int)FwTextPropType.ktptParaColor, (int)FwTextPropVar.ktpvDefault, vc.BackColor.ToArgb());
			}
		...
		/// </summary>
		protected void InsertParagraphBody(IVwEnv vwenv, int paraHvo, int frag, bool fApplyProps, ContentTypes contentType, StVc vc)
		{
		...
		protected virtual void SetupWsAndDirectionForPara(IVwEnv vwenv, int paraHvo)
		{
			...
			vwenv.set_IntProperty((int)FwTextPropType.ktptRightToLeft, (int)FwTextPropVar.ktpvEnum, fIsRightToLeftPara ? (int)FwTextToggleVal.kttvForceOn : (int)FwTextToggleVal.kttvOff);
		}
		...
		protected void GetWsAndDirectionForPara(int paraHvo, out bool fIsRightToLeftPara, out int wsPara)
		{
			...
			if (BaseDirectionOnParaContents)
			{
				...
				var para = Cache.ServiceLocator.GetInstance<IStTxtParaRepository>().GetObject(paraHvo);
				wsPara = TsStringUtils.GetWsAtOffset(para.Contents, 0);
				if (wsPara <= 0)
				{
					wsPara = DefaultWs;
				}
		...
		public override void Display(IVwEnv vwenv, int hvo, int frag)
		{
			switch (frag)
			{
				case (int)StTextFrags.kfrFootnote:
					{
						...
						var footnote = Cache.ServiceLocator.GetInstance<IStFootnoteRepository>().GetObject(hvo);
				...
				case (int)StTextFrags.kfrText:
					{
						...
						{
							vwenv.AddObjVecItems(StTextTags.kflidParagraphs, this, (int)StTextFrags.kfrPara);
						}
						break;
					}
				...
				case (int)StTextFrags.kfrTranslation:
					{
						...
						var trans = Cache.ServiceLocator.GetInstance<ICmTranslationRepository>().GetObject(hvo);
				...
				case (int)StTextFrags.kfrSegmentFreeTranslations:
					...
					var seg = Cache.ServiceLocator.GetInstance<ISegmentRepository>().GetObject(hvo);
					if (seg.IsLabel)
					{
					...
					else
					{
						...
						vwenv.set_IntProperty((int)FwTextPropType.ktptBackColor, (int)FwTextPropVar.ktpvDefault, (int)ColorUtil.ConvertColorToBGR(SystemColors.Window));
						var tssVal = seg.FreeTranslation.get_String(BackTranslationWS);
		...
		/// </summary>
		protected ITsString OneSpaceString => m_oneSpaceString ?? (m_oneSpaceString = TsStringUtils.MakeString(" ", DefaultWs));
		...
		{
			return HeightEstimator?.EstimateHeight(hvo, frag, dxpAvailWidth) ?? 120;
		}
		...
		{
			var para = m_cache.ServiceLocator.GetInstance<IStTxtParaRepository>().GetObject(paraHvo);
