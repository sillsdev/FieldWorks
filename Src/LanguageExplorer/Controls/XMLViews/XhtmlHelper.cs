...
namespace LanguageExplorer.Controls.XMLViews
{
	...
	public class XhtmlHelper
	{
		...
		private SortedDictionary<string, XElement> m_mapCssClassToXnode = new SortedDictionary<string, XElement>();
		...
		public void WriteCssFile(string sOutputFile, IVwStylesheet vss, CssType type, IPublication pub)
		{
			...
			{
				m_writer.WriteLine("/* Cascading Style Sheet generated by {0} version {1} on {2} {3} */", Assembly.GetEntryAssembly().GetName().Name, Assembly.GetEntryAssembly().GetName().Version,
			...
			switch (sAttr)
			{
				...
				case "color":
					if (fi.m_fontColor.ValueIsSet)
					{
						...
						{
							m_writer.WriteLine("    color: rgb({0},{1},{2});{3}", fi.m_fontColor.Value.R, fi.m_fontColor.Value.G, fi.m_fontColor.Value.B, sInheritance);
						}
						return true;
					}
