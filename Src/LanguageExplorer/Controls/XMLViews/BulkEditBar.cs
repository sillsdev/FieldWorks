...
using SIL.Xml;
using XmlSerializationHelper = SIL.Xml.XmlSerializationHelper;
...
namespace LanguageExplorer.Controls.XMLViews
{
	...
	public class BulkEditBar : UserControl
	{
		...
		XElement m_configurationNode = null;
		/// <summary>
		/// Browse viewer
		/// </summary>
		...
		int GetFlidFromClassDotName(XElement node, string attrName)
		{
...
#region Component Designer generated code
		...
		ReplaceWithMethod MakeReplaceWithMethod(out int newCol)
		{
			...
			return new ReplaceWithMethod(m_cache, m_bv.SpecialCache, fci.Accessor,
				m_bv.ColumnSpecs[fci.ColumnIndex], m_pattern, m_tssReplace);
		}
...
#region BulkEditBar helper methods
			...
			{
				return XmlSerializationHelper.SerializeToString(this, true);
			}
			...
			static private BulkEditTabPageSettings DeserializeTabPageSettings(BulkEditBar bulkEditBar, string tabSettingsKey)
			{
				...
				if (settingsXml.Length > 0)
				{
					...
					string className = doc.Root.Name.LocalName;
					...
					// deserialize
					restoredTabPageSettings = (BulkEditTabPageSettings)SIL.FieldWorks.Common.FwUtils.XmlSerializationHelper.DeserializeXmlString(settingsXml, targetType);
				}
...
#region Tab properties to serialize
		...
		public class BulkReplaceTabPageSettings : BulkEditTabPageSettings
		{
			...
			protected override void SaveSettings(BulkEditBar bulkEditBar)
			{
				...
				string patternAsXml = XmlSerializationHelper.SerializeToString(patternSettings);
				...
				get
				{
					if (m_pattern == null && CanLoadFromBulkEditBar())
					{
						...
						else
						{
							...
							VwPatternSerializableSettings settings = (VwPatternSerializableSettings)SIL.FieldWorks.Common.FwUtils.XmlSerializationHelper.DeserializeXmlString(patternAsXml, typeof(VwPatternSerializableSettings));
							if (settings != null)
...
//			FieldComboItem newSelection = AddStringFieldItemsToCombo(combo, selectedItem, fIsSourceCombo);
		...
		private bool IsColumnWsBothVernacularAndAnalysis(XElement node)
		{
		...
		private void m_clickCopyTargetCombo_SelectedIndexChanged(object sender, EventArgs e)
		{
			...
			if (selectedItem != null)
			{
				...
				if (!String.IsNullOrEmpty(m_sClickEditIf))
				{
					...
					string sWs = StringServices.GetWsSpecWithoutPrefix(FwUtils.ConvertElement(spec));
					if (sWs != null)
	...
	internal abstract class FieldReadWriter : IGhostable
	{
		...
		static public FieldReadWriter Create(XElement node, LcmCache cache, int hvoRootObj)
		{
			...
			if (clid == 0)
				return null;
			int flid = mdc.GetFieldId2(clid, fieldName, true);
			int ws = WritingSystemServices.GetWritingSystem(cache, FwUtils.ConvertElement(node), null, hvoRootObj, flid, 0).Handle;
			if (parts.Length == 2)
			{
				FieldReadWriter frw;
