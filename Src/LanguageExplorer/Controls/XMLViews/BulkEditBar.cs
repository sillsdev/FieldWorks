...
namespace LanguageExplorer.Controls.XMLViews
{
	...
	internal class BulkEditBar : UserControl, IPropertyTableProvider, IPublisherProvider, ISubscriberProvider
	{
		...
		private void InitializeComponent()
		{
			...
			this.ListChoiceChangeToCombo = new SIL.FieldWorks.FwCoreDlgs.FwOverrideComboBox();
			this.ChangeToLabel = new System.Windows.Forms.Label();
			this.m_listChoiceTargetCombo = new SIL.FieldWorks.FwCoreDlgs.FwOverrideComboBox();
			...
			this.BulkCopySourceCombo = new SIL.FieldWorks.FwCoreDlgs.FwOverrideComboBox();
			this.label4 = new System.Windows.Forms.Label();
			this.BulkCopyTargetCombo = new SIL.FieldWorks.FwCoreDlgs.FwOverrideComboBox();
			...
			this.ClickCopyTargetCombo = new SIL.FieldWorks.FwCoreDlgs.FwOverrideComboBox();
			this.label6 = new System.Windows.Forms.Label();
			this.TransduceTab = new System.Windows.Forms.TabPage();
			this.m_transduceSetupButton = new System.Windows.Forms.Button();
			this.TransduceSourceCombo = new SIL.FieldWorks.FwCoreDlgs.FwOverrideComboBox();
			this.label7 = new System.Windows.Forms.Label();
			this.TransduceTargetCombo = new SIL.FieldWorks.FwCoreDlgs.FwOverrideComboBox();
			this.label8 = new System.Windows.Forms.Label();
			this.TransduceProcessorCombo = new SIL.FieldWorks.FwCoreDlgs.FwOverrideComboBox();
			...
			this.FindReplaceTargetCombo = new SIL.FieldWorks.FwCoreDlgs.FwOverrideComboBox();
			this.label12 = new System.Windows.Forms.Label();
			this.m_deleteTab = new System.Windows.Forms.TabPage();
			this.label13 = new System.Windows.Forms.Label();
			this.DeleteWhatCombo = new SIL.FieldWorks.FwCoreDlgs.FwOverrideComboBox();
		...
		/// POS set).
		/// </summary>
		private sealed class InflectionFeatureEditor : IBulkEditSpecControl, IDisposable
		{
			...
			private IBulkEditSpecControl AsIBulkEditSpecControl => this;
			public event FwSelectionChangedEventHandler ValueChanged;
			...
			internal InflectionFeatureEditor(IPublisher publisher, ISubscriber subscriber, XElement configurationNode)
				: this()
			{
			...
			/// Get/Set the property table.
			/// </summary>
			IPropertyTable IBulkEditSpecControl.PropertyTable { get; set; }

			/// <summary>
			/// Get or set the cache. Must be set before the tree values need to load.
			/// </summary>
			LcmCache IBulkEditSpecControl.Cache
			{
				get => m_cache;
				set
				{
			...
			/// The special cache that can handle the preview and check-box properties.
			/// </summary>
			XMLViewsDataCache IBulkEditSpecControl.DataAccess
			{
				get => m_sda ?? throw new InvalidOperationException("Must set the special cache of a BulkEditSpecControl");
				set => m_sda = value;
			}
			...
			/// </summary>
			Control IBulkEditSpecControl.Control => m_tree;

			private void m_tree_TreeLoad(object sender, EventArgs e)
			{
				if (m_InflectionFeatureTreeManager == null)
				{
					m_InflectionFeatureTreeManager = new InflectionFeaturePopupTreeManager(m_tree, m_cache, false, new FlexComponentParameters(AsIBulkEditSpecControl.PropertyTable, m_publisher, _subscriber), AsIBulkEditSpecControl.PropertyTable.GetValue<Form>(FwUtilsConstants.window), m_displayWs);
					m_InflectionFeatureTreeManager.AfterSelect += m_pOSPopupTreeManager_AfterSelect;
				}
			...
			/// </summary>
			IVwStylesheet IBulkEditSpecControl.Stylesheet
			{
				set { }
			}
			...
			/// </summary>
			Button IBulkEditSpecControl.SuggestButton => null;
			...
			/// </summary>
			void IBulkEditSpecControl.DoIt(IEnumerable<int> itemsToChange, ProgressState state)
			{
				var pos = GetPOS();
				...
				if (m_selectedHvo != 0)
				{
					fsTarget = AsIBulkEditSpecControl.Cache.ServiceLocator.GetInstance<IFsFeatStrucRepository>().GetObject(m_selectedHvo);
				}
			...
			/// </summary>
			bool IBulkEditSpecControl.CanClearField => false;

			/// <summary>
			/// Not needed since we said we can't do it.
			/// </summary>
			void IBulkEditSpecControl.SetClearField()
			{
				throw new NotSupportedException();
			}
			...
			/// for each item in the set. Disable items that can't be set.
			/// </summary>
			void IBulkEditSpecControl.FakeDoit(IEnumerable<int> itemsToChange, int tagMadeUpFieldIdentifier, int tagEnable, ProgressState state)
			{
			...
			/// Used by SemanticDomainChooserBEditControl to make suggestions and then call FakeDoIt
			/// </summary>
			void IBulkEditSpecControl.MakeSuggestions(IEnumerable<int> itemsToChange, int tagMadeUpFieldIdentifier, int tagEnabled, ProgressState state)
			{
				throw new NotSupportedException();
			}

			List<int> IBulkEditSpecControl.FieldPath => new List<int>(new[] { LexSenseTags.kflidMorphoSyntaxAnalysis, MoStemMsaTags.kflidPartOfSpeech, MoStemMsaTags.kflidMsFeatures });
			...
			private sealed class InflectionFeaturePopupTreeManager : PopupTreeManager
			{
				...
				/// <summary />
				internal InflectionFeaturePopupTreeManager(TreeCombo treeCombo, LcmCache cache, bool useAbbr, FlexComponentParameters flexComponentParameters, Form parent, int wsDisplay)
		...
		/// POS set).
		/// </summary>
		private sealed class InflectionClassEditor : IBulkEditSpecControl, IDisposable
		{
			private TreeCombo m_tree;
			private LcmCache m_cache;
			private IPublisher m_publisher;
			...
			internal InflectionClassEditor(IPublisher publisher, ISubscriber subscriber, XElement configurationNode)
				: this()
			{
				m_publisher = publisher;
				_subscriber = subscriber;
			...
			private void m_tree_TreeLoad(object sender, EventArgs e)
			{
				if (m_InflectionClassTreeManager == null)
				{
					m_InflectionClassTreeManager = new InflectionClassPopupTreeManager(m_tree, m_cache, new FlexComponentParameters(AsIBulkEditSpecControl.PropertyTable, m_publisher, _subscriber), false, AsIBulkEditSpecControl.PropertyTable.GetValue<Form>(FwUtilsConstants.window), m_displayWs);
					m_InflectionClassTreeManager.AfterSelect += m_pOSPopupTreeManager_AfterSelect;
				}
			...
			List<int> IBulkEditSpecControl.FieldPath => new List<int>(new[] { LexSenseTags.kflidMorphoSyntaxAnalysis, MoStemMsaTags.kflidPartOfSpeech, MoStemMsaTags.kflidInflectionClass });
