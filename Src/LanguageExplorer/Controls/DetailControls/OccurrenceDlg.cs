...
{
	internal sealed class OccurrenceDlg : Form
	{
		...
		internal OccurrenceDlg(IHelpTopicProvider helpTopicProvider, int min, int max, bool paren)
		{
			InitializeComponent();
			if (paren)
			{
		...
		internal int Minimum => m_cboMin.SelectedIndex;

		internal int Maximum => m_cboMax.SelectedIndex == 0 ? -1 : m_cboMax.SelectedIndex;
		...
		private void m_btnOK_Click(object sender, EventArgs e)
		{
			...
			{
				MessageBox.Show(LanguageExplorerControls.ksContextOccurrenceDlgError);
			}
		}
