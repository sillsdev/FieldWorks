...
namespace LanguageExplorer.Controls.DetailControls
{
	...
	internal class IhMissingEntry : InterlinComboHandler
	{
		...
		private bool m_fHideCombo = true;
		...
		public override void SetupCombo()
		{
			...
			AddItemToComboList(LanguageExplorerResources.ksCreateNewEntry_, OnSelectCreateNewEntry, disabledItemProperties, disabledItemProperties == null);
			AddItemToComboList(LanguageExplorerResources.ksVariantOf_, OnSelectVariantOf, disabledItemProperties, disabledItemProperties == null);
			...
			foreach (var mi in MorphItems)
			{
				...
				else
				{
					...
					if (miPrev.m_hvoMorph != 0 && mi.m_hvoMorph == miPrev.m_hvoMorph && miPrev.m_hvoSense > 0)
					{
						...
						tisb.Append(LanguageExplorerResources.ksAddNewSense_);
						tssToDisplay = tisb.GetString();
					}
					else
					{
						...
						var sFmt = LanguageExplorerResources.ksAddNewSenseForX_;
						tisb.Clear();
						tisb.SetIntPropValues((int)FwTextPropType.ktptSuperscript, (int)FwTextPropVar.ktpvEnum, (int)FwSuperscriptVal.kssvOff);
						tisb.SetIntPropValues((int)FwTextPropType.ktptBold, (int)FwTextPropVar.ktpvEnum, (int)FwTextToggleVal.kttvOff);
						tisb.SetIntPropValues((int)FwTextPropType.ktptWs, 0, m_wsUser);
		...
		private void AddUnknownLexEntryToComboList()
		{
			...
			tisb.SetIntPropValues((int)FwTextPropType.ktptSuperscript, (int)FwTextPropVar.ktpvEnum, (int)FwSuperscriptVal.kssvOff);
			tisb.SetIntPropValues((int)FwTextPropType.ktptWs, 0, m_wsUser);
			tisb.Append(LanguageExplorerResources.ksUnknown);
		...
		internal void CreateNewEntry(bool fCreateNow, out ILexEntry le, out IMoForm allomorph, out ILexSense sense)
		{
			...
			if (allomorph == null)
			{
				using (var undoHelper = new UndoableUnitOfWorkHelper(mainCache.ServiceLocator.GetInstance<IActionHandler>(), LanguageExplorerResources.ksUndoAddAllomorphToSimilarEntry, LanguageExplorerResources.ksRedoAddAllomorphToSimilarEntry))
				{
		...
		internal void RunAddNewAllomorphDlg()
		{
			...
			using (var dlg = new AddAllomorphDlg())
			{
				...
				if (dlg.InconsistentType && le.LexemeFormOA != null)
				{
					...
					{
						entryForm = LanguageExplorerResources.ksNoForm;
					}
					...
					var msg1 = string.Format(LanguageExplorerResources.ksSelectedLexEntryXisaY, entryForm, sTypeLe);
					var msg2 = string.Format(LanguageExplorerResources.ksAreYouSureAddZtoX, sTypeNew, tssForm.Text);
							...
							case DialogResult.Yes:
								...
								switch (mmtNew.Guid.ToString())
								{
									...
									case MoMorphTypeTags.kMorphDiscontiguousPhrase:
										...
										if (!haveStemMSA)
										{
											UndoableUnitOfWorkHelper.Do(LanguageExplorerResources.ksUndoAddAllomorph, LanguageExplorerResources.ksRedoAddAllomorph, actionHandler, () =>
											{
		...
		private void OnSelectCreateNewEntry(object sender, EventArgs args)
		{
			...
			{
				MessageBox.Show(exc.Message, LanguageExplorerResources.ksCannotCreateNewEntry);
			}
		}
		...
		private void OnSelectAllomorphOf(object sender, EventArgs args)
		{
			...
			{
				MessageBox.Show(exc.Message, LanguageExplorerResources.ksCannotAddAllomorph);
			}
		}
		...
		private void HandleSelectMorphComboItem(object sender, EventArgs args)
		{
			...
			else
			{
				...
				UndoableUnitOfWorkHelper.Do(LanguageExplorerResources.ksUndoAddSense, LanguageExplorerResources.ksRedoAddSense, m_caches.MainCache.ActionHandlerAccessor,
		...
		private void OnSelectVariantOf(object sender, EventArgs args)
		{
			...
			{
				MessageBox.Show(exc.Message, LanguageExplorerResources.ksCannotAddVariant);
			}
		}
