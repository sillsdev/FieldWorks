...
namespace LanguageExplorer.Controls.DetailControls
{
	...
	/// </summary>
	internal sealed class ChooseAnalysisHandler : IComboHandler
	{
		...
		/// </summary>
		internal event EventHandler AnalysisChosen;
		...
		/// </summary>
		internal AnalysisTree GetAnalysisTree()
		{
		...
		/// </summary>
		internal HvoTssComboItem SelectedItem => m_combo?.SelectedItem as HvoTssComboItem;
		...
		/// </summary>
		internal ChooseAnalysisHandler(LcmCache cache, int hvoSrc, int hvoAnalysis, IComboList comboList)
		{
			m_combo = comboList;
			m_cache = cache;
			Source = hvoSrc;
		...
		public void SetupCombo()
		{
			...
			AddItem(wordform, MakeSimpleString(LanguageExplorerResources.ksNewAnalysis), false, WfiWordformTags.kClassId);
			// Add option to reset to the default
			AddItem(null, MakeSimpleString(LanguageExplorerResources.ksUseDefaultAnalysis), false);
		...
		private void AddAnalysisItems(IWfiAnalysis wa)
		{
			...
			AddItem(wa, MakeSimpleString(LanguageExplorerResources.ksNewWordGloss), false, WfiGlossTags.kClassId);
		}

		private ITsString MakeSimpleString(string str)
		{
			var builder = TsStringUtils.MakeStrBldr();
		...
		internal static ITsString MakeAnalysisStringRep(IWfiAnalysis wa, LcmCache lcmCache, bool fUseStyleSheet, int wsVern)
		{
			...
			if (cmorph == 0)
			{
				return TsStringUtils.MakeString(LanguageExplorerResources.ksNoMorphemes, lcmCache.DefaultUserWs);
			}
		...
		/// <summary />
		internal static ITsTextProps FormTextProperties(LcmCache lcmCache, bool fUseStyleSheet, int wsVern)
		{
			var color = (int)Color.DarkBlue.RGB();
			var bldr = TsStringUtils.MakePropsBldr();
			if (!fUseStyleSheet)
			{
		...
		internal static ITsTextProps GlossTextProperties(LcmCache lcmCache, bool inAnalysisLine, bool fUseStyleSheet)
		{
			var color = (int)Color.DarkRed.RGB();
			var bldr = TsStringUtils.MakePropsBldr();
			if (!fUseStyleSheet)
			{
				bldr.SetIntPropValues((int)FwTextPropType.ktptFontSize, (int)FwTextPropVar.ktpvMilliPoint, s_baseFontSize * 1000);
		...
		/// <summary />
		internal static ITsTextProps PartOfSpeechTextProperties(LcmCache lcmCache, bool inAnalysisLine, bool fUseStyleSheet)
		{
			var color = (int)Color.Green.RGB();
			var bldr = TsStringUtils.MakePropsBldr();
			if (!fUseStyleSheet)
			{
		...
		/// </summary>
		internal SandboxBase Owner { get; set; }
