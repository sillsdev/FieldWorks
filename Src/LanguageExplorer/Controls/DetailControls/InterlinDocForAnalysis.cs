...
namespace LanguageExplorer.Controls.DetailControls
{
	...
	internal partial class InterlinDocForAnalysis : InterlinDocRootSiteBase
	{
		...
		/// See LT-9421.
		/// </summary>
		private int m_cpropPrevForInsert = -1;
		...
		internal InterlinDocForAnalysis()
		{
			InitializeComponent();
			DoSpellCheck = true;
		}
			...
			using (var menu = new ContextMenuStrip())
			{
				try
				{
					...
					if (CanDeleteNote(e.Selection, out _))
					{
						...
						// Add the delete item.
						var item = new ToolStripMenuItem(LanguageExplorerResources.ksDeleteNote);
		...
		private void OnDeleteNote(object sender, EventArgs e)
		{
			...
			UndoableUnitOfWorkHelper.Do(string.Format(LanguageExplorerResources.ksUndoCommand, itemText), string.Format(LanguageExplorerResources.ksRedoCommand, itemText), Cache.ActionHandlerAccessor,
		...
		protected override void SetupUiWidgets(UserControlUiWidgetParameterObject userControlUiWidgetParameterObject)
		{
			...
			Subscriber.Subscribe(LanguageExplorerConstants.ITexts_AddWordsToLexicon, PropertyAddWordsToLexicon_Changed);
		...
		protected override void TearDownUiWidgets()
		{
			...
			Subscriber.Unsubscribe(LanguageExplorerConstants.ITexts_AddWordsToLexicon, PropertyAddWordsToLexicon_Changed);
		}
		#endregion
		...
		internal void TriggerAnnotationSelected(AnalysisOccurrence target, bool fSaveGuess)
		{
		...
		internal virtual void TriggerAnalysisSelected(AnalysisOccurrence target, bool fSaveGuess, bool fMakeDefaultSelection)
		{
		...
		internal virtual void TriggerAnalysisSelected(AnalysisOccurrence target, bool fSaveGuess, bool fMakeDefaultSelection, bool fShow)
		{
		...
		internal AnalysisOccurrence SelectedOccurrence
		{
			...
			set
			{
				...
				Publisher.Publish(new PublisherParameterObject(LanguageExplorerConstants.TextSelectedWord, value != null && value.HasWordform ? value.Analysis.Wordform : null));
			}
		}
		...
		private ArrowChange HandleArrowKeys(KeyEventArgs e)
		{
			                               ...
			                               && (e.KeyCode & Keys.Shift) != Keys.Shift && ((e.KeyCode & Keys.Control) != Keys.Control))
			{
				...
				var haveSelection = GetCurrentSelection(out var clev, out var rgvsli, out var tag, out var ichAnchor, out var ichEnd, out var ws);
				if (!haveSelection)
				{
		...
		private bool DetectUpMove(KeyEventArgs e, IEnumerable<InterlinLineSpec> lines, int lineNum, ISegment curSeg, int curNoteIndex, WhichEnd where, bool isRightToLeft, out bool isUpNewSeg)
		{
			...
			var hasUpMotion = e.KeyCode == Keys.Up || (TextIsRightToLeft ? e.KeyCode == Keys.Right
													&& (where == WhichEnd.Right || where == WhichEnd.Both) : e.KeyCode == Keys.Left && (where == WhichEnd.Left || where == WhichEnd.Both));
		...
		private void CmdAddWordGlossesToFreeTransClick(object sender, EventArgs e)
		{
			...
			UndoableUnitOfWorkHelper.Do(LanguageExplorerResources.ksUndoSetTransFromWordGlosses, LanguageExplorerResources.ksRedoSetTransFromWordGlosses, Cache.ActionHandlerAccessor, () =>
			{
		...
		private static bool SelIsInEmptyTranslation(SelectionHelper helper, int flid, int hvo)
		{
		...
		/// </summary>
		internal bool IsFocusBoxInstalled => ExistingFocusBox != null && Controls.Contains(FocusBox);
		...
		internal void AddNote()
		{
			...
			UowHelpers.UndoExtension(LanguageExplorerResources.InsertNote, Cache.ActionHandlerAccessor, () =>
			{
