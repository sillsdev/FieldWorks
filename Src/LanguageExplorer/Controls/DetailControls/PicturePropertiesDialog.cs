...
namespace LanguageExplorer.Controls.DetailControls
{
	...
	public class PicturePropertiesDialog : Form
	{
		...
		public PicturePropertiesDialog(LcmCache cache, ICmPicture initialPicture, IHelpTopicProvider helpTopicProvider, IApp app)
		...
		public bool Initialize()
		{
			...
			{
				var errorMsg = $"Error creating one of the following directories '{m_cache.LanguageProject.LinkedFilesRootDir}' or '{s_defaultPicturesFolder}'";
		...
		private void InitializeComponent()
		{
			...
			this.pnlPicture = new SIL.FieldWorks.FwCoreDlgs.FwPanel();
			this.pnlCaption = new SIL.FieldWorks.FwCoreDlgs.FwPanel();
		...
		/// </summary>
		public string CurrentFile => m_filePath;

		/// <summary>
		/// Gets the caption the user gave the file.
		/// </summary>
		public ITsString Caption => m_txtCaption.Tss;
		#endregion
		...
		{
			var szPreferred = TextRenderer.MeasureText(lblFilename.Text, lblFilename.Font);
			m_tooltip.SetToolTip(lblFilename, (lblFilename.Width < szPreferred.Width + 8 ? lblFilename.Text : null));
		}
			...
			using (IOpenFileDialog dlg = new OpenFileDialogAdapter())
			{
				...
				while (dialogResult != DialogResult.OK && dialogResult != DialogResult.Cancel)
				{
					...
					if (dialogResult == DialogResult.OK)
					{
						...
						{
							MessageBoxUtils.Show(FwCoreDlgs.kstidInsertPictureReadError, FwCoreDlgs.kstidInsertPictureReadErrorCaption);
		...
		/// <summary>
		/// Makes an empty caption string with the correct writing system.
		/// </summary>
		...
		private void UpdatePicInformation()
		{
			...
			else
			{
				...
				if (!File.Exists(tmpOriginalPath))
				{
					tmpOriginalPath = string.Format(FwCoreDlgs.kstidPictureUnavailable, tmpOriginalPath.Normalize());
					m_grpFileLocOptions.Visible = false;
				}
				else
				{
		...
		private bool IsFolderInLinkedFilesFolder(string sFolder)
		{
			...
			var sExtLinksRoot = s_defaultPicturesFolder;
			...
			{
				var cchDir = sExtLinksRoot.Length;
				return (sFolder.Length == cchDir || (sFolder.Length > cchDir && sFolder[cchDir] == Path.DirectorySeparatorChar));
			}
			return false;
		}
		#endregion
