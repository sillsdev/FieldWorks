...
namespace LanguageExplorer.Controls.DetailControls
{
	...
	internal sealed class InterlinTaggingVc : InterlinVc
	{
		...
		{
			m_cache = cache;
			m_lenEndTag = LanguageExplorerResources.ksEndTagSymbol.Length;
			m_lenStartTag = LanguageExplorerResources.ksStartTagSymbol.Length;
		...
		private void LoadDataForTextTags(int hvoSeg)
		{
			...
			foreach (var tag in ControlServices.GetTaggingReferencingTheseWords(segWords))
			{
		...
		private void EndTagSetup(ITsStrBldr builder)
		{
			...
			if (RightToLeft)
			{
				if (m_fAnalRtl)
				{
					builder.Replace(builder.Length, builder.Length, LanguageExplorerResources.ksStartTagSymbol, null);
				}
				else
				{
					builder.Replace(0, 0, LanguageExplorerResources.ksStartTagSymbol, null);
				}
			}
			else
			{
				if (m_fAnalRtl)
				{
					builder.Replace(0, 0, LanguageExplorerResources.ksEndTagSymbol, null);
				}
				else
				{
					builder.Replace(builder.Length, builder.Length, LanguageExplorerResources.ksEndTagSymbol, null);
				}
			}
		}
		...
		private bool TagStartsWithThisWordform(string ttagLabel)
		{
			...
			{
				return ttagLabel.Substring(0, m_lenStartTag) == LanguageExplorerResources.ksStartTagSymbol;
			}
			return false;
		}
		...
		private bool TagEndsWithThisWordform(string ttagLabel)
		{
			...
			if (clen >= m_lenEndTag)
			{
				return ttagLabel.Substring(clen - m_lenEndTag, m_lenEndTag) == LanguageExplorerResources.ksEndTagSymbol;
			}
			return false;
		}
	}
