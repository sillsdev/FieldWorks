...
namespace LanguageExplorer.Controls.DetailControls
{
	...
	/// </summary>
	internal sealed class IhWordGloss : InterlinComboHandler
	{
		...
		public override void SetupCombo()
		{
			...
			ComboList.Items.Add(new HvoTssComboItem(hvoEmptyGloss, TsStringUtils.MakeString(LanguageExplorerResources.ksNewWordGloss2, m_caches.MainCache.DefaultUserWs)));
		...
		private void AddComboItems(ref int hvoEmptyGloss, ITsStrBldr tsb, IWfiAnalysis wa)
		{
			...
			foreach (var gloss in wa.MeaningsOC)
			{
				...
				foreach (var ws in wsids)
				{
					...
					tsb.SetIntPropValues(oldLen, tsb.Length, (int)FwTextPropType.ktptForeColor, (int)FwTextPropVar.ktpvDefault, (int)Color.Blue.RGB());
					glossCount++;
				}
				...
				if (glossCount == 0 && wsids.Any())
				{
					...
					tsb.Replace(tsb.Length, tsb.Length, LanguageExplorerResources.ksEmpty, tpbUserWs.GetTextProps());
				}
