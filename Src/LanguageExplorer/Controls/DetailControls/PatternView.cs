...
namespace LanguageExplorer.Controls.DetailControls
{
	...
	/// </summary>
	internal sealed class PatternView : RootSiteControl
	{
		internal new event EventHandler SelectionChanged;
		internal event EventHandler<RemoveItemsRequestedEventArgs> RemoveItemsRequested;
		internal event EventHandler<ContextMenuRequestedEventArgs> ContextMenuRequested;
		...
		{
			// anchor IP
			if (!GetSelectionInfo(sel, SelLimitType.Anchor, out var anchorSel, out var curHvo, out var curIch, out var curTag))
			{
				return;
			}
			// end IP
			if (!GetSelectionInfo(sel, SelLimitType.End, out var endSel, out var curEndHvo, out var curEndIch, out var curEndTag))
			{
				return;
			}
			...
			if (vwSel != null)
			{
				...
				vwSel.TextSelInfo(false, out _, out var wholeIch, out _, out var wholeHvo, out var wholeTag, out _);
				vwSel.TextSelInfo(true, out _, out var wholeEndIch, out _, out var wholeEndHvo, out var wholeEndTag, out _);
		...
		private bool GetSelectionInfo(SelectionHelper sel, SelLimitType limit, out IVwSelection vwSel, out int curHvo, out int curIch, out int curTag)
		{
			...
			sel.Selection.TextSelInfo(limit == SelLimitType.End, out var curTss, out curIch, out _, out curHvo, out curTag, out _);
			var ctxt = m_patternControl.GetContext(sel);
			...
			initialSel.TextSelInfo(false, out _, out var initialIch, out _, out var initialHvo, out var initialTag, out _);
			...
			else
			{
				...
				finalSel.TextSelInfo(false, out _, out var finalIch, out _, out var finalHvo, out var finalTag, out _);
		...
		internal IVwSelection SelectAt(object ctxt, int index, bool initial, bool editable, bool install)
		{
			var levels = m_patternControl.GetLevelInfo(ctxt, index);
			if (levels == null)
			{
