...
{
	internal sealed class RawTextVc : StVc
	{
		internal const int kTagUserPrompt = 1000009879; // very large number prevents auto-load.
		private IVwRootBox m_rootb;

		internal RawTextVc(IVwRootBox rootb, LcmCache cache, int wsFirstPara) : base("Normal", wsFirstPara)
		{
			m_rootb = rootb;
			Cache = cache;
		...
		{
			var userPrompt = LanguageExplorerResources.ksEnterOrPasteHere;
