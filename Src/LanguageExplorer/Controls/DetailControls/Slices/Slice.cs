...
namespace LanguageExplorer.Controls.DetailControls.Slices
{
	...
	/// many other things.}
	///</remarks>
	internal class Slice : UserControl, ISlice
	{
		#region Constants
		...
		protected ISlice AsISlice => this;

		#region Properties
		public ISlice NextSlice
		{
			get
			{
		...
		public virtual ContextMenuName HotlinksMenuId
		{
			get
			{
		...
		private ContextMenuName LeftEdgeMenuId
		{
			get
			{
		...
		protected ContextMenuName ContextMenuMenuId
		{
			get
			{
		...
		public void RemoveOldVisibilityMenus()
		{
		...
		public int GetSelectionHvoFromControls()
		{
		...
		/// 4. 'Help...'
		/// </summary>
		public void AddCoreContextMenus(ref Tuple<ContextMenuStrip, List<Tuple<ToolStripMenuItem, EventHandler>>> sliceTreeNodeContextMenuStripTuple)
		{
		...
		public virtual void PrepareToShowContextMenu()
		...
		private bool WrapsAtomic => XmlUtils.GetOptionalBooleanAttributeValue(ConfigurationNode, "wrapsAtomic", false);
		...
		/// </summary>
		private bool IsCollectionNode => ConfigurationNode?.Element("seq") != null && !IsSequenceNode;
		...
		/// null.
		/// </summary>
		public void OverrideBackColor(string backColorName)
		{
			if (Control == null)
			{
				return;
		...
		/// </summary>
		public virtual void BecomeReal(int index)
		{
		}

		private static void SetViewStylesheet(Control control, DataTree tc)
		{
		...
		public virtual void GenerateChildren(XElement node, XElement caller, ICmObject obj, int indent, ref int insPos, ArrayList path, ObjSeqHashMap reuseMap, bool fUsePersistentExpansion)
		{
			...
			if (indentNode != null)
			{
				...
				ContainingDataTree.ApplyLayout(obj, this, indentNode, indent + indentNode.ExtraIndent(), insPos, path, reuseMap, true, out ntr);
			}
			else
			{
				var insPosT = insPos; // don't modify the real one in this test call.
				ntr = ContainingDataTree.ProcessPartChildren(node, path, reuseMap, obj, this, indent + node.ExtraIndent(), ref insPosT, true, null, false, node);
			}
			switch (ntr)
			{
		...
		/// <summary />
		/// <remarks>This could be protected, except for a test,</remarks>
		internal void CreateIndentedNodes(XElement caller, ICmObject obj, int indent, ref int insPos, ArrayList path, ObjSeqHashMap reuseMap, XElement node)
		{
			...
			if (indentNode != null)
			{
				insPos = ContainingDataTree.ApplyLayout(obj, this, indentNode, indent + indentNode.ExtraIndent(), insPos, path, reuseMap, false, out _);
			}
			else
			{
				ContainingDataTree.ProcessPartChildren(node, path, reuseMap, obj, this, indent + node.ExtraIndent(), ref insPos, false, parameter, false, caller);
			}
		}
		...
		/// <summary />
		protected virtual int LabelHeight => m_fontLabel.Height;
		...
		/// Help Topic ID for the slice
		/// </summary>
		private string ChooserDlgHelpTopicID => XmlUtils.GetOptionalAttributeValue(ConfigurationNode, "chooserDlgHelpTopicID");

		/// <summary />
		protected virtual string GetSliceHelpTopicID()
		{
		...
		private string GenerateHelpTopicId(string helpTopicPrefix)
		{
			...
			else
			{
				...
				else if (areaChoice == LanguageExplorerConstants.ListsAreaMachineName)
				{
		...
		/// </summary>
		private string ExpansionStateKey => Expansion == TreeItemState.ktisFixed || Expansion == TreeItemState.ktisCollapsedEmpty ? null
		...
		private bool IsDescendant(ISlice slice)
		{
		...
		/// parent slice).
		/// </summary>
		public ISlice ParentSlice { get; set; }
		...
		/// </summary>
		public void DrawLabel(int y, Graphics gr, int clipWidth)
		{
			DrawLabel(LabelIndent(), y, gr, clipWidth);
		}

		private bool IsObjectNode => (ConfigurationNode.Element("node") != null || ("PhCode" == MyCmObject.ClassName))   // todo: this should tell if the attr (not the nested one) is to a basic type or a cmobject
		...
		public void HandleInsertCommand(string fieldName, string className, string ownerClassName = null)
		{
		...
		/// </returns>
		private bool InsertObjectIfPossible(int newObjectClassId, int ownerClassId, string fieldName, ISlice slice)
		{
			...
			var flid = ((Slice)slice).GetFlid(fieldName);
			...
			{
				insertionPosition = ((Slice)slice).InsertObject(flid, newObjectClassId);
			}
		...
		private ISlice ExpandSubItem(int hvo)
		{
		...
		/// </summary>
		private void FocusSliceOrChild()
		{
			...
			for (var islice = myIndex; islice < cslice; ++islice)
			{
				...
				// Stop if we get past the children of the current object.
				if (!LanguageExplorerServices.StartsWith(slice.Key, myKey))
				{
					break;
				}
			}
		...
		/// </summary>
		public List<ISlice> GetNearbySlices()
		{
			var index = IndexInContainer;
			var closeSlices = new List<ISlice> { this };
		...
		/// <summary />
		public void HandleMergeCommand(bool fLoseNoTextData)
		{
			var obj = GetObjectForMenusToOperateOn();
			if (obj == null)
			{
		...
		/// <summary />
		public void HandleSplitCommand()
		{
			var obj = GetObjectForMenusToOperateOn();
			if (obj == null)
			{
		...
		public virtual bool UpdateDisplayIfNeeded(int hvo, int tag)
		{
			return false;
		}
		...
		/// </summary>
		public virtual void SetWidthForDataTreeLayout(int width)
		{
			if (Width != width)
			{
				Width = width;
