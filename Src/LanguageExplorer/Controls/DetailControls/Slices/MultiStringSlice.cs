...
{
	/// <summary />
	internal class MultiStringSlice : ViewPropertySlice, IMultiStringSlice
	{
		...
		internal MultiStringSlice(ICmObject obj, int flid, int ws, int wsOptional, bool forceIncludeEnglish, bool editable, bool spellCheck)
		{
		...
		public override void PrepareToShowContextMenu()
		{
		...
		/// </summary>
		internal void Reuse(ICmObject obj, int flid, int ws, int wsOptional, bool forceIncludeEnglish, bool editable, bool spellCheck)
		{
		...
		private void SetWssToDisplayForPart(IReadOnlyList<CoreWritingSystemDefinition> wssToDisplay)
		{
			...
			foreach (var slice in ContainingDataTree.Slices.Where(slice => slice is MultiStringSlice multiStringSlice && multiStringSlice.PartRef() == ourPart))
			{
