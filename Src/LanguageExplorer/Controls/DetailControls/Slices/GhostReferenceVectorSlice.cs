...
namespace LanguageExplorer.Controls.DetailControls.Slices
{
	...
	/// </summary>
	internal sealed class GhostReferenceVectorSlice : FieldSlice
	{
		...
		private string BestWsName => XmlUtils.GetOptionalAttributeValue(ConfigurationNode.Element("deParams"), "ws", "analysis");

		// Copied from ReferenceVectorSlice for initializing GhostReferenceVectorLauncher...may not be used.
		private string DisplayNameProperty => XmlUtils.GetOptionalAttributeValue(ConfigurationNode.Element("deParams"), "displayProperty", string.Empty);
	}
}