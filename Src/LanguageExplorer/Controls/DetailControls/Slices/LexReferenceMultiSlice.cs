...
namespace LanguageExplorer.Controls.DetailControls.Slices
{
	...
	internal sealed class LexReferenceMultiSlice : Slice
	{
		...
		/// <summary />
		private void OnShowFieldAlwaysVisible1(object sender, EventArgs args)
		{
			SetFieldVisibility("always");
		}
		/// <summary />
		private void OnShowFieldIfData1(object sender, EventArgs args)
		{
			SetFieldVisibility("ifdata");
		}
		/// <summary />
		private void OnShowFieldNormallyHidden1(object sender, EventArgs args)
		{
		...
		/// <summary />
		private void HandleCreateMenuItem(object sender, EventArgs ea)
		{
		...
		/// <summary />
		private void HandleMoreMenuItem(object sender, EventArgs ea)
		{
			...
			LinkHandler.PublishFollowLinkMessage(ContainingDataTree.Publisher, new FwLinkArgs(LanguageExplorerConstants.LexRefEditMachineName, newKid.Guid));
		}
		...
		/// </summary>
		internal void DeleteFromReference(ILexReference lr)
		{
			if (lr == null)
			{
			...
			using (var dlg = new ConfirmDeleteObjectDlg(PropertyTable.GetValue<IHelpTopicProvider>(LanguageExplorerConstants.HelpTopicProvider)))
			{
				...
				{
					UndoableUnitOfWorkHelper.Do(LanguageExplorerControls.ksUndoDeleteRelation, LanguageExplorerControls.ksRedoDeleteRelation, MyCmObject, () =>
					{
		...
		/// </summary>
		internal void DeleteReference(ILexReference lr)
		{
			if (lr == null)
			{
				throw new FwConfigurationException("Slice:GetObjectHvoForMenusToOperateOn is either messed up or should not have been called, because it could not find the object to be deleted.", ConfigurationNode);
		...
		/// </summary>
		internal void EditReferenceDetails(ILexReference lr)
		{
			if (lr == null)
			{
				throw new FwConfigurationException("Slice:GetObjectHvoForMenusToOperateOn is either messed up or should not have been called, because it could not find the object to be deleted.", ConfigurationNode);
			...
			using (var dlg = new LexReferenceDetailsDlg(PropertyTable.GetValue<IHelpTopicProvider>(LanguageExplorerConstants.HelpTopicProvider)))
			{
				...
				using (var helper = new UndoableUnitOfWorkHelper(Cache.ActionHandlerAccessor, LanguageExplorerControls.ksUndoEditRefSetDetails, LanguageExplorerControls.ksRedoEditRefSetDetails))
				{
