...
{
	/// <summary />
	internal sealed class SummaryCommandControl : UserControl
	{
		/// <summary>
		...
		internal SummaryCommandControl(ISummarySlice summarySlice, SliceHotlinksMenuFactory sliceHotlinksMenuFactory, SliceLeftEdgeContextMenuFactory sliceLeftEdgeContextMenuFactory)
		{
			InitializeComponent();
		...
		protected override void Dispose(bool disposing)
		{
			...
			m_hotLinkFont = null;
			m_timer = null;
			_summarySlice = null; // Client is responsible for this.
		...
		private const int kGapInBetweenButtons = 5; // gap between buttons

		protected override void OnLayout(LayoutEventArgs levent)
		{
			// Too early for layout.
			if (_summarySlice.ContainingDataTree == null)
			{
				return;
			}
			...
			try
			{
				...
				var hotlinksMenuId = _summarySlice.HotlinksMenuId;
				m_hotlinksMenuItems = hotlinksMenuId == ContextMenuName.nullValue ? null : MySliceHotlinksMenuFactory.GetHotlinksMenuItems(_summarySlice, hotlinksMenuId);
				if (m_hotlinksMenuItems == null)
				{
					// Try the ordinary menu, but without the core context menus.
					var leftEdgeMenus = hotlinksMenuId == ContextMenuName.nullValue ? null : MySliceLeftEdgeContextMenuFactory.GetLeftEdgeContextMenu(_summarySlice, hotlinksMenuId, false);
