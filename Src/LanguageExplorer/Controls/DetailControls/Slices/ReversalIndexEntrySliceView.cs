...
namespace LanguageExplorer.Controls.DetailControls.Slices
{
	...
	internal class ReversalIndexEntrySliceView : RootSiteControl
	{
		...
		/// </summary>
		internal const int kFlidIndices = 5001;
		/// <summary>
		/// A dummy flid for the reversal index, which holds the entries for the main sense.
		/// </summary>
		internal const int kFlidEntries = 5002;
		/// <summary />
		internal const int kDummyEntry = -10000;
		/// <summary />
		internal const int kFragMainObject = 1;
		/// <summary />
		internal const int kFragIndices = 2;
		/// <summary />
		internal const int kFragWsAbbr = 3;
		/// <summary />
		internal const int kFragEntryForm = 4;
		/// <summary />
		internal const int kFragIndexMain = 5;
		/// <summary />
		internal const int kFragEntries = 6;
		...
		/// <summary />
		internal ReversalIndexEntrySliceView(int hvo)
		{
		...
		private void OnShowInReversalIndex(object sender, EventArgs e)
		{
			...
			LinkHandler.PublishFollowLinkMessage(Publisher, new FwLinkArgs(LanguageExplorerConstants.ReversalEditCompleteMachineName, ((IReversalIndexEntry)Cache.ServiceLocator.GetObject(hvo)).MainEntry.Guid));
		}
		...
		private int ConvertDummiesToReal(int hvoDummy)
		{
			...
			for (var i = 0; i < countIndices; ++i)
			{
				...
				for (var j = countRealEntries - 1; j >= 0; --j)
				{
					...
					var hvo = revIndex.FindOrCreateReversalEntry(rgsFromDummy, Cache);
		...
		/// </summary>
		internal static void GetFormList(string longNameIn, List<string> forms)
		{
