...
{
	/// <summary />
	internal sealed class DerivMSAReferenceSlice : AtomicReferenceSlice
	{
		/// <summary />
		...
		/// </remarks>
		private void OnReferenceChanged(object sender, FwObjectSelectionEventArgs e)
		{
			...
			if (idxSender > 0)
			{
				...
				while (otherSlice == null || (otherSlice.Indent == Indent && idxOther > 0 && otherSlice.MyCmObject == MyCmObject))
				{
					...
					if (otherSlice is IAtomicReferenceSlice referenceSlice && referenceSlice.Flid == otherFlid)
					{
						break;
					}
				}
				if (otherSlice is IAtomicReferenceSlice atomicReferenceSlice)
				{
		...
		private int GetOtherHvo(ISlice otherSlice, int otherFlid, bool myIsFromPOS, out AtomicReferenceLauncher otherOne)
		{
			otherOne = null;
			var otherHvo = 0;
			if (otherSlice != null)
