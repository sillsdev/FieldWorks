...
{
	internal sealed class MSAReferenceComboBoxSlice : FieldSlice, IMSAReferenceComboBoxSlice, IVwNotifyChange
	{
		...
		/// Constructor.
		/// </summary>
		internal MSAReferenceComboBoxSlice(LcmCache cache, ICmObject obj, int flid, IPersistenceProvider persistenceProvider)
		...
		public override void InitializeFlexComponent(FlexComponentParameters flexComponentParameters)
		{
			...
			m_MSAPopupTreeManager = new MSAPopupTreeManager(m_tree, Cache, list, m_tree.WritingSystemCode, true, flexComponentParameters, PropertyTable.GetValue<Form>(FwUtilsConstants.window));
		...
		/// comfortable size.
		/// </summary>
		private void m_tree_SizeChanged(object sender, EventArgs e)
		{
		...
		private sealed class MSAPopupTreeManager : PopupTreeManager
		{
			...
			/// <summary />
			internal MSAPopupTreeManager(TreeCombo treeCombo, LcmCache cache, ICmPossibilityList list, int ws, bool useAbbr, FlexComponentParameters flexComponentParameters, Form parent)
				: base(treeCombo, cache, flexComponentParameters, list, ws, useAbbr, parent)
			{
				LoadStrings();
			}

			internal ILexSense Sense { get; set; }

			private string FieldName { get; } = string.Empty;

			internal IPersistenceProvider PersistenceProvider { get; set; }
			...
			/// </summary>
			internal TreeNode MakeTargetMenuItem()
			{
				...
				using (var dlg = new MasterCategoryListDlg())
				{
					...
					switch (dlg.ShowDialog(ParentForm))
					{
						...
						case DialogResult.Yes:
							...
							LinkHandler.PublishFollowLinkMessage(_flexComponentParameters.Publisher, new FwLinkArgs(LanguageExplorerConstants.PosEditMachineName, dlg.SelectedPOS.Guid));
