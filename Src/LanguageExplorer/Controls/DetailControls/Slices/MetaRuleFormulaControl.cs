...
namespace LanguageExplorer.Controls.DetailControls.Slices
{
	...
	internal sealed class MetaRuleFormulaControl : RuleFormulaControl
	{
		...
		private const int ktagLeftEnv = -300;
		private const int ktagRightEnv = -301;
		private const int ktagLeftSwitch = -302;
		private const int ktagRightSwitch = -303;

		internal MetaRuleFormulaControl(ISharedEventHandlers sharedEventHandlers, XElement configurationNode)
		...
		public override void Initialize(LcmCache cache, ICmObject obj, int flid, string fieldName, IPersistenceProvider persistProvider, string displayNameProperty, string displayWs)
		{
			...
			_view.Init(obj.Hvo, this, new MetaRuleFormulaVc(cache, PropertyTable), kfragRule, cache.MainCacheAccessor);
		...
		private string DisplayNoOptsMsg()
		{
			...
			switch (cellId)
			{
				case PhMetathesisRuleTags.kidxLeftSwitch:
				case PhMetathesisRuleTags.kidxRightSwitch:
					return LanguageExplorerControls.ksMetaRuleNoOptsMsg;

				case PhMetathesisRuleTags.kidxLeftEnv:
				case PhMetathesisRuleTags.kidxRightEnv:
					return LanguageExplorerControls.ksRuleWordBdryNoOptsMsg;
			}
			return null;
		}
		...
		protected override int GetFlid(int cellId)
		{
			switch (cellId)
			{
				case PhMetathesisRuleTags.kidxLeftEnv:
					return ktagLeftEnv;
				case PhMetathesisRuleTags.kidxLeftSwitch:
					return ktagLeftSwitch;
				case PhMetathesisRuleTags.kidxRightSwitch:
					return ktagRightSwitch;
				case PhMetathesisRuleTags.kidxRightEnv:
					return ktagRightEnv;
			}
			return -1;
		...
		private sealed class MetaRuleFormulaVc : RuleFormulaVcBase
		{
				...
				: base(cache, propertyTable)
			{
				...
				m_inputStr = TsStringUtils.MakeString(LanguageExplorerControls.ksMetaRuleInput, userWs);
				m_resultStr = TsStringUtils.MakeString(LanguageExplorerControls.ksMetaRuleResult, userWs);
				m_leftEnvStr = TsStringUtils.MakeString(LanguageExplorerControls.ksMetaRuleLeftEnv, userWs);
				m_rightEnvStr = TsStringUtils.MakeString(LanguageExplorerControls.ksMetaRuleRightEnv, userWs);
				m_switchStr = TsStringUtils.MakeString(LanguageExplorerControls.ksMetaRuleSwitch, userWs);
			}
