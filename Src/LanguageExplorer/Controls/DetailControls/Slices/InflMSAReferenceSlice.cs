...
namespace LanguageExplorer.Controls.DetailControls.Slices
{
	...
	/// </summary>
	internal sealed class InflMSAReferenceSlice : AtomicReferenceSlice
	{
		/// <summary />
		...
		private void OnReferenceChanged(object sender, FwObjectSelectionEventArgs e)
		{
			...
			if (otherSlice == null && idxSender < ContainingDataTree.Slices.Count)
			{
				...
				{
					otherSlice = ContainingDataTree.Slices[idxOther++];
					if (otherSlice is IReferenceVectorSlice referenceVectorSlice && referenceVectorSlice.Flid == otherFlid)
					{
						break;
					}
					otherSlice = null;
