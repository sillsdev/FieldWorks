...
namespace LanguageExplorer.Controls.DetailControls.Slices
{
	...
	{
		internal RegRuleFormulaControl(ISharedEventHandlers sharedEventHandlers, XElement configurationNode)
		...
		/// the right hand side
		/// </summary>
		internal IPhSegRuleRHS Rhs => (IPhSegRuleRHS)m_obj;

		internal bool CanModifyContextOccurrence
		{
			get
			{
		...
		protected override Tuple<ContextMenuStrip, List<Tuple<ToolStripMenuItem, EventHandler>>> CreateContextMenu()
		{
			...
			if (CanModifyContextOccurrence)
			{
				...
				*/
				menu = ToolStripMenuItemFactory.CreateToolStripMenuItemForContextMenuStrip(menuItems, contextMenuStrip, ContextSetOccurrence_Clicked, LanguageExplorerControls.Occurs_exactly_once);
				...
				*/
				menu = ToolStripMenuItemFactory.CreateToolStripMenuItemForContextMenuStrip(menuItems, contextMenuStrip, ContextSetOccurrence_Clicked, LanguageExplorerControls.Occurs_zero_or_more_times);
				menu.Tag = new Dictionary<string, int>
				{
					{ "min", 0},
					{ "max", -1}
				...
				*/
				menu = ToolStripMenuItemFactory.CreateToolStripMenuItemForContextMenuStrip(menuItems, contextMenuStrip, ContextSetOccurrence_Clicked, LanguageExplorerControls.Occurs_one_or_more_times);
				menu.Tag = new Dictionary<string, int>
				{
					{ "min", 1},
					{ "max", -1}
				...
				ToolStripMenuItemFactory.CreateToolStripMenuItemForContextMenuStrip(menuItems, contextMenuStrip, ContextSetOccurrence_Clicked, LanguageExplorerControls.Set_occurrence_min_and_max);
			}
		...
		/// </summary>
		internal void SetContextOccurrence(int min, int max)
		{
			var sel = SelectionHelper.Create(_view);
			var cellId = GetCell(sel);
			var obj = GetCmObject(sel, SelLimitType.Anchor);
			...
			var index = -1;
			UndoableUnitOfWorkHelper.Do(LanguageExplorerControls.ksRegRuleUndoSetOccurrence, LanguageExplorerControls.ksRegRuleRedoSetOccurrence, ctxt, () =>
			{
		...
		private static int GetIndex(IPhPhonContext ctxt, IPhPhonContext envCtxt, out IPhSequenceContext seqCtxt)
		{
		...
		/// </summary>
		internal void GetContextOccurrence(out int min, out int max)
		{
			var sel = SelectionHelper.Create(_view);
			var obj = GetCmObject(sel, SelLimitType.Anchor);
		...
		private sealed class RegRuleFormulaVc : RuleFormulaVcBase
		{
			internal const int kfragRHS = 200;
			private const int kfragRule = 201;
