...
namespace LanguageExplorer.Controls.DetailControls.Slices
{
	...
	internal class RuleFormulaControl : ButtonLauncher, IPatternControl
	{
		...
		internal RuleFormulaControl()
		{
			InitializeComponent();
		}

		internal RuleFormulaControl(ISharedEventHandlers sharedEventHandlers, XElement configurationNode)
			: this()
		{
		...
		internal RootSite RootSite => _view;

		internal InsertionControl InsertionControl { get; set; }
		...
		/// </summary>
		internal bool IsFeatsNCContextCurrent
		{
			get
			{
		...
		/// </summary>
		internal bool IsNCContextCurrent
		{
			get
			{
				var ctxt = CurrentContext;
		...
		/// </summary>
		internal bool IsPhonemeContextCurrent
		{
			get
			{
				var ctxt = CurrentContext;
		...
		internal IPhSimpleContext CurrentContext
		{
			get
			{
		...
		internal ICmObject CurrentObject
		{
			get
			{
		...
		protected override void HandleChooser()
		{
			...
			var cellId = -1;
			UndoableUnitOfWorkHelper.Do(LanguageExplorerControls.ksRuleUndoUpdateEnv, LanguageExplorerControls.ksRuleRedoUpdateEnv, selectedEnv, () =>
			{
		...
		private void m_insertionControl_Insert(object sender, InsertEventArgs e)
		{
			...
			switch (option.Type)
			{
				case RuleInsertType.Phoneme:
					...
					var phonemeObj = DisplayChooser(LanguageExplorerControls.ksRulePhonemeOpt, LanguageExplorerControls.ksRulePhonemeChooserLink, LanguageExplorerConstants.PhonemeEditMachineName, "RulePhonemeFlatList", phonemes);
				...
				case RuleInsertType.NaturalClass:
					...
					var ncObj = DisplayChooser(LanguageExplorerControls.ksRuleNCOpt, LanguageExplorerControls.ksRuleNCChooserLink, LanguageExplorerConstants.NaturalClassEditMachineName, "RuleNaturalClassFlatList", natClasses);
					...
					using (var featChooser = new PhonologicalFeatureChooserDlg())
					{
						...
						if (res == DialogResult.OK)
						{
							UndoableUnitOfWorkHelper.Do(undo, redo, m_cache.ActionHandlerAccessor, () =>
							{
								...
								featNC.Name.SetUserWritingSystem(string.Format(LanguageExplorerControls.ksRuleNCFeatsName, RuleName));
		...
		private void RemoveItemsRequested(object sender, RemoveItemsRequestedEventArgs e)
		{
			...
			var cellId = -1;
			var cellIndex = -1;
			UndoableUnitOfWorkHelper.Do(LanguageExplorerControls.ksRuleUndoRemove, LanguageExplorerControls.ksRuleRedoRemove, m_cache.ActionHandlerAccessor, () =>
			{
		...
		public void SetContextFeatures()
		{
			...
			using (var featChooser = new PhonologicalFeatureChooserDlg())
			{
				...
				featChooser.Title = LanguageExplorerControls.ksRuleFeatsChooserTitle;
