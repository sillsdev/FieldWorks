	...
	internal partial class InterlinMaster : InterlinMasterBase, IFocusablePanePortion
	{
		...
		internal InterlinMode GetLineMode()
		{
			switch (m_tabCtrl.SelectedIndex)
			{
				case (int)TabPageSelection.Gloss:
					return PropertyTable.GetValue(LanguageExplorerConstants.ITexts_AddWordsToLexicon, false) ? InterlinMode.GlossAddWordsToLexicon : InterlinMode.Gloss;
		...
		private void FinishInitTabPages()
		{
			if (!XmlUtils.GetOptionalBooleanAttributeValue(m_configurationParametersElement, "editable", true))
			{
				...
				m_tpRawText.ToolTipText = string.Format(LanguageExplorerResources.ksBaseLineNotEditable);
			}
		}
		...
		internal void SaveBookMark()
		{
			...
			if (m_bookmarks.TryGetValue(new Tuple<string, Guid>(MyRecordList.Id, RootStText.Guid), out mark))
			{
				...
				mark.Save(curAnalysis, MyRecordList.Id.Equals(LanguageExplorerConstants.InterlinearTexts), IndexOfTextRecord);
			}
			else
			{
			...
			try
			{
				...
				switch (m_tabCtrl.SelectedIndex)
				{
					case ktpsInfo:
						...
						if (_paneBarButtons != null && _paneBarButtons.ContainsKey(LanguageExplorerConstants.ShowHiddenFields_interlinearEdit))
						{
							panelButton = _paneBarButtons[LanguageExplorerConstants.ShowHiddenFields_interlinearEdit];
					...
					case ktpsGloss:
						...
						if (_paneBarButtons != null && _paneBarButtons.ContainsKey(LanguageExplorerConstants.ITexts_AddWordsToLexicon))
						{
							panelButton = _paneBarButtons[LanguageExplorerConstants.ITexts_AddWordsToLexicon];
							panelButton.Visible = panelButton.Enabled = true;
						}
						break;
					case ktpsAnalyze:
		...
		{
			if (MyRecordList.Id.Equals(LanguageExplorerConstants.InterlinearTexts))
			{
				return hvoRoot;
			}
		...
		internal TabPageSelection InterlinearTab
		{
			get
			{
				var val = PropertyTable.GetValue(LanguageExplorerConstants.InterlinearTab, TabPageSelection.RawText.ToString());
			...
			set
			{
				PropertyTable.SetProperty(LanguageExplorerConstants.InterlinearTab, value.ToString(), true);
		...
		protected override void UpdateContextHistory()
		{
			...
			link.LinkProperties.Add(new LinkProperty(LanguageExplorerConstants.InterlinearTab, InterlinearTab.ToString()));
