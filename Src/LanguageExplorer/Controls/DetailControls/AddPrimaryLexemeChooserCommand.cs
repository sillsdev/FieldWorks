...
{
	/// <summary />
	internal sealed class AddPrimaryLexemeChooserCommand : ChooserCommand
	{
		private readonly ILexEntryRef m_lexEntryRef;
		private readonly Form m_parentWindow;

		/// <summary />
		internal AddPrimaryLexemeChooserCommand(LcmCache cache, bool fCloseBeforeExecuting, string sLabel, IPropertyTable propertyTable,
		...
		public override ObjectLabel Execute()
		{
			...
			using (var dlg = new LinkEntryOrSenseDlg())
			{
				...
				if (dlg.ShowDialog(m_parentWindow) == DialogResult.OK)
				{
					...
					{
						LanguageExplorerServices.ReportLexEntryCircularReference(m_lexEntryRef.Owner, obj, true);
					}
				}
			}
			return null;
