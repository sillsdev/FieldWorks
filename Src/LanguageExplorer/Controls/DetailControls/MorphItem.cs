...
namespace LanguageExplorer.Controls.DetailControls
{
	internal struct MorphItem : IComparable
	{
		...
		/// </summary>
		internal int m_hvoMorph;
		/// <summary>
		/// typically derived from the variant component lexeme
		/// </summary>
		internal int m_hvoMainEntryOfVariant;
		internal int m_hvoSense;
		internal int m_hvoMsa;
		internal ILexEntryInflType m_inflType;
		internal ILexEntryRef m_entryRef;
		internal ITsString m_name;
		internal string m_nameSense;
		internal string m_nameMsa;

		internal MorphItem(MorphItemOptions options)
		...
		internal MorphItem(int hvoMorph, ITsString tssName)
			: this(hvoMorph, 0, tssName)
		{
		}

		internal MorphItem(int hvoMorph, int hvoMainEntryOfVariant, ITsString tssName)
			: this(hvoMorph, hvoMainEntryOfVariant, tssName, 0, null, 0, null)
		{
		}

		internal MorphItem(int hvoMorph, ITsString tssName, int hvoSense, string nameSense, int hvoMsa, string nameMsa)
		...
		internal MorphItem(int hvoMorph, int hvoMainEntryOfVariant, ITsString tssName, int hvoSense, string nameSense, int hvoMsa, string nameMsa)
		{
			m_hvoMorph = hvoMorph;
		...
		/// </summary>
		internal ILexEntry GetPrimaryOrOwningEntry(LcmCache cache)
		{
