...
namespace LanguageExplorer.Controls.DetailControls
{
	...
	internal partial class InterlinTaggingChild : InterlinDocRootSiteBase
	{
		...
		/// </summary>
		internal List<AnalysisOccurrence> SelectedWordforms { get; set; }
		...
		private void Tag_Item_Click(object sender, EventArgs e)
		{
			...
			if (item.Checked)
			{
				UndoableUnitOfWorkHelper.Do(LanguageExplorerResources.ksUndoDeleteTextTag, LanguageExplorerResources.ksRedoDeleteTextTag, Cache.ActionHandlerAccessor, () => RemoveTextTagInstance(item.Possibility));
			}
			else
			{
				UndoableUnitOfWorkHelper.Do(LanguageExplorerResources.ksUndoAddTextTag, LanguageExplorerResources.ksRedoAddTextTag, Cache.ActionHandlerAccessor, () => MakeTextTagInstance(item.Possibility));
			}
		...
		private static ISet<ITextTag> FindAllTagsReferencingOccurrenceList(List<AnalysisOccurrence> occurrences)
		{
			...
			return ControlServices.GetTaggingReferencingTheseWords(occurrences);
		}
		...
		{
			/// <summary />
			internal TagPossibilityMenuItem(ICmPossibility poss)
			{
				Possibility = poss;
			}

			internal ICmPossibility Possibility { get; }
		}
	}
}