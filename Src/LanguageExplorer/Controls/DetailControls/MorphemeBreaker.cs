...
namespace LanguageExplorer.Controls.DetailControls
{
	...
	/// </summary>
	internal sealed class MorphemeBreaker
	{
		...
		internal MorphemeBreaker(CachePair caches, string input, int hvoSbWord, int wsVern, SandboxBase sandbox)
		{
			m_caches = caches;
		...
		private void HandleMorpheme(string stMorph, int ccTrailing, bool fMonoMorphemic)
		{
			...
			{
				MessageBox.Show(null, e.Message, LanguageExplorerResources.ksWarning, MessageBoxButtons.OK);
		...
		internal static string DoBasicFinding(string input, string[] breakMarkers, string[] prefixMarkers, string[] postfixMarkers)
		{
			var fullForm = input;
		...
		/// </summary>
		internal static List<string> BreakIntoMorphs(string morphFormWithMarkers, string baseWord)
		{
		...
		/// </summary>
		internal int IchSel { get; set; }

		/// <summary>
		/// Reestablish a selection, if possible.
		/// </summary>
		internal void MakeSel()
		{
			if (m_tagSel == -1)
			{
				return;
