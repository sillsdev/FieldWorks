...
namespace LanguageExplorer.Controls.DetailControls
{
	internal partial class FocusBoxController : UserControl, IFlexComponent, ISelectOccurrence, ISuppressDefaultKeyboardOnKillFocus
	{
		...
		private void SetToolTips()
		{
			...
			toolTip.SetToolTip(btnUndoChanges, AppendShortcutToToolTip(Command.CmdUndo, LanguageExplorerResources.ksUndoAllChangesHere, Keys.None));
		...
		private void ChangeOrCreateSandbox(AnalysisOccurrence selected)
		{
			...
			var color = (int)DefaultBackColor.RGB();
		...
		internal void UpdateRealFromSandbox(string uowBaseText, bool fSaveGuess, AnalysisOccurrence nextWordform)
		{
			...
			try
			{
				...
				UowHelpers.UndoExtension(uowBaseText ?? LanguageExplorerResources.ApproveAnalysis.Replace("_", string.Empty), Cache.ActionHandlerAccessor, () => ApproveAnalysisAndMove(fSaveGuess, nextWordform));
			}
			finally
			{
