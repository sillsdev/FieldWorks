...
namespace SIL.FieldWorks.ScrImportComponents
{
	...
	{
		#region Paratext Project struct (i.e. PTProjects)
		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Stores the information that's bound to the Paratext project combo boxes.
		/// </summary>
		/// ------------------------------------------------------------------------------------
		public class PTProject
		{
			private static PTProject s_empty;
			...
			/// Constructor
			/// </summary>
			/// <param name="sName">Short project name</param>
			/// <param name="lName">Long project name</param>
			/// --------------------------------------------------------------------------------
			public PTProject(string sName, string lName)
			{
				ShortName = sName;
				LongName = lName;
			}
			...
			/// --------------------------------------------------------------------------------
			/// <summary>
			/// Returns the formatted long and short names.
			/// </summary>
			/// --------------------------------------------------------------------------------
			public override string ToString()
			{
				return (this == s_empty ? LongName :
					string.Format("{0} ({1})", LongName, ShortName));
			}

			/// --------------------------------------------------------------------------------
			/// <summary>
			/// Returns an object representing an empty PTProject.
			/// </summary>
			/// --------------------------------------------------------------------------------
			public static PTProject Empty
			{
				get
				{
					if (s_empty == null)
					{
						string none = ScrImportComponents.kstidImportWizNoProjType;
						s_empty = new PTProject(none, none);
					}
					return s_empty;
				}
			}
		...
		#region ImportWizard Data Members
		...
		/// <summary></summary>
		protected List<PTProject> m_PTLangProj = new List<PTProject>();
		/// <summary></summary>
		...
		private SIL.FieldWorks.Common.Controls.WizardStepPanel stepsPanel;
		private System.Windows.Forms.Label lblOverview;
		private System.Windows.Forms.Label lblFinish;
		private System.Windows.Forms.Panel panStep0;
		private System.Windows.Forms.Panel panStep1;
		private System.Windows.Forms.Panel panStep2_PT;
		private System.Windows.Forms.Panel panStep2_Other;
		private System.Windows.Forms.Panel panStep3;
		private System.Windows.Forms.Panel panStep4;
		/// <summary>Protected for Tests</summary>
		protected Button m_btnAddScrMapping;
		/// <summary>Protected for Tests</summary>
		protected SIL.FieldWorks.Common.Controls.SFFileListBuilder sfFileListBuilder;
		private System.Windows.Forms.ContextMenu m_cmMappings;
		private System.Windows.Forms.MenuItem m_mnuExclude;
		/// <summary></summary>
		...
		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Gets and Sets the ParaText ScriptureText.
		/// </summary>
		/// ------------------------------------------------------------------------------------
		protected SCRIPTUREOBJECTSLib.ISCScriptureText3 PTScriptureText
		{
			get
			{
				try
				{
					if (m_scParatextText == null)
						m_scParatextText = new SCRIPTUREOBJECTSLib.SCScriptureTextClass();

					return m_scParatextText;
				}
				catch
				{
					return null;
		...
		private void InitializeComponent()
		{
			...
			this.panStep2_PT.Controls.Add(this.label22);
		...
		protected override void OnClosing(CancelEventArgs e)
		{
			...
			try
			{
				...
				RegistryKey key = FwApp.App.SettingsKey.CreateSubKey("ImportDialog");
		...
		private void RestoreWindowsSettings()
		{
			try
			{
				RegistryKey key = FwApp.App.SettingsKey.OpenSubKey("ImportDialog");
		...
		/// -----------------------------------------------------------------------------------
		/// <summary>
		/// Attempts to find the relevent Paratext projects on a user's system.
		/// </summary>
		...
		{
			string[] shortNames = ParatextHelper.GetParatextShortNames(PTScriptureText);
			...
			// Go through the list of text short names, load the paratext project for
			// each and add the long names to the combo. boxes.
			...
			for (int i = 0; i < shortNames.Length; i++)
			{
				try
				{
					PTScriptureText.Load(shortNames[i]);
					// To be valid, this project must have at least one Scripture book present.
					if (PTScriptureText.BooksPresent.Substring(0, ScrReference.LastBook).Contains("1"))
						fullNames[i] = PTScriptureText.FullName;
					else
						fullNames[i] = shortNames[i] = null;
				}
				catch
				{
					MessageBox.Show(this, string.Format(
						ScrImportComponents.kstidImportWizCantLoadPTProj, shortNames[i]),
			...
			// Sort by full name
			Array.Sort(fullNames, shortNames);

			for (int i = 0; i < shortNames.Length; i++)
			{
				if (shortNames[i] != null || fullNames[i] != null)
					m_PTLangProj.Add(new PTProject(shortNames[i], fullNames[i]));
			}
		...
		private void LoadParatextProjectCombos()
		{
			...
			if (cboPTBackTrans.Items.Count == 0)
			{
				...
				cboPTBackTrans.Items.Add(PTProject.Empty);
			...
			if (cboPTTransNotes.Items.Count == 0)
			{
				...
				cboPTTransNotes.Items.Add(PTProject.Empty);
		...
		{
			return ((PTProject)cbo.Items[cbo.SelectedIndex]).ShortName;
		}

		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Determines whether a PT Project matches the specified Paratext Project short name.
		/// </summary>
		/// <param name="shortName">Short name of the project.</param>
		/// <returns>
		/// 	The ParaText Project that matches the specified Paratext Project short name;
		/// otherwise, an empty PTProject.
		/// </returns>
		/// ------------------------------------------------------------------------------------
		private PTProject FindPTProject(string shortName)
		{
			foreach (PTProject proj in m_PTLangProj)
			{
				if (proj.ShortName.Equals(shortName))
					return proj;
			}
			return new PTProject(string.Empty, string.Empty);
		}
		...
		protected void m_btnModify_Click(object sender, System.EventArgs e)
		{
			...
			else
			{
				...
				m_MappingDialog.Initialize((m_projectType == ProjectTypes.Paratext), mapping,
					m_StyleSheet, m_cache, isAnnotationMapping, fLockBtDomain);
			...
			using (new WaitCursor(this))
			{
				switch (m_currentStep)
				{
					case (int)WizSteps.ProjType:
						...
						if (rbParatext6.Checked && m_projectType != ProjectTypes.Paratext)
						{
							if (!PrepareToGetParatextProjectSettings())
								return false;
						}
						else if (rbOther.Checked && m_projectType != ProjectTypes.Other)
		...
		private void panStep1_VisibleChanged(object sender, System.EventArgs e)
		{
			...
			{
				string[] shortNames = ParatextHelper.GetParatextShortNames(PTScriptureText);
				if (shortNames == null && rbParatext6.Checked)
				{
					rbOther.Checked = true;
					//rbParatext6.Checked = false;
				}
			}
		}
		...
		private void m_btnHelp_Click(object sender, System.EventArgs e)
		{
			...
			ShowHelp.ShowHelpTopic(FwApp.App, (string)panSteps[m_currentStep].Tag);
		}
