...
namespace SIL.FieldWorks.ScrImportComponents
{
	...
	internal class ImportWizardWrapper : ImportWizard
	{
		...
		internal class DummyCharacterMappingSettings : CharacterMappingSettings
		{
			...
			public DummyCharacterMappingSettings(ImportMappingInfo mapping, FwStyleSheet styleSheet,
				FdoCache cache)
				: base(mapping, styleSheet, cache, false)
			{
			...
			protected override void SaveMapping()
			{
				...
				m_mapping.IcuLocale = m_MappingDialogDummyData.IcuLocale;
		...
		private ImportMappingInfo m_MappingDialogDummyData = null;
	...
	public class ImportWizardTests: ScrInMemoryFdoTestBase
	{
		...
		protected override void CreateTestData()
		{
			...
			m_scr.ImportSettingsOC.Add(settings);
			settings.Name.UserDefaultWritingSystem = Scripture.kDefaultImportSettingsName;
		...
		public override void Initialize()
		{
			...
			m_settings = (ScrImportSet)ReflectionHelper.GetField(m_importWizard, "m_settings");
		}
		...
		{
			m_scr.ImportSettingsOC.RemoveAll();
			...
			ScrImportSet settings = (ScrImportSet)ReflectionHelper.GetField(m_importWizard, "m_settings");
			...
			ScrImportSet actualImportSettings =
				(ScrImportSet)m_scr.FindImportSettings(TypeOfImport.Other);
			...
			Assert.AreEqual("Default", actualImportSettings.Name.UserDefaultWritingSystem);
		...
		public void MultipleSettings_AddPT6Set()
		{
			...
			settings = (ScrImportSet)ReflectionHelper.GetField(m_importWizard, "m_settings");
			...
			ScrImportSet actualImportSettings =
				(ScrImportSet)m_scr.FindImportSettings(TypeOfImport.Paratext6);
			...
			Assert.AreEqual("Other", actualImportSettings.Name.UserDefaultWritingSystem);
		...
		public void MultipleSettings_AllImportTypes()
		{
			...
			Assert.AreEqual("Paratext6", actualImportSettings.Name.UserDefaultWritingSystem);

			// Another will be for Paratext 5 (as default)
			actualImportSettings =
				(ScrImportSet)m_scr.FindImportSettings(TypeOfImport.Paratext5);
			Assert.IsNotNull(actualImportSettings);
		...
		/// ------------------------------------------------------------------------------------
		private void VerifyImportFiles(ScrImportSet settings, ImportDomain domain,
		...
		public void AddMapping_toNonEmptyList()
		{
			...
			Assert.IsNull(mapping.IcuLocale, "ICU Locale should be based on context");
