...
//			}
		...
		public string WritingSystem
		{
			...
			{
				CheckDisposed();
				return ((NamedWritingSystem)cboWritingSys.SelectedItem).IcuLocale;
			}
		}
		...
		private string DataEncoding
		{
			get
			{
				ILgWritingSystemFactory wsf = this.m_cache.LanguageWritingSystemFactoryAccessor;
				return ((NamedWritingSystem)cboWritingSys.SelectedItem).EnsureRealWs(wsf).LegacyMapping;
			}
		}
		...
		public void Save()
		{
			...
			if (!chkExclude.Checked)
			{
				...
				if (styleItem.Type != StyleType.kstCharacter ||
					m_mapping.StyleName == FdoResources.DefaultParaCharsStyleName)
				{
				...
				m_mapping.IcuLocale = WritingSystem;
			}
		}
		#endregion
