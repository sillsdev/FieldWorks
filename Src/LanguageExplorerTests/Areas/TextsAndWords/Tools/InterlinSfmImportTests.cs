...
namespace LanguageExplorerTests.Areas.TextsAndWords.Tools
{
	...
	{
		private const string input1 = @"\_sh v3.0  943  S Texts
\id Abu
\name Abu Nawas
		...
		public void BasicConversion()
		{
			...
			using (var outputStream = new MemoryStream(output))
			{
				using (var reader = new StreamReader(outputStream))
				{
					...
					VerifyText(phrase1, new[] { "$$", "Uun", "kono'", "serita'", ",", "dau-dau", "(", "tu", ")", "kisa", "Abu", "Nawas", "." }, new HashSet<string>(new[] { ".", ",", "$$", "(", ")" }), "qaa-x-kal");
					...
					VerifyText(phrase2, new[] { "Abu", "Nawas", "kerjo", "ta'", "rojo" }, new HashSet<string>(), "qaa-x-kal");
					...
					VerifyText(phrase3, new[] { "John", "added", "this" }, new HashSet<string>(), "qaa-x-kal");
					...
					VerifyText(phrase4, new[] { "A", "second", "text", "in", "two", "parts" }, new HashSet<string>(), "qaa-x-kal");
					...
					VerifyText(phrase5, new[] { "second", "para" }, new HashSet<string>(), "qaa-x-kal");
					...
					VerifyText(phrase6, new[] { "second", "para", "second", "sentence" }, new HashSet<string>(), "qaa-x-kal");
				}
			}
		}

		// \x85 is ellipsis in cp1252, \u2026
		private const string input2 = @"\_sh v3.0  943  S Texts
" + "\\id Abu\x2026" + @"
\name Abu Nawas
...
\t John added this" + "\x017D";
		...
		public void EncodingConverters()
		{
			...
			encConv.AddConversionMap("XXYTestConverter", "1252", ECInterfaces.ConvType.Legacy_to_from_Unicode, "cp", string.Empty, string.Empty, ECInterfaces.ProcessTypeFlags.CodePageConversion);
		...
		private const string input3 = @"\_sh v3.0  943  S Texts
\id
\ab Abu
\name Abu Nawas
\com a funny story (folk tale?) about the relationship between Abu Nawas and a king
...
";
		...
		public void MultipleTexts()
		{
			...
			using (var outputStream = new MemoryStream(output))
			{
				using (var reader = new StreamReader(outputStream))
				{
					...
					VerifyItem(textElt, "./item[@type='title-abbreviation']", "en", "Abu");
					var phrase1 = textElt.XPathSelectElement("./paragraphs/paragraph/phrases/phrase");
					VerifyText(phrase1, new[] { "John", "added", "this" }, new HashSet<string>(), "qaa-x-kal");
		...
		private static List<InterlinearMapping> GetMappings()
		{
			var mappings = new List<InterlinearMapping>
			{
				...
				{
					Marker = "name",
					Destination = InterlinDestination.Title,
					WritingSystem = "fr"
				},
				new InterlinearMapping()
				{
					Marker = "au",
					Destination = InterlinDestination.Source,
					WritingSystem = "en"
				},
				new InterlinearMapping()
				{
					Marker = "com",
					Destination = InterlinDestination.Comment,
					WritingSystem = "en"
				},
				new InterlinearMapping()
				{
					Marker = "p", Destination = InterlinDestination.ParagraphBreak
				},
				new InterlinearMapping()
				{
					Marker = "ref",
					Destination = InterlinDestination.Reference,
					WritingSystem = "en"
				},
				new InterlinearMapping()
				{
				...
				{
					Marker = "f",
					Destination = InterlinDestination.FreeTranslation,
					WritingSystem = "en"
				},
				new InterlinearMapping()
				{
					Marker = "lt",
					Destination = InterlinDestination.LiteralTranslation,
					WritingSystem = "en"
				},
				new InterlinearMapping()
				{
					Marker = "nt",
					Destination = InterlinDestination.Note,
					WritingSystem = "en"
				}
			};
			return mappings;
		...
		private static void VerifyText(XElement phrase, string[] items, HashSet<string> punctItems, string lang)
		{
