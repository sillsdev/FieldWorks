...
namespace LanguageExplorerTests.Areas.TextsAndWords.Tools
{
	public sealed class BIRDFormatImportTests : MemoryOnlyBackendProviderTestBase
	{
		...
		public void ImportParatextExportBasic()
		{
			...
			{
				var li = new InterlinearTextImport(Cache);
				IText text = null;
				...
				var result = li.ImportInterlinear(options, out text);
		...
		public void ImportWordsWithMultipleWss()
		{
			...
			using (var stream = new MemoryStream(Encoding.ASCII.GetBytes(xml.ToCharArray())))
			{
				...
				var li = new InterlinearTextImportMerge(Cache);
				var result = li.ImportInterlinear(options, out importedText);
				Assert.True(result, "ImportInterlinear was not successful.");
		...
		public void UglyEmptyDataShouldNotCrash()
		{
			...
			using (var stream = new MemoryStream(Encoding.ASCII.GetBytes(xml.ToCharArray())))
			{
				Assert.DoesNotThrow(() => li.ImportInterlinear(new DummyProgressDlg(), stream, 0, out _));
		...
		public void TestImportMergeFlexTextWithSegnumItem()
		{
			...
			{
				var result = li.ImportInterlinear(new DummyProgressDlg(), firstStream, 0, out var text);
				...
				{
					var mergeResult = li.ImportInterlinear(new DummyProgressDlg(), secondStream, 0, out text);
		...
		public void OneOfEachElementTypeTest()
		{
			...
			using (var stream = new MemoryStream(Encoding.ASCII.GetBytes(xml.ToCharArray())))
			{
				li.ImportInterlinear(new DummyProgressDlg(), stream, 0, out _);
				using (var firstEntry = Cache.LanguageProject.Texts.GetEnumerator())
				{
					firstEntry.MoveNext();
					var imported = firstEntry.Current;
		...
		public void TestImportMergeInELANData()
		{
			...
			using (var firstStream = new MemoryStream(Encoding.ASCII.GetBytes(firstxml.ToCharArray())))
			{
				li.ImportInterlinear(new DummyProgressDlg(), firstStream, 0, out var text);
				Assert.AreEqual(0, li.NumTimesDlgShown, "The user should not have been prompted to merge on the initial import.");
				Assert.AreEqual(new Guid("AAAAAAAA-AAAA-AAAA-AAAA-AAAAAAAAAAAA"), text.Guid, "Guid not maintained during import.");
				using (var secondStream = new MemoryStream(Encoding.ASCII.GetBytes(secondxml.ToCharArray())))
				{
					li.ImportInterlinear(new DummyProgressDlg(), secondStream, 0, out text);
					Assert.AreEqual(1, li.NumTimesDlgShown, "The user should have been prompted to merge the text with the same Guid.");
					Assert.AreEqual(new Guid("AAAAAAAA-AAAA-AAAA-AAAA-AAAAAAAAAAAA"), text.Guid, "Guid not maintained during import.");
					...
					{
						li.ImportInterlinear(new DummyProgressDlg(), thirdStream, 0, out text);
		...
		public void TestImportTwiceWithoutMerge()
		{
			...
			var li = new InterlinearTextImportNoMerge(Cache);
			using (var firstStream = new MemoryStream(Encoding.ASCII.GetBytes(importxml.ToCharArray())))
			{
				li.ImportInterlinear(new DummyProgressDlg(), firstStream, 0, out var firstText);
				Assert.AreEqual(0, li.NumTimesDlgShown, "The user should not have been prompted to merge on the initial import.");
				...
				{
					li.ImportInterlinear(new DummyProgressDlg(), secondStream, 0, out var secondText);
					Assert.AreEqual(1, li.NumTimesDlgShown, "The user should have been prompted to merge the text with the same Guid.");
		...
		public void TestImportCreatesReusesExistingSpeaker()
		{
			...
			using (var stream = new MemoryStream(Encoding.ASCII.GetBytes(xml.ToCharArray())))
			{
				li.ImportInterlinear(new DummyProgressDlg(), stream, 0, out var text);
		...
		private static void VerifyMediaLink(IText imported)
		{
		...
		/// <summary>InterlinearTextImport for testing the choice NOT to merge, without actually showing the dialog during unit tests</summary>
		private sealed class InterlinearTextImportNoMerge : InterlinearTextImport
		{
			/// <summary />
			public int NumTimesDlgShown { get; private set; }

			/// <summary />
			public InterlinearTextImportNoMerge(LcmCache cache) : base(cache)
			{
				NumTimesDlgShown = 0;
			}
