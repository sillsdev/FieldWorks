...
namespace LanguageExplorerTests.DictionaryConfiguration
{
	public class CssGeneratorTests : MemoryOnlyBackendProviderRestoredForEachTestTestBase
	{
		...
		private ReadOnlyPropertyTable _readOnlyPropertyTable;
		...
		public override void FixtureSetup()
		{
			...
			_readOnlyPropertyTable = new ReadOnlyPropertyTable(_flexComponentParameters.PropertyTable);
		...
		private GeneratorSettings DefaultSettings => new GeneratorSettings(Cache, _readOnlyPropertyTable, false, false, null);

		private string GenerateCssFromConfiguration(DictionaryConfigurationModel model)
		{
			return CssGenerator.GenerateCssFromConfiguration(model, _readOnlyPropertyTable);
		}

		[Test]
		public void GenerateCssForConfiguration_NullModelThrowsNullArgument()
		{
			Assert.Throws(typeof(ArgumentNullException), () => CssGenerator.GenerateCssFromConfiguration(null, _readOnlyPropertyTable));
		}
		...
		public void GenerateLetterHeaderCss_CssUsesDefinedStyleInfo()
		{
			...
			letHeadStyle.SetExplicitParaIntProp((int)FwTextPropType.ktptAlign, (int)FwTextPropVar.ktpvEnum, (int)FwTextAlign.ktalCenter);
		...
		public void GenerateCssForConfiguration_SimpleConfigurationGeneratesValidCss()
		{
			...
			Assert.IsTrue(Regex.Match(cssResult, @"\.lexentry\s*{\s*clear:both;\s*white-space:pre-wrap;").Success, "Css for root node(lexentry) did not generate 'clear' and 'white-space' rules match");
			// verify that the css result contains a line similar to: .lexentry .headword {
			Assert.IsTrue(Regex.Match(cssResult, @"\.lexentry>\s*\.mainheadword>\s*span\s*{.*").Success, "Css for child node(headword) did not generate a specific match");
		}
		...
		public void GenerateCssForConfiguration_SharedConfigurationGeneratesValidCss()
		{
			...
			Assert.IsTrue(Regex.Match(cssResult, @"\.sharedsubentries\s*\.sharedsubentry>\s*\.mainheadword>\s*span\s*{.*").Success, "Css for child node(headword) did not generate a match{0}{1}", Environment.NewLine, cssResult);
		}
		...
		public void GenerateCssForConfiguration_BeforeAfterSpanConfigGeneratesBeforeAfterCss()
		{
			...
			Assert.IsTrue(Regex.Match(cssResult, @"\.mainheadword>\s*span\s*:\s*first-child:before\s*{\s*content\s*:\s*'Z';\s*}").Success, "css before rule with Z content not found on headword");
			Assert.IsTrue(Regex.Match(cssResult, @"\.mainheadword>\s*span\s*:\s*last-child:after\s*{\s*content\s*:\s*'A';\s*}").Success, "css after rule with A content not found on headword");
		}
		...
		public void GenerateCssForConfiguration_BeforeAfterSpanConfigGeneratesApostropheBeforeBetweenAfterCss()
		{
			...
			Assert.IsTrue(Regex.Match(cssResult, @"\.mainheadword>\s*span\s*:\s*first-child:before\s*{\s*content\s*:\s*'\\\s*'beforeText\\'\s*';\s*}").Success, "css before rule with 'beforeText' content not found on headword");
			Assert.IsTrue(Regex.Match(cssResult, @"\.mainheadword>\s*.mainheadwor\s*\+\s*.mainheadwor:before\s*{\s*content\s*:\s*'\\\s*'betweenText\\'\s*';\s*}").Success, "css before rule with 'betweenText' content not found on headword");
			Assert.IsTrue(Regex.Match(cssResult, @"\.mainheadword>\s*span\s*:\s*last-child:after\s*{\s*content\s*:\s*'\\\s*'afterText\\'\s*';\s*}").Success, "css after rule with 'afterText' content not found on headword");
		}
		...
		public void GenerateCssForConfiguration_BeforeAfterGroupingSpanWorks()
		{
			...
			Assert.IsTrue(Regex.Match(cssResult, @"\.grouping_hwg\s*:before\s*{\s*content\s*:\s*'{';\s*}").Success, "css before rule for the grouping node was not generated");
			Assert.IsTrue(Regex.Match(cssResult, @"\.grouping_hwg\s*:after\s*{\s*content\s*:\s*'}';\s*}").Success, "css after rule for the grouping node was not generated");
			// Check result for before and after rules equivalent to .headword span:first-child{content:'Z';} and .headword span:last-child{content:'A'}
			Assert.IsTrue(Regex.Match(cssResult, @"\.grouping_hwg>\s*\.mh>\s*span\s*:\s*first-child:before\s*{\s*content\s*:\s*'Z';\s*}").Success, "css before rule with Z content not found on headword");
			Assert.IsTrue(Regex.Match(cssResult, @"\.grouping_hwg>\s*\.mh>\s*span\s*:\s*last-child:after\s*{\s*content\s*:\s*'A';\s*}").Success, "css after rule with A content not found on headword");
		}
		...
		public void GenerateCssForConfiguration_BeforeAfterGroupingParagraphWorks()
		{
			...
			var cssResult = GenerateCssFromConfiguration(model);
			// Check the result for before and after rules for the group
			Assert.IsTrue(Regex.Match(cssResult, @"\.grouping_hwg\s*{\s*display\s*:\s*block;\s*}").Success, "paragraph selection did not result in block display for css");
		}
		...
		public void GenerateCssForConfiguration_BetweenSpaceIsNotAddedAfterSingleHeadword()
		{
			...
			Assert.IsTrue(Regex.Match(cssResult, @".*\.lexentry\s*>\s*\.mainheadword>\s*span\.writingsystemprefix\s*\~\s*span\.writingsystemprefix:before{.*content:' ';.*}",
					RegexOptions.Singleline).Success, "Between selector not generated.");
		}
		...
		public void GenerateCssForConfiguration_BeforeAfterConfigGeneratesBeforeAfterCss_SubentryHeadword()
		{
			...
			Assert.IsTrue(Regex.Match(cssResult, @"\.subentries\s*\.subentry>\s*\.headword>\s*span\s*:\s*first-child:before\s*{\s*content\s*:\s*'Z';\s*}").Success, "css before rule with Z content not found on headword");
			Assert.IsTrue(Regex.Match(cssResult, @"\.subentries\s*\.subentry>\s\.headword>\s*span\s*:\s*last-child:after\s*{\s*content\s*:\s*'A';\s*}").Success, "css after rule with A content not found on headword");
		}
		...
		public void GenerateCssForConfiguration_DefinitionOrGlossBeforeAfterConfigGeneratesBeforeAfterCss()
		{
			...
			//SUT
			var cssResult = GenerateCssFromConfiguration(model);
			Assert.IsTrue(Regex.Match(cssResult, @".lexentry> .senses .sense> .definitionorgloss> span:first-child:before{.*content:'<';.*}", RegexOptions.Singleline).Success, "Before not generated.");
			Assert.IsTrue(Regex.Match(cssResult, @".lexentry> .senses .sense> .definitionorgloss> span\+span\[lang\|=\'en\']:before{.*content:',';.*}", RegexOptions.Singleline).Success, "Between not generated.");
			Assert.IsTrue(Regex.Match(cssResult, @".lexentry> .senses .sense> .definitionorgloss> span:last-child:after{.*content:'>';.*}", RegexOptions.Singleline).Success, "After not generated.");
		}
		...
		{
			const int hangingIndent = -15 * 1000;
		...
		{
			const int parentHangingIndent = -8 * 1000;
			const int childHangingIndent = -10 * 1000;
			const string childStyleName = "Dictionary-Paragraph-Padding-Hanging-Child";
		...
		public void GenerateCssForStyleName_ComplexFormsUnderSenses_FirstSenseAndFollowingSenseRulesGenerated()
		{
			const int parentHangingIndent = -8 * 1000;
			const int childHangingIndent = -10 * 1000;
			const int grandChildHangingIndent = -3 * 1000;
			const string grandChildStyleName = "Dictionary-Paragraph-Padding-Hanging-GrandBaby";
			const string childStyleName = "Dictionary-Paragraph-Padding-Hanging-Child";
			var parentStyle = GenerateParagraphStyle("Dictionary-Paragraph-Padding-Hanging-Parent");
			var childStyle = GenerateParagraphStyle(childStyleName);
		...
		{
			const string grandChildStyleName = "Dictionary-Parallel-Passage-Reference-GrandBaby";
			const string childStyleName = "Dictionary-Parallel-Passage-Reference-Child";
			...
			var childDeclaration = CssGenerator.GenerateCssStyleFromLcmStyleSheet(childStyleName, CssGenerator.DefaultStyle, examples, _lcmStyleSheet, _writingSystemManager.get_EngineOrNull(CssGenerator.DefaultStyle), true);
			Assert.That(childDeclaration[0].ToString(), Contains.Substring("page-break-inside:avoid"));
			var parentDeclaration = CssGenerator.GenerateCssStyleFromLcmStyleSheet(parentStyle.Name, CssGenerator.DefaultStyle, examples, _lcmStyleSheet, _writingSystemManager.get_EngineOrNull(CssGenerator.DefaultStyle), true);
		...
		public void GenerateCssForConfiguration_CharStyleSubscriptWorks()
		{
			...
			Assert.IsTrue(Regex.Match(cssResult, @".languageexplorertests.dictionaryconfiguration.testrootclass>\s*span\[lang|='fr']\{.*position\:relative\*top\:-0.2em.*", RegexOptions.Singleline).Success,
		...
		public void GenerateCssForConfiguration_CharStyleBasicUnderlineWorks()
		{
			...
			var fontInfo = new FontInfo
			{
				m_underline = { ExplicitValue = FwUnderlineType.kuntSingle },
				m_underlineColor = { ExplicitValue = Color.HotPink }
			};
			style.SetWsStyle(fontInfo, Cache.DefaultVernWs);
			var headwordNode = new ConfigurableDictionaryNode
			{
		...
		public void GenerateCssForConfiguration_CharStyleDoubleUnderlineWorks()
		{
			...
			var fontInfo = new FontInfo
			{
				m_underline = { ExplicitValue = FwUnderlineType.kuntDouble },
				m_underlineColor = { ExplicitValue = Color.Khaki }
			};
			style.SetWsStyle(fontInfo, Cache.DefaultVernWs);
			var headwordNode = new ConfigurableDictionaryNode
			{
		...
		public void GenerateCssForConfiguration_CharStyleDashedUnderlineWorks()
		{
			...
			var fontInfo = new FontInfo
			{
				m_underline = { ExplicitValue = FwUnderlineType.kuntDashed },
				m_underlineColor = { ExplicitValue = Color.Black }
			};
			style.SetWsStyle(fontInfo, Cache.DefaultVernWs);
			var headwordNode = new ConfigurableDictionaryNode
			{
		...
		public void GenerateCssForConfiguration_CharStyleStrikethroughWorks()
		{
			...
			var fontInfo = new FontInfo
			{
				m_underline = { ExplicitValue = FwUnderlineType.kuntStrikethrough },
				m_underlineColor = { ExplicitValue = Color.Black }
			};
			style.SetWsStyle(fontInfo, Cache.DefaultVernWs);
			var headwordNode = new ConfigurableDictionaryNode
		...
		public void GenerateCssForConfiguration_GramInfoFieldsWork()
		{
			...
			{
				FieldDescription = "MLInflectionClass",
				Style = "FooStyle"
			};
		...
		public void GenerateCssForConfiguration_GeneratesComplexFormTypesBeforeBetweenAfter()
		{
			...
			//SUT
			var cssResult = GenerateCssFromConfiguration(model);
			VerifyRegex(cssResult, @".lexentry> .visiblecomplexformbackrefs> .complexformtypes .complexformtype> .name:before{\s*content:'<';\s*}", "Before not generated:");
			VerifyRegex(cssResult, @".lexentry> .visiblecomplexformbackrefs> .complexformtypes .complexformtype> .name> .nam\+ .nam:before{\s*content:',';\s*}", "Between not generated:");
			VerifyRegex(cssResult, @".lexentry> .visiblecomplexformbackrefs> .complexformtypes .complexformtype> .name:after{\s*content:'>';\s*}", "After not generated:");
		}
		...
		public void GenerateCssForConfiguration_SenseComplexFormsNotSubEntriesHeadWord()
		{
			...
			var model = new DictionaryConfigurationModel
			{
				Parts = new List<ConfigurableDictionaryNode> { entry }
			};
			PopulateFieldsForTesting(entry);
			//SUT
			var cssResult = GenerateCssFromConfiguration(model);
		...
		public void GenerateCssForConfiguration_SenseShowGramInfoFirstWorks()
		{
			...
			Assert.IsTrue(Regex.Match(cssResult, @"\.lexentry>\s*\.senses\s*>\s*\.sharedgrammaticalinfo\s*>\s*\.morphosyntaxanalysisra\s*{.*font-family\s*:\s*'foofoo'\,serif.*}",
		...
		public void GenerateCssForConfiguration_GramInfoFirstHasNoBetweenMaterialWorks()
		{
			...
			//SUT
			var cssResult = GenerateCssFromConfiguration(model);
			Assert.IsTrue(Regex.Match(cssResult, @"\.entry>\s*\.senses>\s*span.sensecontent\+\s*span\:before\{\s*content\:\'\*\'\;",
		...
		public void GenerateCssForConfiguration_WritingSystemAudioWorks()
		{
			...
			Assert.IsTrue(Regex.Match(cssResult, @"a.en-Zxxx-x-audio{.*text-decoration:none;.*}", RegexOptions.Singleline).Success, "Audio not generated.");
		}
		...
		public void GenerateCssForConfiguration_SenseNumberCharStyleWorks()
		{
			...
			// SUT
			var cssResult = GenerateCssFromConfiguration(model);
			Assert.IsTrue(Regex.Match(cssResult, @".*\.lexentry>\s*\.senses\s*>\s*\.sensecontent\s*\.sensenumber",
					RegexOptions.Singleline).Success, "sense number style selector was not generated.");
		...
		public void GenerateCssForConfiguration_ReversalSenseNumberWorks()
		{
			...
			var senses = new ConfigurableDictionaryNode
			{
				...
				{
					DisplayEachSenseInAParagraph = true,
					NumberStyle = "Dictionary-RevSenseNum"
				},
				Children = new List<ConfigurableDictionaryNode> { gloss }
			};
			var entry = new ConfigurableDictionaryNode
		...
		{
			var style = GenerateStyle("Normal");
		...
		public void GenerateCssForConfiguration_PictureSubfieldsBeforeBetweenAfterIsAreGenerated()
		{
			...
			// SUT
			var cssResult = GenerateCssFromConfiguration(config);

			const string senseNumberBefore = @".entry> .pictures .picture> .captionContent .sensenumbertss:before\{\s*content:'\[';";
			...
			const string senseNumberAfter = @".entry> .pictures .picture> .captionContent .sensenumbertss:after\{\s*content:'\]';";
			...
			const string senseNumberBetween = @".entry> .pictures .picture> .captionContent .sensenumbertss> .sensenumberts\+ .sensenumberts:before\{\s*content:', ';";
			...
			const string captionBefore = @".entry> .pictures .picture> .captionContent .caption:before\{\s*content:'\{';";
			...
			const string captionAfter = @".entry> .pictures .picture> .captionContent .caption:after\{\s*content:'\}';";
			...
			const string captionBetween = @".entry> .pictures .picture> .captionContent .caption> .captio\+ .captio:before\{\s*content:' ';";
		...
		public void GenerateCssForConfiguration_PictureBeforeBetweenAfterIsAreGenerated()
		{
			...
			const string pictureBefore = @".entry> .pictures> div:first-child:before\{\s*content:'\[';";
			...
			const string pictureAfter = @".entry> .pictures> div:last-child:after\{\s*content:'\]';";
			...
			const string pictureBetween = @".entry> .pictures> div\+ div:before\{\s*content:', ';";
		...
		public void GenerateCssForCustomFieldWithSpaces()
		{
			...
			var customConfig = new ConfigurableDictionaryNode
			{
				FieldDescription = "Custom Location",
				DictionaryNodeOptions = new DictionaryNodeListOptions(),
				Style = "FooStyle",
				IsCustomField = true,
				...
				CellarPropertyType.ReferenceCollection, Guid.Empty))
			{
				...
				Assert.IsTrue(Regex.Match(cssResult, regexExpected1, RegexOptions.Singleline).Success, "expected custom-location rule not generated");
				const string regexExpected2 = @"\.lexentry>\s\.custom-location \.custom-locatio>\s\.name{[^}]*}";
				Assert.IsTrue(Regex.Match(cssResult, regexExpected2, RegexOptions.Singleline).Success, "expected custom-location>name rule not generated");
				const string regexExpected3 = @"\.lexentry>\s\.custom-location \.custom-locatio>\s\.abbreviation{[^}]*}";
				Assert.IsTrue(Regex.Match(cssResult, regexExpected3, RegexOptions.Singleline).Success, "expected custom-location>abbreviation rule not generated");
			}
		}
		...
		public void GenerateCssForDuplicateConfigNodeWithSpaces()
		{
			...
			// SUT
			var cssResult = GenerateCssFromConfiguration(model);
			const string regexExpected1 = @"\.lexentry>\s\.note_test-one{[^}]*}";
			Assert.IsTrue(Regex.Match(cssResult, regexExpected1, RegexOptions.Singleline).Success, "expected duplicated config node rename rule not generated");
		}
		...
		public void GenerateCssForDuplicateConfigNodeWithPunc()
		{
			...
			// SUT
			var cssResult = GenerateCssFromConfiguration(model);
			const string regexExpected1 = @"\.lexentry>\s\.note_-test{[^}]*}";
			Assert.IsTrue(Regex.Match(cssResult, regexExpected1, RegexOptions.Singleline).Success, "expected duplicated config node rename rule not generated");
		}
		...
		public void GenerateCssForCollectionBeforeAndAfter()
		{
			...
			const string regexItem2 = @".entry> .pronunciations .pronunciation> .form> span:first-child:before\{\s*content:'\[';\s*\}";
			...
			const string regexItem3 = @".entry> .pronunciations .pronunciation> .form> span:last-child:after\{\s*content:'\]';\s*\}";
			...
			const string regexCollection1 = @".entry> .pronunciations> .pronunciation\+ .pronunciation:before\{\s*content:', ';\s*\}";
			...
			const string regexCollection2 = @".entry> .pronunciations:before\{\s*content:'\{Pron: ';\s*\}";
			...
			const string regexCollection3 = @".entry> .pronunciations:after\{\s*content:'\} ';\s*\}";
		...
		public void GenerateCssForConfiguration_SpecificLanguageColorIsNotOverridenByParagraphStyle()
		{
			...
			//SUT
			var result = GenerateCssFromConfiguration(model);
		...
		private static void EnableAllListOptions(DictionaryNodeOptions options)
		{
			...
			switch (options)
			{
				...
				case DictionaryNodeListOptions dictionaryNodeListOptions:
					...
					break;
				case DictionaryNodeWritingSystemOptions dictionaryNodeWritingSystemOptions:
		...
		private void GenerateBulletStyle(string name)
		{
			var fontInfo = new FontInfo
			{
				m_fontColor = { ExplicitValue = Color.Green },
				m_fontSize = { ExplicitValue = 14000 },
				m_fontName = { ExplicitValue = "Arial" },
				m_bold = { ExplicitValue = true },
				m_italic = { ExplicitValue = true },
				m_underline = { ExplicitValue = FwUnderlineType.kuntNone },
				m_backColor = { ExplicitValue = Color.Blue }
			};
			...
			{
				m_fontColor = { ExplicitValue = Color.Red },
				...
				case (FwUnderlineType.kuntSingle):
					{
						...
						Assert.That(css, Contains.Substring("text-decoration-color:" + HtmlColor.FromRgb(underlineColor.R, underlineColor.G, underlineColor.B)), "underline color missing");
						break;
					}
				case (FwUnderlineType.kuntDashed):
					{
						Assert.That(css, Contains.Substring("border-bottom:1px dashed"), "dashed underline not applied");
						Assert.That(css, Contains.Substring("border-bottom-color:" + HtmlColor.FromRgb(underlineColor.R, underlineColor.G, underlineColor.B)), "underline color missing");
						break;
					}
				...
				case (FwUnderlineType.kuntStrikethrough):
					{
						...
						Assert.That(css, Contains.Substring("text-decoration-color:" + HtmlColor.FromRgb(underlineColor.R, underlineColor.G, underlineColor.B)), "strike through color missing");
						break;
					}
