...
namespace LanguageExplorerTests.DictionaryConfiguration.Migration
{
	public class FirstBetaMigratorTests : MemoryOnlyBackendProviderRestoredForEachTestTestBase
	{
		...
		private FirstBetaMigrator _migrator;
		...
		public void MigrateFrom83Alpha_UpdatesVersion()
		{
			...
			_migrator.MigrateFrom83Alpha(_logger, alphaModel, new DictionaryConfigurationModel { Parts = new List<ConfigurableDictionaryNode>() }); // SUT
			Assert.AreEqual(DictionaryConfigurationServices.VersionCurrent, alphaModel.Version);
		}
...
name='French Reversal Index' writingSystem='fr' version='21' lastModified='2020-03-11' allPublications='true' isRootBased='false'>
  <ConfigurationItem name='Reversal Entry' isEnabled='true' style='Reversal-Normal' styleType='paragraph' field='ReversalIndexEntry' cssClassNameOverride='reversalindexentry'/>
				...
				var filePath1 = Path.Combine(configLocations, "fr" + LanguageExplorerConstants.DictionaryConfigurationFileExtension);
...
name='French Reversal Index 2' writingSystem='fr' version='21' lastModified='2020-03-11' allPublications='true' isRootBased='false'>
  <ConfigurationItem name='Reversal Entry' isEnabled='true' style='Reversal-Normal' styleType='paragraph' field='ReversalIndexEntry' cssClassNameOverride='reversalindexentry'/>
				...
				var filePath2 = Path.Combine(configLocations, "fr2" + LanguageExplorerConstants.DictionaryConfigurationFileExtension);
...
name='French Reversal Index 3' writingSystem='fr' version='21' lastModified='2020-03-11' allPublications='true' isRootBased='false'>
  <ConfigurationItem name='Reversal Entry' isEnabled='true' style='Reversal-Normal' styleType='paragraph' field='ReversalIndexEntry' cssClassNameOverride='reversalindexentry'/>
				...
				var filePath3 = Path.Combine(configLocations, "fr3" + LanguageExplorerConstants.DictionaryConfigurationFileExtension);
				File.WriteAllText(filePath3, content3);
				_migrator.MigrateIfNeeded(); // SUT
				var todoList = DictionaryConfigurationServices.GetConfigsNeedingMigration(Cache, DictionaryConfigurationServices.VersionCurrent);
		...
		public void MigrateFrom83Alpha_ItemsMovedIntoGroupsAreMoved()
		{
			...
			_migrator.MigrateFrom83Alpha(_logger, alphaModel, defaultModelWithGroup); // SUT
		...
		public void MigrateFrom83Alpha_GroupPlacedAtEndIfNoPreceedingSiblingFound()
		{
			...
			CssGeneratorTests.PopulateFieldsForTesting(defaultModelWithGroup);
			// SUT
			_migrator.MigrateFrom83Alpha(_logger, alphaModel, defaultModelWithGroup);
		...
		public void MigrateFrom83Alpha_GroupPropertiesClonedFromNewDefaults()
		{
			...
			const string before = "[";
			const string after = "]";
			const string style = "retro";
			const string label = "Integrity";
			var groupNode = new ConfigurableDictionaryNode
			{
				Before = before,
				After = after,
		...
		public void MigrateFrom83Alpha_ConflatesMainEntriesForLexemey([Values(true, false)] bool isHybrid)
		{
			...
			var mainEntryNode = new ConfigurableDictionaryNode
			{
				Label = "Main Entries",
				FieldDescription = LexEntry,
				Children = new List<ConfigurableDictionaryNode> { extantChildNode, mainGroupingNode }
			};
			var componentsNode = new ConfigurableDictionaryNode
			{
				FieldDescription = "Components",
				Before = componentsBefore
			};
			var hiddenByCustomProblemNode = new ConfigurableDictionaryNode
			{
				Label = ComponentReferences,
				FieldDescription = "ComplexFormEntryRefs"
			};
			var extantChildUnderComplexNode = new ConfigurableDictionaryNode
			{
				FieldDescription = KidField,
				Before = "This is not mine: "
			};
			var otherUniqueChildNode = new ConfigurableDictionaryNode
			{
				FieldDescription = "ComplexKid"
			};
			...
			_migrator.MigrateFrom83Alpha(_logger, alphaModel, betaModel); // SUT
		...
		{
			var reversalModel = new DictionaryConfigurationModel
			{
				WritingSystem = "en"
			};
		...
		public void MigrateFrom83Alpha_ExtendedNoteChildrenAreMigrated()
		{
			...
			// SUT
			_migrator.MigrateFrom83Alpha(_logger, alphaModel, defaultModel);
		...
		public void MigrateFromConfig83AlphaToBeta10_UpdatesEtymologyCluster()
		{
			...
			var rootModel = _migrator.LoadBetaDefaultForAlphaConfig(alphaModel);
			_migrator.MigrateFrom83Alpha(_logger, alphaModel, rootModel);
		...
		public void MigrateFromConfig83AlphaToBeta10_PathologicalEtymologyCaseDoesNotThrow()
		{
			...
			var rootModel = _migrator.LoadBetaDefaultForAlphaConfig(alphaModel);
			Assert.DoesNotThrow(() => _migrator.MigrateFrom83Alpha(_logger, alphaModel, rootModel));
		}

		private static void TestForWritingSystemOptionsType(ConfigurableDictionaryNode configNode, WritingSystemType expectedWsType)
		{
		...
		public void MigrateFrom83Alpha_DoesNotAddDirectChildrenToSharingParents() // LT-18286
		{
			...
			_migrator.MigrateFrom83Alpha(_logger, version12Model, version16Model); // SUT
		...
		public void MigrateFrom83Alpha_RemovesErroneouslyAddedChildren() // LT-18286
		{
			...
			_migrator.MigrateFrom83Alpha(_logger, version16Model, version17Model); // SUT
		...
		{
			DictionaryConfigurationServices.PerformActionOnNodes(model.PartsAndSharedItems, node =>
			{
				if (!string.IsNullOrEmpty(node.ReferenceItem))
				{
					Assert.IsTrue(node.Children == null || !node.Children.Any(), "Reference Item and children are exclusive:\n" + DictionaryConfigurationServices.BuildPathStringFromNode(node));
				}
			});
		}
	}
