...
#if !WIN32
/** Normal style name
 * MarkS: The comment at AfStylesDlg.cpp:23 makes this definition seem
 * undesirable to do here. But I don't want to link in AfStylesDlg.o. How this definition
 * is done could be changed but I'll leave it here for now.
 */
static const wchar * g_pszwStyleNormal = OleStringLiteral(L"Normal");

/**
 * Register our COM parts and notify of our CreateCom method.
 */
static GenericFactory g_fact(
	_T("SIL.WorldPad.DataAccess"),
	&CLSID_WpDa,
	_T("SIL.WorldPad.DataAccess"),
	_T("Apartment"),
	&WpDa::CreateCom);
...
/**
 * @brief Method to support using GenericFactory to create an instance.
 * @param punkCtl
 * @param riid
 * @param ppv
 */
void WpDa::CreateCom(IUnknown *punkCtl, REFIID riid, void ** ppv)
{
	fprintf(stderr, "DEBUG: entering WpDa::CreateCom()\n");
	AssertPtr(ppv);
	Assert(!*ppv);
	if (punkCtl)
		ThrowHr(WarnHr(CLASS_E_NOAGGREGATION));

	ComSmartPtr<WpDa> qwpda;
	qwpda.Attach(NewObj WpDa()); // ref count initialy 1
	CheckHr(qwpda->QueryInterface(riid, ppv));
}

STDMETHODIMP WpDa::QueryInterface(REFIID riid, void **ppv)
{ // TODO-Linux: (MarkS) just use COMBase
	AssertPtr(ppv);
	if (!ppv)
	...
	*ppv = NULL;
	...
	else if (riid == IID_IWpDa)
		*ppv = static_cast<IWpDa *>(this);
...
#else
		//TODO-Linux: do we want WpApp ; can we work around it? Maybe can just set wsf another way if desired. Consider other related uses of wsf from MainApp in this file.
		fprintf(stderr, "Warning: Part of WpDa::putref_WritingSystemFactory is not yet implemented in Linux. Writing system factory not set.\n");
#endif
		return S_OK;
	}
	return E_FAIL;
...
----------------------------------------------------------------------------------------------*/
void WpDa::InitNew(StrAnsi staFileName, int * pfiet, ISimpleMainWnd * pwpwnd, ISimpleChildWnd * pwcwnd)
{
#if WIN32
...
#if WIN32
	ISimpleMainWndPtr qwpwndLauncher = pwpwnd->LauncherWindow();
#else //!WIN32
	ISimpleMainWndPtr qwpwndLauncher;
	HRESULT hr = pwpwnd->LauncherWindow(&qwpwndLauncher);
#endif //!WIN32
	if (!qwpwndLauncher)
...
#else // !WIN32
	// TODO-Linux
	fprintf(stderr,"Warning: Part of WpDa::InitNew is not implemented. Writing system factory not fetched.\n");
	ILgWritingSystemFactoryPtr qwsf = NULL;
#endif // !WIN32
	int wsDefault;
...
void WpDa::InitNewEmpty()
{
	...
	static const OleStringLiteral literalEmpty = L"";
	CheckHr(qtsf->MakeStringRgch(literalEmpty, 0, wsDefault, &qtss));
...
/*----------------------------------------------------------------------------------------------
	...
	@param pttp			- properties from the current selection, to use when loading into an
							empty window
----------------------------------------------------------------------------------------------*/
void WpDa::LoadIntoEmpty(StrAnsi staFileName, int * pfiet, ISimpleMainWnd * pwpwnd,
	ISimpleChildWnd * pwcw, ITsTextProps * pttp, int * pfls)
{
#if WIN32
	WaitCursor wc; // creates a wait cursor and makes it active until the end of the method.
#else // TODO-Linux
...
#else // !WIN32
			fprintf(stderr, "Warning: WpDa::GetTextFromFile is not fully implemented.\n");
			// TODO-Linux: (MarkS) transfer WIN32 logic to here if we end up using this
			// code in the WPX load+save library in Linux.
			*pfiet = kfietUtf8;
#endif // !WIN32
...
----------------------------------------------------------------------------------------------*/
bool WpDa::SaveToFile(StrAnsi staFileName, int fiet, ISimpleMainWnd * pwpwnd)
{
#if WIN32
	WaitCursor wc; // creates a wait cursor and makes it active until the end of the method.
#else // TODO-Linux
...
#if WIN32 // TODO-Linux: (MarkS) use this logic in Linux if needed
	...
	return true;
#else //!WIN32
	fprintf(stderr,"Warning: WpDa::SaveToFile not fully implemented.\n");
	return false;
#endif //!WIN32
}
...
#else //!WIN32
	ISimpleStylesheetPtr qsts;
	CheckHr(m_pwpwnd->GetStylesheet(&qsts));
#endif //!WIN32
	ITsTextPropsPtr qttp;
#if WIN32
...
#else // !WIN32
	SmartBstr bstrNormalstring(g_pszwStyleNormal);
	int cchNormal = BstrLen(bstrNormalstring);
	CheckHr(qsts->GetStyleRgch(cchNormal, bstrNormalstring, &qttp));
#endif // !WIN32
	int nVar, nVal;
...
// TODO-Linux: Implement any needed methods.

bool WpDa::GetWsDescription(const achar * pszWsKey, StrApp & str)
{
	fprintf(stderr,"Warning: WpDa::GetWsDescription not implemented.\n");
	return false;
}
bool WpDa::DeleteRegWsKey()
{
	fprintf(stderr,"Warning: WpDa::DeleteRegWsKey not implemented.\n");
	return false;
}
#endif // !WIN32
...
#else
	// TODO-Linux
	fprintf(stderr,"Warning: WpDa::InitWithWsInRegistry not fully implemented. Not getting writing systems from registry.\n");
#endif
	if (!vstrEncs.Size())
		return;				// Nothing more to do.

	static const OleStringLiteral literal___ = L"___";
...
#if WIN32
		if (_tcscmp(vstrEncs[istr].Chars(), literal___) == 0)
			continue;
#else //!WIN32
		if (UnicodeString(vstrEncs[istr].Chars()) == UnicodeString(literal___))
			continue;
#endif //!WIN32
...
void WpDa::PersistAllWs(HWND hwndMain)
{
	...
	qprog->SetStep(1);
#else // !WIN32
	// TODO-Linux: implement in Linux if desired (and a couple hashifWIN32's below)

	fprintf(stderr, "Warning: WpDa::PersistAllWs not fully implemented.\n");
#endif // !WIN32
...
#else // !WIN32
		fprintf(stderr, "Warning: WpDa::SetUpWsFactory not fully implemented. Not setting or creating writing system factory.\n");
		m_qwsf = NULL; // TODO-Linux
#endif // !WIN32
		if (!m_qwsf)
		{
