...
#include "Main.h"
#else //!WIN32
#include "common.h" // Generic/common.h
#include "WpDa.h"
#include "res/resource.h"
#include "AppCore/Res/AfAppRes.h"
#include "AppCore/WriteXml.h"
#include <winuser.h>
#include <WinBase.h>
#include "AppCore/FwStyledText.h"
#endif //!WIN32
#pragma hdrstop
...
/*----------------------------------------------------------------------------------------------
	...
	@param fDtd if should put DOCTYPE in output file
----------------------------------------------------------------------------------------------*/
HRESULT WpDa::SaveXml(BSTR bstrFile, ISimpleMainWnd * pwpwnd, ComBool fDtd)
{
	// TODO-Linux: should be BEGIN_COM_METHOD/END_COM_METHOD around this method
...
#if !WIN32
	// WpDa::InitNew() isn't called in Linux WpGTK, so set m_pwpwnd here in this entry point / method.
	m_pwpwnd = pwpwnd;
#endif
...
#else // !WIN32
	// TODO-Linux
	fprintf(stderr, "Warning: WpDa::SaveXml error handling not implemented.\n");
#endif // !WIN32
		return E_FAIL;
	}
	catch (...)
...
----------------------------------------------------------------------------------------------*/
void WpDa::SaveXmlToStream(IStream * pstrm, ISimpleMainWnd * pwpwnd, bool fDtd)
{
...
#else //!WIN32
	ISimpleStylesheetPtr qissStylesheet;
	pwpwnd->GetStylesheet(&qissStylesheet);
	WriteXmlStyles(pstrm, qissStylesheet, setws);
#endif //!WIN32
	WriteXmlBody(pstrm);
...
#else
void WpDa::WriteXmlStyles(IStream * pstrm, ISimpleStylesheet * pasts, Set<int> & setws)
#endif
{
	SetUpWsFactory();
...
#else //!WIN32
	const int nMaxNumberOfStyles = 100;

	int styleHvos[nMaxNumberOfStyles];
	int styleHvosLength = nMaxNumberOfStyles;
	for (int i=0; i < styleHvosLength; i++)
		styleHvos[i] = -1;
	CheckHr(pasts->GetArrayOfStyleHvos(styleHvosLength, styleHvos));
#endif //!WIN32

#if WIN32
	for (int ist = 0; ist < vhcStyles.Size(); ist++)
#else // !WIN32
	for (int ist = 0; styleHvos[ist] != -1; ist++)
#endif // !WIN32
	{
...
----------------------------------------------------------------------------------------------*/
void WpDa::WriteXmlPageSetup(IStream * pstrm, ISimpleMainWnd * pwpwnd)
{
	SetUpWsFactory();
...
#else //!WIN32
	int topMargin, bottomMargin, leftMargin, rightMargin, headerMargin, footerMargin;
	int pageSize, pageHeight, pageWidth, pageOrientation;

	pwpwnd->TopMargin(&topMargin);
	pwpwnd->BottomMargin(&bottomMargin);
	pwpwnd->LeftMargin(&leftMargin);
	pwpwnd->RightMargin(&rightMargin);
	pwpwnd->HeaderMargin(&headerMargin);
	pwpwnd->FooterMargin(&footerMargin);
	pwpwnd->PageSize(&pageSize);
	pwpwnd->PageHeight(&pageHeight);
	pwpwnd->PageWidth(&pageWidth);
	pwpwnd->PageOrientation(&pageOrientation);
...
template class Vector<ElemTypeInfo>;
...
template class Vector<SeqPropInfo>;
...
struct WpXmlImportData
{
	...
	WpXmlImportData(WpDa * pda, ISimpleMainWnd * pwpwnd, ISimpleMainWnd * pwpwndLauncher,
		ISimpleStylesheet * pwpsts, bool fOverwriteEnc);
	~WpXmlImportData();
...
#if !WIN32
	static void ConvertHmhvostuToTwoVectorInt(HashMap<HVO, StrUni> * inHmhvostu,
		Vector<int> * outHmKeys, Vector<int> * outHmValueHvos, ISimpleStylesheet * stylesheet);
#endif
	...
	HRESULT m_hr;
	WpDaPtr m_qda;
	ISimpleMainWndPtr m_qwpwnd;			// main window, for storing page-setup information
	ISimpleMainWndPtr m_qwpwndTop;		// current visible window, for giving modal error messages
	ISimpleStylesheetPtr m_qwpsts;
	IStreamPtr m_qstrm;
...
/*----------------------------------------------------------------------------------------------
	Constructor.
----------------------------------------------------------------------------------------------*/
WpXmlImportData::WpXmlImportData(WpDa * pda, ISimpleMainWnd * pwpwnd, ISimpleMainWnd * pwpwndLauncher,
	ISimpleStylesheet * pwpsts, bool fOverwriteWs)
	:	m_qda(pda),
		m_qwpwnd(pwpwnd),
		...
		m_qwpsts(pwpsts)
{
	...
	{
		HVO hvoNextStyleHvo;
		pwpsts->NextStyleHVO(&hvoNextStyleHvo); // TODO-Linux: just pass in m_hvoNextStyle if that works.
		m_hvoNextStyle = hvoNextStyleHvo;
	} else {
...
#else // !WIN32
		// TODO-Linux: implement if desired
		fprintf(stderr, "Warning: WpXmlImportData::CreateErrorMessage MessageBox not shown. Message: %s\n", staMsg.Chars());
#endif // !WIN32
	}
//	else if (m_cErrMsg == 1)
//	{
...
#else // !WIN32
		// TODO-Linux: implement if desired
		fwprintf(stderr, L"Warning: WpXmlImportData::FinalErrorMessage MessageBox not shown. Message: %s\n", strMsg.Chars());
#endif // !WIN32
	}
	return nRet;
}
...
#else // !WIN32
		// TODO-Linux: implement if desired
		fprintf(stderr, "Warning: WpXmlImportData::AbortErrorMessage MessageBox not shown. Message: %s\n", staMsg.Chars());
#endif // !WIN32
	}
}
...
#if !WIN32
/**
 * @summary Convert a HashMap<HVO, StrUni> into two Vector<int>s. The keys of the hashmap
 * will be put into one vector and the values of the hashmap will be placed into the other
 * vector. The values in the hashmap should be strings matching names of styles in the
 * stylesheet, and they will first be looked up and translated into HVOs to be placed into
 * the output vector for values.
 * Note that there doesn't have to be any relationship between the HashMap's HVO keys and
 * its StrUni value style names. This is a helper function to break apart the BasedOn
 * and NextStyle hashmaps into Vector<int>s that can be more easily passed over COM.
 * @param inHmhvostu input hashmap of HVOs and style names
 * @param outHmKeys output vector of HVOs from the hashmap's keys
 * @param outHmValueHvos output vector of HVOs based on the hashmap's values. If a name doesn't match an HVO, -1 will be set instead of the HVO.
 * @param stylesheet stylesheet to use to translate style names to style HVOs
 */
void WpXmlImportData::ConvertHmhvostuToTwoVectorInt(HashMap<HVO, StrUni> * inHmhvostu,
	Vector<int> * outHmKeys, Vector<int> * outHmValueHvos, ISimpleStylesheet * stylesheet)
{
	// Check arguments
	if (NULL == inHmhvostu)
	{
		fprintf(stderr, "WpXmlImportData::ConvertHmhvostuToTwoVectorInt: Error: argument inHmhvostu can not be null.\n");
		return;
	}
	if (NULL == outHmKeys)
	{
		fprintf(stderr, "WpXmlImportData::ConvertHmhvostuToTwoVectorInt: Error: argument outHmKeys can not be null.\n");
		return;
	}
	if (NULL == outHmValueHvos)
	{
		fprintf(stderr, "WpXmlImportData::ConvertHmhvostuToTwoVectorInt: Error: argument outHmValueHvos can not be null.\n");
		return;
	}
	if (NULL == stylesheet)
	{
		fprintf(stderr, "WpXmlImportData::ConvertHmhvostuToTwoVectorInt: Error: argument stylesheet can not be null.\n");
		return;
	}

	// Process

	HashMap<HVO, StrUni>::iterator hmIteratorEnd = inHmhvostu->End();
	for (HashMap<HVO, StrUni>::iterator hmIterator = inHmhvostu->Begin();
		hmIterator != hmIteratorEnd; ++hmIterator)
	{
		outHmKeys->Push(hmIterator.GetKey());

		// First convert value from style name to style hvo
		StrUni valueString = hmIterator.GetValue();
		HVO valueHvo;
		stylesheet->GetStyleHvoByName(valueString.Bstr(), &valueHvo);
		if (valueHvo < 0)
			fprintf(stderr, "WpXmlImportData::ConvertHmhvostuToTwoVectorInt: Warning: inHmhvostu contained stylename value that was not in the stylesheet.\n");
		outHmValueHvos->Push(valueHvo);
	}
}
#endif
...
#else // !WIN32
	// TODO-Linux: implement if desired
	fprintf(stderr, "Warning: WpXmlImportData::HandleExternalEntityRef MessageBox not shown. Message: %s\n", stab.Chars());
#endif // !WIN32
#endif // 99
	...
	case keltySection:
		...
		case kwpsStyles:
		{
			...
			for (ipv = 0; ipv < pxid->m_vwpcClosed.Size(); ipv++)
			{
				...
				prghvoStyles[ipv] = (HVO)(long)pxid->m_vpvClosed[ipv];
			}
			pxid->m_qwpsts->AddLoadedStyles(prghvoStyles, cpv, pxid->m_hvoNextStyle);

			// Set basedOn and nextStyle information in DA
#if WIN32
			pxid->m_qwpsts->FixStyleReferenceAttrs(kflidStStyle_BasedOn,
				pxid->m_hmhvostuBasedOn);
#else //!WIN32
			// Create two int Vectors to let us pass the basedOn information over COM

			Vector<int> basedOn_StylesToModify; // list of hvos of styles to modify an attribute of
			Vector<int> basedOn_AttributeValuesToSet; // list of hvos, which are attribute values to set
			ConvertHmhvostuToTwoVectorInt(&pxid->m_hmhvostuBasedOn, &basedOn_StylesToModify,
				&basedOn_AttributeValuesToSet, pxid->m_qwpsts);

			pxid->m_qwpsts->FixStyleReferenceAttrs(kflidStStyle_BasedOn,
				&basedOn_StylesToModify[0], &basedOn_AttributeValuesToSet[0],
				basedOn_StylesToModify.Size());
#endif //!WIN32
			...
			// Create two int Vectors to let us pass the nextStyle information over COM

			Vector<int> next_StylesToModify; // list of hvos of styles to modify an attribute of
			Vector<int> next_AttributeValuesToSet; // list of hvos, which are attribute values to set
			ConvertHmhvostuToTwoVectorInt(&pxid->m_hmhvostuNextStyle, &next_StylesToModify,
				&next_AttributeValuesToSet, pxid->m_qwpsts);

			pxid->m_qwpsts->FixStyleReferenceAttrs(kflidStStyle_Next,
				&next_StylesToModify[0], &next_AttributeValuesToSet[0], next_StylesToModify.Size());
#endif //!WIN32
		...
		case kwpsBody:
			...
			for (ipv = 0; ipv < cpv; ipv++)
			{
				...
				prghvoPara[ipv] = (HVO)(long)pxid->m_vpvClosed[ipv];
			}
...
void WpXmlImportData::SetIntegerField(WpXmlImportData * pxid, int nVal)
{
	...
	case kwpcPageInfo:
		pwpwnd = reinterpret_cast<ISimpleMainWnd *>(pvObject);
		switch (etiProp.m_wpf)
		{
		case kwpfTopMargin:
...
void WpXmlImportData::SetUnicodeField(WpXmlImportData * pxid, BSTR bstr)
{
	...
	StrUni stu;
	int ws;
	static const OleStringLiteral literal_keyman = L"keyman";
	...
	case kwpcOws:
		...
		case kwpfKeyboardType:
#if WIN32
			if (wcscmp(bstr, literal_keyman) == 0)
#else //!WIN32
			if (UnicodeString(bstr) == UnicodeString(literal_keyman))
#endif //!WIN32
...
#else // !WIN32
		//TODO-Linux
		fprintf(stderr, "Warning: WpXmlImportData::HandleTextPropStartTag not fully implemented for External Links.\n");
#endif // !WIN32
...
void WpXmlImportData::HandleStringEndTag(void * pvUser, const XML_Char * pszName)
{
	...
	if (strcmp(pszName, "Str") == 0 || strcmp(pszName, "AStr") == 0)
	{
		if (pxid->m_stuChars.Length() == 0 && pxid->m_qtpbStr)
		{
			...
			ITsStrFactoryPtr qtsf;
			qtsf.CreateInstance(CLSID_TsStrFactory);
			qtsf->MakeStringWithPropsRgch(OleStringLiteral(L""), 0, qttp, &qtss);
...
----------------------------------------------------------------------------------------------*/
HRESULT WpDa::LoadXml(BSTR bstrFile, ISimpleMainWnd * pwpwnd, ISimpleMainWnd * pwpwndLauncher, int * pfls)
{
	// TODO-Linux: surround method with BEGIN_COM_METHOD/END_COM_METHOD
	AssertPtrN(bstrFile);
	if (!bstrFile)
	{
		*pfls = kflsAborted;
...
#if WIN32
	ISimpleStylesheetPtr qwpsts = dynamic_cast<ISimpleStylesheet *>(pwpwnd->GetStylesheet());
#else //!WIN32
	ISimpleStylesheetPtr qwpsts;
	CheckHr(pwpwnd->GetStylesheet(&qwpsts));
#endif //!WIN32
...
#else // !WIN32
	// TODO-Linux
	fprintf(stderr, "Warning: WpDa::ParseLanguageDefinitionFile not implemented.\n");
#endif // !WIN32
}
