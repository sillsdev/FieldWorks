...
#else // !WIN32
	const wchar_t * m_pszEth;
	const wchar_t * m_pszISO;
...
#else //!WIN32
	int cchEthCode = UnicodeString(pszEthCode).length();
#endif //!WIN32
	if (cchEthCode < 3)
		return pszEthCode;
...
#else //!WIN32
			int nCmp = StrUni(g_rgeil[ivMid].m_pszEth).Compare(StrUni(pszEthCode));
#endif //!WIN32
			if (nCmp == 0)
#if WIN32
				return g_rgeil[ivMid].m_pszISO;
#else // !WIN32
				return StrUni(g_rgeil[ivMid].m_pszISO);
#endif // !WIN32
			else if (nCmp < 0)
				iv = ivMid + 1;
			else
...
#if WIN32
		int cmp = wcscmp(g_rgeil[iv].m_pszEth, pszEthCode);
#else //!WIN32
		int cmp = StrUni(g_rgeil[iv].m_pszEth).Compare(StrUni(pszEthCode));
#endif //!WIN32
		if (iv < g_ceil && !cmp)
			return StrUni(g_rgeil[iv].m_pszISO);
	}
