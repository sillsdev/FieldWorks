...
<!-- Since version 1.0.9 is the first one to be released, this should never be used (except in tests)! -->
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	xmlns:o="http://www.wycliffe.net/scripture/namespace/version_1.0.8"
	 xpath-default-namespace="http://www.wycliffe.net/scripture/namespace/version_1.0.8">
	...
	<xsl:template match="o:oxes/@version">
		<xsl:attribute name="version">1.0.9</xsl:attribute>
	</xsl:template>

	<xsl:template match="o:oxesText/@type">
		<xsl:variable name="val" select="."/>
		<xsl:variable name="ver">1.0.9</xsl:variable>
		<xsl:attribute name="type">
			<xsl:value-of select="substring-before($val, '1.0.8')"/>
	...
	<xsl:template match="*">
		<xsl:element name="{local-name()}"
			namespace="http://www.wycliffe.net/scripture/namespace/version_1.0.9">
