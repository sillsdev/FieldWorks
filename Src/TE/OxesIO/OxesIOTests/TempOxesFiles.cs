...
namespace SIL.OxesIO
{
	...
	public class TempOxesFiles
	{
		...
		internal static string MinimalValidFile(string path)
		{
			...
			bldr.AppendLine("<oxes xmlns='http://www.wycliffe.net/scripture/namespace/version_1.1.4'>");
			...
			if (String.IsNullOrEmpty(path))
				path = FileUtils.GetTempFile("oxes");
			FileUtils.WriteStringtoFile(path, bldr.ToString(), Encoding.UTF8);
			return path;
		}
		...
		internal static string EmptyOxesFile(string path)
		{
			if (String.IsNullOrEmpty(path))
				path = FileUtils.GetTempFile("oxes");
			string write = "<oxes xmlns='http://www.wycliffe.net/scripture/namespace/version_1.1.4'></oxes>";
			FileUtils.WriteStringtoFile(path, write, Encoding.UTF8);
			return path;
		}
	}
}
