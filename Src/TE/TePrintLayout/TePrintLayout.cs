		...
		ITeView, IBtAwareView
	{
		...
		protected override void Dispose(bool disposing)
		{
			...
			if (disposing)
			{
				...
				// unsubscribe to the FilterChanged. This is especially important when
				// running tests since we dispose the scripture publication after each test
				// but the GC probably doesn't run before the next test starts. Eventually
				// the GC will remove us from the event handler, but when running tests this
				// is to late.
		...
		protected void AddDivisionsForBooks()
		{
			...
			else
			{
				...
				foreach (int bookHvo in m_bookFilter.BookHvos)
				{
					...
					DivisionLayoutMgr div = GetDivisionLayoutMgr(true, bookHvo);
					...
					{
						div = GetDivisionLayoutMgr(false, bookHvo);
						div.Name = name + "_Main";
						AddDivision(div);
					}
					iBook++;
		...
		/// </remarks>
		/// ------------------------------------------------------------------------------------
		protected virtual DivisionLayoutMgr GetDivisionLayoutMgr(bool fIntroDivision,
			int hvoBook)
		{
			// An intro division always has just one column
			IPubDivision pubDivision = m_publication.DivisionsOS[0];
			int numberOfColumns = fIntroDivision ? 1 : pubDivision.NumColumns;

			IVwLayoutStream sharedStream = GetSharedSubstream();

			return new TeDivisionLayoutMgr(
				GetPrintLayoutConfigurer(fIntroDivision, hvoBook, sharedStream, m_BackTranslationWS),
				pubDivision, m_filterInstance, numberOfColumns, fIntroDivision);
		}
		...
		/// ------------------------------------------------------------------------------------
		protected virtual TePrintLayoutConfig GetPrintLayoutConfigurer(bool fIntroDivision,
			int hvoBook, IVwLayoutStream sharedStream, int ws)
		{
			return new TePrintLayoutConfig(m_cache, m_stylesheet, m_publication,
				m_viewType, m_filterInstance, m_printDateTime, fIntroDivision, hvoBook,
				sharedStream, ws);
		}
		#endregion
		...
		public override int DefaultFontSize
		{
			get
			{
				...
				List<PubPageInfo> pageSizes = TePublicationsInit.GetPubPageSizes(m_publication.Name,
					(new LgWritingSystem(m_cache, m_cache.DefaultUserWs)).ICULocale);
		...
		public RegistryBoolSetting UsfmResourceViewerVisible
		{
			get
			{
				return new RegistryBoolSetting(FwSubKey.TE, m_cache.ServerName,
					m_cache.DatabaseName, "USFMResourcesVisible" + Name, false);
			}
		}
		#endregion
		...
		public override void RefreshDisplay()
		{
			...
			int bookIndex = (selHelper == null) ? -1 :
				LocationTracker.GetBookIndex(selHelper, SelectionHelper.SelLimitType.Anchor);
			int sectionIndex = (selHelper == null || bookIndex < 0) ? -1 :
				LocationTracker.GetSectionIndexInBook(selHelper, SelectionHelper.SelLimitType.Anchor);
			...
			if (Visible && selHelper != null && bookIndex >= 0 && sectionIndex >= 0)
			{
		...
		protected override EditingHelper  GetInternalEditingHelper()
		{
			...
			TeEditingHelper result = new TeEditingHelper(this, m_cache, m_filterInstance, m_viewType);
...
//				RefreshDisplay();
		...
		/// ------------------------------------------------------------------------------------
		private void OnBookFilterChanged(object sender, EventArgs e)
		{
			SelectionHelper selHelper = SelectionHelper.Create(FocusedRootBox != null ?
				FocusedRootBox.Selection : null, this);

			RefreshDisplay();

			if (selHelper != null)
				selHelper.MakeBest(true);
		}
		...
		protected bool OnUpdateInformationBar(object args)
		{
				...
				TePrintLayoutConfig.GetDependentRootTag(m_cache), out selLevInfo))
			{
				...
				editingHelper.SetInformationBarForSelection((int)ScrBook.ScrBookTags.kflidTitle,
					m_cache.GetOwnerOfObject(selLevInfo.hvo));
			}
			else
			...
			SelectionHelper.SelLimitType selLimitType)
		{
			if (selHelper == TeEditingHelper.CurrentSelection && FocusedDivision != null)
				return ((DivisionLayoutMgr)FocusedDivision).Configurer.MainObjectId;
			...
			if (selHelper.Selection != null && selHelper.Selection.RootBox != null)
			{
				...
				if (frag == (int)FootnoteFrags.kfrRootInPageSeq && m_divisions.Count > 0)
				{
					...
					int[] footnoteHvos = m_cache.GetVectorProperty(hvo,
						configurer.DependentRootTag, true);
					...
					if (footnoteHvos.Length > 0)
						return m_cache.GetOwnerOfObject(footnoteHvos[0]);
				}
				return hvo;
			}
		...
		public virtual int GetBookIndex(SelectionHelper selHelper,
			SelectionHelper.SelLimitType selLimitType)
		{
			int hvoBook = GetBookHvo(selHelper, selLimitType);

			if (hvoBook < 0)
				return -1;

			return m_bookFilter.GetBookIndex(hvoBook);
		}
		...
		/// ------------------------------------------------------------------------------------
		public virtual int GetSectionHvo(SelectionHelper selHelper,
			SelectionHelper.SelLimitType selLimitType)
		{
			int sectionLevel = GetSectionLevel(selHelper, selLimitType);
			return sectionLevel >= 0 ? selHelper.GetLevelInfo(selLimitType)[sectionLevel].hvo : -1;
		}
		...
		public virtual int GetSectionIndexInBook(SelectionHelper selHelper,
			SelectionHelper.SelLimitType selLimitType)
		{
			int hvoSection = GetSectionHvo(selHelper, selLimitType);
			if (hvoSection < 0)
				return -1;

			int hvoOwner = m_cache.GetOwnerOfObject(hvoSection);
			int flidOwner = m_cache.GetOwningFlidOfObject(hvoSection);
			return m_cache.GetObjIndex(hvoOwner, flidOwner, hvoSection);
		}
