	...
	public class TeEditingHelper : FwEditingHelper
	{
				...
				HandleFootnoteAnchorIconSelected(CurrentSelection.Selection, (hvo, flid, ws, ich) =>
				{
					...
					if (flid == StTxtParaTags.kflidContents)
					{
						...
						IScrFootnote footnote = m_repoScrFootnote.GetObject(
							StringUtils.GetGuidFromRun(tssWithFootnoteOrc, tssWithFootnoteOrc.get_RunAt(ich)));
		...
		private Guid GetOrcHotLinkStrProp(IVwSelection vwsel)
		{
			...
			if (footnoteGuid == Guid.Empty)
			{
				...
				footnoteGuid = StringUtils.GetGuidFromProps(vttpTmp[0], desiredTypes, out foundType);
			}
			return footnoteGuid;
		}
			...
			if (targetRef.IsBookTitle)
			{
				foreach (IStTxtPara para in book.TitleOA.ParagraphsOS)
				{
					verseText.Add(new VerseTextSubstring(para.Contents, -1,
						para.IndexInOwner, ScrBookTags.kflidTitle));
					ichStart = 0;
				}
				return verseText;
			}
							...
							if (verse.StartRef <= targetRef && targetRef <= verse.EndRef)
							{
								...
								if (!IsNumber(verseTextInPara.Text)) // skip chapter number strings
								{
									...
									verseText.Add(new VerseTextSubstring(verseTextInPara, iSection,
										iPara, ScrSectionTags.kflidContent));
									break;
								}
							}
			...
			bool fMatchWholeWord, out int iSection, out int iPara, out int ichStart, out int ichEnd)
		{
			...
			foreach (VerseTextSubstring verseText in verseTextList)
			{
				...
				if (StringUtils.FindWordFormInString(tssTextToFind, verseText.Tss,
			...
			out ITsString tssSel, out int wsAlt)
		{
			...
			else if (propTag == SimpleRootSite.kTagUserPrompt)
			{
				...
				tssSel = StringUtils.MakeTss(string.Empty, wsAlt);
			...
			out int iFootnote)
		{
			...
			if (!IsSelectionInUserPrompt && IsSelectionInOneEditableProp(vwsel))
				...
				tssSelectedText = StringUtils.MakeTss(string.Empty, m_cache.DefaultVernWs);
			...
			tssSelectedText = StringUtils.GetCleanTsString(tssSelectedText, ScrStyleNames.ChapterAndVerse);
			...
			else
			{
				...
				if (footnote != null)
				{
					...
					int ichDel = StringUtils.DeleteOrcFromBuilder(tssBldr, footnote.Guid);
					if (ichDel >= 0 && ichSel > ichDel)
						ichSel--;

					StringUtils.InsertOrcIntoPara(footnote.Guid, FwObjDataTypes.kodtNameGuidHot,
		...
		private int FindBtFootnoteIndex(int ichBtSel, ITsString btTss)
		{
			...
			{
				if (StringUtils.GetHotObjectGuidFromProps(btTss.get_Properties(iRun)) != Guid.Empty)
				{
		...
		private ITsString GetCleanSelectedText(out int startOffset, out int endOffset)
		{
			...
			if (CurrentSelection.IsRange)
			{
				...
				return StringUtils.GetCleanTsString(tssQuote, ScrStyleNames.ChapterAndVerse);
			}
			return null;
		}
		...
		public string CleanSelectedWord
		{
			get
			{
				...
				ITsString tssWord = StringUtils.GetCleanTsString(CurrentSelection.SelectedWord,
