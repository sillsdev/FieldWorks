...
namespace SIL.FieldWorks.TE
{
	...
	public partial class ImportedBooks : Form, IBookVersionAgent
	{
		...
		List<IScrBook> m_newBooks = new List<IScrBook>();
		List<IScrBook> m_overwrittenBooks = new List<IScrBook>();
		#endregion
		...
		protected ImportedBooks(FdoCache cache, IScrDraft booksImported, IScrDraft backupVersion,
			IHelpTopicProvider helpTopicProvider, FwApp app)
		{
			InitializeComponent();
		...
		public ImportedBooks(FdoCache cache, FwStyleSheet styleSheet,
			...
			IHelpTopicProvider helpTopicProvider, FwApp app) :
			this(cache, booksImported, backupVersion, helpTopicProvider, app)
		{
		...
		private void btnCompare_Click(object sender, EventArgs e)
		{
				...
				"You should not see this Backup"))
			{
				...
				//m_cache.ActionHandlerAccessor.DiscardToMark(0);
		...
		public void MakeBackupIfNeeded(BookMerger bookMerger)
		{
			...
			{
				if (m_backupVersion.FindBook(bookMerger.BookCurr.CanonicalNum) == null)
			...
			foreach (ListViewItem item in lstImportedBooks.Items)
			{
				...
				using (ProgressDialogWithTask progressDlg = new ProgressDialogWithTask(m_owningForm))
				{
					...
					UndoableUnitOfWorkHelper.Do("Automerge", "Automerge",
						m_cache.ServiceLocator.GetInstance<IActionHandler>(), () =>
					{
						progressDlg.RunTask(true, bookMerger.DetectDifferences);
					});
#endif
				}
		...
		protected IScrBook OverwriteBook(IScrBook bookCurr, IScrBook bookImported)
		{
			if (bookCurr != null)
			{
				...
				if (m_cache.ActionHandlerAccessor != null)
				{
					...
					// BEFORE the delete.
					ReplaceInFilterFixer fixer1 = new ReplaceInFilterFixer(bookCurr, null, m_app);
					m_cache.ActionHandlerAccessor.AddAction(fixer1);
				}

				bool fBackedUp = false;
				// Move existing book to backup.
				for (int destIndex = 0; destIndex < m_backupVersion.BooksOS.Count; destIndex++)
				{
					if (m_backupVersion.BooksOS[destIndex].CanonicalNum == bookCurr.CanonicalNum)
					{
					...
					if (m_backupVersion.BooksOS[destIndex].CanonicalNum > bookCurr.CanonicalNum)
					{
						m_backupVersion.BooksOS.Insert(destIndex, bookCurr);
						fBackedUp = true;
						break;
					}
				}
				...
				if (!fBackedUp)
				{
					// Didn't find this book in the collection and didn't find any place in the
					// existing list to insert this, so add it to the end.
					m_backupVersion.BooksOS.Add(bookCurr);
				}
			}
			IScrBook newCopy = m_scr.CopyBookToCurrent(bookImported);
			ReplaceInFilterFixer fixer = new ReplaceInFilterFixer(null, newCopy, m_app);
			...
			List<IScrSection> sectionsToRemove)
		{
			using (ProgressDialogWithTask progress = new ProgressDialogWithTask(this))
			{
				...
				progress.RunTask(true, bookMerger.DoPartialOverwrite, sectionsToRemove);
			}
		}
