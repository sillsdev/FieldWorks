...
namespace SIL.FieldWorks.TE
{
	...
	{
		#region Private class FootnoteMarkerIndexUndoAction
		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Undo/Redo action that clears the footnote marker index cache
		/// </summary>
		/// ------------------------------------------------------------------------------------
		private class FootnoteMarkerIndexUndoAction : UndoActionBase
		{
			private FdoCache m_cache;

			/// --------------------------------------------------------------------------------
			/// <summary>
			/// Initializes a new instance of the <see cref="T:FootnoteMarkerIndexUndoAction"/> class.
			/// </summary>
			/// <param name="cache">The FDO cache.</param>
			/// --------------------------------------------------------------------------------
			public FootnoteMarkerIndexUndoAction(FdoCache cache)
			{
				m_cache = cache;
			}
			...
			/// --------------------------------------------------------------------------------
			/// <summary>
			/// Clears the footnote marker index cache.
			/// </summary>
			/// --------------------------------------------------------------------------------
			public void Do()
			{
		...
		private System.Windows.Forms.TabPage tpgChapVrsNumbers;
		private System.Windows.Forms.TabPage tpgFootnotes;
		private System.Windows.Forms.RadioButton m_btnLatinNumbers;
		private System.Windows.Forms.RadioButton m_btnScriptNumbers;
		private FwOverrideComboBox m_cboScriptLanguages;
		/// <summary>
		/// Required designer variable.
		/// </summary>
		private System.ComponentModel.Container components = null;
		private System.Windows.Forms.CheckBox m_chkUseLatinNumsOnExport;
		...
		private System.Windows.Forms.Label lblRefSepr;
		private System.Windows.Forms.Label lblChapterVerseSep;
		private System.Windows.Forms.Label lblVerseBridge;
		private System.Windows.Forms.Label lblVerseSep;
		private System.Windows.Forms.TextBox m_txtRefSep;
		private System.Windows.Forms.TextBox m_txtChapterVerseSep;
		private System.Windows.Forms.TextBox m_txtVerseSep;
		private System.Windows.Forms.TextBox m_txtVerseBridge;
		private SIL.FieldWorks.TE.FootnotePropertiesSelector fpsFootnoteOptions;
...
//			LoadOneLanguage("kstidEthiopic", '\u1369');
		...
		private void FillVersificationSchemes()
		{
			...
			{
				Paratext.ScrVers vers = (Paratext.ScrVers)iVers;
		...
		private void m_btnOK_Click(object sender, System.EventArgs e)
		{
			using (new WaitCursor(this))
			{
				string undo;
				string redo;
				TeResourceHelper.MakeUndoRedoLabels("kstidUndoRedoScriptureProperties", out undo,
					out redo);
				IRootSite activeView = ((FwMainWnd)Owner).ActiveView;
				using (UndoTaskHelper undoTaskHelper = new UndoTaskHelper(m_cache.MainCacheAccessor,
						  activeView != null ? activeView.CastAsIVwRootSite() : null, undo, redo, true))
				{
					try
					{
		...
		{
			Paratext.ScrVers selectedVers =
				(Paratext.ScrVers)(m_cboVersification.SelectedIndex + 1);
		...
		private void ProcessFootnoteSettings()
		{
			...
			List<PropChangedInfo> propChangeds = new List<PropChangedInfo>();
			if (fSettingsChanged)
			{
				// Call PropChanged so that the footnotes get updated
				IScripture scripture = m_cache.LangProject.TranslatedScriptureOA;
				int i = 0;
				foreach (IScrBook book in scripture.ScriptureBooksOS)
				{
					int nFootnotes = book.FootnotesOS.Count;
					if (nFootnotes > 0)
					{
						propChangeds.Add(new PropChangedInfo(book.Hvo,
							(int)ScrBook.ScrBookTags.kflidFootnotes, 0, nFootnotes, nFootnotes));
						propChangeds.Add(new PropChangedInfo(scripture.Hvo,
							(int)Scripture.ScriptureTags.kflidScriptureBooks, i, 1, 1));
					}
					i++;
				}

				m_cache.ActionHandlerAccessor.AddAction(new PropChangedUndoAction(m_cache, false,
					PropChangeType.kpctNotifyAll, propChangeds));
				...
				// Need to make sure we clear the cache of the marker indexes
				FootnoteMarkerIndexUndoAction markerUndoAction =
					new FootnoteMarkerIndexUndoAction(m_cache);
				m_cache.ActionHandlerAccessor.AddAction(markerUndoAction);
				markerUndoAction.Do();

				SelectionHelper selHelper = SelectionHelper.Create(m_rootSite.CastAsIVwRootSite());
				IRootSite activeRootSite = ((FwMainWnd)Owner).ActiveView;
				bool fInFootnotePane = (activeRootSite != m_rootSite);
				SelectionHelper activeSelHelper = selHelper;
				if (fInFootnotePane)
					activeSelHelper = SelectionHelper.Create(activeRootSite.CastAsIVwRootSite());

				if (selHelper != null)
				{
					m_cache.ActionHandlerAccessor.AddAction(
						new UndoSelectionAction(m_rootSite.CastAsIVwRootSite(), true,
						selHelper.Selection));
				}

				PropChangedUndoAction undoAction = new PropChangedUndoAction(m_cache, true,
					PropChangeType.kpctNotifyAll, propChangeds);
				m_cache.ActionHandlerAccessor.AddAction(undoAction);
				if (selHelper != null)
				{
					m_cache.ActionHandlerAccessor.AddAction(
						new UndoSelectionAction(m_rootSite.CastAsIVwRootSite(), false,
						selHelper.Selection));
				}

				// Need to fire PropChanged, so we just use the undoAction for that...
				undoAction.FirePropChangeds();
		...
		/// time.</remarks>
		/// ------------------------------------------------------------------------------------
		protected virtual bool ConvertChapterVerseNumbers(IStTxtPara para, char zeroDigit,
		...
		/// ------------------------------------------------------------------------------------
		private void UpdateVerseBridgesInParagraph(StTxtPara para, string oldBridge, string newBridge)
		{
