...
namespace SIL.FieldWorks.TE
{
	...
	public class SavedVersionsDialog : Form, IBookVersionAgent, IFWDisposable
	{
		...
		private float m_zoomDraft;
		private float m_zoomFootnote;
		/// <summary></summary>
		...
		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Initializes a new instance of the <see cref="SavedVersionsDialog"/> class.
		/// </summary>
			...
			float zoomFactorDraft, float zoomFactorFootnote)
		{
			...
			foreach (IScrDraft savedVersion in m_scr.ArchivedDraftsOC)
			{
				...
				{
					TreeNode bookNode = new TreeNode((book as ScrBook).BestUIName + " " +
		...
		private void btnSave_Click(object sender, EventArgs e)
		{
			...
			using (SaveVersionDialog dlg = new SaveVersionDialog(m_cache))
			{
				dlg.ShowDialog();
		...
		{
			ShowHelp.ShowHelpTopic(FwApp.App, "khtpSavedVersionsDialog");
		}
		...
		protected void m_treeArchives_AfterSelect(object sender, System.Windows.Forms.TreeViewEventArgs e)
		{
			...
			if (node != null && node.Tag is ScrBook)
			{
				...
				int bookId = ((ScrBook)node.Tag).CanonicalNum;
				IScrBook sameBookInDB = ScrBook.FindBookByID(m_scr, bookId);
		...
		/// </summary>
		/// <param name="sender">The source of the event.</param>
		/// <param name="e">The <see cref="System.Windows.Forms.TreeNodeMouseClickEventArgs"/>
	...
	{
		private int m_hvoOld;
		private int m_hvoNew;
		private FdoCache m_cache;
		...
		/// <param name="cache"></param>
		public ReplaceInFilterFixer(int hvoOld, int hvoNew, FdoCache cache)
		{
			m_hvoOld = hvoOld;
			m_hvoNew = hvoNew;
			m_cache = cache;
		...
		/// <param name="hvoOld"></param>
		/// <param name="hvoNew"></param>
		private void Swap(int hvoOld, int hvoNew)
		{
			foreach (Form form in FwApp.App.MainWindows)
				Swap(hvoOld, hvoNew, form);
			FwApp.App.Synchronize(new SyncInfo(SyncMsg.ksyncReloadScriptureControl, 0, 0), m_cache);
		}

		private void Swap(int hvoOld, int hvoNew, Form activeForm)
		{
			FilteredScrBooks filter;
			try
			{
				filter = ReflectionHelper.GetProperty(activeForm, "BookFilter") as FilteredScrBooks;
			}
			catch
			{
				return;
			}
			if (filter == null || filter.Cache != m_cache)
				return;
			if (hvoOld != 0)
			{
				int index = filter.BookHvos.IndexOf(hvoOld);
				filter.Remove(index);
			}
			if (hvoNew != 0)
			{
				filter.Add(new int[] { hvoNew });
			}
		}

		private int GetCanonicalNumber(int hvo, FdoCache cache)
		{
			return cache.GetIntProperty(hvo, (int) ScrBook.ScrBookTags.kflidCanonicalNum);
		}
