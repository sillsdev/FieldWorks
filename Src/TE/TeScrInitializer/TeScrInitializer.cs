...
namespace SIL.FieldWorks.TE
{
	...
	public class TeScrInitializer
	{
		...
		/// </summary>
		/// -------------------------------------------------------------------------------------
		protected TeScrInitializer(FdoCache cache, IHelpTopicProvider helpTopicProvider)
		{
			m_cache = cache;
		...
		/// </returns>
		/// ------------------------------------------------------------------------------------
		public static bool Initialize(FdoCache cache, IHelpTopicProvider helpTopicProvider,
			IProgress progressDlg)
		{
			TeScrInitializer scrInitializer = new TeScrInitializer(cache, helpTopicProvider);
		...
		/// ------------------------------------------------------------------------------------
		public static void EnsureProjectValid(FdoCache cache,
			IHelpTopicProvider helpTopicProvider, IProgress existingProgressDlg)
		{
			TeScrInitializer scrInitializer = new TeScrInitializer(cache, helpTopicProvider);
		...
		/// --------------------------------------------------------------------------------
		public static void EnsureProjectComponentsValid(FwApp app,
			IHelpTopicProvider helpTopicProvider, IProgress existingProgressDlg)
		{
			EnsureProjectValid(app.Cache, helpTopicProvider, existingProgressDlg);

			// TE-8621: Don't try to upgrade the database unless we're the project server and no one is connected
			if (!app.Cache.ProjectId.IsLocal || app.Cache.NumberOfRemoteClients > 0)
				return;

			ILangProject lp = app.Cache.LangProject;

			TeScrBookRefsInit.EnsureFactoryScrBookRefs(app.Cache, existingProgressDlg);
			TePublicationsInit.EnsureFactoryPublications(lp, existingProgressDlg);
			TeStylesXmlAccessor.EnsureCurrentStylesheet(app, existingProgressDlg);
		...
		protected bool Initialize(IProgress progressDlg)
		{
			...
			using (var dlg = new ProgressDialogWithTask(Form.ActiveForm))
			{
				try
				{
					dlg.RunTask(progressDlg, true, InitializeScriptureProject);
				}
		...
		/// <returns></returns>
		/// ------------------------------------------------------------------------------------
		private object InitializeScriptureProject(IProgress progressDialog, object[] parameters)
		{
			ILangProject lp = m_cache.LanguageProject;
			if (m_scr == null)
			{
				m_scr = lp.TranslatedScriptureOA = m_cache.ServiceLocator.GetInstance<IScriptureFactory>().Create();
				...
				TeScrBookRefsInit.SetNamesAndAbbreviations(progressDialog, m_cache);
			}
		...
		protected void EnsureScriptureTextsValid()
		{
			foreach (IScrBook book in m_scr.ScriptureBooksOS)
			{
				...
				foreach (IScrSection section in book.SectionsOS)
				{
					...
					{
						IStTxtPara para = m_cache.ServiceLocator.GetInstance<IScrTxtParaFactory>().CreateWithStyle(
							section.ContentOA, TeEditingHelper.GetDefaultStyleForContext(sectionContext, false));
					}
				}
			}
		}
			...
			List<FootnoteOrcLocation> footnotes, List<string> issuesToReport)
		{
			...
			for (int i = 0; i < orcLocations.Count; i++)
			{
				...
				else
				{
					...
					byte[] footnoteObjData = StringUtils.GetObjData(footnote.Guid, (byte)FwObjDataTypes.kodtNameGuidHot);
