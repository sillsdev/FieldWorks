	...
	public class TeScrNoteCategoriesInit
	{
			...
			IProgress existingProgressDlg)
		{
			...
			if (scr.NoteCategoriesOA == null || newVersion != scr.NoteCategoriesOA.ListVersion)
			{
				...
				{
					dlg.RunTask(existingProgressDlg, true, CreateScrNoteCategories, scrNoteCategories, scr);
				}
			}
		}
		...
		/// ------------------------------------------------------------------------------------
		private static object CreateScrNoteCategories(IProgress dlg, object[] parameters)
		{
