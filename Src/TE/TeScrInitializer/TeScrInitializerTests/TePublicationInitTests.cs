...
namespace SIL.FieldWorks.TE
{
	...
	internal class TestTePublicationsInit : TePublicationsInit
	{
		...
		/// </summary>
		/// ------------------------------------------------------------------------------------
		internal TestTePublicationsInit(IScripture scr) : base(scr)
		{
		}
		...
		{
			CreateFactoryPublicationInfo(new DummyProgressDlg(), doc.SelectSingleNode("PublicationDefaultsForNewProject"));
		}
		...
		{
			CreateHfSets(new DummyProgressDlg(), hfSetNodes);
		}
	...
	public class TePublicationInitTests : ScrInMemoryFdoTestBase
	{
		...
		public void GetWritingSystemTest()
		{
			...
			// get existing attributes
			Assert.AreEqual(Cache.ServiceLocator.WritingSystems.DefaultAnalysisWritingSystem.Handle, //TestLangProj DefaultAnalWs is english
				m_pubInitializer.GetWritingSystem(attribs, "test-english", wsDefault));
			Assert.AreEqual(Cache.ServiceLocator.WritingSystems.DefaultVernacularWritingSystem.Handle, //TestLangProj DefaultVernWs is french
		...
		public void GetWritingSystemTest_InvalidWS()
		{
			XmlDocument doc = new XmlDocument();
			doc.LoadXml("<mytag test-english=\"en\" test-kalaba=\"fr\" " +
				"test-empty=\"\" " +
		...
		public void CreatePublicationInfo_Inches_ExplicitValues()
		{
			...
			doc.LoadXml("<?xml version=\"1.0\" encoding=\"UTF-8\"?>" +
				"<PublicationDefaultsForNewProject date=\"2007-08-20\" " +
					"DTDver=\"61A20AFA-56A9-4717-9014-0CBF99C9F368\" " +
			...
			IFdoOwningCollection<IPublication> oc = m_scr.PublicationsOC;
			IPublication pub = oc.ToArray()[0];
			...
			ICmResource pubResource = (from res in m_scr.ResourcesOC
					where res.Name.Equals("TePublications")
		...
		public void CreatePublicationsHeaderFooterLiteralWS()
		{
			...
			//Run CreatePublications()
			m_scr.HeaderFooterSetsOC.Clear();
			m_pubInitializer.CallCreatePublicationInfo(doc);

			//get HeaderFooterSets
			Assert.AreEqual(1, m_scr.HeaderFooterSetsOC.Count,
				"Wrong number of HeaderFooterSets was created");
			IFdoOwningCollection<IPubHFSet> oc = m_scr.HeaderFooterSetsOC;
			IPubHFSet hfSet = oc.ToArray()[0];

			//get DefaultHeader
			IPubHeader hf = hfSet.DefaultHeaderOA;

			//verify Outside
			ITsString tss = hf.OutsideAlignedText;
			...
			//verify Outside
			tss = hf.OutsideAlignedText;
			Assert.AreEqual(1, tss.RunCount);
			Assert.AreEqual(0, tss.Length);
			ttpRun = tss.get_Properties(0);
			wsExpected = Cache.WritingSystemFactory.UserWs;
			ttpExpected = StyleUtils.CharStyleTextProps(null, wsExpected);
			if (!TsTextPropsHelper.PropsAreEqual(ttpExpected, ttpRun, out sWhy))
				Assert.Fail(sWhy);
		...
		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Overwrites the existing hf sets.
		/// </summary>
		/// ------------------------------------------------------------------------------------
		[Test]
