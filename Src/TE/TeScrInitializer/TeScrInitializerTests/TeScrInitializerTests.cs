...
namespace SIL.FieldWorks.TE
{
	...
	internal class TestTeScrInitializer : TeScrInitializer
	{
		...
		/// ------------------------------------------------------------------------------------
		public TestTeScrInitializer(FdoCache cache) : base(cache, null)
		{
		}
	...
	public class TeScrInitializerTests : ScrInMemoryFdoTestBase
	{
		...
		public void CreateScrBookRefs()
		{
			using (ProgressDialogWithTask progressDlg = new ProgressDialogWithTask(Form.ActiveForm))
				ReflectionHelper.CallMethod(typeof(TeScrBookRefsInit), "SetNamesAndAbbreviations", progressDlg, Cache);
		...
		public void FixOrcsWithoutProps_OrphanedFootnoteAndValidPicture()
		{
			...
			ICmPicture pict = Cache.ServiceLocator.GetInstance<ICmPictureFactory>().Create(filename,
				factory.MakeString("Test picture", Cache.DefaultVernWs), StringUtils.LocalPictures);
