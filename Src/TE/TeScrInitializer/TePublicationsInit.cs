...
namespace SIL.FieldWorks.TE
{
	/// ----------------------------------------------------------------------------------------
	/// <summary>
	/// Class providing static method that TE calls to perform one-time initialization of
	/// publications and publication-related parameters in a new Scripture project
	/// </summary>
	/// ----------------------------------------------------------------------------------------
	public class TePublicationsInit : SettingsXmlAccessorBase
	{
		...
		/// <summary>bool indicates if we are under test, not running TE.exe; set only by derived test class</summary>
		protected bool m_fUnderTest = false;
		...
		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Initializes a new instance of the <see cref="T:TePublicationsInit"/> class. This
		...
		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Initializes a new instance of the <see cref="T:TePublicationsInit"/> class.
		/// </summary>
		...
		{
			dlg.RunTask(progressDlg, true, new BackgroundTaskInvoker(CreateFactoryPublicationInfo),
				doc);
		}
		#endregion
		...
		/// </param>
		/// ------------------------------------------------------------------------------------
		protected object CreateFactoryPublicationInfo(IAdvInd4 progressDlg,
			...
			XmlNodeList hfSetNodes)
		{
			...
			foreach (XmlNode divisionNode in divisionNodes)
			{
				...
				PubPageLayout pageLayout = new PubPageLayout();
			...
			if (hfElementNodes != null)
			{
				foreach(XmlNode elementNode in hfElementNodes)
				{
						...
						if (sType != string.Empty)
						{
							...
							if (guid != Guid.Empty)
							{
								...
								propsBldr.SetIntPropValues((int)FwTextPropType.ktptWs,
									0, m_scr.Cache.LangProject.DefaultUserWritingSystem);
					...
					else
					{
						...
						int ws = GetWritingSystem(attributes, "ws", m_scr.Cache.DefaultUserWs);
		...
		private ITsString EmptyTsString
		{
			get
			{
				...
				{
					ITsStrFactory strFactory = TsStrFactoryClass.Create();
					m_tssEmpty = strFactory.MakeString("", m_scr.Cache.DefaultUserWs);
				}
				return m_tssEmpty;
			}
		}
		...
		protected List<PubPageInfo> GetPubPageSizes(XmlNode pubsRootNode,
			string publicationName, string icuLocale)
		{
			...
			{
				XmlNode nameNode = pageSizeInfo.SelectSingleNode("Name[@iculocale='" + icuLocale + "']");
				if (nameNode == null)
		...
		{
			get { return "9E835785-F508-4b58-844B-02A3D6EC3579"; }
		}
