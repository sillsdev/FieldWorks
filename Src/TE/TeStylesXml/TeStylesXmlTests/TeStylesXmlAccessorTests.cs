...
namespace SIL.FieldWorks.TE
{
	...
	class DummyTeStylesXmlAccessor : TeStylesXmlAccessor
	{
		...
		/// <param name="teStyles"></param>
		/// ------------------------------------------------------------------------------------
		public void InitLoading(IAdvInd4 progressDlg, IScripture scr, XmlNode teStyles)
		{
			m_scr = scr;
			base.CreateStyles(progressDlg, scr.StylesOC, teStyles);
		}
		...
		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Exposes the m_replacedStyles member
		/// </summary>
		/// ------------------------------------------------------------------------------------
		public List<StyleReplacement> ReplacedStyles
		{
			get { return m_replacedStyles; }
		}

		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Exposes the m_deletedStyles member
		/// </summary>
		/// ------------------------------------------------------------------------------------
		public List<string> DeletedStyles
		{
			get { return m_deletedStyles; }
		}
		...
		{
			get { return s_htReservedStyles.Count; }
		}
	...
	/// ----------------------------------------------------------------------------------------
	[TestFixture]
	public class TeStylesXmlAccessorTests_WithoutCache
	{
	...
	{
		#region data members
		private StStyle m_styleOrig;
		private StStyle m_styleReplace;
		#endregion
		...
		public void TestGetType_InvalidContextForType()
		{
			...
			XmlDocument doc = new XmlDocument();
			doc.LoadXml("<?xml version=\"1.0\" encoding=\"UTF-8\"?>" +
				"<tag userlevel=\"0\" id=\"Yaddah\" context=\"general\" type=\"paragraph\">" + "\n" +
				"</tag>");
	...
	/// ----------------------------------------------------------------------------------------
	[TestFixture]
	public class TeStylesXmlAccessorTests_WithCache : InDatabaseFdoTestBase
	{
		...
		public void CreateScrStylesTest_NewDB()
		{
			...
			doc.LoadXml("<?xml version=\"1.0\" encoding=\"UTF-8\"?>" + "\n" +
				...
				"<tag userlevel=\"0\" id=\"Chapter_Number\" structure=\"heading\" use=\"verse\" context=\"backTranslation\" type=\"paragraph\">" + "\n" +
				"<sfm>\\c</sfm>" + "\n" +
				...
				"</tag>" + "\n" +
				// Hebrew Title
				"<tag userlevel=\"1\" id=\"Hebrew_Title\" structure=\"heading\" context=\"text\" type=\"paragraph\">" + "\n" +
				"<sfm>\\d</sfm>" + "\n" +
				...
				"<paragraph basedOn=\"Paragraph\" next=\"Paragraph\" lineSpacing=\"12 pt\" alignment=\"center\" background=\"white\" indentLeft=\"0\" indentRight=\"0\" firstLine=\"8 pt\" spaceBefore=\"0\" spaceAfter=\"0\" border=\"top\"/>" + "\n" +
				"</tag>" + "\n" +
				// Paragraph (must have non-real context, structure, use, etc. to test the
				// creation of internal styles)
				"<tag userlevel=\"0\" id=\"Paragraph\" structure=\"heading\" use=\"list\" context=\"general\" type=\"character\">" + "\n" +
				"<usage iculocale=\"en\">Generic prose paragraph</usage>" + "\n" +
				"<usage iculocale=\"es\">Un párrafo general</usage>" + "\n" +
				"<sfm>\\p</sfm>" + "\n" +
				"<font type=\"default\"/>" + "\n" +
				"<paragraph next=\"Gumby\" firstLine=\"8 pt\" basedOn=\"Mamba\"/>" + "\n" +
				"</tag>" + "\n" +
		...
		public void TestEnsureCompatibleFactoryStyle_Easy()
		{
			...
			Assert.AreEqual(0, acc.ReplacedStyles.Count);
			Assert.AreEqual(0, acc.DeletedStyles.Count);
		}
		...
		public void TestEnsureCompatibleFactoryStyle_FactoryStyleToInternal()
		{
			...
			FdoOwningCollection<IStStyle> styles = m_scr.StylesOC;
		...
		public void TestEnsureCompatibleFactoryStyle_RedefineUserStyle()
		{
			...
			DummyTeStylesXmlAccessor acc = new DummyTeStylesXmlAccessor(m_scr, null);
			IStStyle newFactoryStyle = acc.EnsureCompatibleFactoryStyle(userDefStyle,
			...
			Assert.AreEqual(1, acc.ReplacedStyles.Count);
			StyleReplacement repl = (StyleReplacement)acc.ReplacedStyles[0];
			Assert.AreEqual("Paragraph", repl.oldStyle);
			Assert.AreEqual("Paragraph_User", repl.newStyle);
			Assert.AreEqual(0, acc.DeletedStyles.Count);
		}
		#endregion
