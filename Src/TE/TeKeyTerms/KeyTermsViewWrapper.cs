...
namespace SIL.FieldWorks.TE
{
	...
	public class KeyTermsViewWrapper : ChecksViewWrapper
	{
		...
		protected void AssignVernacularEquivalentInternal()
		{
			...
			string keyTermText = StringUtils.GetCleanTextFromTsString(tss, ScrStyleNames.ChapterAndVerse, true,
		...
		public void UpdateKeyTermEquivalents()
		{
			using (ProgressDialogWithTask progressDlg = new ProgressDialogWithTask(ParentForm))
			{
				...
				progressDlg.RunTask(true, UpdateKeyTermEquivalents);
			}
		}
		...
		/// <returns></returns>
		/// ------------------------------------------------------------------------------------
		private object UpdateKeyTermEquivalents(IProgress progressDlg, params object[] parameters)
		{
			string undo, redo;
			TeResourceHelper.MakeUndoRedoLabels("kstidUndoRedoAssignVern", out undo, out redo);
		...
		private void ktTree_AfterSelect(object sender, TreeViewEventArgs e)
		{
			...
			if (obj != null && obj is IChkTerm)
			{
				using (new WaitCursor(TopLevelControl))
					RenderingsControl.LoadRenderingsForKeyTerm((IChkTerm)obj,
		...
		private bool IsValidTSS(ITsString tss)
		{
			if (tss == null)
				return false;
			String keyterm = StringUtils.GetCleanTextFromTsString(tss,
