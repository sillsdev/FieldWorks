...
namespace SIL.FieldWorks.TE
{
	...
	internal class DiffViewWrapper: SplitGrid
	{
		...
		internal void FixupColumnHeights()
		{
			...
			GetRow(kButtonRow).Height = GetRow(kButtonRow).MinimumHeight;
		}
		...
		internal void FixupHostedControlVisiblity()
		{
			...
			{
				if (!GetRow(row).Visible)
					continue;

				GetControl(row, kRevisionColumn).Visible = true;
				GetControl(row, kCurrentColumn).Visible = true;
			}
		}
#endif
		/// ------------------------------------------------------------------------------------
