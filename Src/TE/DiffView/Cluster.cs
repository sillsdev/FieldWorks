...
namespace SIL.FieldWorks.TE
{
	...
	public class Cluster : IComparable
	{
		...
		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Gets an array of the hvos of the Revision items for this cluster.
		/// </summary>
		/// ------------------------------------------------------------------------------------
		public int[] HvosItemsRev
		{
			get { return GetHvoArrayOfItems(itemsRev); }
		}

		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Gets an array of the hvos of the Current items for this cluster.
		/// </summary>
		/// ------------------------------------------------------------------------------------
		public int[] HvosItemsCurr
		{
			get { return GetHvoArrayOfItems(itemsCurr); }
		}

		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Gets an array of the hvos of the Source items for this cluster, i.e. the list of
		/// of added items.
		/// </summary>
		/// ------------------------------------------------------------------------------------
		public int[] HvosItemsSource
		{
			get { return GetHvoArrayOfItems(SourceItems); }
		}
		...
		public bool	SpansParaBreak
		{
			get
			{
				...
				{
					int hvoFirst = this.itemsCurr[0].myHvo;
					for (int iCurr = 1; iCurr < this.itemsCurr.Count; iCurr++)
					{
						if (this.itemsCurr[iCurr].myHvo != hvoFirst)
							return true;
					}
				}
				...
				{
					int hvoFirst = this.itemsRev[0].myHvo;
					for (int iRev = 1; iRev < this.itemsRev.Count; iRev++)
					{
						if (this.itemsRev[iRev].myHvo != hvoFirst)
							return true;
					}
				}
		...
		private int[] GetHvoArrayOfItems(List<OverlapInfo> itemList)
		{
			int[] hvoArray = new int[itemList.Count];
			for (int i = 0; i < itemList.Count; i++)
				hvoArray[i] = itemList[i].myHvo;
			return hvoArray;
		}
	...
	#region OverlapInfo class
		...
		public class OverlapInfo
		{
			...
			/// <summary>
			/// myBook signifies this OverlapInfo as being part of either the Revision book
			/// or the Current book (this is important to the algorithm that determines overlap clusters)
			/// </summary>
			public int myBook;
			/// <summary>constant used to signify that this is part of the Revision book</summary>
			public const int kRevision = 0;
			/// <summary>constant used to signify that this is part of the Current book</summary>
			public const int kCurrent = 1;
			...
			/// <summary>hvo of the section or paragraph that this OverlapInfo represents </summary>
			public int myHvo;

			/// <summary>hvo of the StText that owns the paragraph (if this OverlapInfo is for
			/// a ScrVerse)</summary>
			public int myParaOwner;
			...
			/// </summary>
			/// ------------------------------------------------------------------------------------
			public OverlapInfo(IScrSection section, int bookType)
			{
			...
			/// </summary>
			/// ------------------------------------------------------------------------------------
			public OverlapInfo(IStTxtPara para, BCVRef refMin, BCVRef refMax, int bookType)
			{
				// Set the max and the min to that supplied by our caller
			...
			public OverlapInfo Clone()
			{
				...
				toReturn.myBook = this.myBook;
				toReturn.indexInOwner = this.indexInOwner;
				toReturn.myHvo = this.myHvo;
	...
	public class ClusterListHelper
	{
		...
		/// </returns>
		/// ------------------------------------------------------------------------------------
		public static List<Cluster> DetermineSectionOverlapClusters(IScrBook bookCurr, IScrBook bookRev,
			FdoCache cache)
		{
			...
			foreach (IScrSection section in bookRev.SectionsOS)
			{
				...
				sectionProxyListRev.Add(new OverlapInfo(section, OverlapInfo.kRevision));
			...
			{
				sectionProxyListCurr.Add(new OverlapInfo(section, OverlapInfo.kCurrent));
				Debug.Assert(BCVRef.GetChapterFromBcv(section.VerseRefMin) > 0); // catch test that forgot to set section refs
			}
			...
			List<ScrVerse> scrVersesRev, FdoCache cache)
		{
			...
			{
				scrVerseProxyListRev.Add(new OverlapInfo(scrVersesRev[iVerseRev], OverlapInfo.kRevision, iVerseRev));
			...
			{
				scrVerseProxyListCurr.Add(new OverlapInfo(scrVersesCurr[iVerseCurr], OverlapInfo.kCurrent, iVerseCurr));
				// When there is no content at the start of Scripture, it won't have a reference greater than 0.
			...
			for (int iClstrItem = 0; iClstrItem < cluster.itemsCurr.Count; iClstrItem++)
			{
				...
				if (verseCurr.StartRef == verseRev.StartRef && verseCurr.EndRef == verseRev.EndRef)
				{
					correlationFactor = ParagraphCorrelation.DetermineStringCorrelation(
						...
						(verseRev.Text != null) ? verseRev.Text.Text : null, m_cache.UnicodeCharProps);
				...
				iClstrItemCurr--, iClstrItemRev--)
			{
				...
				if (verseCurr.StartRef == verseRev.StartRef && verseCurr.EndRef == verseRev.EndRef)
				{
					correlationFactor = ParagraphCorrelation.DetermineStringCorrelation(verseCurr.Text.Text,
						verseRev.Text.Text, m_cache.UnicodeCharProps);
		...
		protected void CreateBasicOverlapClusters()
		{
			...
			while (proxyListRevCopy.Count > 0 || proxyListCurrCopy.Count > 0)
			{
				...
				while (queue.Count > 0)
				{
					...
					// has been used
					if (currentProxy.myBook == OverlapInfo.kRevision)
					{
				...
				foreach (OverlapInfo oi in proxyListForCluster)
				{
					...
					if (oi.myBook == OverlapInfo.kRevision)
