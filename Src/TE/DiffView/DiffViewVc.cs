...
namespace SIL.FieldWorks.TE
{
	...
	internal class DiffViewVc: DraftViewVc
	{
		...
		public override void Display(IVwEnv vwenv, int hvo, int frag)
		{
			...
			switch(frag)
			{
				case (int)StTextFrags.kfrPara:
				{
					...
					bool highlightWholePara = false;
					bool fDisplayMissingParaPlaceholderAfter = false;
					if (diff != null)
					{
						highlightWholePara = diff.IncludesWholePara(hvo, m_fRev, m_cache);

						// If the given paragraph has differences to be highlighted,
						//  add appropriate properties
						if ((diff.GetHvo(m_fRev) == hvo || highlightWholePara) && m_fNeedHighlight)
						{
		...
		/// ------------------------------------------------------------------------------------
		private bool NeedsBoundaryHighlight(Difference rootDiff, int hvoPara)
		{
			...
			if (rootDiff != null &&
				...
				ParaBreakNeedsHighlight(rootDiff, hvoPara))
			{
		...
		/// </summary>
		/// <param name="hvo">The hvo that Display is attempting to draw.</param>
		/// <returns>the difference that the view constructor needs at this time</returns>
		/// ------------------------------------------------------------------------------------
		private Difference FindDiff(int hvo)
		{
