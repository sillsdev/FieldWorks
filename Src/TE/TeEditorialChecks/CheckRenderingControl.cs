...
namespace SIL.FieldWorks.TE.TeEditorialChecks
{
		...
		IVwNotifyChange, IMessageFilter
	{
		...
		private readonly float m_szOfFontAt100Pcnt = FontInfo.kDefaultFontSize / 1000F;
		...
		/// </summary>
		/// ------------------------------------------------------------------------------------
		public CheckRenderingControl() : this(null, null)
		{
		}
		...
		protected virtual void OnLoadSettings(RegistryKey key)
		{
			if (m_dataGridView != null && key != null)
			{
				...
				float zoomFactor = RegistryHelper.ReadFloatSetting(key, "ZoomFactor" + Name, 1.5f);
