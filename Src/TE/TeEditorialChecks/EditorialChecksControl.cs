...
namespace SIL.FieldWorks.TE.TeEditorialChecks
{
	...
	public partial class EditorialChecksControl : CheckControl, IxCoreColleague
	{
		...
		private Dictionary<Guid, string> m_chkHelpIds;
		...
		/// </summary>
		/// ------------------------------------------------------------------------------------
		public EditorialChecksControl(): this(null, null, null, null, null)
		{
		}
		...
		{
			groupName = StringUtils.GetUiString(Scripture.Resources, groupName);
		...
		private void OnHelpClicked(object sender, EventArgs e)
		{
			...
			ShowHelp.ShowHelpTopic(FwApp.App, "khtpScrChecks");
		}
		...
		private void OnAfterTreeNodeChecked(object sender, TreeViewEventArgs e)
		{
			...
			if (((Control)pnlOuter).Text != bldr.ToString())
			{
				((Control)pnlOuter).Text = bldr.ToString();
				if (m_checkErrorsList != null)
		...
		private bool IsValidCharIncomplete
		{
			get
			{
				ValidCharacters valChars = ValidCharacters.Load(CurrVernWritingSystem);
		...
		private bool IsMatchedPairsListEmpty
		{
			get
			{
				...
				MatchedPairList pairs = MatchedPairList.Load(ws.MatchedPairs,
					ws.get_UiName(m_cache.DefaultUserWs));
		...
		private IWritingSystem CurrVernWritingSystem
		{
			get
			{
				...
				return lgwsf.get_EngineOrNull(m_cache.DefaultVernWs);
			}
		}
		...
		/// </summary>
		/// ------------------------------------------------------------------------------------
		public override void OnDocking(object sender, DockingEventArgs e)
		{
			base.OnDocking(sender, e);
		...
		/// </summary>
		/// ------------------------------------------------------------------------------------
		public void RefreshHistoryPaneSize()
		{
			if (Floaty.DockMode == DockStyle.Top)
			{
				...
				LoadHistoryPaneInfo("HistoryPaneRatioDockedTop", true);
			}
			else
			{
				...
				LoadHistoryPaneInfo(Floaty.DockMode == DockStyle.None ?
					"HistoryPaneRatioUndocked" : "HistoryPaneRatioDockedSide", false);
			}
		}
		...
		{
			if (m_nodeLastRightClickedOn == null)
				ShowHelp.ShowHelpTopic(FwApp.App, "khtpScrChecksOverview");
			else
			{
				string topicId;
				if (m_chkHelpIds.TryGetValue(m_nodeLastRightClickedOn.ScrCheck.CheckId, out topicId))
					ShowHelp.ShowHelpTopic(FwApp.App, topicId);
				else
					ShowHelp.ShowHelpTopic(FwApp.App, "khtpScrChecksUndocumented");
			}
		...
		{
			if (ValidCharactersDlg.RunDialog(m_cache, this, FwApp.App))
			{
				if (IsValidCharIncomplete)
					EnsureCheckIsUnselected(StandardCheckIds.kguidCharacters);
			}
