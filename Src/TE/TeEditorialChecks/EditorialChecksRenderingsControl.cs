...
namespace SIL.FieldWorks.TE.TeEditorialChecks
{
	...
	public partial class EditorialChecksRenderingsControl : CheckRenderingControl, IxCoreColleague
	{
		...
		/// </summary>
		/// ------------------------------------------------------------------------------------
		public EditorialChecksRenderingsControl(): this(null, null, null)
		{
		}
		...
		protected override object GetPreSortRow()
		{
			...
			return (error != null && error.IsValidObject() ? error.Guid : Guid.Empty);
		}
		...
		protected bool OnScrChecksIgnored(object args)
		{
			...
			CheckingError error = SelectedCheckingError;
			if (error != CheckingError.Empty && error.ResolutionStatus != NoteStatus.Closed)
			{
				string undo;
				string redo;
				...
				using (new UndoTaskHelper(m_cache.MainCacheAccessor, null, undo, redo, true))
					error.ResolutionStatus = NoteStatus.Closed;
			}
		...
		protected bool OnUpdateScrChecksIgnored(object args)
		{
			...
			CheckingError error = SelectedCheckingError;
			itemProps.Enabled = (error != CheckingError.Empty && error.ResolutionStatus == NoteStatus.Open);
			return true;
		}
		...
		protected bool OnScrChecksInconsistency(object args)
		{
			...
			CheckingError error = SelectedCheckingError;
			if (error != CheckingError.Empty && error.ResolutionStatus != NoteStatus.Open)
			{
				string undo;
				string redo;
				TeResourceHelper.MakeUndoRedoLabels("kstidUndoRedoResolutionStatusChange",
					out undo, out redo);
				using (new UndoTaskHelper(m_cache.MainCacheAccessor, null, undo, redo, true))
					error.ResolutionStatus = NoteStatus.Open;
			}
		...
		protected bool OnUpdateScrChecksInconsistency(object args)
		{
			...
			itemProps.Update = true;
			CheckingError error = SelectedCheckingError;
			itemProps.Enabled = (error != CheckingError.Empty && error.ResolutionStatus == NoteStatus.Closed);
			return true;
		}
			...
			bool fResolve)
		{
			...
			using (UndoTaskHelper undoHelper =
					new UndoTaskHelper(m_cache.MainCacheAccessor, null, undo, redo, true))
			{
				if (fResolve)
					error.ResolutionStatus = NoteStatus.Closed;

				using (CheckingErrorCommentDlg dlg = new CheckingErrorCommentDlg(error, StyleSheet))
				{
		...
		protected bool	OnUpdateScrChecksAddAsValidCharacter(object args)
		{
			...
			if (itemProps.Visible)
			{
				...
				itemProps.Enabled = (StringUtils.IsValidChar(error.CitedText, null, m_cache.UnicodeCharProps) &&
					validChars != null && !validChars.Contains(error.CitedText));
			}
			return true;
		}
		...
		{
			ILgWritingSystemFactory lgwsf = m_cache.LanguageWritingSystemFactoryAccessor;
			IWritingSystem ws = lgwsf.get_EngineOrNull(m_cache.DefaultVernWs);
		...
		protected override void OnSaveSettings(RegistryKey key)
		{
			...
			try
			{
				if (key != null && error != null && error != CheckingError.Empty)
					key.SetValue("SelectedCheckingError", error.Guid);
			}
			catch { }
		}
		...
		{
			if (tag == (int)ScrScriptureNote.ScrScriptureNoteTags.kflidResolutionStatus &&
	...
	public class EditorialChecksGrid : CheckGrid
	{
		...
		private FwTextBoxColumn m_detailsCol = null;

		private bool m_detailCellIsForNonPrintableChar = false;
		private bool m_contentsStale = false;
		...
		/// result grid.</summary>
		CharacterInfoToolTip m_charChkResultToolTip;
		...
		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Initializes a new instance of the <see cref="EditorialChecksGrid"/> class.
		/// </summary>
		/// ------------------------------------------------------------------------------------
		...
		public bool IsRowIgnoredWithAnnotation(int iRow)
		{
			...
			if (error == null || error.ResolutionStatus != NoteStatus.Closed)
				return false;

			// Go through the paragraphs and make sure
			// there is some text in at least one of them.
			foreach (IStTxtPara para in error.ResolutionOA.ParagraphsOS)
			{
		...
		protected override void OnCellMouseEnter(DataGridViewCellEventArgs e)
		{
				...
				e.ColumnIndex == EditorialChecksRenderingsControl.kDetailsCol)
			{
				...
				if (error != null && error.CheckId == StandardCheckIds.kguidCharacters)
				{
					...
					m_charChkResultToolTip.Show(this, error.CitedText, fnt);
				}
			}
		}
		...
		protected override void OnCellPainting(DataGridViewCellPaintingEventArgs e)
		{
			...
			if (!DrawMissingGlyphImage(error, e))
			{
				if (e.ColumnIndex == EditorialChecksRenderingsControl.kRefCol &&
					error.BeginObjectRA != null && error.BeginObjectRA.Owner is StFootnote)
				{
		...
		private bool DrawMissingGlyphImage(CheckingError error, DataGridViewCellPaintingEventArgs e)
		{
			...
			if (!Win32.AreCharGlyphsInFont(error.CitedText, e.CellStyle.Font))
			{
		...
		private void HandleToolTip()
		{
			...
			else
			{
				...
				m_commentToolTip.Show(error.ResolutionOA,
		...
		public override void PropChanged(int hvo, int tag, int ivMin, int cvIns, int cvDel)
		{
			switch (tag)
			{
				...
				case (int)ScrBookAnnotations.ScrBookAnnotationsTags.kflidNotes:
					...
					// owning hvo is zero (because those are the ones that were deleted).
					for (int i = m_list.Count - 1; i >= 0; i--)
					{
						CheckingError error = m_list[i] as CheckingError;
						if (error != null && error.OwnerHVO == 0)
						{
							m_list.RemoveAt(i);
