...
namespace SIL.FieldWorks.TE
{
	...
	public class ExportXml
	{
		...
		private void ExportTE()
		{
			...
			m_writer.WriteAttributeString("xmlns", "http://www.wycliffe.net/scripture/namespace/version_1.1.4");
			m_writer.WriteStartElement("oxesText");
			m_writer.WriteAttributeString("type", "Wycliffe-1.1.4");
		...
		/// ------------------------------------------------------------------------------------
		private object ExportScripture(IProgress progressDlg, object[] parameters)
		{
			string sCanon = null;
			switch (m_what)
			{
		...
		/// <param name="progressDlg">The progress dialog.</param>
		/// ------------------------------------------------------------------------------------
		private void ExportBook(ref string sCanon, IScrBook book, IProgress progressDlg)
		{
			...
			if (!String.IsNullOrEmpty(objData))
			{
				switch (objData[0])
				{
					case (char) FwObjDataTypes.kodtNameGuidHot:
						...
						break;
					default:
						throw new InvalidOperationException("From a back translation " +
							"we should not export any objects except a reference to a note.");
				}
			}
		}
		...
		{
			FwObjDataTypes odt;
			Guid footnoteRef = StringUtils.GetGuidFromProps(ttp,
