...
namespace SIL.FieldWorks.TE
{
	...
	public class XmlScrNote
	{
			...
			: this()
		{
			...
			m_typeName = ann.AnnotationTypeRA.Name.GetAlternative(ann.Cache.FallbackUserWs);
			...
			if (ann.BeginObjectRAHvo != ann.EndObjectRAHvo && ann.EndObjectRA != null)
		...
		{
			Debug.Assert(ann.AnnotationTypeRAHvo != 0, "The annotation type is not set!");
			if (ann.AnnotationTypeRAHvo == 0)
				return;

			if (m_guidType == LangProject.kguidAnnTranslatorNote)
				AnnotationType = XmlNoteType.Translator;
			else if (m_guidType == LangProject.kguidAnnConsultantNote)
			...
			IScrScriptureNote scrNote)
		{
				 ...
				 scrRef, true, out iSection, out iPara, out ichStart, out ichEnd))
			{
				...
				if (iSection == -1)
				{
					scrNote.BeginObjectRAHvo = scrNote.EndObjectRAHvo =
						book.TitleOA.ParagraphsOS[iPara].Hvo;
				}
				else
				{
					scrNote.BeginObjectRAHvo = scrNote.EndObjectRAHvo =
						book.SectionsOS[iSection].ContentOA.ParagraphsOS[iPara].Hvo;
				}
			}
		}
