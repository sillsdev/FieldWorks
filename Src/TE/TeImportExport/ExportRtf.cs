...
namespace SIL.FieldWorks.TE
{
	...
	public class RtfStyle : BaseStyleInfo
	{
		...
		/// ------------------------------------------------------------------------------------
		public RtfStyle(StStyle style, int defaultWs)
			: base(style)
		{
			m_defaultWs = defaultWs;
		}
		...
		{
			return StringUtils.Compose(ToString(styleName, styleTable, m_defaultWs));
		}
		...
		{
			if (IsParagraphStyle)
				return StringUtils.Compose(ParaStyleToString(styleName, styleTable, ws));
			else
				return StringUtils.Compose(CharStyleToString(styleName, styleTable, ws));
		}
		...
		private void AddFontInfoName(List<string> fontNames, FontInfo fontInfo)
		{
			...
			if (!fontNames.Contains(fontName) && !StStyle.IsMagicFontName(fontName))
	...
	public class RtfStyleInfoTable : StyleInfoTable
	{
		...
		public RtfStyleInfoTable(FdoCache cache, bool defaultRightToLeft) :
			base( ScrStyleNames.Normal, (cache == null) ? null : cache.LanguageWritingSystemFactoryAccessor)
		{
			m_cache = cache;
			...
			{
				for (int i = 0; i < m_cache.LanguageEncodings.Count; i++)
				{
					ILgWritingSystem ws = m_cache.LanguageEncodings.Item(i);
					AddFontName(ws.DefaultMonospace);
					AddFontName(ws.DefaultSansSerif);
					AddFontName(ws.DefaultSerif);
					AddFontName(ws.DefaultBodyFont);
				}
			}
		...
		private void AddFontName(string fontName)
		{
			if (fontName != null && fontName != string.Empty &&
				fontName[0] != '<' && !m_fontTable.ContainsKey(fontName))
			{
		...
		/// </summary>
		/// ------------------------------------------------------------------------------------
		private void ExportFontTable(StreamWriter writer)
		{
		...
		/// ------------------------------------------------------------------------------------
		private void ExportColorTable(StreamWriter writer)
		{
		...
		/// <param name="writer">The stream writer.</param>
		/// ------------------------------------------------------------------------------------
		private void ExportStyleTable(StreamWriter writer)
		{
	...
	public class ExportRtf
	{
		...
		public ExportRtf(string fileName, FdoCache cache, FilteredScrBooks filter,
			ExportContent content, int ws, FwStyleSheet styleSheet)
		{
			m_fileName = fileName;
			m_cache = cache;
			m_bookFilter = filter;
		...
		public bool Run()
		{
			...
			if (File.Exists(m_fileName))
			{
				...
				string sCaption = Application.ProductName;
			...
			try
			{
				m_writer = new StreamWriter(m_fileName, false, Encoding.ASCII);
			}
			catch (Exception e)
			{
		...
		private void ExportKeyTerms()
		{
			using (ProgressDialogWithTask progressDlg = new ProgressDialogWithTask(Form.ActiveForm))
			{
				...
				progressDlg.SetRange(1, m_cache.LangProject.KeyTermsList.PossibilitiesOS.Count);
				progressDlg.Position = 1;
				progressDlg.CancelButtonVisible = true;
				progressDlg.Cancel += new CancelHandler(OnExportCancel);

				progressDlg.RunTask(true, new BackgroundTaskInvoker(ExportKeyTerms));
			}
		}
		...
		/// ------------------------------------------------------------------------------------
		private object ExportKeyTerms(IAdvInd4 progressDlg, params object[] parameters)
		{
		...
		private void ExportTE()
		{
			...
			using (ProgressDialogWithTask progressDlg = new ProgressDialogWithTask(Form.ActiveForm))
			{
				...
				progressDlg.RunTask(true, new BackgroundTaskInvoker(ExportTE));
			}
		}
		...
		/// <returns></returns>
		/// ------------------------------------------------------------------------------------
		private object ExportTE(IAdvInd4 progressDialog, params object[] parameters)
		{
		...
		protected void BuildStyleTable()
		{
			...
			LgWritingSystem ws = new LgWritingSystem(m_cache, defaultWs);
			m_defaultWsIsRTL = ws.RightToLeft;
			...
			{
				StStyle style = new StStyle(m_cache, m_styleSheet.get_NthStyle(i));
				string fontName = m_styleSheet.GetFaceNameFromStyle(style.Name, defaultWs, wsf);
				m_rtfStyleTable.Add(style.Name, new RtfStyle(style, defaultWs));
			}
		...
		/// ------------------------------------------------------------------------------------
		private void ExportBook(ScrBook book)
		{
			// Export the title paragraphs
			foreach (StTxtPara titlePara in book.TitleOA.ParagraphsOS)
		...
		/// ------------------------------------------------------------------------------------
		private void ExportBookSection(ScrSection section)
		{
			// count the section for progress
			m_progressDlg.Step(0);

			// Export the header paragraphs
			foreach (StTxtPara headerPara in section.HeadingOA.ParagraphsOS)
				ExportParagraph(headerPara);

			// Export the content paragraphs
			foreach (StTxtPara contentPara in section.ContentOA.ParagraphsOS)
		...
		private ITsString GetBackTranslationText(IStTxtPara para)
		{
			...
			ITsString btText = trans.Translation.GetAlternative(m_btWS).UnderlyingTsString;
			if (btText != null && btText.Length != 0)
				return btText;
		...
		/// </summary>
		/// ------------------------------------------------------------------------------------
		protected void ExportParagraph(StTxtPara para)
		{
			...
			for (int run = 0; run < tss.RunCount; run++)
			{
				...
				if (runText.Length == 1 && runText[0] == StringUtils.kchObject)
				{
						...
						objData[0] == (char)FwObjDataTypes.kodtNameGuidHot)
					{
						...
						if (hvoFootnote != 0) // footnote note found
						{
							if (m_content == ExportContent.Scripture)
								ExportFootnote(new ScrFootnote(m_cache, hvoFootnote));
							else
								ExportFootnote(new ScrFootnote(m_cache, hvoFootnote), m_btWS);
						}
					}
				}
				else
		...
		{
			m_writer.Write(StringUtils.Compose(ConvertString(text)));
		}
		...
		public static string ConvertString(String text)
		{
			...
			foreach (char ch in text)
			{
				...
				else if (ch == '{' || ch == '}')
				{
					...
					bldr.Append(ch);
				else if (chValue == 0x2028)	// line break
					bldr.Append(@"\line ");
				else
		...
		/// ------------------------------------------------------------------------------------
		protected void ExportFootnote(ScrFootnote footnote)
		{
		...
		/// ------------------------------------------------------------------------------------
		protected void ExportFootnote(ScrFootnote footnote, int wsAlt)
		{
			...
			{
				tss = footnotePara.Contents.UnderlyingTsString;
			}
			else
		{
			...
			ITsTextProps paraProps = ((StTxtPara)footnote.ParagraphsOS[0]).StyleRules;
		...
		private void ExportKeyTermsData()
		{
			...
			m_progressDlg.SetRange(1, selectedTerms.Count);
		...
		private SortedDictionary<string, KeyTermDictionary> SelectKeyTerms(ICmPossibilityList keyTermsList)
		{
			...
			foreach (CmPossibility keyTermCategory in keyTermsList.PossibilitiesOS)
			{
				string name = StringUtils.Compose(keyTermCategory.ToString());
		...
		/// <returns></returns>
		/// ------------------------------------------------------------------------------------
		private KeyTermDictionary SelectKeyTermsForCategory(CmPossibility keyTermCategory)
		{
			KeyTermDictionary result = new KeyTermDictionary();
			foreach (IChkTerm keyTerm in keyTermCategory.SubPossibilitiesOS)
			{
				string name = StringUtils.Compose(keyTerm.ToString());
		...
		/// </summary>
		/// <param name="keyTerm">The key term.</param>
		/// <param name="termRefs">The term refs.</param>
		/// ------------------------------------------------------------------------------------
		/// ------------------------------------------------------------------------------------
		private void ExportSingleKeyTerm(string keyTerm, List<IChkRef> termRefs)
		{
			...
			foreach (IChkRef checkRef in termRefs)
			{
				...
				{
					rendering = wordForm.Form.GetAlternative(m_cache.DefaultVernWs);
