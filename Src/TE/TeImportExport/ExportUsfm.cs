...
using VersificationTable = SILUBS.SharedScrUtils.VersificationTable;
...
namespace SIL.FieldWorks.TE
{
	...
	public class FileWriter : IFWDisposable
	{
		...
		public virtual void Write(string outString)
		{
			...
			m_buffer.Append(TsStringUtils.Compose(outString));
		}
		...
		public virtual void WriteLine(string outString)
		{
			...
			WriteLine(TsStringUtils.Compose(outString), false);
		}
	...
	public class ExportUsfm : IFWDisposable
	{
		...
		private readonly FileNameFormat m_fileNameFormat;
		private readonly string m_outputSpec;
		private readonly bool m_splitByBook;
		private readonly SortedDictionary<string, string> m_markerMappings;
		private readonly List<string> m_footnoteContentMarkers;
		...
		public void Run(Form dialogOwner)
		{
			...
			if (m_splitByBook)
			{
				for (int bookIndex = 0; bookIndex < m_bookFilter.BookCount; bookIndex++)
				{
					...
					if (!m_overwriteAll && File.Exists(filename))
					{
						...
						{
							DialogResult result = MessageBox.Show(dialogOwner,
			...
			using (ProgressDialogWithTask progressDlg = new ProgressDialogWithTask(dialogOwner, m_cache.ThreadHelper))
			{
				try
				{
					...
					progressDlg.RunTask(Export, writeFiles);
				}
				catch (WorkerThreadException e)
				{
					if (e.InnerException is InvalidOrMissingStyFileException)
					{
						...
						MessageBox.Show(dialogOwner, msg, m_app.ApplicationName, MessageBoxButtons.OK,
		...
		/// ------------------------------------------------------------------------------------
		private object Export(IThreadedProgress progressDlg, object[] parameters)
		{
		...
		/// ------------------------------------------------------------------------------------
		private void ExportScripture(IThreadedProgress progressDlg, bool[] writeFiles)
		{
		...
		protected void CreateParatextProjectFiles()
		{
			...
			if (m_fileNameFormat != null)
			{
				...
				WriteParatextSsfFile(m_paratextProjFolder, styleFileName, ref ldsFileName);
		...
		/// ------------------------------------------------------------------------------------
		private void WriteParatextSsfFile(string folder, string styleFileName, ref string ldsFileName)
		{
			...
			bool fExistingSsfFileValidated = false;
			IEnumerable<string> validPtProjects = ParatextHelper.ShortNames;
			if (validPtProjects.Any(sName => sName == m_p6ShortName))
			{
				Debug.Assert(ParatextHelper.IsProjectWritable(m_p6ShortName));
				fExistingSsfFileValidated = true;
				fUpdateExisting = true;
			}
			else if (validPtProjects.Any())
			{
			...
			{
				// We only ignore errors during the update attempt if the existing Paratext project
				// could not be validated above. While this may seem bizarre and nearly brain-dead,
				// the reason is that we suspect the failure is probably due to a corrupt file. In
				// that case, we just attempt to overwrite it and get on with life. If Paratext is
				// installed, we assume the file is not corrupt since Paratext successfully loaded it.
				if (fExistingSsfFileValidated)
					throw;
			}
