...
namespace SIL.FieldWorks.TE
{
	...
	public class FileWriter : IFWDisposable
	{
		...
		protected virtual void Dispose(bool disposing)
		{
			...
			if (m_out != null)
			{
				// Do not try to write it out here,
				// since the required StringBuilder may have been finalized already.
				m_out.Close();
				m_out = null;
			}
	...
	public class ExportUsfm : IFWDisposable
	{
		...
		private List<string> m_annotationQueue;
		...
		public ExportUsfm(FdoCache cache, FilteredScrBooks filter, string outputFolder,
			...
			:
			this(cache, filter, outputFolder)
		{
			m_splitByBook = true;
		...
		/// ------------------------------------------------------------------------------------
		public ExportUsfm(FdoCache cache, FilteredScrBooks filter, string outputFile)
		{
			...
			m_annotationQueue = new List<string>(8);
			...
			m_icuLocales = new string[] { new LgWritingSystem(m_cache, m_defaultAnalWS).ICULocale };
		...
		/// ------------------------------------------------------------------------------------
		private object Export(IAdvInd4 progressDlg, params object[] parameters)
		{
		...
		public int[] RequestedAnalysisWss
		{
			...
			set
			{
				...
				for (int i = 0; i < numWS; i++)
					m_icuLocales[i] = new LgWritingSystem(m_cache, m_requestedAnalWS[i]).ICULocale;
			}
		}
		...
		/// ------------------------------------------------------------------------------------
		private void ExportScripture(IAdvInd4 progressDlg, bool[] writeFiles)
		{
		...
		/// ------------------------------------------------------------------------------------
		protected void ExportBook(IAdvInd4 progressDlg, IScrBook book)
		{
			...
			m_currentBookAbbrev = book.Abbrev.VernacularDefaultWritingSystem;
			...
			if (m_exportScripture)
				bookName = book.Name.VernacularDefaultWritingSystem;
			else if (m_exportBackTrans)
				bookName = book.Name.GetAlternative(m_nonInterleavedBtWs);
		...
		protected void ExportParagraph(IStTxtPara para)
		{
			...
			ITsString paraText;
			if (m_exportScripture)
				paraText = para.Contents.UnderlyingTsString;
			else
			{	// get the tss of the requested back translation
				if (!m_exportBackTrans)
					return;
				ICmTranslation trans = para.GetBT();
				TsStringAccessor tssa = (trans != null ?
					trans.Translation.GetAlternative(m_nonInterleavedBtWs) : null);

				if (tssa != null)
					paraText = tssa.UnderlyingTsString;
				else
				{
			...
			if (style != null && style.Structure == StructureValues.Heading)
			{
				...
				ScrSection section = (ScrSection)(new StText(m_cache, para.OwnerHVO).Owner);
			...
			out RefElement runType, out int number, out bool invalidSyntax, out string numText)
		{
			...
			if (StStyle.IsStyle(runProps, ScrStyleNames.VerseNumber))
			{
				runType = RefElement.Verse;
				numText = tssPara.get_RunText(0).Trim();
				number = VerseBeginNumToInt(numText, out invalidSyntax);
			}
			else if (StStyle.IsStyle(runProps, ScrStyleNames.ChapterNumber))
			{
		...
		private bool ParaBeginsWithImplicitFirstVerse(ITsString tssPara, int iChapterRun)
		{
			...
			if (StStyle.IsStyle(tssPara.get_Properties(iChapterRun), ScrStyleNames.ChapterNumber))
			...
			if (StStyle.IsStyle(tssPara.get_Properties(indexOfNextRunAfterChapter),
			...
			for (int iRun = iChapterRun; iRun < tssPara.RunCount; iRun++)
			{
				...
				charStyleName = runProps.GetStrPropValue((int)FwTextPropType.ktptNamedStyle);
			...
			ICmTranslation backTranslation, int paraHvo, ExportMode exportMode)
		{
			...
			for (int iRun = 0; iRun < tssPara.RunCount; iRun++)
			{
				...
				if (runText.Length > 0 && runText[0] == (char)0x2028)
				{
			...
			RefElement type, string givenNumber)
		{
			...
			for (int iWs = 0; iWs < m_requestedAnalWS.Length; iWs++)
			{
				...
				ITsString tssBt = cmTrans.Translation.GetAlternative(m_requestedAnalWS[iWs]).UnderlyingTsString;
		...
		/// </summary>
		/// <param name="paraMarker">the paragraph marker that was used for the vernacular
		/// <param name="backTranslation">the back translation structured string</param>
		/// paragraph</param>
			...
			ICmTranslation backTranslation)
		{
			...
			for (int iWs = 0; iWs < m_requestedAnalWS.Length; iWs++)
			{
				...
				ITsString tssBt =
					backTranslation.Translation.GetAlternative(m_requestedAnalWS[iWs]).UnderlyingTsString;
				if (tssBt.Length == 0)
					continue;
		...
		protected List<ChapterVerseInfo>[] ParseBackTranslationVerses(ICmTranslation cmTrans)
		{
			...
			for (int iWs = 0; iWs < m_requestedAnalWS.Length; iWs++)
			{
				...
				ITsString tss = cmTrans.Translation.GetAlternative(m_requestedAnalWS[iWs]).UnderlyingTsString;
		...
		/// ------------------------------------------------------------------------------------
		private void ExportFootnote(ScrFootnote footnote, ExportMode exportMode, int iWs)
		{
			...
			for (int iPara = 0; iPara < nParas; iPara++)
			{
				...
				else
				{
					...
					ICmTranslation trans = para.GetBT();
					tss = (trans == null) ? null :
						trans.Translation.GetAlternative(m_requestedAnalWS[iWs]).UnderlyingTsString;
				}
		...
		private void ExportNotesForThisPara(int paraHvo)
		{
			...
			// loop through the relevant annotations
			ScrScriptureNote annotation;
			BCVRef noteBeginRef;
			int annotationIndex = 0;
			while (annotationIndex < m_annotationList.Count)
			{
				annotation = (ScrScriptureNote)m_annotationList[annotationIndex];
				noteBeginRef = annotation.BeginRef;

				// if note ref is equal (or prior) to the current end reference,
				// and if the note matches the given paragraph hvo
				if (noteBeginRef <= currentEndRef && annotation.BeginObjectRAHvo == paraHvo)
				{
		...
		private void ExportNote(IScrScriptureNote annotation)
		{
			...
			foreach (StTxtPara para in annotation.DiscussionOA.ParagraphsOS)
				m_file.WriteLine(para.Contents.UnderlyingTsString.Text);
		}
		...
		protected void CreateAnnotationList(int bookOrd)
		{
			if (!ExportNotesDomain)
				return;
			FdoOwningSequence<IScrScriptureNote> notes = m_scr.BookAnnotationsOS[bookOrd - 1].NotesOS;
		...
		protected void CreateStyleTables()
		{
			if (m_exportParatextProjectFiles)
			{
				int defaultWs = m_cache.DefaultVernWs;
				LgWritingSystem ws = new LgWritingSystem(m_cache, defaultWs);
				m_UsfmStyFileAccessor = new UsfmStyFileAccessor(ws.RightToLeft,
		...
		protected virtual void LoadStyleTables()
		{
			...
			doc.Load(DirectoryFinder.FWCodeDirectory + @"\Translation Editor\Testyles.xml");
		...
		protected void SetStyleMapping(string styleName, XmlNode paraStyleContext, string marker)
		{
			...
			if (m_markerMappings.ContainsKey(key))
			{
				...
				MessageBox.Show(string.Format(TeResourceHelper.GetResourceString("kstidMultipleStyleExportCodes"), styleName, m_markerMappings[key]), Application.ProductName);
			}
			else
			{
		...
		private void CopyVersificationFile()
		{
			...
			{
				string sourceVerseFile = Path.Combine(DirectoryFinder.GetFWCodeSubDirectory("Translation Editor"),
					versificationName);
				try
				{
		...
		{
			string styFile = Path.Combine(
				DirectoryFinder.GetFWCodeSubDirectory("Translation Editor"), "usfm.sty");
			m_UsfmStyFileAccessor.ReadStylesheet(styFile);
		}
