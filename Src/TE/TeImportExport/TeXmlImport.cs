...
#pragma warning restore 219
		...
		private void ProcessBookTitle(XmlReader reader)
		{
			...
			if (!String.IsNullOrEmpty(sName))
				m_scrBook.Name.VernacularDefaultWritingSystem =
					TsStringUtils.MakeTss(sName, m_cache.DefaultVernWs);
			...
			List<IScrScriptureNote> annotationsForPara, List<BTSegment> segmentBTsForPara)
		{
			...
			if (File.Exists(sPath))
			{
				pict = m_cache.ServiceLocator.GetInstance<ICmPictureFactory>().
					Create(sPath, m_ParaBldr.StringBuilder.GetString(), CmFolderTags.DefaultPictureFolder);
			}
			else
			{
		...
		private ITsString GetString(ITsStrBldr bldr, int ws)
		{
			...
			return TsStringUtils.MakeTss("", ws);
		}
			...
			List<IScrScriptureNote> annotationsForPara, List<BTSegment> segmentBTsForPara)
		{
			...
			foreach (int ws in m_BTStatus.Keys)
			{
				...
				if (!String.IsNullOrEmpty(status))
				{
					...
					trans.Status.set_String(ws, TsStringUtils.MakeTss(status, ws));
				}
			}
		...
		private int GetWsForLang(string sLang)
		{
			...
			WritingSystemServices.FindOrCreateWritingSystem(m_cache, FwDirectoryFinder.TemplateDirectory, IetfLanguageTagHelper.ToIetfLanguageTag(sLang),
				true, false, out ws);
			return ws.Handle;
		}
