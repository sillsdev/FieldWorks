...
namespace SIL.FieldWorks.TE
{
	...
	public class TeXmlImporter : TeImporter
	{
		...
		/// <summary>Mapper from RFC lang codes (string) to writing systems (int)</summary>
		private RfcWritingSystem m_rfcWs;
		/// <summary>
		...
		private void ProcessBookTitle(XmlReader reader)
		{
			...
			if (!String.IsNullOrEmpty(sName))
				m_scrBook.Name.VernacularDefaultWritingSystem = sName;
			ProcessTitle(xnTitle, ScrStyleNames.MainBookTitle, m_hvoTitle);
		}
		...
		/// -----------------------------------------------------------------------------------
		private void ProcessTitle(XmlNode xnTitle, string sStyle, int hvoOwner)
		{
			...
			StoreXmlParagraphData(hvoOwner, sStyle, annotationsForPara, segmentBTsForPara);
		}
			...
			List<IScrScriptureNote> annotationsForPara, List<BTSegment> segmentBTsForPara)
		{
			...
			{
				pict = new CmPicture(m_cache, sPath, m_ParaBldr.StringBuilder.GetString(),
			...
			else
			{
				...
				else
				{
					...
					sPath = Path.Combine(m_cache.LangProject.ExternalLinkRootDir, sPath);
				}
				pict = new CmPicture(m_cache, sPath, m_ParaBldr.StringBuilder.GetString(),
					EditingHelper.DefaultPictureFolder);
				sPath = String.Format("Pictures{0}{1}", Path.DirectorySeparatorChar, src);
				if (File.Exists(Path.Combine(m_cache.LangProject.ExternalLinkRootDir, sPath)))
					pict.PictureFileRA.InternalPath = sPath;
			}
			foreach (int ws in m_BTStrBldrs.Keys)
				pict.Caption.SetAlternative(m_BTStrBldrs[ws].GetString(), ws);
			return pict.Guid;
		}
		...
		private void ProcessFootnote(XmlNode node)
		{
			...
			m_CurrFootnote = ScrFootnote.InsertFootnoteAt(m_scrBook, m_iCurrFootnote,
				m_ParaBldr.StringBuilder, m_ParaBldr.Length, m_scr.GetFootnoteMarker(sParaStyle));
			...
			StoreXmlParagraphData(m_CurrFootnote.Hvo, sParaStyle, annotationsForPara, segmentBTsForPara);
		...
		private void SaveParagraphStringBuilders()
		{
			...
			foreach (int ws in m_BTStrBldrs.Keys)
			{
				m_SavedBTStrs.Add(ws, m_BTStrBldrs[ws].GetString());
				m_BTStrBldrs[ws].Clear();
			}
			m_SavedBTNumberStrs.Clear();
			foreach (int ws in m_BTNumberStrBldrs.Keys)
			{
				m_SavedBTNumberStrs.Add(ws, m_BTNumberStrBldrs[ws].GetString());
				m_BTNumberStrBldrs[ws].Clear();
			}
		...
		private ITsTextProps GetBackTransChapterNumberProps(int ws)
		{
			...
			{
				ttp = StyleUtils.CharStyleTextProps(ScrStyleNames.ChapterNumber, m_wsVern);
		...
		private ITsTextProps GetBackTransVerseNumberProps(int ws)
		{
			...
			if (m_fInFootnote)
			{
				...
				{
					ttp = StyleUtils.CharStyleTextProps(ScrStyleNames.VerseNumberInNote, m_wsVern);
			...
			else
			{
				...
				{
					ttp = StyleUtils.CharStyleTextProps(ScrStyleNames.VerseNumber, m_wsVern);
		...
		/// builders.
		/// </summary>
		/// <param name="hvoOwner">The HVO of the owning StText</param>
		/// <param name="sStyle">The name of the paragraph style</param>
		/// <param name="annotationsForPara">List of any annotations added or modified for this
		/// paragraph</param>
		/// <param name="segmentBTsForPara"></param>
		/// -----------------------------------------------------------------------------------
		private void StoreXmlParagraphData(int hvoOwner, string sStyle,
			List<IScrScriptureNote> annotationsForPara, List<BTSegment> segmentBTsForPara)
		{
			m_ParaBldr.ParaStylePropsProxy = StyleProxyListManager.GetXmlParaStyleProxy(
			...
			IStTxtPara para = m_ParaBldr.CreateParagraph(hvoOwner);
			...
			foreach (int ws in m_BTStrBldrs.Keys)
			{
				...
				if (tsb.Length > 0)
				{
					...
					trans.Translation.SetAlternative(tsb.GetString(), ws);
				}
			}
			foreach (int ws in m_BTStatus.Keys)
			{
				...
				if (!String.IsNullOrEmpty(status))
				{
					...
					trans.Status.SetAlternative(status, ws);
				}
			}
...
//#endif
		...
		private void ReadXmlStringElement(XmlNode node, int ws, string sStyle, ITsStrBldr tsb)
		{
			...
			if (node.HasChildNodes)
			{
				foreach (XmlNode xn in node.ChildNodes)
				{
					...
					{
						string textToAdd = MiscUtils.CleanupForXmlSpec(xn.InnerText);
		...
		{
			return m_rfcWs.GetWsFromRfcLang(sLang, m_sOXESDir);
		}
			...
			List<IScrScriptureNote> annotationsForPara, List<BTSegment> segmentBTsForPara)
		{
			...
			if (fHeading)
				StoreXmlParagraphData(sect.HeadingOAHvo, sStyle, annotationsForPara, segmentBTsForPara);
			else
				StoreXmlParagraphData(sect.ContentOAHvo, sStyle, annotationsForPara, segmentBTsForPara);
		}
		...
		/// -----------------------------------------------------------------------------------
		/// <summary>
		/// Create and return the standard XML reader settings we want for this import.  There
		/// is no validation, comments are ignored, and whitespace is significant.
		/// </summary>
		/// -----------------------------------------------------------------------------------
