...
namespace SIL.FieldWorks.TE
{
	...
	public abstract class TeImporter : IFWDisposable
	{
		...
		/// </summary>
		protected StFootnote m_CurrFootnote;
		...
		protected int m_hvoTitle;
		/// <summary>Current section we are adding to</summary>
		protected IScrSection m_currSection;
		/// <summary>ID of StText that is the section heading</summary>
		protected int m_hvoSectionHeading;
		/// <summary>ID of StText that is the section content</summary>
		protected int m_hvoSectionContent;
		...
		protected Dictionary<ScrScriptureNote.ScrNoteKey, ScrScriptureNote> m_existingAnnotations;
		#endregion

		#region Methods shared by subclasses
		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Constructor needed by subclass constructors.
		/// </summary>
		/// ------------------------------------------------------------------------------------
		protected TeImporter()
		{
		}
		...
		{
			string sImportMsgFmt = TeResourceHelper.GetResourceString(stid);
			m_importCallbacks.StatusMessage = string.Format(sImportMsgFmt, (m_scrBook == null)?
				ScrReference.NumberToBookCode(m_nBookNumber) : (m_scrBook as ScrBook).BestUIName);
		}
		...
		protected void StartBookTitle()
		{
			...
			StText title = new StText(m_cache, m_hvoTitle);
			int cTitleParas = title.ParagraphsOS.Count;
			for (int i = 0; i < cTitleParas; i++)
				title.ParagraphsOS.RemoveAt(0);
		}
		...
		{
			m_scrBook = m_undoManager.AddNewBook(m_nBookNumber, SavedVersionDescription, out m_hvoTitle);
		...
		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Gets the descriptionof the saved version; gets the default desciption if not set.
		/// </summary>
		/// ------------------------------------------------------------------------------------
		...
		protected void FinalizePrevSection()
		{
			...
			m_currSection.AdjustReferences(!m_fInScriptureText);
			...
			if (m_currSection.HeadingOA.ParagraphsOS.Count == 0)
			{
				...
				paraBldr.CreateParagraph(m_hvoSectionHeading);
			}
			...
			if (m_currSection.ContentOA.ParagraphsOS.Count == 0)
			{
				...
				paraBldr.StringBuilder.SetIntPropValues(0, 0, (int)FwTextPropType.ktptWs,
					(int)FwTextPropVar.ktpvDefault, m_wsVern);
				paraBldr.CreateParagraph(m_hvoSectionContent);
			}
		...
		{
			FdoOwningSequence<IScrScriptureNote> curBookNotes =
				m_scr.BookAnnotationsOS[m_nBookNumber - 1].NotesOS;
			m_existingAnnotations =
				new Dictionary<ScrScriptureNote.ScrNoteKey, ScrScriptureNote>(curBookNotes.Count);
			foreach (ScrScriptureNote ann in curBookNotes)
			{
	...
	public class TeSfmImporter : TeImporter
	{
		...
		protected IStTxtPara m_lastPara;
		...
		/// </summary>
		protected StFootnote m_CurrBTFootnote;
		/// <summary>
		...
		private bool HandleSpecialTargets()
		{
			...
			switch(m_styleProxy.MappingTarget)
			{
				...
				case MappingTargetType.FigureFilename:
					...
					if (!FileUtils.IsPathNameValid(m_sSegmentText))
					{
				...
				case MappingTargetType.TitleShort:
					{
						...
						if (m_scrBook != null)
							m_scrBook.Name.SetAlternative(m_sSegmentText.Trim(), ws);
		...
		private void InsertSegment()
		{
			...
			{
				// We must be processing a BT segment
			}
		...
		protected void InsertPicture()
		{
			...
			if (m_currPictureInfo == null)
			{
				string[] tokens = m_sSegmentText.Split(new char[] {'|'});
				if (tokens.Length < 6)
				...
				picture = new CmPicture(m_cache, EditingHelper.DefaultPictureFolder,
					m_currentRef.BBCCCVVV, m_scr as IPictureLocationBridge, sDescription,
					srcFilename, sLayoutPos, sLocationRange, sCopyright, sCaption,
					PictureLocationRangeType.ReferenceRange, "100");
			}
			else
			{
				...
				if (String.IsNullOrEmpty(srcFilename))
					srcFilename = Path.Combine(DirectoryFinder.FWCodeDirectory, "MissingPictureInImport.bmp");
				picture = new CmPicture(m_cache, EditingHelper.DefaultPictureFolder,
					m_currentRef.BBCCCVVV, m_scr as IPictureLocationBridge,
					m_currPictureInfo.Description, srcFilename,
			...
			picture.AppendPicture(GetWsForContext(m_ParaBldr.StringBuilder),
				m_ParaBldr.StringBuilder);
		...
		/// ------------------------------------------------------------------------------------
		protected IStTxtPara FindCorrespondingVernParaForAnnotation(BCVRef targetRef)
		{
			m_lastPara = null;
			if (m_scrBook is ScrBook)
			{
				m_lastPara = (m_scrBook as ScrBook).FindCorrespondingVernParaForSegment(null,
					targetRef, 0, ref m_iCurrSection);
			}

			if (m_lastPara == null)
			{
				m_lastPara = (m_scr as Scripture).FindCorrespondingVernParaForSegment(null,
					targetRef, 0, ref m_iCurrSection);
			}
			return m_lastPara;
		}
		...
		/// ------------------------------------------------------------------------------------
		protected CmPicture FindCorrespondingPicture(int index)
		{
			if (index >= m_CurrParaPictures.Count)
				return null;
			return (CmPicture)m_CurrParaPictures[index];
		}
		...
		/// found</returns>
		/// ------------------------------------------------------------------------------------
		protected StFootnote FindCorrespondingFootnote(string styleId)
		{
			...
			StFootnote footnote = FindCorrespondingFootnote(styleId, index);
			if (footnote != null)
		...
		/// found</returns>
		/// ------------------------------------------------------------------------------------
		protected StFootnote FindCorrespondingFootnote(string styleId, int index)
		{
			try
			{
				FootnoteInfo finfo = (FootnoteInfo)m_CurrParaFootnotes[index];
		...
		{
			FinalizePrevParagraph(); // Add any previously encountered paragraph to the database.
			FinalizePrevSection();
			if (m_scrBook != null)
			{
				lock (this)
				{
					m_sPrevBook = (m_scrBook as ScrBook).BestUIName;
				}
			}
			...
			CurrentBook.Name.UserDefaultWritingSystem = (CurrentBook as ScrBook).BestUIName;
		...
		{
			if (!m_fInScriptureText && !m_fCurrentSectionIsIntro && m_hvoSectionHeading != 0)
			{
				m_fCurrentSectionIsIntro = true;
				StText text = new StText(m_cache, m_hvoSectionHeading);
		...
		private void SetBookName()
		{
			...
			if (m_styleProxy.StyleId == ScrStyleNames.MainBookTitle && sTitle.Length > 0 &&
				String.IsNullOrEmpty(CurrentBook.Name.GetAlternative(ws)))
			{
				CurrentBook.Name.SetAlternative(sTitle, ws);
		...
		protected void FinalizePrevParagraph()
		{
			...
			if (m_ParaBldr.Length > 0 && m_scrBook != null)
			{
				...
				if (m_fInBookTitle)
				{
					m_lastPara = m_ParaBldr.CreateParagraph(m_hvoTitle);
				}
				else
				{
					...
					m_lastPara = m_ParaBldr.CreateParagraph(
						m_fInSectionHeading ? m_hvoSectionHeading : m_hvoSectionContent);
				}
			}
			else if (((SIL.FieldWorks.TE.ImportStyleProxy)m_ParaBldr.ParaStylePropsProxy).Function == FunctionValues.StanzaBreak &&
				!m_fInBookTitle && m_currSection != null)
			{
				// Stanza Breaks are the exception to the rule. We expect these to be empty paragraphs!
				m_lastPara = m_ParaBldr.CreateParagraph(m_hvoSectionContent);
			}
				...
				foreach (KeyValuePair<int, ITsStrBldr> kvp in m_BTStrBldrs)
				{
					...
					if (length > 0 || m_BtFootnoteStrBldrs.Count > 0)
					{
						...
						if (fAppend)
						{
							...
							bldr.ReplaceTsString(0, 0, trans.Translation.GetAlternative(ws).UnderlyingTsString);
						}
						trans.Translation.GetAlternative(ws).UnderlyingTsString = bldr.GetString();
						fFoundSomeText = true;
					}
				}
		...
		private void ReportBTTextNotPartOfPara()
		{
			...
			string sCharStyle = null;
			sCharStyle = bldr.get_Properties(0).GetStrPropValue(
				(int)FwTextPropType.ktptNamedStyle);

			if (sCharStyle == null)
				sCharStyle = ResourceHelper.GetResourceString("kstidDefaultParaCharStyleName");
		...
		private void AddBTPictureCaptionsAndCopyrights()
		{
			...
			{
				CmPicture picture = FindCorrespondingPicture(index);
				...
				if (tssCaption != null)
					picture.Caption.SetAlternative(tssCaption, pictureInfo.m_ws);
				if (!String.IsNullOrEmpty(pictureInfo.m_copyright))
				{
					picture.PictureFileRA.Copyright.SetAlternative(pictureInfo.m_copyright,
						pictureInfo.m_ws);
				}
			}
		...
		protected void BeginFootnote()
		{
			...
			m_CurrFootnote = ScrFootnote.InsertFootnoteAt(m_scrBook, m_iCurrFootnote,
				strbldr, ichMarker, sFootnoteMarker);
		...
		protected void LoadImportMappingProxies()
		{
			...
			LoadImportMappingProxies(m_styleProxies, (m_settings as ScrImportSet).Mappings(MappingSet.Main));

			if (fHasNonInterleavedNotes)
				LoadImportMappingProxies(m_notesStyleProxies, (m_settings as ScrImportSet).Mappings(MappingSet.Notes));
		}
			...
			IEnumerable mappings)
		{
			...
			{
				if (mapping.BeginMarker == ScrImportSet.s_markerBook)
					continue;
			...
			styleProxies[ScrImportSet.s_markerBook] =
