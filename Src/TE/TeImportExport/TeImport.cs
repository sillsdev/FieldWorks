	...
	public abstract class TeImporter : IFWDisposable
	{
		...
		protected void UpdateProgressDlgForBook(string stid)
		{
			...
			m_importCallbacks.StatusMessage = string.Format(sImportMsgFmt, (m_scrBook == null)?
				ScrReference.NumberToBookCode(m_nBookNumber) : m_scrBook.BestUIName);
		}
				...
				case MappingTargetType.TitleShort:
					{
						...
						if (m_scrBook != null)
							m_scrBook.Name.set_String(ws, StringUtils.MakeTss(m_sSegmentText.Trim(), ws));
...
#pragma warning restore 219
		...
		protected void InsertPicture()
		{
			...
			if (m_currPictureInfo == null)
			{
				...
				picture = picFactory.Create(EditingHelper.DefaultPictureFolder, m_currentRef.BBCCCVVV,
		...
		protected void ProcessBackTransSegment()
		{
			...
			else
			{
				...
				strbldr = StringUtils.MakeTss("", m_wsCurrBtPara).GetBldr();
				Debug.Assert(m_wsCurrBtPara != 0);
		...
		private void ProcessStartOfBook()
		{
			...
			CurrentBook.Name.UserDefaultWritingSystem =
				StringUtils.MakeTss(CurrentBook.BestUIName, m_cache.DefaultUserWs);
		...
		private void SetBookName()
		{
			...
			{
				CurrentBook.Name.set_String(ws, StringUtils.MakeTss(sTitle, ws));
