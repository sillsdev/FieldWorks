...
namespace SIL.FieldWorks.TE
{
	...
	public class TeImportManager
	{
		...
		protected TeImportManager(FwMainWnd mainWnd, ITeImportCallbacks importCallbacks,
			...
			: this(mainWnd.Cache, mainWnd.StyleSheet, helpTopicProvider, app)
		{
			m_mainWnd = mainWnd;
		...
		public static void ImportSf(FwMainWnd mainWnd, ITeImportCallbacks importCallbacks,
			IHelpTopicProvider helpTopicProvider, FwApp app)
		{
			TeImportManager mgr = new TeImportManager(mainWnd, importCallbacks, helpTopicProvider, app);
			mgr.ImportSf();
		}
		...
		/// ------------------------------------------------------------------------------------
		private void DoImport(IScrImportSet importSettings, string updateDescription)
		{
			try
			{
		...
		private ScrReference InternalImport(IScrImportSet importSettings, bool fDisplayUi)
		{
			...
			if (m_importedSavedVersion != null && (!m_importedSavedVersion.IsValidObject ||
				m_importedSavedVersion.BooksOS.Count == 0) && !fPartialBtImported)
			{
		...
		protected ScrReference CompleteImport(ScrReference firstImported)
		{
			...
			if (m_importedSavedVersion != null && m_importedSavedVersion.IsValidObject &&
				m_importedSavedVersion.BooksOS.Count > 0)
			{
				...
				{
					Logger.WriteEvent("No imported books left. Discarding imported version.");
