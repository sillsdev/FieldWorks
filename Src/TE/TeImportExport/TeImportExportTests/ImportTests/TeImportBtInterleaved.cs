...
namespace SIL.FieldWorks.TE.ImportTests
{
	...
	public class TeImportBtInterleaved: TeImportTestsBase
	{
		...
		public void BackTranslationInterleaved()
		{
			...
			Assert.AreEqual("Kmain Ktitle", m_importer.ScrBook.Name.GetAlternative(
				m_wsVern));
			// and its back translation
			m_importer.ProcessSegment("Main Title", @"\btmt");
			Assert.AreEqual("Exodus", m_importer.ScrBook.Name.GetAlternative(
				m_wsAnal));
			...
			Assert.AreEqual(1, title.Contents.UnderlyingTsString.RunCount);
			AssertEx.RunIsCorrect(title.Contents.UnderlyingTsString, 0, "Kmain Ktitle", null, DefaultVernWs);
			// verify that back translation of title was added to the DB
			Assert.AreEqual(1, title.TranslationsOC.Count);

			ICmTranslation transl = CmTranslation.CreateFromDBObject(Cache, title.TranslationsOC.HvoArray[0]);
			Assert.AreEqual(LangProject.kguidTranBackTranslation, transl.TypeRA.Guid);
			ITsString tss = transl.Translation.AnalysisDefaultWritingSystem.UnderlyingTsString;
			...
			StTxtPara heading = (StTxtPara)section0.HeadingOA.ParagraphsOS[0];
			Assert.AreEqual(StyleUtils.ParaStyleTextProps("Intro Section Head"),
				heading.StyleRules);
			Assert.AreEqual(1, heading.Contents.UnderlyingTsString.RunCount);
			AssertEx.RunIsCorrect(heading.Contents.UnderlyingTsString, 0, "Kintro Ksection",
				null, DefaultVernWs);

			// Check the BT of the intro section para
			Assert.AreEqual(1, heading.TranslationsOC.Count);
			transl = CmTranslation.CreateFromDBObject(Cache, heading.TranslationsOC.HvoArray[0]);
			Assert.AreEqual(LangProject.kguidTranBackTranslation, transl.TypeRA.Guid);
			tss = transl.Translation.AnalysisDefaultWritingSystem.UnderlyingTsString;
			Assert.AreEqual(1, tss.RunCount);
			...
			StTxtPara para = (StTxtPara)section0.ContentOA.ParagraphsOS[0]; // intro para
			...
			AssertEx.RunIsCorrect(para.Contents.UnderlyingTsString, 0, "Kintro Kparagraph", null, DefaultVernWs);

			// Check the BT of the intro para
			Assert.AreEqual(1, para.TranslationsOC.Count);
			transl = CmTranslation.CreateFromDBObject(Cache, para.TranslationsOC.HvoArray[0]);
			Assert.AreEqual(LangProject.kguidTranBackTranslation, transl.TypeRA.Guid);
			tss = transl.Translation.AnalysisDefaultWritingSystem.UnderlyingTsString;
			Assert.AreEqual(1, tss.RunCount);
			...
			heading = (StTxtPara)section1.HeadingOA.ParagraphsOS[0];
			Assert.AreEqual(StyleUtils.ParaStyleTextProps("Section Head"), heading.StyleRules);
			Assert.AreEqual(1, heading.Contents.UnderlyingTsString.RunCount);
			AssertEx.RunIsCorrect(heading.Contents.UnderlyingTsString, 0, "Kscripture Ksection",
				null, DefaultVernWs);
			...
			para = (StTxtPara)section1.ContentOA.ParagraphsOS[0]; //first para
			Assert.AreEqual(StyleUtils.ParaStyleTextProps("Paragraph"), para.StyleRules);
			Assert.AreEqual(expectedParaRunCount, para.Contents.UnderlyingTsString.RunCount);
			...
			para = (StTxtPara)section1.ContentOA.ParagraphsOS[1]; //second para
			...
			// verify completed section head was added to DB (for 1:1-2)
			book = (ScrBook)CmObject.CreateFromDBObject(Cache, m_importer.ScrBook.Hvo, true); //refresh cache
			...
			heading = (StTxtPara)section2.HeadingOA.ParagraphsOS[0];
			AssertEx.RunIsCorrect(heading.Contents.UnderlyingTsString, 0, "Kscripture Ksection2",
				null, DefaultVernWs);

			// This scripture section heading has no BT
			Assert.AreEqual(0, heading.TranslationsOC.Count);
			...
			para = (StTxtPara)section2.ContentOA.ParagraphsOS[0]; //first para
			Assert.AreEqual(StyleUtils.ParaStyleTextProps("Paragraph"), para.StyleRules);
			Assert.AreEqual(expectedParaRunCount, para.Contents.UnderlyingTsString.RunCount);
			tssPara = para.Contents.UnderlyingTsString;
			...
			para = (StTxtPara)section2.ContentOA.ParagraphsOS[1]; //first para
			Assert.AreEqual(StyleUtils.ParaStyleTextProps("Paragraph"), para.StyleRules);
			Assert.AreEqual(1, para.Contents.UnderlyingTsString.RunCount);
			tssPara = para.Contents.UnderlyingTsString;
		...
		public void VerseNumbersRepeatedInBackTrans()
		{
			...
			StTxtPara heading = (StTxtPara)section.HeadingOA.ParagraphsOS[0];
			Assert.AreEqual(StyleUtils.ParaStyleTextProps("Section Head"), heading.StyleRules);
			Assert.AreEqual(1, heading.Contents.UnderlyingTsString.RunCount);
			AssertEx.RunIsCorrect(heading.Contents.UnderlyingTsString, 0, "Kscripture Ksection",
				null, DefaultVernWs);
			...
			Assert.AreEqual(1, heading.TranslationsOC.Count);
			ICmTranslation transl = CmTranslation.CreateFromDBObject(Cache, heading.TranslationsOC.HvoArray[0]);
			Assert.AreEqual(LangProject.kguidTranBackTranslation, transl.TypeRA.Guid);
			ITsString tss = transl.Translation.AnalysisDefaultWritingSystem.UnderlyingTsString;
			Assert.AreEqual(1, tss.RunCount);
			AssertEx.RunIsCorrect(tss, 0, "Scripture Section", null, m_wsAnal);
			...
			// verify completed section head was added to DB (for 1:1-3)
			book = (ScrBook)ScrBook.CreateFromDBObject(Cache, m_importer.ScrBook.Hvo, true); //refresh cache
			...
			StTxtPara para = (StTxtPara)section.ContentOA.ParagraphsOS[0]; //first para
			Assert.AreEqual(StyleUtils.ParaStyleTextProps("Paragraph"), para.StyleRules);
			Assert.AreEqual(expectedParaRunCount, para.Contents.UnderlyingTsString.RunCount);
			ITsString tssPara = para.Contents.UnderlyingTsString;
			AssertEx.RunIsCorrect(tssPara, 0, "1", "Chapter Number", DefaultVernWs);
		...
		public void BackTranslationScriptDigits()
		{
			...
			// Check the BT of the first scripture para
			Assert.AreEqual(1, para.TranslationsOC.Count);
			ICmTranslation transl = CmTranslation.CreateFromDBObject(Cache, para.TranslationsOC.HvoArray[0]);
			Assert.AreEqual(LangProject.kguidTranBackTranslation, transl.TypeRA.Guid);
			ITsString tss = transl.Translation.AnalysisDefaultWritingSystem.UnderlyingTsString;
			Assert.AreEqual(expectedParaRunCount, tss.RunCount);
			AssertEx.RunIsCorrect(tss, 0, "1", "Chapter Number", m_wsAnal);
		...
		public void BackTranslationFootnotes_ToolboxExportFormatBt()
		{
			...
			ITsString tssVernFoot = footnotePara.Contents.UnderlyingTsString;
			...
			// Verify BT text
			ICmTranslation trans = para.GetBT();
			ITsString tssBT = trans.Translation.AnalysisDefaultWritingSystem.UnderlyingTsString;
			...
			ITsString tssFootnoteBT = footnoteBT.Translation.GetAlternative(m_wsAnal).UnderlyingTsString;
		...
		public void HandleToolboxStylePictures_AllMarkersPresent_InterleavedBT()
		{
			...
			using (DummyFileMaker filemaker = new DummyFileMaker("junk.jpg", true))
			{
				...
				int hvoPic = Cache.GetIdFromGuid(guid);
				ICmPicture picture = new CmPicture(Cache, hvoPic);
				try
				{
					...
					Assert.AreEqual("Dibujo del bebe Moises en una canasta", picture.Description.GetAlternative(
		...
		public void BackTranslationFootnotes_BtvtAndBtfAfterVern()
		{
			...
			ICmTranslation transl = CmTranslation.CreateFromDBObject(Cache, para.TranslationsOC.HvoArray[0]);
			Assert.AreEqual(LangProject.kguidTranBackTranslation, transl.TypeRA.Guid);
			ITsString tssBT = transl.Translation.AnalysisDefaultWritingSystem.UnderlyingTsString;
			...
			ICmTranslation footnoteBT = CmTranslation.CreateFromDBObject(Cache, footnotePara.TranslationsOC.HvoArray[0]);
			Assert.AreEqual(LangProject.kguidTranBackTranslation, footnoteBT.TypeRA.Guid);
			ITsString tssFootnoteBT = footnoteBT.Translation.GetAlternative(m_wsAnal).UnderlyingTsString;
			...
			footnote = GetFootnote(1);
			footnotePara = (StTxtPara)footnote.ParagraphsOS[0];
			...
			para = (StTxtPara)section.ContentOA.ParagraphsOS[1]; // second para
			Assert.AreEqual(StyleUtils.ParaStyleTextProps("Paragraph"), para.StyleRules);
			Assert.AreEqual(expectedPara2RunCount, para.Contents.UnderlyingTsString.RunCount);
			tssPara = para.Contents.UnderlyingTsString;
			...
			transl = CmTranslation.CreateFromDBObject(Cache, para.TranslationsOC.HvoArray[0]);
			Assert.AreEqual(LangProject.kguidTranBackTranslation, transl.TypeRA.Guid);
			tssBT = transl.Translation.AnalysisDefaultWritingSystem.UnderlyingTsString;
			...
			tssBT = transl.Translation.GetAlternative(wsGerman).UnderlyingTsString;
			...
			footnote = GetFootnote(2);
			footnotePara = (StTxtPara)footnote.ParagraphsOS[0];
			Assert.AreEqual(1, footnotePara.TranslationsOC.Count);
			footnoteBT = CmTranslation.CreateFromDBObject(Cache, footnotePara.TranslationsOC.HvoArray[0]);
			Assert.AreEqual(LangProject.kguidTranBackTranslation, footnoteBT.TypeRA.Guid);
			tssFootnoteBT = footnoteBT.Translation.GetAlternative(m_wsAnal).UnderlyingTsString;
			Assert.AreEqual(3, tssFootnoteBT.RunCount);
			// Check all the runs
			...
			tssFootnoteBT = footnoteBT.Translation.GetAlternative(wsGerman).UnderlyingTsString;
		...
		public void BackTranslationFootnotes_FootnoteWithFollowingVerseText()
		{
			...
			StTxtPara para = (StTxtPara)section.ContentOA.ParagraphsOS[0]; //first para
			Assert.AreEqual(StyleUtils.ParaStyleTextProps("Paragraph"), para.StyleRules);
			Assert.AreEqual(5, para.Contents.UnderlyingTsString.RunCount);
			ITsString tssPara = para.Contents.UnderlyingTsString;
			AssertEx.RunIsCorrect(tssPara, 0, "1", "Chapter Number", DefaultVernWs);
			AssertEx.RunIsCorrect(tssPara, 1, "1", "Verse Number", DefaultVernWs);
		...
		public void BackTranslationFootnotes_FootnoteWithFootnoteTextAfterVerseText()
		{
			...
			StTxtPara para = (StTxtPara)section.ContentOA.ParagraphsOS[0]; //first para
			Assert.AreEqual(StyleUtils.ParaStyleTextProps("Paragraph"), para.StyleRules);
			Assert.AreEqual(4, para.Contents.UnderlyingTsString.RunCount);
			ITsString tssPara = para.Contents.UnderlyingTsString;
			AssertEx.RunIsCorrect(tssPara, 0, "1", "Chapter Number", DefaultVernWs);
			AssertEx.RunIsCorrect(tssPara, 1, "1", "Verse Number", DefaultVernWs);
		...
		public void BackTranslationFootnotes_ExplicitBtMarkerInterveningText()
		{
			...
			StTxtPara para = (StTxtPara)section.ContentOA.ParagraphsOS[0]; //first para
			Assert.AreEqual(StyleUtils.ParaStyleTextProps("Paragraph"), para.StyleRules);
			Assert.AreEqual(6, para.Contents.UnderlyingTsString.RunCount);
			ITsString tssPara = para.Contents.UnderlyingTsString;
			AssertEx.RunIsCorrect(tssPara, 0, "1", "Chapter Number", DefaultVernWs);
			AssertEx.RunIsCorrect(tssPara, 1, "1", "Verse Number", DefaultVernWs);
			AssertEx.RunIsCorrect(tssPara, 2, "meri peheli ayat", null, DefaultVernWs);
		...
		public void BackTransWithIntraParaChapterNum()
		{
			...
			IScrBook book = new ScrBook(Cache, m_importer.UndoInfo.ImportedSavedVersion.BooksOS[0].Hvo);
			IScrSection section = book.SectionsOS[0];
			Assert.AreEqual(1, section.ContentOA.ParagraphsOS.Count);
			...
			ICmTranslation trans = para.GetBT();
			TsStringAccessor bt = trans.Translation.AnalysisDefaultWritingSystem;
			Assert.AreEqual("1one 2two", bt.Text);
			for (int i = 0; i < bt.UnderlyingTsString.RunCount; i++)
			{
				string s = bt.UnderlyingTsString.get_RunText(i);
				Assert.IsTrue("" != s);
			}
			Assert.AreEqual(4, bt.UnderlyingTsString.RunCount);
			Assert.AreEqual("2", bt.UnderlyingTsString.get_RunText(2));
		...
		public void BackTransWithNoExplicitBTPara()
		{
			...
			ICmTranslation trans = para.GetBT();
			TsStringAccessor bt = trans.Translation.AnalysisDefaultWritingSystem;
			Assert.AreEqual("11one", bt.Text);
			Assert.AreEqual(3, bt.UnderlyingTsString.RunCount);
			ITsString tssBt = bt.UnderlyingTsString;
			AssertEx.RunIsCorrect(tssBt, 0, "1", ScrStyleNames.ChapterNumber, m_wsAnal);
			AssertEx.RunIsCorrect(tssBt, 1, "1", ScrStyleNames.VerseNumber, m_wsAnal);
			AssertEx.RunIsCorrect(tssBt, 2, "one", null, m_wsAnal);
		}
		...
		public void BackTransWithVernWords()
		{
			...
			TsStringAccessor bt = trans.Translation.AnalysisDefaultWritingSystem;
			Assert.AreEqual("El king va a su hogar", bt.Text);
			Assert.AreEqual(3, bt.UnderlyingTsString.RunCount);
			ITsString tssBt = bt.UnderlyingTsString;
			AssertEx.RunIsCorrect(tssBt, 0, "El ", null, m_wsAnal);
			AssertEx.RunIsCorrect(tssBt, 1, "king", "Untranslated Word", m_wsVern);
			AssertEx.RunIsCorrect(tssBt, 2, " va a su hogar", null, m_wsAnal);

			StTxtPara paraContent = (StTxtPara)section.ContentOA.ParagraphsOS[0];
			...
			Assert.AreEqual("11one", bt.Text);
			Assert.AreEqual(3, bt.UnderlyingTsString.RunCount);
			tssBt = bt.UnderlyingTsString;
			AssertEx.RunIsCorrect(tssBt, 0, "1", ScrStyleNames.ChapterNumber, m_wsAnal);
			AssertEx.RunIsCorrect(tssBt, 1, "1", ScrStyleNames.VerseNumber, m_wsAnal);
			AssertEx.RunIsCorrect(tssBt, 2, "one", "Untranslated Word", m_wsVern);
		}
		...
		public void DiffWSCharStyle()
		{
			...
			TsStringAccessor bt = trans.Translation.AnalysisDefaultWritingSystem;
			Assert.AreEqual("11this is my text with a German word", bt.Text);
			Assert.AreEqual(5, bt.UnderlyingTsString.RunCount);
			ITsString tssBt = bt.UnderlyingTsString;
			AssertEx.RunIsCorrect(tssBt, 0, "1", ScrStyleNames.ChapterNumber, m_wsAnal);
			AssertEx.RunIsCorrect(tssBt, 1, "1", ScrStyleNames.VerseNumber, m_wsAnal);
		...
		public void TwoWithDiffCharStyle()
		{
			...
			StTxtPara paraContent = (StTxtPara)section.ContentOA.ParagraphsOS[0];
			ICmTranslation trans = paraContent.GetBT();
			TsStringAccessor bt_de = trans.Translation.GetAlternative(ws_de);
			ITsString tssBt = bt_de.UnderlyingTsString;
			Assert.AreEqual("11This is my text with no Spanish words.", bt_de.Text);
			Assert.AreEqual(3, bt_de.UnderlyingTsString.RunCount);
			AssertEx.RunIsCorrect(tssBt, 0, "1", ScrStyleNames.ChapterNumber, ws_de);
			AssertEx.RunIsCorrect(tssBt, 1, "1", ScrStyleNames.VerseNumber, ws_de);
			AssertEx.RunIsCorrect(tssBt, 2, "This is my text with no Spanish words.", null, ws_de);

			TsStringAccessor bt_es = trans.Translation.GetAlternative(ws_es);
			Assert.AreEqual("11Hi, I'm a Spanish word.", bt_es.Text);
			Assert.AreEqual(4, bt_es.UnderlyingTsString.RunCount);
			tssBt = bt_es.UnderlyingTsString;
			AssertEx.RunIsCorrect(tssBt, 0, "1", ScrStyleNames.ChapterNumber, ws_es);
			AssertEx.RunIsCorrect(tssBt, 1, "1", ScrStyleNames.VerseNumber, ws_es);
			AssertEx.RunIsCorrect(tssBt, 2, "Hi, I'm a Spanish ", "Emphasis", ws_es);
			AssertEx.RunIsCorrect(tssBt, 3, "word.", null, ws_es);
		}
		...
		[ExpectedException(typeof(ScriptureUtilsException),
			ExpectedMessage = "Back translation does not correspond to the " +
				"preceding vernacular paragraph:\r\n\t\\btp B \r\nThe style for a back translation paragraph must " +
				"match the style for the vernacular paragraph. A back translation paragraph must " +
				"belong to the immediately preceding vernacular paragraph.\r\nThe style \"Paragraph\" " +
				"does not match the vernacular paragraph style \"Title Main\".\r\nAttempting to read EXO")]
		...
		public void OnlyBT()
		{
			...
			m_scrInMemoryCache.AddRunToMockedPara(para1, "verse 2", null);
			m_scrInMemoryCache.AddRunToMockedPara(para1, "3", ScrStyleNames.VerseNumber);
			m_scrInMemoryCache.AddRunToMockedPara(para1, "verse 3", null);
			...
			Assert.AreNotEqual(book.Hvo, m_importer.UndoInfo.BackupVersion.BooksOS.HvoArray[0]);
		...
		public void OnlyBT_WithOneFootnote()
		{
			...
			ScrSection section = (ScrSection)book.SectionsOS[0];
			StTxtPara para1 = (StTxtPara)section.ContentOA.ParagraphsOS[0];
			StFootnote footnote = m_scrInMemoryCache.AddFootnote(book, para1, 6, "Kfootnote Ktext");
			section.AdjustReferences();
			...
			Assert.AreEqual("footnote text",
				((StTxtPara)footnote.ParagraphsOS[0]).GetBT().Translation.GetAlternativeTss(m_wsAnal).Text);
		}
		...
		public void OnlyBT_SecondaryTitle()
		{
			...
			ITsTextProps secondaryProps = propFact.MakeProps(ScrStyleNames.SecondaryBookTitle,
				m_wsVern, 0);
			string mainTitle = "Joshua";
			bldr.ReplaceRgch(0, 0, mainTitle, mainTitle.Length, null);
			string secondaryTitle = "The adventures of";
			bldr.ReplaceRgch(0, 0, secondaryTitle, secondaryTitle.Length, secondaryProps);
			ITsTextProps titleProps = propFact.MakeProps(ScrStyleNames.MainBookTitle,
				m_wsVern, 0);
			StText titleText = m_scrInMemoryCache.AddTitleToMockedBook(book.Hvo, bldr.GetString(),
				titleProps);
			...
			ICmTranslation titleTrans = ((StTxtPara)titleText.ParagraphsOS[0]).GetBT();
			AssertEx.RunIsCorrect(titleTrans.Translation.GetAlternative(m_wsAnal).UnderlyingTsString,
		...
		public void OnlyBT_MultipleFootnotes()
		{
			...
			StFootnote footnote1 = m_scrInMemoryCache.AddFootnote(book, para1, origLength + 6,
				"Kfootnote Ktext");
			// Insert a footnote after the "2" in verse 2
			StFootnote footnote2 = m_scrInMemoryCache.AddFootnote(book, para1, origLength + 9,
				"Ksecond Kfootnote");
			section.AdjustReferences();
			...
			Assert.AreEqual("BT footnote text",
				((StTxtPara)footnote1.ParagraphsOS[0]).GetBT().Translation.GetAlternativeTss(m_wsAnal).Text);
			...
			Assert.AreEqual("BT another footnote",
				((StTxtPara)footnote2.ParagraphsOS[0]).GetBT().Translation.GetAlternativeTss(m_wsAnal).Text);
		}
		...
		public void OnlyBT_IntroPara()
		{
			...
			StTxtPara titlePara = (StTxtPara)m_scrInMemoryCache.AddTitleToMockedBook(book.Hvo, "Exodo").ParagraphsOS[0];
			ITsString tssTitleOrig = titlePara.Contents.UnderlyingTsString;
			...
			m_scrInMemoryCache.AddRunToMockedPara(para, "Que bueno que usted quiere leer este libro.", null);
			...
			Assert.AreEqual(titlePara.Hvo, book.TitleOA.ParagraphsOS.HvoArray[0]);
			AssertEx.AreTsStringsEqual(tssTitleOrig, titlePara.Contents.UnderlyingTsString);
			...
			Assert.AreEqual(1, book.SectionsOS.Count);
			Assert.AreEqual(introSection.Hvo, book.SectionsOS.HvoArray[0]);

			// Verify that the text of the intro para was not changed
			Assert.AreEqual(1, introSection.ContentOA.ParagraphsOS.Count);
			Assert.AreEqual(para.Hvo, introSection.ContentOA.ParagraphsOS.HvoArray[0]);
		...
		public void OnlyBT_MinorSectionHead()
		{
			...
			IScrSection sectionNormal = m_scrInMemoryCache.AddSectionToMockedBook(book.Hvo);
			m_scrInMemoryCache.AddParaToMockedText(sectionNormal.HeadingOAHvo,
				StyleUtils.ParaStyleTextProps(ScrStyleNames.SectionHead));
			StTxtPara paraC1 = m_scrInMemoryCache.AddParaToMockedSectionContent(sectionNormal.Hvo,
			...
			m_scrInMemoryCache.AddRunToMockedPara(paraC1, "Se levanto otro faraon que no tenia buen gusto.", null);
			sectionNormal.AdjustReferences();

			IScrSection sectionMinor = m_scrInMemoryCache.AddSectionToMockedBook(book.Hvo);
			m_scrInMemoryCache.AddParaToMockedText(sectionMinor.HeadingOAHvo,
				StyleUtils.ParaStyleTextProps("Section Head Minor"));
			StTxtPara paraCMinor1 = m_scrInMemoryCache.AddParaToMockedSectionContent(sectionMinor.Hvo,
				ScrStyleNames.NormalParagraph);
			m_scrInMemoryCache.AddRunToMockedPara(paraCMinor1, "2", ScrStyleNames.VerseNumber);
			m_scrInMemoryCache.AddRunToMockedPara(paraCMinor1, "Los israelitas tenian una vida miserable.", null);
			sectionMinor.AdjustReferences();
			...
			Assert.AreEqual(2, book.SectionsOS.Count);
			Assert.AreEqual(sectionNormal.Hvo, book.SectionsOS.HvoArray[0]);
			StTxtPara heading = (StTxtPara)sectionNormal.HeadingOA.ParagraphsOS[0];
			...
			StTxtPara para = (StTxtPara)sectionNormal.ContentOA.ParagraphsOS[0]; //first para
			...
			Assert.AreEqual(sectionMinor.Hvo, book.SectionsOS.HvoArray[1]);
			heading = (StTxtPara)sectionMinor.HeadingOA.ParagraphsOS[0];
			...
			para = (StTxtPara)sectionMinor.ContentOA.ParagraphsOS[0]; //first para
		...
		public void VerseInMultipleParagraphs()
		{
			...
			m_scrInMemoryCache.AddSectionHeadParaToSection(section2.Hvo, "Segunda Seccion",
				ScrStyleNames.SectionHead);
			...
			m_scrInMemoryCache.AddRunToMockedPara(para21, "Segunda versiculo", null);
			StTxtPara para22 = m_scrInMemoryCache.AddParaToMockedSectionContent(section2.Hvo, "Line1");
			m_scrInMemoryCache.AddRunToMockedPara(para22, "Segunda estrofa", null);
			StTxtPara para23 = m_scrInMemoryCache.AddParaToMockedSectionContent(section2.Hvo, "Line2");
			m_scrInMemoryCache.AddRunToMockedPara(para23, "Dritte Strophe", null);
			StTxtPara para24 = m_scrInMemoryCache.AddParaToMockedSectionContent(section2.Hvo, "Line2");
			m_scrInMemoryCache.AddRunToMockedPara(para24, "Vierte Strophe", null);
