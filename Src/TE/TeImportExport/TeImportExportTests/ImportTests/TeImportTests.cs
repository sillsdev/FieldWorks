...
namespace SIL.FieldWorks.TE.ImportTests
{
	...
	public class DummyTeImporter : TeSfmImporter
	{
		...
		/// <summary></summary>
		public static CmAnnotationDefn s_consultantNoteDefn = null;
		/// <summary></summary>
		public static CmAnnotationDefn s_translatorNoteDefn = null;
		...
		/// <param name="settings">The settings object to be populated</param>
		/// ------------------------------------------------------------------------------------
		static public void MakeParatextTestSettings(ScrImportSet settings)
		{
		...
		/// </summary>
		/// ------------------------------------------------------------------------------------
		public StFootnote CurrentFootnote
		{
			get
			{
				CheckDisposed();
		...
		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Exposes the <see cref="TeImporter.m_hvoTitle"/> variable.
		/// </summary>
		/// ------------------------------------------------------------------------------------
		public int HvoTitle
		{
		...
		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Exposes the <see cref="TeImporter.m_hvoSectionHeading"/> variable.
		/// </summary>
		/// ------------------------------------------------------------------------------------
		public int HvoSectionHeading
		{
			get
			{
				CheckDisposed();
				return m_hvoSectionHeading;
			}
		}

		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Exposes the <see cref="TeImporter.m_hvoSectionContent"/> variable.
		/// </summary>
		/// ------------------------------------------------------------------------------------
		public int HvoSectionContent
		{
			get
			{
				CheckDisposed();
				return m_hvoSectionContent;
			}
		}
		...
		public IStFootnote GetFootnote(int iFootnoteIndex)
		{
			...
			// force book to reload
			IScrBook book = new ScrBook(m_cache, ScrBook.Hvo);
		...
		/// </returns>
		/// ------------------------------------------------------------------------------------
		public StFootnote FindCorrespondingFootnote(int ws, string styleId)
		{
			CheckDisposed();
			...
			ImportDomain prevImportDomain, bool fInScriptureText)
		{
			...
			m_hvoSectionHeading = section.HeadingOAHvo;
			m_hvoSectionContent = section.ContentOAHvo;
	...
	public class DummyUndoImportManager : UndoImportManager
	{
		...
		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Override for tests that use the in-memory cache.
		/// </summary>
		...
		/// <returns></returns>
		/// ------------------------------------------------------------------------------------
		protected override IScrBook CreateNewScrBook(int bookNum, out int hvoTitle)
		{
			if (m_inMemoryCache == null)
				return base.CreateNewScrBook(bookNum, out hvoTitle);
			IScrBook book = m_inMemoryCache.AddBookToMockedScripture(bookNum, string.Empty);
			StText title = m_inMemoryCache.AddTitleToMockedBook(book.Hvo, string.Empty);
			hvoTitle = title.Hvo;
			return book;
		}
	}
	...
	#region DummyScrImportSet
	class DummyScrImportSet : ScrImportSet
	{
		/// ------------------------------------------------------------------------------------
		/// <summary>
		...
		/// ------------------------------------------------------------------------------------
		public override string ParatextScrProj
		{
			get { return base.ParatextScrProj; }
			set
			{
				m_ParatextScrProject = (value == string.Empty) ? null : value;
			}
		}
		...
		/// <remarks>Setter has side-effect of loading the mappings</remarks>
		/// ------------------------------------------------------------------------------------
		public override string ParatextBTProj
		{
			get { return base.ParatextBTProj; }
			set
			{
				m_ParatextBTProject = (value == string.Empty) ? null : value;
			}
		}
		...
		/// <remarks>Setter has side-effect of loading the mappings</remarks>
		/// ------------------------------------------------------------------------------------
		public override string ParatextNotesProj
		{
			get { return base.ParatextNotesProj; }
			set
			{
				m_ParatextNotesProject = (value == string.Empty) ? null : value;
			}
		}
	}
	#endregion
	...
	public class TeImportTestInMemory : TeImportTestsBase
	{
		...
		public void FindCorrespondingVernParaForSegment()
		{
			...
			int hvoLastPara =
				lastSection.ContentOA.ParagraphsOS.HvoArray[lastSection.ContentOA.ParagraphsOS.Count - 1];
		...
		{
			CheckDisposed();
			Assert.AreEqual(@"P0|c:\temp\mypic.jpg|P2|P3|P4",
		...
		/// SOWrapper.ExternalPictureFolders.
		/// </summary>
		/// <exception cref="NUnit.Framework.IgnoreException"> Thrown if the My Pictures folder
		/// is not set.
		/// </exception>
		/// ------------------------------------------------------------------------------------
		[Test]
		...
		public void EnsurePictureFilePathIsRooted_NotRooted_NotFoundInExternalFolders()
		{
			...
			foreach (string sFolder in sow.ExternalPictureFolders)
			{
				...
				Assert.IsFalse(File.Exists(sPath), "Test is invalid because " + sPath + "exists.");
			}
		...
		public void ProcessSegment_UseMappedLanguage()
		{
			...
			Assert.AreEqual(1, m_importer.NormalParaStrBldr.RunCount);
			int wsExpected = Cache.LanguageEncodings.GetWsFromIcuLocale("xkal");
		...
		public void ProcessSegmentBasic()
		{
			...
			Assert.AreEqual("Main Title!", m_importer.ScrBook.Name.VernacularDefaultWritingSystem);
		...
		public void ProcessSegmentAdvanced()
		{
			...
			IScrBook book = (IScrBook)CmObject.CreateFromDBObject(Cache, m_importer.ScrBook.Hvo, true); //refresh cache
			Assert.AreEqual("This is a header", book.Name.VernacularDefaultWritingSystem);
			...
			Assert.AreEqual("This is a header", m_importer.ScrBook.Name.VernacularDefaultWritingSystem);
			book = (ScrBook)ScrBook.CreateFromDBObject(Cache, m_importer.ScrBook.Hvo, true); //refresh cache
			Assert.AreEqual(0, book.SectionsOS.Count);
			...
			StTxtPara title = (StTxtPara)book.TitleOA.ParagraphsOS[0];
			Assert.AreEqual(StyleUtils.ParaStyleTextProps(ScrStyleNames.MainBookTitle), title.StyleRules);
			Assert.AreEqual(3, title.Contents.UnderlyingTsString.RunCount);
			AssertEx.RunIsCorrect(title.Contents.UnderlyingTsString, 0, "The Gospel According to", "Title Secondary", DefaultVernWs);
			string sBrkChar = new string((char)0x2028, 1);
			AssertEx.RunIsCorrect(title.Contents.UnderlyingTsString, 1, sBrkChar + "Waldo" + sBrkChar, null, DefaultVernWs);
			AssertEx.RunIsCorrect(title.Contents.UnderlyingTsString, 2, "Dude!", "Title Tertiary", DefaultVernWs);
			...
			Assert.AreEqual(StyleUtils.ParaStyleTextProps("Section Head"), heading.StyleRules);
			Assert.AreEqual(1, heading.Contents.UnderlyingTsString.RunCount);
			AssertEx.RunIsCorrect(heading.Contents.UnderlyingTsString, 0, "Section Head One", null, DefaultVernWs);
			...
			StTxtPara para = (StTxtPara)prevSection.ContentOA.ParagraphsOS[0]; //first para
			Assert.AreEqual(StyleUtils.ParaStyleTextProps("Line1"), para.StyleRules);
			Assert.AreEqual(3, para.Contents.UnderlyingTsString.RunCount);
			AssertEx.RunIsCorrect(para.Contents.UnderlyingTsString, 0, "1", "Chapter Number", DefaultVernWs);
			AssertEx.RunIsCorrect(para.Contents.UnderlyingTsString, 1, "5-10", "Verse Number", DefaultVernWs);
			AssertEx.RunIsCorrect(para.Contents.UnderlyingTsString, 2, "verse five to ten text", null, DefaultVernWs);
			// verify refs of completed scripture text section (2:5-10)
		...
		public void ProcessHugeParagraphs_SplitAtChapterBreak()
		{
			...
			IScrBook book = (IScrBook)CmObject.CreateFromDBObject(Cache, m_importer.ScrBook.Hvo, true);
			IScrSection section = book.SectionsOS[0];
			...
			Assert.AreEqual(StyleUtils.ParaStyleTextProps("Line1"), para.StyleRules);
			Assert.AreEqual(13, para.Contents.UnderlyingTsString.RunCount);
			AssertEx.RunIsCorrect(para.Contents.UnderlyingTsString, 0, "1", "Chapter Number", DefaultVernWs);
			for (int verse = 1; verse <= 6; verse++)
			{
				AssertEx.RunIsCorrect(para.Contents.UnderlyingTsString, verse * 2 - 1, verse.ToString(), "Verse Number", DefaultVernWs);
				AssertEx.RunIsCorrect(para.Contents.UnderlyingTsString, verse * 2, sContents, null, DefaultVernWs);
			}
		...
		public void ProcessHugeParagraphs_SplitAtVerseBreak()
		{
			...
			StTxtPara para = (StTxtPara)section.ContentOA.ParagraphsOS[0]; //first para
			Assert.AreEqual(StyleUtils.ParaStyleTextProps("Line1"), para.StyleRules);
			Assert.AreEqual(41, para.Contents.UnderlyingTsString.RunCount);
			AssertEx.RunIsCorrect(para.Contents.UnderlyingTsString, 0, "1", "Chapter Number", DefaultVernWs);
		...
		public void ProcessHugeParagraphs_SplitAtPunctuation()
		{
			...
			Assert.AreEqual(2, para.Contents.UnderlyingTsString.RunCount);
			AssertEx.RunIsCorrect(para.Contents.UnderlyingTsString, 0, "1", "Chapter Number", DefaultVernWs);
			AssertEx.RunIsCorrect(para.Contents.UnderlyingTsString, 1, sPara1, null, DefaultVernWs);
		...
		[ExpectedException(typeof(ScriptureUtilsException),
		 ExpectedMessage = "Book introduction within Scripture text.\r\n\r\n\\ip Bad intro para\r\nAttempting to read EXO  Chapter: 1  Verse: 1")]
		...
		/// ------------------------------------------------------------------------------------
		[Test]
		[ExpectedException(typeof(ScriptureUtilsException), ExpectedMessage = "Book introduction within Scripture text.\r\n\r\n\\is B\r\nAttempting to read EXO  Chapter: 1  Verse: 1")]
		...
		public void DetectUnmappedMarkersInImport()
		{
			...
			Assert.AreEqual(StyleUtils.ParaStyleTextProps("Intro Section Head"),
				heading.StyleRules);
			Assert.AreEqual(1, heading.Contents.UnderlyingTsString.RunCount);
			AssertEx.RunIsCorrect(heading.Contents.UnderlyingTsString, 0, "Intro Section Head", null, DefaultVernWs);
			...
			Assert.AreEqual(1, book.SectionsOS[0].ContentOA.ParagraphsOS.Count);
			StTxtPara content = (StTxtPara)book.SectionsOS[0].ContentOA.ParagraphsOS[0];
			Assert.AreEqual(StyleUtils.ParaStyleTextProps("\\ipnew"), content.StyleRules);
			Assert.AreEqual(1, content.Contents.UnderlyingTsString.RunCount);
			AssertEx.RunIsCorrect(content.Contents.UnderlyingTsString, 0, "Intro paragraph text", null, DefaultVernWs);
			...
			foreach (StStyle style in m_scr.StylesOC)
			{
			...
			//IScrBook book = (IScrBook)CmObject.CreateFromDBObject(Cache, m_importer.ScrBook.Hvo, true); //refresh cache
			Assert.AreEqual(1, book.SectionsOS[1].ContentOA.ParagraphsOS.Count);
			content = (StTxtPara)book.SectionsOS[1].ContentOA.ParagraphsOS[0];
			Assert.AreEqual(StyleUtils.ParaStyleTextProps("Paragraph"), content.StyleRules);
			Assert.AreEqual(3, content.Contents.UnderlyingTsString.RunCount);
			...
			//IScrBook book = (IScrBook)CmObject.CreateFromDBObject(Cache, m_importer.ScrBook.Hvo, true); //refresh cache
			Assert.AreEqual(2, book.SectionsOS[1].ContentOA.ParagraphsOS.Count);
			content = (StTxtPara)book.SectionsOS[1].ContentOA.ParagraphsOS[1];
...
//			VerifyBldrRun(0, "1", "Chapter Number");
		...
		public void BookTitle_EmptyPara()
		{
			...
			// ************** process an intro section head, test MakeSection() method ************
			m_importer.ProcessSegment("Background Material", @"\is");
			Assert.AreEqual(null, m_importer.ScrBook.Name.VernacularDefaultWritingSystem);
			Assert.AreEqual(1, m_importer.ScrBook.TitleOA.ParagraphsOS.Count);
			Assert.AreEqual(2, m_importer.BookNumber);
			Assert.IsNotNull(m_importer.CurrentSection);
		}
		...
		public void ProcessSegment04_CharStyles()
		{
			...
			StText text = new StText(Cache, m_importer.HvoSectionContent);
			StTxtPara para = (StTxtPara)text.ParagraphsOS[text.ParagraphsOS.Count - 1];
			Assert.AreEqual(StyleUtils.ParaStyleTextProps("Paragraph"), para.StyleRules);
			Assert.AreEqual(1, para.Contents.UnderlyingTsString.RunCount);
			AssertEx.RunIsCorrect(para.Contents.UnderlyingTsString, 0, "text with char style", "Key Word", DefaultVernWs);
			// verify state of NormalParaStrBldr
			Assert.AreEqual(1, m_importer.NormalParaStrBldr.RunCount);
			...
			para = (StTxtPara)text.ParagraphsOS[text.ParagraphsOS.Count - 1];
			Assert.AreEqual(StyleUtils.ParaStyleTextProps("Paragraph"), para.StyleRules);
			Assert.AreEqual(1, para.Contents.UnderlyingTsString.RunCount);
			AssertEx.RunIsCorrect(para.Contents.UnderlyingTsString, 0, "text with char style", "Key Word", DefaultVernWs);
			// verify state of NormalParaStrBldr
			...
			m_importer.ProcessSegment("", @"\p");
			// verify the first paragraph, from the db
			text = new StText(Cache, m_importer.HvoSectionContent);
			para = (StTxtPara)text.ParagraphsOS[text.ParagraphsOS.Count - 1];
			Assert.AreEqual(StyleUtils.ParaStyleTextProps("Paragraph"), para.StyleRules);
			Assert.AreEqual(1, para.Contents.UnderlyingTsString.RunCount);
			AssertEx.RunIsCorrect(para.Contents.UnderlyingTsString, 0, "text with char style", "Key Word", DefaultVernWs);
		...
		public void ProcessSegment05_Footnotes()
		{
			...
			IStFootnote footnote = GetFootnote(iFootnoteIndex);
			AssertEx.RunIsCorrect(footnote.FootnoteMarker.UnderlyingTsString, 0,
				"a", ScrStyleNames.FootnoteMarker, m_wsVern);
			FdoOwningSequence<IStPara> footnoteParas = footnote.ParagraphsOS;
			Assert.AreEqual(1, footnoteParas.Count);
			para = (StTxtPara)footnote.ParagraphsOS[0];
			Assert.AreEqual
				(StyleUtils.ParaStyleTextProps(ScrStyleNames.NormalFootnoteParagraph),
				para.StyleRules);
			Assert.AreEqual(3, para.Contents.UnderlyingTsString.RunCount);
			AssertEx.RunIsCorrect(((StTxtPara)footnoteParas[0]).Contents.UnderlyingTsString, 0,
				"beginning of footnote", null, m_wsVern);
			AssertEx.RunIsCorrect(((StTxtPara)footnoteParas[0]).Contents.UnderlyingTsString, 1,
				"text with char style", "Key Word", m_wsVern);
			AssertEx.RunIsCorrect(((StTxtPara)footnoteParas[0]).Contents.UnderlyingTsString, 2,
		...
		public void ProcessSegment05_FootnoteOnSectionWithBT()
		{
			...
			mark = m_importer.UndoInfo.ImportedSavedVersion.FindBook(41);
			...
			Assert.AreEqual("footnote1", para.Contents.UnderlyingTsString.Text);

			// verify the section content text
			para = (StTxtPara)section.ContentOA.ParagraphsOS[0];
			Assert.AreEqual("11verse one text", para.Contents.UnderlyingTsString.Text);

			// verify the section head text
			para = (StTxtPara)section.HeadingOA.ParagraphsOS[0];
			Assert.AreEqual("section" + StringUtils.kchObject, para.Contents.UnderlyingTsString.Text);

			// verify the BT text
			Assert.AreEqual(1, para.TranslationsOC.Count);
			ICmTranslation trans = para.GetBT();
			Assert.AreEqual("BT for section", trans.Translation.AnalysisDefaultWritingSystem.UnderlyingTsString.Text);
		}
		...
		public void ProcessSegment05_FootnoteFollowedByVerseText()
		{
			...
			Assert.AreEqual(1, footnote.ParagraphsOS.Count);
			IStTxtPara para = (IStTxtPara)footnote.ParagraphsOS[0];
			Assert.AreEqual("this is a footnote", para.Contents.UnderlyingTsString.Text);
			...
			Assert.AreEqual("11verse one text." + StringUtils.kchObject + "some more verse text emphasis done", para.Contents.UnderlyingTsString.Text);
		}
		#endregion
		...
		public void ProcessSegment06_StartWithCharStyle()
		{
			...
			Assert.AreEqual(StyleUtils.ParaStyleTextProps("Intro Paragraph"), para.StyleRules);
			Assert.AreEqual(2, para.Contents.UnderlyingTsString.RunCount);
			AssertEx.RunIsCorrect(para.Contents.UnderlyingTsString, 0, "text", "Quoted Text", DefaultVernWs);
			AssertEx.RunIsCorrect(para.Contents.UnderlyingTsString, 1, " continuation", null, DefaultVernWs);
		}
		#endregion
		...
		public void ProcessSegment07_TextBeforeMainTitle()
		{
			...
			ScrBook book = (ScrBook)ScrBook.CreateFromDBObject(Cache, m_importer.ScrBook.Hvo, true); //refresh cache
			VerifyNewSectionExists(book, 0);
		...
		public void ProcessStanzaBreak()
		{
			...
			StTxtPara stanzaBreakPara = (StTxtPara)section.ContentOA.ParagraphsOS[4];
		...
		public void FinalizePrevSection_HeadingAndContentEmpty()
		{
			...
			IScrBook gen = m_scrInMemoryCache.AddBookToMockedScripture(1, "Genesis");
			IScrSection section = m_scrInMemoryCache.AddIntroSectionToMockedBook(gen.Hvo);
			...
			AssertEx.AreTsStringsEqual(tssExpected, para.Contents.UnderlyingTsString);
		...
		public void SkipIntroMaterial()
		{
			...
			IScrSection section = book.SectionsOS[0];
			para = (StTxtPara)section.HeadingOA.ParagraphsOS[0];
			Assert.AreEqual("My First Section", para.Contents.UnderlyingTsString.Text);

			// Look at the text of the first paragraph in the section
			Assert.AreEqual(1, section.ContentOA.ParagraphsOS.Count);
			para = (StTxtPara)section.ContentOA.ParagraphsOS[0];
			Assert.AreEqual("11Some verse one text. Emphasis more text", para.Contents.UnderlyingTsString.Text);
		}
		#endregion
		...
		public void HandleUSFMStylePicturesPictureMissing()
		{
			...
			m_importer.ProcessSegment("", @"\c");
			m_importer.ProcessSegment("", @"\p");
			m_importer.ProcessSegment(@"User-supplied picture|Q:\the\answer\is\42.jpg" +
			...
			Assert.AreEqual(@"Q:\the\answer\is\42.jpg", picture.PictureFileRA.InternalPath);
		...
		public void HandleToolboxStylePictures_CatAfterCat()
		{
			...
			using (DummyFileMaker filemaker = new DummyFileMaker("junk.jpg", true))
			{
				...
				m_importer.ProcessSegment("", @"\p");
				m_importer.ProcessSegment(filemaker.Filename, @"\cat");
				m_importer.ProcessSegment(@"c:\MissingPicture.jpg", @"\cat");
				m_importer.FinalizeImport();
				IScrBook exodus = m_importer.UndoInfo.ImportedSavedVersion.BooksOS[0];
				IScrSection section = exodus.SectionsOS[0];
				StTxtPara para = (StTxtPara)section.ContentOA.ParagraphsOS[0];
				...
				hvoPic = Cache.GetIdFromGuid(guid);
				picture = new CmPicture(Cache, hvoPic);
				Assert.IsNull(picture.Caption.VernacularDefaultWritingSystem.Text);
				Assert.IsTrue(picture.PictureFileRA.InternalPath == picture.PictureFileRA.AbsoluteInternalPath);
				Assert.AreEqual(@"c:\MissingPicture.jpg", picture.PictureFileRA.InternalPath);
		...
		/// ------------------------------------------------------------------------------------
		[ExpectedException(typeof(ScriptureUtilsException),
		   ExpectedMessage = "Invalid figure file name property\r\n\r\n\\cat InvalidFile|.jpg||\r\nAttempting to read EXO  Chapter: 1  Verse: 1")]
		[Test]
		public void HandleToolboxStylePictures_InvalidFigFilename()
		{
			...
			using (DummyFileMaker filemaker = new DummyFileMaker("junk.jpg", true))
			{
				...
				m_importer.ProcessSegment("", @"\c");
				m_importer.ProcessSegment("", @"\p");
				m_importer.ProcessSegment("InvalidFile|.jpg||", @"\cat");
			}
		}
			...
			using (DummyFileMaker filemaker = new DummyFileMaker("junk.jpg", true))
			{
				...
				try
				{
					...
					Assert.AreEqual(Path.Combine(Path.GetTempPath(), "back translation for junk.jpg"),
						picture.Caption.GetAlternative(DefaultAnalWs).Text);
				}
				finally
				{
					if (picture != null)
					{
						File.Delete(Path.Combine(DirectoryFinder.FWDataDirectory,
		...
		public void TitleShortGetsSetToBookTitle()
		{
			...
			Assert.AreEqual("Exodus", m_importer.ScrBook.Name.VernacularDefaultWritingSystem);
		...
		public void ImportAnnotations_Simple()
		{
			...
			// verify that the verse text of the first scripture para is in the db correctly
			Assert.AreEqual(2, section.ContentOA.ParagraphsOS.Count);
			int paraHvo = section.ContentOA.ParagraphsOS.HvoArray[1];
		...
		public void ImportAnnotations_NonInterleaved_ConsultantNoteFile()
		{
			...
			// make sure that notes will get imported
			m_importer.Settings.ImportAnnotations = true;
			((ScrImportSet)m_importer.Settings).AddFile("dummy.sfm", ImportDomain.Annotations,
				"en", DummyTeImporter.s_consultantNoteDefn.Hvo);
			DummyTeImporter.SetUpMappings((ScrImportSet)m_importer.Settings);
			m_importer.Initialize();
			...
			int hvoBook = m_importer.UndoInfo.ImportedSavedVersion.BooksOS[0].Hvo;

			book = (IScrBook)CmObject.CreateFromDBObject(Cache, hvoBook, true); //refresh cache
			IScrSection section = book.SectionsOS[0];
			// verify that the verse text of the first scripture para is in the db correctly
			Assert.AreEqual(1, section.ContentOA.ParagraphsOS.Count);
			int paraHvo = section.ContentOA.ParagraphsOS.HvoArray[0];
		...
		public void ImportAnnotations_InterleavedInBT()
		{
			...
			IScrScriptureNote note = m_scrInMemoryCache.AddAnnotation(para, 01001001, NoteType.Consultant);
			StTxtPara discussionPara = (StTxtPara)note.DiscussionOA.ParagraphsOS[0];
			m_scrInMemoryCache.AddRunToMockedPara(discussionPara, "Annotation for verse 1 ",
				m_scrInMemoryCache.Cache.DefaultAnalWs);
		...
		/// </summary>
		/// <param name="objhvo">ID of annotated object (usually an StTxtPara)</param>
			...
			int endScrRef, string sText, NoteType type)
		{
			...
			Assert.AreEqual(objhvo, annotation.BeginObjectRAHvo);
			Assert.AreEqual(objhvo, annotation.EndObjectRAHvo);
		...
		public void ImportAnnotations_WithIdenticalAnnotation()
		{
			...
			IScrSection section1 = m_scrInMemoryCache.AddSectionToMockedBook(exodus.Hvo, false);
			StTxtPara para1 = m_scrInMemoryCache.AddParaToMockedSectionContent(section1.Hvo,
				ScrStyleNames.Normal);
			m_scrInMemoryCache.AddRunToMockedPara(para1, "And I ran from Egypt",
			...
			IScrScriptureNote origNote1 = m_scrInMemoryCache.AddAnnotation(para1, new BCVRef(2, 1, 1),
				NoteType.Translator, "This is an annotation.");
			m_scrInMemoryCache.AddRunToMockedPara(para1, "2", ScrStyleNames.VerseNumber);
			m_scrInMemoryCache.AddRunToMockedPara(para1, "More of the same",
				m_scrInMemoryCache.Cache.DefaultVernWs);
			// This annotation should survive the import since it is in a different verse from the
			// annotation being imported.
			IScrScriptureNote origNote2 = m_scrInMemoryCache.AddAnnotation(para1, new BCVRef(2, 1, 2),
				NoteType.Translator, "This is an annotation.");
		...
		public void ImportAnnotations_EmbeddedCharacterRuns()
		{
			...
			FdoOwningSequence<IScrScriptureNote> notes = m_scr.BookAnnotationsOS[1].NotesOS;
			...
			{
				Assert.AreEqual(para.Hvo, annotation.BeginObjectRAHvo);
				Assert.AreEqual(para.Hvo, annotation.EndObjectRAHvo);
				...
				StTxtPara annPara = (StTxtPara)annotation.DiscussionOA.ParagraphsOS[0];
			...
			ITsString tssAnn = ((StTxtPara)note.DiscussionOA.ParagraphsOS[0]).Contents.UnderlyingTsString;
			...
			note = notes[1];
			tssAnn = ((StTxtPara)note.DiscussionOA.ParagraphsOS[0]).Contents.UnderlyingTsString;
