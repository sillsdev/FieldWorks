...
namespace SIL.FieldWorks.FDO.CoreTests.PersistingLayerTests
{
	...
	public abstract class PersistingBackendProviderTestBase : FdoTestBase
	{
		...
		public void BasicDataTypes()
		{
			...
			var xmlOriginalValue = StringUtils.GetXmlRep(irOriginalValue, Cache.WritingSystemFactory, Cache.WritingSystemFactory.UserWs, true);
			...
			var xmlOriginalBlankValue = StringUtils.GetXmlRep(irOriginalBlankValue, Cache.WritingSystemFactory, Cache.WritingSystemFactory.UserWs, true);
			// ITsTextProps
			...
			string xmlOriginalValueRules = StringUtils.GetXmlRep(rOriginalvalue, Cache.WritingSystemFactory);
			...
			StringUtils.GetXmlRep(nameEnValue, Cache.WritingSystemFactory, 0, true);
			// Set LP's Name.
			//lp.Name.set_String(
			//    englishWsHvo,
			//    nameEnValue);
			...
			var xmlRestoredValue = StringUtils.GetXmlRep(irRestoredValue, Cache.WritingSystemFactory, Cache.WritingSystemFactory.UserWs, true);
			...
			var xmlRestoredBlankValue = StringUtils.GetXmlRep(irRestoredBlankValue, Cache.WritingSystemFactory, Cache.WritingSystemFactory.UserWs, true);
			...
			var xmlRestoredValueRules = StringUtils.GetXmlRep(rRestoredValue, Cache.WritingSystemFactory);
	...
	public sealed class XMLTests : PersistingBackendProviderTestBase
	{
		...
		private FdoCache OpenExistingFile(string filename)
		{
			return FdoCache.CreateCacheFromExistingData(
				new TestProjectId(FDOBackendProviderType.kXMLWithMemoryOnlyWsMgr, filename), "en", new ThreadHelper());
		}
				...
				foreach (var targetBackendStartupParameters in m_targetInfo)
				{
					...
					using (FdoCache sourceCache = createSource ? FdoCache.CreateCacheWithNewBlankLangProj(
						...
						FdoCache.CreateCacheFromExistingData(
							new TestProjectId(sourceBackendStartupParameters.ProjectId.Type,
								sourceBackendStartupParameters.ProjectId.Path), "en", threadHelper))
					{
						// BEP is a singleton, so we shouldn't call Dispose on it. This will be done
						// by service locator.
						var sourceDataSetup = GetMainBEPInterface(sourceCache);
					...
					using (var targetCache = FdoCache.CreateCacheWithNoLangProj(
						new TestProjectId(targetBackendStartupParameters.ProjectId.Type, null), "en", threadHelper))
					{
						// BEP is a singleton, so we shouldn't call Dispose on it. This will be done
						// by service locator.
						var targetDataSetup = GetMainBEPInterface(targetCache);
						try
						{
						targetDataSetup.InitializeFromSource(
							new TestProjectId(targetBackendStartupParameters.ProjectId.Type,
									targetBackendStartupParameters.ProjectId.Path),
								sourceBackendStartupParameters, "en", threadHelper);
						}
						catch (Exception)
						{
