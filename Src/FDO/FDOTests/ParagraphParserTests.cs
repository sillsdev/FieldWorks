...
namespace SIL.FieldWorks.FDO.FDOTests
{
	...
	public class InterlinearTestBase : MemoryOnlyBackendProviderRestoredForEachTestTestBase
	{
		...
		internal class FdoValidator : ParagraphValidator
		{
			...
			internal static ITsString GetTssStringValue(ParagraphAnnotatorForParagraphBuilder tapb, int iSegment, int iSegForm, out int ws)
			{
				...
				ws = StringUtils.GetWsAtOffset(tssStringValue, 0);
		...
		internal class ConceptualModelXmlParagraphValidator : FdoValidator
		{
			...
			private void CompareAnalyses(object expectedNode, IAnalysis actualAnalysis, string context)
			{
				...
				if (hvoAnalysisExpected == -1)
				{
					...
					Assert.AreEqual(expectedText, analysis.GetForm(StringUtils.GetWsAtOffset(m_para.Contents, 0)).Text, context);
		...
		internal class ParagraphAnnotator
		{
			...
			virtual internal IWfiWordform SetAlternateCase(int iSegment, int iSegForm, StringCaseStatus targetState, out string alternateCaseForm)
			{
				...
				IWfiWordform wfAlternateCase = WfiWordformServices.FindOrCreateWordform(m_cache, StringUtils.MakeTss(alternateCaseForm, ws));
		...
		internal class ParagraphBuilder
		{
			...
			internal IAnalysis ExportCbaNodeToReal(int iSegment, int iSegForm)
			{
				...
				if (hvoDesiredTarget == -1)
				{
					...
					analysis = wfFactory.Create(StringUtils.MakeTss(word, wsWord));
				}
				else
				{
			...
			internal IStTxtPara BuildParagraphContent(XmlNode paraDefn)
			{
				...
				if (m_para.Contents == null)
				{
				   // make sure it has an empty content
					m_para.Contents = StringUtils.MakeTss("", m_cache.DefaultVernWs);
				}
				return m_para;
			}
			...
			internal IWfiWordform SetDefaultWord(int iSegment, int iSegForm)
			{
				...
				IWfiWordform actualWordform = WfiWordformServices.FindOrCreateWordform(m_cache, StringUtils.MakeTss(stringValue, ws));
				XmlNode fileItem = SegmentFormNode(iSegment, iSegForm);
	...
	public class ParagraphParserTests: InterlinearTestBase
	{
		...
		public void CheckValidGuessesAfterInsertNewWord_LT8467()
		{
			...
			paraGlossed.Contents = StringUtils.MakeTss("xxxcrayzee xxxyouneek xxxsintents.", Cache.DefaultVernWs);
			...
			paraGuessed.Contents = StringUtils.MakeTss("xxxcrayzee xxxguessless xxxyouneek xxxsintents.", Cache.DefaultVernWs);
