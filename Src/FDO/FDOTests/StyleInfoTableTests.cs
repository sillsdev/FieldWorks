...
namespace SIL.FieldWorks.FDO.FDOTests
{
	...
	/// ----------------------------------------------------------------------------------------
	[TestFixture]
	public class StyleInfoTableTests : InMemoryFdoTestBase
	{
		/// ------------------------------------------------------------------------------------
		/// <summary>
		...
		public void ConnectStyles()
		{
			...
			props.SetStrPropValue((int)FwTextPropType.ktptFontFamily, StStyle.DefaultHeadingFont);
			...
			sectionInfo.FontInfoForWs(InMemoryFdoCache.s_wsHvos.En).m_fontName.ExplicitValue = "Arial";
			...
			Assert.AreEqual("<default serif>", fontInfo.m_fontName.Value);
			...
			Assert.AreEqual(Color.Black, fontInfo.m_fontColor.Value);
			Assert.AreEqual("<default sans serif>", fontInfo.m_fontName.Value);
			Assert.AreEqual(16000, entry.LineSpacing.m_lineHeight);
			Assert.AreEqual(0, entry.LeadingIndent);
			Assert.AreEqual(0, entry.TrailingIndent);
			Assert.AreEqual(0, entry.SpaceAfter);
			Assert.AreEqual(0, entry.SpaceBefore);
			Assert.AreEqual(FwSuperscriptVal.kssvOff, fontInfo.m_superSub.Value);
			fontInfo = entry.FontInfoForWs(InMemoryFdoCache.s_wsHvos.En);
