...
namespace SIL.FieldWorks.FDO.FDOTests.DataMigrationTests
{
	...
	public sealed class DataMigrationTests7000019 : DataMigrationTestsBase
	{
		...
		public void DataMigration7000019Test()
		{
			...
			// Do the migration.
			m_dataMigrationManager.PerformMigration(dtoRepos, 7000019);
			...
			using (var wsManager = new PalasoWritingSystemManager(new LocalFileWritingSystemStore(storePath, globalStore), globalStore))
			{
				IWritingSystem kalaba = wsManager.Get("x-kal");
				Assert.AreEqual("Kalaba", kalaba.LanguageSubtag.Name);
				Assert.AreEqual("Kal", kalaba.Abbreviation);
				Assert.IsTrue(kalaba.LanguageSubtag.IsPrivateUse);
