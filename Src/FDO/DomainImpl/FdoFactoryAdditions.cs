...
#pragma warning restore 168
		...
		{
			ITsString tssFullForm = StringUtils.MakeTss(entryFullForm, m_cache.DefaultVernWs);
			...
			entryComponents.GlossAlternatives.Add(StringUtils.MakeTss(senseGloss, m_cache.DefaultAnalWs));
		...
		public ILexEntry Create(LexEntryComponents entryComponents)
		{
			...
			var tssGloss =
				entryComponents.GlossAlternatives.DefaultIfEmpty(StringUtils.MakeTss("", m_cache.DefaultAnalWs)).
					FirstOrDefault();
			...
			foreach (ITsString tss in entryComponents.LexemeFormAlternatives)
				newEntry.SetLexemeFormAlt(StringUtils.GetWsAtOffset(tss, 0), tss);

			foreach (ITsString tss in entryComponents.GlossAlternatives)
				newEntry.SensesOS[0].Gloss.set_String(StringUtils.GetWsAtOffset(tss, 0), tss);
	...
	internal partial class MoAffixAllomorphFactory
	{
		...
		public IMoAffixAllomorph CreateCircumfix(ILexEntry entry, ILexSense sense, ITsString lexemeForm, IMoMorphType morphType)
		{
			...
			lexemeAllo.Form.set_String(StringUtils.GetWsAtOffset(lexemeForm, 0), lexemeForm);
			...
			var wsVern = StringUtils.GetWsAtOffset(lexemeForm, 0);
...
#pragma warning disable 168
	...
	internal partial class SegmentFactory
	{
		...
		/// ------------------------------------------------------------------------------------
		internal ISegment Create(IStTxtPara para, int beginOffset)
		{
	...
	internal partial class WfiWordformFactory
	{
		...
		{
			var wfNew = Create();
			var wsForm = StringUtils.GetWsAtOffset(tssForm, 0);
	...
	internal partial class CmPossibilityListFactory : ICmPossibilityListFactoryInternal
	{
		...
		public ICmPossibilityList CreateUnowned(string listName, int ws)
		{
			...
			var result = new CmPossibilityList(m_cache, dataReader.GetNextRealHvo(), Guid.NewGuid());
			...
			int footnoteIndex, string footnoteMarkerStyleName)
		{
			...
			using (var reader = new StringReader(sTextRepOfFootnote))
			{
				...
				foreach (XmlNode bla in tagList[0].ChildNodes)
				{
					...
					if (bla.Name == "M")
					{
						createdFootnote.FootnoteMarker = StringUtils.MakeTss(bla.InnerText,
	...
	internal partial class ScriptureFactory
	{
		...
		public IScripture Create()
		{
			...
			// Indicate that cetrtain clean-up actions have been taken. (Since this
