...
namespace SIL.FieldWorks.FDO.DomainImpl
{
	...
	internal partial class MoDerivAffMsa
	{
		...
		private ITsString InterlinearAffix(ITsString tssFromPartOfSpeech, ITsString tssToPartOfSpeech)
		{
			...
			tisb.AppendTsString(StringUtils.MakeTss(">", m_cache.WritingSystemFactory.UserWs));
	...
	internal partial class MoStemMsa
	{
		...
		private ITsString InterlinearStem(ITsString tssPartOfSpeech)
		{
			...
			if (PartOfSpeechRA == null)
				return StringUtils.MakeTss(Strings.ksNotSure, userWs);
			...
			if (InflectionClassRA != null)
			{
				bldr.ReplaceTsString(cch, cch, StringUtils.MakeTss("  (", userWs));
				cch = bldr.Length;
			...
			if (features != null)
				bldr.ReplaceTsString(cch, cch, StringUtils.MakeTss(features.ShortName, userWs));
	...
	internal partial class MoInflAffMsa
	{
		...
		protected override void AddObjectSideEffectsInternal(AddObjectEventArgs e)
		{
			switch (e.Flid)
			{
				case MoInflAffMsaTags.kflidSlots:
					...
					// This is called before the actual change is made, so evaluating the Affixes now leaves out the new one.
					var newGuids = (from msa in target.Affixes select msa.Guid).Concat(new Guid[] { target.Guid }).ToArray();
		...
		protected override void RemoveObjectSideEffectsInternal(RemoveObjectEventArgs e)
		{
			switch (e.Flid)
			{
				case MoInflAffMsaTags.kflidSlots:
					...
					// This is called before the actual change is made, so evaluating the Affixes now includes the deleted one.
					var newGuids = (from msa in target.Affixes where msa.Guid != target.Guid select msa.Guid).ToArray();
		...
		{
			if (PartOfSpeechRA == null)
				return StringUtils.MakeTss(Strings.ksInflectsAnyCat, m_cache.WritingSystemFactory.UserWs);
	...
	internal partial class MoForm
	{
		...
		public ITsString FormMinusReservedMarkers
		{
			...
			set
			{
				var wsVern = StringUtils.GetWsAtOffset(value, 0);
