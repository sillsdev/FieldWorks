...
#pragma warning restore 219
	...
	internal partial class CmPossibility
	{
		...
		private static ITsString BestAlternative(FdoCache cache, ICmPossibility pss, int wsMagic, int flid, string defValue)
		{
			...
			if (tss == null || tss.Length == 0)
				tss = StringUtils.MakeTss(defValue, cache.WritingSystemFactory.UserWs);
			return tss;
		}
		...
		internal static ITsString BestAnalysisOrVernAbbr(FdoCache cache, int hvo)
		{
			...
			if (tss == null || tss.Length == 0)
			{
				tss = StringUtils.MakeTss(Strings.ksQuestions, cache.WritingSystemFactory.UserWs);
				// JohnT: how about this?
				//return StringUtils.MakeTss("a " + this.GetType().Name + " with no name", cache.WritingSystemFactory.UserWs);
			}
			return tss;
		}
	...
	class ParsedParagraphOffsetsMethod : ParagraphOffsetsMethod
	{
		...
		{
			m_ich += m_segment.AnalysesRS[m_ianalysis].GetForm(StringUtils.GetWsAtOffset(Baseline, m_ich)).Length;
			if (m_ich > m_length)
			{
