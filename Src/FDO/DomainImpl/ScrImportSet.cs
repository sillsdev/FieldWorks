	...
	internal partial class ScrImportSet
	{
		...
		private void DoCommonNonModelSetup()
		{
			if (m_paratextAdapter == null)
				m_paratextAdapter = new Paratext6Proxy(m_cache.ThreadHelper);
		...
		private bool ParatextProjectsAccessible(out StringCollection projectsNotFound)
		{
			...
			string filename = Path.Combine(m_paratextAdapter.ProjectDir, m_ParatextScrProject + ".ssf");
			if (!FileUtils.IsFileReadableAndWritable(filename) ||
				m_paratextAdapter.GetProjectBooks(m_ParatextScrProject).Count == 0)
			{
			...
			{
				filename = Path.Combine(m_paratextAdapter.ProjectDir, m_ParatextBTProject + ".ssf");
				if (!FileUtils.IsFileReadableAndWritable(filename) ||
					m_paratextAdapter.GetProjectBooks(m_ParatextBTProject).Count == 0)
					projectsNotFound.Add(m_ParatextBTProject);
			}

			if (m_ParatextNotesProject != null)
			{
				filename = Path.Combine(m_paratextAdapter.ProjectDir, m_ParatextNotesProject + ".ssf");
				if (!FileUtils.IsFileReadableAndWritable(filename) ||
					m_paratextAdapter.GetProjectBooks(m_ParatextNotesProject).Count == 0)
		...
		/// </summary>
		/// <exception cref="T:NotSupportedException">If project is not a support type</exception>
		/// ------------------------------------------------------------------------------------
		...
		/// <exception cref="T:InvalidOperationException">thrown if basic import settings do not
		/// exist</exception>
		...
		/// <exception cref="T:NotSupportedException">If project is not a supported type</exception>
		/// ------------------------------------------------------------------------------------
					...
					case TypeOfImport.Paratext6:
						{
							...
							booksPresent = m_paratextAdapter.GetProjectBooks(ParatextScrProj);
							break;
						}
					case TypeOfImport.Paratext5:
					case TypeOfImport.Other:
