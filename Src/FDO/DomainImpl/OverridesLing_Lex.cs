...
namespace SIL.FieldWorks.FDO.DomainImpl
{
	...
	internal partial class LexEntry
	{
		...
		/// form, except that entry refs that belong to an entry which is a variant of something
		/// are excluded, even if complex (See LT-9566).
		...
		/// Optimize JohnT: we probably need some caching here, otherwise, it's going to be
		/// O(n-squared) in the number of LexEntryRefs.
		/// </summary>
		/// ------------------------------------------------------------------------------------
		[VirtualProperty(CellarPropertyType.ReferenceSequence, "LexEntryRef")]
		public IEnumerable<ILexEntryRef> ComplexFormEntryBackRefs
		{
			get
			{
				ILexEntryRefRepository repo = Services.GetInstance<ILexEntryRefRepository>();
				return
					from ILexEntryRef lexRef
						in repo.GetComplexFormRefsWithPrimaryEntryOrSense(this)
					where repo.GetVariantEntryRefsWithMainEntryOrSense(lexRef.Owner).Count() == 0
					select lexRef;
				// Old version was the results of
		...
		/// ------------------------------------------------------------------------------------
		[VirtualProperty(CellarPropertyType.ReferenceSequence, "LexEntryRef")]
		public IEnumerable<ILexEntryRef> AllComplexFormEntryBackRefs
		{
			get
			{
				ILexEntryRefRepository repo = Services.GetInstance<ILexEntryRefRepository>();
				return
					from ILexEntryRef lexRef
						in repo.GetComplexFormRefsWithPrimaryEntryOrSense(ThisAndAllSenses)
					where repo.GetVariantEntryRefsWithMainEntryOrSense(lexRef.Owner).Count() == 0
					select lexRef;
				// Original query.
				//string qry = String.Format("SELECT DISTINCT pl.Src " +
				...
				//    "LEFT OUTER JOIN LexEntry_EntryRefs er2 ON er2.Src=er.Src AND er2.Dst<>er.Dst " +
				//    "LEFT OUTER JOIN LexEntryRef ler2 ON ler2.Id=er2.Dst AND ler2.RefType=0 " +
				//    "WHERE (pl.Dst={0} OR pl.Dst IN (SELECT SenseId FROM fnGetSensesInEntry$({0}))) " +
				//    " AND ler2.Id IS NULL",
				//    this.m_hvo);
			}
		}
		...
		{
			if (e.Flid == LexEntryTags.kflidSenses)
			{
			...
			if (e.Flid == LexEntryTags.kflidAlternateForms && e.Index == 0)
			{
		...
		protected override void RemoveObjectSideEffectsInternal(RemoveObjectEventArgs e)
		{
			...
			else if (e.Flid == LexEntryTags.kflidAlternateForms && e.Index == 0)
			{
				if (!Cache.ObjectsBeingDeleted.Contains(this))
				{
		...
		internal void UpdateHomographs(string oldHf)
		{
			...
			UpdateHomographs(oldHf, HomographForm);
		}

		private void UpdateHomographs(string oldHf, string newHf)
		{
			if (oldHf == newHf)
				return;
			var repo = Services.GetInstance<ILexEntryRepository>();
			// This should include this somewhere.
			...
			int start = oldHomographs.IndexOf(this);
			if (start >= 0)
			{
				if (oldHomographs.Count == 2)
				{
					// removing one of two homograph, the survivor will not be a homograph.
					oldHomographs[1 - start].HomographNumber = 0;
				}
				else
				{
					// Decrement the number for any larger homograph.
					for (int i = start + 1; i < oldHomographs.Count; i++)
						oldHomographs[i].HomographNumber--;
				}
			}
		...
		{
			UpdateHomographs(HomographForm, "");
		...
		public ITsString HeadWord
		{
			get
			{
				return StringServices.HeadWordStaticForWs(this, Cache.DefaultVernWs);
			}
		}
		...
		/// objects owned by this LexEntry that define this entry as a variant (that is, RefType is zero).
		...
		{
			return StringServices.HeadWordStaticForWs(this, wsVern);
		}
		...
		{
			return StringServices.HeadWordStaticForWs(entry, entry.Cache.DefaultVernWs);
		}
	...
	internal partial class LexSense
	{
		...
		/// Enhance JohnT: Generate PropChanged on changes to LexEntryRef.PrimaryLexemes
		/// or LexEntryRef.RefType.
		/// </summary>
		/// ------------------------------------------------------------------------------------
		[VirtualProperty(CellarPropertyType.ReferenceSequence, "LexEntryRef")]
		public IEnumerable<ILexEntryRef> ComplexFormEntryBackRefs
		{
			...
			{
				ILexEntryRefRepository repo = Services.GetInstance<ILexEntryRefRepository>();
				return repo.GetComplexFormRefsWithPrimaryEntryOrSense(this);
			}
		}
		...
		{
			var le = Cache.ServiceLocator.GetInstance<ILexEntryRepository>().GetObject(EntryID) as LexEntry;
			...
			get
			{
				if (Name != null)
				{
					if (Name.AnalysisDefaultWritingSystem != null &&
						Name.AnalysisDefaultWritingSystem.Length > 0)
					{
						return Name.AnalysisDefaultWritingSystem;
					}
					else if (Name.VernacularDefaultWritingSystem != null &&
						Name.VernacularDefaultWritingSystem.Length > 0)
					{
						return Name.VernacularDefaultWritingSystem;
					}
					else if (Name.BestAnalysisVernacularAlternative != null &&
						Name.BestAnalysisVernacularAlternative.Length > 0 &&
				...
				if (Abbreviation != null)
				{
					if (Abbreviation.AnalysisDefaultWritingSystem != null &&
						Abbreviation.AnalysisDefaultWritingSystem.Length > 0)
					{
						return Abbreviation.AnalysisDefaultWritingSystem;
					}
					else if (Abbreviation.VernacularDefaultWritingSystem != null &&
						Abbreviation.VernacularDefaultWritingSystem.Length > 0)
					{
						return Abbreviation.VernacularDefaultWritingSystem;
					}
					else if (Abbreviation.BestAnalysisVernacularAlternative != null &&
						Abbreviation.BestAnalysisVernacularAlternative.Length > 0 &&
						Abbreviation.BestAnalysisVernacularAlternative.Text != "***")
					{
						return Abbreviation.BestAnalysisVernacularAlternative;
					}
				}
	...
	internal partial class LexReference
	{
		...
		{
			var flid = m_cache.MetaDataCache.GetFieldId2(LexEntryTags.kClassId, "LexEntryReferences", false);

			List<ILexReference> backrefs = Services.GetInstance<ILexReferenceRepository>().GetReferencesWithTarget(entry).ToList();
			if (backrefs.Contains(this))
			{
				if (!fAdded)
					backrefs.Remove(this);
			...
			var guids = backrefs.Select(lexRef => lexRef.Guid).ToArray();
	...
	internal partial class LexEntryRef
	{
		...
		private void UpdateComplexFormEntryBackRefs(ICmObject thingAddedOrRemoved, bool fAdded)
		{
			...
			if (entry != null)
			{
				List<ICmObject> backrefs = entry.ComplexFormEntryBackRefs.Cast<ICmObject>().ToList();
				if (backrefs.Contains(this))
				{
					if (!fAdded)
						backrefs.Remove(this);
				...
				Cache.ServiceLocator.GetInstance<IUnitOfWorkService>().RegisterVirtualAsModified(entry,
					"ComplexFormEntryBackRefs", backrefs);
				backrefs.Clear();
				backrefs.AddRange(entry.AllComplexFormEntryBackRefs.Cast<ICmObject>());
				if (backrefs.Contains(this))
				{
					if (!fAdded)
						backrefs.Remove(this);
				...
				Cache.ServiceLocator.GetInstance<IUnitOfWorkService>().RegisterVirtualAsModified(entry,
					"AllComplexFormEntryBackRefs", backrefs);
				return;
			}
			var sense = thingAddedOrRemoved as LexSense;
			if (sense != null)
			{
				List<ICmObject> backrefs = sense.ComplexFormEntryBackRefs.Cast<ICmObject>().ToList();
				if (backrefs.Contains(this))
				{
					if (!fAdded)
						backrefs.Remove(this);
		...
		protected override void RemoveObjectSideEffectsInternal(RemoveObjectEventArgs e)
		{
			switch (e.Flid)
			{
				case LexEntryRefTags.kflidPrimaryLexemes:
					...
					UpdateComplexFormEntryBackRefs(e.ObjectRemoved, false);
					break;
				...
				case LexEntryRefTags.kflidComponentLexemes:
					...
					RemoveComponentFromPrimaryLexemes(e.ObjectRemoved);
					break;
			}
		...
		private void RemoveComponentFromPrimaryLexemes(ICmObject objRemoved)
		{
			// The same object being removed from ComponentLexemes needs to also be removed
			// from PrimaryLexemes.
			var ffound = false;
			foreach (var obj in PrimaryLexemesRS)
			{
				if (obj.Hvo != objRemoved.Hvo)
					continue;
				ffound = true;
				break;
			}
