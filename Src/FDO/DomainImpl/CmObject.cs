// --------------------------------------------------------------------------------------------
// Copyright (C) 2002-2009 SIL International. All rights reserved.
//
...
namespace SIL.FieldWorks.FDO.DomainImpl
{
	...
	internal partial class CmObject : ICmObjectInternal, ICmObjectOrIdInternal, ICmObjectOrSurrogate, IReferenceSource
	{
		...
		public virtual void MergeObject(ICmObject objSrc, bool fLoseNoStringData)
		{
			...
			foreach (int flid in mdc.GetFields(ClassID, true, (int)CellarPropertyTypeFilter.All))
			{
					...
					case (int)CellarPropertyType.BigString: // 17
						{
							...
							myTss = StringUtils.MergeString((ITsString)srcCurrentValue, myTss, fLoseNoStringData);
							if (mySetMethod != null)
		...
		protected virtual void ReplaceInternal(int flid, IEnumerable<ICmObject> thingsToRemove, IEnumerable<ICmObject> thingsToAdd)
		{
			...
			if (set is IFdoSet<ICmObject>)
				((IFdoSet<ICmObject>)set).Replace(thingsToRemove, thingsToAdd);
			else
		...
		protected virtual object GetVirtualPropertyForSDA(int flid)
		{
			...
			switch ((CellarPropertyType)flidType)
			{
				...
				case CellarPropertyType.ReferenceSequence:
					...
					if (retval is IEnumerable<ICmObject>)
						return retval;
					...
					if (retval is IFdoVector)
						return ((IFdoVector)retval).Objects;
		...
		private void SetCustomPropertyForSDA(int flid, object newValue, bool useAccessor)
		{
			...
			if (useAccessor)
			{
				...
				if (dataType == CellarPropertyType.ReferenceAtomic
					|| dataType == CellarPropertyType.ReferenceCollection
					|| dataType == CellarPropertyType.ReferenceSequence)
				{
					if (oldValue != null)
						((ICmObjectInternal)oldValue).RemoveIncomingRef(this);
					if (newValue != null)
						((ICmObjectInternal)newValue).AddIncomingRef(this);
				}
			}
			...
			if (dataType == CellarPropertyType.OwningAtomic && oldValue != null)
			{
				...
				ICmObjectInternal obj = null;
				var id = oldValue as ICmObjectId;
				if (id != null)
					obj = (ICmObjectInternal)m_cache.ServiceLocator.GetObject(id);
				else
					obj = oldValue as ICmObjectInternal;
				obj.DeleteObject();
			}
		}
			...
			foreach (var flid in mdc.GetFields(ClassID, true, (int)CellarPropertyTypeFilter.All))
			{
				...
				switch (dataType)
				{
					...
					case CellarPropertyType.String:
						...
						writer.WriteRaw(StringUtils.GetXmlRep(tsString, wsf, wsHvo));
						break;
		...
		public virtual ICmObject ReferenceTargetOwner(int flid)
		{
			...
			if (mdc.IsCustom(flid))
			{
				...
				if (listGuid != Guid.Empty)
					return m_cache.ServiceLocator.GetInstance<ICmPossibilityListRepository>().GetObject(listGuid);
			}
