...
namespace SIL.FieldWorks.FDO.DomainImpl
{
	...
	internal sealed class AnalysisAdjuster
	{
		...
		public static void AdjustAnalysis(IStTxtPara para, ITsString oldContents)
		{
			...
			TsStringDiffInfo diffInfo = StringUtils.GetDiffsInTsStrings(oldContents, para.Contents);
			if (diffInfo != null)
		...
		/// </summary>
		/// <returns>true if we deleted any segments.</returns>
		private void DiscardDeletedSegments()
		{
			...
			foreach (ISegment seg in m_para.SegmentsOS)
			{
						 ...
						 m_oldEndOffsets[iSeg] >= iJustPastEditInOldContents)
				{
					...
					else
					{
						...
						//Note: this will not choose TE footnote makers for EOSPositions. StringUtils.kChObject
						collectorSeg.Run();
			...
			m_iFirstDeletedSegment = iFirstDel;

			m_para.SegmentsOS.Replace(iFirstDel, m_cSegsDeleted, new ICmObject[0]);

			// Fix any tags that just lost their Begin or End Segment (won't be both)
			FixSingleHangingReference(iFirstDel);
		}

		private void FixSingleHangingReference(int iFirstSegmentDeleted)
		{
			FixHangingReferences(iFirstSegmentDeleted, -1);
		}

		/// <summary>
		/// Fix a reference where one (or both of the end points' Segments just got deleted
		/// </summary>
		/// <param name="iFirstSegmentDeleted"></param>
		/// <param name="cOldAnalyses"></param>
		private void FixHangingReferences(int iFirstSegmentDeleted, int cOldAnalyses)
		{
			ISegment newSeg;
			int cNewAnalyses;
			foreach (var iar in m_oldParaRefs)
			{
				...
				if (!iar.BegRef().IsValid && !iar.EndRef().IsValid)
				{
					...
					var iseg = Math.Max(0, iFirstSegmentDeleted - 1);
				...
				{
					if (cOldAnalyses > 0 && iFirstSegmentDeleted > 0)
					{
						// segment break deleted, try to preserve position in previous segment
						newSeg = m_para.SegmentsOS[iFirstSegmentDeleted - 1];
					...
					else
					{
						...
						newSeg = m_para.SegmentsOS[iFirstSegmentDeleted]; // Get next Segment after deleted one
						iar.ChangeToDifferentSegment(newSeg, true, false);
				...
				{
					newSeg = m_para.SegmentsOS[iFirstSegmentDeleted - 1]; // Get segment before deleted one
					cNewAnalyses = newSeg.AnalysesRS.Count;
					iar.ChangeToDifferentSegment(m_para.SegmentsOS[iFirstSegmentDeleted - 1], false, true);
		...
		/// and setting up m_newAnalysisGroups (which gives new Segment boundaries)
		/// </summary>
		void AnalyzeNewText()
		{
			...
			while (m_wordFinder.Position < m_baseline.Length)
			{
				...
				if (m_iFirstAnalysisToFix < m_oldAnalyses.Count && ichEndWordBL <= ichFirstDiffBL)
				{
					...
					else if (m_oldAnalyses[m_iFirstAnalysisToFix].Wordform != null)
					{
						...
						// good.
						var wsAtOffset = StringUtils.GetWsAtOffset(m_baseline, ichStartWordBL);
		...
		private int GetLastAnalysisToFix(out int iTokenIndex)
		{
			...
			for (iLastAnalysisToFix = m_oldAnalyses.Count - 1; iLastAnalysisToFix >= m_iFirstAnalysisToFix;)
			{
				...
				{
					var oldWordform = analysis.Wordform.Form.get_String(StringUtils.GetWsAtOffset(m_baseline, startWord));
