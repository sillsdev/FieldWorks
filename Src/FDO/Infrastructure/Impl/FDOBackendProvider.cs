...
namespace SIL.FieldWorks.FDO.Infrastructure.Impl
{
	...
	internal abstract partial class FDOBackendProvider : IDataSetup, IDataReader, IDataStorer
	{
		...
		private void StartupInternalWithDataMigrationIfNeeded()
		{
			var currentDataStoreVersion = StartupInternal(ModelVersion);

			if (currentDataStoreVersion > ModelVersion)
				throw new FwStartupException(Properties.Resources.kstidProjectIsForNewerVersionOfFw);
			...
			if (currentDataStoreVersion != ModelVersion)
			{
				...
				else
				{
					...
					DoMigration(currentDataStoreVersion);
				}
			}
		}

		private void DoMigration(int currentDataStoreVersion)
		{
			HashSet<ICmObjectOrSurrogate> newbies;
			HashSet<ICmObjectId> goners;
			HashSet<ICmObjectOrSurrogate> dirtballs = DoMigrationBasics(currentDataStoreVersion, out goners, out newbies);
			Commit(newbies, dirtballs, goners);
		}

		private HashSet<ICmObjectOrSurrogate> DoMigrationBasics(int currentDataStoreVersion, out HashSet<ICmObjectId> goners, out HashSet<ICmObjectOrSurrogate> newbies)
		{
			...
			m_dataMigrationManager.PerformMigration(dtoRepository, ModelVersion);
...
#pragma warning restore 219
		...
		public void StartupExtantLanguageProject(IProjectIdentifier projectId, bool fBootstrapSystem)
		{
			...
			try
			{
				StartupInternalWithDataMigrationIfNeeded();
