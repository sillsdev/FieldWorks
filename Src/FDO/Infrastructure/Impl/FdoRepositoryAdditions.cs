...
namespace SIL.FieldWorks.FDO.Infrastructure.Impl
{
	...
	internal partial class StFootnoteRepository
	{
		...
		public IStFootnote GetFootnoteFromObjData(string objData)
		{
			if (String.IsNullOrEmpty(objData))
				return null;
			Guid objGuid = StringUtils.GetHotGuidFromObjData(objData);
	...
	internal partial class ScrFootnoteRepository
	{
		...
		{
			Guid objGuid = StringUtils.GetHotObjectGuidFromProps(ttp);
			if (objGuid == Guid.Empty)
				return null;
	...
	internal partial class WfiWordformRepository : IWfiWordformRepositoryInternal
	{
		...
		public bool TryGetObject(ITsString tssForm, bool fIncludeLowerCaseForm, out IWfiWordform wf)
		{
			...
			{
				int ws = StringUtils.GetWsAtOffset(tssForm, 0);
				...
				if (lcForm != tssForm.Text)
					TryGetObject(StringUtils.MakeTss(lcForm, ws), false, out wf);
			}
			return wf != null;
		}
		...
		{
			var ws = StringUtils.GetWsAtOffset(tssTarget, 0);
	...
	internal partial class LexEntryRepository : ILexEntryRepositoryInternal
	{
		...
		/// </summary>
		public List<ILexEntry> CollectHomographs(FdoCache cache, string sForm, IMoMorphType morphType)
		{
					...
					if (le.Hvo != hvo && (homographForm == sForm || lexemeHomograph == sForm))
					{
						...
						foreach (var mmt in types)
						{
							...
							if (mmt.Guid == MoMorphTypeTags.kguidMorphBoundRoot ||
								mmt.Guid == MoMorphTypeTags.kguidMorphBoundStem ||
								mmt.Guid == MoMorphTypeTags.kguidMorphRoot ||
								mmt.Guid == MoMorphTypeTags.kguidMorphParticle ||
								mmt.Guid == MoMorphTypeTags.kguidMorphPhrase ||
								mmt.Guid == MoMorphTypeTags.kguidMorphDiscontiguousPhrase)
							{
	...
	internal partial class LexEntryRefRepository
	{
		...
		/// </summary>
		public IEnumerable<ILexEntryRef> GetComplexFormRefsWithPrimaryEntryOrSense(ICmObject mainEntryOrSense)
		{
			var entry = mainEntryOrSense as LexEntry;
			if (entry != null)
		...
		/// LexEntry/LexSense as one of the primary entries or senses.
		/// </summary>
		public IEnumerable<ILexEntryRef> GetComplexFormRefsWithPrimaryEntryOrSense(IEnumerable<ICmObject> targets)
		{
			return from item in targets from entry in GetComplexFormRefsWithPrimaryEntryOrSense(item) select entry;
		}
	}
	#endregion
