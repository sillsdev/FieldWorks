...
namespace SIL.FieldWorks.FDO.Cellar
{
	...
	/// ----------------------------------------------------------------------------------------
	public class InheritableStyleProp<T>
	{
		#region Data members
		private T m_value;
	...
	public class BaseStyleInfo
	{
		...
		public virtual void SetPropertiesBasedOnStyle(IStStyle style)
		{
			...
			m_style = style;
			m_name = style.Name;
			string desc = style.Usage.RawUserDefaultWritingSystem;
			if (desc != null) // Only overwrite existing description if not null (needed for USFM export)
				m_usage = desc;
		...
		private void CreateFontInfoOverrides(FdoCache cache)
		{
			...
			foreach (FdoReferenceCollection<ILgWritingSystem> wsCollection in
				new FdoReferenceCollection<ILgWritingSystem>[] {
					cache.LangProject.AnalysisWssRC,
					cache.LangProject.VernWssRC })
			{
				// Create a FontInfo for each available writing system
				foreach (ILgWritingSystem ws in wsCollection)
					m_fontInfoOverrides[ws.Hvo] = new FontInfo();
			}
		}
		...
		public string RealFontNameForWs(int ws)
		{
			...
			if (StStyle.IsMagicFontName(fontName))
				return m_owningTable.ResolveMagicFontName(fontName, ws != -1 ? ws : m_cache.DefaultUserWs);
			return fontName;
		}
		#endregion
