...
namespace SIL.FieldWorks.FDO.DomainServices
{
	...
	public class ParagraphParser : IFWDisposable
	{
											  ...
											  ref ITsString tssFirstWordOfNextSegment)
		{
			...
			do
			{
				...
				if (StringUtils.GetWsAtOffset(m_tssPara, ichMin) != m_paraWs)
				{
	...
	public class WordMaker
	{
		...
		{
			return StringUtils.GetWsAtOffset(m_tss, ich);
		}
		...
		{
			if (IsLeadSurrogate(st[ich]) && ich + 2 <= st.Length)
				return ich + 2;
			return ich + 1;
		}
		...
		{
			return StringUtils.IsWhite(m_cpe, m_st, ich);
		}
		...
		public ITsString NextWord(out int ichMin, out int ichLim)
		{
			...
			for (; m_ich < m_cch; m_ich = NextChar(m_st, m_ich))
			{
				...
				if (fThisWordForming)
				{
					string sStyleName = m_tss.get_StringPropertyAt(m_ich, (int)FwTextPropType.ktptNamedStyle);
