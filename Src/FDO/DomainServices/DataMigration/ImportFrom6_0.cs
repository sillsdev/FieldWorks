...
// ReSharper restore InconsistentNaming
	{
		...
		bool m_fVerboseDebug = false;

		/// <summary>
		/// Constructor for run-time debugging.
		/// </summary>
		public ImportFrom6_0(bool fDebug)
		{
		...
		public bool Import(string pathname, string projectName, out string projectFile)
		{
			...
			if (Path.GetExtension(pathname).ToLowerInvariant() == ".xml")
			{
				if (!IsValid6_0Xml(pathname))
				{
					...
					MessageBoxUtils.Show(Form.ActiveForm, Strings.ksBackupXMLFileTooOld,
			...
			else
			{
				using (var zipFile = new ZipFile(pathname))
				{
					...
					foreach (ZipEntry entry in zipFile)
					{
						...
						else if (entry.Name.ToLowerInvariant().EndsWith(".bak") && entry.IsFile)
						{
							fHasBak = true;
							if (fHasXml)
								break;
					...
					if (!fHasBak)
					{
						...
						// really need to show it here, we should pass in the owning form instead of relying on
						// Form.ActiveForm since it can return null if no .Net forms have focus.
						MessageBoxUtils.Show(Form.ActiveForm, Strings.ksZipNotFieldWorksBackup, Strings.ksCannotConvert);
						return false;
					}
					else
					{
		...
		private bool UnzipFile(ZipFile zipFile, ZipEntry entry, string message,
			out string tempPath)
		{
			...
			using (var stream = zipFile.GetInputStream(entry))
			{
				...
				using (var progressDialog = new ProgressDialogWithTask(form))
				{
					progressDialog.Title = Strings.ksConverting;
					progressDialog.ProgressBarStyle = ProgressBarStyle.Continuous;
					progressDialog.Maximum = (int) entry.Size;
					...
					{
						bool fCanceled = !(bool)progressDialog.RunTask(true, ExtractFile, stream, tempStream, message);
						tempStream.Close();
						stream.Close();
						if (fCanceled)
						{
		...
		public bool CallDbProgram(string args, string progressMsg, string errorMsgFmt)
		{
			using (var process = CreateAndInitProcess(m_dbPath, args))
			{
				...
				using (var progressDialog = new ProgressDialogWithTask(form))
				{
					progressDialog.ProgressBarStyle = ProgressBarStyle.Marquee; // Can't get actual progress from external program
					progressDialog.Title = Strings.ksConverting;
					if (!(bool)progressDialog.RunTask(true, ProcessFile, process, progressMsg))
					{
						// user canceled
						return false;
					}
					if (process.ExitCode != 0 || !string.IsNullOrEmpty(m_errorMessages))
					{
						...
						// really need to show it here, we should pass in the owning form instead of relying on
						// Form.ActiveForm since it can return null if no .Net forms have focus.
						MessageBoxUtils.Show(Form.ActiveForm, msg, Strings.ksCannotConvert);
						return false;
					}
					return true;
				}
		...
		public bool ImportFrom6_0Xml(string pathname, string folderName, string projectFile)
		{
				...
				'"' + pathname + "\" \"" + projectFile + '"'))
			{
				...
				using (var progressDialog = new ProgressDialogWithTask(form))
				{
					...
					if (!(bool)progressDialog.RunTask(true, ProcessFile, process, message))
					{
						// user canceled
						retval = false;
					}
		...
		private object ExtractFile(IProgress progressDlg, object[] parameters)
		{
		...
		/// </summary>
		/// <returns>true if completed, false if canceled</returns>
		public object ProcessFile(IProgress progressDlg, object[] parameters)
		{
