	...
	public static class WritingSystemServices
	{
			...
			int hvo, int flid, int wsDefault)
		{
			...
			if (xa != null)
			{
				...
				if (wsSpec.StartsWith(StringUtils.WsParamLabel))
					wsSpec = wsSpec.Substring(StringUtils.WsParamLabel.Length);
			...
			if (sWs != null)
			{
				...
				switch (sWs)
				{
					...
					case "all pronunciation":
						...
						break;
					default:
						if (sWs.StartsWith(StringUtils.WsParamLabel))
							sWs = sWs.Substring(StringUtils.WsParamLabel.Length);
			...
			int hvoObj, int flid, IWritingSystem currentWS, out int wsMagic)
		{
			...
			switch (wsSpec)
			{
				...
				case "current":
					...
					break;
				case "reversal":
					actualWS = GetReversalIndexEntryWritingSystem(cache, hvoObj, wsDefault).Handle;
					break;
				case "analysis vernacular":
				case "av":
				...
				case kwsVernInParagraph:
					...
					switch (sda.get_IntProp(hvo, CmObjectTags.kflidClass))
					{
						case CmBaseAnnotationTags.kClassId:
							...
							retWs = StringUtils.GetWsAtOffset(((IStTxtPara) ann.BeginObjectRA).Contents, ann.BeginOffset);
							break;
					...
					if (hvoPara != 0)
					{
						retWs = StringUtils.GetWsAtOffset(cache.ServiceLocator.GetInstance<IStTxtParaRepository>().GetObject(hvoPara).Contents, 0);
					}
					else
					{
						retWs = defaultVernWs;
		...
		private static void UpdateWritingSystemListField(FdoCache cache, ICmObject obj, int flid, string origWsId, string newWsId)
		{
			...
			{
				string str = wsId == origWsId ? newWsId : wsId;
			...
			cache.DomainDataByFlid.set_UnicodeProp(obj.Hvo, flid, sb.Length == 0 ? null : sb.ToString());
		}
