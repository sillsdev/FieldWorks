...
namespace SIL.FieldWorks.FDO.DomainServices
{
	...
	public class AnalysisOccurrence
	{
		...
		{
			get { return Segment.AnalysesRS[Index]; }
			set
			{
		...
		public int BaselineWs
		{
			get
			{
				return StringUtils.GetWsAtOffset(Paragraph.Contents, GetMyBeginOffsetInPara());
			}
		}
		...
		private ISegment GetNextSegment(ISegment seg)
		{
			...
			if (iseg == para.SegmentsOS.Count - 1)
			{
				...
				var nextPara = GetNextParagraph(para) as IStTxtPara;
				if (nextPara == null || nextPara.SegmentsOS == null || nextPara.SegmentsOS.Count == 0)
					return null;
		...
		private ISegment GetPreviousSegment(ISegment seg)
		{
			...
			if (iseg == 0)
			{
				...
				var prevPara = GetPreviousParagraph(para) as IStTxtPara;
		...
		public AnalysisOccurrence NextAnalysisOccurrence()
		{
			if (Index == Segment.AnalysesRS.Count - 1)
			{
				...
				var nextSeg = GetNextSegment(Segment);
		...
		public AnalysisOccurrence PreviousAnalysisOccurrence()
		{
			if (Index == 0)
			{
				...
				var prevSeg = GetPreviousSegment(Segment);
