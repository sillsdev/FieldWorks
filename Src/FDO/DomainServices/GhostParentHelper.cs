...
namespace SIL.FieldWorks.FDO.DomainServices
{
	...
	public class GhostParentHelper
	{
		...
		/// <returns></returns>
		public int GetOwnerOfBasicProperty(int hvo)
		{
			int hvoBasicOwner = hvo; // by default, we assume hvo is the owner
			if (IsGhostOwnerClass(hvo))
			{
				if (IsGhostOwnerChildless(hvo))
					return 0;
				// this owning object has a child, so get the string from it
				hvoBasicOwner = GetFirstChildFromParent(hvo);
			}
			return hvoBasicOwner;
		}
		...
		/// <summary>
		/// Like GetOwnerOfBasicProperty, but will create a child if necessary so as never to return zero.
		/// Caller must ensure we are in a UOW.
		/// </summary>
		public int FindOrCreateOwnerOfBasicProp(int hvoItem, int flidBasicProp)
		{
			int hvoOwnerOfBasicProp = GetOwnerOfBasicProperty(hvoItem);
			if (hvoOwnerOfBasicProp == 0)
			{
				// create the first child for this ghost owner
				int clidCreate = ClassToCreate(hvoItem, flidBasicProp);
				hvoOwnerOfBasicProp = GetSda().MakeNewObject(clidCreate, hvoItem, m_flidOwning, m_indexToCreate);
			}
			return hvoOwnerOfBasicProp;
		}

		private ISilDataAccessManaged GetSda()
		{
		...
		/// </summary>
		public bool IsGhostOwnerChildless(int hvoItem)
		{
		...
		private int GetFirstChildFromParent(int hvoParent)
		{
