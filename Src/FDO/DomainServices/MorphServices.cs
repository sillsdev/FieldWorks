...
namespace SIL.FieldWorks.FDO.DomainServices
{
	...
	public static class MorphServices
	{
		...
		{
			var wss = (from key in formCollector.Keys select StringUtils.GetWsAtOffset(key, 0)).Distinct().ToArray();
			...
			{
				int ws = StringUtils.GetWsAtOffset(key, 0);
			...
			where TMoFormRepository : IRepository<TMoForm>
		{
			...
			int wsForm = StringUtils.GetWsAtOffset(tssMorphForm, 0);
		...
		public static IMoForm MakeMorph(ILexEntry owningEntry, ITsString tssfullForm)
		{
			...
			var wsVern = StringUtils.GetWsAtOffset(tssfullForm, 0);
		...
		public static MorphComponents BuildMorphComponents(FdoCache cache, ITsString formWithMarkers, Guid guidDefaultMorphType)
		{
			...
			bool fVern = cache.ServiceLocator.WritingSystems.CurrentVernacularWritingSystems.Contains(StringUtils.GetWsAtOffset(formWithMarkers, 0));
			...
			else
			{
				...
				morphComponents.TssForm = StringUtils.MakeTss(form, StringUtils.GetWsAtOffset(formWithMarkers, 0));
