...
namespace SIL.FieldWorks.FDO.DomainServices
{
	...
	public static class StringServices
	{
		...
		internal static ITsString HeadWordStaticForWs(ILexEntry entry, int wsVern)
		{
			...
			tisb.Append(CitationFormWithAffixTypeStaticForWs(entry, wsVern));
			...
			var nHomograph = entry.HomographNumber;
			if (nHomograph > 0)
			{
		...
		public static string ShortName1StaticForWs(ILexEntry entry, int wsVern)
		{
			...
			return LexemeFormStaticForWs(entry, wsVern);
		}
		...
		internal static string LexemeFormStaticForWs(ILexEntry entry, int wsVern)
		{
			...
			// Try the first alternate form with the wsVern WS.
			return FirstAlternateForm(entry, wsVern);
		}
		...
		internal static string FirstAlternateForm(ILexEntry entry, int wsVern)
		{
			...
			return DefaultHomographString();
		}

		/// <summary>
		/// The string we use for HomographForm if we can't find any plausible form.
		/// </summary>
		internal static string DefaultHomographString()
		{
		...
		public static string CitationFormWithAffixTypeStaticForWs(ILexEntry entry, int wsVern)
		{
			...
			string form = ShortName1StaticForWs(entry, wsVern);
					...
					| (int)CellarPropertyTypeFilter.String | (int)CellarPropertyTypeFilter.BigString))
				{
					...
					switch ((CellarPropertyType)mdc.GetFieldType(flid))
					{
						...
						case CellarPropertyType.BigString:
							...
							ITsString newStr = stringModifier(oldStr);
							if (oldStr != newStr)
						...
						case CellarPropertyType.MultiBigUnicode:
							multiStringModifier(sda.get_MultiStringProp(obj.Hvo, flid));
							break;
					}
				}
			}
