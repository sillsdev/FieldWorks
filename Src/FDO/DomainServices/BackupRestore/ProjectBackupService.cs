...
namespace SIL.FieldWorks.FDO.DomainServices.BackupRestore
{
	...
	public class ProjectBackupService
	{
		...
		public string BackupProject(Form progressDialogOwner)
		{
			...
			try
			{
				...
				using (var progressDlg = new ProgressDialogWithTask(progressDialogOwner))
				{
					...
					{
						//var fwdataFailed =
						//    (from item in m_failedFiles where item.EndsWith(FwFileExtensions.ksFwDataXmlFileExtension) select item).
						//        FirstOrDefault() != null;
						var bldr = new StringBuilder();
						foreach (var item in m_failedFiles)
						{
							if (bldr.Length > 0)
								bldr.Append(", ");
							bldr.Append(Path.GetFileName(item));
						}
						var msg = string.Format(Strings.ksCouldNotBackupSomeFiles, bldr);
						if (MessageBox.Show(progressDialogOwner, msg, Strings.ksWarning, MessageBoxButtons.YesNo, MessageBoxIcon.Warning) !=
							DialogResult.Yes)
						{
