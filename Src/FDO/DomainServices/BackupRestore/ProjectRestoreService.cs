...
namespace SIL.FieldWorks.FDO.DomainServices.BackupRestore
{
	...
	public class ProjectRestoreService
	{
		...
		/// </summary>
		/// <param name="threadHelper">The thread helper to perform actions on the main UI
		/// thread.</param>
		...
		/// ------------------------------------------------------------------------------------
		public void RestoreProject(ThreadHelper threadHelper)
		{
			...
			if (extension == FwFileExtensions.ksFw60BackupFileExtension || extension == ".xml")
				ImportFrom6_0Backup(fileSettings);
			...
			if (DirectoryFinder.IsSubFolderOfProjectsDirectory(m_restoreSettings.ProjectPath) && !suppressConversion)
				ClientServerServices.Current.Local.ConvertToDb4oBackendIfNeeded(threadHelper, null, m_restoreSettings.FullProjectPath);

			CleanupAfterRestore(true);
		}

		private void ImportFrom6_0Backup(BackupFileSettings fileSettings)
		{
			ImportFrom6_0 importer = new ImportFrom6_0();
			string projFile;
