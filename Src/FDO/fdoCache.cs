...
namespace SIL.FieldWorks.FDO
{
	...
	public sealed partial class FdoCache
	{
		...
		#region Enums
		private enum LangProjStartupType
		{
			DoNothing,
			LoadExisting,
			LoadExistingNoInit,
			LoadExistingWithNoBootstrap,
			CreateNewBlank,
			CreateCopy,
		}
		#endregion
		...
		{
			return CreateCacheInternal(projectId, userWsIcuLocale, LangProjStartupType.DoNothing,
				null, threadHelper);
		}
			...
			ThreadHelper threadHelper)
		{
			FdoCache createdCache = CreateCacheInternal(projectId, userWsIcuLocale,
				LangProjStartupType.CreateNewBlank, null, threadHelper);
		...
		public static FdoCache CreateCacheFromExistingData(IProjectIdentifier projectId,
			string userWsIcuLocale, ThreadHelper threadHelper)
		{
			return CreateCacheFromExistingData(projectId, userWsIcuLocale, threadHelper, true);
		}
		...
		/// <param name="threadHelper">The thread helper used for invoking actions on the main
		/// UI thread.</param>
		/// <param name="fBootstrap">True to bootstrap the existing system, false to skip that
		/// step</param>
		/// ------------------------------------------------------------------------------------
		internal static FdoCache CreateCacheFromExistingData(IProjectIdentifier projectId,
			string userWsIcuLocale, ThreadHelper threadHelper, bool fBootstrap)
		{
			return CreateCacheInternal(projectId, userWsIcuLocale,
				fBootstrap ? LangProjStartupType.LoadExisting : LangProjStartupType.LoadExistingWithNoBootstrap,
				null, threadHelper);
		}
		...
		{
			return CreateCacheInternal(projectId, userWsIcuLocale, LangProjStartupType.CreateCopy,
				sourceCache, threadHelper);
		}
		...
		/// <param name="userWsIcuLocale">The ICU locale of the default user WS.</param>
		/// <param name="startupType">The way the language project will be created (from
		/// an existing source, just a new/blank, or a copy of an existing)</param>
		/// <param name="sourceCache">The FdoCache to copy for a startup type of CreateCopy</param>
		/// <param name="threadHelper">The thread helper used for invoking actions on the main
		/// UI thread.</param>
		/// ------------------------------------------------------------------------------------
		private static FdoCache CreateCacheInternal(IProjectIdentifier projectId,
			string userWsIcuLocale, LangProjStartupType startupType, FdoCache sourceCache,
			ThreadHelper threadHelper)
		{
			...
			switch (startupType)
			{
				case LangProjStartupType.LoadExisting:
				case LangProjStartupType.LoadExistingNoInit:
					dataSetup.StartupExtantLanguageProject(projectId, true);
					break;
				case LangProjStartupType.LoadExistingWithNoBootstrap:
					dataSetup.StartupExtantLanguageProject(projectId, false);
					break;
				case LangProjStartupType.CreateNewBlank:
					dataSetup.CreateNewLanguageProject(projectId);
					break;
				case LangProjStartupType.CreateCopy:
					dataSetup.InitializeFromSource(projectId, sourceCache);
					break;
				case LangProjStartupType.DoNothing:
					return createdCache;
				default:
					throw new InvalidEnumArgumentException("startupType", (int) startupType, typeof (LangProjStartupType));
			}

			if (startupType == LangProjStartupType.LoadExisting)
				createdCache.Initialize(userWsIcuLocale);
		...
		/// ------------------------------------------------------------------------------------
		public static string CreateNewLangProj(IProgress progressDlg, params object[] parameters)
		{
			...
			using (FdoCache cache = CreateCacheInternal(new SimpleProjectId(FDOBackendProviderType.kXML, dbFileName),
				userIcuLocale, LangProjStartupType.LoadExistingNoInit, null, threadHelper))
			{
				if (progressDlg != null)
				{
					progressDlg.Step(0);
			...
			return ClientServerServices.Current.Local.ConvertToDb4oBackendIfNeeded(
				threadHelper, progressDlg != null ? progressDlg.Form : null, dbFileName);
		}
		...
		public ILgWritingSystemFactory WritingSystemFactory
		{
			...
			{
				CheckDisposed();
				return m_serviceLocator.GetInstance<ILgWritingSystemFactory>();
			}
		}
		...
		/// ------------------------------------------------------------------------------------
		public string GetOutlineNumber(ICmObject obj, int flid, bool fFinPer, bool fIncTopOwner)
		{
			CheckDisposed();
			...
			while (true)
			{
				...
				if (ownerFlid == 0 || mdc.GetFieldType(ownerFlid) != (int)CellarPropertyType.OwningSequence)
					break;
				var ihvo = DomainDataByFlid.GetObjIndex(owner.Hvo, ownerFlid, target.Hvo);
