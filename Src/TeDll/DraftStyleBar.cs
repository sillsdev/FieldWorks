...
namespace SIL.FieldWorks.TE
{
	...
	public class DraftStyleBar : RootSite
	{
		...
		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// For the draft style bar, we really don't need to save selections since the
		/// associated draft view will determine where we really are and this view only retains
		/// focus while the user is clicking/dragging in it.
		/// </summary>
		/// ------------------------------------------------------------------------------------
		public override void SaveSelection()
		{
			CheckDisposed();
			// Don't set m_savedSelection.
		}
		...
		public override void MakeRoot()
		{
			...
			bool fSiblingEditable = ((IRootSite)Group.ScrollingController).EditingHelper.Editable;
		...
		private void SelectAssociatedPara(SelectionHelper origSelection, IVwRootSite view)
		{
			SelLevInfo paraInfoAnchor = origSelection.GetLevelInfoForTag(
				(int)StText.StTextTags.kflidParagraphs, SelectionHelper.SelLimitType.Top);
			SelLevInfo paraInfoEnd = origSelection.GetLevelInfoForTag(
				(int)StText.StTextTags.kflidParagraphs, SelectionHelper.SelLimitType.Bottom);
			...
			if (paraInfoAnchor.hvo != m_prevPara1Hvo || paraInfoEnd.hvo != m_prevPara2Hvo)
			{
				...
				StTxtPara para1 = new StTxtPara(m_fdoCache, paraInfoAnchor.hvo);
				StTxtPara para2 = new StTxtPara(m_fdoCache, paraInfoEnd.hvo);
		...
		/// ------------------------------------------------------------------------------------
		private void AdjustSelectionForPrompt(SelectionHelper helper, StTxtPara paraAnchor,
			StTxtPara paraEnd)
		{
			SelectionHelper.SelLimitType limit = SelectionHelper.SelLimitType.Anchor;
			if ((helper.IsFlidInLevelInfo((int)ScrSection.ScrSectionTags.kflidHeading, limit) ||
				helper.IsFlidInLevelInfo((int)ScrBook.ScrBookTags.kflidTitle, limit)) &&
