...
namespace SIL.FieldWorks.TE
{
	...
	public class DraftViewTests : DraftViewTestBase
	{
		...
		public void GetPassageAsString()
		{
			...
			m_draftView.TeEditingHelper.GotoVerse(new ScrReference(2, 1, 1, ScrVers.English));
			...
			m_draftView.TeEditingHelper.GotoVerse(new ScrReference(2, 1, 6, ScrVers.English));
			// Get the current Scripture passage info for cursor location
			m_draftView.TeEditingHelper.GetSelectedScrElement(out tag, out hvoSel);
		...
		public void GoToVerseMissing()
		{
			...
			Assert.IsTrue(m_draftView.TeEditingHelper.GotoVerse(
				new ScrReference(2, 1, 2, ScrVers.English)));
...
//		}
		...
		public void GoToVerse_AfterEmptyParaAtEndOfBook()
		{
			...
			Assert.IsTrue(m_draftView.TeEditingHelper.GotoVerse(
				new ScrReference(2, 1, 10, ScrVers.English)));
			...
			Assert.AreEqual(new ScrReference(2, 1, 7, ScrVers.English),
		...
		public void GoToVerse_Missing_EmptyPara()
		{
			...
			Assert.IsTrue(m_draftView.TeEditingHelper.GotoVerse(
				new ScrReference(2, 1, 4, ScrVers.English)));
			// Make sure selection changed
			m_draftView.TeEditingHelper.GetSelectedScrElement(out tag, out hvoSel);
			Assert.AreEqual(ScrSectionTags.kflidContent, tag);
			Assert.IsTrue(hvoSelInit != hvoSel);
		...
		public void GoToImplicitVerse()
		{
			...
			Assert.IsTrue(m_draftView.TeEditingHelper.GotoVerse(
				new ScrReference(2, 1, 1, ScrVers.English)));
		...
		public void DeleteFootnoteWithBt()
		{
			...
			TsStringUtils.InsertOrcIntoPara(footnote1.Guid, FwObjDataTypes.kodtNameGuidHot, btTssBldr, 4, 4, wsBt);
			TsStringUtils.InsertOrcIntoPara(footnote2.Guid, FwObjDataTypes.kodtNameGuidHot, btTssBldr, 8, 8, wsBt);
			TsStringUtils.InsertOrcIntoPara(footnote3.Guid, FwObjDataTypes.kodtNameGuidHot, btTssBldr, 14, 14, wsBt);
		...
		private ICmPicture CreateAndInsertPicture(int index, out string fileNameString)
		{
			...
			return Cache.ServiceLocator.GetInstance<ICmPictureFactory>().Create(fileNameString,
				...
				CmFolderTags.DefaultPictureFolder);
		}
		...
		public void InsertFootnote_MultiParaRangeSelection()
		{
			...
			ITsString tssExpected = TsStringUtils.MakeTss(string.Empty, Cache.DefaultVernWs);
		...
		public void InsertFootnote_InTitle_WithPrompt()
		{
			...
			para.Contents = TsStringUtils.MakeTss(string.Empty, Cache.DefaultVernWs);
			m_draftView.RefreshDisplay();
			...
			AssertEx.AreTsStringsEqual(TsStringUtils.MakeTss(string.Empty, Cache.DefaultVernWs), footnotePara.Contents);
		}
		#endregion
