...
namespace SIL.FieldWorks.TE
{
	...
	public class TestImportManager : TeImportManager
	{
		...
		/// ------------------------------------------------------------------------------------
		protected TestImportManager(TeMainWnd mainWnd) : base(mainWnd, mainWnd)
		{
		}
		#endregion
	...
	public class TestTeMainWnd : TeMainWnd
	{
		...
		/// <summary></summary>
		public UserView m_DraftViewSpec;
		...
		public override string InformationBarText
		{
			...
			{
				CheckDisposed();
				return m_dummyInfoBarText;
			}
			set
			{
				CheckDisposed();
				m_dummyInfoBarText = value;
			}
		}
		...
		{
			CreateDraftView(m_DraftViewSpec, TeViewType.DraftView, new SBTabItemProperties(null));
		}
		...
		public override EditingHelper EditingHelper
		{
			get
			{
				...
				if (m_editingHelper == null)
				{
					...
					m_editingHelper = new TestTeEditingHelper(this, Cache, FilterInstance,
	...
	{
		private List<SyncInfo> m_syncs = new List<SyncInfo>();
		...
		/// ------------------------------------------------------------------------------------
		public TestTeApp(string[] rgArgs) : base(rgArgs)
		{
		}
		...
		public bool OpenMainWindow()
		{
			...
			Form mainWnd = NewMainWindow(null, false, true);
			if (mainWnd != null)
		...
		internal void VerifySynchMessageExists(FdoCache cache, SyncMsg msg, int hvo, int tag)
		{
			...
			foreach (SyncInfo info in m_syncs)
			{
				if (info.msg == msg && info.hvo == hvo && info.flid == tag)
					foundMessage = true;
			}
	...
	public class TeAppTestsWithRestart : BaseTest
	{
		...
		public void UndoImport_NewBook()
		{
			...
			m_firstMainWnd.Import(settings);
			m_testTeApp.VerifySynchMessageExists(cache, SyncMsg.ksyncReloadScriptureControl, 0, 0);
	...
	public class TeAppTestsWithoutRestart : BaseTest
	{
		...
		public void CleanUp()
		{
			...
			if (m_fMainWindowOpened && m_firstMainWnd != null)
			{
				...
				{
					m_firstMainWnd.Cache.Undo(out ures);
		...
		public void VerifyStylesInComboForTitle()
		{
			...
			m_firstDraftView.TeEditingHelper.SetInsertionPoint(
				(int)ScrBook.ScrBookTags.kflidTitle, 0, 0);
		...
		public void RemoveCharFormattingWithCtrlSpace()
		{
			...
			SelectionHelper selHelper = m_firstDraftView.SelectRangeOfChars(1, 2, 0, 0, 1);
		...
		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Test getting the local server name.
		/// </summary>
		/// ------------------------------------------------------------------------------------
		[Test]
		public void GetLocalServerTest()
		{
			CheckDisposed();

			Assert.AreEqual(m_sSvrName, m_testTeApp.GetLocalServer_Bkupd());
		}
		...
		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Test CloseDbAndWindows and ReopenDbAndOneWindow
		/// </summary>
		/// ------------------------------------------------------------------------------------
		[Test]
		[Category("LongRunning")]
		public void CloseAndReOpenTests()
		{
			Assert.IsTrue(m_fMainWindowOpened);

			// cast to object to get rid of compiler warning
			Assert.IsTrue((object)m_testTeApp is IBackupDelegates,
				"TeApp/FwApp doesn't implement IBackupDelegates");

			Assert.AreEqual(1, m_testTeApp.MainWindows.Count);
			Assert.AreEqual(m_sSvrName, m_firstMainWnd.Cache.ServerName);

			// Test CloseDbAndWindows
			bool fClosed = m_testTeApp.CloseDbAndWindows_Bkupd(m_sSvrName, m_sDbName, false);
			Assert.IsTrue(fClosed, "App not closed.");
			// only 1 window should now be open and it should be the dummy window.
			Assert.AreEqual(1, m_testTeApp.MainWindows.Count);
			Assert.IsInstanceOfType(typeof(FwDummyWnd), m_testTeApp.MainWindows[0]);

			// be sure the DB connection got closed
			string sSql = string.Format("Server={0}; Database=master; User ID = sa;"
				+ "Password=inscrutable; Pooling=false;", m_sSvrName);
			SqlConnection sqlConMaster = new SqlConnection(sSql);
			sqlConMaster.Open();
			...
			sqlComm.CommandText = string.Format(@"SELECT *
				...
				on [sdb].[dbid] = [sproc].[dbid] and [sdb].[name] = '{0}'
				WHERE [sproc].[hostname] = '{1}'
				AND [sproc].[program_name] = 'SIL FieldWorks'",
				m_sDbName, Environment.MachineName);
			SqlDataReader sqlreader =
				sqlComm.ExecuteReader(System.Data.CommandBehavior.SingleResult);

			bool fAnyRows = sqlreader.Read();
			sqlreader.Close();
			sqlConMaster.Close();
			Assert.IsFalse(fAnyRows, "Got rows after closing.");

			// Test ReopenDbAndOneWindow
			m_testTeApp.ReopenDbAndOneWindow_Bkupd(m_sSvrName, m_sDbName);
			Assert.AreEqual(1, m_testTeApp.MainWindows.Count);
			m_firstMainWnd = (TestTeMainWnd)m_testTeApp.MainWindows[0];
			Assert.AreEqual(m_sSvrName, m_firstMainWnd.Cache.ServerName);
			Assert.AreEqual(m_sDbName.ToLower(), m_firstMainWnd.Cache.DatabaseName.ToLower());
		}
		#endregion
		...
		public void CheckDbVerCompatibilityTest()
		{
			...
			// bool CheckDbVerCompatibility_Bkupd(string svrName, string dbName)
		}
