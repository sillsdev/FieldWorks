...
namespace SIL.Utils
{
	...
	public class ErrorReporter : Form
	{
		...
		public static bool ReportException(Exception error, RegistryKey applicationKey, string emailAddress)
		{
		...
		/// <returns>True if the error was lethal and the user chose to exit the application,
		/// false otherwise.</returns>
		public static bool ReportException(Exception error, RegistryKey applicationKey, string emailAddress, Form parent, bool isLethal)
		{
			...
			if (!string.IsNullOrEmpty(error.HelpLink) && error.HelpLink.IndexOf("::/") > 0 && !string.IsNullOrEmpty(error.Message))
			{
				...
				var lengthOfBasicMessage = error.Message.IndexOf("\r\n\r\n");
				...
				var iSeparatorBetweenFileNameAndTopic = error.HelpLink.IndexOf("::/");
				var sHelpFile = error.HelpLink.Substring(0, iSeparatorBetweenFileNameAndTopic);
				var sHelpTopic = error.HelpLink.Substring(iSeparatorBetweenFileNameAndTopic + 3);
				var caption = ReportingStrings.kstidFieldWorksErrorCaption;
				var appExit = ReportingStrings.kstidFieldWorksErrorExitInfo;
			...
			using (var e = new ErrorReporter(isLethal, emailAddress))
			{
		...
		{
			var e = new ErrorReporter(false, emailAddress)
			{
				m_fUserReport = true
			};
		...
		{
			var e = new ErrorReporter(false, emailAddress, true, errorText)
			{
				m_fUserReport = true
			};
			e.ShowDialog(applicationKey, null, parent);
		...
		{
			ErrorReport.AddProperty(label, contents);
		}
		...
		{
			AutoScaleMode = AutoScaleMode.Font; //put this here so Visual Studio designer works.
			InitializeComponent();
			...
			{
				// the test environment already prohibits dialogs but will save the contents of assertions in some log.
		...
		private static void UpdateCrashCount(RegistryKey applicationKey, string sPropName)
		{
			...
			var count = (int)applicationKey.GetValue(sPropName, 0) + 1;
			applicationKey.SetValue(sPropName, count);
			ErrorReport.AddProperty(sPropName, count.ToString());
		}
		...
		private static void UpdateAppRuntime(RegistryKey applicationKey)
		{
			...
			var cmin = csec / 60;
			ErrorReport.AddProperty("TotalRuntime", $"{cmin / 60}:{cmin % 60:d2}:{csec % 60:d2}");
			if (secBeforeCrash > 0)
			{
				var minBeforeCrash = secBeforeCrash / 60;
				ErrorReport.AddProperty("RuntimeBeforeCrash", $"{minBeforeCrash / 60}:{minBeforeCrash % 60:d2}:{secBeforeCrash % 60:d2}");
			}
		}
		#endregion
		...
		private void btnClose_Click(object sender, System.EventArgs e)
		{
			...
			if (radEmail.Checked)
			{
				...
				{
					MessageBox.Show(this, ReportingStrings.kstidSendFailed, ReportingStrings.kstidSendFailedCaption, MessageBoxButtons.OK, MessageBoxIcon.Warning);
					radSelf.Checked = true;
					return;
				}
			}
			else if (radSelf.Checked)
			{
				if (m_emailAddress != null)
				{
					body = m_fUserReport ? string.Format(ReportingStrings.kstidPleaseEmailThisTo0WithASuitableSubject, m_emailAddress, body)
						: string.Format(ReportingStrings.ksPleaseEMailThisTo0WithThisExactSubject12, m_emailAddress, s_emailSubject, body);
				}
