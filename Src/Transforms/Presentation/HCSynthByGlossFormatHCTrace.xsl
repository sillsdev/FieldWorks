...
-->
	...
	<xsl:param name="prmIconPath">
		<xsl:text>file:///C:/fwrepo/fw/DistFiles/Language Explorer/Configuration/Words/Analyses/TraceParse/</xsl:text>
	</xsl:param>
	...
	<!-- Keys -->
	<xsl:key name="Allomorph" match="//Allomorph" use="Gloss"/>
	<!-- Variables -->
	...
	<xsl:variable name="success">
		<xsl:choose>
			<xsl:when test="//ParseCompleteTrace/@success='true'">
				<xsl:text>true</xsl:text>
			</xsl:when>
			<xsl:otherwise>
	...
	<xsl:template match="Synthesis">
		<html>
			...
			<body style="font-family:Times New Roman">
				<h1>
					<xsl:text>Synthesis of </xsl:text>
					<span>
	...
	<xsl:template name="DetermineIfMoreToShow">
		...
		<xsl:choose>
			<xsl:when test="$traceRoot[name() = 'ParseCompleteTrace']">
				<!-- The synthesis is complete, so there is no more to show. -->
			</xsl:when>
			<xsl:otherwise>
	...
	<xsl:template name="ResultSection">
		...
		<xsl:choose>
			<xsl:when test="$success='true'">
				<p>
					<xsl:text>This analysis synthesized successfully.  The following are the forms that synthesized (the first line is the form, the second line shows the morpheme breaks via a plus sign):</xsl:text>
				</p>
				<div class="">
					<xsl:if test="$prmVernacularRTL='Y'">
						<xsl:attribute name="style">direction:rtl; text-align:right</xsl:attribute>
					</xsl:if>
					<xsl:call-template name="ShowSuccessfulSyntheses"/>
				</div>
			</xsl:when>
			<xsl:otherwise>
				<p>
					<span>
						...
						<xsl:text>This word failed to synthesize successfully.</xsl:text>
					</span>
				</p>
		...
		<xsl:choose>
			<xsl:when test="name() = 'MorphologicalRuleSynthesisTrace' and not(FailureReason) and $moreToShow != 'Y'">
				<span style="unicode-bidi:embed">
					...
					<xsl:text>&#xa0;&#xa0;(Reason: This is a duplicate synthesis and has been pruned.)</xsl:text>
				</span>
			</xsl:when>
					...
					<span style="unicode-bidi:embed">
						...
						<xsl:choose>
							...
							<xsl:when test="@type = 'disjunctiveAllomorph'">
								<xsl:text>The valid synthesis '</xsl:text>
								<span>
									<xsl:attribute name="style">
										<xsl:call-template name="GetVernacularFont"/>
									</xsl:attribute>
								...
								<xsl:text>' takes precedence over this synthesis.</xsl:text>
							</xsl:when>
							<xsl:when test="@type = 'affixInflFeats'">
								<xsl:text>The synthesis's inflection features '</xsl:text>
							...
							<xsl:when test="@type = 'excludedStemName'">
								<xsl:text>The synthesis contains inflectional features that match the stem name '</xsl:text>
								<xsl:value-of select="StemName"/>
									...
									<xsl:when test="Allomorph">
										...
										<span>
											...
											<span>
												...
												<xsl:text> (</xsl:text>
												<xsl:value-of select="Allomorph/Gloss"/>
												<xsl:text> )</xsl:text>
											</span>
										</span>
										<xsl:text>': </xsl:text>
							...
							<xsl:when test="@type = 'partialParse'">
								<xsl:text>This synthesis does not include all analyzed morphemes.</xsl:text>
							</xsl:when>
							...
							<xsl:when test="@type = 'pos'">
								<xsl:text>The synthesis's part of speech '</xsl:text>
...
-->
	<xsl:template name="ShowMorph">
		...
		<span class="interblock">
			<table cellpadding="0" cellspacing="0">
				<xsl:attribute name="style">
					<xsl:text>cursor:default;color:</xsl:text>
					<xsl:choose>
						<xsl:when test="parent::AffixPermutation[count(preceding-sibling::AffixPermutation)=0]">
							<xsl:choose>
								<xsl:when test="$success='true'">
									<xsl:value-of select="$sSuccessColor"/>
								</xsl:when>
				...
				<tr>
					<td>
						...
						<xsl:choose>
							...
							<xsl:otherwise>
								<xsl:value-of select="Allomorph/Category"/>
								<!--								<xsl:call-template name="ShowMsaInfo">
									<xsl:with-param name="msa" select="Allomorph/Category"/>
								</xsl:call-template>-->
							</xsl:otherwise>
						</xsl:choose>
					</td>
	...
	<!--
		- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
		ShowSuccessfulSyntheses
		Show all forms that were generataed
		Parameters: none
		- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	-->
	<xsl:template name="ShowSuccessfulSyntheses">
		<table border="0">
	...
	<!--
		- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
		ShowSuccessfulSynthesis
		Show a successful synthesis
		Parameters: none
		- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	-->
	<xsl:template name="ShowSuccessfulSynthesis">
		<tr>
			<td>
				<table border="1">
					<xsl:attribute name="style">
					...
					<tr>
						...
						<xsl:for-each select="Result">
							<td valign="top">
								...
								<!-- remove morpheme boundary markers -->
								<xsl:value-of select="translate($sForm,'+','')"/>
							</td>
						</xsl:for-each>
					</tr>
					<tr>
		...
		<xsl:if test="$parseNodes">
			<xsl:for-each select="$parseNodes">
				...
				<xsl:variable name="template">
					<xsl:choose>
						<xsl:when test="name($lastTemplateTrace) = 'TemplateSynthesisTraceIn'">
							<xsl:value-of select="$lastTemplateTrace/AffixTemplate"/>
						</xsl:when>
						<xsl:when test="name($lastTemplateTrace) = 'TemplateSynthesisTraceOut'">
							<xsl:text/>
						</xsl:when>
						<xsl:otherwise>
							<xsl:value-of select="$curTemplate"/>
						...
						<td style="border:solid; border-width:thin;">
										...
										<xsl:when test="name() = 'ParseCompleteTrace'">
											...
											<tr>
												<td valign="top" colspan="2">
													...
													<xsl:text>Synthesis completed.</xsl:text>
												</td>
											</tr>
											<tr>
												<td valign="top">
																	...
																	<td>
																		...
																		<xsl:choose>
																			<xsl:when test="$prmVernacularRTL='Y'">
																				...
																				<xsl:text>(Synthesis succeeded)&#xa0;&#xa0;</xsl:text>
																			</xsl:when>
																			<xsl:otherwise>
																				<xsl:text>&#xa0;&#xa0;(Synthesis succeeded!)</xsl:text>
																			</xsl:otherwise>
							...
							<div>
								...
								<xsl:choose>
									...
									<xsl:when test="name() = 'ParseCompleteTrace'">
										<!-- The synthesis is complete, so there are no more records. -->
									</xsl:when>
									<xsl:otherwise>
										<xsl:call-template name="ShowTracePath">
											<xsl:with-param name="traceRoot" select="."/>
	...
	<!--
		- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
		StripOffNulls
		Remove any nulls from the synthesized form
		Parameters: form - the synthesized form
		- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	-->
	<xsl:template name="StripOffNulls">
		<xsl:param name="form"/>
		<xsl:variable name="sNull" select="'^0'"/>
		<xsl:choose>
			<xsl:when test="contains($form,$sNull)">
				<xsl:variable name="sBefore">
					<xsl:value-of select="substring-before($form,$sNull)"/>
				</xsl:variable>
				...
				<xsl:choose>
					...
					<xsl:when test="$lenBefore=($len - 2)">
						<!-- null is at the end -->
						<xsl:value-of select="$sBefore"/>
					</xsl:when>
					<xsl:otherwise>
						<!-- null is in the middle somewhere -->
						<xsl:call-template name="StripOffNulls">
							<xsl:with-param name="form" select="concat($sBefore,substring-after($form,$sNull))"/>
						</xsl:call-template>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:when>
	...
	<xsl:template name="TraceSection">
		...
		<div id="TraceSection" style="display:block">
			...
			<p>
				...
				<xsl:text> has a path to a successful synthesis.  Any item shown in </xsl:text>
				<span>
					<xsl:attribute name="style">
						<xsl:text>color:</xsl:text>
						<xsl:value-of select="$sFailureColor"/>
				...
				<xsl:text> does not have a path to a successful synthesis (i.e. this path failed to produce a successful form).  The reason for the failure is shown at the end of a line as </xsl:text>
				<span>
					<xsl:attribute name="style">
						<xsl:text>color:</xsl:text>
						<xsl:value-of select="$sFailureColor"/>
			...
			<p>
				<xsl:text>This particular synthesizer works as follows:</xsl:text>
				<ol>
					<li>Given a possible analysis, it then builds the word in a generative fashion, from the root out.</li>
					<li>When there are any affixes, it tries them in various permuations of the affixes.</li>
					<li>When there are any inflectional templates that might apply, it applies each template in the order of suffixes from the root out followed by prefixes from the root out.</li>
					<li>Then it applies any phonological rules in their generative order.</li>
					<li>The result is then declared to be successful.</li>
				</ol>
			</p>
			<p>Click on the box by a morpheme to follow a path.</p>
			<div>
				<xsl:if test="$prmVernacularRTL='Y'">
					<xsl:attribute name="style">direction:rtl; text-align:right</xsl:attribute>
				</xsl:if>
