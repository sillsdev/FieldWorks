...
namespace SIL.FieldWorks.ParatextLexiconPlugin
{
	internal class LcmLexicon : DisposableBase, Lexicon, WordAnalyses, IVwNotifyChange
	{
		...
		internal LcmLexicon(string scrTextName, string projectId, LcmCache cache, int defaultVernWs)
		{
		...
		public IEnumerable<Lexeme> Lexemes
		{
			get
			{
				...
				foreach (IWfiWordform wordform in m_cache.ServiceLocator.GetInstance<IWfiWordformRepository>().AllInstances())
				{
					...
					if (wordFormWs != null)
						lexemes.Add(new LcmWordformLexeme(this, new LexemeKey(LexemeType.Word, wordFormWs.Normalize())));
				}
				return lexemes;
			}
		}
		...
		private bool TryGetLexeme(LexemeKey key, out Lexeme lexeme)
		{
			if (key.Type == LexemeType.Word)
			{
				...
				{
					lexeme = new LcmWordformLexeme(this, key);
					return true;
				}
			}
			else
			{
				...
				{
					lexeme = new LcmLexEntryLexeme(this, key);
					return true;
				}
			}
		...
		public Lexeme CreateLexeme(LexemeType type, string lexicalForm)
		{
			if (type == LexemeType.Word)
				return new LcmWordformLexeme(this, new LexemeKey(type, lexicalForm));
			...
			return new LcmLexEntryLexeme(this, new LexemeKey(type, lexicalForm, num));
		}
		...
		public void RemoveLexeme(Lexeme lexeme)
		{
			...
			{
				var entryLexeme = (LcmLexEntryLexeme)lexeme;
				ILexEntry entry;
		...
		{
			return new LcmWordAnalysis(word, lexemes);
		}
		...
		private bool GetWordAnalysis(IWfiAnalysis analysis, out WordAnalysis lexemes)
		{
			...
			lexemes = new LcmWordAnalysis(analysis.Wordform.Form.get_String(DefaultVernWs).Text.Normalize(), lexemeArray);
			return true;
		}
		...
		internal LcmLexEntryLexeme GetEntryLexeme(ILexEntry entry)
		{
			CreateEntryIndexIfNeeded();
			LexemeType type = GetLexemeTypeForMorphType(entry.PrimaryMorphType);
			HomographNumber hn = m_homographNumbers.GetOrCreateValue(entry);
			string form = entry.LexemeFormOA == null ? string.Empty : entry.LexemeFormOA.Form.VernacularDefaultWritingSystem.Text ?? string.Empty;
			return new LcmLexEntryLexeme(this, new LexemeKey(type, form.Normalize(), hn.Number));
		}

		internal void OnLexemeAdded(Lexeme lexeme)
		{
			if (LexemeAdded != null)
				LexemeAdded(this, new LcmLexemeAddedEventArgs(lexeme));
		}

		internal void OnLexiconSenseAdded(Lexeme lexeme, LexiconSense sense)
		{
			if (LexiconSenseAdded != null)
				LexiconSenseAdded(this, new LcmLexiconSenseAddedEventArgs(lexeme, sense));
		}

		internal void OnLexiconGlossAdded(Lexeme lexeme, LexiconSense sense, LanguageText gloss)
		{
			if (LexiconGlossAdded != null)
				LexiconGlossAdded(this, new LcmLexiconGlossAddedEventArgs(lexeme, sense, gloss));
		}
		...
		{
			private readonly LcmLexicon m_lexicon;

			public LexEntryComparer(LcmLexicon lexicon)
			{
				m_lexicon = lexicon;
			}
