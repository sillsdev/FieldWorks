...
{
	#region LcmWordformLexeme class
	/// <summary>
	/// </summary>
	internal class LcmWordformLexeme : Lexeme
	{
		private readonly LexemeKey m_key;
		private readonly LcmLexicon m_lexicon;

		public LcmWordformLexeme(LcmLexicon lexicon, LexemeKey key)
		{
			m_lexicon = lexicon;
			m_key = key;
		}
		...
		{
			var other = obj as LcmWordformLexeme;
		...
		private class WfiGlossLexiconSense : LexiconSense
		{
			...
			public WfiGlossLexiconSense(LcmLexicon lexicon, LexemeKey lexemeKey, IWfiGloss gloss)
			{
				m_lexicon = lexicon;
			...
			public IEnumerable<LanguageText> Glosses
			{
				get
				{
					...
					IMultiUnicode lcmGlosses = m_gloss.Form;
					foreach (CoreWritingSystemDefinition ws in m_lexicon.Cache.ServiceLocator.WritingSystems.CurrentAnalysisWritingSystems)
					{
						ITsString tssGloss = lcmGlosses.StringOrNull(ws.Handle);
						if (tssGloss != null)
							glosses.Add(new LcmLanguageText(ws.Id, tssGloss.Text.Normalize()));
					}
					return glosses;
				}
			}
			...
			public LanguageText AddGloss(string language, string text)
			{
				...
				m_lexicon.OnLexiconGlossAdded(new LcmWordformLexeme(m_lexicon, m_lexemeKey), this, lexGloss);
				return lexGloss;
			}
