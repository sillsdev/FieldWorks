...
{
	#region LcmLexEntryLexeme class
	/// <summary>
	/// </summary>
	internal class LcmLexEntryLexeme : Lexeme
	{
		private readonly LexemeKey m_key;
		private readonly LcmLexicon m_lexicon;

		public LcmLexEntryLexeme(LcmLexicon lexicon, LexemeKey key)
		{
			m_lexicon = lexicon;
			m_key = key;
		}
			...
			get
			{
				...
				foreach (ILexReference lexRef in entry.LexEntryReferences.Union(entry.AllSenses.SelectMany(s => s.LexSenseReferences)))
				{
					...
					foreach (ICmObject obj in lexRef.TargetsRS)
					{
						...
						{
							relations.Add(new LcmLexicalRelation(m_lexicon.GetEntryLexeme(otherEntry),
		...
		{
			var other = obj as LcmLexEntryLexeme;
		...
		private class LexSenseLexiconSense : LexiconSense
		{
			...
			public LexSenseLexiconSense(LcmLexicon lexicon, LexemeKey lexemeKey, ILexSense lexSense)
			{
				m_lexicon = lexicon;
			...
			public IEnumerable<LanguageText> Definitions
			{
				get
				{
					...
					foreach (CoreWritingSystemDefinition ws in m_lexicon.Cache.ServiceLocator.WritingSystems.CurrentAnalysisWritingSystems)
					{
						...
						if (tss != null)
							defs.Add(new LcmLanguageText(ws.Id, tss.Text.Normalize()));
					}
					return defs;
				}
			}
			...
			public IEnumerable<LanguageText> Glosses
			{
				get
				{
					...
					foreach (CoreWritingSystemDefinition ws in m_lexicon.Cache.ServiceLocator.WritingSystems.CurrentAnalysisWritingSystems)
					{
						...
						if (tss != null)
							glosses.Add(new LcmLanguageText(ws.Id, tss.Text.Normalize()));
					}
					return glosses;
				}
			}
				...
				NonUndoableUnitOfWorkHelper.Do(m_lexSense.Cache.ActionHandlerAccessor, () =>
					{
						...
						lexGloss = new LcmLanguageText(language, text);
					});
				m_lexicon.OnLexiconGlossAdded(new LcmLexEntryLexeme(m_lexicon, m_lexemeKey), this, lexGloss);
				return lexGloss;
			}
			...
			public IEnumerable<LexiconSemanticDomain> SemanticDomains
			{
				get
				{
					return m_lexSense.SemanticDomainsRC.Select(sd => new LcmSemanticDomain(sd.ShortName.Normalize())).ToArray();
				}
			}
