...
namespace LCMBrowser
{
	...
	public class InspectorGrid : DataGridView
	{
		...
		public InspectorGrid()
		{
			...
			m_clrShading = CalculateColor(DefaultCellStyle.ForeColor, DefaultCellStyle.BackColor, 25);
		...
		{
			var currLevel = m_list[currRow].Level;
			firstRowInBlock = lastRowInBlock = currRow;
			for (var i = firstRowInBlock + 1; i < RowCount && m_list[i].Level > currLevel; i++)
			{
				lastRowInBlock = i;
			}
		}
		...
		private Rectangle GetBlockRectangle()
		{
			...
			var rcFirst = GetCellDisplayRectangle(0, Math.Min(maxRow, m_firstRowInShadind), false);
			var rcLast = GetCellDisplayRectangle(0, Math.Min(maxRow, m_lastRowInShading), false);
			if (rcFirst.Height > 0)
			{
				rc.Y = rcFirst.Y;
			}
		...
		protected override void OnCellPainting(DataGridViewCellPaintingEventArgs e)
		{
			...
			if (e.Handled || e.RowIndex < 0 || e.ColumnIndex < 0 || m_list == null || m_list.Count == 0 || m_list[e.RowIndex] == null)
			{
				return;
			}
			...
			var io = m_list[e.RowIndex];
			var rcText = e.CellBounds;
			...
			if (isIndentedCell)
			{
				...
				var dx = ((rcHotSpot.Right - rcText.X) + 5);
				rcText.X += dx;
				rcText.Width -= dx;
			}

			// Draw the background color for the cell.
			using (var br = new SolidBrush(DefaultCellStyle.BackColor))
			{
				if (isSelected)
				{
			...
			if (io.HasChildren)
			{
				...
				e.Graphics.DrawImage(m_list.IsExpanded(e.RowIndex) ? Properties.Resources.kimidCollapse : Properties.Resources.kimidExpand, rcHotSpot);
			}
		}
		...
		/// Draws the borders.
		/// </summary>
		private void DrawBorders(DataGridViewCellPaintingEventArgs e, bool isIndentedCell, Color clrFadedBottomBorder, int dxBottomBorder)
		{
			if (e.RowIndex < 0)
			{
				return;
			...
			Point pt1;
			Point pt2;
			using (var pen = new Pen(m_clrGrid))
			{
			...
			if (isIndentedCell)
			{
				...
				var rc = new Rectangle(pt1.X, pt1.Y, pt2.X - pt1.X + 1, 1);
				using (var br = new LinearGradientBrush(rc, clrFadedBottomBorder, m_clrGrid, (float)0.0))
				{
		...
		private void DrawTreeLines(DataGridViewCellPaintingEventArgs e, Rectangle rcHotSpot, int level)
		{
			using (var pen = new Pen(SystemColors.GrayText))
			{
				...
				var pt1 = new Point(rcHotSpot.X + rcHotSpot.Width / 2, rcHotSpot.Y + rcHotSpot.Height / 2);
				var pt2 = new Point(rcHotSpot.Right + 4, pt1.Y);
		...
		protected override void OnKeyDown(KeyEventArgs e)
		{
			...
			var i = CurrentCellAddress.Y;
			if (i >= 0 && i < m_list.Count && m_list[i].HasChildren)
			{
				if ((e.KeyCode == Keys.Left && m_list.IsExpanded(i)) || (e.KeyCode == Keys.Right && !m_list.IsExpanded(i)))
				{
					ToggleExpand(i);
				}
			}
		...
		protected override void OnCellClick(DataGridViewCellEventArgs e)
		{
			...
			var io = m_list[e.RowIndex];
			var rc = GetCellDisplayRectangle(0, e.RowIndex, true);
			var rcHotSpot = GetExpandCollapseRect(rc, io.Level);
		...
		private void ToggleExpand(int irow)
		{
			...
			var rc = GetCellDisplayRectangle(0, irow, true);
			var dy = ClientRectangle.Bottom - rc.Bottom;
			rc.Y = rc.Bottom;
			rc.Height = dy;
		...
		public void CreateDefaultColumns()
		{
			...
			if (LCMBrowserForm.m_updateFlag)
			{
				col.ReadOnly = false;
			}
