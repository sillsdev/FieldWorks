	...
	TsTextProps ** ppzttp)
{
	...
	if (ptph->Find(prgtip, ctip, prgtsp, ctsp, uHash, &pzttp))
	{
...
TsTextProps::~TsTextProps(void)
{
	TsPropsHolder * ptph = TsPropsHolder::GetPropsHolder(false);
...
/*----------------------------------------------------------------------------------------------
	AddRef.
----------------------------------------------------------------------------------------------*/
STDMETHODIMP_(ULONG) TsTextProps::AddRef(void)
{
	Assert(m_cref > 0);
	return ++m_cref;
}
...
/*----------------------------------------------------------------------------------------------
	Release.
----------------------------------------------------------------------------------------------*/
STDMETHODIMP_(ULONG) TsTextProps::Release(void)
{
	Assert(m_cref > 0);
	if (--m_cref > 0)
		return m_cref;
	...
	case ktptBold:
		{
			...
			stu.Format(L"{%s:%s}", tpt==ktptItalic ? L"ital" : L"bold", pchVal);
			break;
		}
...
----------------------------------------------------------------------------------------------*/
TsPropsHolder * TsPropsHolder::GetPropsHolder(bool fCreate)
{
	// Used to be: Get the property pool from thread local storage. If it hasn't yet been created
	// and fCreate is true, go ahead and create it.
	//
	// (EberhardB/TimS): If we use thread local storage we're getting problems when the .NET
	// Finalizer disposes TsTextProps because the main thread might already have exited (which
	// called ThreadDetach which already released the thread local storage) [since our DLLs are
	// marked as STA the Finalizer calls are executed on the same thread as the main thread].
	// Since we aren't really supporting multithreading in the C++ code anyways we decided to use
	// a global variable to hold the property pool instead.

	//TextServGlobals * ptsg = TextServGlobals::GetTsGlobals();

	//if (!ptsg->m_ptph)
	//{
	//	if (!fCreate)
	//		return NULL;
	//	ptsg->m_ptph = NewObj TsPropsHolder;
	//}
	//return ptsg->m_ptph;
...
/*----------------------------------------------------------------------------------------------
	Get the string holder from thread local storage. If it hasn't been created yet, go ahead
	and create it.
----------------------------------------------------------------------------------------------*/
TsStrHolder * TsStrHolder::GetStrHolder(void)
{
	...
	if (!ptsg->m_ptsh)
		ptsg->m_ptsh = NewObj TsStrHolder;
	return ptsg->m_ptsh;
}
...
const OLECHAR * TsStrHolder::GetString(int hstu)
{
	...
	{
		int ishe = IndexFromCookie(hstu);
		return m_vshe[ishe].m_stu.Chars();
	}
	else
		return NULL;
}
...
/*----------------------------------------------------------------------------------------------
	AddRef.
----------------------------------------------------------------------------------------------*/
STDMETHODIMP_(ULONG) TsPropsBldr::AddRef(void)
{
	Assert(m_cref > 0);
	return ++m_cref;
}
...
/*----------------------------------------------------------------------------------------------
	Release.
----------------------------------------------------------------------------------------------*/
STDMETHODIMP_(ULONG) TsPropsBldr::Release(void)
{
	Assert(m_cref > 0);
	if (--m_cref > 0)
		return m_cref;
...
STDMETHODIMP TsPropsBldr::GetTextProps(ITsTextProps ** ppttp)
{
	...
	TsTextProps::Create(m_vtip.Begin(), m_vtip.Size(), vtsp.Begin(), vtsp.Size(), ppttp);
