...
namespace TestFwKernel
{
	...
	class TestTsString : public unitpp::suite
	{
		...
		void testStringText()
		{
			...
			unitpp::assert_true("qtss1->get_Text returned proper value", sbstr1 == g_pszTest);
			SmartBstr sbstr2;
			hr = m_qtssTwoRuns->get_Text(&sbstr2);
			unitpp::assert_eq("qtss2->get_Text succeeded", S_OK, hr);
			unitpp::assert_true("qtss2->get_Text returned proper value", sbstr2 == g_pszTest);
			...
			unitpp::assert_true("qtss1->GetChars returned proper value", sbstr1 == g_pszTest);
			hr = m_qtssTwoRuns->GetChars(0, cch2, &sbstr2);
			unitpp::assert_eq("qtss2->GetChars succeeded", S_OK, hr);
			unitpp::assert_true("qtss2->GetChars returned proper value", sbstr2 == g_pszTest);
			...
			unitpp::assert_true("qtss1->FetchChars returned proper value", stu1 == g_pszTest);
			...
			unitpp::assert_true("qtss2->FetchChars returned proper value", stu2 == g_pszTest);
			...
			unitpp::assert_true("qtss1->LockText returned proper value", stu1 == g_pszTest);
			...
			unitpp::assert_true("qtss2->LockText returned proper value", stu2 == g_pszTest);
...
#define MUSICAL_SYMBOL_COMBINING_STEM L"\xD834\xDD65" // 1D165
			...
			for (int itss = 0; itss <= 1; itss++)
			{
				...
				for (int inm = 0; inm < g_cnmNormalizations; inm++)
				{
					...
					unitpp::assert_true(sta.Chars(),
						wcscmp(sbstrResult.Chars(), rgpsz[inm]) == 0);
					ITsTextPropsPtr qttpInput;
					ITsTextPropsPtr qttpResult;
		...
		void testPartialComposition()
		{
			...
			for (int inm = 0; inm < g_cnmNormalizations; inm++)
			{
				...
				unitpp::assert_true(sta.Chars(), wcscmp(sbstrResult.Chars(), rgpsz[inm]) == 0);
				int crun;
				hr = qtssResult->get_RunCount(&crun);
...
#define o_WITH_DOT_ABOVE L"\x022F"				// composition of o COMBINING_DOT_ABOVE
		...
		void testStackedDiacritics()
		{
			...
			staOutput1.Format("<Str>%n"
				"<Run ws=\"xstk\" fontsize=\"20000\" fontsizeUnit=\"mpt\" forecolor=\"green\">"
			...
			staOutput2.Format("<Str>%n"
			   ...
			   "<Run ws=\"xstk\" fontsize=\"20000\" fontsizeUnit=\"mpt\" forecolor=\"2f60ff\">"
					"\xCC\xA5</Run>%n"
				"<Run ws=\"xstk\" fontsize=\"20000\" fontsizeUnit=\"mpt\" forecolor=\"green\">"
					"\xCC\x96</Run>%n"
				"<Run ws=\"xstk\" fontsize=\"20000\" fontsizeUnit=\"mpt\" forecolor=\"red\">"
					"\xCC\x8B</Run>%n"
				"<Run ws=\"xstk\" fontsize=\"20000\" fontsizeUnit=\"mpt\" forecolor=\"green\">"
					"lc\xCC\x98\xCC\xA0o</Run>%n"
				"<Run ws=\"xstk\" fontsize=\"20000\" fontsizeUnit=\"mpt\" forecolor=\"blue\">"
					"\xCC\x82</Run>%n"
				"<Run ws=\"xstk\" fontsize=\"20000\" fontsizeUnit=\"mpt\" forecolor=\"green\">"
					"m\xCC\xBC</Run>%n"
				"<Run ws=\"xstk\" fontsize=\"20000\" fontsizeUnit=\"mpt\" forecolor=\"red\">"
				...
				"<Run ws=\"xstk\" fontsize=\"20000\" fontsizeUnit=\"mpt\" forecolor=\"black\">"
					"\xCC\x82</Run>%n"
				"<Run ws=\"xstk\" fontsize=\"20000\" fontsizeUnit=\"mpt\" forecolor=\"green\">"
					" W\xC8\xAF</Run>%n"
				"<Run ws=\"xstk\" fontsize=\"20000\" fontsizeUnit=\"mpt\" forecolor=\"black\">"
...
#define NUM_OFFSETS 14
		...
		void TestOneSubstring(ITsString * ptss, int ichMin, int ichLim, char * label)
		{
	...
	public:
		...
		void testSubstring()
		{
			...
			OLECHAR * prgchTestMulti = L"This is a longer string for more complex tests";
		...
		virtual void Setup()
		{
			...
			TsStrBldr * pztsb1 = 0;
			TsStrBldr::Create(g_pszTest, g_cchTest, rgrun, 1, &pztsb1);
			pztsb1->GetString(&m_qtssOneRun);
			pztsb1->Release();
			...
			TsStrBldr * pztsb2 = 0;
			TsStrBldr::Create(g_pszTest, g_cchTest, rgrun, 2, &pztsb2);
			pztsb2->GetString(&m_qtssTwoRuns);
			pztsb2->Release();
