...
/*----------------------------------------------------------------------------------------------
	Implementation of IStreamWrapper.
	Hungarian: zstf
----------------------------------------------------------------------------------------------*/
class TsStreamWrapper : public ITsStreamWrapper
{
public:
	static void CreateCom(IUnknown * punkOuter, REFIID iid, void ** ppv);

	TsStreamWrapper();
	virtual ~TsStreamWrapper();
	...
	// ITsStreamWrapper methods
	STDMETHOD(get_Stream)( IStream ** ppstrm);
	STDMETHOD(get_Contents)(BSTR * pbstr);
	STDMETHOD(put_Contents)(BSTR bstr);
	STDMETHOD(WriteTssAsXml)(ITsString * ptss, ILgWritingSystemFactory * pwsf, int cchIndent,
		int ws, ComBool fWriteObjData);
	STDMETHOD(ReadTssFromXml)(ILgWritingSystemFactory * pwsf,ITsString ** pptss);

protected:
	int m_cref;
	StrAnsiStreamPtr m_qstas;
};
