...
/*----------------------------------------------------------------------------------------------
	AddRef.
----------------------------------------------------------------------------------------------*/
template<class TxtBuf> STDMETHODIMP_(ULONG) TsStrBase<TxtBuf>::AddRef(void)
{
	AssertObj(this);
	Assert(m_cref > 0);
	return ++m_cref;
...
/*----------------------------------------------------------------------------------------------
	Release.
----------------------------------------------------------------------------------------------*/
template<class TxtBuf> STDMETHODIMP_(ULONG) TsStrBase<TxtBuf>::Release(void)
{
	AssertObj(this);
	Assert(m_cref > 0);
	if (--m_cref > 0)
...
template<class TxtBuf> STDMETHODIMP TsStrBase<TxtBuf>::get_MinOfRun(int irun, int * pichMin)
{
	...
	if ((uint)irun >= (uint)Crun())
		ThrowHr(WarnHr(E_INVALIDARG));
	AssertObj(this);

	*pichMin = IchMinRun(irun);
...
template<class TxtBuf> STDMETHODIMP TsStrBase<TxtBuf>::get_RunText(int irun, BSTR * pbstr)
{
	...
	if (Cch())
	{
		*pbstr = SysAllocStringLen(Prgch() + IchMinRun(irun), CchRun(irun));
		if (!*pbstr)
			ThrowOutOfMemory();
	}
	else
	...
	BSTR * pbstr)
{
	...
	*pbstr = NULL;
	if (ichMin < ichLim)
	{
		*pbstr = SysAllocStringLen(Prgch() + ichMin, ichLim - ichMin);
		if (!*pbstr)
			ThrowOutOfMemory();
	}
	END_COM_METHOD(g_fact, IID_ITsString);
	...
	ITsString ** pptssRet)
{
	...
	DataReaderRgb drr(Prgch() + ichMin, cch * isizeof(OLECHAR));
	if (Crun() == 1)
	{
		// optimized special case.
		TsStrSingle::Create(&drr, cch, Prun(0)->m_qttp, pptssRet);
		return S_OK;
	}
	int irunMin = IrunAt(ichMin); // run containing first character
	if (ichLim == ichMin)
	{
		TsStrSingle::Create(&drr, cch, Prun(irunMin)->m_qttp, pptssRet);
		return S_OK;
	}
	int irunLast = IrunAt(ichLim - 1); // run containing last character
	...
	OLECHAR * prgch)
{
	...
	if (ichMin < ichLim)
		CopyItems(Prgch() + ichMin, prgch, ichLim - ichMin);
...
template<class TxtBuf> STDMETHODIMP TsStrBase<TxtBuf>::GetBldr(ITsStrBldr ** pptsb)
{
	...
	TsStrBldr::Create(Prgch(), Cch(), Prun(0), Crun(), pptsb);
...
template<class TxtBuf> STDMETHODIMP TsStrBase<TxtBuf>::GetIncBldr(ITsIncStrBldr ** pptisb)
{
	...
	TsIncStrBldr::Create(Prgch(), Cch(), Prun(0), Crun(), pptisb);
	...
	ComBool * pfEqual)
{
	...
	*pfEqual = false;
	...
	if (cch != Cch() || wcscmp(pwrgch, Prgch()) != 0)
	{
	...
	ComBool fUseRFC4646)
{
	...
	for (int irun = 0; irun < crun; ++irun)
	{
		...
		if (sbstrPropVal.Length() && !fWriteObjData)
		{
			...
			if (chType == kodtPictEven || chType == kodtPictOdd
		...
		for (int itsp = 0; itsp < ctsp; itsp++)
		{
			...
			if (tpt == ktptObjData)
			{
				...
				case kodtEmbeddedObjectData:
					Assert(sbstrPropVal.Chars()[0] != kodtEmbeddedObjectData);
					break;
...
void TsStrSingle::Create(DataReader * pdrdr, int cch, ITsTextProps * pttp, TsStrSingle ** ppsts)
{
...
#if 0
	// Use empty properties if there are no characters.
	// We thought this might be a good idea at one point, because it is slightly
	// weird for empty strings to be unequal, just because of different text
	// props. However, if we don't keep the props in an empty string, there is no
	// way for an empty paragraph to know what writing system etc. it is supposed to have.
	// Also, there is no way to use a template object that has empty strings to
	// indicate expected encodings for certain properties.
	if (!cch)
	{
		pttp = NULL;
	}
	...
	// Create default properties if none are passed in.
	if (!pttp)
		TsTextProps::Create(NULL, 0, NULL, 0, &qsts->m_run.m_qttp);
	else
...
/*----------------------------------------------------------------------------------------------
	AddRef.
----------------------------------------------------------------------------------------------*/
STDMETHODIMP_(ULONG) TsIncStrBldr::AddRef(void)
{
	Assert(m_cref > 0);
	return ++m_cref;
}
...
/*----------------------------------------------------------------------------------------------
	Release.
----------------------------------------------------------------------------------------------*/
STDMETHODIMP_(ULONG) TsIncStrBldr::Release(void)
{
	Assert(m_cref > 0);
	if (--m_cref > 0)
		return m_cref;
...
STDMETHODIMP TsIncStrBldr::SetStrPropValue(int ttpt, BSTR bstrVal)
{
	...
	return m_qtpb->SetStrPropValue(ttpt, bstrVal);
...
// The class factory for StreamWrapper.
static GenericFactory g_factSf(
	_T("FieldWorks.TsStreamWrapper"),
	&CLSID_TsStreamWrapper,
	_T("FieldWorks stream factory"),
	_T("Apartment"),
	&TsStreamWrapper::CreateCom);


/*----------------------------------------------------------------------------------------------
	Static method called by the class factory to create a string builder.
----------------------------------------------------------------------------------------------*/
void TsStreamWrapper::CreateCom(IUnknown * punkOuter, REFIID iid, void ** ppv)
{
	AssertPtr(ppv);
	Assert(!*ppv);
	...
	ComSmartPtr<TsStreamWrapper> qzsf;

	qzsf.Attach(NewObj TsStreamWrapper());
	CheckHr(qzsf->QueryInterface(iid, ppv));
}

/*----------------------------------------------------------------------------------------------
	Constructor
----------------------------------------------------------------------------------------------*/
 TsStreamWrapper::TsStreamWrapper()
{
	ModuleEntry::ModuleAddRef();
	m_cref = 1;
	StrAnsiStream::Create(&m_qstas);
}
...
/*----------------------------------------------------------------------------------------------
	QueryInterface.
----------------------------------------------------------------------------------------------*/
STDMETHODIMP TsStreamWrapper::QueryInterface(REFIID iid, void ** ppv)
{
	AssertPtr(ppv);
	if (!ppv)
		return WarnHr(E_POINTER);
	*ppv = NULL;
	...
	if (iid == IID_IUnknown)
		*ppv = static_cast<IUnknown *>(this);
	else if (iid == IID_ITsStreamWrapper)
		*ppv = static_cast<ITsStreamWrapper *>(this);
	else if (iid == IID_ISupportErrorInfo)
	{
		*ppv = NewObj CSupportErrorInfo(this, IID_ITsStreamWrapper);
		return S_OK;
	}
	else
		return E_NOINTERFACE;
...
/*----------------------------------------------------------------------------------------------
	AddRef.
----------------------------------------------------------------------------------------------*/
STDMETHODIMP_(ULONG) TsStreamWrapper::AddRef(void)
{
	Assert(m_cref > 0);
	return ++m_cref;
}
...
/*----------------------------------------------------------------------------------------------
	Release.
----------------------------------------------------------------------------------------------*/
STDMETHODIMP_(ULONG) TsStreamWrapper::Release(void)
{
	Assert(m_cref > 0);
	if (--m_cref > 0)
		return m_cref;
...
----------------------------------------------------------------------------------------------*/
STDMETHODIMP TsStreamWrapper::get_Stream( IStream ** ppstrm)
{
	BEGIN_COM_METHOD;
	ChkComOutPtr(ppstrm);
	*ppstrm = m_qstas;
	(*ppstrm)->AddRef();
	END_COM_METHOD(g_factSf, IID_ITsStreamWrapper);
}

/*----------------------------------------------------------------------------------------------
	Get the current contents of the stream, bytes zero-extended to make a BSTR.
----------------------------------------------------------------------------------------------*/
STDMETHODIMP TsStreamWrapper::get_Contents(BSTR * pbstr)
{
	BEGIN_COM_METHOD;
	ChkComOutPtr(pbstr);
	int len = m_qstas->m_sta.Length();
	if (len == 0)
		return S_OK;// result is null
	BSTR bstr = ::SysAllocStringLen(NULL, len);
	if (bstr == NULL)
		ThrowHr(WarnHr(E_OUTOFMEMORY));
	::MultiByteToWideChar(28591, 0, m_qstas->m_sta.Chars(), m_qstas->m_sta.Length(),
		bstr, m_qstas->m_sta.Length());
	*pbstr = bstr;
	END_COM_METHOD(g_factSf, IID_ITsStreamWrapper);
...
/*----------------------------------------------------------------------------------------------
	Set the current contents of the stream, bytes zero-extended to make a BSTR.
----------------------------------------------------------------------------------------------*/
STDMETHODIMP TsStreamWrapper::put_Contents(BSTR bstr)
{
	BEGIN_COM_METHOD;
	ChkComBstrArgN(bstr);
	int cch = BstrLen(bstr);
	char * pch;
	m_qstas->m_sta.SetSize(cch, &pch);
	LARGE_INTEGER pos;
	pos.QuadPart = 0;
	CheckHr(m_qstas->Seek(pos, STREAM_SEEK_SET, NULL));
	if (cch) // error to call wdtmb with 0 chars.
	{
		if (!::WideCharToMultiByte(28591, 0, bstr, cch, pch, cch, NULL, NULL))
			ThrowHr(WarnHr(E_FAIL)); // zero return value indicates error
	}
	END_COM_METHOD(g_factSf, IID_ITsStreamWrapper);
}

/*----------------------------------------------------------------------------------------------
	Call ptss->WriteAsXml(this.Stream, cchIndent, ws, fWriteObjData).
----------------------------------------------------------------------------------------------*/
STDMETHODIMP TsStreamWrapper::WriteTssAsXml(ITsString * ptss, ILgWritingSystemFactory * pwsf,
	int cchIndent, int ws, ComBool fWriteObjData)
{
	...
	CheckHr(ptss->WriteAsXml(m_qstas, pwsf, cchIndent, ws, fWriteObjData));
...
public:
	...
	void LogMessage(const char * pszMsg) { }; // no logging currently.
	...
	//:> Data used by the various methods, including the callback functions.
	...
	FILE * m_pfileLog;				// Old fashioned C FILE pointer for log output file (not used here).
	...
	// We do not use it, but one routine in FwXmlStrings requires it to be defined.
	...
	ILgWritingSystemFactory * m_pwsf;
};
...
int FwXmlImportData::GetWsFromIcuLocale(const char * pszWs, int stidErrMsg)
{
	int ws;
	StrUni stuWs(pszWs);
	// This is necessary to be sure it has been created, if the factory knows how.
	// GetWsFromStr knows about all Wss in a database factory, but not the registry-based one.
	IWritingSystemPtr qwseng;
	CheckHr(m_pwsf->get_Engine(stuWs.Bstr(), &qwseng));

	CheckHr(m_pwsf->GetWsFromStr(stuWs.Bstr(), &ws));
	if (ws == 0)
		ThrowHr(WarnHr(E_FAIL));
	return ws;
}

// Since we're not logging errors, we don't need the error resource strings.
// But the shared code needs the constants to be defined
#ifndef kstidXmlErrorMsg001
#define kstidXmlErrorMsg001 0
#endif

#ifndef kstidXmlDebugMsg003
#define kstidXmlDebugMsg003 0
#endif

#ifndef kstidXmlDebugMsg005
#define kstidXmlDebugMsg005 0
#endif

#ifndef kstidXmlErrorMsg011
#define kstidXmlErrorMsg011 0
#endif

#ifndef kstidXmlErrorMsg033
#define kstidXmlErrorMsg033 0
#endif

#ifndef kstidXmlErrorMsg039
#define kstidXmlErrorMsg039 0
#endif

#ifndef kstidXmlErrorMsg042
#define kstidXmlErrorMsg042 0
#endif

#ifndef kstidXmlErrorMsg043
#define kstidXmlErrorMsg043 0
#endif

#ifndef kstidXmlErrorMsg049
#define kstidXmlErrorMsg049 0
#endif
...
#ifndef kstidXmlErrorMsg056
#define kstidXmlErrorMsg056 0
#endif

#ifndef kstidXmlErrorMsg066
#define kstidXmlErrorMsg066 0
#endif

#ifndef kstidXmlErrorMsg067
#define kstidXmlErrorMsg067 0
#endif

#ifndef kstidXmlErrorMsg068
#define kstidXmlErrorMsg068 0
#endif

#ifndef kstidXmlErrorMsg069
#define kstidXmlErrorMsg069 0
#endif

#ifndef kstidXmlErrorMsg070
#define kstidXmlErrorMsg070 0
#endif

#ifndef kstidXmlErrorMsg071
#define kstidXmlErrorMsg071 0
#endif

#ifndef kstidXmlErrorMsg072
#define kstidXmlErrorMsg072 0
#endif

#ifndef kstidXmlErrorMsg073
#define kstidXmlErrorMsg073 0
#endif

#ifndef kstidXmlErrorMsg074
#define kstidXmlErrorMsg074 0
#endif

#ifndef kstidXmlErrorMsg132
#define kstidXmlErrorMsg132 0
#endif

#ifndef kstidXmlErrorMsg133
#define kstidXmlErrorMsg133 0
#endif

#ifndef kstidXmlErrorMsg155
#define kstidXmlErrorMsg155 0
#endif
// Implement the FwXml string-reading methods for our FwXmlImportData struct.
#include "..\cellar\FwXmlString.cpp"

//These two functions are copied from Cellar/FwXml.cpp (to avoid a drastic reorganization of
//the cellar files into libraries).
/*----------------------------------------------------------------------------------------------
...
----------------------------------------------------------------------------------------------*/
const char * FwXml::GetAttributeValue(const XML_Char ** prgpszAtts, const char * pszName)
{
	if (!prgpszAtts)
		return NULL;
...
// This just handles the outermost tag, which had better be "Str", by setting up
// the FwXml handler.
void HandleOuterStartTag(void * pvUser, const XML_Char * pszName,
	const XML_Char ** prgpszAtts)
{
	FwXmlImportData * pxid = reinterpret_cast<FwXmlImportData *>(pvUser);
	if (strcmp(pszName, "Str"))
		ThrowHr(WarnHr(E_FAIL));
	XML_SetElementHandler(pxid->m_parser, FwXml::HandleStringStartTag,
		FwXml::HandleStringEndTag);
	pxid->m_fInString = true;
}

void HandleOuterEndTag(void * pvUser, const XML_Char * pszName)
{
	// nothing to do.
}
...
/*----------------------------------------------------------------------------------------------
	Parse the XML data to create a TsString..
----------------------------------------------------------------------------------------------*/
STDMETHODIMP TsStreamWrapper::ReadTssFromXml(ILgWritingSystemFactory * pwsf, ITsString ** pptss)
{
	BEGIN_COM_METHOD;
	ChkComArgPtr(pwsf);
	ChkComOutPtr(pptss);
	...
	xid.m_parser = XML_ParserCreate(NULL);

	XML_SetUserData(xid.m_parser, &xid);

	XML_SetElementHandler(xid.m_parser, HandleOuterStartTag,
		HandleOuterEndTag);
	XML_SetCharacterDataHandler(xid.m_parser, FwXml::HandleCharData);

	xid.m_startOuterHandler = HandleOuterStartTag;
	xid.m_endOuterHandler = HandleOuterEndTag;
	try
	{
		for (;;)
		{
			...
			CheckHr(m_qstas->Read(pBuffer, READ_SIZE, &cbRead));
			...
			CheckHr(m_qstas->Seek(libMove, STREAM_SEEK_CUR, &ulibPos));
			if (ulibPos.LowPart == (uint)m_qstas->m_sta.Length())
			{
	...
	// now make the string. A chunk of this code is copied from FwXmlImportData::HandleEndTag2,
	// but I don't see a straightforward way to refactor for sharing it.
	// A more natural approach for this context would be to use string builders and props builders,
	// but I wanted to reuse the main string parsing code, which is aimed at converting directly
	// from XML to the format stored in the database. This means making the binary form of the
	// format data, and then deserializing it.
	Vector<byte> vbBin;

	// Calculate the number of bytes needed for run information, and assign or
	// allocate the output buffer.
	int crun = xid.m_vbri.Size();
	Assert(crun >= xid.m_vrpi.Size());
	int cbBin = isizeof(crun);
	if (crun)
	{
		cbBin += crun * isizeof(FwXml::BasicRunInfo);
		for (int i = 0; i < xid.m_vrpi.Size(); ++i)
		{
			cbBin += 2;
			cbBin += xid.m_vrpi[i].m_vbRawProps.Size();
		}
	}
	vbBin.Resize(cbBin);
	...
	if (crun)
	{
		memcpy(prgbBin + cbBin, xid.m_vbri.Begin(),
			crun * isizeof(FwXml::BasicRunInfo));
		cbBin += crun * isizeof(FwXml::BasicRunInfo);
		for (int i = 0; i < xid.m_vrpi.Size(); ++i)
		{
			prgbBin[cbBin] = xid.m_vrpi[i].m_ctip;
			++cbBin;
			prgbBin[cbBin] = xid.m_vrpi[i].m_ctsp;
			++cbBin;
			memcpy(prgbBin + cbBin, xid.m_vrpi[i].m_vbRawProps.Begin(),
				xid.m_vrpi[i].m_vbRawProps.Size());
			cbBin += xid.m_vrpi[i].m_vbRawProps.Size();
		}
	}
	ITsStrFactoryPtr qtsf;
	qtsf.CreateInstance(CLSID_TsStrFactory);
	CheckHr(qtsf->DeserializeStringRgb(xid.m_stuChars.Bstr(), prgbBin, cbBin,
		pptss));
