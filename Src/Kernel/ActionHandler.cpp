...
STDMETHODIMP_(UCOMINT32) ActionHandler::AddRef(void)
{
	Assert(m_cref > 0);
	return ++m_cref;
}

STDMETHODIMP_(UCOMINT32) ActionHandler::Release(void)
{
	Assert(m_cref > 0);
	if (--m_cref > 0)
		return m_cref;
...
/*----------------------------------------------------------------------------------------------
	${IActionHandler#BeginNonUndoableTask}
----------------------------------------------------------------------------------------------*/
STDMETHODIMP ActionHandler::BeginNonUndoableTask()
{
	Assert(false);
	return E_NOTIMPL;
}
...
/*----------------------------------------------------------------------------------------------
	${IActionHandler#EndNonUndoableTask}
----------------------------------------------------------------------------------------------*/
STDMETHODIMP ActionHandler::EndNonUndoableTask()
{
	Assert(false);
	return E_NOTIMPL;
}
...
//#endif // _DEBUG
	...
	ComBool f;
	CheckHr(puact->get_IsDataChange(&f));
	if (f)
	{
...
void ActionHandler::CleanUpEmptyTasks()
{
	...
	{
		ComBool f;
		CheckHr(m_vquact[iuact]->get_IsDataChange(&f));
		if (f)
...
STDMETHODIMP ActionHandler::get_TasksSinceMark(ComBool fUndo, ComBool * pf)
{
	...
	*pf = false; // default answer.
	...
	for (int iuact = iuactStart; iuact < iuactEnd; iuact++)
	{
		...
		CheckHr(m_vquact[iuact]->get_IsDataChange(&fIsDataChange));
		if (fIsDataChange)
		{
			*pf = true;
			return S_OK;
...
template class Vector<IUndoActionPtr>;
template class Vector<SmartBstr>;
