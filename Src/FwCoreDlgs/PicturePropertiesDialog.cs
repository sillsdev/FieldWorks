...
namespace SIL.FieldWorks.FwCoreDlgs
{
	...
	public class PicturePropertiesDialog : Form, IFWDisposable
	{
		...
		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Gets the path to the file the user chose.
		/// </summary>
		/// ------------------------------------------------------------------------------------
		...
		private void m_btnBrowseDest_Click(object sender, EventArgs e)
		{
			...
			using (FolderBrowserDialog dlg = new FolderBrowserDialog())
			{
				...
				dlg.Description = String.Format(FwCoreDlgs.kstidSelectExternalLinksSubFolder,
		...
		{
			if (!IsFolderInExternalLinksFolder(proposedDestFolder))
			{
				MessageBox.Show(this, String.Format(FwCoreDlgs.kstidDestFolderMustBeInExtLinks,
					m_cache.LangProject.ExternalLinkRootDir),
		...
		private void UpdatePicInformation()
		{
			...
			else
			{
				...
				{
					m_grpFileLocOptions.Visible = !FileIsInExternalLinksFolder(tmpOriginalPath);
				}
			}
		...
		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Determines whether file is in the external links folder.
		/// </summary>
		/// <param name="sFilePath">The file path.</param>
		/// ------------------------------------------------------------------------------------
		private bool FileIsInExternalLinksFolder(string sFilePath)
		{
		...
		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Determines whether given folder is in external links folder.
		/// </summary>
		/// <param name="sFolder">The full path of the folder to check.</param>
		/// ------------------------------------------------------------------------------------
		private bool IsFolderInExternalLinksFolder(string sFolder)
		{
			if (!Directory.Exists(sFolder))
				return false;
			sFolder = sFolder.ToLowerInvariant().TrimEnd(Path.DirectorySeparatorChar);
			string sExtLinksRoot = m_cache.LangProject.ExternalLinkRootDir;
