...
namespace SIL.FieldWorks.FwCoreDlgs
{
	...
	public partial class PunctuationDlg : Form
	{
			...
			: this()
		{
			...
			m_fntVern = new Font(m_ws.DefaultFontName, 12);
			...
			contextCtrl.Initialize(cache, wsContainer, m_ws, m_app, m_fntVern, gridPatterns);
		...
		/// ------------------------------------------------------------------------------------
		private static string GetLocalizedContextPosition(ContextPosition contextPos)
		{
		...
		private void CellEditTextBoxTextChanged(object sender, EventArgs e)
		{
			...
			{
				int code = ch;
				if ((m_chrPropEng.get_IsControl(code) ||
		...
		void LoadException(ArgumentException e)
		{
			ErrorReporter.ReportException(e, m_app.SettingsKey,
				m_app.SupportEmailAddress, this, false);
		}
		...
		{
			gridMatchedPairs.CurrentCell = gridMatchedPairs[kiColOpen, gridMatchedPairs.NewRowIndex];
		...
		private void contextCtrl_BeforeTextTokenSubStringsLoaded(string filename)
		{
			...
			contextCtrl.CheckParameters["ValidQuotationMarks"] =
				qmarks.QMarksList.ToString(" ", delegate(QuotationMarks item, StringBuilder bldr)
				{
					bldr.Append(item.Opening);
					bldr.Append(" ");
					bldr.Append(item.Closing);
				});
		}
	...
	public class QuotationLangList : List<QuotationLang>
	{
		...
		{
			if (string.IsNullOrEmpty(qmarks.LocaleOfLangUsedFrom))
