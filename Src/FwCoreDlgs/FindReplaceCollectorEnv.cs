...
using SIL.FieldWorks.FDO; // for FwStyleSheet
...
namespace SIL.FieldWorks.FwCoreDlgs
{
	...
	public class FindCollectorEnv : FindReplaceCollectorEnvBase
	{
		...
		public class LocationInfo
		{
			...
			/// --------------------------------------------------------------------------------
			public LocationInfo(List<StackItem> locationStack, int cPropPrev, int tag,
				int ichMin, int ichLim)
			{
				m_location = new SelLevInfo[locationStack.Count];
				m_tag = tag;
				m_ichMin = ichMin;
				m_ichLim = ichLim;
				m_cpropPrev = locationStack.Count > 0 ?
					locationStack[locationStack.Count - 1].m_cpropPrev.GetCount(tag) : cPropPrev;
				int iSelLevInfo = locationStack.Count - 1;
				for (int i = 0; i < locationStack.Count; i++, iSelLevInfo--)
				{
					...
					if (i > 0)
					{
						...
						int tagLevel = m_location[iSelLevInfo].tag;
						...
						m_location[iSelLevInfo].cpropPrevious = cpropPrevious;
					}
					else
					{
						m_location[iSelLevInfo].cpropPrevious = cPropPrev;
					}
				}
			...
			/// --------------------------------------------------------------------------------
			/// <summary>
			/// Copy constructor for the <see cref="T:LocationInfo"/> class.
			/// </summary>
