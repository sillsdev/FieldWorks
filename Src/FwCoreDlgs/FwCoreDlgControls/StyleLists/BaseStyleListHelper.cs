...
namespace SIL.FieldWorks.FwCoreDlgControls
{
	...
	/// </summary>
	/// ----------------------------------------------------------------------------------------
	public abstract class BaseStyleListHelper
	{
		...
		/// <summary>Stores a list of explicit stylenames to include in the displayed list. If this
		/// is set, then all other included/excluded contexts, functions, etc. are ignored and this
		/// list overrides the normal filtering. If this is null, then normal filtering happens.</summary>
		...
		/// client explicitly requested them to be shown (as in the styleds dialog) or because
		...
		/// a call to the
		/// <see cref="AddStyles(SIL.FieldWorks.FDO.FwStyleSheet)"/>
		...
		public List<string> ExplicitStylesToDisplay
		{
			...
			set { m_explicitStylesToDisplay = value; }
		}
		...
		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Add an item to a control.
		/// </summary>
		/// ------------------------------------------------------------------------------------
		protected abstract void AddStyleListItem(StyleListItem item);
		...
		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Do common Refresh work for m_styleItemList member.
		/// </summary>
		/// <param name="selectedStyle">The selected style.</param>
		/// ------------------------------------------------------------------------------------
		protected void RefreshStyleItemList(string selectedStyle)
		{
			if (m_styleItemList != null) // Added JohnT for robustness and to keep tests working.
			{
				...
				List<StyleListItem> itemsList = new List<StyleListItem>(m_styleItemList.Values);
				...
				StyleListItem defaultParaCharsStyle = null;
				foreach (StyleListItem item in itemsList)
				{
					if (item.Name == FdoResources.DefaultParaCharsStyleName)
					{
						defaultParaCharsStyle = item;
						break;
					}
				}
			...
			string[] pseudoStyles)
		{
			...
			m_styleItemList.Add(FdoResources.DefaultParaCharsStyleName,
