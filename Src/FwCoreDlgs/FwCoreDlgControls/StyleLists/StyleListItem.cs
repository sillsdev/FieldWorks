...
namespace SIL.FieldWorks.FwCoreDlgControls
{
	...
	public class StyleListItem : IComparable
	{
		...
		public string Name
		{
			get
			{
				return IsDefaultParaCharsStyle ? FdoResources.DefaultParaCharsStyleName : m_styleInfo.Name;
			}
		}
		...
		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Mark a style as being "in-use" or not.
		/// </summary>
		/// <param name="inUse"></param>
		/// ------------------------------------------------------------------------------------
		public void SetInUse(bool inUse)
		{
			if (m_styleInfo == null || m_styleInfo.RealStyle == null)
				throw new InvalidOperationException("InUse can only be set if the StyleInfo is based on a real style");

			// We don't want the user to be able to undo this...
			using (new SuppressSubTasks(m_styleInfo.RealStyle.Cache))
			{
				Debug.Assert(m_itemType != StyleListItemType.DataProperty);
				m_styleInfo.RealStyle.InUse = inUse;
				m_styleInfo.UserLevel = m_styleInfo.RealStyle.UserLevel;
			}
		}
