...
namespace SIL.FieldWorks.FwCoreDlgControls
{
	...
	public partial class FwFontTab : UserControl, IFWDisposable, IStylesTab
	{
		...
		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Initializes a new instance of the <see cref="T:FwFontTab"/> class.
		/// </summary>
		/// ------------------------------------------------------------------------------------
		public FwFontTab()
		{
		...
		protected void ValueChanged(object sender, EventArgs e)
		{
			if (!m_dontUpdateInheritance && sender != null)
			{
				...
				SaveToInfo((StyleInfo)m_currentStyleInfo);
		...
		public void FillFontInfo(FdoCache cache)
		{
			...
			foreach (NamedWritingSystem ws in cache.LangProject.GetActiveNamedWritingSystems())
			{
				item = new ListViewItem(ws.Name);
				item.Tag = ws.Hvo;
				item.SubItems.Add(string.Empty);
				m_lstWritingSystems.Items.Add(item);
			}
		...
		{
			get { return m_currentStyleInfo.Inherits || m_currentWs > 0; }
		}
		...
		private bool IsInherited(Control c)
		{
			...
			if (c is FwColorCombo && ((FwColorCombo)c).ColorValue == Color.Empty)
				return true;
