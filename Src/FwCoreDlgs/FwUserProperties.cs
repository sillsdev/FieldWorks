...
{
	#region IFwUserProperties interface
	/// ----------------------------------------------------------------------------------------
	/// <summary>
	/// Public interface (exported with COM wrapper) for the FW User Properties dialog box
	/// </summary>
	/// ----------------------------------------------------------------------------------------
	[ComVisible(true)]
	[GuidAttribute("5E7A6D24-5285-4221-BF27-4F78DD245C63")]
	public interface IFwUserProperties
	{
		/// -----------------------------------------------------------------------------------
		/// <summary>
	...
	/// </summary>
	/// ----------------------------------------------------------------------------------------
	[ProgId("FwCoreDlgs.FwUserProperties")]
	// Key attribute to hide the "clutter" from System.Windows.Forms.Form
	[ClassInterface(ClassInterfaceType.None)]
	[GuidAttribute("12115F9F-1998-4f47-9C5F-D0E81AFE1C29")]
	[ComVisible(true)]
	public class FwUserProperties : Form, IFWDisposable, IFwUserProperties
	{
		...
		public FwUserProperties()
		{
			...
			this.lblSelectFeatures.Tag = FwCoreDlgs.ksSelectAvailFeatsForX;
			this.lblAvailableFeatures.Tag = FwCoreDlgs.ksAvailFeatsForX;
			this.lblAvailableData.Tag = FwCoreDlgs.ksAvailDataForX;
		...
		protected void PopulateUserList()
		{
			...
			for (int i = 0; i < m_Cache.LangProject.UserAccountsOC.Count; i++)
			{
				...
				int hvo = m_Cache.LangProject.UserAccountsOC.HvoArray[i];
				IUserConfigAcct ucacTemp = UserConfigAcct.CreateFromDBObject(m_Cache, hvo);
		...
		private void CreateNewLoginAndUcac(ListViewItem lvi)
		{
			...
			if (rgbSid != null)
			{
				UserConfigAcct ucacTemp = new UserConfigAcct();
				m_Cache.LangProject.UserAccountsOC.Add(ucacTemp);
				ucacTemp.Sid = rgbSid;
		...
		{
			//this opens the connection and logs in so that the db contents can be accessed
			SqlConnection sqlConMaster = new SqlConnection(string.Format("Server={0}; Database=master; User ID = sa; Password=inscrutable; " +
				"Pooling=false;", m_Cache.ServerName));
			sqlConMaster.Open();

			byte[] rgbNewSid = new byte[16];
			try
			{
				try
				{
					...
					sqlComm.CommandText = string.Format("CREATE LOGIN {0} WITH PASSWORD = '{1}', CHECK_POLICY=off",
										sLoginName, sPassword);
					sqlComm.ExecuteNonQuery();

					//get the sid to return
					rgbNewSid = GetSid(sLoginName);
				}
				catch(SqlException sqle)
				{
					//just display the sql error message (sqle.message) for now
					//and recover nicely
					MessageBox.Show(sqle.Message);

					//make the new sid null to indicate that there is no sid because
		...
		{
			byte[] rgbOldSid = GetSid(sOldLoginName);
			string sSid = ToHexString(rgbOldSid);
			...
			try
			{
				try
				{
					// get the data from the old login
					//Get password, database name, and language
					SqlCommand sqlComm = sqlConMaster.CreateCommand();
					sSql = string.Format("SELECT password, dbname, language FROM syslogins where [sid] = {0}",
						sSid);
					...
					if (sqlreader.Read())
					{
						sPassword = sqlreader.GetString(0);
						sDbName = sqlreader.GetString(1);
						sLanguage = sqlreader.GetString(2);
						found = true;
					}
					else
						found = false;
					...
					//only do the following if the old login was found in syslogins
					if (found)
					{
						//drop the old login first (because you can't have two logins with
						//the same sid
						DropLogin(sOldLoginName);

						//create the new login based on all of the old data,
						...
						*/
						sqlComm.CommandText = string.Format("CREATE LOGIN"
							+ " {0} WITH PASSWORD '{1}', CHECK_POLICY=off, DEFAULT_DATABASE ='{2}', DEFAULT_LANGUAGE='{3}', SID={4}",
						sNewLoginName, sPassword, sDbName, sLanguage, sSid);
						sqlComm.ExecuteNonQuery();
					}
					else
					{
						//make the old sid null to indicate that there is no sid because
						//this login does not exist
						rgbOldSid = null;
					}
				}
		...
		protected byte[] GetSid(string sLoginName)
		{
			...
			try
			{
				try
				{
					...
					SqlCommand sqlComm = sqlConMaster.CreateCommand();
					string sSql =
						string.Format("SELECT [sid] FROM syslogins where [name] = '{0}'", sLoginName);
					sqlComm.CommandText = sSql;
					SqlDataReader sqlreader =
						sqlComm.ExecuteReader(System.Data.CommandBehavior.SingleResult);
					if (sqlreader.Read())
					{
						sqlreader.GetBytes(0, 0, rgbSid, 0, 16);
					}
					sqlreader.Close();
				}
				catch(SqlException sqle)
		...
		protected string GetLoginName(byte[] rgbSid)
		{
			...
			try
			{
				try
				{
					// get the login name of the sid
					SqlCommand sqlComm = sqlConMaster.CreateCommand();

					string sSid = ToHexString(rgbSid);
					string sSql = string.Format("SELECT [name] FROM syslogins where [sid] = {0}", sSid);
					...
					if (sqlreader.Read())
					{
						sLoginName = sqlreader.GetString(0);
					}
					sqlreader.Close();
				}
				catch(SqlException sqle)
		...
		protected bool RenameLogin(ListViewItem lvi, string sNewName)
		{
			...
			string sOldName = GetLoginName(((UserConfigAcct)(lvi.Tag)).Sid);
			...
			if (rgbNewSid != null)  //make sure the copy worked.  Sid will be null if it didn't.
			{
				...
				//		the fdo db
				IUserConfigAcct ucacTemp = UserConfigAcct.CreateFromDBObject(
					m_Cache, ((UserConfigAcct)(lvi.Tag)).Hvo);
		...
		protected bool DropLogin(string sLoginName)
		{
			...
			try
			{
				try
				{
					//drop the login from the syslogins table
					SqlCommand sqlComm = sqlConMaster.CreateCommand();

					sqlComm.CommandText = string.Format("EXEC sp_droplogin @loginame = '{0}'", sLoginName);
					sqlComm.ExecuteNonQuery();
					dropped = true;
				}
				catch(SqlException sqle)
		...
		private void PopulateTabControl(ListViewItem lvi, string sName)
		{
			...
			else*/ if(lvi.Tag != null && !lvi.Tag.Equals(FwCoreDlgs.ksNew))
			{
				...
				lblAvailableFeatures.Text =
					string.Format((string)(lblAvailableFeatures.Tag), sName);
				lblSelectFeatures.Text = string.Format((string)(lblSelectFeatures.Tag), sName);
				optMaintenanceAccess.Checked = ((UserConfigAcct)(lvi.Tag)).HasMaintenance;
		...
		#region IFwUserProperties Members
		...
		/// <returns></returns>
		/// ------------------------------------------------------------------------------------
		int IFwUserProperties.ShowDialog()
		{
			CheckDisposed();
		...
		private void btnDelete_Click(object sender, System.EventArgs e)
		{
			...
			if (lvwUsers.SelectedItems.Count > 0)
			{
				...
				if (result.Equals(DialogResult.Yes))
				{
					//drop the login
					bool dropped =
						DropLogin(GetLoginName(((UserConfigAcct)(lviToDelete.Tag)).Sid));
					if (dropped)
					{
						//delete ucac from the UserAccountsOC in the language project
						m_Cache.LangProject.UserAccountsOC.Remove(
							((UserConfigAcct)(lviToDelete.Tag)).Hvo);
