	...
	public class StyleInfo : BaseStyleInfo
	{
		...
		private void AddBorderInfo(StringBuilder text, bool useBiDiLabels)
		{
			...
			if (m_borderColor.IsExplicit)
				sDetails = string.Format(Strings.kstidBorderDetailsWithColor,
					ColorUtil.ColorToName(m_borderColor.Value), sWidth);
			else
				sDetails = string.Format(Strings.kstidBorderDetailsWithoutColor, sWidth);
		...
		private void AddBulletInfo(StringBuilder text)
		{
			if (m_bulletInfo.IsExplicit)
			{
				...
				else
				{
					...
					{
						AppendItem(tmp, Strings.kstidSchema,
			...
			MsrSysType userMeasurementType)
		{
			...
			{
				AppendMeasure(text,
					(useBiDiLabels) ? Strings.ksLeading : Strings.ksLeft,
			...
			{
				AppendMeasure(text,
					(useBiDiLabels) ? Strings.ksTrailing : Strings.ksRight,
