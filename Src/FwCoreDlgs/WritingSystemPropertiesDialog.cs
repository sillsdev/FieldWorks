...
namespace SIL.FieldWorks.FwCoreDlgs
{
	...
	public class WritingSystemPropertiesDialog : Form, IFWDisposable
	{
		...
		private void SetupEthnologueCode(IWritingSystem ws)
		{
			...
			SetLanguageCodeLabels(!string.IsNullOrEmpty(ws.LanguageSubtag.ISO3Code) ? ws.LanguageSubtag.ISO3Code : strNone);
		}
		...
		{
			var wssToUpdate = new List<Tuple<string, string>>();
			foreach (KeyValuePair<IWritingSystem, IWritingSystem> kvp in m_tempWritingSystems)
			{
				IWritingSystem tempWs = kvp.Key;
				...
				else if (tempWs.Modified)
				{
					if (tempWs.Id != origWs.Id)
						wssToUpdate.Add(Tuple.Create(origWs.Id, tempWs.Id));
					origWs.Copy(tempWs);
					m_fChanged = true;
				}
			}
			...
			if (m_cache != null && wssToUpdate.Count > 0)
			{
				...
				{
					foreach (Tuple<string, string> ws in wssToUpdate)
						WritingSystemServices.UpdateWritingSystemFields(m_cache, ws.Item1, ws.Item2);
				});
			}
			m_wsManager.Save();
		}
		...
		private void m_fwTextBoxTestWs_Enter(object sender, EventArgs e)
		{
			...
			var bldr = StringUtils.MakeTss(string.Empty, tempWs.Handle).GetBldr();
