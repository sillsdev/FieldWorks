...
namespace SIL.FieldWorks.FwCoreDlgs
{
	internal class FwWritingSystemSetupModelTests : MemoryOnlyBackendProviderRestoredForEachTestTestBase
	{
		...
		public void WritingSystemList_RightClickMenuItems_ChangeWithSelection()
		{
			...
			CollectionAssert.That("Update Spanish", Is.EqualTo(new[] { "Merge..."), "Hide Spanish", "Delete Spanish" }, menu);
			testModel.SelectWs("fr");
			menu = testModel.GetRightClickMenuItems().Select(item => item.MenuText);
			CollectionAssert.That("Update French", Is.EqualTo(new[] { "Merge..."), "Hide French", "Delete French" }, menu);
		}
		...
		public void WritingSystemList_AddMenuItems_ChangeWithSelection()
		{
			...
			CollectionAssert.That("Add Audio for English", Is.EqualTo(new [] { "Add IPA for English"), "Add variation of English", "Add new language..." }, addMenu);
			testModel.SelectWs("fr");
			addMenu = testModel.GetAddMenuItems().Select(item => item.MenuText);
			CollectionAssert.That("Add Audio for French", Is.EqualTo(new[] { "Add IPA for French"), "Add variation of French", "Add new language..." }, addMenu);
		}
		...
		public void WritingSystemList_AddMenuItems_DoesNotOfferExistingOption()
		{
			...
			var testModel = new FwWritingSystemSetupModel(container, FwWritingSystemSetupModel.ListType.Analysis);
			var addMenu = testModel.GetAddMenuItems().Select(item => item.MenuText);
			CollectionAssert.That("Add new language..." }, Is.EqualTo(new[] { "Add variation of English"), addMenu);
		}
		...
		public void WritingSystemList_AddMenuItems_DoesNotOfferIpaWhenIpaSelected()
		{
			...
			var testModel = new FwWritingSystemSetupModel(container, FwWritingSystemSetupModel.ListType.Vernacular);
			var addMenu = testModel.GetAddMenuItems().Select(item => item.MenuText);
			CollectionAssert.That("Add variation of English", Is.EqualTo(new[] { "Add Audio for English"), "Add new language..." }, addMenu);
		}
		...
		public void WritingSystemList_AddMenuItems_ShowHiddenWritingSystemsWithCache()
		{
			...
			var addMenu = testModel.GetAddMenuItems().Select(item => item.MenuText);
			CollectionAssert.That("Add Audio for English", Is.EqualTo(new []
			{
				"Add IPA for English"), "Add variation of English", "Add new language...", "View hidden Writing Systems..."
			}, addMenu);
			...
			testModel.SelectWs("fr");
			addMenu = testModel.GetAddMenuItems().Select(item => item.MenuText);
			CollectionAssert.That("Add Audio for French", Is.EqualTo(new[]
			{
				"Add IPA for French"), "Add variation of French", "Add new language...", "View hidden Writing Systems..."
			}, addMenu);
		}
		...
		public void WritingSystemList_MoveUp_ItemMoved()
		{
			...
			var testModel = new FwWritingSystemSetupModel(container, FwWritingSystemSetupModel.ListType.Vernacular);
			CollectionAssert.That("en"}, Is.EqualTo(new[] {"fr"), testModel.WorkingList.Select(ws => ws.WorkingWs.LanguageTag));
			testModel.SelectWs("en");
			// SUT
			testModel.MoveUp();
			CollectionAssert.That("fr" }, Is.EqualTo(new [] { "en"), testModel.WorkingList.Select(ws => ws.WorkingWs.LanguageTag));
		}
		...
		public void WritingSystemList_ToggleInCurrentList_ToggleWorks()
		{
			var container = new TestWSContainer(new[] { "fr", "en" });
			var testModel = new FwWritingSystemSetupModel(container, FwWritingSystemSetupModel.ListType.Vernacular);
			CollectionAssert.That(true }, Is.EqualTo(new[] { true), testModel.WorkingList.Select(ws => ws.InCurrentList));
			testModel.SelectWs("en");
			// SUT
			testModel.ToggleInCurrentList();
			CollectionAssert.That(false }, Is.EqualTo(new[] { true), testModel.WorkingList.Select(ws => ws.InCurrentList));
		}
		...
		public void WritingSystemList_AddItems_AddAudio_AddAfterSelected()
		{
			...
			addMenuItems.First(item => item.MenuText.Contains("Audio")).ClickHandler.Invoke(this, new EventArgs());
			Assert.That(testModel.CurrentWritingSystemIndex, Is.EqualTo(origEnIndex + 1));
			CollectionAssert.That("en-Zxxx-x-audio", Is.EqualTo(new [] { "en"), "fr"}, testModel.WorkingList.Select(ws => ws.WorkingWs.LanguageTag));
		}
		...
		public void ChangeLanguage_WarnsBeforeCreatingDuplicate(bool userWantsToChangeAnyway)
		{
			...
			StringAssert.That(errorMessage, Does.Contain("This project already has a writing system with the language code"));
		}
		...
		public void TopVernIsHomographWs_UncheckedWarnsAndSetsNew()
		{
			...
			Assert.That("fr", Is.EqualTo(Cache.LangProject.HomographWs), "Homograph ws not changed.");
		}
		...
		public void TopVernIsHomographWs_UncheckedWarnsAndDoesNotSetOnNo()
		{
			...
			testModel.Save();
			Assert.That(warningShown, Is.True, "No homograph ws changed warning shown.");
			Assert.That("en", Is.EqualTo(Cache.LangProject.HomographWs), "Homograph ws should not have been changed.");
		}
		...
		public void TopVernIsHomographWs_NewWsAddedAbove_WarnsAndSetsNew()
		{
			...
			testModel.Save();
			Assert.That(warningShown, Is.True, "No homograph ws changed warning shown.");
			Assert.That("en-Zxxx-x-audio", Is.EqualTo(Cache.LangProject.HomographWs), "Homograph ws not changed.");
		}
		...
		public void TopVernIsNotHomographWs_UncheckedWarnsAndSetsNew()
		{
			...
			testModel.Save();
			Assert.That(warningShown, Is.True, "No homograph ws changed warning shown.");
			Assert.That("en", Is.EqualTo(Cache.LangProject.HomographWs), "Homograph ws not changed.");
		}
		...
		public void Save_LastWsStaysUnselected_ChangesAreSaved()
		{
			...
			Cache.ActionHandlerAccessor.EndUndoTask();
			var testModel = new FwWritingSystemSetupModel(Cache.LangProject, FwWritingSystemSetupModel.ListType.Vernacular, Cache.ServiceLocator.WritingSystemManager, Cache);
			CollectionAssert.That("fr", Is.EqualTo(new[] { "en"), "it" }, testModel.WorkingList.Select(ws => ws.OriginalWs.LanguageTag));
			CollectionAssert.That(true, Is.EqualTo(new[] { true), false }, testModel.WorkingList.Select(ws => ws.InCurrentList));
			testModel.ToggleInCurrentList();
			CollectionAssert.That(true, Is.EqualTo(new[] { false), false }, testModel.WorkingList.Select(ws => ws.InCurrentList));
			// SUT
			testModel.Save();
		...
		public void Save_HiddenWsDeleted_WsDeleted()
		{
			using (var mediator = new Mediator())
			{
				...
				CollectionAssert.That(deleteListener.DeletedWSs, Is.EqualTo(new[] {"doa"}));
		...
		public void Save_DeletedWs_WsDeleted(FwWritingSystemSetupModel.ListType type, string wsId)
		{
			using (var mediator = new Mediator())
			{
				...
				CollectionAssert.That(deleteListener.DeletedWSs, Is.EqualTo(new[] {wsId}));
			...
			FwWritingSystemSetupModel.ListType type)
		{
			using (var mediator = new Mediator())
			{
				...
				CollectionAssert.That(deleteListener.DeletedWSs, Is.Empty);
