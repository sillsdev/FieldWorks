...
{
	internal class FwWritingSystemSetupModelTests
		: MemoryOnlyBackendProviderRestoredForEachTestTestBase
	{
		...
		public void SelectionForSpecialCombo_HasDefaultScriptAndRegion_GivesScriptRegionVariant()
		{
			...
			var testModel = new FwWritingSystemSetupModel(
				container,
				FwWritingSystemSetupModel.ListType.Vernacular
			);
			Assert.That(
		...
		public void SelectionForSpecialCombo_ChangesOnSelectionChange_GivesScriptRegionVariant()
		{
			...
			Assert.That(
				testModel.CurrentWsSetupModel.SelectionForSpecialCombo,
				Is.EqualTo(WritingSystemSetupModel.SelectionsForSpecialCombo.None)
			);
		...
		public void SelectionForSpecialCombo_LockedForDefaultEnglish()
		{
			...
			string expectedErrorMessage = string.Format(
				FwCoreDlgs.kstidCantChangeEnglishSRV,
				"English"
			);
			string errorMessage = null;
			var wssModel = new FwWritingSystemSetupModel(
				container,
				FwWritingSystemSetupModel.ListType.Vernacular
			)
			{
				ShowMessageBox = (text, isResponseRequested) =>
				{
					errorMessage = text;
					Assert.That(isResponseRequested, Is.False);
					return false;
				},
			}.CurrentWsSetupModel;
			wssModel.CurrentScriptCode = "Cyrilic";
			Assert.That(
				wssModel.CurrentScriptCode,
				Is.EqualTo("Latn"),
				"script code should be reset to Latin"
			);
			Assert.That(
				wssModel.SelectionForSpecialCombo,
				Is.EqualTo(WritingSystemSetupModel.SelectionsForSpecialCombo.None),
				"script"
			);
		...
		public void AdvancedConfiguration_NonCustomLangScriptRegion_IsDisabled()
		{
			...
			Assert.That(
				testModel.ShowAdvancedScriptRegionVariantView,
				Is.False,
				"Model should not show advanced view for normal data"
			);
		}
		...
		public void AdvancedConfiguration_CustomScript_IsEnabled()
		{
			...
			Assert.That(
				testModel.ShowAdvancedScriptRegionVariantView,
				Is.True,
				"Model should show advanced view for Custom script"
			);
		}
		...
		public void AdvancedConfiguration_StandardAndPrivateUse_IsEnabled()
		{
			...
			Assert.That(
				testModel.ShowAdvancedScriptRegionVariantView,
				Is.True,
				"Model should show advanced view when there are multiple variants"
			);
		}
		...
		public void AdvancedConfiguration_ClearingAdvanced_ShowsWarning_ClearsCustomContent()
		{
			...
			Assert.That(
				testModel.CurrentWsSetupModel.CurrentIso15924Script.IsPrivateUse,
				Is.False
			);
		}
		...
		public void AdvancedConfiguration_NonGraphiteFont_GraphiteFontOptionsAreDisabled()
		{
			...
			Assert.That(
				testModel.EnableGraphiteFontOptions,
				Is.False,
				"Non Graphite fonts should not have the EnableGraphiteFontOptions available"
			);
		}
		...
		public void AdvancedConfiguration_AdvancedScriptRegionVariantCheckboxVisible(
			string languageTag,
			bool expectedResult
		)
		{
			...
			Assert.That(
				testModel.ShowAdvancedScriptRegionVariantCheckBox,
		...
		public void AdvancedConfiguration_GraphiteFont_GraphiteFontOptionsAreEnabled()
		{
			...
			Assert.That(
				testModel.EnableGraphiteFontOptions,
				Is.True,
				"Graphite fonts should have the EnableGraphiteFontOptions available"
			);
		}
		...
		public void AdvancedConfiguration_NoDefaultFont_GraphiteFontOptionsAreDisabled()
		{
			...
			Assert.That(
				testModel.EnableGraphiteFontOptions,
				Is.False,
				"EnableGraphiteFeatures should not be available without a default font"
			);
		}
		...
		public void WritingSystemList_RightClickMenuItems_ChangeWithSelection()
		{
			...
			Assert.That(
				menu,
				Is.EqualTo(
					new[] { "Merge...", "Update Spanish", "Hide Spanish", "Delete Spanish" }
				)
			);
			...
			Assert.That(
				menu,
				Is.EqualTo(new[] { "Merge...", "Update French", "Hide French", "Delete French" })
			);
		}

		[TestCase(FwWritingSystemSetupModel.ListType.Vernacular, true)]
		[TestCase(FwWritingSystemSetupModel.ListType.Analysis, false)]
		public void WritingSystemList_RightClickMenuItems_CannotMergeOrDeleteEnglishAnalyWs(
		...
		)
		{
			...
			Assert.That(
				menu.First(m => m.MenuText.StartsWith("Hide")).IsEnabled,
				Is.EqualTo(canDelete),
				"English can be hidden from the Vernacular but not the Analysis WS List"
			);
			Assert.That(
				menu.First(m => m.MenuText.StartsWith("Hide")).MenuText, /* REVIEW (Hasso) contain? */
				Is.EqualTo("Hide English")
			);
			Assert.That(
				menu.First(m => m.MenuText.StartsWith("Delete")).IsEnabled,
				Is.EqualTo(canDelete),
				"English can be deleted from the Vernacular but not the Analysis WS List"
			);
			Assert.That(
				menu.First(m => m.MenuText.StartsWith("Delete")).MenuText, /* REVIEW (Hasso) contain? */
				Is.EqualTo("Delete English")
			);
		}
		...
		public void WritingSystemList_RightClickMenuItems_NewWs()
		{
			...
			var menu = testModel.GetRightClickMenuItems();
			Assert.That(
				menu.First(m => m.MenuText.StartsWith("Update")).IsEnabled,
				Is.False,
				"Update should be disabled"
			);
			Assert.That(
				menu.First(m => m.MenuText.StartsWith("Hide")).IsEnabled,
				Is.False,
		...
		public void WritingSystemList_AddMenuItems_ChangeWithSelection()
		{
			...
			Assert.That(
				...
				Is.EqualTo(
					...
					{
						"Add IPA for English",
						"Add Audio for English",
						"Add variation of English",
						"Add new language...",
					}
				)
			);
			...
			Assert.That(
				...
				Is.EqualTo(
					...
					{
						"Add IPA for French",
						"Add Audio for French",
						"Add variation of French",
						"Add new language...",
					}
				)
			);
		...
		public void WritingSystemList_AddMenuItems_AddLanguageWarnsForVernacular()
		{
			...
			var addLanguageMenu = testModel
				.GetAddMenuItems()
				.First(item => item.MenuText.Contains("Add new language"));
		...
		public void WritingSystemList_AddMenuItems_AddLanguageDoesNotWarnForAnalysis()
		{
			...
			var testModel = new FwWritingSystemSetupModel(
				container,
				FwWritingSystemSetupModel.ListType.Analysis
			);
			testModel.AddNewVernacularLanguageWarning = () =>
			{
				warned = true;
		...
		{
			var container = new TestWSContainer(
				new[] { "auc" },
				new[] { "en", "en-fonipa", "en-Zxxx-x-audio" }
			);
			var testModel = new FwWritingSystemSetupModel(
				container,
				FwWritingSystemSetupModel.ListType.Analysis
			...
			Assert.That(
				addMenu,
				Is.EqualTo(new[] { "Add variation of English", "Add new language..." })
			);
		}
			...
			Assert.That(
				...
				Is.EqualTo(
					new[]
					{
						...
						"Add variation of English",
						"Add new language...",
						"View hidden Writing Systems...",
					}
				)
			);

			SetupHomographLanguagesInCache();
			testModel = new FwWritingSystemSetupModel(
				Cache.LangProject,
				FwWritingSystemSetupModel.ListType.Vernacular,
				Cache.ServiceLocator.WritingSystemManager,
				Cache
		...
		public void WritingSystemList_MoveUp_ItemMoved()
		{
			...
			Assert.That(
				testModel.WorkingList.Select(ws => ws.WorkingWs.LanguageTag),
				Is.EqualTo(new[] { "fr", "en" })
			);
		...
		public void WritingSystemList_ToggleInCurrentList_ToggleWorks()
		{
			...
			Assert.That(
				testModel.WorkingList.Select(ws => ws.InCurrentList),
		...
		public void WritingSystemList_CanMerge(
			string toMerge,
			string[] options,
			bool expectedResult
		)
		...
		public void WritingSystemList_CanMerge_CantMergeNewWs(
			...
			[Values("Audio", "variation")] string variantType
		) // test only Audio and Variation because IPA requires the Cache
		{
			var wss = new[] { "de" };
			...
			addMenuItems
				.First(item => item.MenuText.Contains(variantType))
		...
		[TestCase(
			...
			new[] { "en-GB", "en" },
			true
		)] // Can delete variants of English
		[TestCase(
			FwWritingSystemSetupModel.ListType.Analysis,
			"en-fonipa",
			new[] { "en-fonipa", "en" },
		...
		[TestCase(
			...
			new[] { "fr", "en" },
			true
		)] // Can delete English from the Vernacular list
		...
		[TestCase(
			...
			new[] { "fr", "en" },
			true
		)] // Can delete if there is more than one
		public void WritingSystemList_CanDelete(
			FwWritingSystemSetupModel.ListType type,
			string toRemove,
			string[] options,
			bool expectedResult
		...
		public void WritingSystemList_CanDelete_CanDeleteDuplicateEnglishAnalysis()
		{
			...
			var addMenuItems = testModel.GetAddMenuItems();
			addMenuItems
				.First(item => item.MenuText.Contains("variation"))
				.ClickHandler.Invoke(this, new EventArgs());
			Assert.That(testModel.CurrentWsSetupModel.CurrentLanguageTag, Is.EqualTo("en"));
			Assert.That(
				testModel.CanDelete(),
				Is.EqualTo(true),
				"should be able to delete the newly-created English [in]variant"
			);
			testModel.SelectWs(0);
			Assert.That(
				testModel.CanDelete(),
				Is.EqualTo(false),
				"should not be able to delete the original English"
			);
		}
		...
		public void MergeTargets_SkipsNewAndCurrent()
		{
			...
			addMenuItems
				.First(item => item.MenuText.Contains("Audio"))
				.ClickHandler.Invoke(this, new EventArgs());
			testModel.SelectWs("en");
			// SUT
			...
			Assert.That(
				mergeTargets[0].WorkingWs.LanguageTag, /* REVIEW (Hasso) contain? */
				Is.EqualTo("fr")
			);
		}
		...
		public void WritingSystemList_AddItems_AddAudio_CustomNameUsed()
		{
			...
			addMenuItems
				...
				.ClickHandler.Invoke(this, new EventArgs());
			Assert.That(
				testModel.CurrentWsSetupModel.CurrentLanguageTag,
				Is.EqualTo("en-Zxxx-x-audio")
			);
			Assert.That(
				testModel.LanguageName, /* REVIEW (Hasso) contain? */
				Is.EqualTo("Testing")
			);
		}
		...
		public void WritingSystemList_AddItems_AddAudio_AddAfterSelected()
		{
			...
			Assert.That(
				testModel.WorkingList.Select(ws => ws.WorkingWs.LanguageTag),
				Is.EqualTo(new[] { "en", "en-Zxxx-x-audio", "fr" })
			);
		}
		...
		public void Model_NewWritingSystemAddedInManagerAndList()
		{
			...
			mockWsManager.Setup(manager =>
				manager.Replace(It.IsAny<CoreWritingSystemDefinition>())
			);
		...
		public void Model_ChangesContainerOnlyOnSave()
		{
			...
			Assert.That(
				container.VernacularWritingSystems.First().Abbreviation,
				Is.EqualTo("fr")
			);
			Assert.That(
			...
			Assert.That(
				(
					(SimpleRulesCollationDefinition)
						container.VernacularWritingSystems.First().DefaultCollation
				).SimpleRules,
				Is.EqualTo("Z z Y y X x")
			);
		}
		...
		public void Model_WritingSystemListUpdated_CalledOnChange()
		{
			...
			testModel.Save();
			Assert.That(
				writingSystemListUpdatedCalled,
				Is.True,
				"WritingSystemListUpdated should have been called after this change"
			);
		}
		...
		public void Model_WritingSystemChanged_CalledOnAbbrevChange()
		{
			...
			testModel.Save();
			Assert.That(
				writingSystemChanged,
				Is.True,
				"WritingSystemUpdated should have been called after this change"
			);
		}
		...
		public void Model_WritingSystemChanged_NotCalledOnIrrelevantChange()
		{
			...
			Assert.That(
				writingSystemChanged,
				Is.False,
				"WritingSystemUpdated should not have been called after this change"
			);
		}
		...
		public void WritingSystemTitle_ChangesByType(FwWritingSystemSetupModel.ListType type)
		{
			...
			Assert.That(
				testModel.Title,
				Does.Contain(string.Format("{0} Writing System Properties", type))
			);
		}
		...
		public void LanguageName_ChangesAllRelated()
		{
			...
			Assert.That(
				testModel.CurrentWsSetupModel.CurrentLanguageName,
				Is.EqualTo(newLangName)
			);
			Assert.That(
				testModel.LanguageCode,
				Does.Not.Contain("qaa"),
			...
			Assert.That(
				testModel.CurrentWsSetupModel.CurrentLanguageTag, /* REVIEW (Hasso) contain? */
				Is.EqualTo("en")
			);
			Assert.That(
				testModel.LanguageCode,
		...
		public void WritingSystemName_ChangesOnSwitch()
		{
			...
			testModel.SelectWs("en-fonipa");
			Assert.That(
				testModel.WritingSystemName,
				Is.EqualTo("English (International Phonetic Alphabet)")
			);
		}
		...
		{
			var container = new TestWSContainer(
				new[] { "fr", "fr-Arab", "fr-GB", "fr-fonipa", "es" }
			);
			var testModel = new FwWritingSystemSetupModel(
				container,
				FwWritingSystemSetupModel.ListType.Vernacular
		...
		public void ChangeLanguage_WarnsBeforeCreatingDuplicate(bool userWantsToChangeAnyway)
		{
			...
			string errorMessage = null;
			testModel.ShowMessageBox = (text, isResponseRequested) =>
			{
				errorMessage = text;
				Assert.That(isResponseRequested, Is.True);
				return userWantsToChangeAnyway;
			};
			testModel.ShowChangeLanguage = ShowChangeLanguage;
			...
			if (userWantsToChangeAnyway)
			{
				...
				Assert.That(
					testModel.CurrentWsSetupModel.CurrentLanguageName,
					Is.EqualTo("TestName"),
					"all WS's for the language should change"
				);
			}
			else
			{
				...
				Assert.That(
					testModel.CurrentWsSetupModel.CurrentLanguageName,
					Is.EqualTo("Spanish"),
					"other WS's shouldn't have changed, either"
				);
				testModel.SelectWs("es-fonipa");
				Assert.That(
					testModel.CurrentWsSetupModel.CurrentLanguageName,
					Is.EqualTo("Spanish"),
					"variant WS's shouldn't have changed, either"
				);
			}
			Assert.That(
				errorMessage,
				Does.Contain("This project already has a writing system with the language code")
			);
		}
		...
		public void ChangeLanguage_CorrectScriptSelected(
			string oldWs,
			string newLang,
			string expectedWs
		)
		...
		public void EthnologueLink_UsesLanguageCode()
		{
			...
			Assert.That(
				testModel.EthnologueLabel,
				Does.EndWith("fra"),
				"Label didn't end with language code"
			);
			Assert.That(
				testModel.EthnologueLink,
				Does.EndWith("fra"),
				"Link didn't end with language code"
			);
		}
		...
		public void Converters_NoEncodingConverters_ReturnsListWithOnlyNone()
		{
			...
			Assert.That(
				converters.First(), /* REVIEW (Hasso) contain? */
				Is.EqualTo("<None>")
			);
		}
		...
		public void Converters_ModifyEncodingConverters_Ok()
		{
			...
			testModel.EncodingConverterKeys = () =>
			{
				return new[] { "Test2", "Test" };
			};
			testModel.ShowModifyEncodingConverters = TestShowModifyConverters;
			testModel.ModifyEncodingConverters();
			Assert.That(
				testModel.CurrentLegacyConverter, /* REVIEW (Hasso) contain? */
				Is.EqualTo("Test")
			);
		}
		...
		{
			var container = new TestWSContainer(new[] { "en" });
			container.VernacularWritingSystems.First().NumberingSystem =
				NumberingSystemDefinition.CreateCustomSystem("abcdefghij");
			var testModel = new FwWritingSystemSetupModel(
				container,
				FwWritingSystemSetupModel.ListType.Vernacular
			);
			...
			Assert.That(
				testModel.CurrentWsSetupModel.CurrentNumberingSystemDefinition.IsCustom,
				Is.True
			);
			Assert.That(
				testModel.CurrentWsSetupModel.CurrentNumberingSystemDefinition.Digits, /* REVIEW (Hasso) contain? */
				Is.EqualTo("abcdefghij")
			);
			// Test switching to default switches back to default digits
			testModel.CurrentWsSetupModel.CurrentNumberingSystemDefinition =
				NumberingSystemDefinition.Default;
			Assert.That(
				testModel.CurrentWsSetupModel.CurrentNumberingSystemDefinition.Digits, /* REVIEW (Hasso) contain? */
		...
		{
			var container = new TestWSContainer(
				new[] { "en", "fr", "en-Zxxx-x-audio" },
				null,
				new[] { "en", "en-Zxxx-x-audio" }
			);
			var testModel = new FwWritingSystemSetupModel(
				container,
				FwWritingSystemSetupModel.ListType.Vernacular
		...
		public void CurrentWsListChanged_HideSelected_Returns_True()
		{
			...
			var menu = testModel.GetRightClickMenuItems();
			menu.First(item => item.MenuText.Contains("Hide"))
		...
		public void CurrentWsListChanged_DeleteSelected_Returns_True()
		{
			...
			var menu = testModel.GetRightClickMenuItems();
			menu.First(item => item.MenuText.Contains("Delete"))
				.ClickHandler(this, EventArgs.Empty);
			Assert.That(testModel.CurrentWsListChanged, Is.True);
		}
		...
		public void MergeSelected_SaveDoesNotCrashWithNoCache()
		{
			...
			testModel.ConfirmMergeWritingSystem = (
				string merge,
				out CoreWritingSystemDefinition tag
			) =>
			{
			...
			var menu = testModel.GetRightClickMenuItems();
			menu.First(item => item.MenuText.Contains("Merge"))
				.ClickHandler(this, EventArgs.Empty);
			Assert.DoesNotThrow(() => testModel.Save());
			Assert.That(container.VernacularWritingSystems.Count, Is.EqualTo(1));
		}
		...
		public void Delete_UserRepents_NoChange()
		{
			...
			Assert.That(testModel.CurrentWsListChanged, Is.False);
			Assert.That(
				testModel.WorkingList.Select(li => li.WorkingWs.Id),
				Is.EquivalentTo(wsList)
			);
		...
		public void TopVernIsHomographWs_UncheckedWarnsAndSetsNew()
		{
			...
			testModel.ShouldChangeHomographWs = ws =>
			{
				warningShown = true;
				return true;
			};
			...
			Assert.That(
				Cache.LangProject.HomographWs,
				Is.EqualTo("fr"),
				"Homograph ws not changed."
			);
		}
		...
		public void TopVernIsHomographWs_UncheckedWarnsAndDoesNotSetOnNo()
		{
			...
			Assert.That(
				Cache.LangProject.HomographWs,
				Is.EqualTo("en"),
				"Homograph ws should not have been changed."
			);
		}
		...
		public void TopVernIsNotHomographWs_UncheckedWarnsAndSetsNew()
		{
			SetupHomographLanguagesInCache();
			Assert.That(
				Cache.LangProject.VernWss,
				Is.EqualTo("en fr"),
				"Test data setup incorrect, english should be first followed by french"
			);
			Assert.That(
				Cache.LangProject.CurVernWss,
				Is.EqualTo("en fr"),
				"Test data setup incorrect, english should be first followed by french"
			);
		...
		public void CurrentVernacularList_ToggleSaved()
		{
			...
			Assert.That(
				Cache.LangProject.CurVernWss,
				Is.EqualTo("en"),
				"French should have been removed from the CurrentVernacular list on Save"
			);
		}
		...
		public void Save_LastWsStaysUnselected_ChangesAreSaved()
		{
			...
			Assert.That(
				testModel.WorkingList.Select(ws => ws.OriginalWs.LanguageTag),
				Is.EqualTo(new[] { "en", "fr", "it" })
			);
			Assert.That(
				testModel.WorkingList.Select(ws => ws.InCurrentList),
			...
			Assert.That(
				Cache.LangProject.CurVernWss,
				Is.EqualTo("fr"),
				"Only French should remain selected after save"
			);
		}
		...
		public void HiddenWsModel_AllCtorArgsPassed()
		{
			...
			)
			{
				...
				ViewHiddenWritingSystems = model =>
				{
					...
					Assert.That(
						model.Items.Any(i => i.WS.Equals(addedWs)),
						Is.False,
						$"{addedWs.DisplayLabel} is not quite 'hidden' anymore"
					);
					...
					Assert.That(
						...
						Does.EndWith(
							string.Format(FwCoreDlgs.XWillBeDeleted, deletedWs.DisplayLabel)
						)
					);
		...
		public void HiddenWsShown()
		{
			...
			)
			{
				ViewHiddenWritingSystems = model =>
					model.Items.Add(
						new HiddenWSListItemModel(GetOrSetWs("hid"), false) { WillAdd = true }
					),
			};
			...
			Assert.That(
				testModel.CurrentWsListChanged,
				Is.True,
				"Showing a WS changes the 'current' showing list"
			);
			Assert.That(
				testModel.WorkingList.Select(li => li.OriginalWs.Id),
		...
		public void Save_HiddenWsDeleted_WsDeleted()
		{
			using (var mediator = new Mediator())
			{
				...
				Cache
					.ServiceLocator.GetInstance<ILexEntryFactory>()
					.Create()
					.CitationForm.set_String(ws.Handle, "some data");
				Cache.ActionHandlerAccessor.EndUndoTask();
				var testModel = new FwWritingSystemSetupModel(
					Cache.LangProject,
					FwWritingSystemSetupModel.ListType.Vernacular,
				...
				)
				{
					ViewHiddenWritingSystems = model =>
						model.Items.Add(
							new HiddenWSListItemModel(ws, false) { WillDelete = true }
						),
				};
				...
				Assert.That(
					WritingSystemServices.FindAllWritingSystemsWithText(Cache),
					Is.Not.Contains(ws.Handle)
				);
			}
		}
			...
			using (var mediator = new Mediator())
			{
				...
				if (type == FwWritingSystemSetupModel.ListType.Vernacular)
				{
					Assert.That(
						Cache.LangProject.CurVernWss,
						Is.EqualTo("en"),
						"Only English should remain selected after save"
					);
					Assert.That(
						Cache.LangProject.VernWss,
						Is.EqualTo("en"),
						"Only English should remain after save"
					);
				...
				{
					Assert.That(
						Cache.LangProject.CurAnalysisWss,
						Is.EqualTo("en"),
						"Only English should remain selected after save"
					);
					Assert.That(
						Cache.LangProject.AnalysisWss,
						Is.EqualTo("en"),
						"Only English should remain after save"
					);
				...
				Assert.That(
					WritingSystemServices.FindAllWritingSystemsWithText(Cache),
					Is.Not.Contains(GetOrSetWs(wsId).Handle)
				);
			}
		}
		...
		)
		{
			using (var mediator = new Mediator())
			{
				...
				Assert.That(
					wasDeleteConfirmed,
					Is.False,
					"shouldn't confirm 'deleting' a WS that will only be hidden"
				);
		...
		public void Save_HiddenWs_WsHidden(FwWritingSystemSetupModel.ListType type, string wsId)
		{
			using (var mediator = new Mediator())
			{
				...
				testModel
					.GetRightClickMenuItems()
					.First(item => item.MenuText.Contains("Hide"))
					.ClickHandler(this, EventArgs.Empty);
				testModel.Save();
		...
		[TestCase(FwWritingSystemSetupModel.ListType.Vernacular, "fr")]
		[TestCase(FwWritingSystemSetupModel.ListType.Analysis, "tpi")]
		public void Save_WsDeletedRestoredAndHidden_WsHidden(
			FwWritingSystemSetupModel.ListType type,
			string wsId
		)
		{
		...
		)
		{
			using (var mediator = new Mediator())
			{
				...
				Assert.That(
					Cache.LangProject.CurVernWss,
					Is.EqualTo("en fr"),
					"Both should remain selected after save"
				);
				Assert.That(
					Cache.LangProject.VernWss,
					Is.EqualTo("en fr"),
					"Both should remain after save"
				);
				Assert.That(
					Cache.LangProject.CurAnalysisWss,
					Is.EqualTo("en tpi"),
		...
		private void AssertTokPisinDataIntact()
		{
			...
			var entry = Cache.LangProject.LexDbOA.Entries.First();
			Assert.That(
				entry.SensesOS.First().Gloss.get_String(tpi).Text,
				Is.EqualTo("tpiSense")
			);
		}
		...
		public void GetSpellingDictionaryComboBoxItems_HasDefaultForWs()
		{
			...
			Assert.That(
				constructedItem,
				Is.Not.Null,
				"A default item matching the ws id should be in the list."
			);
		}
		...
		public void MergeWritingSystemTest_MergeWorks()
		{
			...
			testModel
				.GetRightClickMenuItems()
				.First(item => item.MenuText == "Merge...")
				.ClickHandler.Invoke(null, null);
			testModel.Save();
			Assert.That(
				entry.SummaryDefinition.get_String(en.Handle).Text,
				Does.StartWith("Queens English")
			);
		}
		...
		private void SetupHomographLanguagesInCache()
		{
			...
			Cache.LangProject.VernacularWritingSystems.AddRange(
				Cache.LangProject.CurrentVernacularWritingSystems
			);
		...
		private bool TestShowModifyConverters(
			string originalconverter,
			out string selectedconverter
		)
		{
		...
		{
			private readonly List<CoreWritingSystemDefinition> _vernacular =
				new List<CoreWritingSystemDefinition>();
			private readonly List<CoreWritingSystemDefinition> _analysis =
				new List<CoreWritingSystemDefinition>();
			private readonly List<CoreWritingSystemDefinition> _curVern =
				new List<CoreWritingSystemDefinition>();
			private readonly List<CoreWritingSystemDefinition> _curAnaly =
				new List<CoreWritingSystemDefinition>();

			public TestWSContainer(
				string[] vernacular,
				string[] analysis = null,
				string[] curVern = null,
				string[] curAnaly = null
			)
			{
