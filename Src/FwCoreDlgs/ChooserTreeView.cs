...
namespace SIL.FieldWorks.Common.Controls
{
	...
	public class ChooserTreeView : TriStateTreeView
	{
		...
		/// </summary>
		/// ------------------------------------------------------------------------------------
		public virtual List<int> SelectedHvos
		{
			get
			{
				List<int> selectedHvos = new List<int>();
				foreach (TreeNode node in Nodes)
					GetSelectedPossibilites(node, selectedHvos);
		...
		/// </summary>
		/// ------------------------------------------------------------------------------------
		protected virtual void GetSelectedPossibilites(TreeNode node, List<int> selectedHvos)
		{
			if (IsNodeSelected(node))
			{
				ICmPossibility poss = node.Tag as ICmPossibility;
				if (poss != null)
					selectedHvos.Add(poss.Hvo);
			}

			foreach (TreeNode childNode in node.Nodes)
				GetSelectedPossibilites(childNode, selectedHvos);
		}
		...
		{
			string text = StringUtils.NormalizeToNFC(GetPossibilityText(possibility));
		...
		public void UpdateSelectedLabel()
		{
			if (m_lblSelectedCategories != null)
			{
				...
				m_lblSelectedCategories.Text = (bldr.Length == 0 ?
					SIL.FieldWorks.FwCoreDlgs.Properties.Resources.kstidNoPossibilitySelectedText :
					m_lblSelectedCategories.Text = bldr.ToString().TrimEnd(',', ' '));
			}
		}
