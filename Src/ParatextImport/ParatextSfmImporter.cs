...
namespace ParatextImport
{
	...
	public sealed class BTPictureInfo
	{
		...
		/// ------------------------------------------------------------------------------------
		public BTPictureInfo(string captionText, string sCopyright, int ws, string filename, int lineNumber, string segment,
			BCVRef reference)
		{
	...
	/// </remarks>
	public class ParatextSfmImporter : IDisposable
	{
		...
		/// </summary>
		internal class ToolboxPictureInfo
		{
			/// <summary>String builder for the picture caption.</summary>
			internal ITsIncStrBldr Caption;
			...
			/// </summary>
			internal ITsString TssCaption => Caption?.GetString();

			/// <summary>
			/// Determines whether we have a Description specified for the given writing system.
			/// </summary>
			/// <param name="ws">The writing system ID.</param>
			internal bool HasDescriptionForWs(int ws)
			{
				return Description != null && Description.ContainsKey(ws);
			}
			...
			internal void AddDescriptionVariant(string description, int ws)
			{
		...
		/// </summary>
		private string m_savedVersionDescription;
		...
		/// </summary>
		private ImportStyleProxy m_ScrSectionHeadParaProxy;

		/// <summary>
		/// Background Section Head paragraph style proxy
		/// </summary>
		private ImportStyleProxy m_DefaultIntroSectionHeadParaProxy;

		/// <summary>
		/// Default Scripture paragraph style proxy
		/// </summary>
		private ImportStyleProxy m_DefaultScrParaProxy;

		/// <summary>
		/// Default Background paragraph style proxy
		/// </summary>
		private ImportStyleProxy m_DefaultIntroParaProxy;
		...
		private bool m_fInBookTitle;

		/// <summary>Currently processing a section heading</summary>
		private bool m_fInSectionHeading;
		...
		private bool m_fInVerseTextParagraph;
		...
		private int m_iCurrFootnote;

		/// <summary>The current footnote; null if <see cref="m_fInFootnote"/> is both false.</summary>
		protected IScrFootnote m_CurrFootnote;

		/// <summary>Text props for chapter number character style</summary>
		private ITsTextProps m_ttpChapterNumber;

		/// <summary>Previous book number (in a string)</summary>
		private string m_sPrevBook;
		...
		private List<string> m_unmappedMarkers = new List<string>();
		...
		private Dictionary<string, ImportStyleProxy> m_notesStyleProxies = new Dictionary<string, ImportStyleProxy>();

		/// <summary>The last paragraph that was created</summary>
		private IScrTxtPara m_lastPara;

		#region Special paragraph style proxies, used as fallbacks in case import data lacks a para style:

		/// <summary>
		/// Book Title paragraph style proxy
		/// </summary>
		private ImportStyleProxy m_BookTitleParaProxy;

		/// <summary>Default Footnote paragraph style proxy</summary>
		private ImportStyleProxy m_DefaultFootnoteParaProxy;

		/// <summary>Default Annotation style proxy</summary>
		private ImportStyleProxy m_DefaultAnnotationStyleProxy;
		...
		/// </summary>
		private ITsStrBldr m_BTFootnoteStrBldr;

		/// <summary>
		/// List of pictures in current paragraph in the vernacular
		/// </summary>
		private List<ICmPicture> m_CurrParaPictures = new List<ICmPicture>();
		...
		/// </summary>
		private List<BTPictureInfo> m_BTPendingPictures = new List<BTPictureInfo>();
		...
		/// </summary>
		private List<BtFootnoteBldrInfo> m_BtFootnoteStrBldrs = new List<BtFootnoteBldrInfo>();

		/// <summary>
		/// Collection of ScrAnnotationInfo objects with information about
		/// imported notes that are waiting to be merged.
		/// </summary>
		private List<ScrAnnotationInfo> m_PendingAnnotations = new List<ScrAnnotationInfo>();

		/// <summary>
		/// Dictionary of indexes to the current footnote in the back translation(s) (one entry for
		/// each WS).
		/// </summary>
		private Dictionary<int, int> m_BTfootnoteIndex = new Dictionary<int, int>();
		...
		private int m_wsOfPrevImportStream = -1;
		...
		private bool m_fBackTransVerseNumPending;
		...
		private List<int> m_btPendingVerseNumAdded = new List<int>();

		/// <summary>True if chapter number is ready to be inserted into the back translation(s).</summary>
		private bool m_fBackTransChapterNumPending;
...
#pragma warning restore 414
		...
		/// </returns>
		internal static ScrReference Import(IScrImportSet settings, LcmCache cache, LcmStyleSheet styleSheet, UndoImportManager undoManager, ParatextImportUi importCallbacks)
		{
		...
		/// <summary>
		/// Initializes a new instance of the ParatextSfmImporter class
		/// </summary>
		/// <param name="settings">Import settings object (filled in by wizard)</param>
		/// <param name="cache">The cache used to import to and get misc. info. from.</param>
		/// <param name="styleSheet">Stylesheet from which to get scripture styles.</param>
		...
		/// </summary>
		private string DefaultSvDescription => Properties.Resources.kstidStandardFormatImportSvDesc;
		...
		private void UpdateProgressDlgForBook(string baseFormat)
		{
		...
		private void StartBookTitle()
		{
			...
			var cTitleParas = m_Title.ParagraphsOS.Count;
			for (var i = 0; i < cTitleParas; i++)
			{
		...
		/// </summary>
		private void PrepareToImportNewBook()
		{
			UpdateProgressDlgForBook(Properties.Resources.kstidImportingBook);
			CurrentBook = m_undoManager.AddNewBook(m_nBookNumber, SavedVersionDescription, out m_Title);
		...
		/// </summary>
		private string SavedVersionDescription
		{
			get
			{
		...
		protected void FinalizePrevSection()
		{
			...
			var bcvRef = (m_firstImportedRef.IsEmpty)
				? m_currSection.VerseRefMin
		...
		/// </summary>
		private void SetBookAnnotations()
		{
			var curBookNotes = m_scr.BookAnnotationsOS[m_nBookNumber - 1].NotesOS;
			m_existingAnnotations = new Dictionary<ScrNoteKey, IScrScriptureNote>(curBookNotes.Count);
			foreach (var ann in curBookNotes)
			{
		...
		private void FindOrCreateAnnotation(ScrAnnotationInfo info, ICmObject annotatedObj)
		{
			...
			// insert this note.
			ann = m_undoManager.InsertNote(info.startReference, info.endReference, annotatedObj, GetAnnotDiscussionParaBldr(info), info.guidAnnotationType);
		...
		private void StartingNewBook()
		{
			...
			lock (this)
				m_sPrevBook = CurrentBook.BestUIName;
		}
		...
		/// </summary>
		protected IScrBook CurrentBook { get; set; }
		...
		internal static string RemoveControlCharacters(string sText)
		{
			...
			var bldr = new StringBuilder(sText.Length);
		...
		protected void Initialize()
		{
			...
			// of a book someday.
			var endRef = new ScrReference(m_settings.EndRef, m_scr.Versification);
			var startRef = new ScrReference(m_settings.StartRef, m_scr.Versification);
			...
			{
				var cChapters = ScrReference.GetNumberOfChaptersInRange(SOWrapper.BooksPresent, startRef, endRef);
				var nMax = m_settings.ImportTranslation ? cChapters : 0;
			...
			m_BookTitleParaProxy = new ImportStyleProxy(ScrStyleNames.MainBookTitle, StyleType.kstParagraph, m_wsVern, ContextValues.Title, m_styleSheet);
			Debug.Assert(m_BookTitleParaProxy.Context == ContextValues.Title);

			m_ScrSectionHeadParaProxy = new ImportStyleProxy(ScrStyleNames.SectionHead, StyleType.kstParagraph, m_wsVern, ContextValues.Text, m_styleSheet);
			m_DefaultIntroSectionHeadParaProxy = new ImportStyleProxy(ScrStyleNames.IntroSectionHead, StyleType.kstParagraph, m_wsVern, ContextValues.Intro, m_styleSheet);

			m_DefaultScrParaProxy = new ImportStyleProxy(ScrStyleNames.NormalParagraph, StyleType.kstParagraph, m_wsVern, ContextValues.Text, m_styleSheet);

			m_DefaultIntroParaProxy = new ImportStyleProxy(ScrStyleNames.IntroParagraph, StyleType.kstParagraph, m_wsVern, ContextValues.Intro, m_styleSheet);

			m_DefaultFootnoteParaProxy = new ImportStyleProxy(ScrStyleNames.NormalFootnoteParagraph, StyleType.kstParagraph, m_wsVern, ContextValues.Note, m_styleSheet);

			m_DefaultAnnotationStyleProxy = new ImportStyleProxy(ScrStyleNames.Remark, StyleType.kstParagraph, m_wsAnal, ContextValues.Annotation, m_styleSheet);

			// Make a paragraph builder. We will keep re-using this every time we build a paragraph.
			m_ParaBldr = new StTxtParaBldr(m_cache)
			{
				ParaStylePropsProxy = m_DefaultIntroParaProxy
			};
			...
			m_scrTranslatorAnnotationDef = m_cache.ServiceLocator.GetInstance<ICmAnnotationDefnRepository>().GetObject(CmAnnotationDefnTags.kguidAnnTranslatorNote);
		}
		...
		/// </summary>
		/// <remarks>Virtual for testing override purposes.</remarks>
		private void LoadScriptureProject()
		{
		...
		/// imported.</exception>
		private static void StopImport()
		{
		...
		internal bool PrevRunIsVerseNumber(ITsStrBldr bldr)
		{
			if (bldr == null)
			{
				return false;
			...
			var propsOfLastRun = bldr.get_Properties(bldr.RunCount - 1);

			return (propsOfLastRun.GetStrPropValue((int)FwTextPropType.ktptNamedStyle) == ScrStyleNames.VerseNumber);
		}
		...
		protected void ProcessSegment()
		{
			...
			else if (CurrentBook == null && m_settings.ImportTranslation && m_settings.ImportBackTranslation)
			{
				throw new ScriptureUtilsException(SUE_ErrorCode.UnexcludedDataBeforeIdLine,
					SOWrapper.CurrentFileName, SOWrapper.CurrentLineNumber, m_sMarker + " " + m_sSegmentText,
					string.Empty, string.Empty, string.Empty);
			...
			if (m_styleProxy.StyleId == ScrStyleNames.FootnoteTargetRef)
			{
				if (m_fInFootnote && m_CurrFootnote != null && m_fInterpretFootnoteSettings)
				{
					...
					var proxy = (ImportStyleProxy)m_ParaBldr.ParaStylePropsProxy;
			...
			{
				if (!m_fInScriptureText && m_styleProxy.Context == ContextValues.Text && m_styleProxy.Structure == StructureValues.Body)
				{
			...
			if (ProcessingParagraphStart)
			{
				m_vernParaStyleProxy = (m_styleProxy.StyleType == StyleType.kstParagraph) ? m_styleProxy : m_BookTitleParaProxy;
			...
			var isVerse = HandleVerseNumbers();
		...
		private void CheckForPendingFootnoteText()
		{
			...
			if ((m_currDomain != MarkerDomain.Footnote || m_sMarker != m_sFootnoteEndMarker) && m_styleProxy.Context != ContextValues.Text)
			{
		...
		{
			var styleProxies = (m_importDomain == ImportDomain.Annotations ? m_notesStyleProxies : m_styleProxies);
		...
		private bool HandleSpecialTargets()
		{
			...
			switch (m_styleProxy.MappingTarget)
			{
				...
				case MappingTargetType.Figure:
					...
					if (m_importDomain == ImportDomain.BackTrans || (m_styleProxy.Domain & MarkerDomain.BackTrans) != 0)
					{
					...
					else
					{
						...
						{
							throw new ScriptureUtilsException(SUE_ErrorCode.InvalidPictureParameters, SOWrapper.CurrentFileName, e);
						}
						if (m_styleProxy.EndMarker != null)
						{
							SetInCharacterStyle(); // treat this like a character style
				...
				case MappingTargetType.FigureCaption:
					...
					else
					{
						...
						m_currPictureInfo.Caption.SetIntPropValues((int)FwTextPropType.ktptWs, (int)FwTextPropVar.ktpvDefault, m_wsVern);
				...
				case MappingTargetType.FigureCopyright:
					...
					{
						if (!string.IsNullOrEmpty(m_currPictureInfo?.Copyright))
						{
				...
				case MappingTargetType.FigureDescription:
					{
						var ws = m_styleProxy.WritingSystem <= 0 ? m_wsAnal : m_styleProxy.WritingSystem;
				...
				case MappingTargetType.FigureFilename:
					if (!string.IsNullOrEmpty(m_currPictureInfo?.PictureFilename))
					{
				...
				case MappingTargetType.FigureLayoutPosition:
					if (!string.IsNullOrEmpty(m_currPictureInfo?.LayoutPos))
					{
				...
				case MappingTargetType.FigureRefRange:
					if (!string.IsNullOrEmpty(m_currPictureInfo?.LocationRange))
					{
				...
				case MappingTargetType.TitleShort:
					{
						var ws = m_styleProxy.WritingSystem;
						...
						CurrentBook?.Name.set_String(ws, TsStringUtils.MakeString(m_sSegmentText.Trim(), ws));
		...
		internal string EnsurePictureFilePathIsRooted(string textRep)
		{
			...
			var iFileNameStart = textRep.IndexOf('|');
			...
			var pictureTextRep = textRep.Substring(iFileNameStart + 1);
			var iFileNameEnd = pictureTextRep.IndexOf('|');
			if (iFileNameEnd <= 0)
			{
				return textRep; // text representation invalid
			}

			var fileName = pictureTextRep.Substring(0, iFileNameEnd);

			// Picture text representation already has folder information for file.
			return textRep.Substring(0, iFileNameStart + 1) + GetRootedPath(fileName) + pictureTextRep.Substring(iFileNameEnd);
		}
			...
			if (IsRelativePath(fileName))
			{
				try
				{
					...
					if (fullPath == null)
					{
						...
						foreach (var folder in SOWrapper.ExternalPictureFolders)
						{
					...
					return fullPath ?? Path.Combine(SOWrapper.ExternalPictureFolders[0], fileName);
				}
		...
		private bool SkipIntroMaterial()
		{
			...
			if (m_fCurrentSectionIsIntro && !m_settings.ImportBookIntros && m_styleProxy.StyleType == StyleType.kstCharacter)
			{
			...
			if (m_context == ContextValues.Intro)
			{
				...
				if (m_fInSectionHeading && !m_fCurrentSectionIsIntro && m_styleProxy.Structure == StructureValues.Heading)
				{
					m_fInScriptureText = true;
				}
				if (m_fInScriptureText)
				{
					var verse = SOWrapper.SegmentFirstRef.Verse;
		...
		private void HandleNoteDomain()
		{
			...
			if (m_styleProxy.Function == FunctionValues.Verse)
			{
				...
				if (!string.IsNullOrEmpty(m_sSegmentText))
				{
					AddNewAnnotation();
				}
				return;
			...
			else
			{
				...
				{
					var runProps = m_styleProxy.TsTextProps;
					if (m_styleProxy.WritingSystem <= 0)
					{
						var tpb = runProps.GetBldr();
						tpb.SetIntPropValues((int)FwTextPropType.ktptWs, 0, m_SOWrapper.CurrentWs(m_wsAnal));
				...
				{
					var info = m_PendingAnnotations[m_PendingAnnotations.Count - 1];
		...
		private void AddNewAnnotation(ImportStyleProxy paraProxy, ITsTextProps runProps)
		{
			var paraBldr = new StTxtParaBldr(m_cache)
			{
				ParaStylePropsProxy = paraProxy
			};
			paraBldr.AppendRun(m_sSegmentText, runProps);
			var guidAnnotationType = Guid.Empty;
			try
			{
				...
				{
					guidAnnotationType = m_cache.ServiceLocator.GetInstance<ICmAnnotationDefnRepository>().GetObject(m_SOWrapper.CurrentAnnotationType).Guid;
				}
			}
			catch
			{
			...
			var lastRef = SOWrapper.SegmentLastRef;
		...
		{
			foreach (var info in m_PendingAnnotations)
			{
				if (m_importDomain == ImportDomain.Annotations)
				{
					...
					m_lastPara = m_currentRef.Verse == 0 ? null : FindCorrespondingVernParaForAnnotation(info.startReference);
				}

				info.bldrsDiscussion[0].TrimTrailingSpaceInPara();

				FindOrCreateAnnotation(info, m_lastPara ?? (ICmObject)CurrentBook);
			}
		...
		/// </summary>
		/// <param name="info">The information about a annotation being imported.</param>
		private StTxtParaBldr GetAnnotDiscussionParaBldr(ScrAnnotationInfo info)
		{
		...
		private void HandleBookTitle(ref bool fInsertSegment)
		{
			...
			else if (m_fInBookTitle)
			{
				...
				if (m_styleProxy.StyleType == StyleType.kstCharacter && m_context == ContextValues.General)
				{
					fInsertSegment = true;
				}
		...
		private void HandleFootnotes(ref bool fInsertSegment)
		{
			...
			if (m_fInFootnote && (m_styleProxy.StyleType == StyleType.kstParagraph || (m_fGotFootnoteMarker && m_styleProxy.StyleId == ScrStyleNames.FootnoteMarker)))
			{
				EndFootnote();
			}
			if (!m_fInFootnote)
		...
		private void ProcessVerseNumbers()
		{
			if (m_settings.ImportTranslation)
			{
				...
				if (m_importDomain == ImportDomain.Main)
				{
					...
					var sVerseRef = GetVerseRefAsString(0);
		...
		private void HandleOtherParagraphStyles()
		{
			...
			m_fInVerseTextParagraph = (m_context == ContextValues.Text && m_styleProxy.Structure == StructureValues.Body);
		}
		...
		private void InsertSegment()
		{
			...
			{
				// If appropriate, insert drop-cap chapter number
				if (m_fChapterNumberPending && m_context == ContextValues.Text && m_styleProxy.Structure == StructureValues.Body && !m_fInFootnote)
				{
					AddDropChapterNumToPara(m_ParaBldr.StringBuilder, 0);
				}
				...
				{
					var fFoundPunctuation = false;
					for (var ich = kMaxParaSizeForVerseAndPuncBrk - m_ParaBldr.Length;
						...
						ich++)
					{
						...
						var sSave = m_sSegmentText.Substring(ich + 1);
		...
		{
			ICmPicture picture;
			var picFactory = m_cache.ServiceLocator.GetInstance<ICmPictureFactory>();
			if (m_currPictureInfo == null)
			{
				var tokens = m_sSegmentText.Split('|');
				if (tokens.Length < 6)
				{
					throw new ArgumentException("The USFM format for a Picture was invalid");
				}
				var sDescription = tokens[0];
				var srcFilename = tokens[1];
				if (string.IsNullOrEmpty(srcFilename))
				{
					srcFilename = Path.Combine(FwDirectoryFinder.CodeDirectory, "MissingPictureInImport.bmp");
				}
				var sLayoutPos = tokens[2];
				var sLocationRange = tokens[3];
				var sCopyright = tokens[4];
				var sCaption = tokens[5];
			...
			{
				var srcFilename = m_currPictureInfo.PictureFilename;
				if (string.IsNullOrEmpty(srcFilename))
				{
		...
		/// </summary>
		private void ProcessBackTransSegment()
		{
			if (m_styleProxy.Function == FunctionValues.Chapter)
			{
				ProcessStartOfChapter();
			...
			if (CurrentBook == null)
			{
				...
				// doesn't exist.
				throw new ScriptureUtilsException(SUE_ErrorCode.BackTransMissingVernBook, null, null, ScrReference.NumberToBookCode(m_nBookNumber), null, null, true);
			}
			...
			var fThisSegmentEndsAFootnote = false;
			if (m_importDomain == ImportDomain.Main)
			{
				var wsBT = BackTransWS;
				...
				else if (m_fInFootnote && (m_currDomain & MarkerDomain.Footnote) != 0 && m_styleProxy.StyleType == StyleType.kstParagraph)
				{
					EndFootnote();
					fThisSegmentEndsAFootnote = (m_currDomain & MarkerDomain.Footnote) == 0;
				}
			...
			else
			{
				if (ProcessingParagraphStart)
				{
					if (m_importDomain == ImportDomain.Main)
					{
						...
						if (m_vernParaStyleProxy.StyleId != m_styleProxy.StyleId && m_styleProxy.Style.Type != StyleType.kstCharacter)
						{
			...
			if ((m_currDomain & MarkerDomain.Footnote) != 0 && !fThisSegmentEndsAFootnote)
			{
				...
				if (m_styleProxy.StyleType == StyleType.kstCharacter && m_styleProxy.Context != ContextValues.EndMarker)
				{
					SetInCharacterStyle();
				}
			}
			else if (m_styleProxy.StyleType == StyleType.kstCharacter && m_styleProxy.Function != FunctionValues.Verse)
			{
				SetInCharacterStyle();
			}
			...
			if (m_fInSectionHeading && strbldr.Length > 0 && ProcessingParagraphStart && m_importDomain == ImportDomain.Main)
			{
		...
		/// </summary>
		private bool SkipThisBook => m_nSkipBookNumber == m_nBookNumber;
		...
		{
			Debug.Assert(wsBT > 0);
			var ttpBtSeg = m_styleProxy.TsTextProps;
			...
			if (m_styleProxy.Function == FunctionValues.Verse)
			{
				...
				else
				{
					...
					BCVRef firstRef = 0;
					BCVRef lastRef = 0;
					if (BCVRef.VerseToScrRef(m_sSegmentText, out sLiteralVerse, out sRemainingText, ref firstRef, ref lastRef))
					{
		...
		/// later.
		/// </summary>
		private void ProcessBtParaStart()
		{
			if (m_vernParaStyleProxy == null || m_vernParaStyleProxy.Context != m_styleProxy.Context)
			{
				m_iNextBtPara = 0;
				m_iCurrSection = 0;
			}
		...
		private IScrTxtPara FindCorrespondingVernParaForAnnotation(BCVRef targetRef)
		{
			m_lastPara = null;
			if (CurrentBook != null)
			{
				m_lastPara = CurrentBook.FindPara(null, targetRef, 0, ref m_iCurrSection);
			}

			return m_lastPara ?? (m_lastPara = m_scr.FindPara(null, targetRef, 0, ref m_iCurrSection));
		}
		...
		/// <returns>The corrersponding StTxtPara, or null if no matching para is found</returns>
		protected IScrTxtPara FindCorrespondingVernParaForSegment(IStStyle style, BCVRef targetRef, out bool fAppend)
		{
			fAppend = false;

			var iCurrSectionTemp = m_iCurrSection;
			m_lastPara = CurrentBook.FindPara(style, targetRef, m_iNextBtPara, ref m_iCurrSection);
		...
		private ICmPicture FindCorrespondingPicture(int index)
		{
			return index >= m_CurrParaPictures.Count ? null : m_CurrParaPictures[index];
		}
		...
		private void EndBTFootnote()
		{
			...
			{
				m_BtFootnoteStrBldrs.Add(new BtFootnoteBldrInfo(m_wsCurrBtPara, m_BTFootnoteStrBldr, m_sBtFootnoteParaStyle, ichMarker, m_currentRef));
			}
			m_fInFootnote = false;
		...
		/// Translation.
		/// </summary>
		private void AddChapterAndVerseNumsToBackTranslations()
		{
			m_fBackTransVerseNumPending = true;
			m_btPendingVerseNumAdded.Clear();

			foreach (var kvp in m_BTStrBldrs)
			{
		...
		private void AddPendingVerseAndChapterNumsToBackTrans(string sVerseRef, int ws, ITsStrBldr bldr)
		{
			...
			AddTextToPara(sVerseRef, StyleUtils.CharStyleTextProps(ScrStyleNames.VerseNumber, ws), bldr);
		...
		{
			if (m_fInCharStyle && (m_sMarker == m_sCharStyleEndMarker || m_sCharStyleEndMarker == null))
			{
		...
		private void ProcessStartOfBook()
		{
			...
			if (m_importDomain != ImportDomain.Main || !m_settings.ImportTranslation)
			{
				...
				CurrentBook = m_undoManager.PrepareBookNotImportingVern(m_nBookNumber, m_fImportingToMain);

				if (m_importDomain == ImportDomain.BackTrans && CurrentBook == null && m_settings.ImportBackTranslation)
				{
				...
				SOWrapper.SetBookCheckSum(CurrentBook);
				return;
			}
			...
			var idText = m_sSegmentText.Trim();
			var spacePos = idText.IndexOf(" ");
			CurrentBook.IdText = spacePos == -1 ? string.Empty : idText.Substring(spacePos + 1);
		...
		private void ProcessBookTitleStart()
		{
			...
			// set paragraph style property for new para
			m_ParaBldr.ParaStylePropsProxy = (m_styleProxy.StyleType == StyleType.kstParagraph) ? m_styleProxy : m_BookTitleParaProxy;
		...
		private void CheckForSectionHeadInIntroMaterial()
		{
			...
			{
				if (para.StyleRules == StyleUtils.ParaStyleTextProps(ScrStyleNames.SectionHead))
				{
		...
		{
			AddBookTitleSegment(m_ParaBldr.StringBuilder, m_ParaBldr.ParaStylePropsProxy, m_vernTextProps);
		}
		...
		private void AddBookTitleSegment(ITsStrBldr bldr, IParaStylePropsProxy titleParaStyleProxy, ITsTextProps props)
		{
			...
			AddTextToPara(m_sSegmentText, m_styleProxy == titleParaStyleProxy ? props : m_styleProxy.TsTextProps, bldr);
		}
		...
		{
			var sTitle = m_sSegmentText.TrimEnd();
			var ws = m_styleProxy.WritingSystem;
			if (ws == -1)
			{
				ws = GetWsForImportDomain();
			...
			var tssBookName = CurrentBook.Name.get_String(ws);
			if (m_styleProxy.StyleId == ScrStyleNames.MainBookTitle && sTitle.Length > 0 && tssBookName != null && String.IsNullOrEmpty(tssBookName.Text))
			{
		...
		private void ProcessSectionHeadStart()
		{
			...
			if (m_settings.ImportTranslation && (m_currSection == null || !m_currSection.IsValidObject || m_currSection.ContentOA.ParagraphsOS.Count > 0))
			{
				MakeSection();
			}
		...
		private void AddSectionHeadSegment()
		{
			...
			if (m_styleProxy != m_ParaBldr.ParaStylePropsProxy && m_styleProxy.StyleType == StyleType.kstParagraph)
			{
				FinalizePrevParagraph();
				// set paragraph style property for new para
				m_ParaBldr.ParaStylePropsProxy = m_styleProxy;
		...
		private void PrepareForFirstScriptureSection()
		{
			...
			// material.
			if (m_settings.ImportTranslation && m_importDomain == ImportDomain.Main && (m_currSection == null || !m_currSection.IsValidObject ||
		...
		{
			return m_importDomain == ImportDomain.Main ? m_wsVern : m_SOWrapper.CurrentWs(m_wsAnal);
		}
		...
		private void FinalizeBook()
		{
			...
			if (m_currSection == null && m_settings.ImportTranslation && CurrentBook != null && m_importDomain == ImportDomain.Main)
			{
				MakeSection();
			}
			FinalizePrevSection();
		...
		/// whatever is missing.
		/// </summary>
		private void FinalizePrevTitle()
		{
			...
			{
				var titleParaBldr = new StTxtParaBldr(m_cache)
				{
					ParaStylePropsProxy = m_BookTitleParaProxy
				};
		...
		/// </summary>
		private void ResetStateVariablesForNewBook()
		{
			m_currSection = null;
			m_iCurrSection = -1;
			m_iCurrFootnote = 0;
		...
		{
			var refFirst = SOWrapper.SegmentFirstRef;
			var refLast = SOWrapper.SegmentLastRef;
			...
			var nSegFirst = refFirst.Segment;
			var nSegLast = refLast.Segment;

			// build the reference string to return using first verse info
			var sRef = nSegFirst > 0 ? string.Concat(sVerseFirst, Convert.ToChar(nSegFirst + 96)) : sVerseFirst;
			...
			if (sVerseFirst != sVerseLast || nSegFirst != nSegLast)
			{
				...
				sRef = string.Concat(sRef, m_scr.BridgeForWs(wsBt), sVerseLast, (nSegLast > 0) ? Convert.ToChar(nSegLast + 96).ToString() : string.Empty);
			}
			return sRef;
		}
		...
		private void AddTextToPara(string sText)
		{
		...
		private void AddTextToPara(string sText, ITsTextProps pttpProps, ITsStrBldr strbldr)
		{
			...
			if (sText.Length <= 0 || (sText == " " && m_styleProxy.StyleType != StyleType.kstCharacter))
			{
				return;
			}
			...
			// a ws is specified.
			int var;
			var ws = pttpProps.GetIntPropValues((int)FwTextPropType.ktptWs, out var);
			if (ws == -1)
			{
				ws = GetWsForContext(strbldr);
				var tpb = pttpProps.GetBldr();
		...
		private int GetWsForContext(ITsStrBldr strbldr)
		{
			...
			if (strbldr != null)
			{
				var cch = strbldr.Length;
				if (cch > 0)
				{
					var ttpCur = strbldr.get_PropertiesAt(cch - 1);
					int var;
					ws = ttpCur.GetIntPropValues((int)FwTextPropType.ktptWs, out var);
				}
			}
			if (ws <= 0)
			{
		...
		private void AddDropChapterNumToPara(ITsStrBldr bldr, int ws)
		{
			Debug.Assert(ScrStyleNames.ChapterNumber == m_ttpChapterNumber.GetStrPropValue((int)FwTextPropType.ktptNamedStyle));

			// Compute the properties for the chapter number. If this is in BT, ws will be
			// non-zero; we use the specified ws.
			var chapterNumProps = m_ttpChapterNumber;
			var fVernacular = (ws == 0);
			if (!fVernacular)
			{
				var propsBldr = chapterNumProps.GetBldr();
				propsBldr.SetIntPropValues((int)FwTextPropType.ktptWs, 0, ws);
			...
			// space if necessary.
			var cchBldr = bldr.Length;
			if (cchBldr > 0 && bldr.GetChars(cchBldr - 1, cchBldr) != " ")
			{
				var spaceProps = GetTextPropsWithWS(ws, bldr);
				AddTextToPara(" ", spaceProps, bldr);
			}

			// now insert the chapter number
			var sChapterNumber = fVernacular ? m_scr.ConvertToString(m_nChapter) : m_nChapter.ToString();
		...
		/// new paragraph.
		/// </summary>
		private void FinalizePrevParagraph()
		{
			...
			if (m_ParaBldr.Length > 0 && CurrentBook != null)
			{
				m_ParaBldr.TrimTrailingSpaceInPara();
				...
				else
				{
					...
					m_lastPara = (IScrTxtPara)m_ParaBldr.CreateParagraph(m_fInSectionHeading ? m_sectionHeading : m_sectionContent);
				}
			}
		...
		/// </summary>
		private void AddBackTranslations()
		{
		...
		{
			var bldr = m_BTStrBldrs.Keys.Select(key => m_BTStrBldrs[key]).First();
			var sCharStyle = bldr.get_Properties(0).GetStrPropValue((int)FwTextPropType.ktptNamedStyle) ?? StyleUtils.DefaultParaCharsStyleName;
			...
			throw new ScriptureUtilsException(
				...
				m_SOWrapper.CurrentFileName, CurrentBook.BookId,
		...
		private ICmTranslation CreateCmTranslationForPendingBT(int ws, string btText, out bool fAppend)
		{
			fAppend = false;
			if (m_prevImportDomain == ImportDomain.BackTrans || (m_prevImportDomain == ImportDomain.Main && !m_settings.ImportTranslation))
			{
				var targetRef = (BtChapterNumPending(ws) && m_vernParaStyleProxy.Style.Structure == StructureValues.Body) ? new BCVRef(m_prevRef) : new BCVRef(m_currentRef);
				if (m_vernParaStyleProxy.Style.Context != ContextValues.Intro && targetRef.Verse == 0)
				{
					targetRef.Verse = 1;
				}
				...
				var iSectionTmp = m_iCurrSection;
				if (FindCorrespondingVernParaForSegment(m_vernParaStyleProxy.Style, targetRef, out fAppend) == null)
				{
					// Got an unexpected BT paragraph segment. Can't find a corresponding
					// vernacular paragraph.
					var sParaContents = btText;
			...
			var trans = m_lastPara.GetOrCreateBT();
			return trans;
		}
		...
		/// </summary>
		/// <param name="ws">The writing system of the BT</param>
		/// <param name="bldr">The string builder for the back translation paragraph text</param>
		...
		{
			for (var index = 0; index < m_BTPendingPictures.Count; index++)
			{
				var picture = FindCorrespondingPicture(index);
				var pictureInfo = m_BTPendingPictures[index];
				if (picture == null)
				{
				...
				var tssCaption = pictureInfo.m_strbldrCaption.GetString();
				if (tssCaption != null)
				{
		...
		/// </summary>
		/// <param name="bldr">The string builder</param>
		private static void TrimTrailingSpace(ITsStrBldr bldr)
		{
			var length = bldr.Length;
			if (length == 0)
			{
		...
		private void BeginFootnote()
		{
			...
			var proxy = (m_styleProxy.StyleType == StyleType.kstParagraph) ? m_styleProxy : m_DefaultFootnoteParaProxy;
			...
			else if (m_settings.ImportTypeEnum == TypeOfImport.Other)
			{
				...
				var propsOfLastRun = strbldr.get_Properties(strbldr.RunCount - 1);
				var sStyleOfLastRun = propsOfLastRun.GetStrPropValue((int)FwTextPropType.ktptNamedStyle);
			...
			m_CurrFootnote = CurrentBook.InsertFootnoteAt(m_iCurrFootnote, strbldr, ichMarker);
		...
		/// </summary>
		private bool InMainImportDomain => m_prevImportDomain != ImportDomain.Main;
		...
		/// </summary>
		private void LoadImportMappingProxies()
		{
			Debug.Assert(m_settings != null);
			var fHasNonInterleavedNotes = SOWrapper.HasNonInterleavedNotes;
		...
		private void LoadImportMappingProxies(Dictionary<string, ImportStyleProxy> styleProxies, IEnumerable mappings)
		{
			...
			styleProxies[ScriptureServices.kMarkerBook] = new ImportStyleProxy(null, StyleType.kstParagraph, 0, ContextValues.Book, m_styleSheet);
		}
		...
		/// </remarks>
		protected void AddImportStyleProxyForMapping(ImportMappingInfo mapping, Dictionary<string, ImportStyleProxy> styleProxies)
		{
			...
			if (styleName == null && !mapping.IsExcluded && mapping.MappingTarget == MappingTargetType.TEStyle)
			{
				return;
			}
			...
			if (styleName == StyleUtils.DefaultParaCharsStyleName && !mapping.IsExcluded)
			{
				...
				proxy = new ImportStyleProxy(null, 0, ws, ContextValues.EndMarker, mapping.Domain, m_styleSheet);
			...
			if (!string.IsNullOrEmpty(mapping.EndMarker))
			{
				...
				var wsStuffAfterMarker = -1;
				var endMarkerDomain = MarkerDomain.Default;
