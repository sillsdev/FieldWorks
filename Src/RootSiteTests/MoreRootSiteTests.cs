...
namespace SIL.FieldWorks.Common.RootSites
{
	...
	public class MoreRootSiteTests : RootsiteBasicViewTestsBase
	{
		...
		public override void FixtureSetup()
		{
			...
			// the use.
			ReflectionHelper.SetField(Type.GetType("SIL.LCModel.DomainImpl.ScrFootnote, SIL.LCModel", true), "s_maxAllowedParagraphs", 5);
		}
		...
		{
			ShowForm(TestLanguages.English, DisplayType.kAll);

			var pt = m_basicView.IPLocation;
			Assert.IsTrue(m_basicView.ClientRectangle.Contains(pt), "IP is not in Draft View's client area.");
		...
		public void AdjustScrollRange_InResponseToScrollingAndResizing()
		{
			...
			// 2 fragments each
			var expectedHeight = 2 * (6 * m_basicView.SelectionHeight + DummyBasicViewVc.kMarginTop * rcSrcRoot.Height / DummyBasicViewVc.kdzmpInch)
			...
			expectedHeight = 4 * (6 * m_basicView.SelectionHeight + DummyBasicViewVc.kMarginTop * rcSrcRoot.Height / DummyBasicViewVc.kdzmpInch);
			...
			var nNewWidth = m_basicView.SelectionWidth * 3 / 4;
			...
			expectedHeight = 4 * (12 * m_basicView.SelectionHeight + DummyBasicViewVc.kMarginTop * rcSrcRoot.Height / DummyBasicViewVc.kdzmpInch);
		...
		public void AnotherScrollToEnd()
		{
			...
			var pt = m_basicView.ScrollPosition;
			var rect = m_basicView.DisplayRectangle;
			Assert.AreEqual(-pt.Y + m_basicView.ClientRectangle.Height, rect.Height, "Scroll position is not at the very end");
		}
		...
		/// </summary>
		[Test]
		[Platform(Exclude = "Linux", Reason = "TODO-Linux: This test is too dependent on mono ScrollableControl behaving the same as .NET")]
		public void AdjustScrollRange_VScroll_PosAtTop()
		{
			ShowForm(TestLanguages.English, DisplayType.kAll);
			var view = m_basicView;
#pragma warning disable 219
			var dydWindheight = view.ClientRectangle.Height;
#pragma warning restore 219
			...
			var nPos = -view.ScrollPosition.Y;
			var nHeight = view.DisplayRectangle.Height;

			var fRet = view.AdjustScrollRange(0, 0, 0, 0);
...
#pragma warning restore
		...
		public void AdjustScrollRange_VScroll_PosAlmostAtEnd()
		{
			...
			var maxScrollPos = view.DisplayRectangle.Height - dydWindheight;
		...
		public void AdjustScrollRange_VScroll_ScrollRangeLessThanClientRectangle()
		{
			...
			var dydSomewhere = maxScrollPos;
			view.ScrollPosition = new Point(0, maxScrollPos);
			var nChange = maxScrollPos;
			var nHeight = view.DisplayRectangle.Height;

			var fRet = view.AdjustScrollRange(0, 0, -nChange, 0);
			var nPos = 0;
		...
		public void AdjustScrollRangeTestHScroll()
		{
			ShowForm(TestLanguages.English, DisplayType.kAll);
			var view = m_basicView;
			var dxdWindwidth = view.ClientRectangle.Width;
			view.SkipLayout = true;
			view.HScroll = true;
			...
			view.ScrollPosition = new Point(0, 0);
			var nPos = -view.ScrollPosition.X;
			var nWidth = view.DisplayRectangle.Width;
			...
			// Now test scroll range < ClientRectangle
			var dxdSomewhere = nPos;
			view.ScrollPosition = new Point(scrollMax, 0);
			var nChange = view.DisplayRectangle.Width - dxdWindwidth;
		...
		/// <summary>
		/// Test the RootSite.AdjustScrollRange1 method for simultaneous use of the horizontal
		...
		public void AdjustScrollRangeTestHVScroll()
		{
			...
			var maxYScroll = view.DisplayRectangle.Height - view.ClientRectangle.Height;
			var maxXScroll = view.DisplayRectangle.Width - view.ClientRectangle.Width;

			// 1. Test: Thumb position is at bottom left
			view.ScrollPosition = new Point(10, maxYScroll);
			var nXPos = -view.ScrollPosition.X;
			var nYPos = -view.ScrollPosition.Y;
			var nWidth = view.DisplayRectangle.Width;
			var nHeight = view.DisplayRectangle.Height;

			var fRet = view.AdjustScrollRange(30, 0, -40, 0);
			nWidth += 30;
			nXPos += 30;
			nYPos -= 40;
		...
		public void IsParagraphProps_Basic()
		{
			...
			using (var rgvsliTemp = MarshalEx.ArrayToNative<SelLevInfo>(clev))
			{
				...
				var rgvsliEnd = new SelLevInfo[clev];
...
#region Merge multiple types of translations
		...
		public void MergeTranslationssWhenParasMerge_BothParasHaveTranslations()
		{
			...
			// both paragraphs
			var book = Cache.ServiceLocator.GetInstance<IScrBookRepository>().GetObject(m_hvoRoot);
			var text1 = book.FootnotesOS[0];
			var para1 = (IStTxtPara)text1.ParagraphsOS[0];
			var para2 = AddParaToMockedText(text1, "TestStyle");
			...
			var bt1 = AddBtToMockedParagraph(para1, m_wsEng);
			AddRunToMockedTrans(bt1, m_wsEng, "BT1", null);
			var bt2 = AddBtToMockedParagraph(para2, m_wsEng);
			...
			var levelInfo = new SelLevInfo[2];
...
#region Split BT tests
		...
		public void SplitBTs_BothParasHaveBt()
		{
			...
			var book = Cache.ServiceLocator.GetInstance<IScrBookRepository>().GetObject(m_hvoRoot);
			var text1 = book.FootnotesOS[0];
			var para = (IScrTxtPara)text1.ParagraphsOS[0];
			...
			var para1 = (IStTxtPara)text1.ParagraphsOS[0];
			var para2 = (IStTxtPara)text1.ParagraphsOS[1];
...
#region Merge BT tests - Backspace
		...
		public void MergeBTsWhenParasMerge_BothParasHaveBt()
		{
			...
			var trans1 = AddBtToMockedParagraph(para1, m_wsEng);
			AddRunToMockedTrans(trans1, m_wsEng, "BT1", null);
			var trans2 = AddBtToMockedParagraph(para2, m_wsEng);
		...
		public void MergeBTsWhenParasMerge_FirstParaHasNoBt()
		{
			...
			var book = Cache.ServiceLocator.GetInstance<IScrBookRepository>().GetObject(m_hvoRoot);
			var text1 = (IStText)book.FootnotesOS[0];
			var para1 = (IStTxtPara)text1.ParagraphsOS[0];
			var para2 = AddParaToMockedText(text1, "TestStyle");
			AddRunToMockedPara(para2, DummyBasicView.kSecondParaEng, m_wsEng);
			// We'll just re-use our "vernacular" WS for the back translation, for testing purposes.
			...
			Assert.AreEqual(DummyBasicView.kFirstParaEng + DummyBasicView.kSecondParaEng, para1.Contents.Text);
			using (var translations = para1.TranslationsOC.GetEnumerator())
			{
				translations.MoveNext();
				var transl = translations.Current;
		...
		public void MergeBTsWhenParasMerge_BothParasHaveBtMultiWs()
		{
			...
			var wsfr = m_wsf.GetWsFromStr("fr");
		...
		public void NoMergeBTsWhenSecondParaDeleted_AnchorInSurvivingPara()
		{
			...
			levelInfo[0].cpropPrevious = 0;
			levelInfo[0].ihvo = 0;
			var ichEndPara1 = DummyBasicView.kFirstParaEng.Length;
			var ichEndPara2 = DummyBasicView.kSecondParaEng.Length;
			rootBox.MakeTextSelection(0, 2, levelInfo, StTxtParaTags.kflidContents, 0, ichEndPara1, ichEndPara2, 0, true, 1, null, true);
			TypeBackspace();
		...
		public void NoMergeBTsWhenSecondParaDeleted_AnchorInDyingPara()
		{
			...
			var ichEndPara1 = DummyBasicView.kFirstParaEng.Length;
			var ichEndPara2 = DummyBasicView.kSecondParaEng.Length;
			rootBox.MakeTextSelection(0, 2, levelInfo, StTxtParaTags.kflidContents, 0, ichEndPara2, ichEndPara1, 0, true, 0, null, true);
			TypeBackspace();
		...
		public void PreserveSecondBTWhenFirstParaDeleted()
		{
			...
			levelInfo[0].cpropPrevious = 0;
			levelInfo[0].ihvo = 0;
			rootBox.MakeTextSelection(0, 2, levelInfo, StTxtParaTags.kflidContents, 0, 0, 0, 0, true, 1, null, true);
			TypeBackspace();
		...
		public void MergeBTsWhenParasMerge_ThreeParasWithBt()
		{
			...
			var para3 = AddParaToMockedText(text1, "TestStyle");
			...
			var trans2 = AddBtToMockedParagraph(para2, m_wsEng);
			AddRunToMockedTrans(trans2, m_wsEng, "BT2", null);
			var trans3 = AddBtToMockedParagraph(para3, m_wsEng);
			...
			levelInfo[0].ihvo = 0;
			var ich = DummyBasicView.kFirstParaEng.Length;
			rootBox.MakeTextSelection(0, 2, levelInfo, StTxtParaTags.kflidContents, 0, ich, 0, 0, true, 2, null, true);
			TypeBackspace();
		...
		public void MergeBTsWhenParasMerge_ThreeParas_FromMiddleOfPara1ToMiddleOfPara2()
		{
			...
			var ichEnd = 2;
			rootBox.MakeTextSelection(0, 2, levelInfo, StTxtParaTags.kflidContents, 0, ichAnchor, ichEnd, 0, true, 2, null, true);
			TypeBackspace();
...
#region Tests for displaying/hiding selection when losing focus
		...
		public void LoseFocusToNonView_RangeSel()
		{
			...
			levelInfo[0].cpropPrevious = 0;
			levelInfo[0].ihvo = 0;
			rootBox.MakeTextSelection(0, 2, levelInfo, StTxtParaTags.kflidContents, 0, 0, 3, 0, true, 0, null, true);
			...
			using (var control = new Control())
			{
				...
				Assert.IsTrue(rootBox.Selection.IsEnabled, "Selection should still be enabled if non-view window got focus");
			}
		}
		...
		public void LoseFocusToView_RangeSel_FlagSet()
		{
			...
			Assert.IsTrue(rootBox.Selection.IsEnabled, "Selection should still be enabled if the ShowRangeSelAfterLostFocus flag is set");
		}
		...
		public void LoseFocusToNonView_IP()
		{
			...
			using (var control = new Control())
			{
				...
				Assert.IsFalse(rootBox.Selection.IsEnabled, "Selection should not be enabled if non-view window got focus if we have an IP");
			}
		}
...
#region helper methods
		...
		{
			var rootBox = m_basicView.RootBox;
			var vg = m_basicView.get_ScreenGraphics(rootBox);
			try
			{
				var wsTemp = -1;
				bool fWasComplex;
