...
{
	nodeFactory = &CommonASTNode::factory;
}
...
RefAST ASTFactory::create(int type, const std::string& txt)
{
	RefAST t = create();
 ...
 *  an AST node type, then create a default one: CommonAST.
 */
RefAST ASTFactory::create(const AST* tr)
{
 ...
 */
RefAST ASTFactory::dup(const AST* t)
{
	return create(t);		// if t==null, create returns null
}
/** Duplicate tree including siblings of root. */
RefAST ASTFactory::dupList(const AST* t)
{
...
RefAST ASTFactory::make(std::vector<RefAST> nodes)
{
	if ( nodes.size()==0 )
		return RefAST(0);
	RefAST root = nodes[0];
	RefAST tail = RefAST(0);
	if (root) {
		root->setFirstChild(RefAST(0));	// don't leave any old pointers set
	}
	// link in children;
