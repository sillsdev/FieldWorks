...
{
	AssertObj(this);
	Assert((unsigned int)ieIns <= (unsigned int)m_ieLim);
	Assert(ceIns >= 0);
	// Checks for overflow.
	...
	if (ieIns < m_ieLim)
		memmove(m_prge + ieIns + ceIns, m_prge + ieIns, sizeof(T) * (m_ieLim - ieIns));
...
{
	AssertObj(this);
	Assert((unsigned int)ieMin <= (unsigned int)ieLim && (unsigned int)ieLim <= (unsigned int)m_ieLim);
	...
	if (prge)
	{
		...
		memcpy(prge, m_prge + ieMin, (ieLim - ieMin)*sizeof(T));
...
{
	AssertObj(this);
	Assert((unsigned int)ieMin <= (unsigned int)ieLimDel && (unsigned int)ieLimDel <= (unsigned int)m_ieLim);
