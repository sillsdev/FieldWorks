...
----------------------------------------------------------------------------------------------*/
Rect GrSlotOutput::BoundingBox(Font & font)
{
	// Original code: Note that this calls GlyphMetricLogUnits four times.
	// Due to performance reasons I (EberhardB) commented that code and
	// duplicate the code from GrSlotAbstract::GetGlyphMetricAux.
	//
	//Rect rectBB;
	...
	gr::Point ptAdvances;
	gr::Rect rectBb;
	font.getGlyphMetrics(m_chwActual, rectBb, ptAdvances);

	rectBb.left += m_xsPositionX;
	if (IsSpace())
		rectBb.right = m_xsPositionX + ptAdvances.x;
	else
		rectBb.right += m_xsPositionX;
	rectBb.top += m_ysPositionY;
	rectBb.bottom += m_ysPositionY;
	return rectBb;
}
