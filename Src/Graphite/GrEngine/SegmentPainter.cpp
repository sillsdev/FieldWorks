...
//#define kchwHardLineBreak (utf16)0x2028
//#endif
	...
	float ydLineTop, float ydLineBottom, bool bOn)
{
	...
	float xdSegLeft = ScaleXToDest(0);
	...
	// Allow subclasses to make adjustments if needed.
	AdjustHighlightRectangles(ydLineTop, ydLineBottom, xdSegLeft, xdSegRight, vrd);
	...
	float * prgxdLefts, float * prgxdRights, float * prgydUnderTops)
{
	...
	for (ils = 0; ils < std::min(cxdRet, size_t(cxdMax)); ils++)
	{
