...
class GrGlyphAttrRun
{
	...
	{
		return (ByteCountHeader(fxdGlatVersion) + (m_cAttrs * isizeof(data16)));
	}
...
protected:
	...
	int GlyphAttr16BitValue(int ibMin, int ibLim, data16 nAttrID);
...
public:
	...
	bool ReadFromFont(GrIStream & gloc_strm, int cGlyphs,
		GrIStream & glat_strm, long lGlatStart, int fxdSilfVersion);
