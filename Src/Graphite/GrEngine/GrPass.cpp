	...
	int * pnRet, int * pcslotGot, int * pislotFinalBreak)
{
	...
	if (psstrmOut->PastEndOfPositioning(true))
		//||	// sufficiently past inserted break point
		//	(m_ipass == ptman->NumberOfLbPasses() + 1
		//	&& *pislotFinalBreak > -1
		//	&& *pislotFinalBreak < psstrmOut->WritePos()))
	{
		DoCleanUpSegLim(ptman, psstrmOut, twsh);
		psstrmOut->MarkFullyWritten();
	}
		...
		if (psstrmIn->SlotsPending() < 1 || !ptman->Pass(m_ipass-1)->DidResyncSkip())
		{
			...
			else
			{
				...
				*pcslotGot = cslotGot;
				...
				//for (int ix = 0; ix < psstrmOut->WritePos(); ix++)
				//{
				//	GrSlotState * pslotX = psstrmOut->SlotAt(ix);
				//	Assert(pslotX->DirHasBeenProcessed());
				//	int temp;
				//	temp = 3;
				//}
