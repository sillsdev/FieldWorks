...
typedef uint8_t utf8;			// UTF-8 encoded Unicode codepoints
typedef uint16_t gid16;	// glyph ID
typedef uint16_t data16;	// generic 16-bit data
typedef uint32_t data32;	// generic 32-bit data
typedef uint32_t fontTableId32;	// ID to pass to getTable()
#else
/* These should be correct on 64 bit architectures, they need testing */
#ifndef GR_FW // annoying--FW uses a standard defintion of byte and the compiler burps, even though the defns are the same :-(
