...
namespace SIL.FieldWorks.Common.FwAvalonia.Region
{
	...
	public static class LexicalEditFirstSlice
	{
		...
		public static ViewDefinitionModel CompileFromLayoutDirectory(string partsDirectory, ViewDefinitionCompiler compiler = null)
		{
			...
			try
			{
				var partsXml = LoadMergedParts(partsDirectory);
				if (partsXml == null)
				{
					return null;
				}
		...
		private static string LoadMergedParts(string partsDirectory)
		{
			...
			var merged = new XElement("PartInventory", partsFiles.Select(XElement.Load));
			...
			IReadOnlyDictionary<string, string> baseClassMap)
		{
			...
			var layoutFile = XElement.Load(layoutPath);
			var layout = layoutFile.Descendants("layout").FirstOrDefault(l =>
				(string)l.Attribute("class") == className
				&& (string)l.Attribute("type") == "detail"
