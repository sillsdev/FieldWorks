...
namespace FwAvaloniaTests
{
	...
	public class Path3BundleTests
	{
		...
		public void FirstSlice_Path3Bundle_AssemblesAllLanes_WithExplicitStatus()
		{
			...
			// WinForms visual lane: the committed legacy baseline. There is no WinForms baseline
			...
			// Workflow/accessibility lane: PreviewHostUiaTests exercises the real Windows UIA tree
			// against the preview host, but it requires an interactive desktop session
			// (Environment.UserInteractive) and lives in a separate net48 desktop-lane test project. It
			...
			// exists and still contains the coverage it claims; it cannot prove the lane passed. See the
			// fieldworks-uia2-parity-testing skill for the manual/desktop-lane run that actually proves
			// this lane.
			...
			// Performance lane: DataTreeTimingBaselines.json is gitignored/generated (not committed), so
			...
			// so this lane cannot honestly be called proven.
				...
				lanes = new Dictionary<string, object>
				{
					...
					["workflow-accessibility"] = new
					{
						...
						note = workflowAccessibilitySuitePresent
							? "suite present and contains the names/order coverage, but it requires a manual/desktop-lane UIA run (interactive session) to actually prove this lane; it is not invoked from this headless test"
							: "suite file not found, or missing the expected coverage — investigate before trusting this lane at all"
					},
					["performance"] = new
					{
			...
			Assert.That(workflowAccessibilitySuitePresent, Is.True,
				"the PreviewHostUiaTests suite backing the workflow-accessibility lane must exist and retain its names/order coverage");
