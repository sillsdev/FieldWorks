...
<layout class='LexEntry' type='detail' name='Menus'>
  ...
  <part ref='Senses'/>
	...
	/// <summary>
	/// The gestures that open a detail row's menus, and which menu each one maps to. Every
	...
	/// answers the context gestures with the in-string `contextMenu=` menu; an unbound value
	...
	public class DetailMenuRequestTests
	{
		...
		// gestures. Headless lists Apps and Win32 adds Shift+F10, so Apps covers both;
...
#pragma warning restore 618
		...
		public void ContextMenuKey_OnTheFocusedLabelCell_RaisesTheSliceMenuRequest()
		{
			...
			// Tab focus reaches the row through its field-options button; the gesture is
			...
			Assert.That(requests, Has.Count.EqualTo(1),
				"the keyboard menu gesture opens the same menu right-click does");
			Assert.That(requests[0].Kind, Is.EqualTo(DetailMenuKind.SliceMenu));
		}
		...
		// One gesture must never raise two requests: the value box tunnels ahead of the
		// row handler.
		[AvaloniaTest]
		...
		public void ContextMenuKey_InTheValueBox_RaisesTheContextMenuRequest()
		{
			...
			Assert.That(requests, Has.Count.EqualTo(1),
				"the keyboard menu gesture works inside the value");
			Assert.That(requests[0].Kind, Is.EqualTo(DetailMenuKind.ContextMenu));
		}

		// A keyboard gesture anchors to the field rather than to the last mouse position;
		// pointer gestures still open at the pointer.
		[AvaloniaTest]
		public void RightClick_ReportsAPointerGesture_AnchoredAtTheControlItCameFrom()
		{
			var (window, view, requests) = Show(Field("CitationForm", DetailFieldKind.Text,
			...
			Assert.That(requests, Has.Count.EqualTo(1));
			Assert.That(requests[0].IsPointerGesture, Is.True, "right-click opens at the pointer");
		...
		public void ContextMenuKey_InTheValueBox_AnchorsToTheEditField_NotThePointer()
		{
			...
			Assert.That(requests, Has.Count.EqualTo(1));
			Assert.That(requests[0].IsPointerGesture, Is.False,
				"a keyboard gesture carries no pointer position");
			Assert.That(requests[0].AnchorControl, Is.SameAs(box),
		...
		public void ContextMenuKey_OnTheLabelCell_AnchorsToThatCell()
		{
			...
			Assert.That(requests[0].AnchorControl, Is.Not.Null,
				"the label cell is the anchor for a keyboard gesture on the row");
		}
		...
		// in-string menu. Neither gesture may produce the other's menu.
		[AvaloniaTest]
