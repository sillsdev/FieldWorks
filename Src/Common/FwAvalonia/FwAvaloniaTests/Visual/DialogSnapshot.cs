...
namespace FwAvaloniaTests.VisualChecks
{
	...
	/// prefixed file name -- <c>Output/Snapshots/&lt;Prefix&gt;-&lt;NN&gt;-&lt;stage&gt;.png</c>
	/// (e.g.
	/// <c>Output/Snapshots/InsertEntry-01-initial.png</c>,
	/// <c>Output/Snapshots/Browse-05-selected.png</c>) -- so
	...
	/// the capture looks right -- overlap, clipping, stray strikethrough, lost highlight,
	/// alignment, density --
	...
	public static class DialogSnapshot
	{
		...
		/// <c>"InsertEntry-01-initial"</c> -> <c>Output/Snapshots/InsertEntry-01-initial.png</c>)
		/// so a person can
		...
		/// </summary>
		public static string Capture(Control control, string name, double width = 420, double height = 320,
			string prefixOverride = null)
		{
			...
			var fileName = string.IsNullOrWhiteSpace(prefixOverride) || name.StartsWith(prefixOverride + "-")
				? name
				: prefixOverride + "-" + name;
