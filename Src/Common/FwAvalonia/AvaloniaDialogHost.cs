			...
			try
			{
				...
				})
				{
					...
					}))
					{
						...
						// never received pointer input. Form.ActiveForm is whatever's truly topmost/focused
						// right now, so preferring it establishes the correct nested-owner chain. For every
