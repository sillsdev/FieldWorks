...
namespace SIL.FieldWorks.Common.FwAvalonia.ViewDefinition
{
	...
	public static class LayoutImportCoverage
	{
		private static readonly HashSet<string> HandledLayoutFileElements =
			new HashSet<string>(StringComparer.Ordinal) { "LayoutInventory", "layoutType", "layout", "part", "indent" };

		private static readonly HashSet<string> HandledPartsFileElements =
			new HashSet<string>(StringComparer.Ordinal) { "PartInventory", "bin", "part", "slice", "seq", "obj" };

		/// <summary>Runs the census and the import pass and returns the aggregated result.</summary>
			...
			IReadOnlyList<LayoutSourceFile> partsFiles)
		{
			...
			var resolver = new DictionaryPartResolver(mergedParts);
