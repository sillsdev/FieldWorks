...
namespace SIL.FieldWorks.Common.FwAvalonia.ViewDefinition
{
	...
	public sealed class ViewDefinitionOverrideStore
	{
		...
		/// <paramref name="onLoadError"/> rather than crashing compose -- the legacy Inventory
		/// drops stale
		/// overrides too).
		/// </summary>
		...
		/// <summary>
		/// Persists <paramref name="patch"/> for its (ClassName, LayoutName) and refreshes the
		/// cache. An
		...
		/// <summary>The on-disk path for a (class, layout) patch (also the file the loader
		/// reads).</summary>
