...
namespace SIL.FieldWorks.Common.FwAvalonia.Detail
{
	...
	/// <summary>Which configured menu a context gesture maps to.</summary>
	...
	/// command target.
	///
	/// A pointer gesture opens the menu at the pointer. A gesture with no pointer (context-menu
	/// key, Shift+F10, or activating the field-options button) anchors it under
	...
	public sealed class DetailMenuRequest
	{
			...
			bool isPointerGesture)
		{
			...
			IsPointerGesture = isPointerGesture;
		}

		/// <summary>
		/// The request for a ContextRequested gesture: a right-click opens at the pointer, while
		/// the context-menu key and Shift+F10 carry none, so the menu anchors under the surface.
		/// </summary>
		/// <param name="surface">The control the gesture arrived on; becomes the anchor.</param>
		/// <param name="e">Read only for whether the gesture carries a pointer.</param>
		/// <param name="field">The row the menu acts on.</param>
		/// <param name="kind">Which configured menu the gesture maps to.</param>
		...
		public static DetailMenuRequest FromAnchor(Control anchor, DetailField field,
			DetailMenuKind kind)
			=> new DetailMenuRequest(field, kind, anchor, isPointerGesture: false);

		public DetailField Field { get; }
		public DetailMenuKind Kind { get; }

		/// <summary>
		/// The control the gesture came from; the menu anchors under it when
		/// <see cref="IsPointerGesture"/> is false. Null when the request had no source control.
		/// </summary>
		public Control AnchorControl { get; }

		/// <summary>
		/// False when the gesture carried no pointer position (context-menu key, Shift+F10, or
		...
		/// </summary>
		public bool IsPointerGesture { get; }
	}
