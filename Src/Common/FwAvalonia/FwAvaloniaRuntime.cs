...
namespace SIL.FieldWorks.Common.FwAvalonia
{
	...
	public static class FwAvaloniaRuntime
	{
		...
		/// <see cref="EnsureInitialized"/> -- without this production DLL referencing
		/// Avalonia.Headless.
		/// xWorks integration tests that drive the product UI
		/// (RecordEditView/RecordBrowseView/...)
		...
		/// <c>[SetUpFixture]</c> sets this to a headless builder before any test runs; production
		/// leaves it
		/// null and behavior is identical to calling <see cref="FwAvaloniaHost.BuildAvaloniaApp"/> directly.
		/// Only honored on the first (winning) <see cref="EnsureInitialized"/> call -- once the
		/// runtime is
		...
		/// <summary>
		/// Idempotently sets up the Avalonia app for in-process net48 hosting. Also treats an
		...
		/// per test session, without ever calling this method -- calling
		/// <c>SetupWithoutStarting</c> again
		/// on top of that would throw (Avalonia only allows one <c>Setup</c> per process).
		/// </summary>
		public static void EnsureInitialized()
		{
			if (s_initialized || Application.Current != null)
				return;
