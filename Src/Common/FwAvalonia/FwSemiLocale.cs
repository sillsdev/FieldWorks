...
{
	/// <summary>
	/// Interim measure, not the destination. Maps the current UI culture onto the locale each
	/// Semi theme actually supports. Both Semi.Avalonia.SemiTheme and Ursa.Themes.Semi.SemiTheme
	/// default to (and reset to) zh-CN on an unrecognized Locale, so callers must always pass one
	/// of these results, never the raw UI culture.
	/// </summary>
	/// <remarks>
	/// This picks the best of the vendor's own translations; it does not make the wording
	/// FieldWorks-owned. Semi still decides the text of user-visible chrome (the built-in TextBox
