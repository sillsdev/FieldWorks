...
namespace SIL.FieldWorks
{
	...
	public static class FieldWorks
	{
		...
		private static RegistryKey s_defaultAppKey = FwRegistryHelper.FieldWorksRegistryKey;
			...
			try
			{
				...
				if (!string.IsNullOrEmpty(appArgs.BackupFile))
				{
					...
					if (s_teApp == null && s_flexApp == null)
						return; // Restore was cancelled or failed, or another process took care of it.
					if (!String.IsNullOrEmpty(s_LinkDirChangedTo))
					{
						...
						return; // Failed to launch properly
		...
		/// ------------------------------------------------------------------------------------
		private static void SetUICulture(FwAppArgs args)
		{
		...
		private static FdoCache CreateCache(ProjectId projectId)
		{
			...
			WriteSplashScreen(string.Format(Properties.Resources.kstidLoadingProject,
				projectId.UiName));
			FdoCache cache;
			try
			{
				cache = FdoCache.CreateCacheFromExistingData(projectId, s_sWsUser, s_threadHelper);
			}
			catch (FdoFileLockedException ex)
			{
				CloseSplashScreen();
				MessageBox.Show(string.Format(Properties.Resources.ksLockFileLocked, ex.ProjectName), Properties.Resources.ksErrorCaption,
		...
		/// ------------------------------------------------------------------------------------
		private static object CommitAndDisposeCache(IProgress progressDlg, object[] parameters)
		{
		...
		private static bool SafelyReportException(Exception error, IFwMainWnd parent, bool isLethal)
		{
			...
			using (new IgnoreAppMessageProccessing(s_flexApp))
			{
				...
				RegistryKey appKey = s_defaultAppKey;
		...
		/// ------------------------------------------------------------------------------------
		private static ProjectId DetermineProject(FwAppArgs args, FwApp app)
		{
			...
			return ShowWelcomeDialog(app, app, projId, projectOpenError);
		}
		...
		/// </returns>
		/// ------------------------------------------------------------------------------------
		private static bool LaunchProject(FwAppArgs args, FwApp app, ref ProjectId projectId)
		{
			while (true)
			{
				if (projectId == null)
				...
				catch (FwStartupException e)
				{
					...
					Logger.Init(FwUtils.ksSuiteName);
					projectId = ShowWelcomeDialog(app, app, projectId, e);
				}
			}
		}
		...
		/// </returns>
		/// ------------------------------------------------------------------------------------
		private static ProjectId ShowWelcomeDialog(FwApp app, IHelpTopicProvider helpTopicProvider,
		...
		internal static ProjectId ChooseLangProject(Form dialogOwner, IHelpTopicProvider helpTopicProvider)
		{
			...
			using (var dlg = new ChooseLangProjectDialog(helpTopicProvider))
			{
		...
		internal static void FileProjectSharingLocation(Form dialogOwner, FwApp fwApp)
		{
			...
			if (dlg.ProjectsSharedChecked)
			{
				...
				UpdateProjectsSharing(true, dialogOwner, fwApp, projectPath, parentDirectory, projectsDirectory);
			}
			else
			{
		...
		/// ------------------------------------------------------------------------------------
		private static void UpdateProjectsSharing(bool fShareProjects, Form  dialogOwner,
			FwApp fwApp, string projectPath, string parentDirectory, string projectsDirectory)
		{
			...
			if (!ClientServerServices.Current.Local.WillProjectBeConverted(projectPath, parentDirectory, projectsDirectory))
			{
				...
				ClientServerServices.Current.Local.SetProjectSharing(fShareProjects, s_threadHelper);
				return;
			}
			...
			string projectPath, ProjectId oldProjectId)
		{
			...
			using (ProgressDialogWithTask progressDlg = new ProgressDialogWithTask(null))
			{
				...
				do
				{
					...
					try
					{
						...
						if (ProjectRestoreService.HandleRestoreFileErrors(null, ResourceHelper.GetResourceString("ksRestoreFailed"),
							restoreSettings.Settings.Backup.File, () => restoreService.RestoreProject(s_threadHelper)))
						{
			...
			FdoCache existingCache, Form dialogOwner)
		{
			...
			try
			{
				...
				backupService.BackupProject(dialogOwner);
			}
			...
			bool fOpeningNewProject)
		{
			...
			IProgress progressDlg = (s_splashScreen == null) ? null :
				s_splashScreen.ProgressBar;

			Form fwMainWindow;
			try
			{
			// Construct the new window, of the proper derived type
				fwMainWindow = app.NewMainAppWnd(progressDlg, fNewCache, wndCopyFrom, fOpeningNewProject);
		...
		internal static void KickOffAppFromOtherProcess(FwAppArgs args)
		{
			s_threadHelper.Invoke(() =>
			{
				...
				else
				{
					...
					// ENHANCE (TimS): Since the starting of the app might take a while, it might
					// be nice to show a progress dialog or something.
		...
		private static bool InitializeFirstApp(FwApp app, ProjectId projectId)
		{
			...
			if (!FwRegistrySettings.DisableSplashScreenSetting && !s_noUserInterface)
				ShowSplashScreen(app);

			try
			{
				IProgress progressDlg = (s_splashScreen == null) ? null : s_splashScreen.ProgressBar;
				...
				if (s_cache == null)
					return false; // typically lock file was locked; we have already reported this, just quit quietly.

				if (s_noUserInterface)
					return true;

				if (InitializeApp(app, progressDlg))
				{
			...
			{
				CloseSplashScreen();
				// We should be getting a main window by now, so the help button on the dialog
				// will work if needed.
				CheckForMovingExternalLinkDirectory(app);
			}
			return false;
		}
		...
		private static void MoveExternalLinkDirectoryAndFiles(FwApp app)
		{
			...
			{
				app.Cache.LangProject.LinkedFilesRootDir =
					Path.Combine(app.Cache.ProjectId.ProjectFolder, "LinkedFiles");
			});
		...
		/// ------------------------------------------------------------------------------------
		private static IHelpTopicProvider GetHelpTopicProvider(string appAbbrev)
		{
		...
		private static void ExecuteWithAppsShutDown(string abbrevOfDefaultAppToStart, Func<ProjectId> action)
		{
			...
			{
				s_allowFinalShutdown = true; // mustn't suppress any longer.
				ExitIfNoAppsRunning();
			}
		}
		...
		private static bool ComplainToUserAboutNewWs(string wsLabel, string projectName)
		{
			...
			Form owner = s_splashScreen != null ? ((IProgress)s_splashScreen).Form : Form.ActiveForm;
	...
	internal sealed class IgnoreAppMessageProccessing : IDisposable
	{
		...
		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Initializes a new instance of the <see cref="T:IgnoreAppMessageProccessing"/> class.
		/// </summary>
