...
namespace SIL.FieldWorks.Common.PrintLayout
{
	...
	/// ----------------------------------------------------------------------------------------
	[TestFixture]
	public class HeaderFooterVcTests
	{
		...
		{
			m_vc.SetDa(m_fdoCache.MainCacheAccessor);
		...
		public void TestGetStrForProjectNameGuid()
		{
			...
			Assert.AreEqual("TestLangProj", m_vc.GetStrForGuid(projectNameGuid).Text);
		}
		...
		public void TestLeftElementFlid_Duplex()
		{
			...
			Assert.AreEqual(PubHeader.PubHeaderTags.kflidInsideAlignedText,
				m_vc.LeftElementFlid);

			// Even page, left bound
			m_pageInfo.m_pageNumber = 478;
			m_pageInfo.m_bindingSide = BindingSide.Left;
			Assert.AreEqual(PubHeader.PubHeaderTags.kflidOutsideAlignedText,
				m_vc.LeftElementFlid);
		...
		public void TestRightElementFlid_Duplex()
		{
			...
			m_pageInfo.m_pageNumber = 1;
			m_pageInfo.m_bindingSide = BindingSide.Left;
			Assert.AreEqual(PubHeader.PubHeaderTags.kflidOutsideAlignedText, m_vc.RightElementFlid);

			// Even page, left bound
			m_pageInfo.m_pageNumber = 478;
			m_pageInfo.m_bindingSide = BindingSide.Left;
			Assert.AreEqual(PubHeader.PubHeaderTags.kflidInsideAlignedText, m_vc.RightElementFlid);
