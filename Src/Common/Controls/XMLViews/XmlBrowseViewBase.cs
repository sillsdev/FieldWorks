...
namespace SIL.FieldWorks.Common.Controls
{
	...
	/// </summary>
	/// ----------------------------------------------------------------------------------------
	public class XmlBrowseViewBase : RootSite, IVwNotifyChange
	{
		#region Events
		...
		/// </summary>
		public event SIL.FieldWorks.Common.Utils.FwSelectionChangedEventHandler SelectionChangedEvent;
		/// <summary>
		...
		public bool OnRequestDebugInfo(object commandObj)
		{
			...
			using (MemoryStream stream = new MemoryStream())
			{
				...
				{
					m_xbvvc.LogStream = sw;
					int cv = m_fdoCache.MainCacheAccessor.get_VecSize(m_hvoRoot, m_fakeFlid);
					if (cv > 0)
					{
						int hvoObjSel = m_fdoCache.MainCacheAccessor.get_VecItem(m_hvoRoot,
							m_fakeFlid, m_selectedIndex < 0 ? 0 : m_selectedIndex);
						m_fdoCache.PropChanged(null, PropChangeType.kpctNotifyAll,
							hvoObjSel, m_tagMe, 0, 0, 0);
						this.Update(); // causes the PropChanged to actually invoke the VC.
					}
					else
					{
						sw.WriteLine("The record list is empty.");
					}
					sw.Flush();
					using (StreamReader sr = new StreamReader(stream))
					{
						stream.Seek(0, SeekOrigin.Begin);
						MessageBox.Show(this, sr.ReadToEnd(), "DEBUG: Row generation info");
						m_xbvvc.LogStream = null;
					}
				}
			}
			return true;
		...
		public override void RefreshDisplay()
		{
			...
			{
				// TODO: JohnT: review - starting the FLEX in the Lexicon tool and immediately pressing
				//  F5 would cause an exception here.  This is a band-aid until further review.
				try
				{
					// skip dummy objects blown away by the Refresh.
					if (m_fdoCache.IsValidObject(m_hvoRoot))
					{
						int hvoNewObj = m_fdoCache.MainCacheAccessor.get_VecItem(m_hvoRoot, m_fakeFlid, m_selectedIndex);
		...
		public int SelectedObject
		{
			get
			{
				...
				if (SelectedIndex < 0)
					return 0;
				if (!m_fdoCache.MainCacheAccessor.get_IsPropInCache(m_hvoRoot, m_fakeFlid,
					(int)CellarModuleDefns.kcptReferenceSequence, 0))
				{
					return 0; // The only time this happens is during refresh.
				}
				return m_fdoCache.MainCacheAccessor.get_VecItem(m_hvoRoot, m_fakeFlid, SelectedIndex);
			}
		}
		...
		internal int RowCount
		{
			...
			{
				CheckDisposed();
				return m_fdoCache.MainCacheAccessor.get_VecSize(m_hvoRoot, m_fakeFlid);
			}
		}
		...
		public int SelectedIndex
		{
			...
			set
			{
				...
				if (!m_fdoCache.MainCacheAccessor.get_IsPropInCache(m_hvoRoot, m_fakeFlid,
					(int)CellarModuleDefns.kcptReferenceSequence, 0))
				{
					// We can tolerate it being a size of zero, but it does have to be in the cache.
					Debug.Assert(false, "Property is not cached at all.");
					throw new InvalidOperationException("RecordList property is not cached at all.");
				}
				...
				if (cobj == 0)
				{
					...
					if (SelectionChangedEvent != null)
						SelectionChangedEvent(this, new FwObjectSelectionEventArgs(0, -1));
					return;
				}
				...
				if (hvoObjNewSel != m_hvoOldSel)
				{
					...
					if (SelectionChangedEvent != null && hvoObjNewSel != 0)
					{
						...
						cobj = m_fdoCache.MainCacheAccessor.get_VecSize(m_hvoRoot, m_fakeFlid);
					}
				}
				...
				if (oldIndex >= 0 && oldIndex < cobj)
				{
					...
					int hvoObjOldSel = m_fdoCache.MainCacheAccessor.get_VecItem(m_hvoRoot,
						m_fakeFlid, oldIndex);
					try
					{
						m_fdoCache.PropChanged(null, PropChangeType.kpctNotifyAll,
							hvoObjOldSel, m_tagMe, 0, 0, 0);
					}
					catch (Exception)
					{
				...
				// Note: this will probably destroy the new selection, so save the selection info in order to restore it
				// afterwards.
				SelectionHelper selection = null;
				if (RootBox != null)
					selection = SelectionHelper.GetSelectionInfo(RootBox.Selection, this);
				try
				{
					m_fdoCache.PropChanged(null, PropChangeType.kpctNotifyAll,
						hvoObjNewSel, m_tagMe, 0, 0, 0);
				}
				catch (Exception)
				{
					m_bv.RaiseSelectionDrawingFailure();
				...
				if (!ReadOnlySelect && selection != null) // TE-6912: This RestoreSelection scrolled back to top
				{
					using (new IgnorePropChanged(this.Cache, PropChangedHandling.SuppressAll))
					{
				...
				if (m_wantScrollIntoView)
				{
					...
					if (CanFocus)
						this.Focus(); // Note: used to be part of DoSelectAndScroll, but I'm not sure why...
				}
				Update();
			}
		}
		...
		private int GetNewSelectionObject(int index)
		{
			...
			if (Cache.IsDummyObject(hvoObjNewSel) &&
				m_nodeSpec != null && XmlUtils.GetOptionalBooleanAttributeValue(m_nodeSpec, "convertDummiesSelected", false) == true)
			{
				RequestConversionToRealEventArgs args = new RequestConversionToRealEventArgs(hvoObjNewSel, 0, m_nodeSpec, true);
				args.OwningFlid = m_fakeFlid;
				m_xbvvc.OnRequestConversionToReal(this, args);
				if (args.RealObject != null)
				{
					if (hvoObjNewSel == m_hvoOldSel)
						m_hvoOldSel = args.RealObject.Hvo;
					hvoObjNewSel = args.RealObject.Hvo;
				}
			}
			return hvoObjNewSel;
		}
		...
		/// <returns></returns>
		internal int GetRowIndexFromSelection(IVwSelection sel)
		{
			if (sel == null)
				return -1;
			try
			{
				int clev = sel.CLevels(true); // end-point, so in a long drag we get the mouse-up row.
		...
		protected void DoSelectionSideEffects(IVwSelection sel)
		{
			...
			if (!m_fMouseUpEnabled)
				SetSelectedIndex(GetRowIndexFromSelection(sel));
		}
		...
		protected override void OnMouseUp(MouseEventArgs e)
		{
			...
			try
			{
				int newSelectedIndex = GetRowIndexFromSelection(MakeSelectionAt(e));
		...
		public virtual int RootObjectHvo
		{
			...
			set
			{
				...
				m_hvoOldSel = 0;
				int chvo = m_fdoCache.MainCacheAccessor.get_VecSize(m_hvoRoot, m_fakeFlid);
				if (chvo == 0)
				{
					m_selectedIndex = -1;
				}
				...
				{
					int hvo = m_fdoCache.MainCacheAccessor.get_VecItem(m_hvoRoot, m_fakeFlid, 0);
					if (m_fdoCache.IsValidObject(hvo))
					{
						SelectedIndex = 0;
					}
					else
					{
						...
						chvo = m_fdoCache.MainCacheAccessor.get_VecSize(m_hvoRoot, m_fakeFlid);
						if (chvo == 0)
							m_selectedIndex = -1;
						else
							SelectedIndex = 0;
			...
			FdoCache cache, Mediator mediator, BrowseViewer bv)
		{
			...
			// Get the special tag used for updating highlighting.
			m_tagMe = m_xbvvc.TagMe;
		}
		...
		protected override void Dispose( bool disposing )
		{
			...
			if (disposing)
			{
				if (m_fdoCache != null && m_fdoCache.MainCacheAccessor != null)
					m_fdoCache.MainCacheAccessor.RemoveNotification(this);
			}
		...
		/// </summary>
		/// ------------------------------------------------------------------------------------
		protected override void RestoreAfterRefresh()
		{
			base.RestoreAfterRefresh();
			OnRestoreScrollPosition(null);
		}
		...
		{
			if (irow < 0 ||
				irow > m_fdoCache.MainCacheAccessor.get_VecSize(m_hvoRoot, m_fakeFlid))
			{
		...
		public int HvoAt(int index)
		{
			...
			return m_fdoCache.GetVectorItem(m_hvoRoot, m_fakeFlid, index);
		}
		...
		public override void MakeRoot()
		{
			...
			base.MakeRoot();
			m_fdoCache.MainCacheAccessor.AddNotification(this);
		...
		public override void SelectionChanged(IVwRootBox prootb, IVwSelection vwselNew)
		{
			...
			Application.Idle += new EventHandler(RemoveRootBoxSelection_OnApplicationIdle);
		}

		void RemoveRootBoxSelection_OnApplicationIdle(object sender, EventArgs e)
		{
			// Only one chance at processing this event - so remove it first.
			Application.Idle -= new EventHandler(RemoveRootBoxSelection_OnApplicationIdle);
			if (m_rootb == null)
				return;
			...
			if (sel != null && !sel.IsRange)
			{
				...
				if (!sel.IsEditable ||
					(m_fSelectedRowHighlighting != SelectionHighlighting.none && GetRowIndexFromSelection(sel) != m_selectedIndex))
				{
