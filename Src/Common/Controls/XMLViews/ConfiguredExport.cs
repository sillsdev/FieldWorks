...
namespace SIL.FieldWorks.Common.Controls
{
	...
	public class ConfiguredExport : CollectorEnv
	{
		...
		public void Initialize(FdoCache cache, TextWriter w, string sDataType, string sFormat,
			string sOutPath, string sBodyClass)
		{
			m_writer = w;
		...
		private CurrentContext WriteClassStartTag(int hvoItem)
		{
			...
			int clid = m_cache.ServiceLocator.GetInstance<ICmObjectRepository>().GetObject(hvoItem).ClassID;
		...
		{
			if (m_cache.ServiceLocator.GetInstance<ILexEntryRepository>().GetObject(hvoItem).ExcludeAsHeadword)
				return;
