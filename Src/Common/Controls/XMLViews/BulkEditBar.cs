...
#pragma warning restore 219
		...
		private void OnTargetComboItemChanged(FieldComboItem selectedItem)
		{
			...
			if (m_expectedListItemsClassId != 0)
			{
				using (var targetFieldItem = new TargetFieldItem(selectedItem.ToString(),
					selectedItem.ColumnIndex, m_expectedListItemsClassId))
				{
	...
	internal class IntOnSubfieldChooserBEditControl : IntChooserBEditControl
	{
		...
		{
			int val = (m_combo.SelectedItem as IntComboItem).Value;
			ITsString tssVal = StringUtils.MakeTss(m_combo.SelectedItem.ToString(), m_cache.DefaultUserWs);
			int i = 0;
			// Report progress 50 times or every 100 items, whichever is more
			// (but no more than once per item!)
			int interval = Math.Min(100, Math.Max(itemsToChange.Count / 50, 1));
	...
	class ComplexListChooserBEditControl : IBulkEditSpecControl
	{
		...
		public virtual void DoIt(Set<int> itemsToChange, ProgressState state)
		{
				...
				m_cache.ServiceLocator.GetInstance<IActionHandler>(), () =>
			{
				...
				foreach (int hvoItem in itemsToChange)
				{
					...
					sda.Replace(hvoItem, m_flid, 0, oldVals.Count, newHvos, newHvos.Length);
				}
			});
		}
		...
		{
			oldVals = (from hvo in (m_cache.DomainDataByFlid as ISilDataAccessManaged).VecProp(hvoItem, m_flid)
		...
		private ITsString BuildValueString(List<ICmObject> chosenObjs)
		{
			...
			ITsString tssVal = bldr.Length > 0 ? bldr.GetString() :
				StringUtils.MakeTss("", m_cache.ServiceLocator.WritingSystemManager.UserWs);
			return tssVal;
		}
		#endregion
	...
	{
		// A set of the HVOs of all the LexEntryRefs that are variants. We can adjust the variant type only of things
		// that are variants, so we use this to filter the candidates.
		Set<int> m_variantEntryRefs;
		...
		protected override bool DisableItem(int hvoItem)
		{
			if (m_variantEntryRefs == null)
			{
				m_variantEntryRefs = new Set<int>(
					from ler in Cache.ServiceLocator.GetInstance<ILexEntryRefRepository>().AllInstances()
					where ler.RefType == 0
					select ler.Hvo);
			}
