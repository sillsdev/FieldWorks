...
namespace SIL.FieldWorks.Common.Controls
{
	...
	public class BulkEditBar : UserControl, IFWDisposable
	{
		...
		/// </summary>
		List<int> m_bulkEditListItemsGhostFields = new List<int>();
			...
			: this()
		{
			...
			{
				uint classId = cache.MetaDataCacheAccessor.GetClassId(className);
			...
			if (!String.IsNullOrEmpty(bulkEditListItemsGhostFieldsValue))
			{
				...
				foreach (string classAndField in bulkEditListItemsGhostFields)
					m_bulkEditListItemsGhostFields.Add(GetFlidFromClassDotName(m_cache, classAndField));
			}
			MakeItems();
			...
			m_clickCopySepBox.WritingSystemFactory = m_cache.LanguageWritingSystemFactoryAccessor;
			m_clickCopySepBox.WritingSystemCode = m_cache.DefaultAnalWs;
			...
			m_bcNonEmptyTargetControl.WritingSystemFactory = m_cache.LanguageWritingSystemFactoryAccessor;
			...
			m_trdNonEmptyTargetControl.WritingSystemFactory = m_cache.LanguageWritingSystemFactoryAccessor;
		...
		private void populateListChoiceTargetCombo()
		{
			...
			m_listChoiceTargetCombo.Items.Clear();
		...
		int GetNamedList(XmlNode node, string attrName)
		{
			return BulkEditBar.GetNamedList(m_cache, node, attrName);
		}
		...
		/// and return the HVO of the indicated list.
		...
		/// <param name="attrName"></param>
		/// <returns></returns>
		internal static int GetNamedList(FdoCache cache, XmlNode node, string attrName)
		{
			string owningClass;
			string property;
			...
			{
				case "LangProject":
					owner = (CmObject)cache.LangProject;
					break;
				case "LexDb":
					owner = (CmObject)cache.LangProject.LexDbOA;
					break;
				default:
					throw new ConfigurationException(String.Format("List {0}.{1} currently not supported.", owningClass, property));
			}
			System.Reflection.PropertyInfo pi = owner.GetType().GetProperty(key + "OAHvo");
			if (pi == null)
		...
		private void InitializeComponent()
		{
			...
			this.m_clickCopySepBox.AllowMultipleLines = false;
		...
		List<ListClassTargetFieldItem> ListItemsClassesInfo(Set<int> classes)
		{
			...
			foreach (int classId in classes)
			{
				...
				XmlViewsUtils.TryFindPluralFormFromClassId(m_cache, tbl, (uint)classId, out pluralOfClass);
		...
		void m_deleteWhatCombo_SelectedIndexChanged(object sender, EventArgs e)
		{
			...
			if (fWasShowEnabled != m_bv.BrowseView.Vc.ShowEnabled && m_bv.BrowseView.RootBox != null)
		...
		private void SetEnabledForAllItems()
		{
			...
			foreach (int hvoItem in m_items)
				cda.CacheIntProp(hvoItem, XmlBrowseViewBaseVc.ktagItemEnabled, AllowDeleteItem(hvoItem));
		}
		...
		internal void UpdateEnableItems(int hvoItem)
		{
			...
			int clsid = m_cache.GetClassOfObject(hvoItem);
			if (clsid == LexSense.kclsidLexSense)
			{
				int hvoOwner = m_cache.GetOwnerOfObject(hvoItem);
				int clsOwner = m_cache.GetClassOfObject(hvoOwner);
				if (clsOwner != LexEntry.kclsidLexEntry)
					return; // subsenses are always OK to delete, change can't have any effect.
				ISilDataAccess sda = m_cache.MainCacheAccessor;
				int chvo = sda.get_VecSize(hvoOwner, (int)LexEntry.LexEntryTags.kflidSenses);
				if (chvo == 1)
				...
				{
					int hvoSense = sda.get_VecItem(hvoOwner, (int)LexEntry.LexEntryTags.kflidSenses, i);
					int wasEnabled = sda.get_IntProp(hvoSense, XmlBrowseViewBaseVc.ktagItemEnabled);
					int enabled = AllowDeleteItem(hvoSense);
					if (enabled != wasEnabled)
					{
						cda.CacheIntProp(hvoSense, XmlBrowseViewBaseVc.ktagItemEnabled, enabled);
						sda.PropChanged(null, (int)PropChangeType.kpctNotifyAll,
							hvoSense, XmlBrowseViewBaseVc.ktagItemEnabled,
							0, 0, 0);
					}
				}
			}
		...
		private int AllowDeleteItem(int hvoItem)
		{
			...
			if (clsid == LexSense.kclsidLexSense && m_ghostParentHelper == null)
			{
				int hvoOwner = m_cache.GetOwnerOfObject(hvoItem);
				int clsOwner = m_cache.GetClassOfObject(hvoOwner);
				if (clsOwner != LexEntry.kclsidLexEntry)
		...
		private bool CanDeleteItemOfClassOrGhostOwner(int hvoItem, int clsid)
		{
			...
			if (m_cache.IsSameOrSubclassOf(clsid, m_expectedListItemsClassId))
				return true;
		...
		{
			ISilDataAccess sda = m_bv.Cache.MainCacheAccessor;
			int oldCol = sda.get_IntProp(m_bv.RootObjectHvo, XmlBrowseViewVc.ktagActiveColumn);
			int newCol = oldCol;
			...
			using (new ReconstructPreservingBVScrollPosition(m_bv))
			{
				...
				using (new SIL.FieldWorks.Common.Utils.WaitCursor(this))
				{
					...
					else
					{
						try
						{
							...
							if (m_operationsTabControl.SelectedTab == m_listChoiceTab)
							{
								if (m_itemIndex >= 0)
								{
									...
									bei.BulkEditControl.FakeDoit(ItemsToChange(false), XmlBrowseViewVc.ktagAlternateValue,
										XmlBrowseViewVc.ktagItemEnabled, state);
							...
							else if (m_operationsTabControl.SelectedTab == m_findReplaceTab)
							{
								...
								if (method == null)
									return;
								method.FakeDoit(ItemsToChange(false), XmlBrowseViewVc.ktagAlternateValue,
									XmlBrowseViewVc.ktagItemEnabled, state);
							}
						...
						{
							m_cache.VwOleDbDaAccessor.AutoloadPolicy = AutoloadPolicies.kalpLoadForThisObject;
						}
		...
		ReplaceWithMethod MakeReplaceWithMethod(out int newCol)
		{
			...
			return new ReplaceWithMethod(m_cache, fci.Accessor,
		...
		ClearMethod MakeClearMethod(out int newCol)
		{
			...
			return new ClearMethod(m_cache, fci.Accessor,
		...
		BulkCopyMethod MakeBulkCopyMethod(out int newCol)
		{
			...
			return new BulkCopyMethod(m_cache, fci.Accessor,
				m_bv.ColumnSpecs[fci.ColumnIndex] as XmlNode,
				fciSrc.Accessor,
		...
		private void DeleteSelectedObjects(ProgressState state)
		{
			...
			{
				if (sda.get_IntProp(hvo, XmlBrowseViewBaseVc.ktagItemEnabled) != 0)
				{
					if (VerifyRowDeleteAllowable(hvo))
					{
						//allow deletion for the class we expect to be bulk editing.
						int hvoToDelete = 0;
						if (m_cache.IsSameOrSubclassOf(m_cache.GetClassOfObject(hvo), m_expectedListItemsClassId))
							hvoToDelete = hvo;
						else if (m_ghostParentHelper != null)
							hvoToDelete = m_ghostParentHelper.GetOwnerOfBasicProperty(hvo);
						if (hvoToDelete != 0)
			...
			try
			{
				...
				CmObject.DeleteObjects(idsToDelete, m_cache, true, VwClearInfoAction.kciaRemoveAllObjectInfo,
					fUndo, state);
		...
		private void InitDeleteTab()
		{
			...
			foreach (BulkEditItem bei in m_beItems)
			{
				...
				if (bei != null && bei.BulkEditControl.CanClearField)
				{
					...
					TargetFieldItem tfi = new TargetFieldItem(label, icol);
...
//				&& m_transduceTargetCombo.SelectedIndex >= 0;
		...
		private void EnablePreviewApplyForListChoice()
		{
			...
			{
				if ((m_beItems[m_itemIndex].BulkEditControl as ComplexListChooserBEditControl).ChosenHvos.Length > 0)
				{
					m_ApplyButton.Enabled = true;
					m_previewButton.Enabled = true;
				}
		...
		private void m_operationsTabControl_SelectedIndexChanged(object sender, EventArgs e)
		{
			...
			if (m_cache.MainCacheAccessor.get_IntProp(m_bv.RootObjectHvo, XmlBrowseViewVc.ktagActiveColumn) != 0)
			{
		...
		private void m_findReplaceSetupButton_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				if (String.IsNullOrEmpty(sWs))
				{
					...
					if (!String.IsNullOrEmpty(sTransduce))
					{
						...
						if (parts.Length == 2)
						{
							...
							if (clid != 0)
							{
								uint flid = mdc.GetFieldId2(clid, fieldName, true);
								if (flid != 0)
								{
									ws = FieldReadWriter.GetWsFromMetaData(0, (int)flid, m_cache);
									if (ws == 0)
				...
				else if (!LangProject.GetWsRequiresObject(sWs))
				{
					...
					ws = LangProject.InterpretWsLabel(m_cache, sWs, -50, 0, 0, null);
				}
			}
			catch
			{
			...
			FwFindReplaceDlg findDlg = new FwFindReplaceDlg();
			...
			findDlg.SetDialogValues(m_cache, m_pattern, m_bv.BrowseView.StyleSheet, TopLevelControl.Handle, FwApp.App, ws);
		...
		/// ------------------------------------------------------------------------------------
		protected void InitStringCombo(ComboBox combo, bool fIsSourceCombo)
		{
		...
		private FieldComboItem AddStringFieldItemsToCombo(ComboBox combo, FieldComboItem selectedItem, bool fIsSourceCombo)
		{
			...
			foreach (XmlNode node in m_bv.ColumnSpecs)
			{
				...
				{
					if (fIsSourceCombo)
						accessor = new ManyOnePathSortItemReadWriter(m_cache, node, m_bv);
					else
		...
		private void xbv_ClickCopy(object sender, ClickCopyEventArgs e)
		{
			...
			{
				ICmObject co = CmObject.CreateFromDBObject(m_cache, e.Hvo);
			...
			m_cache.MainCacheAccessor.BeginUndoTask(XMLViewsStrings.ksUndoClickCopy, XMLViewsStrings.ksRedoClickCopy);
			BulkEditBar.ForceRefreshOnUndoRedo(m_cache.MainCacheAccessor);
			selectedItem.Accessor.SetNewValue(e.Hvo, tssNew, true);
			m_cache.MainCacheAccessor.EndUndoTask();
		}
		...
		int GetExpectedListItemsClassFromSelectedItem(FieldComboItem selectedItem)
		{
			...
			else
			{
				...
				{
					listItemsClassId = (selectedItem as TargetFieldItem).ExpectedListItemsClass;
				}
				else
				{
					...
					listItemsClassId = m_cache.GetOwnClsId(field);
				}
			}
		...
		internal static void CommitChanges(int hvo, string commitChanges, FdoCache cache, int ws)
		{
			...
			{
				CmObject cmo = (CmObject)CmObject.CreateFromDBObject(cache, hvo, false);
				System.Reflection.MethodInfo mi = cmo.GetType().GetMethod(commitChanges, new Type[] {typeof(int)});
				if (mi == null)
	...
	{
		TargetFieldItem m_tfi = null;
		private bool m_fForceReload;
		...
		/// </summary>
		public bool ForceReload { get { return m_fForceReload; } internal set { m_fForceReload = value; } }
	}
	...
	public interface IBulkEditSpecControl
	{
		...
		/// tagEnabled to zero (and issue PropChanged) for any objects that can't be
	...
	internal abstract class DoItMethod : IGetReplacedObjects
	{
		...
		{
			return IsMultilingual(flid, m_cache.MetaDataCacheAccessor);
		}

		static internal bool IsMultilingual(int flid, IFwMetaDataCache mdc)
		{
			int cst = mdc.GetFieldType((uint)flid);
			switch (cst & 0x1f) // mask strips virtual bit
			{
		...
		public void FakeDoit(Set<int> itemsToChange, int tagFakeFlid, int tagEnable, ProgressState state)
		{
			...
			foreach (int hvo in itemsToChange)
			{
				...
				if (fEnable)
					m_cda.CacheStringProp(hvo, tagFakeFlid, NewValue(hvo));
				m_cda.CacheIntProp(hvo, tagEnable, (fEnable ? 1 : 0));
				// We'll do a Reconstruct after processing them all.
				//m_sda.PropChanged(null, (int)PropChangeType.kpctNotifyAll, hvo, tagEnable, 0,0,0);
			}
		}
		...
		public void Doit(Set<int> itemsToChange, ProgressState state)
		{
			...
			BulkEditBar.ForceRefreshOnUndoRedo(m_sda);
		...
		{
			int hvoItemReal = hvoItem;
			if (m_cache.IsDummyObject(hvoItem))
			{
				// Try to make it real.
				if (m_cache.GetClassOfObject(hvoItem) == WfiWordform.kclsidWfiWordform)
				{
					// Only class we currently know how to do.
					hvoItemReal = WfiWordform.ConvertDummyToReal(m_cache, hvoItem).Hvo;
					m_replacedObjects[hvoItem] = hvoItemReal;
				}
			}
			m_accessor.SetNewValue(hvoItemReal, tss);
		}
	...
	internal class BulkCopyMethod : DoItMethod
	{
		...
		public BulkCopyMethod(FdoCache cache, FieldReadWriter dstAccessor,
			XmlNode spec, FieldReadWriter srcAccessor, ITsString tssSep, NonEmptyTargetOptions options)
			: base(cache, dstAccessor, spec)
		{
	...
	internal class TransduceMethod : DoItMethod
	{
		...
		public TransduceMethod(FdoCache cache, FieldReadWriter dstAccessor, XmlNode spec, FieldReadWriter srcAccessor,
	...
	internal class ReplaceWithMethod : DoItMethod
	{
		...
		public ReplaceWithMethod(FdoCache cache, FieldReadWriter accessor, XmlNode spec, IVwPattern pattern, ITsString replacement)
			: base(cache, accessor, spec)
		{
			m_pattern = pattern;
	...
	internal class ClearMethod : DoItMethod
	{
		...
		public ClearMethod(FdoCache cache, FieldReadWriter accessor, XmlNode spec)
			: base(cache, accessor, spec)
		{
			ITsStrFactory tsf = TsStrFactoryClass.Create();
	...
	/// </summary>
	internal class FieldComboItem
	{
		string m_name; // string to show in menu
		int m_icol; // column index to show preview.
		...
		public FieldReadWriter Accessor
		{
	...
	class IntChooserBEditControl : BulkEditSpecControl, IGetReplacedObjects
	{
		...
		public override void DoIt(Set<int> itemsToChange, ProgressState state)
		{
			...
			foreach (int hvoItem in itemsToChange)
			{
				...
				UpdateListItemToNewValue(sda, hvoItemReal, val, valOld);
				IssuePropChanged(sda, hvoItemReal, val, valOld);
				...
				if (m_flid == (int)WfiWordform.WfiWordformTags.kflidSpellingStatus)
					FixSpellingStatus(hvoItemReal, val);
			}
			m_cache.EndUndoTask();
		}
		...
		protected override void UpdateListItemToNewValue(ISilDataAccess sda, int hvoItem, int newVal, int oldVal)
		{
			...
			if (m_ghostParentHelper != null)
			{
				...
				hvoOwningInt = m_ghostParentHelper.FindOrCreateOwnerOfBasicProp(hvoItem, m_flid);
			}
		...
		{
			string text = m_cache.GetMultiStringAlt(hvoItem, (int)WfiWordform.WfiWordformTags.kflidForm, m_cache.DefaultVernWs).Text;
			if (String.IsNullOrEmpty(text))
				return; // probably can't happen?
			EnchantHelper.SetSpellingStatus(text, m_cache.DefaultVernWs,
				m_cache.LanguageWritingSystemFactoryAccessor, (int)val == (int)SpellingStatusStates.correct);
		}
		...
		public override void FakeDoit(Set<int> itemsToChange, int tagFakeFlid, int tagEnabled, ProgressState state)
		{
			...
			IVwCacheDa cda = m_cache.VwCacheDaAccessor;
			int val = (m_combo.SelectedItem as IntComboItem).Value;
			ITsString tssVal = m_cache.MakeUserTss(m_combo.SelectedItem.ToString());
			int i = 0;
			...
			foreach (int hvoItem in itemsToChange)
			{
				...
				int valOld = 0;
				bool fEnable = true; // by default we want to create a new value
				if (TryGetOriginalListValue(sda, hvoItem, out valOld))
					fEnable = valOld != val;
				if (fEnable)
		...
		protected override bool TryGetOriginalListValue(ISilDataAccess sda, int hvoItem, out int value)
		{
			...
			int hvoOwningInt = hvoItem;
			if (m_ghostParentHelper != null)
				hvoOwningInt = m_ghostParentHelper.GetOwnerOfBasicProperty(hvoItem);
			if (hvoOwningInt == 0)
				return false;
	...
	class BooleanChooserBEditControl : IntChooserBEditControl
	{
		...
		{
			Debug.Assert(newVal == 0 || newVal == 1, String.Format("Expected value {0} to be boolean.", newVal.ToString()));
			sda.SetBoolean(hvoOwner, m_flid, Convert.ToBoolean(newVal));
		}
	}

	internal class IntOnSubfieldChooserBEditControl : IntChooserBEditControl
	{
		private int m_flidSub;
		/// <summary>
		/// Initialized with a string like "0:no;1:yes".
		/// </summary>
		/// <param name="itemList"></param>
		...
		public override void FakeDoit(Set<int> itemsToChange, int tagFakeFlid, int tagEnabled, ProgressState state)
		{
			...
			foreach (int hvoItem in itemsToChange)
			{
				...
				if (hvoField == 0)
					continue;
				int valOld = sda.get_IntProp(hvoField, m_flidSub);
	...
	abstract class BulkEditSpecControl : IBulkEditSpecControl, IGhostable
	{
		...
		protected virtual void IssuePropChanged(ISilDataAccess sda, int hvoItem, int newVal, int oldVal)
		{
			// override
		}
	...
	class FlatListChooserBEditControl : IBulkEditSpecControl, IGhostable, IDisposable
	{
		...
		protected virtual void UpdateListItemToNewValue(ISilDataAccess sda, int hvoItem, int hvoSel, int hvoOld)
		{
			...
			if (hvoOld == 0 && m_ghostParentHelper != null)
			{
				// it's possible that hvoItem is actually the owner of a object that needs to be created
				// for hvoSel to be set.
				hvoOwningAtomic = m_ghostParentHelper.FindOrCreateOwnerOfBasicProp(hvoItem, m_flidAtomicProp);
			}
			sda.SetObjProp(hvoOwningAtomic, m_flidAtomicProp, hvoSel);
			IssuePropChanged(sda, hvoOwningAtomic, hvoSel, hvoOld);
		}
			...
			ProgressState state)
		{
			...
			foreach(int hvoItem in itemsToChange)
			{
				...
				int hvoOld = GetOriginalListValue(sda, hvoItem);
				bool fEnable = hvoOld != hvoSel;
				if (fEnable)
					cda.CacheStringProp(hvoItem, tagFakeFlid, item.AsTss);
				cda.CacheIntProp(hvoItem, tagEnabled, (fEnable ? 1 : 0));
			}
		}
		...
		private List<HvoLabelItem> GetLabeledList()
		{
			int tagName = m_useAbbr ?
							(int)FDO.Cellar.CmPossibility.CmPossibilityTags.kflidAbbreviation :
							(int)FDO.Cellar.CmPossibility.CmPossibilityTags.kflidName;
			int flidItems =
				(int)FDO.Cellar.CmPossibilityList.CmPossibilityListTags.kflidPossibilities;
			int chvo = m_cache.MainCacheAccessor.get_VecSize(m_hvoList, flidItems);
			List<HvoLabelItem> al = new List<HvoLabelItem>(chvo);
			for (int i = 0; i < chvo; i++)
			{
				int hvoChild = m_cache.MainCacheAccessor.get_VecItem(m_hvoList, flidItems, i);
		...
		internal ITsString GetItemLabel(int hvoChild, int tagName)
		{
			...
			ITsString tssLabel =
				m_cache.MainCacheAccessor.get_MultiStringAlt(hvoChild, tagName, m_ws);

			// If that doesn't work, try using the default user writing system.
			if (tssLabel == null || string.IsNullOrEmpty(tssLabel.Text))
			{
				tssLabel = m_cache.MainCacheAccessor.get_MultiStringAlt(hvoChild,
					tagName, m_cache.DefaultUserWs);
			}
			...
			{
				tssLabel = m_cache.MainCacheAccessor.get_MultiStringAlt(hvoChild,
					tagName, m_cache.FallbackUserWs);
			}
	...
	class GhostParentHelper
	{
		FdoCache m_cache = null;

		///<summary>
		/// this has the information of the property owning missing (ghost) objects that
		/// own the atomic field we wish to update.
		///</summary>
		ClassAndPropInfo m_cpiForOwningFlidForGhost = null;
		FDOGhostSequencePropertyVirtualHandler m_vh = null;
		...
		/// </summary>
		/// <param name="cache"></param>
		/// <param name="ghostListFlid"></param>
		/// <returns></returns>
		internal static GhostParentHelper CreateGhostParentHelper(FdoCache cache, int ghostListFlid)
		{
			if (ghostListFlid == 0)
				return null;
			return new GhostParentHelper(cache, ghostListFlid);
		}

		private GhostParentHelper(FdoCache cache, XmlNode colSpec)
			: this(cache, GetGhostListField(cache, colSpec))
		{
		}

		private GhostParentHelper(FdoCache cache, int ghostListFlid)
		{
			m_cache = cache;
			SetupGhostOwningFlidInfo(cache, ghostListFlid);
		}
		...
		private static int GetGhostListField(FdoCache cache, XmlNode colSpec)
		{
			int ghostListFlid = 0;
			if (XmlUtils.GetOptionalAttributeValue(colSpec, "ghostListField") != null)
				ghostListFlid = BulkEditBar.GetFlidFromClassDotName(cache, colSpec, "ghostListField");
			return ghostListFlid;
		}


		/// <summary>
		/// If column field is configured to support ghost items.
		/// </summary>
		internal bool IsGhostableColumn
		{
			get { return m_cpiForOwningFlidForGhost != null;  }
		}

		internal int GetOwnerOfBasicProperty(int hvo)
		{
		...
		internal int FindOrCreateOwnerOfBasicProp(int hvoItem, int flidBasicProp)
		{
			int hvoOwnerOfBasicProp = GetOwnerOfBasicProperty(hvoItem);
			if (hvoOwnerOfBasicProp == 0)
			{
				int clidCreate = m_vh.GetDefaultClassToCreateForGhost(hvoItem);
				if (clidCreate == 0)
					clidCreate = m_cache.GetOwnClsId(flidBasicProp);
				// create the first child for this ghost owner
				hvoOwnerOfBasicProp = CreateFirstChildForGhostOwner(hvoItem, (int)m_cpiForOwningFlidForGhost.flid, clidCreate);
			}
			Debug.Assert(m_cache.ClassIsOrInheritsFrom((uint)m_cache.GetClassOfObject(hvoOwnerOfBasicProp), m_cpiForOwningFlidForGhost.signatureClsid),
				String.Format("Unexpected ghost item of class {0}", m_cache.GetClassOfObject(hvoOwnerOfBasicProp).ToString()));
			return hvoOwnerOfBasicProp;
		}

		internal bool IsGhostOwnerChildless(int hvoItem)
		{
			return m_cpiForOwningFlidForGhost.isVector ? m_cache.GetVectorSize(hvoItem, (int)m_cpiForOwningFlidForGhost.flid) == 0 :
					m_cache.GetObjProperty(hvoItem, (int)m_cpiForOwningFlidForGhost.flid) == 0;
		}

		private int GetFirstChildFromParent(int hvoParent)
		{
			int objFirstChild = 0;
			if (m_cpiForOwningFlidForGhost.isVector)
				objFirstChild = m_cache.GetVectorItem(hvoParent, (int)m_cpiForOwningFlidForGhost.flid, 0);
			else
				objFirstChild = m_cache.GetObjProperty(hvoParent, (int)m_cpiForOwningFlidForGhost.flid);
			return objFirstChild;
		}

		internal bool IsGhostOwnerClass(int hvo)
		{
			return m_cpiForOwningFlidForGhost.sourceClsid == m_cache.GetClassOfObject(hvo);
		}

		internal uint GhostOwnerClass
		{
			get { return m_cpiForOwningFlidForGhost.sourceClsid; }
		}

		private int CreateFirstChildForGhostOwner(int hvoGhostOwner, int owningFlidForGhost, int classToCreate)
		{
			int hvoNewChild;
			// create a new item that we can update with the new value.
			hvoNewChild = m_cache.CreateObject(classToCreate, hvoGhostOwner, (int)owningFlidForGhost, 0);
			m_cache.MainCacheAccessor.PropChanged(null, (int)PropChangeType.kpctNotifyAll,
				hvoGhostOwner, (int)owningFlidForGhost, 0, 1, 0);
			return hvoNewChild;
		}
	...
	class ComplexListChooserBEditControl : IBulkEditSpecControl
	{
		...
		public ComplexListChooserBEditControl(FdoCache cache, Mediator mediator,  XmlNode colSpec)
			...
			BulkEditBar.GetNamedList(cache, colSpec, "list"),
		...
		public ComplexListChooserBEditControl(int flid, int hvoList, string displayNameProperty,
			string fieldName, string displayWs)
		{
			m_hvoList = hvoList;
			m_flid = flid;
		...
		/// </summary>
		internal int[] ChosenHvos
		{
			get { return m_chosenHvos; }
			set { m_chosenHvos = value; }
		}
		...
		private ITsString BuildValueString(int[] chosenHvos)
		{
			ITsStrBldr bldr = TsStrBldrClass.Create();
			ITsString sep = null; // also acts as first-time flag.
			foreach (int hvoItem in chosenHvos)
			{
				CmObject item = CmObject.CreateFromDBObject(m_cache, hvoItem) as CmObject;
				PropertyInfo pi = item.GetType().GetProperty(m_displayNameProperty);
				ITsString tss = (ITsString)pi.GetValue(item, null);
				if (sep == null)
				{
					// first time create it
					sep = m_cache.MakeUserTss(", ");
				}
				else
				{
	...
	class VariantEntryTypesChooserBEditControl : ComplexListChooserBEditControl
	{
		...
		Set<int> m_variantEntryRefs = null;
		...
		/// <summary>
		/// kludge: filter to allow only variant entry references.
		/// </summary>
		/// <param name="hvoItem"></param>
		/// <returns></returns>
		protected override bool DisableItem(int hvoItem)
		{
			if (m_variantEntryRefs == null)

			{
				m_variantEntryRefs = new Set<int>();
				Dictionary<int, List<int>> dict = new Dictionary<int,List<int>>();
				LexEntry.LoadAllVariantEntryRefs(m_cache, dict);
				// go through each list and add the values to our set.
				foreach (List<int> refs in dict.Values)
					m_variantEntryRefs.AddRange(refs);
			}
			return !m_variantEntryRefs.Contains(hvoItem);
		}

		public override void DoIt(Set<int> itemsToChange, ProgressState state)
		{
			m_variantEntryRefs = null;	// reset the filtered entry refs cache.
	...
	class MorphTypeChooserBEditControl : FlatListChooserBEditControl
	{
			...
			ProgressState state)
		{
			...
			foreach (int hvoLexEntry in itemsToChange)
			{
				...
				if (fEnable)
					cda.CacheStringProp(hvoLexEntry, tagFakeFlid, item.AsTss);
				cda.CacheIntProp(hvoLexEntry, tagEnabled, (fEnable ? 1 : 0));
				i++;
			}
		}
	...
	internal abstract class FieldReadWriter : IGhostable
	{
		...
		public void SetNewValue(int hvo, ITsString tss)
		{
			SetNewValue(hvo, tss, false);
		}
		public void SetNewValue(int hvo, ITsString tss, bool fSendPropChanged)
		{
			SetNewValue(hvo, tss, fSendPropChanged, true);
		}

		protected abstract void SetNewValue(int hvo, ITsString tss, bool fSendPropChanged, bool fUpdateCreateModifyTime);
		...
		static internal int GetWsFromMetaData(int wsIn, int flid, FdoCache cache)
		{
			...
			if (ws == FDO.LangProj.LangProject.kwsAnal || ws == FDO.LangProj.LangProject.kwsAnals
				|| ws == FDO.LangProj.LangProject.kwsAnalVerns)
			{
				return cache.DefaultAnalWs;
			}
			else if (ws == FDO.LangProj.LangProject.kwsVern || ws == FDO.LangProj.LangProject.kwsVerns
				|| ws == FDO.LangProj.LangProject.kwsVernAnals)
			{
		...
		static public FieldReadWriter Create(XmlNode node, FdoCache cache, int hvoRootObj)
		{
			...
			if (transduceField == null || transduceField.Length == 0)
				return null;
			...
			if (clid == 0)
				return null;
			uint flid = mdc.GetFieldId2(clid, fieldName, true);
			int ws = LangProject.GetWritingSystem(node, cache, null, hvoRootObj, (int)flid, 0);
			if (parts.Length == 2)
			{
				FieldReadWriter frw = null;
				// parts are divided into class.propname
				if (DoItMethod.IsMultilingual((int)flid, mdc))
					frw = new OwnMlPropReadWriter(cache, (int)flid, ws);
				else
					frw = new OwnStringPropReadWriter(cache, (int)flid, GetWsFromMetaData(ws, (int)flid, cache));
				frw.InitForGhostItems(cache, node);
				return frw;
			}

			// parts.Length is 3. We have class.objectpropname.propname
			uint clidDst = mdc.GetDstClsId(flid);
			int fieldType = mdc.GetFieldType((uint)flid);
			uint flid2 = mdc.GetFieldId2(clidDst, parts[2], true);
			uint clidCreate = clidDst;	// default
			...
			if (DoItMethod.IsMultilingual((int)flid2, mdc))
			{
				Debug.Assert(ws != 0);
				// If it's a multilingual field and we didn't get a writing system, we can't transduce this field.
				if (ws == 0)
					return null;
				if (fieldType == (int)FieldType.kcptOwningAtom)
					return new OwnAtomicMlPropReadWriter(cache, (int)flid2, ws, (int)flid, (int)clidCreate);
				else if (fieldType == (int)FieldType.kcptOwningSequence)
					return new OwnSeqMlPropReadWriter(cache, (int)flid2, ws, (int)flid, (int)clidCreate);
				else
			...
			{
				if (fieldType == (int)FieldType.kcptOwningAtom)
					return new OwnAtomicStringPropReadWriter(cache, (int)flid2, ws, (int)flid, (int)clidCreate);
				else if (fieldType == (int)FieldType.kcptOwningSequence)
					return new OwnSeqStringPropReadWriter(cache, (int)flid2, ws, (int)flid, (int)clidCreate);
				else
					return null; // can't handle yet
			}
	...
	/// </summary>
	internal class ManyOnePathSortItemReadWriter : FieldReadWriter
	{
		FdoCache m_cache;
		XmlNode m_colSpec;
		BrowseViewer m_bv;
		...
		private ManyOnePathSortItem GetManyOnePathSortItem(int hvo)
		{
			ICmObject obj = CmObject.CreateFromDBObject(m_cache, hvo);
			return new ManyOnePathSortItem(obj);	// just return an item based on the the RootObject
		}

		private void EnsureFinder()
		{
			if (m_finder == null)
				m_finder = LayoutFinder.CreateFinder(m_cache, m_colSpec, m_bv.BrowseView.Vc);
		}
		...
		/// <param name="fSendPropChanged"></param>
		/// <param name="fUpdateCreateModifyTime"></param>
		protected override void SetNewValue(int hvo, ITsString tss, bool fSendPropChanged, bool fUpdateCreateModifyTime)
		{
			throw new Exception("The method or operation is not implemented.");
		}
	...
	internal class OwnStringPropReadWriter : FieldReadWriter
	{
		...
		public override ITsString CurrentValue(int hvo)
		{
			...
			if (m_ghostParentHelper != null)
				hvoStringOwner = m_ghostParentHelper.GetOwnerOfBasicProperty(hvo);
			if (hvoStringOwner == 0)
		...
		protected override void SetNewValue(int hvo, ITsString tss, bool fSendPropChanged, bool fUpdateCreateModifyTime)
		{
			int hvoStringOwner = hvo;
			if (m_ghostParentHelper != null)
				hvoStringOwner = m_ghostParentHelper.FindOrCreateOwnerOfBasicProp(hvo, m_flid);
			m_sda.SetString(hvoStringOwner, m_flid, tss);
			if (fSendPropChanged)
				m_sda.PropChanged(null, (int)PropChangeType.kpctNotifyAll, hvoStringOwner, m_flid, 0, 0, 0);
			else if (fUpdateCreateModifyTime)
				// if we don't send a prop change, update the modified date
				m_cache.CreateModifyManager.PropChanged(hvoStringOwner, m_flid, 0, 0, 0);
		}
	...
	internal class OwnAtomicStringPropReadWriter : OwnStringPropReadWriter
	{
		...
		protected override void SetNewValue(int hvo, ITsString tss, bool fSendPropChanged, bool fUpdateCreateModifyTime)
		{
			...
			// If the object existed previously, just send prop change on the string prop, as that is all that changed.
			base.SetNewValue(ownedAtomicObj, tss, fSendPropChanged && fHadObject, fUpdateCreateModifyTime && fHadObject);
			if (fSendPropChanged && !fHadObject)
			{
				// since the object is newly created, send one on the object property, but NOT on the string one,
				// since nothing can be displaying that already.
				m_sda.PropChanged(null, (int) PropChangeType.kpctNotifyAll, hvo, m_flidObj, 0, 1, 0);
			}
			else if (fUpdateCreateModifyTime && !fHadObject)
			{
				// if we don't send a prop change, update the modified date
				m_cache.CreateModifyManager.PropChanged(hvo, m_flidObj, 0, 1, 0);
			}
		}
	}
	...
	internal class OwnSeqStringPropReadWriter : OwnStringPropReadWriter
	{
		...
		protected override void SetNewValue(int hvo, ITsString tss, bool fSendPropChanged, bool fUpdateCreateModifyTime)
		{
			...
			// If the object existed previously, just send prop change on the string prop, as that is all that changed.
			base.SetNewValue(firstSeqObj, tss, fSendPropChanged && fHadOwningItem, fUpdateCreateModifyTime && fHadOwningItem);
			if (fSendPropChanged && !fHadOwningItem)
			{
				// If the first object in the sequence is newly created, send one on the seq property, but NOT on the string one,
				// since nothing can be displaying that already.
				m_sda.PropChanged(null, (int) PropChangeType.kpctNotifyAll, hvo, m_flidObj, 0, 1, 0);
			}
			else if (fUpdateCreateModifyTime && !fHadOwningItem)
			{
				// if we don't send a prop change, update the modified date
				m_cache.CreateModifyManager.PropChanged(hvo, m_flidObj, 0, 1, 0);
			}
	...
	internal class OwnMlPropReadWriter : OwnStringPropReadWriter
	{
		...
		protected override void SetNewValue(int hvo, ITsString tss, bool fSendPropChanged, bool fUpdateCreateModifyTime)
		{
			...
			if (fSendPropChanged)
				m_sda.PropChanged(null, (int)PropChangeType.kpctNotifyAll, hvoStringOwner, m_flid, m_ws, 0, 0);
			else if (fUpdateCreateModifyTime)
				// if we don't send a prop change, update the modified date
				m_cache.CreateModifyManager.PropChanged(hvoStringOwner, m_flid, m_ws, 0, 0);
		}
	}
