...
namespace SIL.FieldWorks.Common.Controls
{
	...
	public class XmlVc : FwBaseVc
	{
		...
		/// <summary>
		/// This also flags that an object number should not be displayed until the first line
		/// (paragraph) of that object is displayed.
		/// </summary>
		bool m_fDelayNumber = false;
		int m_hvoDelayedNumber = 0;
		...
		private bool IsItemEmpty(int hvo, int fragId)
		{
			...
			string[] keys = XmlViewsUtils.ChildKeys(m_cache, m_sda, node, hvo, m_layouts, command.Caller, m_stringTable, WsForce);
		...
		public override void DisplayVec(IVwEnv vwenv, int hvo, int flid, int frag)
		{
			...
			if (flid == LexEntryTags.kflidSenses)
			...
			{
				int flidSenseMsa = LexSenseTags.kflidMorphoSyntaxAnalysis;
				...
				if (fAllMsaSame)
				{
					...
					try
					{
						vwenv.AddObj(rghvo[0], this, childFrag);
					}
					finally
					{
			...
			if (fShowAsParagraphs && chvo > 0)
			{
				...
				if (!String.IsNullOrEmpty(sBefore) || DelayedNumberExists)
				{
					...
					ITsString tssNumber = GetDelayedNumber(specialAttrsNode, vwenv is TestCollectorEnv);
					string sStyle = XmlUtils.GetAttributeValue(listDelimitNode, "beforeStyle");
					if (!String.IsNullOrEmpty(sStyle))
					{
						ITsStrBldr bldr = tssBefore.GetBldr();
					...
					if (tssNumber != null)
					{
						...
						ITsStrBldr tsb = tssBefore.GetBldr();
				...
				vwenv.AddString(m_cache.TsStrFactory.MakeString(StringUtils.kChHardLB.ToString(),
			...
			for (ihvo = 0; ihvo < chvo; ++ihvo)
			{
				...
				if (fNumber)
				{
						...
						if (sTag[ich] == '%' && ich + 1 < sTag.Length)
						{
							...
							else if (sTag[ich] == 'O')
							{
								if (m_mdc.get_IsVirtual(flid))
									...
									sNum = m_cache.GetOutlineNumber(m_cache.ServiceLocator.GetInstance<ICmObjectRepository>().GetObject(rghvo[ihvo]), false, true);
								break;
							}
							else if (sTag[ich] == 'z')
							{
								...
								string sOutline = m_cache.GetOutlineNumber(m_cache.ServiceLocator.GetInstance<ICmObjectRepository>().GetObject(rghvo[ihvo]), false, true);
								int cchPeriods = 0;
					...
					if (fDelayNumber)
					{
						...
						m_tssDelayedNumber = tisb.GetString();
						m_fDelayNumber = true;
						m_hvoDelayedNumber = hvo;
					}
					else
					{
						vwenv.AddString(tss);
						m_tssDelayedNumber = null;
						m_fDelayNumber = false;
					}
					// This groups senses by placing graminfo before the number, and omitting it if the same as the
					// previous sense in the entry.  This isn't yet supported by the UI, but may well be requested in
					// the future.  (See LT-9663.)
			...
			if (fSingleGramInfoFirst)
				m_fIgnoreGramInfo = false;
			// Reset the flag for delaying displaying a number.
			if (m_fDelayNumber && hvo == m_hvoDelayedNumber)
			{
				m_fDelayNumber = false;
				m_tssDelayedNumber = null;
			}
		...
		/// </summary>
		private int[] GetVector(ISilDataAccess sda, int hvo, int tag)
		{
		...
		private void AddMultipleAlternatives(IEnumerable<int> wsIds, IVwEnv vwenv, int hvo, int flid, XmlNode frag, bool fCurrentHvo)
		{
			...
			try
			{
				switch (frag.Name)
				{
					...
					case "configureMlString":
						{
							...
							{
								if (hvoTarget != hvo)
									DisplayOtherObjStringAlt(flid, wsIds.First(), vwenv, hvoTarget);
								else
					...
					case "para":
						{
							ProcessProperties(frag, vwenv);
							// Note: the following assertion is indicative of not setting colSpec
							// attribute multipara="true"
...
//			XmlNode partNode = GetPartFromParentNode(parentNode, rootObjClass);
		...
		private void ProcessPartRef(XmlNode frag, int hvo, IVwEnv vwenv)
		{
			...
			if (fSingleGramInfoFirst && m_fIgnoreGramInfo)
				return;
		...
		private void InsertLiteralString(XmlNode frag, IVwEnv vwenv, string attrName)
		{
			...
			ITsString tss = m_cache.TsStrFactory.MakeString(item,
				m_cache.ServiceLocator.WritingSystemManager.UserWs);
			ITsString tssNumber = GetDelayedNumber(frag, vwenv is TestCollectorEnv);
			string sStyle = XmlUtils.GetAttributeValue(frag, attrName + "Style");
			if (!String.IsNullOrEmpty(sStyle))
			{
		...
		private bool DelayedNumberExists
		{
			get
			{
				return m_fDelayNumber && m_tssDelayedNumber != null && m_tssDelayedNumber.Length > 0;
			}
		}

		private ITsString GetDelayedNumber(XmlNode frag, bool fTest)
		{
