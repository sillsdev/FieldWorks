...
namespace SIL.FieldWorks.Common.Controls
{
	...
	public class XmlRDEBrowseViewVc : XmlBrowseViewBaseVc
	{
		...
		internal override List<XmlNode> ColumnSpecs
		{
			get
			{
				CheckDisposed();

				return base.ColumnSpecs;
			}
			set
			{
				CheckDisposed();

				base.ColumnSpecs = value;
				InstallVirtualHandlersForColumns();
			}
		}
			...
			lock (this)
			{
				while (!done)
				{
					...
					foreach (int hvoSense in EditableObjectsIds())
					{
						...
						{
							// causes the following sql : "select * from CmObject where id='hvoSense'"
							int clsid = m_cache.GetClassOfObject(hvoSense);
							if (clsid <= 0)
							{
