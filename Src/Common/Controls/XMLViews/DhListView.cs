		...
		private List<int> ColumnDisplayOrder
		{
			set
			{
				if (AllowColumnReorder == true && (value != null))
				{
					...
					IntPtr hHeader = SendMessage(this.Handle, LVM_SETCOLUMNORDERARRAY, value.Count, value.ToArray());
				}
			}
			get
			{
				...
				else
				{
					...
					IntPtr hHeader = SendMessage(this.Handle, LVM_GETCOLUMNORDERARRAY, columnCount, columnOrders);
