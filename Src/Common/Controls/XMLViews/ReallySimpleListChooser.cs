...
namespace SIL.FieldWorks.Common.Controls
{
	...
	public class ReallySimpleListChooser : Form, IFWDisposable
	{
			...
			IVwStylesheet stylesheet)
		{
			...
			Text = String.Format(XMLViewsStrings.ksChooseX, fieldName);
		...
		private void GenerateDefaultPage(ITsString tssTitle, ITsString tssDesc)
		{
			...
			{
				title = tssTitle.Text;
				int wsHandle = StringUtils.GetWsAtOffset(tssTitle, 0);
			...
			if (tssDesc != null)
			{
				...
				{
					int wsHandle = StringUtils.GetWsAtOffset(tssDesc, 0);
					ws = m_cache.ServiceLocator.WritingSystemManager.Get(wsHandle);
