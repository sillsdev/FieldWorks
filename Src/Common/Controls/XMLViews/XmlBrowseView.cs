...
namespace SIL.FieldWorks.Common.Controls
{
	...
	public class XmlBrowseView : XmlBrowseViewBase
	{
		...
		public override bool HandleTabAsControl
		{
			get
			{
				return false;	// tab should move the cursor between cells in the table.
			}
		}
		...
		protected override void OnMouseUp(MouseEventArgs e)
		{
			...
			try
			{
				...
				int newSelectedIndex = GetRowIndexFromSelection(vwsel);
					...
					if (vwsel != null && vwsel.SelType == VwSelType.kstText)
					{
						...
						if (icol != Vc.OverrideAllowEditColumn + 1)
						{
							...
							if (vwSelWord != null)
							{
								...
								hvoNewSelRow = m_fdoCache.MainCacheAccessor.get_VecItem(m_hvoRoot, m_fakeFlid,
																						newSelectedIndex);
								int hvoObj, tag, ws;
								bool fAssocPrev;
		...
		public override void SelectionChanged(IVwRootBox prootb, IVwSelection vwselNew)
		{
			...
			try
			{
				...
				if (!ReadOnlySelect && !m_fHandlingMouseUp)
				{
					...
					base.SelectionChanged(prootb, vwselNew);
					// The previous call can invalidate the selection.  It's rare, but quite
					...
					else
					{
						...
						prootb.MakeTextSelection(ihvoRoot, cvsli, rgvsli, tagTextProp, cpropPrevious,
							ichAnchor, ichEnd, ws, fAssocPrev, ihvoEnd, ttpBogus, true);
						DoSelectionSideEffects(prootb.Selection);
					}
