...
using SIL.FieldWorks.FDO;			// for FdoCache
...
namespace SIL.FieldWorks.Common.Controls
{
	...
	public class XmlViewsUtils
	{
		...
		public static bool TryFindPluralFormFromClassId(FdoCache cache, StringTable tbl, uint clsId, out string titleStr)
		{
			titleStr = null;
			if (clsId != 0)
			{
				string className = cache.GetClassName(clsId);
		...
		public static string[] FindParams(XmlNode input)
		{
			...
			return StringUtils.ListToStringArray(ap.Parameters);
		}
		...
		public static string FindWsParam(XmlNode node)
		{
			...
			foreach (string s in paramList)
			{
				...
				if (s.StartsWith(LangProject.WsParamLabel))
				{
					return s.Substring(LangProject.WsParamLabel.Length);
				}
			}
			return "";
		}
		...
		/// <returns></returns>
		/// ------------------------------------------------------------------------------------
		public static int FindIndexOfMatchingNode(List<XmlNode> nodes, XmlNode target)
		{
			int index = 0;
			foreach(XmlNode node in nodes)
			{
		...
		static public string[] Assemble(string[] items)
		{
			...
			foreach(string s in items)
				bldr.Append(s);
			return new string[] {bldr.ToString()};
		}
		...
		/// </summary>
		/// ------------------------------------------------------------------------------------
		static internal string[] ChildKeys(FdoCache fdoCache, XmlNode layout, int hvo,
			LayoutCache layoutCache, XmlNode caller, StringTable stringTbl, int wsForce)
		{
			...
			foreach (XmlNode child in layout.ChildNodes)
			{
				if (child is XmlComment)
					continue;
				result = Concatenate(result, StringsFor(fdoCache, child, hvo, layoutCache, caller, stringTbl, wsForce));
			}
			return result;
		}
		...
		static private string[] AssembleChildKeys(FdoCache fdoCache, XmlNode layout, int hvo,
			LayoutCache layoutCache, XmlNode caller, StringTable stringTbl, int wsForce)
		{
			return Assemble(ChildKeys(fdoCache, layout, hvo, layoutCache, caller, stringTbl, wsForce));
		}
		...
		internal static string DisplayWsLabel(IWritingSystem qws, FdoCache cache)
		{
			if (qws == null)
				return "";
		...
		static string AddMultipleAlternatives(FdoCache cache, Set<int> wsIds, int hvo, int flid, XmlNode frag)
		{
			...
			{
				string val = cache.MainCacheAccessor.get_MultiStringAlt(hvo, flid, ws).Text;
				if (val == null || val.Length == 0)
					continue; // doesn't even count as 'first'
				if (fLabel)
				{
					IWritingSystem wsEngine = cache.LanguageWritingSystemFactoryAccessor.get_EngineOrNull(ws);
					result += DisplayWsLabel(wsEngine, cache);
				}
				if (fFirst)
				{
					fFirst = false;
		...
		internal static string DisplayMultiSep(XmlNode frag, FdoCache cache)
		{
		...
		/// </summary>
		/// ------------------------------------------------------------------------------------
		static private string[] ProcessMultiLingualChildren(FdoCache fdoCache, XmlNode frag, int hvo,
			LayoutCache layoutCache, XmlNode caller, StringTable stringTbl, int wsForce)
		{
			string sWs = XmlUtils.GetOptionalAttributeValue(frag, "ws");
			if (sWs == null)
			...
			try
			{
				Set<int> wsIds = LangProject.GetAllWritingSystems(frag, fdoCache, s_qwsCurrent, 0, 0);
				...
				{
					s_qwsCurrent = fdoCache.LanguageWritingSystemFactoryAccessor.get_EngineOrNull(WSId);
					result = Concatenate(result, ChildKeys(fdoCache, frag, hvo, layoutCache, caller, stringTbl, wsForce));
				}
			}
			finally
			{
		...
		static private XmlNode GetLayoutNodeForChild(FdoCache fdoCache, int hvoTarget, int flid, string targetLayoutName,
			XmlNode layout, LayoutCache layoutCache)
		{
			XmlNode layoutTarget = XmlVc.GetNodeForPart(hvoTarget, targetLayoutName, true, fdoCache.MainCacheAccessor, layoutCache);
			if (layoutTarget == null)
		...
		public static int GetWsFromString(string wsParam, FdoCache cache)
		{
			...
			switch (wsParam)
			{
				...
				case "pronunciation":
					return cache.LangProject.DefaultPronunciationWritingSystem;
				case "":
					return cache.DefaultAnalWs;		// Most likely value.
				default:
					return cache.LanguageWritingSystemFactoryAccessor.GetWsFromStr(wsParam);
			}
		}
	}
	...
	{
		public ReplaceParamWithDefault() {}
