...
//		}
	...
	public class SortMethodFinder : LayoutFinder
	{
			...
			: base(cache, layoutName, colSpec, null, app)
		{
			...
			if (wsSpec != null && wsSpec.StartsWith(StringUtils.WsParamLabel))
		...
		public override string[] SortStrings(IManyOnePathSortItem item, bool sortedFromEnd)
		{
			...
			string key = GetKey(layout, rootObject, item, 0, sortedFromEnd);
			if (key == null)
			{
				...
				if (key == null)
				{
					...
					if (key == null)
					{
						...
						key = keyCmObjectUsing.SortKey ?? "";
						if (sortedFromEnd)
							key = StringUtils.ReverseString(key);

						key = key + " " + keyCmObjectUsing.SortKey2Alpha;
					}
				}
			}
