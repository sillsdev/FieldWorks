...
namespace SIL.FieldWorks.Common.Controls
{
	...
	public class LayoutCache
	{
		...
		/// <summary>
		/// Layout Version Number (last updated by SteveMc, May 18, 2010, for 7000017).
		/// </summary>
		/// <remarks>Note: often we also want to update BrowseViewer.kBrowseViewVersion.</remarks>
		public const int LayoutVersionNumber = 10;
		...
		static char[] ktagMarkers = new[] { '-', '#', '%' };	// backported from 7.1 (See LT-11353.)
		...
		public XmlNode GetNode(int clsid, string layoutName, bool fIncludeLayouts)
		{
			...
			for( ; ; )
			{
				...
				if (classId == 0 && useName == origName)
				{
					...
					// view in preference to a default view of Object.  Backported from 7.1 (See LT-11353.)
	...
	public class PartOwnershipTree : FwDisposableBase
	{
		...
		private Set<int> GetDescendents(int hvoCommonAncestor, int relativesFlid)
		{
			...
			if (pathSpec == null)
				throw new ArgumentException(String.Format("Expected to find part ({0}) in ParentClassPathsToChildren", xpathToPart));
