...
namespace XMLViewsTests
{
	public class ConfiguredExportTests : MemoryOnlyBackendProviderRestoredForEachTestTestBase
	{
		...
		public void XHTMLExportGetDigraphMapsFromICUSortRules_TertiaryIgnorableDoesNotCrash()
		{
			...
			using(var stream = new MemoryStream())
			{
				using(var writer = new StreamWriter(stream))
				{
					...
					Assert.That(() => data = exporter.GetDigraphs(ws, out mapChars, out ignoreSet), Throws.Nothing);
		...
		public void XHTMLExportGetLeadChar_SurrogatePairDoesNotCrash()
		{
			...
			Assert.That(() => data = ConfiguredExport.GetLeadChar(entryLetter, "ipo", wsDigraphMap, wsCharEquivalentMap, wsIgnorableCharMap, null, Cache), Throws.Nothing);
		...
		public void XHTMLExportGetLeadChar_MultigraphsInIgnoreListAreIgnored()
		{
			...
			Assert.That(() => data = ConfiguredExport.GetLeadChar("ignore-a", "guq", wsDigraphMap, wsCharEquivalentMap, wsIgnorableCharMap, null, Cache), Throws.Nothing);
			Assert.That(data, Is.EqualTo("a"));
			// test for the shortest of the ignore rules
			Assert.That(() => data = ConfiguredExport.GetLeadChar("ia", "guq", wsDigraphMap, wsCharEquivalentMap, wsIgnorableCharMap, null, Cache), Throws.Nothing);
			Assert.That(data, Is.EqualTo("a"));
		}
		...
		public void XHTMLExportGetLeadChar_PrimaryCollationProceedsSecondary()
		{
			...
			Assert.That(() => data = ConfiguredExport.GetLeadChar("cha", "guq", wsDigraphMap, wsCharEquivalentMap, wsIgnorableCharMap, null, Cache), Throws.Nothing);
		...
		public void XHTMLExportGetLeadChar_UsesCaseAlias()
		{
			...
			var wsCharEquivalentMap = new Dictionary<string, Dictionary<string, string>>();
			var wsIgnorableCharMap = new Dictionary<string, ISet<string>>();
			Assert.That(() => data = ConfiguredExport.GetLeadChar(headword, "tkr", wsDigraphMap, wsCharEquivalentMap, wsIgnorableCharMap, null, Cache), Throws.Nothing);
