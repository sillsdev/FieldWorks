...
namespace SIL.FieldWorks.Common.Controls
{
	...
	public class XmlBrowseViewBaseVc : XmlVc
	{
					...
					if (version != BrowseViewer.kBrowseViewVersion)
					{
						...
						switch (version)
						{
							...
							case 12:
								...
								savedCols = savedCols.Replace("root version=\"12\"", "root version=\"14\"");
		...
		private string RemoveWeatherColumn(string savedCols)
		{
...
//		}
		...
		private void AddTableCell(IVwEnv vwenv, int hvo, int index, int hvoRoot, int icolActive, int cAdjCol, int icol)
		{
			...
			if (xn != null)
			{
				...
				string sWs = XmlUtils.GetAttributeValue(node, "ws");
				if (sWs != null && sWs.StartsWith(StringUtils.WsParamLabel))
					sWs = sWs.Substring(StringUtils.WsParamLabel.Length);
				if (sWs == "reversal")
				{
					...
					if (m_wsReversal == 0)
					{
						...
						{
							ICmObject obj = repo.GetObject(hvo);
					...
					m_wsBest = m_wsReversal;
				}
				else if (fNeedObject && hvoBest == 0)
					m_wsBest = m_cache.DefaultAnalWs;	// we may need the object, but we don't have one!
				else
					m_wsBest = WritingSystemServices.GetWritingSystem(m_cache, m_sda,xn,
						null,
						hvoBest,
						flidBest,
				...
				if (m_wsBest != 0)
				{
					IWritingSystem ws = m_cache.ServiceLocator.WritingSystemManager.Get(m_wsBest);
					if (ws != null)
						...
						if (mi != null)
						{
							...
							if (sWs != null)
							{
								...
								if (sWs == "reversal")
									...
									ws = XmlViewsUtils.GetWsFromString(sWs, m_cache);
								if (ws != 0)
								{
