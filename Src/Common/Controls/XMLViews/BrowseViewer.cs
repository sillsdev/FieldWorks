...
namespace SIL.FieldWorks.Common.Controls
{
	...
	public enum BrowseColumnFilterKind
	{
		...
		NonBlank,
		/// <summary>Match rows whose cell spans more than one line (multipara columns).</summary>
		MoreThanOneLine,
		/// <summary>Match rows whose cell is exactly one (non-empty) line (multipara columns).</summary>
		ExactlyOneLine,
		/// <summary>Exact match on the localized "yes" token (sortType=YesNo preset).</summary>
		Yes,
		/// <summary>Exact match on the localized "no" token (sortType=YesNo preset).</summary>
		No,
		/// <summary>Exact match on the localized "0" token (sortType=integer "Zero" preset).</summary>
		Zero,
		/// <summary>Match rows whose integer value is in the inclusive range [min, max] supplied via text "min,max".</summary>
		IntRange
	}

	/// <summary>
	/// The match style of the legacy FilterBar "Filter For…" dialog (<c>SimpleMatchDlg</c>), surfaced so the
	/// owned Avalonia filter flyout can request the SAME matcher class through
	/// <see cref="BrowseViewer.MakePatternColumnFilter"/>. Each value maps 1:1 to a
	/// <c>SIL.FieldWorks.Filters</c> matcher (Anywhere/Begin/End/Exact/RegExp).
	/// </summary>
	public enum BrowsePatternMatchType
	{
	...
	/// <summary>
	/// The date relation of the legacy FilterBar "Restrict Date…" dialog (<c>SimpleDateMatchDlg</c>), surfaced so
	/// the owned Avalonia filter flyout can request the SAME <c>DateTimeMatcher</c> through
	/// <see cref="BrowseViewer.MakeDateColumnFilter"/>. Each value maps 1:1 to a
	/// <c>DateTimeMatcher.DateMatchType</c> (On/NotRange/Before/After/Range); the product edge maps between them.
	/// </summary>
	public enum BrowseDateMatchKind
	{
		/// <summary>On the chosen day (a one-day inclusive range — the dialog's default).</summary>
		On,
		/// <summary>Not on the chosen day (the inverse of the one-day range).</summary>
		NotOn,
...
//		bool m_doHScroll = false;
		...
		/// <summary>
		/// Stage 3 (Avalonia browse): the raw value of an arbitrary column-spec attribute, or null when out of
		/// range / absent. The owned header/filter UI reads <c>cansortbylength</c>, <c>multipara</c>, and
		/// <c>sortType</c> through this to gate the SAME type-specific sort toggles and filter presets the legacy
		/// header context menu / FilterBar gate on those attributes.
		/// </summary>
		public string GetColumnSpecAttribute(int icol, string attrName)
		{
			CheckDisposed();
		...
		/// <summary>
		/// Configure-Columns (Avalonia, P1): the full catalog of columns that COULD be shown — the legacy
		/// ColumnConfigureDialog's "possible columns" set (<see cref="XmlBrowseViewBaseVc.PossibleColumnSpecs"/>)
		/// — each projected to a stable <see cref="BrowseColumnInfo"/> key/label/ws-flag the Avalonia dialog
		/// and owned column model consume. The key is the column's layout-token identity (see
		/// <see cref="ColumnKey"/>) so a persisted/configured column re-resolves by identity, not position.
		/// </summary>
		public IReadOnlyList<BrowseColumnInfo> GetAvailableColumns()
		{
			CheckDisposed();
			var possible = m_xbv.Vc.PossibleColumnSpecs ?? new List<XmlNode>();
			var result = new List<BrowseColumnInfo>(possible.Count);
			foreach (XmlNode node in possible)
				result.Add(new BrowseColumnInfo(ColumnKey(node), ColumnLabel(node), ColumnHasWsParam(node)));
			return result;
		}

		/// <summary>Configure-Columns (Avalonia, P1): the stable key of the currently-shown column at <paramref name="icol"/>.</summary>
		public string GetColumnKey(int icol)
		{
			CheckDisposed();
			return ColumnKey(ColumnSpecs[icol]);
		}

		// The stable per-column identity used by Configure-Columns persistence and re-resolution: the legacy
		// columns differ on (layout, ws, label) — two "Form" columns can share a layout but target different
		// writing systems — so the key composes those three. Mirrors the legacy dialog's duplicate test, which
		// treats (label, ws) as the column's identity. Kept deterministic so a persisted key round-trips.
		private static string ColumnKey(XmlNode node)
		{
			if (node == null)
				return string.Empty;
			var layout = XmlUtils.GetOptionalAttributeValue(node, "layout") ?? string.Empty;
			var ws = XmlUtils.GetOptionalAttributeValue(node, "ws")
				?? XmlUtils.GetOptionalAttributeValue(node, "originalWs") ?? string.Empty;
			var label = XmlUtils.GetOptionalAttributeValue(node, "label")
				?? XmlUtils.GetOptionalAttributeValue(node, "originalLabel") ?? string.Empty;
			return layout + "|" + ws + "|" + label;
		}

		private static string ColumnLabel(XmlNode node)
		{
			var label = XmlUtils.GetLocalizedAttributeValue(node, "originalLabel", null)
				?? XmlUtils.GetLocalizedAttributeValue(node, "label", null);
			return label ?? XmlUtils.GetOptionalAttributeValue(node, "label") ?? string.Empty;
		}

		// Mirrors the legacy ColumnConfigureDialog "ws combo is enabled" rule: the column carries a
		// configurable writing-system parameter (a $ws param, or a plain ws attribute).
		private static bool ColumnHasWsParam(XmlNode node)
		{
			if (!string.IsNullOrEmpty(XmlViewsUtils.FindWsParam(node)))
				return true;
			return !string.IsNullOrEmpty(XmlUtils.GetOptionalAttributeValue(node, "ws"));
		}

		/// <summary>
		/// Configure-Columns apply path (Avalonia, P1): replace the shown columns with the ordered set named
		/// by <paramref name="orderedKeys"/> (keys from <see cref="GetAvailableColumns"/>), then drive the
		/// legacy <see cref="InstallNewColumns"/> so the still-live viewer underneath stays in sync (width
		/// preservation + sorter re-init). A key with no matching possible/current column is skipped; the
		/// resolved list keeps the requested order. Returns the keys actually installed (the survivors).
		/// </summary>
		public IReadOnlyList<string> InstallColumnsByKey(IReadOnlyList<string> orderedKeys)
		{
			CheckDisposed();
			if (orderedKeys == null)
				return new List<string>();
			// Build a key->node map from the union of currently-shown specs (so an active column with applied
			// edits is preserved as-is) and the possible-columns catalog (so a hidden column can be re-added).
			var byKey = new Dictionary<string, XmlNode>(StringComparer.Ordinal);
			foreach (XmlNode node in m_xbv.Vc.PossibleColumnSpecs ?? new List<XmlNode>())
			{
				var key = ColumnKey(node);
				if (!byKey.ContainsKey(key))
					byKey[key] = node;
			}
			foreach (XmlNode node in ColumnSpecs)
				byKey[ColumnKey(node)] = node; // a shown column overrides the catalog node (same identity)

			var newSpecs = new List<XmlNode>(orderedKeys.Count);
			var installedKeys = new List<string>(orderedKeys.Count);
			foreach (var key in orderedKeys)
			{
				if (byKey.TryGetValue(key, out var node) && !newSpecs.Contains(node))
				{
					newSpecs.Add(node);
			...
			if (newSpecs.Count == 0)
				return installedKeys; // refuse to install an empty column set (legacy keeps at least one)

			InstallNewColumns(newSpecs);
...
//		}
		...
		internal void InstallNewColumns(List<XmlNode> newColumnSpecs)
		{
			...
			m_colWidths = null;
			// The Avalonia-mirror per-column finder cache is keyed positionally by the OLD column set; the
			// column list just changed (show/hide/reorder via Configure-Columns), so drop it. It rebuilds
			// lazily against the new ColumnSpecs on the next GetRowCellStrings/GetRowCellTsString.
			m_avaloniaColumnFinders = null;
...
#pragma warning restore CS0219
		...
		{
			return MakeColumnSorter(dataColumnIndex, ascending, false, false);
		}

		/// <summary>
		/// Rendering-cutover (F1) + header-toggle parity: as <see cref="MakeColumnSorter(int,bool)"/>, but also
		/// applies the legacy header context-menu toggles by setting the SAME
		/// <c>StringFinderCompare.SortedFromEnd</c>/<c>SortedByLength</c> flags the WinForms header sets — so the
		/// owned Avalonia "Sort From End"/"Sort By Length" toggles reorder identically.
		/// </summary>
		public RecordSorter MakeColumnSorter(int dataColumnIndex, bool ascending, bool sortedFromEnd, bool sortedByLength)
		{
			...
			comparer.SortedFromEnd = sortedFromEnd;
			comparer.SortedByLength = sortedByLength;
			if (!ascending)
				comparer.Reverse();
		...
		public RecordFilter MakeColumnFilter(int dataColumnIndex, BrowseColumnFilterKind kind, string text)
		{
			...
			switch (kind)
			{
				...
				case BrowseColumnFilterKind.NonBlank:
					...
					break;
				case BrowseColumnFilterKind.MoreThanOneLine:
					matcher = new MoreThanOneLineMatcher();
					break;
				case BrowseColumnFilterKind.ExactlyOneLine:
					matcher = new ExactlyOneLineMatcher();
					break;
				case BrowseColumnFilterKind.Yes:
					matcher = new ExactMatcher(MakeExactPattern(XMLViewsStrings.ksYes));
					break;
				case BrowseColumnFilterKind.No:
					matcher = new ExactMatcher(MakeExactPattern(XMLViewsStrings.ksNo));
					break;
				case BrowseColumnFilterKind.Zero:
					matcher = new ExactMatcher(MakeExactPattern(XMLViewsStrings.ksZero));
					break;
				case BrowseColumnFilterKind.IntRange:
					if (!TryParseIntRange(text, out var min, out var max))
						return null;
					matcher = new RangeIntMatcher(min, max);
					break;
				default:
		...
		/// <summary>
		/// Builds the "Filter For…" pattern filter (the legacy <c>FindComboItem</c>/<c>SimpleMatchDlg</c> path):
		/// the SAME managed matcher class the WinForms dialog produces for the chosen match style, over the
		/// column's finder. The pattern carries case-sensitivity (<paramref name="matchCase"/>); regex match is
		/// requested via <see cref="BrowsePatternMatchType.Regex"/>. Returns null for an empty pattern.
		/// </summary>
		public RecordFilter MakePatternColumnFilter(int dataColumnIndex, string pattern,
			BrowsePatternMatchType matchType, bool matchCase)
		{
			CheckDisposed();
			...
			var ivwPattern = MakeFilterForPattern(pattern, colWs.Handle, matchType, matchCase);
			IMatcher matcher;
			switch (matchType)
			{
				case BrowsePatternMatchType.AtStart:
					matcher = new BeginMatcher(ivwPattern);
					break;
				case BrowsePatternMatchType.AtEnd:
					matcher = new EndMatcher(ivwPattern);
					break;
				case BrowsePatternMatchType.WholeItem:
					matcher = new ExactMatcher(ivwPattern);
					break;
				case BrowsePatternMatchType.Regex:
					matcher = new RegExpMatcher(ivwPattern);
					break;
				default:
					matcher = new AnywhereMatcher(ivwPattern);
					break;
			}
			matcher.WritingSystemFactory = m_cache.WritingSystemFactory;
			return new FilterBarCellFilter(finder, matcher);
		...
		/// <summary>
		/// Builds a <c>stringList</c> enumerated-value filter (FilterBar's per-value exact match, or its
		/// "Exclude X" inverse): an exact whole-cell match on the localized value, inverted when
		/// <paramref name="exclude"/> — the SAME <c>ExactMatcher</c>/<c>InvertMatcher</c> pair FilterBar uses.
		/// </summary>
		public RecordFilter MakeStringListColumnFilter(int dataColumnIndex, string value, bool exclude)
		{
			CheckDisposed();
			var specs = m_xbv.Vc.ColumnSpecs;
			if (dataColumnIndex < 0 || dataColumnIndex >= specs.Count || value == null)
				return null;
			var colSpec = specs[dataColumnIndex];
			var finder = LayoutFinder.CreateFinder(m_cache, colSpec, m_xbv.Vc, m_propertyTable.GetValue<IApp>("App"));

			IMatcher matcher = new ExactMatcher(MakeExactPattern(value));
			if (exclude)
				matcher = new InvertMatcher(matcher);
			matcher.WritingSystemFactory = m_cache.WritingSystemFactory;
			return new FilterBarCellFilter(finder, matcher);
		}

		/// <summary>
		/// The enumerated display values for a <c>stringList</c> column, read the SAME way <c>FilterBar</c>
		/// reads them (<c>BrowseView.GetStringList</c> off the column spec), or null when the column is not a
		/// stringList column / out of range.
		/// </summary>
		public string[] GetColumnStringList(int dataColumnIndex)
		{
			CheckDisposed();
			var specs = m_xbv.Vc.ColumnSpecs;
			if (dataColumnIndex < 0 || dataColumnIndex >= specs.Count)
				return null;
			return m_xbv.GetStringList(specs[dataColumnIndex]);
		}

		/// <summary>
		/// Builds the "Restrict Date…" date filter (the legacy <c>RestrictDateComboItem</c>/<c>SimpleDateMatchDlg</c>
		/// path): the SAME <c>DateTimeMatcher</c> the WinForms dialog produces for the chosen relation, over the
		/// column's finder. <paramref name="handleGenDate"/> matches GenDate values rather than DateTime. The
		/// dialog's [start, start+endOfDay] one-day-range semantics for On/NotOn live in the date dialog VM (the
		/// product edge), so this just builds the matcher from the supplied range — mirroring how the WinForms
		/// dialog's ResultingMatcher fed the same (start, end, type) into the matcher.
		/// </summary>
		public RecordFilter MakeDateColumnFilter(int dataColumnIndex, BrowseDateMatchKind kind, DateTime start,
			DateTime end, bool handleGenDate)
		{
			CheckDisposed();
			var specs = m_xbv.Vc.ColumnSpecs;
			if (dataColumnIndex < 0 || dataColumnIndex >= specs.Count)
			...
			DateTimeMatcher.DateMatchType type;
			switch (kind)
			{
				case BrowseDateMatchKind.NotOn:
					type = DateTimeMatcher.DateMatchType.NotRange;
					break;
				case BrowseDateMatchKind.OnOrBefore:
					type = DateTimeMatcher.DateMatchType.Before;
					break;
				case BrowseDateMatchKind.OnOrAfter:
					type = DateTimeMatcher.DateMatchType.After;
					break;
				case BrowseDateMatchKind.Between:
					type = DateTimeMatcher.DateMatchType.Range;
					break;
				default:
					type = DateTimeMatcher.DateMatchType.On;
					break;
			}
			var matcher = new DateTimeMatcher(start, end, type) { HandleGenDate = handleGenDate };
			matcher.WritingSystemFactory = m_cache.WritingSystemFactory;
			return new FilterBarCellFilter(finder, matcher);
		}

		/// <summary>
		/// The possibility-list items of a chooser (<c>bulkEdit</c>/<c>chooserFilter</c>) column — read the SAME
		/// way the legacy <c>ListChoiceComboItem</c> resolves its list (<c>BulkEditBar.GetNamedList(..., "list")</c>)
		/// — as LCModel-free (key=guid, name=display, depth) items, or null when the column is not a chooser column.
		/// </summary>
		public IReadOnlyList<BrowseChooserItem> GetColumnChooserList(int dataColumnIndex)
		{
			CheckDisposed();
			var specs = m_xbv.Vc.ColumnSpecs;
			if (dataColumnIndex < 0 || dataColumnIndex >= specs.Count)
				return null;
			var colSpec = specs[dataColumnIndex];
			var beSpec = GetChooserBeSpec(colSpec);
			if (string.IsNullOrEmpty(beSpec))
				return null;

			// Some chooser columns (for example "chooserFilter=external" with a dynamic-loader-based filter)
			// do not define a list-backed possibility list. They should not be treated as list-backed chooser
			// columns, otherwise the Avalonia filter flyout crashes while trying to resolve a mandatory "list"
			// attribute.
			if (string.IsNullOrEmpty(XmlUtils.GetOptionalAttributeValue(colSpec, "list", "")))
				return null;

			var list = BulkEditBar.GetNamedList(m_cache, colSpec, "list");
			if (list == null)
				return null;
			var items = new List<BrowseChooserItem>();
			foreach (ICmPossibility poss in list.PossibilitiesOS)
				AddChooserPossibility(items, poss, 0);
			return items;
		}

		// Whether a column spec carries a chooser bulkEdit/chooserFilter attribute (gates the "Choose…" entry),
		// mirroring the FilterBar.MakeCombo gate (beSpec = bulkEdit, falling back to chooserFilter).
		private static string GetChooserBeSpec(XmlNode colSpec)
		{
			var beSpec = XmlUtils.GetOptionalAttributeValue(colSpec, "bulkEdit", "");
			if (string.IsNullOrEmpty(beSpec))
				beSpec = XmlUtils.GetOptionalAttributeValue(colSpec, "chooserFilter", "");
			return beSpec;
		}

		// Adds a possibility (and its sub-possibilities, depth+1) as flat chooser items in document order — the
		// same flattening the Avalonia chooser renders as an indented list.
		private void AddChooserPossibility(List<BrowseChooserItem> items, ICmPossibility poss, int depth)
		{
			if (poss == null)
				return;
			var name = poss.ShortName ?? poss.Guid.ToString();
			items.Add(new BrowseChooserItem(poss.Guid.ToString(), name, depth));
			foreach (ICmPossibility sub in poss.SubPossibilitiesOS)
				AddChooserPossibility(items, sub, depth + 1);
		}

		/// <summary>
		/// Builds the "Choose…" list-choice filter (the legacy <c>ListChoiceComboItem</c> path): the SAME
		/// <c>ColumnSpecFilter</c> (a <c>ListChoiceFilter</c>) the WinForms chooser produces from the chosen
		/// possibility items, resolved from <paramref name="chosenKeys"/> (possibility guid strings) to hvos —
		/// mirroring <c>FilterBar.ListChoiceComboItem.MakeFilter</c>'s default (no explicit filterType) path with
		/// <c>ListMatchOptions.Any</c>. Returns null for an empty selection or a non-chooser column.
		/// </summary>
		public RecordFilter MakeListChoiceColumnFilter(int dataColumnIndex, IReadOnlyList<string> chosenKeys)
		{
			CheckDisposed();
			if (chosenKeys == null || chosenKeys.Count == 0)
				return null;
			var specs = m_xbv.Vc.ColumnSpecs;
			if (dataColumnIndex < 0 || dataColumnIndex >= specs.Count)
				return null;
			...
			foreach (var key in chosenKeys)
			{
				if (string.IsNullOrEmpty(key) || !Guid.TryParse(key, out var guid))
					continue;
				if (m_cache.ServiceLocator.ObjectRepository.TryGetObject(guid, out var obj) && obj != null)
					hvos.Add(obj.Hvo);
			}
			if (hvos.Count == 0)
				return null;
		...
		// dialog's Match Case checkbox), regex flag for the Regex match style, diacritic-insensitive (the
		// dialog's diacritic option is a deferred P2 no-op here), with the column's collating set up.
		private IVwPattern MakeFilterForPattern(string str, int ws, BrowsePatternMatchType matchType, bool matchCase)
		{
			...
			pattern.UseRegularExpressions = matchType == BrowsePatternMatchType.Regex;
			pattern.Pattern = TsStringUtils.MakeString(str, ws);
			pattern.IcuLocale = m_cache.WritingSystemFactory.GetStrFromWs(ws);
			SimpleStringMatcher.SetupPatternCollating(pattern, m_cache);
			return pattern;
		}

		// Mirrors FilterBar.MatchExactPattern: a case/diacritic-insensitive whole-cell pattern (the YesNo /
		// integer presets feed it the localized literal, e.g. "Yes"/"No"/"0").
		private IVwPattern MakeExactPattern(string str)
		{
		...
		// Parses the "min,max" range text the integer presets pass (e.g. "1,2147483647" for Greater Than Zero).
		private static bool TryParseIntRange(string text, out int min, out int max)
		{
			min = 0;
			max = 0;
			if (string.IsNullOrEmpty(text))
