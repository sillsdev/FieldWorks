...
{
	/// <summary>
	/// This interface is implemented by a custom combo box controls that support visual styles.
	/// </summary>
	public interface IVisualStyleComboBox
	{
		/// <summary>
		/// Gets the state.
		/// </summary>
		/// <value>The state.</value>
		ComboBoxState State { get; set; }
	...
	/// </summary>
	public interface IComboList : IVisualStyleComboBox
	{
		/// <summary></summary>
	...
	/// </summary>
	public class FwComboBox : UserControl, IFWDisposable, IVwNotifyChange, IComboList
	{
		#region Events
		...
		// The list box, usually only shown if the down arrow is clicked.
		ComboListBox m_comboListBox;
		...
		Rectangle ContentRectangle
		{
			get
			{
				...
				if (!m_hasBorder && (!FwComboBox.SupportsButtonStyle || !m_useVisualStyleBackColor || m_dropDownStyle != ComboBoxStyle.DropDownList))
					return ClientRectangle;
		...
		{
			VisualStyleRenderer renderer = CreateRenderer(m_state, m_useVisualStyleBackColor, m_dropDownStyle == ComboBoxStyle.DropDownList,
				ContainsFocus, m_hasBorder);
			if (renderer != null)
			{
		...
		public int SelectedIndex
		{
			...
			{
				CheckDisposed();
				return m_comboListBox.SelectedIndex;
			}
			set
			{
				CheckDisposed();
				m_comboListBox.SelectedIndex = value;
			}
		}
		/// ------------------------------------------------------------------------------------
		/// <summary>
		...
		public IVwStylesheet StyleSheet
		{
			...
			{
				CheckDisposed();
				return m_comboListBox.StyleSheet;
			}
			set
			{
				CheckDisposed();

				m_comboListBox.StyleSheet = value;
		...
		public object SelectedItem
		{
			...
			{
				CheckDisposed();
				return m_comboListBox.SelectedItem;
			}
			set
			{
				CheckDisposed();

				m_comboListBox.SelectedItem = value;
				if (value != null)
					m_comboTextBox.Tss = m_comboListBox.TextOfItem(value);
			}
		}
		...
		public override string Text
		{
			...
			set
			{
				...
				m_comboListBox.SelectedIndex = m_comboListBox.FindStringExact(value);
			}
		}
		...
		public ITsString Tss
		{
			...
			set
			{
				...
				m_comboListBox.SelectedIndex = m_comboListBox.FindIndexOfTss(value);
			}
		}
		/// ------------------------------------------------------------------------------------
		/// <summary>
		...
		public int WritingSystemCode
		{
			...
			set
			{
				...
				m_comboListBox.WritingSystemCode = value;
			}
		}
		...
		public int DropDownWidth
		{
			...
			{
				CheckDisposed();
				return m_comboListBox.FormWidth;
			}
			set
			{
				CheckDisposed();

				m_fListWidthSet = true; // list width no longer tracks control width.
				m_comboListBox.FormWidth = value;
			}
		}
		...
		public ILgWritingSystemFactory WritingSystemFactory
		{
			...
			set
			{
				...
				m_comboListBox.WritingSystemFactory = value;
			}
		}
		...
		//		This turned out not to be a standard for the listbox of drop downs. (bummer)
		//		protected override void OnMouseWheel(MouseEventArgs e)
		//		{
		//			// make sure we're not getting someone else's msg
		//			if (!this.m_fwTextBox.Focused)
		//				return;
		//
		//			// probably not needed...
		//			base.OnMouseWheel (e);
		...
		//				if (m_listBox.SelectedIndex < m_listBox.Items.Count-1)
		...
		public int FindStringExact(string str)
		{
			...
			return m_comboListBox.FindStringExact(str);
		}
		...
		public int FindStringExact(ITsString tss)
		{
			...
			return m_comboListBox.FindIndexOfTss(tss);
		}

		internal void HideListBox()
		{
			CheckDisposed();

			m_comboListBox.HideForm();
		}
		...
		private void ShowListBox()
		{
			...
			if (sz != m_comboListBox.Form.Size)
				m_comboListBox.Form.Size = sz;
			m_comboListBox.Launch(Parent.RectangleToScreen(Bounds), workingArea);
		...
		public void PropChanged(int hvo, int tag, int ivMin, int cvIns, int cvDel)
		{
			...
			if (tag != InnerFwTextBox.ktagText)
				return;
			m_comboListBox.SelectedIndex = m_comboListBox.FindIndexOfTss(m_comboTextBox.Tss);
		...
		private void m_listBox_SelectedIndexChanged(object sender, EventArgs e)
		{
			...
			if (m_comboListBox.Form.Visible && !m_comboListBox.KeepDropDownListDuringSelection)
			{
				HideListBox();
				fNeedFocus = true;
			}
			if (m_comboListBox.SelectedIndex == -1)
			{
			...
			else
			{
				...
				PreviousTextBoxText = m_comboTextBox.Tss;	// save for future use (if needed)
				m_comboTextBox.Tss = m_comboListBox.SelectedTss;
			}
			if (fNeedFocus)
		...
		{
			if (!m_comboListBox.KeepDropDownListDuringSelection)
			{
				HideListBox();
				m_comboTextBox.Focus();
			}
		...
		protected override void OnKeyDown(KeyEventArgs e)
		{
			...
			switch (e.KeyCode)
			{
				case Keys.Down:
				{
					if (e.Alt)
					{
						ShowListBox(); // opens the box
						e.Handled = true;
						return;
					}
					// If we don't have any items, we certainly can't highlight them!
					if (m_comboListBox.Items.Count == 0)
						return;
					// don't increment if already at the end
					if (m_comboListBox.SelectedIndex < m_comboListBox.Items.Count-1)
						m_comboListBox.SelectedIndex += 1;
					break;
				}
				case Keys.Up:
				{
					...
					else
					{
						...
						if (m_comboListBox.Items.Count == 0)
							return;
						// don't scroll up past first item
						if (m_comboListBox.SelectedIndex > 0 )
							m_comboListBox.SelectedIndex -= 1;
					}
					break;
				}
		...
		private void m_comboTextBox_KeyPress(object sender, KeyPressEventArgs e)
		{
			...
			if (m_dropDownStyle == ComboBoxStyle.DropDownList)
			{
				if (!Char.IsControl(e.KeyChar))
				{
					...
					m_comboListBox.HighlightItemStartingWith(e.KeyChar.ToString());
				}
				e.Handled = true;
			}
		}
		...
		{
			if (Application.RenderWithVisualStyles)
				State = m_comboListBox.Form.Visible ? ComboBoxState.Pressed : ComboBoxState.Normal;
		}
	...
	/// </summary>
	public class ComboListBox : FwListBox, IComboList
	{
		#region Data members
		...
		public int FormWidth
		{
			...
			{
				CheckDisposed();
				return m_listForm.Width;
			}
			set
			{
				CheckDisposed();
				m_listForm.Width = value;
			}
		}
		...
		public int FormHeight
		{
			...
			{
				CheckDisposed();
				return m_listForm.Height;
			}
			set
			{
				CheckDisposed();
				m_listForm.Height = value;
			}
		}
		...
		{
			HasBorder = true;
			this.Dock = DockStyle.Fill; // It fills the list form.
			// Create a form to hold the list.
			m_listForm = new Form();
			m_listForm.Size = this.Size;
		...
		public void AdjustSize(int maxWidth, int maxHeight)
		{
			...
			FormWidth = Math.Max(100, Math.Min(width, maxWidth));
		...
		public void HighlightItemStartingWith(string start)
		{
			...
			if (this.HighlightedItem != null)
				itemStr = TextOfItem(this.HighlightedItem).Text.Trim();
				...
				itemStr.ToLower().StartsWith(start.ToLower()))
			{
				...
				iPreviousHighlighted = this.HighlightedIndex;
			...
			iEnding = this.Items.Count - 1;
			fFound = FindAndHighlightItemStartingWith(iStarting, iEnding, start);
		...
		private bool FindAndHighlightItemStartingWith(int iStarting, int iEnding, string startKey)
		{
			...
			{
				itemStr = TextOfItem(this.Items[i]).Text.Trim();
				if (itemStr.ToLower().StartsWith(startKey.ToLower()))
				{
					// Highlight this item
					this.HighlightedItem = this.Items[i];
					fFound = true;
					break;
				}
			}
	...
	{
		ComboListBox m_comboListbox;
		bool m_fGotMouseDown = false; // true after a mouse down occurs anywhere at all.
		...
		public bool PreFilterMessage(ref Message m)
		{
			...
			switch ((Win32.WinMsgs)m.Msg)
			{
				...
				case Win32.WinMsgs.WM_LBUTTONUP:
				{
					...
					{
						int xPos = Utils.MiscUtils.LoWord(m.LParam);	// LOWORD(m.LParam);
						int yPos = Utils.MiscUtils.HiWord(m.LParam);	// HIWORD(m.LParam);
					...
					else if (c.GetType().ToString() == "SIL.FieldWorks.IText.Sandbox")
					{
						...
						int x1Pos = Utils.MiscUtils.LoWord(m.LParam);	// LOWORD(m.LParam);
						int y1Pos = Utils.MiscUtils.HiWord(m.LParam);	// HIWORD(m.LParam);
	...
	{
		IVisualStyleComboBox m_comboBox;
	...
	internal class DropDownButton : Button
	{
		...
		VisualStyleRenderer Renderer
		{
			get
			{
				...
				VisualStyleElement element = null;
				if (FwComboBox.SupportsButtonStyle)
				{
					ComboBoxState curState;
					if (m_comboBox.UseVisualStyleBackColor && m_comboBox.DropDownStyle == ComboBoxStyle.DropDownList)
					{
					...
					element = VisualStyleElement.CreateElement(FwComboBox.COMBOBOX_CLASS, CP_DROPDOWNBUTTONRIGHT, (int)curState);
				}
				else
				{
					ComboBoxState curState;
					if (m_comboBox.State == ComboBoxState.Pressed)
						curState = m_isPressed ? ComboBoxState.Pressed : ComboBoxState.Normal;
					else
						curState = m_comboBox.State;
					element = VisualStyleElement.CreateElement(FwComboBox.COMBOBOX_CLASS, CP_DROPDOWNBUTTON, (int)curState);
				}
				return new VisualStyleRenderer(element);
			}
		}
