	...
	public class InnerFwTextBox : SimpleRootSite, IVwNotifyChange, IWritingSystemAndStylesheet
	{
		...
		public ITsString Tss
		{
			get
			{
				...
				tss = StringUtils.RemoveIntProp(tss, (int)FwTextPropType.ktptFontSize);
		...
		private static ITsString ReplaceAll(ITsString tss, string oldSubStr, string newSubStr)
		{
			...
			List<ITsString> lines = StringUtils.Split(tss, new[] { oldSubStr }, StringSplitOptions.None);
		...
		public override string Text
		{
			...
			{
				CheckDisposed();
				Tss = StringUtils.MakeTss(value, WritingSystemCode);
			}
		}
		...
		public string SelectedText
		{
			...
			{
				CheckDisposed();
				SelectedTss = StringUtils.MakeTss(value, WritingSystemCode);
			}
		}
