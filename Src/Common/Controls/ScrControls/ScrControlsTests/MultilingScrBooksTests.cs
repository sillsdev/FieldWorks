...
namespace SIL.FieldWorks.Common.Controls.FwControls
{
	...
	/// </summary>
	[TestFixture]
	public class MultilingScrBooksTest: BaseTest
	{
		...
		{
			Assert.AreEqual("Gen", m_mlscrBook.GetBookAbbrev(Scripture.kiOtMin));
			Assert.AreEqual("Mat", m_mlscrBook.GetBookAbbrev(Scripture.kiNtMin));
			Assert.AreEqual("Rev", m_mlscrBook.GetBookAbbrev(Scripture.kiNtMax));
		}
