...
namespace SIL.FieldWorks.Common.Controls
{
	...
	public class ScrPassageControl : System.Windows.Forms.UserControl, IMessageFilter
	{
		...
		// This is the character on drop-down button when the drop-down portion of the control
		// is not showing. The font for the button must be Marlett to get an arrow character.
		private const string kstrDownArrow = "6";
		...
		/// </summary>
		/// ------------------------------------------------------------------------------------
		public void ReloadBookNames()
		{
			if (m_mulScrBooks is DBMultilingScrBooks)
				BookLabels = m_mulScrBooks.GetBookNames(true);
		}
		...
		private void PaintNonThemeButton(Graphics graphics, ButtonState state)
		{
			...
			using (Font fnt = new Font("Marlett", 10))
			{
				using (StringFormat sf = new StringFormat())
				{
					...
					graphics.DrawString(kstrDownArrow, fnt,
						(Enabled ? SystemBrushes.ControlText : SystemBrushes.ControlDarkDark),
						btnScrPsgDropDown.ClientRectangle, sf);
				}
			}
		}
