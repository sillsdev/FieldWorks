...
namespace SIL.FieldWorks.Common.Controls
{
	...
	public class ScrPassageDropDown : DropDownContainer
	{
		...
		private bool m_snapToDefaultButton = false;
		private int m_buttonHeight = 18;
		private int m_bookButtonWidth = 100;
		private int m_cvButtonWidth = 30;
		...
		/// </summary>
		/// ------------------------------------------------------------------------------------
		public int BookButtonPreferredWidth
		{
			get
			{
				CheckDisposed();
				return m_bookButtonWidth;
			}
			set
			{
				CheckDisposed();
				m_bookButtonWidth = value;
			}
		}
		...
		/// </summary>
		/// ------------------------------------------------------------------------------------
		public int CVButtonPreferredWidth
		{
			get
			{
				CheckDisposed();
				return m_cvButtonWidth;
			}
			set
			{
				CheckDisposed();
				m_cvButtonWidth = value;
			}
		}
		...
		/// </summary>
		/// ------------------------------------------------------------------------------------
		public bool SnapToDefaultButton
		{
			get
			{
				CheckDisposed();
				return m_snapToDefaultButton;
			}
			set
			{
				CheckDisposed();
				m_snapToDefaultButton = value;
			}
		}
		...
		public int CurrentBook
		{
			get
			{
				...
				if (m_nowShowing == ListTypes.Books)
					return m_buttons[m_currButton].BCVValue;
		...
		public ScrReference CurrentScRef
		{
			...
			{
				CheckDisposed();
				return new ScrReference(m_scRef);
			}
		}
		...
		protected override void OnKeyDown(KeyEventArgs e)
		{
			...
			switch(e.KeyCode)
			{
				case Keys.Up:    buttonToGoTo = m_buttons[m_currButton].ButtonAbove; break;
				case Keys.Left:  buttonToGoTo = m_buttons[m_currButton].ButtonLeft;  break;
				case Keys.Right: buttonToGoTo = m_buttons[m_currButton].ButtonRight; break;
				case Keys.Down:
					if ((e.Modifiers & Keys.Alt) > 0)
					{
						...
						buttonToGoTo = m_buttons[m_currButton].ButtonBelow;
					break;

				case Keys.Enter:
					if (m_nowShowing == ListTypes.Verses || m_fBooksOnly)
					{
						m_scRef.Verse = m_fBooksOnly ? 1 : m_buttons[m_currButton].BCVValue;
						ScrPassageControl.ScReference = m_scRef;
						Close(false);
						return;
					}

					ButtonSelected(m_buttons[m_currButton]);
					break;
			...
			if (buttonToGoTo > -1)
			{
				m_buttons[m_currButton].ShadeWhenMouseOver = false;
		...
		/// ------------------------------------------------------------------------------------
		public void Close(bool fCancel)
		{
			CheckDisposed();
		...
		private void LoadCVButtons(List<int> list, int initialCV)
		{
			...
			ButtonEnter(m_buttons[m_currButton], null);
			m_saveMousePos = Point.Empty;
			this.ResumeLayout(false);
			...
			out int numberRows, out int buttonWidth)
		{
			...
			if (m_nowShowing == ListTypes.Books)
			{
				...
				numberRows = (int)m_rowsInBookDropDown[numberOfButtons - 1];
			}
			else
			{
				System.Diagnostics.Debug.Assert(numberOfButtons <= m_rowsInCVDropDown.Length);
				numberRows = (int)m_rowsInCVDropDown[numberOfButtons - 1];
			}
		...
		private void SetSizeAndLocationOfPopupDropDown(int width, int height)
		{
			...
			this.Size = dropDownSize;
			this.Location = dropDownLocation;
		}
		...
		{
			Point pt = new Point(m_buttons[m_currButton].Width / 2,
				m_buttons[m_currButton].Height / 2);

			Cursor.Position = m_buttons[m_currButton].PointToScreen(pt);
		}
		#endregion
		...
		private void ButtonMouseMove(object sender, MouseEventArgs e)
		{
			...
			if (button.Index != m_currButton && m_currButton > -1)
				m_buttons[m_currButton].State = ButtonState.Normal;
		...
		///  ------------------------------------------------------------------------------------
		/// <summary>
		/// Fills an array list from the cache with all the chapters for the book specified
		/// by m_scRef.Book.
		/// </summary>
		/// <returns>An array list containing all the chapters found in the cache for
		/// the book specified by m_scRef.Book.</returns>
		/// ------------------------------------------------------------------------------------
		private List<int> LoadChapterListFromCache()
		{
			List<int> chapterList = new List<int>();
			IScrBook book = ScrPassageControl.ScriptureObject.FindBook(m_scRef.Book);
		...
		private void InternalVerseSelected(ScrDropDownButton button)
		{
			...
			if (VerseSelected != null)
				VerseSelected(m_scRef.Book, m_scRef.Chapter, m_buttons[m_currButton].BCVValue);
