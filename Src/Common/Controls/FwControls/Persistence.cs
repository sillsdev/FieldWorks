...
namespace SIL.FieldWorks.Common.Controls
{
	...
	/// <see cref="Persistence"/> class. However, forms which have controls that must be
	...
	public class Persistence : Component, ISupportInitialize, IFWDisposable
	{
		...
		private const string sDefaultKeyPath = "Software\\SIL\\FieldWorks";
		private string m_DefaultKeyPath = sDefaultKeyPath;
		...
		public RegistryKey SettingsKey
		{
			get
			{
				...
				if (key == null)
				{
					// either Parent is not ISettings or returned null
					key =  Registry.CurrentUser.CreateSubKey(DefaultKeyPath);
				}
				return key;
			}
		}
		...
		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// The default key path in the registry
		/// </summary>
		/// ------------------------------------------------------------------------------------
		[DefaultValue(sDefaultKeyPath)]
		[Category("Behavior")]
		[Description("Determines the default registry key under HKCU.")]
		public string DefaultKeyPath
		{
			get
			{
				...
				return m_DefaultKeyPath;
			}
			set
			{
				CheckDisposed();

				m_DefaultKeyPath = value;
			}
		}
		...
		/// <summary>
		/// Reset DefaultKeyPath property to its default value.
		/// </summary>
		private void ResetDefaultKeyPath()
		{
			DefaultKeyPath = sDefaultKeyPath;
		}
		...
		/// application parameters if not present in registry.
		/// </summary>
		///***********************************************************************************
		...
		public virtual void EndInit()
		{
			...
			if (EnableSaveWindowSettings)
			{
				if (Parent is Form)
				{
					bool fLoadIt = true;
					if(Parent is ISettings)
						fLoadIt = !((ISettings)Parent).KeepWindowSizePos;
					if(fLoadIt)
					{
						LoadWindowPosition();
						LoadWindowState();
