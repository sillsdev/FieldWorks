...
namespace SIL.FieldWorks.Common.Controls
{
	...
	public class CharacterGrid : DataGridView, IFWDisposable
	{
		...
		{
			set { m_sortComparer = (string.IsNullOrEmpty(value) ? null : new TsStringComparer(value)); }
		}
		...
		/// </summary>
		/// ------------------------------------------------------------------------------------
		public void AddCharacters(IEnumerable<string> value, LanguageDefinition langDef)
		{
			CheckDisposed();
			Debug.Assert(!m_loadCharactersFromFont);

			foreach (string ch in value)
				InternalAddCharacter(ch, langDef);
