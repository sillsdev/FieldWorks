...
namespace SIL.FieldWorks.Common.Controls
{
	...
	public class FwColorCombo : UserControl, IFWDisposable, IInheritableFwControl
	{
		...
		/// <summary>
		/// Indicates if this combo should display the unspecified option
		/// </summary>
		protected bool m_fShowUnspecified = true;
		private bool isInheriting = false;

		/// <summary>
		/// Property indicating if this control is currently indicating the use of an inherited value.
		/// </summary>
		public bool IsInherited
		{
			get { return isInheriting; }
			set { isInheriting = value; }
		}
		...
		{
			//if the button is not available it isn't possible to represent an inherited value
			//when we first set things up to show the unspecified button, we usually want to initialize the
			//control to represent an inherited value so these are tied together in this setter.
			set { isInheriting = m_fShowUnspecified = value; }
		}
		...
		private void dropDown_ColorPicked(object sender, EventArgs e)
		{
			...
			IsInherited = dropDown.CurrentColor.IsEmpty;
