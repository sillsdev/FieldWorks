...
//			Debug.WriteLine("Creating object:" + objName);
		...
		/// if possible, otherwise, the root object.
		/// </summary>
		/// <returns></returns>
		private ICmObject DescendantForSlice(Slice slice)
		{
			...
			Slice ancestorSlice = slice.ParentSlice;
			while (ancestorSlice.ParentSlice != null && !ancestorSlice.IsHeaderNode)
				ancestorSlice = ancestorSlice.ParentSlice;
			// otherwise, the parent slice's object is the descendant
			return ancestorSlice.IsHeaderNode ? ancestorSlice.Object : m_root;
		}
