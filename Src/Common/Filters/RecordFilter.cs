...
namespace SIL.FieldWorks.Filters
{
	...
	/// </summary>
	/// ----------------------------------------------------------------------------------------
	public abstract class RecordFilter : IPersistAsXml, IStoresFdoCache, IAcceptsStringTable
	{
		...
		{
			IStoresFdoCache sfc = obj as IStoresFdoCache;
			if (sfc != null)
				sfc.Cache = cache;
		}
		...
		{
			IAcceptsStringTable target = obj as IAcceptsStringTable;
			if (target != null)
		...
		{
			RecordFilter filter = (RecordFilter)DynamicLoader.CreateObject(configuration);
	...
	public class ProblemAnnotationFilter: RecordFilter
	{
		...
		public override void Init(FdoCache cache,XmlNode filterNode)
		{
			...
			{
				uint cls = cache.MetaDataCacheAccessor.GetClassId(name.Trim());
				if (cls <= 0)
					throw new ConfigurationException("The class name '" + name + "' is not valid");
				m_classIds.Add((int)cls);
			}
		}
	...
	public abstract class BaseMatcher : IMatcher, IPersistAsXml, IStoresFdoCache
	{
		...
		public virtual void PersistAsXml(XmlNode node)
		{
			if (m_tssLabel != null)
			{
				ITsStreamWrapper strmWrap = TsStreamWrapperClass.Create();
				strmWrap.WriteTssAsXml(m_tssLabel, m_wsf, 0, 0, false);
				XmlUtils.AppendAttribute(node, "label", strmWrap.Contents);
			}
		}
		...
		public virtual FdoCache Cache
		{
			set
			{
				m_wsf = value.LanguageWritingSystemFactoryAccessor;
				if (m_xmlLabel != null)
				{
					ITsStreamWrapper strmWrap = TsStreamWrapperClass.Create();
					strmWrap.Contents = m_xmlLabel;
					m_tssLabel = strmWrap.ReadTssFromXml(m_wsf);
				}
			}
		}
	...
	public abstract class SimpleStringMatcher : BaseMatcher
	{
		...
		/// </summary>
		/// <returns></returns>
		abstract protected bool CurrentResultDoesMatch();

		/// <summary></summary>
		/// <param name="fAllMatches"></param>
		/// <returns></returns>
		public List<MatchRangePair> GetAllResults()
		{
			MatchRangePair mrpOld = m_currentMatchRangePair;
			while (CurrentResultDoesMatch())
			{
				FindNextMatch();
				if (m_currentMatchRangePair.IchMin == mrpOld.IchMin &&
					m_currentMatchRangePair.IchLim == mrpOld.IchLim)
				{
					break;			// Prevent cycles, eg, for Reg Exp "$" (LT-7041).
				}
				mrpOld = m_currentMatchRangePair;
			}
			return m_results;
		}
		...
		public override FdoCache Cache
		{
			set
			{
				...
				if(m_persistNode != null && m_pattern.Pattern == null)
				{
					...
					int ws = XmlUtils.GetOptionalIntegerValue(m_persistNode, "ws", value.DefaultAnalWs);
	...
	public class ExactMatcher : SimpleStringMatcher
	{
		...
		protected override bool CurrentResultDoesMatch()
		{
			return m_currentMatchRangePair.IchMin == 0 && m_currentMatchRangePair.IchLim == m_tssSource.Length;
		}
	}
	...
	public class EndMatcher : SimpleStringMatcher
	{
		...
		protected override bool CurrentResultDoesMatch()
		{
			return m_currentMatchRangePair.IchLim == m_tssSource.Length;
		}
	}
	...
	/// </summary>
	public class InvertMatcher : BaseMatcher, IAcceptsStringTable, IStoresFdoCache
	{
		IMatcher m_matcher;
	...
	public interface IStringFinder
	{
		...
		/// <param name="item">The item.</param>
		/// <returns></returns>
		string[] Strings(ManyOnePathSortItem item, bool sortedFromEnd);
		string[] SortStrings(ManyOnePathSortItem item, bool sortedFromEnd); // similar key more suitable for sorting.
		ITsString Key(ManyOnePathSortItem item);
		...
		void CollectItems(ICmObject obj, ArrayList collector);
	...
	public class AndFilter : RecordFilter
	{
		...
		public override void InitXml(XmlNode node)
		{
			...
			foreach (XmlNode child in node.ChildNodes)
				m_filters.Add(DynamicLoader.RestoreFromChild(child, "."));
		}
