...
namespace SIL.FieldWorks.Common.PrintLayout
{
	...
	/// </summary>
	/// ----------------------------------------------------------------------------------------
	public class DivisionLayoutMgr : IVwLayoutManager, /* IRootSite, */ IFWDisposable
	{
		...
		/// <summary></summary>
		protected PublicationControl m_publication;
		/// <summary></summary>
		...
		/// <summary></summary>
		protected System.UInt32 m_Hwnd;
		/// <summary>
		...
		private LgWritingSystem m_MainStreamWs;
		...
		internal bool LayoutPage(Page page)
		{
			...
			using (new WaitCursor(m_publication))
			{
				int offsetFromTopOfDiv = page.OffsetFromTopOfDiv(this);
				int ysStartNextPage = 0;
				...
				else
				{
					...
					for (Page nextPage = Publication.PageAfter(page); nextPage != null; )
					{
						...
						if (nextPage.FirstDivOnPage < m_publication.Divisions.Count &&
							m_publication.Divisions[nextPage.FirstDivOnPage] != this)
							break;
		...
		private void AddHeaderAndFooter(Page page, int xpLeftMargin)
		{
			...
			{
				int dypHdrHeight;
				IVwLayoutStream hdrStream = CreateHeaderOrFooter(hfconfig.MakeHeaderVc(page),
					hvoHdrRoot, xpLeftMargin, m_dympHeaderPos, out dypHdrHeight);

				PublicationControl.SetAccessibleStreamName(hdrStream,
					m_publication.AccessibleName + "_Header");
			...
			{
				int dypFtrHeight;
				IVwLayoutStream ftrStream = CreateHeaderOrFooter(hfconfig.MakeFooterVc(page),
					hvoFtrRoot, xpLeftMargin, m_dympFooterPos, out dypFtrHeight);

				PublicationControl.SetAccessibleStreamName(ftrStream,
					m_publication.AccessibleName + "_Footer");
		...
		public virtual void PageBroken(IVwLayoutStream lay, int hPage)
		{
			...
			Page page = m_publication.FindPage(hPage);
			if (page == null)
			{
		...
		protected int GetEstimatedHeight(int dxpWidth)
		{
			...
			IVwGraphics vg = m_publication.PrinterGraphics;
			MainRootBox.Layout(vg, dxpWidth);
			m_publication.ReleaseGraphics(vg);
		...
		protected internal void Configure()
		{
			...
			// markers)
			rootbox.RequestObjCharDeleteNotification(m_publication.EditingHelper);
			...
			ISubordinateStreamDelegate subStreamDelegate)
		{
			...
			PublicationControl.SetAccessibleStreamName(subStream.m_stream,
				m_publication.AccessibleName + "_SubordinateStream");
		...
		{
			get { return 5000 + m_publication.Publication.FootnoteSepThickness; }
		}
		...
		/// </remarks>
		/// ------------------------------------------------------------------------------------
		internal PublicationControl Publication
		{
			get
			{
				CheckDisposed();
		...
		public int AvailablePageHeightInPrinterPixels
		{
			get
			{
				...
				return (m_publication.PageHeightInPrinterPixels - TopMarginInPrinterPixels -
		...
		public int AvailablePageWidthInPrinterPixels
		{
			...
			{
				CheckDisposed();
				return m_publication.PageWidthInPrinterPixels - InsideMarginInPrinterPixels -
		...
		protected internal virtual bool MainStreamIsRightToLeft
		{
			get
			{
				if (m_MainStreamWs == null)
				{
					...
					m_MainStreamWs = new LgWritingSystem(Publication.Cache, ws);
				}
				return m_MainStreamWs.RightToLeft;
			}
		}
		...
		public int LeftMarginInPrinterPixels(int pageNumber)
		{
			...
			if (pageNumber % 2 == 1 || m_publication.PageLayoutMode == MultiPageLayout.Simplex)
			{
				return m_publication.IsLeftBound ? InsideMarginInPrinterPixels :
					OutsideMarginInPrinterPixels;
			}
			else
			{
				return m_publication.IsLeftBound ? OutsideMarginInPrinterPixels :
		...
		public void ScrollSelectionToLocation(IVwSelection sel, int dyPos)
		{
			...
			Publication.ScrollSelectionToLocation(sel, dyPos);
		}
			...
			ref int dysAvailHeight)
		{
			...
			if (dependentRootbox == null)
			{
				...
				dependentRootbox.DataAccess = m_configurer.DataAccess;
				(sda as IVwOleDbDa).CreateDummyID(out hvoRoot);
				// Cache a class for the dummy object consistent with the specified tag. If we
				// fail to do this, a Reconstruct call on the root object will consider the
				// dummy object as invalid, and change it to zero, with disastrous results.
				uint classId =
					m_configurer.DataAccess.MetaDataCache.GetOwnClsId((uint)dependentObjTag);
				(sda as IVwCacheDa).CacheIntProp(hvoRoot,
					(int)CmObjectFields.kflidCmObject_Class, (int)classId);
				dependentRootbox.SetRootObject(hvoRoot, m_configurer.DependentRootVc,
					dependentObjFrag, m_configurer.StyleSheet);
				(sda as IVwCacheDa).CacheVecProp(hvoRoot, dependentObjTag, rgHvo, rgHvo.Length);
			}
			else
			{
				...
				IVwStylesheet ss;
				dependentRootbox.GetRootObject(out hvoRoot, out vc, out dependentObjFrag,
					out ss);
				chvoPresent = sda.get_VecSize(hvoRoot, dependentObjTag);
				// append new objects.
				try
				{
					// propChanged generates a PageBroken notification we need to ignore.
					m_hPageBeingBuilt = hPage;
					(sda as IVwCacheDa).CacheReplace(hvoRoot, dependentObjTag, chvoPresent,
						chvoPresent, rgHvo, rgHvo.Length);
					sda.PropChanged(null, (int)PropChangeType.kpctNotifyAll, hvoRoot,
						dependentObjTag, chvoPresent, rgHvo.Length, 0);
				}
				finally
				{
					m_hPageBeingBuilt = 0;
			...
			for (int ihvo = 0; ihvo < rgHvo.Length; ihvo++)
			{
				...
				rgSelLevInfo[0].ihvo = ihvo + chvoPresent;
			...
			if ((dysHeightUsed * m_numberMainStreamColumns) > dysAvailHeight)
			{
				...
				if (fAllowFail)
				{
					...
					{
						(sda as IVwCacheDa).CacheReplace(hvoRoot, dependentObjTag, chvoPresent,
							chvoPresent + rgHvo.Length, new int[0], 0);
						sda.PropChanged(null, (int)PropChangeType.kpctNotifyAll, hvoRoot,
							dependentObjTag, chvoPresent, 0, rgHvo.Length);
					}
					finally
					{
						m_hPageBeingBuilt = 0;
			...
			if (newHeight > 0)
			{
				AddOrAdjustPageElement(page, ysTopOfPrevElement, newHeight,
					dependentStream, 0);
			}
			dysAvailHeight = dysAvailableHeight;
		}
