...
namespace SIL.FieldWorks.Common.PrintLayout
{
	...
	/// </summary>
	/// ----------------------------------------------------------------------------------------
	public class DummyFirstSubViewVc : VwBaseVc
	{
		/// <summary></summary>
	...
	public class DummyPrintConfigurer : IPrintLayoutConfigurer
	{
		...
		private IVwViewConstructor m_DependentRootVc;
		...
		public virtual int MainObjectId
		{
			get
			{
				...
				SqlConnection sqlConMaster = new SqlConnection(string.Format(@"Server=.\SILFW; " +
					"Database={0}; User ID = sa; Password=inscrutable; Pooling=false;",
					m_fdoCache.DatabaseName));
				sqlConMaster.Open();
				...
				try
				{
					string sectionStartRef;
					switch (m_iDivision)
					{
						case 0:
						default:
							sectionStartRef = "59002014";
							break;
						case 1:
							sectionStartRef = "59003001";
							break;
						case 2:
							sectionStartRef = "59003013";
							break;
						case 3:
							sectionStartRef = "59004001";
							break;
					}
					SqlCommand sqlComm = sqlConMaster.CreateCommand();
					// Select the hvo of the StText of the contents of the desired section.
					string sSql = @"select dst from scrsection_content c " +
						@"join scrSection s on c.src = s.id " +
						@"where s.VerseRefStart = " + sectionStartRef;
					sqlComm.CommandText = sSql;
					sqlreader = sqlComm.ExecuteReader(System.Data.CommandBehavior.SingleResult);
					if (sqlreader.Read())
					{
						Assert.IsFalse(sqlreader.IsDBNull(0));
						hvoContents = sqlreader.GetInt32(0);
					}
					sqlreader.Close();
				}
				finally
	...
	public class DummyPublication : PublicationControl
	{
		...
		public DummyPublication(IPublication pub, FwStyleSheet stylesheet,
			...
			: base(div, stylesheet, pub, printDateTime, fApplyStyleOverrides, false)
		{
	...
	/// ----------------------------------------------------------------------------------------
	[TestFixture]
	public class PrintLayoutTests: BaseTest
	{
		...
		public void CreateAndPrepareToDrawPublicationPages()
		{
			...
			int topOfPage2 = page2.OffsetFromTopOfDiv(m_firstDivision);
		...
		public void FootnoteLayout()
		{
			...
			List<Page> pagesToBeDrawn = m_pub.PrepareToDrawPages(0, 2000);
			...
			Assert.AreEqual(4, cFootnotes, "All footnotes should have gotten laid out.");
		}
		...
		public void CalcPageElementWindowRectangle()
		{
			...
			PageElement element = new PageElement(m_firstDivision, null, false, new Rectangle(720, 1440,
		...
		public void TopOfPageSelection()
		{
			...
			Assert.AreEqual((int)StText.StTextTags.kflidParagraphs, info0[0].tag, "Wrong tag");
			...
			Assert.AreEqual((int)StText.StTextTags.kflidParagraphs, info1[0].tag, "Wrong tag");
		...
		public void BottomOfPageSelection()
		{
			...
			Assert.AreEqual((int)StText.StTextTags.kflidParagraphs, infoBtttom0.tag, "Wrong tag");
			...
			Assert.AreEqual((int)StText.StTextTags.kflidParagraphs, infoBottom1.tag, "Wrong tag");
		...
		public void EditPageBroken()
		{
			...
			int wsPending = -1;
			rootb.OnTyping(vg, "abc", 0, 0, 0, ref wsPending);
			...
			IScrBook james = m_fdoCache.LangProject.TranslatedScriptureOA.ScriptureBooksOS[rgsli[1].ihvo];
			...
			footnotes.OnTyping(vg, "a", 0, 0, 0, ref wsPending);
			m_pub.ReleaseGraphics(vg);
			...
			rgsli2[0].tag = (int)StText.StTextTags.kflidParagraphs;
		...
		public void EditPageBroken_TwoColumns()
		{
			...
			rootb.OnTyping(vg, "a", 0, 0, 0, ref wsPending);
			m_pub.ReleaseGraphics(vg);
			int nInitialPagesBroken = m_firstDivision.m_hPagesBroken.Count;
			...
			vg = m_pub.ScreenGraphics;
			rootb.OnTyping(vg, "\r", 0, 0, 0, ref wsPending);
			m_pub.ReleaseGraphics(vg);
			Assert.AreEqual(m_pub.Pages.Count, m_firstDivision.m_hPagesBroken.Count,
			...
			vg = m_pub.ScreenGraphics;
			rootb.OnTyping(vg, "abc\r", 0, 0, 0, ref wsPending);
			m_pub.ReleaseGraphics(vg);
			...
			// and one to join it to the previous paragraph.
			vg = m_pub.ScreenGraphics;
			rootb.OnTyping(vg, "", 2, 0, 0, ref wsPending);
			m_pub.ReleaseGraphics(vg);
			Assert.AreEqual(m_pub.Pages.Count - 1, m_firstDivision.m_hPagesBroken.Count,
		...
		public void EditPageBroken_TwoColumns_InsertInFootnote()
		{
			...
			rgsli[0].hvo = james.FootnotesOS[rgsli[0].ihvo].Hvo;
			rgsli[0].tag = (int)ScrBook.ScrBookTags.kflidFootnotes;
			IVwSelection sel = footnotes.MakeTextSelInObj(0, 2, rgsli, 0, null, false, true, false, false, true);
			footnotes.OnTyping(vg, "a", 0, 0, 0, ref wsPending);
			m_pub.ReleaseGraphics(vg);
		...
		public void Pub_ThreeDivisions_EachDivStartsNewPage()
		{
			DummyDivision secondDiv = new DummyDivision(
				new DummyPrintConfigurer(m_fdoCache, m_subStream), 2);
			DummyDivision thirdDiv = new DummyDivision(
				new DummyPrintConfigurer(m_fdoCache, m_subStream), 1);
			...
			Assert.AreEqual(0, m_pub.IndexOfPage(firstPage));
			Page lastPage = m_pub.Pages[2];
			Assert.AreEqual(thirdDiv, m_pub.Divisions[lastPage.FirstDivOnPage]);
			Assert.AreEqual(2, m_pub.IndexOfPage(lastPage),
				"IndexOfPage returned wrong value for last page");
			...
			Assert.AreEqual(m_pub.Pages[1], extraPage);
			Assert.AreEqual(m_pub.Pages[2], origPage2);
			Assert.AreEqual(m_pub.Pages[3], lastPage);
			Assert.AreEqual(firstPage, m_pub.FindPage(firstPage.Handle));
			Assert.AreEqual(extraPage, m_pub.FindPage(extraPage.Handle));
			Assert.AreEqual(origPage2, m_pub.FindPage(origPage2.Handle));
			Assert.AreEqual(lastPage, m_pub.FindPage(lastPage.Handle));
		...
		public void Pub_ThreeDivisions_Continuous_WithSeparateFootnoteStreams()
		{
			...
			DummyDivision secondDiv = new DummyDivision(new DummyPrintConfigurer(m_fdoCache, null), 1);
			secondDiv.StartAt = DivisionStartOption.Continuous;
			DummyDivision thirdDiv = new DummyDivision(new DummyPrintConfigurer(m_fdoCache, null), 1);
			...
			Assert.AreEqual(secondDiv, m_pub.Divisions[secondPage.FirstDivOnPage]);
		...
		public void Pub_ThreeDivisions_Continuous_SharedFootnoteStream_RepeatedOrcs()
		{
			...
			thirdDiv.OutsideMargin = secondDiv.OutsideMargin = m_firstDivision.OutsideMargin = 36000; // 1/2 inch
			m_pub.CreatePages();
			Assert.AreEqual(1, m_pub.Pages.Count); // this is just an estimate
			...
			Page secondPage = m_pub.PageAfter(firstPage);
			Assert.AreEqual(m_pub.Pages[1], secondPage);
			Assert.AreEqual(4, firstPage.PageElements.Count, "Should have one element per division, plus one for the footnotes");
			Assert.AreEqual(2, secondPage.PageElements.Count);
			Assert.AreEqual(m_firstDivision, m_pub.Divisions[firstPage.FirstDivOnPage]);
			Assert.AreEqual(thirdDiv, m_pub.Divisions[secondPage.FirstDivOnPage]);
		...
		public void Pub_ThreeDivisions_Continuous_SharedFootnoteStream()
		{
			m_firstDivision.StartAt = DivisionStartOption.Continuous;
			DummyDivision secondDiv = new DummyDivision(
				new DummyPrintConfigurer(m_fdoCache, m_subStream, 1), 1);
			secondDiv.StartAt = DivisionStartOption.Continuous;
			DummyDivision thirdDiv = new DummyDivision(
				new DummyPrintConfigurer(m_fdoCache, m_subStream, 2), 1);
			thirdDiv.StartAt = DivisionStartOption.Continuous;
			m_pub.AddDivision(secondDiv);
			m_pub.AddDivision(thirdDiv);
			...
			Page secondPage = m_pub.PageAfter(firstPage);
			Assert.AreEqual(m_pub.Pages[1], secondPage);
			Assert.AreEqual(3, firstPage.PageElements.Count, "Should have one element per division, plus one for the footnotes");
		...
		public void Pub_MixedColumnDivisions_AlternatingContinuousAndPageBreak()
		{
			...
			m_pub.AddDivision(new DummyDivision(new DummyPrintConfigurer(m_fdoCache, m_subStream, 1), 2));
			m_pub.AddDivision(new DummyDivision(new DummyPrintConfigurer(m_fdoCache, m_subStream, 2), 1));
			m_pub.AddDivision(new DummyDivision(new DummyPrintConfigurer(m_fdoCache, m_subStream, 3), 2));
			...
			m_pub.CreatePages();
			Assert.AreEqual(2, m_pub.Pages.Count); // this is just an estimate
			...
			Page secondPage = m_pub.PageAfter(firstPage);
			Assert.AreEqual(m_pub.Pages[1], secondPage);
			Assert.AreEqual(4, firstPage.PageElements.Count,
				"Should have one element per 1-column division, two for each 2-column division, plus one for the footnotes");
			Assert.AreEqual(4, secondPage.PageElements.Count,
				"Should have one element per 1-column division, two for each 2-column division, plus one for the footnotes");
			Assert.AreEqual(0, firstPage.FirstDivOnPage);
			Assert.AreEqual(2, secondPage.FirstDivOnPage);
			foreach (Page page in m_pub.Pages)
			{
		...
		public void CreatePages_MultiDivContinuous()
		{
			...
			Assert.Greater(m_pub.Pages[1].OffsetFromTopOfDiv(secondDiv), 0);
		}
