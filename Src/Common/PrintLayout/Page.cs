...
namespace SIL.FieldWorks.Common.PrintLayout
{
	...
	public class Page : IPageInfo, IFWDisposable
	{
		...
		internal void LayOut()
		{
			...
			if (!TryToReuseDependentRootStream(out cDepRoot))
			{
				...
				else
				{
					...
					IVwStylesheet ss;
					(m_dependentObjectRootStream as IVwRootBox).GetRootObject(out hvoRoot, out vc, out dependentObjFrag, out ss);
					ISilDataAccess sda = div.Configurer.DataAccess;
					(sda as IVwCacheDa).CacheVecProp(hvoRoot, dependentObjTag, new int[0], 0);
					sda.PropChanged(null, (int)PropChangeType.kpctNotifyAll, hvoRoot, dependentObjTag, 0, 0, cDepRoot);
				}
				LayoutPageCore(0);
				if (m_dependentObjectRootStream != null)
				{
					m_dependentObjectStreamHeight = (m_dependentObjectRootStream as IVwRootBox).Height;
					if (helper != null)
					{
						try
						{
							// See if we can restore the selection; don't crash if we fail.
							helper.MakeRangeSelection(m_dependentObjectRootStream as IVwRootBox, true);
						}
						catch (Exception)
						{
						}
		...
		private bool TryToReuseDependentRootStream(out int cDepRoot)
		{
			...
			ISilDataAccess sda = div.Configurer.DataAccess;
			int chvoPresent = sda.get_VecSize(hvoRoot, dependentObjTag);
			List<PageElement> oldElements = m_pageElements;
			...
			for (int i = 0; i < chvoPresent; i++)
			{
				oldDependentRoots[i] = sda.get_VecItem(hvoRoot, dependentObjTag, i);
			}
...
//			m_fReadyForDrawing = false;
			...
			Rectangle rectElement, Rectangle rectDst)
		{
			...
			int lineHeight = PubControl.Publication.FootnoteSepThickness * rectDst.Height / MiscUtils.kdzmpInch;
