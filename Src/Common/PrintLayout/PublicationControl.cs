...
namespace SIL.FieldWorks.Common.PrintLayout
{
	...
	/// </summary>
	/// ----------------------------------------------------------------------------------------
	public abstract class PublicationControl : UserControl, IFWDisposable,
		ISelectableView, IEditingCallbacks, IRootSite, IVwRootSite, IPublicationView,
		IReceiveSequentialMessages
	{
		...
		/// it's information from (or null to keep the defaults)</param>
		/// <param name="fApplyStyleOverrides">if set to <c>true</c>, apply style overrides.
		/// </param>
		/// <param name="fUniformLineSpacing">if set to <c>true</c> apply uniform line spacing,
		/// rather than proportional spacing.</param>
		/// -----------------------------------------------------------------------------------
		protected PublicationControl(DivisionLayoutMgr divLayoutMgr, FwStyleSheet stylesheet,
			...
			bool fUniformLineSpacing) : this(stylesheet, publication, printDateTime,
			fApplyStyleOverrides, fUniformLineSpacing)
		{
		...
		{
			m_fDisposing = true; // Added for TE-5494 because the lock did not seem to be working.
		...
		protected override void OnMouseDown(MouseEventArgs e)
		{
			...
			//			CommitIfWord(new WordEventArgs(EditingHelper.WordEventSource.MouseClick));
		...
		protected override void OnDoubleClick(EventArgs e)
		{
			...
			// Convert to box coords and pass to root box (if any).
			if (DataUpdateMonitor.IsUpdateInProgress(FocusedRootBox.DataAccess))
		...
		protected override void OnKeyDown(KeyEventArgs e)
		{
			...
			IVwGraphics screenGraphics = ScreenGraphics;
			try
			{
				EditingHelper.OnKeyDown(e, screenGraphics);
			}
			finally
			{
				ReleaseGraphics(screenGraphics);
			}
		}
		...
		protected override void OnKeyPress(KeyPressEventArgs e)
		{
			...
			try
			{
				EditingHelper.OnKeyPress(e, ModifierKeys, screenGraphics);
			}
			finally
			{
				ReleaseGraphics(screenGraphics);
		...
		{
			base.OnPaint (e);
			if (!FindForm().Enabled || m_fDisposing || !AllowPainting)
				return;
		...
		public bool PrintPage(PrintPageEventArgs e, int pageIndex)
		{
			...
			lock (m_pages)
			{
				...
				LayoutToPageIfNeeded(pageIndex, e.Graphics.DpiX, e.Graphics.DpiY);
				...
				Rectangle rect = new Rectangle((e.PageBounds.Left * (int)e.Graphics.DpiX) / 100,
					(e.PageBounds.Top * (int)e.Graphics.DpiY) / 100,
					(e.PageBounds.Width * (int)e.Graphics.DpiX) / 100,
					(e.PageBounds.Height * (int)e.Graphics.DpiY) / 100);
		...
		public EditingHelper EditingHelper
		{
			get
			{
				...
				if (m_editingHelper == null)
					m_editingHelper = new PubEditingHelper(GetInternalEditingHelper(), this);
		...
		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Gets a value indicating whether to cancel the current loop.
		/// </summary>
		/// ------------------------------------------------------------------------------------
		protected bool Cancel
		{
			get { return m_fCancelPageLayout; }
		}
		...
		/// ------------------------------------------------------------------------------------
		protected virtual EditingHelper GetInternalEditingHelper()
		{
			return null;
		}
		#endregion
...
//					page.ReadyForDrawing = true;
		...
		protected internal Page PageFromPrinterY(int x, int y, bool fUp, IVwLayoutStream strm,
			out int dyPageScreen, out int iDiv)
		{
		...
		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Gets a <see cref="Page"/> object from a handle.
		/// </summary>
...
//			}
		...
		{
			ShowHelp.ShowHelpTopic(FwApp.App, "khtpPrint");
		}
		...
		public virtual bool OnViewUpdatePageBreak(object args)
		{
			...
			// Pre-load all the data we are going to need to do this
			//			CmObject.LoadDataForFlids(typeof(ScrSection), m_cache,
			//				new int[] { (int)Scripture.ScriptureTags.kflidScriptureBooks},
			//				ScrBook.FullViewName);
			//			CmObject.LoadDataForFlids(typeof(StTxtPara), m_cache,
			//				new int[] { (int)ScrBook.ScrBookTags.kflidFootnotes,
			//				(int)ScrBook.ScrBookTags.kflidTitle, (int)ScrSection.ScrSectionTags.kflidHeading,
			//				(int)ScrSection.ScrSectionTags.kflidContent}, StText.FullViewName);
			IVwRootBox focusedRootBox = FocusedRootBox;
			using (ProgressDialogWithTask dialog = new ProgressDialogWithTask(FindForm()))
			{
				m_fCancelPageLayout = false;
				...
				dialog.Cancel += new CancelHandler(OnCancelUpdatePageBreaks);

				dialog.RunTask(true, new BackgroundTaskInvoker(UpdatePageBreaks),
					DpiXPrinter, DpiYPrinter);
			}
		...
		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Called to stop updating the page breaks.
		/// </summary>
		/// <param name="sender">The sender.</param>
		/// ------------------------------------------------------------------------------------
		private void OnCancelUpdatePageBreaks(object sender)
		{
			m_fCancelPageLayout = true;
		}
		...
		/// ------------------------------------------------------------------------------------
		private object UpdatePageBreaks(IAdvInd4 dialog, params object[] parameters)
		{
			Debug.Assert(parameters.Length == 2);
			float dpiX = (float)parameters[0];
			float dpiY = (float)parameters[1];
			while (LayoutToPageIfNeeded(m_pages.Count - 1, dpiX, dpiY, dialog) && !Cancel)
				;
			return null;
		}
		...
		private void Print(PrintDocument pd)
		{
			try
			{
				...
				pd.PrintPage += new PrintPageEventHandler(this.PrintLayout_PrintPage);
				pd.Print();
			}
			catch (Exception e)
			{
				...
				Control msgParent = this;
				while (msgParent != null && !(msgParent is Form))
					msgParent = msgParent.Parent;
				MessageBox.Show((Form)msgParent, errorMsg, "", MessageBoxButtons.OK, MessageBoxIcon.Error);
			}
		}
		...
		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Commits outstanding changes in focused division
		/// </summary>
		/// ------------------------------------------------------------------------------------
		public void Commit()
		{
			...
			IVwRootBox rootbox = FocusedRootBox;
			if (rootbox != null)
			{
				IVwSelection sel = rootbox.Selection;
				if (sel != null)
				{
					DataUpdateMonitor.Commit(sel, rootbox.DataAccess);
				}
			}
		}
		...
		/// Refreshes the display :)
		/// </summary>
		/// <param name="fSaveSelection">True to save the selection and restore it afterwards,
		/// false otherwise</param>
		/// ------------------------------------------------------------------------------------
		protected void RefreshDisplay(bool fSaveSelection)
		{
			if (m_pages.Count == 0)
				return;
			...
			SelectionHelper selHelper = (fSaveSelection) ? SelectionHelper.Create(this) : null;
		...
		public void ScrollSelectionToLocation(IVwSelection sel, int dyPos)
		{
			...
			IVwRootBox rootBox = EditedRootBox;
			if (sel == null || rootBox == null)
				return;

			// not working yet... (see GetCoordRects...)
//			InitScreenGraphics();
//			try
//			{
...
//				GetCoordRects(out rcSrcRoot, out rcDstRoot);
//				rcDstRoot.top -= ClientRectangle.Height;
//				rcDstRoot.bottom -= ClientRectangle.Height;
//				rootBox.PrepareToDraw(m_screenVwGraphics, rcSrcRoot, rcDstRoot);
//
...
//				bool fEndBeforeAnchor;
//				GetCoordRects(out rcSrcRoot, out rcDstRoot);
//				sel.Location(m_screenVwGraphics, rcSrcRoot, rcDstRoot, out rcPrimary,
...
//			}
		...
		public void GetTransformAtSrc(IVwRootBox root, Point pt, out Rect rcSrcRoot, out Rect rcDstRoot)
		{
			...
			int iDiv;
			Page page = PageFromPrinterY(pt.X, pt.Y, false, strm, out dyPageScreen, out iDiv);
			if (page == null)
		...
		/// ------------------------------------------------------------------------------------
		public void ScrollSelectionIntoView(IVwSelection sel, VwScrollSelOpts ssoFlag)
		{
			CheckDisposed();
			...
			try
			{
				...
				for (; ; )
				{
					...
					Page pageTop = PageFromPrinterY(rcSelPrinter.Left, rcSelPrinter.Top, false, stream,
						out dysPageTopScreen, out iDivTop);
					...
					Page pageBottom = PageFromPrinterY(rcSelPrinter.Right, rcSelPrinter.Bottom, true, stream,
						out dysPageBottomScreen, out iDivBottom);
		...
		{
			CheckDisposed();
			return EditingHelper.TextRepOfObj(m_cache, guid);
		}
		...
		{
			CheckDisposed();
			return EditingHelper.MakeObjFromText(m_cache, bstrText, selDst, out kodt);
		}
		...
		public void SelectionChanged(IVwRootBox rootb, IVwSelection vwselNew)
		{
			...
			EditingHelper.SelectionChanged(rootb, vwselNew);
		}
		#endregion
