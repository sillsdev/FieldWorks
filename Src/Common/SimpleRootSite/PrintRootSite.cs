...
namespace SIL.FieldWorks.Common.RootSites
{
	...
	/// </summary>
	/// ----------------------------------------------------------------------------------------
	public class PrintRootSite : IVwRootSite
	{
		[DllImport("gdi32")]
		...
		public void Print(PrintDocument pd)
		{
			...
			long x1 = System.DateTime.Now.Ticks;
#endif
			// Try to reduce the impact of reading all the data from the database in pd.Print()
			// below.
			AutoloadPolicies alp = AutoloadPolicies.kalpLoadAllOfClassIncludingAllVirtuals;
			if (m_sda is IVwOleDbDa)
			{
				alp = (m_sda as IVwOleDbDa).AutoloadPolicy;
				(m_sda as IVwOleDbDa).AutoloadPolicy = AutoloadPolicies.kalpLoadAllOfClassIncludingAllVirtuals;
			}
			try
			{
			...
			{
				if (alp != AutoloadPolicies.kalpLoadAllOfClassIncludingAllVirtuals)
					(m_sda as IVwOleDbDa).AutoloadPolicy = alp;
			}
#if false
			long x2 = System.DateTime.Now.Ticks;
		...
		private void SetMargins(PrintPageEventArgs e)
		{
			...
			// Get the top and left unprintable areas. For many printers that's a value
			// between 0.25 and 0.5 inches.
			IntPtr hdc = e.Graphics.GetHdc();
			int unprintableLeft = GetDeviceCaps(hdc, PHYSICALOFFSETX);
			int unprintableTop = GetDeviceCaps(hdc, PHYSICALOFFSETY);
			e.Graphics.ReleaseHdc(hdc);

			// To be useful, the printable rectangle needs to be offset so it indicates
			// the actual part of the page where we can print. The offsets we got above
			// are in pixels, convert to 100ths of an inch like the other coords we have here.
			printable.Offset(unprintableLeft * 100 / dpiX, unprintableTop * 100 / dpiY);

			m_vwPrintContext.SetMargins(
				PixelsFrom100ths(margins.Left - printable.Left, dpiX),
				PixelsFrom100ths(printable.Right - margins.Right, dpiX),
				PixelsFrom100ths((margins.Top - printable.Top) / 2, dpiY),		// heading; get from smarter page setup?
				PixelsFrom100ths(margins.Top - printable.Top, dpiY),			// top
				PixelsFrom100ths(printable.Bottom - margins.Bottom, dpiY),		// bottom
				PixelsFrom100ths((printable.Bottom - margins.Bottom) / 2, dpiY));	// footer; get from smarter page setup?
		}
		...
		/// ------------------------------------------------------------------------------------
		void IVwRootSite.ScrollSelectionIntoView(IVwSelection sel,
