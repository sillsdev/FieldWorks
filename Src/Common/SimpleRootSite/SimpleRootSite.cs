...
namespace SIL.FieldWorks.Common.RootSites
{
	...
	/// ----------------------------------------------------------------------------------------
	/// <summary>
	/// Base class for hosting a FieldWorks view in an application.
	/// </summary>
	/// ----------------------------------------------------------------------------------------
	public class SimpleRootSite : UserControl, IVwRootSite, IRootSite, ISelectionChangeNotifier,
		IxCoreColleague, IEditingCallbacks, IReceiveSequentialMessages, IMessageFilter,
		IFWDisposable /* System.Windows.Forms.UserControl inherits from IDisposable */
	{
		...
		/// that situation by the fact that, in OnLostFocus, this != g_focusRootSite.
		/// </summary>
		private static SimpleRootSite g_focusRootSite;
		...
		// Used to save and restore a selection
		private SelectionHelper m_savedSelection = null;
		...
		public virtual EditingHelper EditingHelper
		{
			get
			{
				CheckDisposed();
				if (m_editingHelper == null)
					m_editingHelper = new EditingHelper(this);
		...
		/// <returns></returns>
		/// -----------------------------------------------------------------------------------
		public virtual bool AdjustScrollRange(IVwRootBox prootb, int dxdSize, int dxdPosition,
		...
		{
			CheckDisposed();
			EditingHelper.SelectionChanged(prootb, vwselNew);
			// Allow special handling when the selection changes.  Motivated by LT-8393.
			if (VwSelectionChanged != null)
				VwSelectionChanged.Invoke(this, new VwSelectionArgs(prootb, vwselNew));
		}
		...
		/// ------------------------------------------------------------------------------------
		public virtual void ScrollSelectionIntoView(IVwSelection sel,
			VwScrollSelOpts scrollOption)
		{
			...
			switch (scrollOption)
			{
				case VwScrollSelOpts.kssoDefault:
					MakeSelectionVisible(sel, true);
					break;
				case VwScrollSelOpts.kssoNearTop:
					ScrollSelectionToLocation(sel, LineHeight);
					break;
				case VwScrollSelOpts.kssoTop:
					ScrollSelectionToLocation(sel, 1);
					break;
				default:
		...
		/// <summary>
		/// This provides a hook for a class that wants to do something a little different
		/// after a Refresh. For example, XmlBrowseView may not have an active selection,
		/// but tries to keep the same line at the top of the screen.
		/// </summary>
		protected virtual void RestoreAfterRefresh()
		{
		...
		{
			return DataUpdateMonitor.Commit(vwsel, DataAccess);
		}
			...
			switch (name)
			{
				case "WritingSystemHvo":
					...
					if (EditingHelper.CurrentSelection == null)
						return;
					string s = (string)Mediator.PropertyTable.GetValue("WritingSystemHvo", "-1");
					...
					if (writingSystemHvo > 0 &&
						writingSystemHvo != SelectionHelper.GetWsOfEntireSelection(m_rootb.Selection))
					{
						EditingHelper.ApplyWritingSystem(writingSystemHvo);
					}
					break;
		...
		/// </remarks>
		/// -----------------------------------------------------------------------------------
		protected void MakeSelectionVisible(IVwSelection sel)
		{
			MakeSelectionVisible(sel, false);
		}
		...
		/// </remarks>
		/// -----------------------------------------------------------------------------------
		protected virtual void MakeSelectionVisible(IVwSelection sel, bool fWantOneLineSpace)
		{
			if (m_rootb == null)
				return;
			...
			if (!vwsel.IsValid)
				return; // can't work with an invalid selection
			...
			// if the entire selection is visible then there is nothing to do
			if (IsSelectionVisible(vwsel, fWantOneLineSpace))
				return;
			...
			using (new HoldGraphics(this))
			{
				...
				if (DoAutoVScroll)
				{
					...
					for (; ; )
					{
						...
						int spacing = fWantOneLineSpace ? LineHeight : 0;
						if (vwsel.IsRange && vwsel.SelType != VwSelType.kstPicture)
						{
							// For a range (that is not a picture) we scroll just far enough to show the end.
							if (fEndBeforeAnchor)
							{
								if (rcIdeal.Top < ydTop + spacing)
									dy = rcIdeal.Top - (ydTop + spacing);
								else if (rcIdeal.Top > ydBottom - 2 * spacing)
									dy = rcIdeal.Top - (ydBottom - 2 * spacing);
							}
							else
							{
								if (rcIdeal.Bottom < ydTop + 2 * spacing)
									dy = rcIdeal.Bottom - (ydTop + 2 * spacing);
								else if (rcIdeal.Bottom > ydBottom - spacing)
									dy = rcIdeal.Bottom - (ydBottom - spacing);
							}
						}
						else
						{
							// For an IP or picture, we at least try to make the whole thing visible.
							if (rcIdeal.Top < ydTop + spacing)
							{
								// move window contents up (less is hidden, neg dy).
								dy = rcIdeal.Top - (ydTop + spacing);
							}
							else if (rcIdeal.Bottom > ydBottom - spacing)
							{
						...
						if (m_rootb != null && m_dxdLayoutWidth > 0)
						{
							...
							PrepareToDraw(rcSrc, rcDst);
							// We can ignore the result because the code below checks whether the
							// scroll range was affected and loops if not.
						}
		...
		/// <param name="rcDstRoot"></param>
		/// <param name="rcSrcRoot"></param>
		private void PrepareToDraw(Rectangle rcSrcRoot, Rectangle rcDstRoot)
		{
			...
			try
			{
				m_rootb.PrepareToDraw(m_graphicsManager.VwGraphics, rcSrcRoot, rcDstRoot);
			}
			finally
			{
				m_fInPaint = false;
		...
		{
			if (m_rootb == null || m_rootb.DoSpellCheckStep())
				Application.Idle -= new EventHandler(Application_Idle);
		}
		...
		public void OnKeymanKeyboardChange(IntPtr wpFlags, IntPtr lpHKL)
		{
			...
			if (m_rootb == null)
				return; // For paranoia.
			IVwSelection vwsel = (IVwSelection)m_rootb.Selection;
			if (vwsel == null)
			...
			ILgKeymanHandler kh = LgKeymanHandlerClass.Create();
			string sKeymanKbd = kh.ActiveKeyboardName;

			int wsMatch = -1;
			if (sKeymanKbd == null || sKeymanKbd == SimpleRootSiteStrings.ksNone)
				sKeymanKbd = "";
			foreach (int ws in vws)
			{
				...
				IWritingSystem lgws = wsf.get_EngineOrNull(ws);
				if (lgws == null)
					continue;

				string sWsKbd = lgws.KeymanKbdName;
				if (sKeymanKbd == sWsKbd)
				{
					wsMatch = ws;
					if (wsMatch == (int)vws[0])
		...
		public object AccessibleRootObject
		{
			get
			{
				...
				if (m_rootb is IOleServiceProvider)
				{
					...
					if (sp == null)
					{
						MessageBox.Show(SimpleRootSiteStrings.ksNullServiceProvider);
						Debug.Assert(false);
					}
		...
		/// want an initial selection made in OnLoad (e.g., InterlinDocChild) or OnGotFocus().
		/// </summary>
		...
		protected override void OnGotFocus(EventArgs e)
		{
			...
			if (wsSel != 0)
			{
				...
				if (qws != null)
					RestoreKeyboardStatus(qws.Locale);
			}
		}
		...
		protected virtual void OnKillFocus(Control newWindow, bool fIsChildWindow)
		{
			...
			if (g_focusRootSite != this)
				newFocusRootSite = g_focusRootSite;
			else
				g_focusRootSite = null;
			...
			EditingHelper.CommitIfWord(new WordEventArgs(EditingHelper.WordEventSource.LoseFocus));
		...
		protected override void OnMouseMove(MouseEventArgs e)
		{
			...
			if (m_rootb != null)
			{
				...
				// REVIEW (TimS): is this code really necessary since its mostly commented out?
				...
				//(FwObjDataTypes)odt == FwObjDataTypes.kodtExternalPathName)
				{
					...
					if (GetExternalLinkSel(out fFoundLinkStyle, out vwsel, out strbFile, mousePos)
						&& fFoundLinkStyle)
					{
						// TODO (EberhardB): Show external link tooltip
		...
		protected override void OnPaint(PaintEventArgs e)
		{
			...
			if (m_fInPaint || m_fInLayout)
			{
				...
				// (TimS) This was causing an infinite loop of paint calls in certain
				// circumstances.  If it seems to be a problem having it commented out
				// then we need to find a better way of doing this.
				//this.Invalidate();
				return;
			}
			m_fInPaint = true;
		...
		protected virtual bool DoContextMenu(IVwSelection invSel, Point pt, Rectangle rcSrcRoot, Rectangle rcDstRoot)
		{
			if (DoSpellCheck)
			{
				...
				if (EditingHelper.DoSpellCheckContextMenu(pt, this, rcSrcRoot, rcDstRoot))
					return true;
			}
			return false;
		}
		...
		protected override void OnKeyPress(KeyPressEventArgs e)
		{
			...
			if (!e.Handled)
			{
				...
				{
					Debug.Assert(m_graphicsManager.VwGraphics != null);
					EditingHelper.OnKeyPress(e, ModifierKeys, m_graphicsManager.VwGraphics);
				}
				// We can't do this inside the using block, because it might destroy ourselves,
				// and dispose of the HoldGraphics object prematurely before the using block
				// finishes.   See LT-7365.
		...
		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// If this is true, it instructs OnKeyPress to handle tab keys by cycling through
		/// controls on a form
		/// </summary>
		/// ------------------------------------------------------------------------------------
		public virtual bool HandleTabAsControl
		{
			get
			{
				CheckDisposed();
		...
		protected override void OnKeyDown(KeyEventArgs e)
		{
			...
			using (new HoldGraphics(this))
			{
				...
				Debug.Assert(m_graphicsManager.VwGraphics != null);
				EditingHelper.OnKeyDown(e, m_graphicsManager.VwGraphics);
			}
		}
		...
		protected override bool IsInputKey(Keys keyData)
		{
			...
			{
				// if we want the tab key to cycle through the controls we return false and
				// let the Control class deal with the cycling. (LT-9049)
				return !HandleTabAsControl;
			}
		...
		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Saves the current selection to be restored with the RestoreSelection() method.
		/// </summary>
		/// ------------------------------------------------------------------------------------
		public virtual void SaveSelection()
		{
			...
			// we can't use EditingHelper.CurrentSelection here because the scroll position
			// of the selection may have changed.
			m_savedSelection = SelectionHelper.Create(this);
		}

		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Restores the selection that was saved with the SaveSelection() method.
		/// </summary>
		/// <param name="scrollIntoView">True to scroll the restored selection back into view,
		/// false otherwise</param>
		/// ------------------------------------------------------------------------------------
		public void RestoreSelection(bool scrollIntoView)
		{
			CheckDisposed();
			if (m_savedSelection == null || m_rootb.Height <= 0)
				return;
			...
			if (scrollIntoView)
				selectionRestored = m_savedSelection.RestoreSelectionAndScrollPos();
			else
				selectionRestored = (m_savedSelection.SetSelection(this, true, false) != null);
		...
		protected virtual void Draw(PaintEventArgs e)
		{
				...
				(!DesignMode || AllowPaintingInDesigner))
			{
				...
				try
				{
					m_rootb.DrawingErrors();
				}
				catch (Exception ex)
				{
					GiveDrawErrMsg(ex, false);
				}
			}
			else
			{
		...
		/// </summary>
		/// -----------------------------------------------------------------------------------
		protected virtual void SwitchFocusHere()
		{
			Focus();
		}
		...
		protected virtual void CallOnTyping(IVwGraphics vg, string str, int cchBackspace,
			int cchDelForward, char chFirst, Rectangle rcSrcRoot, Rectangle rcDstRoot)
		{
			//	Console.WriteLine("CallOnTyping : " + str);

			// The user has pressed Ctrl-Space - do not generate a character.
			if ((ModifierKeys & Keys.Control) == Keys.Control && str.CompareTo(" ") == 0)
			{
				//				IVwSelection vwsel;
				//				ITsTextProps[] vttp;
			...
			{
				m_rootb.OnTyping(vg, str, cchBackspace, cchDelForward, chFirst, ref m_wsPending);
			}
		}
		...
		public virtual void CloseRootBox()
		{
			...
			if (m_Timer != null)
				m_Timer.Stop(); // Can't flash IP, if the root box is toast.
		...
		/// </summary>
		/// -----------------------------------------------------------------------------------
		protected virtual bool IsSelectionFormattable
		{
			get { return true; }
		}
			...
			out string strbFile, Point pt)
		{
			...
			MessageBox.Show(SimpleRootSiteStrings.ksCodeNeedsWork);
		...
		/// </summary>
		/// <param name="e"></param>
		/// <param name="fFatal"></param>
		/// -----------------------------------------------------------------------------------
		protected void GiveDrawErrMsg(Exception e, bool fFatal)
		{
			if (e == null)
				return;
			...
			if (!fFatal)
			{
				...
				{
					if ((string)s_vstrDrawErrMsgs[i] == str)
						return;
				}
			}

			s_vstrDrawErrMsgs.Add(str);

			// If we ever change to another testing program then this needs to change.
			if (Application.ExecutablePath.ToLowerInvariant().IndexOf("nunit") != -1)
			{
				// We are running tests.
				Trace.Assert(false, ResourceHelper.GetResourceString("kstidDrawingError"),
					str);
			}
			else
			{
				MessageBox.Show(
					string.Format(ResourceHelper.GetResourceString("kstidDrawingError"), str),
					Application.ProductName);
			}
		}

		// We don't need BeginUndoTask/EndUndoTask/SetupUndoSelection any more - use
		// UndoTaskHelper class instead.

		// We don't need AfVwRootSite::UndoRedoLabelRes anymore - put your string in
		// FwStrings.resx and use that name (id) directly, so there is no need
		// to translate a id to a resource id.
		...
		/// ------------------------------------------------------------------------------------
		protected virtual bool SelectionIsFootnoteMarker(IVwSelection vwsel)
		{
			// derived classes that support footnotes must override this method
			return false;
		}
		...
		/// ------------------------------------------------------------------------------------
		protected string GetOrcHotLinkStrProp(IVwSelection vwsel)
		{
			if (vwsel == null)
				return null;
			...
			if (cttp != 1)
				return null; // no ORC found

			// get the desired string prop for the first run
			string sGuid =
				vttpTmp[0].GetStrPropValue((int)FwTextPropType.ktptObjData);

			// Check if the selection is a hot link (e.g. footnote marker).
			if (sGuid != null && sGuid != string.Empty &&
				(sGuid[0] == (char)(int)FwObjDataTypes.kodtNameGuidHot ||
				sGuid[0] == (char)(int)FwObjDataTypes.kodtOwnNameGuidHot))
			{
				return sGuid; //we have an ORC hot link; this string contains the Guid info too
			}
			return null; // no ORC hot link found
		}
