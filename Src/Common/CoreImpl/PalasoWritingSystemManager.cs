...
namespace SIL.CoreImpl
{
	...
	/// </summary>
	public class PalasoWritingSystemManager : IWritingSystemManager, IDisposable
	{
		...
		protected virtual void Dispose(bool fDisposing)
		{
			...
			if (fDisposing && !IsDisposed)
			{
				...
				var disposable = GlobalWritingSystemStore as IDisposable;
				if (disposable != null)
					disposable.Dispose();
			...
			set
			{
				...
				lock (m_syncRoot)
				{
					if (m_localStore != value)
					{
						m_localStore = value;
						m_handleWss.Clear();
						foreach (PalasoWritingSystem ws in m_localStore.WritingSystemDefinitions)
						{
			...
			get
			{
				lock (m_syncRoot)
				{
					if (m_globalStore != null)
						return m_globalStore.WritingSystemDefinitions.Cast<IWritingSystem>();
				}
		...
		public IEnumerable<IWritingSystem> LocalWritingSystems
		{
			...
			{
				lock (m_syncRoot)
					return m_localStore.WritingSystemDefinitions.Cast<IWritingSystem>();
			}
		}
		...
		public IEnumerable<IWritingSystem> CheckForNewerGlobalWritingSystems()
		{
			lock (m_syncRoot)
			{
				if (m_globalStore != null)
				{
					...
					foreach (WritingSystemDefinition wsDef in m_localStore.WritingSystemsNewerIn(m_globalStore.WritingSystemDefinitions))
					{
		...
		public IWritingSystem Create(string identifier)
		{
			...
			return Create(languageSubtag, scriptSubtag, regionSubtag, variantSubtag);
		}
		...
		public bool Exists(string identifier)
		{
			lock (m_syncRoot)
			{
				var fExists = m_localStore.Exists(identifier);
				if (!fExists)
				{
					if (identifier.StartsWith("cmn"))
					{
						var ident = identifier.Remove(0, 3).Insert(0, "zh");
						fExists = m_localStore.Exists(ident);
		...
		public IWritingSystem Get(string identifier)
		{
			lock (m_syncRoot)
			{
				...
				if (!m_localStore.TryGet(identifier, out wrsys))
				{
					...
					if (wrsys == null)
						return (IWritingSystem)m_localStore.Get(identifier);	// throw the expected exception...
				}
		...
		public bool TryGetOrSet(string identifier, out IWritingSystem ws)
		{
			lock (m_syncRoot)
			{
				...
				if (Exists(identifier) || (m_globalStore != null && m_globalStore.Exists(identifier)))
				{
		...
		public string LocalStoreFolder
		{
			set
			{
				if (m_localStore is LdmlInFolderWritingSystemStore)
					((LdmlInFolderWritingSystemStore)m_localStore).PathToWritingSystems = value;
			}
		}
		...
		public string GetValidLangTagForNewLang(string langName)
		{
			...
			if (LangTagUtils.IsLanguageCodeValid(isoCode) && !LangTagInUse("x-" + isoCode))
		...
		{
			lock (m_syncRoot)
				return m_localStore.Exists(identifier) || (m_globalStore != null && m_globalStore.Exists(identifier));
		}
		...
		/// <returns></returns>
		public IRenderEngine get_RendererFromChrp(ref LgCharRenderProps chrp)
		{
			#if !__MonoCS__
			...
			try
			{
				VwGraphicsWin32 vg = VwGraphicsWin32Class.Create();
				#if !__MonoCS__
				vg.Initialize(hdc);
				#else
				// Linux VwGraphics handles a null ptr hdc by creating a default
				// graphics context.
				// TODO-Linux: possibly refactor Windows VwGraphics to do the same
				// then calls to CreateDC could be removed, in a few places.
				vg.Initialize(IntPtr.Zero);
				#endif
				vg.SetupGraphics(ref chrp);
				renderEngine = get_Renderer(chrp.ws, vg);
				Marshal.ReleaseComObject(vg);
			}
			finally
			{
				#if !__MonoCS__
	...
	///
	/// </summary>
	public interface IFwWritingSystemStore : IWritingSystemStore
	{
		/// <summary>
		/// Gets the specified writing system if it exists.
		/// </summary>
	...
	/// </summary>
	public class MemoryWritingSystemStore : WritingSystemStoreBase, IFwWritingSystemStore
	{
		/// <summary>
		...
		/// </summary>
		/// <returns></returns>
		protected override LdmlAdaptor CreateLdmlAdaptor()
		{
		...
		{
			List<string> idsToRemove = (from ws in WritingSystemDefinitions
				where ws.MarkedForDeletion
				select ws.StoreID).ToList();
			foreach (string id in idsToRemove)
				Remove(id);

			List<WritingSystemDefinition> allDefs = (from ws in WritingSystemDefinitions
				where CanSet(ws)
				select ws).ToList();
	...
	/// </summary>
	public class FwLdmlAdaptor : LdmlAdaptor
	{
		/// <summary>
		/// Adds the namespaces.
		/// </summary>
		...
		/// <param name="ws">The ws.</param>
		protected override void WriteTopLevelSpecialElements(XmlWriter writer, WritingSystemDefinition ws)
		{
			base.WriteTopLevelSpecialElements(writer, ws);
			var fwWs = (PalasoWritingSystem)ws;
