...
namespace SIL.CoreImpl
{
	...
	public class PalasoWritingSystemManagerTests : FwCOMTestBase // can't derive from BaseTest, but instantiate DebugProcs instead
	{
		...
		public void SerializeDeserialize()
		{
			...
			// serialize
			using (var wsManager = new PalasoWritingSystemManager(new LocalFileWritingSystemStore(storePath)))
			{
		...
		public void GlobalStore()
		{
			...
			using (var globalGlobalStore = new GlobalFileWritingSystemStore(globalStorePath))
			{
				using (var globalStore = globalGlobalStore.Clone())
		...
		public void GlobalStore_WritingSystemsToIgnore()
		{
						...
						new LocalFileWritingSystemStore(storePath1, globalStore), globalStore))
			{
				...
				using (var wsManager = new PalasoWritingSystemManager(
						new LocalFileWritingSystemStore(storePath1, globalStore), globalStore))
				{
					var sharedWss = wsManager.CheckForNewerGlobalWritingSystems();
		...
		public void Create()
		{
			...
			Assert.AreEqual("Roman (Latin)", enWs.ScriptSubtag.Name);
		...
		public void get_RendererFromChrp_Uniscribe()
		{
			...
			IRenderEngine engine = wsManager.get_RendererFromChrp(ref chrp);
		...
		public void get_RendererFromChrp_Graphite()
		{
			...
			engine = wsManager.get_RendererFromChrp(ref chrp);
			Assert.IsNotNull(engine);
			Assert.AreSame(wsManager, engine.WritingSystemFactory);
		...
		public void GetValidLangCodeForNewLang()
		{
			...
			using (var globalStore = new GlobalFileWritingSystemStore(globalStorePath))
			{
				using (var wsManager = new PalasoWritingSystemManager(
