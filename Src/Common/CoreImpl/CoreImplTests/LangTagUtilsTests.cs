...
namespace SIL.CoreImpl
{
	...
	public class LangTagUtilsTests : FwCOMTestBase // can't derive from BaseTest, but instantiate DebugProcs instead
	{
		...
		public void ToLangTag()
		{
			...
			Assert.AreEqual("x-kal", LangTagUtils.ToLangTag("xkal"));
			// private use language, custom ICU variant
			Assert.AreEqual("x-kal-fonipa", LangTagUtils.ToLangTag("xkal__IPA"));
			// private use language, private use region
			Assert.AreEqual("x-kal-XA", LangTagUtils.ToLangTag("xkal_XA"));
			// private use language, private use script
			Assert.AreEqual("x-kal-Fake", LangTagUtils.ToLangTag("xkal_Fake"));
			// language, private use script
			Assert.AreEqual("en-x-Fake", LangTagUtils.ToLangTag("en_Fake"));
			// language, private use script, private use region
			Assert.AreEqual("en-x-Fake-QD", LangTagUtils.ToLangTag("en_Fake_QD"));
			// private use language, script
			Assert.AreEqual("x-zzz-Latn", LangTagUtils.ToLangTag("zzz_Latn"));
			// convert older FW language tags
			Assert.AreEqual("slu", LangTagUtils.ToLangTag("eslu"));
			// other possibilities from FW6.0.6
			Assert.AreEqual("x-bcd", LangTagUtils.ToLangTag("x123"));
			Assert.AreEqual("x-kac", LangTagUtils.ToLangTag("xka2"));
		...
		public void ToIcuLocale()
		{
			...
			// private use language
			Assert.AreEqual("xkal", LangTagUtils.ToIcuLocale("x-kal"));
			// private use language, ICU variant
			Assert.AreEqual("xkal__X_ETIC", LangTagUtils.ToIcuLocale("x-kal-fonipa-x-etic"));
			// private use language, private use region
			Assert.AreEqual("xkal_XA", LangTagUtils.ToIcuLocale("x-kal-XA"));
			// private use language, private use script
			Assert.AreEqual("xkal_Fake", LangTagUtils.ToIcuLocale("x-kal-Fake"));
			// language, private use script
			Assert.AreEqual("en_Fake", LangTagUtils.ToIcuLocale("en-x-Fake"));
			// language, private use script, private use region
			Assert.AreEqual("en_Fake_QD", LangTagUtils.ToIcuLocale("en-x-Fake-QD"));
			// private use language, script
			Assert.AreEqual("xzzz_Latn", LangTagUtils.ToIcuLocale("x-zzz-Latn"));
		}
		...
		public void GetSubtags()
		{
			...
			Assert.IsTrue(LangTagUtils.GetSubtags("x-kal", out languageSubtag, out scriptSubtag, out regionSubtag, out variantSubtag));
			...
			Assert.IsTrue(LangTagUtils.GetSubtags("x-kal-Fake", out languageSubtag, out scriptSubtag, out regionSubtag, out variantSubtag));
			Assert.AreEqual("kal", languageSubtag.Code);
			Assert.IsTrue(languageSubtag.IsPrivateUse);
			...
			Assert.IsTrue(LangTagUtils.GetSubtags("x-kal-XA", out languageSubtag, out scriptSubtag, out regionSubtag, out variantSubtag));
			Assert.AreEqual("kal", languageSubtag.Code);
			Assert.IsTrue(languageSubtag.IsPrivateUse);
			Assert.IsNull(scriptSubtag);
			...
			Assert.IsTrue(LangTagUtils.GetSubtags("en-x-Fake-QD", out languageSubtag, out scriptSubtag, out regionSubtag, out variantSubtag));
			Assert.AreEqual("en", languageSubtag.Code);
			Assert.IsFalse(languageSubtag.IsPrivateUse);
			Assert.AreEqual("Fake", scriptSubtag.Code);
			Assert.IsTrue(scriptSubtag.IsPrivateUse);
