...
namespace SIL.FieldWorks.Common.Framework
{
	...
	public abstract class StylesXmlAccessor : SettingsXmlAccessorBase
	{
		...
		{
			get { return "D8066E53-262D-43f4-914E-A1BDD0693893"; }
		}
		...
		protected override void ProcessResources(ProgressDialogWithTask dlg,
			IProgress progressDlg, XmlNode doc)
		{
			dlg.RunTask(progressDlg, true, CreateStyles, StyleCollection, doc);
		}
		#endregion
		...
		{
#if WANTPORT // (Common) FWR-2788 implement this when needed (if we release a stylesheet change that actually
			// does deprecate or rename styles). We have a replacement for the string crawler to update styles
			...
			ITsPropsBldr propsBldr)
		{
			...
			{
				propsBldr.SetStrPropValue((int)FwTextPropType.ktptWsStyle, StyleInfo.GetOverridesString(overrides));
			}
			...
			ITsPropsBldr propsBldr)
		{
			...
			if (node != null)
			{
				propsBldr.SetIntPropValues((int)FwTextPropType.ktptBulNumScheme,
					(int)FwTextPropVar.ktpvEnum,
			...
			if (node != null)
			{
				...
				propsBldr.SetIntPropValues((int)FwTextPropType.ktptBulNumStartAt,
