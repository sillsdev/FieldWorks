...
namespace SIL.FieldWorks.Common.Framework
{
	...
	public class DummyFwApp : FwApp
	{
		...
		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Override to allow app version to be set for testing.
		/// </summary>
		/// ------------------------------------------------------------------------------------
		protected override int AppVersion
		{
		...
		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Override showing of message box for testing.
		/// </summary>
		/// <param name="dbName"></param>
		/// ------------------------------------------------------------------------------------
		protected override void ShowOldAppWarning(string dbName)
		{
			m_oldAppWarningCalled = true;
		}
		...
		/// ------------------------------------------------------------------------------------
		protected override void ShowNoUpgradeWarning(string dbName, bool type)
		{
			m_noUpgradeWarningCalled = true;
		}
		...
		/// <returns></returns>
		/// ------------------------------------------------------------------------------------
		protected override bool ShouldUpgradeDatabase(string srvName, string dbName)
		{
			m_shouldUpgradeDatabaseCalled = true;
			return false;
		}
		...
		/// ------------------------------------------------------------------------------------
		protected override int GetDbVersion_Internal(string svrName, string dbName)
		{
			Assert.IsTrue(MiscUtils.IsServerLocal(svrName));
			Debug.Assert(!string.IsNullOrEmpty(dbName));

			return m_internalDbVersion == -1 ? base.GetDbVersion_Internal(svrName, dbName) : m_internalDbVersion;
		}
