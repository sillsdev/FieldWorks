...
namespace SIL.FieldWorks.Common.Framework
{
	...
	public sealed class MainWindowDelegate : IMainWindowDelegatedFunctions
	{
			...
			Action<StyleInfo> setPropsToFactorySettings)
		{
			...
			// stack. It hosts an IVwRootSite/SimpleRootSite (the live style PREVIEW pane renders through the
			// Views engine — see the activeViewSite argument and the GetStyleNames selection walk above), which
			// the dialog-conversion rule explicitly routes to the document engine,
			// not this hand-authored MVVM dialog stack. Named-style APPLY over a
