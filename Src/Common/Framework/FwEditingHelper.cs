		...
		protected override void OnCharAux(string stuInput, VwShiftStatus ss, Keys modifiers)
		{
			...
			if (modifiers != Keys.Control && modifiers != Keys.Alt && currSel != null)
			{
				...
				if (ttpTop != null)
				{
					...
					string urlTop = StringUtils.GetURL(sObjData);
					if (urlTop != null)
					{
						...
						if (ttpBottom != null)
						{
							...
							string urlBottom = StringUtils.GetURL(sObjData);
							if (urlBottom != urlTop)
							{
								int nVar;
								...
								stuInput[0] == (char)VwSpecialChars.kscBackspace)
							{
								if (currSel.IsRange)
								{
									...
									string urlBefore = (ttpBefore == null) ? null :
										StringUtils.GetURL(ttpBefore.GetStrPropValue((int)FwTextPropType.ktptObjData));

									ITsTextProps ttpAfter = currSel.PropsAfter;
									string urlAfter = (ttpAfter == null) ? null :
										StringUtils.GetURL(ttpAfter.GetStrPropValue((int)FwTextPropType.ktptObjData));
