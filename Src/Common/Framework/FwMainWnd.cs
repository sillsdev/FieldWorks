...
namespace SIL.FieldWorks.Common.Framework
{
	...
	/// </remarks>
	/// -------------------------------------------------------------------------------------------
	public class FwMainWnd : Form, IFWDisposable, ISettings, IFwMainWnd, IxCoreColleague,
		IRegistryKeyNameModifier, IMainWindowDelegateCallbacks
	{
		#region Data members
		/// <summary></summary>
		public delegate void OpenProjectFromOtherAppDelegate(string[] args);
		/// <summary></summary>
		public OpenProjectFromOtherAppDelegate m_openDelegate;
		...
		private Dictionary<string, IRootSite> m_rgClientWindows; // Control objects get added, but they must implement two interfaces.
		...
		/// <summary></summary>
		protected SIL.FieldWorks.Common.Framework.ButtonsMenusLinker m_ButtonsMenusLinker;
		...
		/// <summary></summary>
		protected DIFwBackupDb m_backupSystem = null;

		/// <summary>This flag gets set to true when the user tries to close this
		/// window while a backup or restore is going on. Since there can be problems
		/// when this window is closed in the middle of a backup or restore, we
		/// just hide this window, set this flag, and the window will really be
		/// closed after the backup or restore is completed.</summary>
		protected bool m_fCloseAfterBackupRestore = false;
		/// <summary>During a restore, if we're trying to restore over the only open window,
		/// we do want to close it so that the database connection is closed.</summary>
		protected bool m_fReallyClose = false;
		...
		public FwMainWnd()
		{
			...
			m_rgClientWindows = new Dictionary<string, IRootSite>();
			m_viewHelper = new ActiveViewHelper(this);

			if (FwApp.App != null) // not in design mode
			{
				CreateMenusAndToolBars();
				CreateSideBarInfoBarAdapter();
			}

			m_openDelegate = new OpenProjectFromOtherAppDelegate(OpenProjectFromOtherApp);
		...
		/// ------------------------------------------------------------------------------------
		public FwMainWnd(FdoCache cache, Form wndCopyFrom) : this()
		{
			Cache = cache;
		...
		protected void InitStyleSheet(int hvoStylesOwner, int tagStylesList)
		{
			...
			StyleSheet.Init(m_cache, hvoStylesOwner, tagStylesList, false);
			InitStyleComboBox();
		}
		...
		protected void ReSynchStyleSheet()
		{
			...
			StyleSheet.Init(m_cache, StyleSheet.RootObjectHvo, StyleSheet.StyleListTag, true);
			InitStyleComboBox();
		}
		#endregion
		...
		private string ToolBarSettingsFile
		{
			get
			{
				...
				return appPath + "\\" + appPrgName + ".ToolBarDef.xml";
			}
		}
		...
		protected virtual void CreateMenusAndToolBars()
		{
			...
			string sMenuToolBarDefinition = Path.Combine(
				Common.Utils.DirectoryFinder.FWCodeDirectory, "FwTMDefinition.xml");

			m_tmAdapter.Initialize(this, AdapterContentControl, m_mediator, SettingsKeyEx.ToString(),
		...
		/// </summary>
		/// ------------------------------------------------------------------------------------
		public virtual void RefreshAllViews()
		{
			CheckDisposed();

			using (new WaitCursor(this))
			{
				bool prevParaIgnoreValue = ParaStyleListHelper.IgnoreListRefresh;
				bool prevCharIgnoreValue = CharStyleListHelper.IgnoreListRefresh;
		...
		public virtual bool OnFinishedInit()
		{
			...
			if (FwApp.App != null)
			{
				...
				m_fFullWindow = RegistryHelper.KeyExists(SettingsKeyEx, "FullWindowSettings");
				...
				{
					statusStrip.Visible = FwRegistrySettings.ShowStatusBarSetting;
					if (SIBAdapter != null)
						SIBAdapter.SideBarVisible = FwRegistrySettings.ShowSideBarSetting;
				}
			}
		...
		/// </summary>
		/// <value>Returns true if this window is to be a copy of another FwMainWnd, in
		...
		public bool KeepWindowSizePos
		{
			get
			{
				...
				// the OnLayout event (See TE-6698).
				return true; // WindowIsCopy;
			}
		}
		...
		/// </summary>
		/// ------------------------------------------------------------------------------------
		public virtual RegistryKey SettingsKeyEx
		{
			get
			{
				CheckDisposed();
		...
		public virtual int StyleSheetOwningFlid
		{
			...
			{
				CheckDisposed();
				return (int)FDO.LangProj.LangProject.LangProjectTags.kflidStyles;
			}
		}
		...
		/// -----------------------------------------------------------------------------------
		/// <summary>
		/// Gets the array of client windows (all of which must implement IRootSite and
		///  ISelectableView)
		/// </summary>
		/// -----------------------------------------------------------------------------------
		public virtual Dictionary<string, IRootSite> ClientWindows
		{
			get
			{
				CheckDisposed();
				return m_rgClientWindows;
			}
		}
		...
		/// <summary>
		/// Flag that the window should close even if the backup system is open.
		/// </summary>
		internal bool ReallyClose
		{
			set { m_fReallyClose = value; }
		}
		#endregion
		...
		protected virtual void SaveSettings(RegistryKey key)
		{
			try
			{
				...
				if (m_sibAdapter != null)
					m_sibAdapter.SaveSettings(ModifyKey(key, true));
			}
			catch (Exception e)
			{
		...
		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Show the project information in the caption bar.
		/// </summary>
		/// ------------------------------------------------------------------------------------
		...
		protected bool OnUpdateViewZoom(object args)
		{
			...
			if (itemProps != null && m_cboZoomPercent != null)
			{
				if (!m_cboZoomPercent.Focused)
				{
					...
					{
						itemProps.Text = m_cboZoomPercent.Text = FwApp.GetResourceString("kstidPageWidth");
		...
		protected bool OnUpdateFileSave(object args)
		{
			...
			if (itemProps != null)
			{
				itemProps.Enabled = (m_cache == null ?
					false :	itemProps.Enabled = m_cache.DatabaseAccessor.IsTransactionOpen());
		...
		protected bool OnUpdateEditUndo(object args)
		{
			...
			if (itemProps != null)
			{
				itemProps.Enabled = (Cache != null && Cache.CanUndo);
				itemProps.Update = true;
				return true;
			}
		...
		protected bool OnUpdateEditRedo(object args)
		{
			...
			if (itemProps != null)
			{
				itemProps.Enabled = (Cache != null && Cache.CanRedo);
				itemProps.Update = true;
				return true;
			}
		...
		protected bool OnUpdateWindowActivate(object args)
		{
			...
			ArrayList menuText = new ArrayList(FwApp.App.MainWindows.Count);

			for (int i = 0; i < FwApp.App.MainWindows.Count; i++)
			{
				IFwMainWnd wnd = (IFwMainWnd)FwApp.App.MainWindows[i];
				string format = wndListInfo.WindowListItemProperties.OriginalText;
				string projName = wnd.Cache.LangProject.Name.UserDefaultWritingSystem;
...
//		}
		...
		{
			return FwApp.App.ShowFindReplaceDialog(false, ActiveView as RootSite);
		}
		...
		{
			return FwApp.App.ShowFindReplaceDialog(true, ActiveView as RootSite);
		}
		...
		{
			if (FwApp.App.FindReplaceDialog == null)
				return FwApp.App.ShowFindReplaceDialog(false, ActiveView as RootSite);

			FwApp.App.FindReplaceDialog.FindNext();
			return true;
		}
		...
		protected bool OnEditFindPrev(object args)
		{
			...
			FwApp.App.FindReplaceDialog.FindPrevious();
			return true;
		}
		...
		protected bool OnEditCut(object args)
		{
			...
			using (new WaitCursor(this)) // creates a wait cursor and makes it active until the end of the method.
			{
				return EditingHelper.CutSelection();
			}
		}
		...
		protected virtual bool OnEditDelete(object args)
		{
			...
			return EditingHelper.DeleteSelection("kstidEditDelete");
		}
		...
		{
			if (DataUpdateMonitor.IsUpdateInProgress(m_cache.MainCacheAccessor) || EditingHelper == null)
				return true; //discard this event

			EditingHelper.PasteClipboard(false);
			return true;
		}
		...
		protected bool OnFileClose(object args)
		{
			...
			try
			{
				...
				if (FwApp.App.MainWindows.Count == 1)
					FwApp.App.LastMainWndClosed(this);
		...
		protected bool OnFileOpen(object args)
		{
			...
			FwApp.App.OpenProjectDialog(this, m_cache.ServerName, wsf);
			return true;
		}
		...
		{
			FwApp.App.TileWindows(this, WindowTiling.SideBySide);
			return true;
		}
		...
		{
			FwApp.App.TileWindows(this, WindowTiling.Stacked);
			return true;
		}
		...
		protected bool OnWindowActivate(object sender)
		{
			if (sender is TMItemProperties)
			{
				...
				FwApp.App.ActivateWindow(i);
			}
			else if (sender is int) // for testing
				FwApp.App.ActivateWindow((int)sender);
		...
		{
			MoreWindowsForm moreWindows = new MoreWindowsForm(FwApp.App.MainWindows);

			if (ShowTestableDialog(moreWindows) == DialogResult.OK)
				FwApp.App.ActivateWindow(moreWindows.SelectedWindow);
		...
		/// <returns></returns>
		/// ------------------------------------------------------------------------------------
		protected bool OnFileBackupRestore(object arg)
		{
			List<IFwMainWnd> wnds = FwApp.App.MainWindows;
			try
			{
				foreach (IFwMainWnd wnd in wnds)
					wnd.EnableWindow(false);

				CreateBackupDirectory();
				m_backupSystem = FwBackupClass.Create();
				m_backupSystem.Init(FwApp.App, Handle.ToInt32());
				m_backupSystem.UserConfigure(FwApp.App, false);
				m_backupSystem.Close();
			}
			finally
			{
				m_backupSystem = null;
		...
		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// If the default backup registry setting (and folder) is not defined, do so.
		/// </summary>
		/// ------------------------------------------------------------------------------------
		protected void CreateBackupDirectory()
		{
			try
			{
				if (!Directory.Exists(FwRegistrySettings.BackupDirectorySetting))
					Directory.CreateDirectory(FwRegistrySettings.BackupDirectorySetting);

				// Write default backup directory to registry. Have to do this to save in registry.
				FwRegistrySettings.BackupDirectorySetting = FwRegistrySettings.BackupDirectorySetting;
			}
			catch
			{
		...
		protected bool OnStatusBar(object args)
		{
			if (!DesignMode)
			{
				...
				FwRegistrySettings.ShowStatusBarSetting = statusStrip.Visible;
			}
		...
		protected bool OnSideBar(object args)
		{
			if (!DesignMode)
			{
				...
				FwRegistrySettings.ShowSideBarSetting = SIBAdapter.SideBarVisible;
			}
		...
		protected bool OnViewFullWindow(object args)
		{
			...
			RegistryGroup regGroup = new RegistryGroup(SettingsKeyEx, "FullWindowSettings");
		...
		protected virtual bool OnEditUndo(object args)
		{
			...
			using (new WaitCursor(this))
			{
				FwApp.App.SuppressSynchronize(m_cache);
				try
				{
					try
					{
						Logger.WriteEvent(m_cache.ActionHandlerAccessor.GetUndoTextN(
							m_cache.ActionHandlerAccessor.UndoableSequenceCount - 1));
					}
					catch
					{
						...
						Logger.WriteEvent("m_cache.ActionHandlerAccessor.UndoableSequenceCount = " +
							m_cache.ActionHandlerAccessor.UndoableSequenceCount);
						Logger.WriteEvent("m_cache.ActionHandlerAccessor.UndoableActionCount = " +
							m_cache.ActionHandlerAccessor.UndoableActionCount);
						Logger.WriteEvent("Undo stack items:");
						try
						{
							for (int i = 0; i < m_cache.ActionHandlerAccessor.UndoableSequenceCount - 1; i++)
							{
								Logger.WriteEvent(m_cache.ActionHandlerAccessor.GetUndoTextN(i));
							}
						}
						catch
						{
					...
					if (ures != UndoResult.kuresSuccess && ures != UndoResult.kuresRefresh)
					{
					...
					if (!fPrivate)
					{
						if (ures == UndoResult.kuresSuccess)
							...
							FwApp.App.Synchronize(new SyncInfo(SyncMsg.ksyncFullRefresh, 0, 0), Cache);
					}
				}
				finally
				{
					FwApp.App.ResumeSynchronize(m_cache);
				}
			}
			return fResult;
		}
		...
		protected bool OnDropDownEditUndo(object args)
		{
			...
			if (m_cache == null || m_cache.ActionHandlerAccessor == null ||
				m_cache.ActionHandlerAccessor.UndoableSequenceCount == 0)
			{
				return false;
			}
			...
			InitializeUndoRedoDropDown(popupInfo, FwApp.GetResourceString("kstidUndo1Action"),
				FwApp.GetResourceString("kstidUndoMultipleActions"),
				FwApp.GetResourceString("kstidUndoRedoCancel"));

			for (int i = m_cache.ActionHandlerAccessor.UndoableSequenceCount - 1;	i >= 0; i--)
			{
				string undoText = m_cache.ActionHandlerAccessor.GetUndoTextN(i);
				if (undoText == null)
					m_UndoRedoDropDown.Actions.Add(FwApp.GetResourceString("kstidUnknownUndo"));
				else
					m_UndoRedoDropDown.Actions.Add(undoText.Replace("&", ""));
			}
		...
		protected bool OnDropDownEditRedo(object args)
		{
			...
			// before proceeding here, make sure we have a cache and an ActionHandlerAccessor.
			if (m_cache == null || m_cache.ActionHandlerAccessor == null ||
				m_cache.ActionHandlerAccessor.RedoableSequenceCount == 0)
			{
				return false;
			}
			...
			if (popupInfo == null)
				return false;
			InitializeUndoRedoDropDown(popupInfo, FwApp.GetResourceString("kstidRedo1Action"),
			...
			if (m_cache != null && m_cache.ActionHandlerAccessor != null)
			{
				for (int i = 0; i < m_cache.ActionHandlerAccessor.RedoableSequenceCount; i++)
				{
					...
					if (redoText == null)
						m_UndoRedoDropDown.Actions.Add(FwApp.GetResourceString("kstidUnknownRedo"));
					else
						m_UndoRedoDropDown.Actions.Add(redoText.Replace("&", ""));
				}
			}
		...
		protected virtual bool OnEditRedo(object args)
		{
			...
			using(new WaitCursor(this))
			{
				...
				try
				{
					Logger.WriteEvent(m_cache.ActionHandlerAccessor.GetRedoTextN(0));
		...
		{
			using (FwHelpAbout helpAboutWnd = new FwHelpAbout(FwApp.App.DropDeadDate))
			{
		...
		{
			using (FwUserProperties propDlg = new FwUserProperties(m_cache, FwApp.App.GetAppFeatures()))
			{
				propDlg.SetDialogProperties(FwApp.App);
				propDlg.ShowDialog();
			}
			return true;
		}
		...
		protected bool OnFormatStyle(object args)
		{
			...
			if (m_delegate.ShowStylesDialog(paraStyleName, charStyleName))
				FwApp.App.Synchronize(new SyncInfo(SyncMsg.ksyncStyle, 0, 0), Cache);
		...
		{
			// If there are multiple apps or multiple users connected to this database, we
			// better reload the cache as it might be stale.
			if (DbOps.GetNumberOfConnectionsToDb(m_cache) > 1)
		...
		public bool OnPageSetup(object args)
		{
			...
			using (IPageSetupDialog dlg = CreatePageSetupDialog(m_cache.DefaultUserWs, pgl, pub, div, FwApp.App,
				FwApp.MeasurementSystem))
			{
				...
				using (new UndoTaskHelper(m_cache.MainCacheAccessor, null, "kstidUndoPageSetupChanges"))
				{
		...
		/// <c>false</c></returns>
		/// ------------------------------------------------------------------------------------
		protected virtual bool PublicationUsesUniformLineSpacing(IPublicationView pubControl)
		{
			return false;
		}
		...
		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Open a new project window from another instance of TE
		/// </summary>
		/// <param name="args">Command line arguments</param>
		/// ------------------------------------------------------------------------------------
		public void OpenProjectFromOtherApp(string[] args)
		{
			CheckDisposed();

			FwApp.App.NewMainWindow(args);
		}
					...
					if (!OnEditUndo(null))
					{
						...
						else
						{
							MessageBox.Show(ResourceHelper.FormatResourceString("kstidUndoFailureReport",
								iClicked + 1, i), Application.ProductName);
						}
						break;
					}
				}
		...
		/// <param name="e"></param>
		/// ------------------------------------------------------------------------------------
		public virtual void SwitchActiveView(object sender, EventArgs e)
		{
			CheckDisposed();

			SideBarButton button = sender as SideBarButton;

			if (button == null)
				return;

			SwitchActiveView(button.Tag as ISelectableView);
		}
		...
		public void StyleChosenFromStylesComboBox(StyleListItem prevStyle, StyleListItem newStyle)
		{
			...
			if (rootSite != null)
			{
				...
				if (newStyle.IsDefaultParaCharsStyle)
					rootSite.EditingHelper.RemoveCharFormattingWithUndo();
				else
				{
					using (new UndoTaskHelper(rootSite.CastAsIVwRootSite(),
						"kstidUndoStyleChanges", false))
					{
						rootSite.EditingHelper.ApplyStyle(newStyle.Name);
						newStyle.SetInUse(true);
					}
				}
			}
		}
			...
			if (rootSite != null)
			{
				if (type == StyleType.kstCharacter)
				{
					...
					if (styleName == null)
						...
						return FdoResources.DefaultParaCharsStyleName;
				}
				else
				{
		...
		private void m_writingSystemSelector_SelectedIndexChanged(object sender, EventArgs e)
		{
			...
			if (box == null)
				return;
			NamedWritingSystem nws = box.SelectedItem as NamedWritingSystem;
			if (nws == null)
				return;
			if (ActiveView == null)
				return;

			Logger.WriteEvent(string.Format("Applying writing system {0}", nws.Name));

			ActiveView.EditingHelper.ApplyWritingSystem(nws.GetLgWritingSystem(m_cache).Hvo);
		...
		{
			if (m_backupSystem == null || m_fReallyClose)
			{
				// Even with backup/restore in progress, we may want to close this window.
				// See TE-7713.
				e.Cancel = DataUpdateMonitor.IsUpdateInProgress(m_cache.MainCacheAccessor);
			}
			else
			{
				// Backup or restore is in progress, so don't allow the window to be closed.
				// Just hide it for now and set a flag so this window will really be closed
				// after the backup or restore is complete. See TE-7239 and TE-7238
				e.Cancel = true;
				m_fCloseAfterBackupRestore = true;
				Hide();
			}
		...
		{
			if (m_cache != null)
				m_cache.ProgressBar = m_progressHandler;

			base.OnActivated(e);

			if (FwApp.App != null && Visible)
				HandleActivation();
			else
			{
				//TE-4914, et al: This code caused the short cut keys in TE to quit working
				...
				//foreach (Form form in FwApp.App.MainWindows)
				//{
				...
				//{
				//    findReplaceDlg.SetOwner(Cache, ActiveView.CastAsIVwRootSite(), Handle,
				//        FwApp.App.FindPattern(Cache));
				//}
			}
		}
		...
		public int ZoomPercentage
		{
			get
			{
				...
				if (ActiveView is SimpleRootSite)
					return (int)Math.Round((((SimpleRootSite)ActiveView).Zoom * 100));
		...
		public void UpdateWritingSystemsInCombobox()
		{
			...
			m_writingSystemSelector.Items.AddRange(m_cache.LangProject.GetActiveNamedWritingSystems().ToArray());
		}
		...
		public IxCoreColleague[] GetMessageTargets()
		{
			...
			foreach (Control control in ClientWindows.Values)
			{
		...
		#region IRegistryKeyNameModifier Members
		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Create a sub-key of the given key using the current DB server and DB name, for
		/// storing settings that are specific to a particular DB.
		/// </summary>
		...
		/// ------------------------------------------------------------------------------------
		public RegistryKey ModifyKey(RegistryKey key, bool fWriteable)
		{
			CheckDisposed();

			if (m_cache == null)
				return key;
			return key.CreateSubKey(m_cache.ServerName + @"\" + m_cache.DatabaseName + @"\" + Name);
		}
		#endregion
	}
	...
	public class ZoomPercentageChangedEventArgs : EventArgs
	{
		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Initializes a new instance of the <see cref="ZoomPercentageChangedEventArgs"/> class.
		/// </summary>
