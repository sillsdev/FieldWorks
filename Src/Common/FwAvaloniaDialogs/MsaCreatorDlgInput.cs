...
namespace FwAvaloniaDialogs
{
	...
	/// feed their <see cref="MSAGroupBox"/> -- seeded from the existing MSA / morph type (the
	/// legacy
	/// <c>m_msaGroupBox.Initialize(..., sandboxMsa)</c>).
	///
	/// The dialog is essentially the <see cref="MSAGroupBox"/> hosted over the entry's read-only context.
	/// </summary>
	public sealed class MsaCreatorDlgInput
	{
		/// <summary>The dialog window title -- "Create New Grammatical Info." for create, or an
		/// edit-context title.</summary>
		...
		/// editing an existing MSA -- the senses whose MorphoSyntaxAnalysisRA is the original).
		/// Null/empty hides the row.
		/// </summary>
		...
		/// list, fed to BOTH POS choosers inside the hosted <see cref="MSAGroupBox"/>.
		/// </summary>
		public IReadOnlyList<FwPosNode> PosNodes { get; set; } = Array.Empty<FwPosNode>();

		/// <summary>
		/// The MSA class the box opens in -- the type of the existing MSA being edited, or the
		/// entry's desired type for
		...
		/// whenever the MSA box's main POS changes while inflectional, refeeding <see cref="MSAGroupBox.SetSlots"/>.
		...
		/// <c>MsaInflectionFeatureListDlg.PopulateTreeFromPos</c>). Re-run when the MSA box's
		/// MAIN POS changes
		...
		/// <summary>
		/// The inflection-feature assignments selected on open (infl/deriv only) -- the launcher
		/// reads them from the
