			...
			out string strDifference)
		{
			...
			for (int i = 0; i < expected.Count; i++)
			{
				...
				if (expectedNode.Name == actualNode.Name)
				{
					...
					if (expectedNode.InnerText != actualNode.InnerText)
					{
						StringBuilder strBldr = new StringBuilder();
						strBldr.Append("Text of node " + expectedNode.Name + " are not equal:" +
							Environment.NewLine);
						strBldr.Append("  Expected: " + expectedNode.InnerText + Environment.NewLine);
						strBldr.Append("  Actual:   " + actualNode.InnerText);
					}
