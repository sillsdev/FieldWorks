...
# === Targets ===
BUILD_TARGETS=$(BUILD_TARGETS) $(WINDIR)\$(BUILD_PRODUCT).$(BUILD_EXTENSION) $(LIB_DST_DIR)\$(BUILD_PRODUCT).lib

!INCLUDE "$(BUILD_ROOT)\bld\_targ.mak"

$(WINDIR)\$(BUILD_PRODUCT).$(BUILD_EXTENSION): $(BUILD_MAIN)
	$(DISPLAY) osql -S%ComputerName%\SILFW ... -Q"DBCC DebugProcs (FREE)"
	-osql -S%ComputerName%\SILFW -Usa -Pinscrutable -dMaster -h-1 -Q"DBCC DebugProcs (FREE)"
