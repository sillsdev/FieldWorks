...
namespace SIL.FieldWorks.Common.Framework
{
	...
	public abstract class StylesXmlAccessor : SettingsXmlAccessorBase
	{
		...
		/// <inheritdoc />
		protected override string DtdRequiredVersion => "1610190E-D7A3-42D7-8B48-C0C49320435F";

		/// <inheritdoc />
		protected override string RootNodeName => "Styles";

		/// <summary>
		/// List of user-modified styles.
		/// </summary>
		protected virtual List<string> UserModifiedStyles => m_userModifiedStyles;
		#endregion
		...
		protected void ResetProps(StyleInfo styleInfo)
		{
			...
			{
				var styleNode = LoadDoc().SelectSingleNode("markup/tag[@id='" + styleInfo.Name.Replace(" ", "_") + "']");
				SetFontProperties(styleInfo.Name, styleNode, (tpt, iVar, iVal) => styleInfo.SetExplicitFontIntProp(tpt, iVal), (tpt, sVal) =>
				{
		...
		{
			var label = XmlUtils.GetOptionalAttributeValue(m_sourceStyles, "label", "");
			m_progressDlg.Title = string.Format(ResourceHelper.GetResourceString("kstidCreatingStylesCaption"), label);
			m_progressDlg.Message = string.Format(ResourceHelper.GetResourceString("kstidCreatingStylesStatusMsg"), string.Empty);
			...
			//Select all styles.
			var markup = GetMarkupNode(m_sourceStyles);
			var tagList = markup.SelectNodes("tag");
		...
		/// </summary>
		protected virtual string AppDefaultFont => StyleServices.DefaultFont;
		...
		private void CreateAndUpdateStyles(XmlNodeList tagList)
		{
			...
			{
				var attributes = styleTag.Attributes;
				var styleName = GetStyleName(attributes);

				// Don't create a style for certain excluded contexts
				var context = GetContext(attributes, styleName);
				if (IsExcludedContext(context))
				{
					continue;
				}
				m_progressDlg.Step(0);
				m_progressDlg.Message = string.Format(ResourceHelper.GetResourceString("kstidCreatingStylesStatusMsg"), styleName);
				var styleType = GetType(attributes, styleName, context);
				var structure = GetStructure(attributes, styleName);
				var function = GetFunction(attributes, styleName);
				...
				{
					var info = m_htReservedStyles[styleName];
					info.created = true;
				}
				...
				{
					var ws = GetWs(usage.Attributes);
					var usageInfo = usage.InnerText;
				...
				var option = OverwriteOptions.All;
				...
				SetFontProperties(styleName, styleTag,
					...
					(tpt, sVal) => m_progressDlg.SynchronizeInvoke.Invoke(() => propsBldr.SetStrPropValue(tpt, sVal)), option);
		...
		private string GetBasedOn(XmlAttributeCollection attributes, string styleName)
		{
			...
			var basedOn = attributes.GetNamedItem("basedOn");
			return basedOn?.Value.Replace("_", " ");
		}
		...
		private ContextValues GetContext(XmlAttributeCollection attributes, string styleName)
		{
			if (m_htReservedStyles.ContainsKey(styleName))
			{
			...
			var sContext = attributes.GetNamedItem("context").Value;
		...
		private StructureValues GetStructure(XmlAttributeCollection attributes, string styleName)
		{
			if (m_htReservedStyles.ContainsKey(styleName))
			{
		...
		private FunctionValues GetFunction(XmlAttributeCollection attributes, string styleName)
		{
			if (m_htReservedStyles.ContainsKey(styleName))
			{
		...
		public StyleType GetType(XmlAttributeCollection attributes, string styleName, ContextValues context)
		{
			if (m_htReservedStyles.ContainsKey(styleName))
			{
		...
		public bool EnsureCompatibleFactoryStyle(IStStyle style, StyleType type, ContextValues context, StructureValues structure, FunctionValues function)
		{
			// If this is a built-in style, but the context or function has changed, update them.
			if (style.IsBuiltIn && (style.Context != context || style.Function != function) && IsValidInternalStyleContext(style, context))
			{
			...
			if (style.Type != type || !CompatibleContext(style.Context, context) || style.Structure != structure || !CompatibleFunction(style.Function, function))
			{
				if (style.IsBuiltIn)
				{
					ReportInvalidInstallation(string.Format(FrameworkStrings.ksCannotRedefineFactoryStyle, style.Name, ResourceFileName));
				}
		...
		public virtual bool IsValidInternalStyleContext(IStStyle style, ContextValues proposedContext)
		{
		...
		/// <summary>
		/// Determine whether the newly proposed function for a style is compatible with its
		/// current function.
		/// </summary>
		...
		protected void DeleteDeprecatedStylesAndDetermineReplacements()
		{
			...
			foreach (var style in m_htOrigStyles.Values)
			{
				...
				{
					m_progressDlg.Step(0);
					m_progressDlg.Message = string.Format(ResourceHelper.GetResourceString("kstidDeletingStylesStatusMsg"), styleName);
					...
					if (fStyleInUse)
					{
						...
						var change = m_sourceStyles.SelectSingleNode("replacements/change[@old='" + styleName.Replace(" ", "_") + "']");
						if (change != null)
						{
							var replStyleName = change.Attributes.GetNamedItem("new").Value.Replace("_", " ");
							...
							{
								ReportInvalidInstallation(string.Format(FrameworkStrings.ksCannotReplaceXwithY, styleName, replStyleName, ResourceFileName));
							}
						...
						else
						{
							...
							var defaultStyleName = DefaultStyleForContext(style.Context, fIsCharStyle);
		...
		protected virtual void ReplaceFormerStyles()
		{
			...
			var nPrevMax = m_progressDlg.Maximum;
		...
		private void SetFontProperties(string styleName, XmlNode styleTag, Action<int, int, int> setIntProp, Action<int, string> setStrProp, OverwriteOptions options)
		{
			// Get character properties
			var fontNode = styleTag.SelectSingleNode("font");
			var fontAttributes = fontNode.Attributes;
			var attr = fontAttributes.GetNamedItem("spellcheck");
			...
			if (attr != null)
			{
				setIntProp((int)FwTextPropType.ktptItalic, (int)FwTextPropVar.ktpvEnum, GetBoolAttribute(fontAttributes, "italic", styleName, ResourceFileName) ? (int)FwTextToggleVal.kttvInvert : (int)FwTextToggleVal.kttvOff);
			}

			attr = fontAttributes.GetNamedItem("bold");
			if (attr != null)
			{
				setIntProp((int)FwTextPropType.ktptBold, (int)FwTextPropVar.ktpvEnum, GetBoolAttribute(fontAttributes, "bold", styleName, ResourceFileName) ? (int)FwTextToggleVal.kttvInvert : (int)FwTextToggleVal.kttvOff);
			}
			...
			if (attr != null)
			{
				setIntProp((int)FwTextPropType.ktptSuperscript, (int)FwTextPropVar.ktpvEnum, GetBoolAttribute(fontAttributes, "superscript", styleName, ResourceFileName) ? (int)FwSuperscriptVal.kssvSuper : (int)FwSuperscriptVal.kssvOff);
			}

			attr = fontAttributes.GetNamedItem("subscript");
			if (attr != null)
			{
				setIntProp((int)FwTextPropType.ktptSuperscript, (int)FwTextPropVar.ktpvEnum, GetBoolAttribute(fontAttributes, "subscript", styleName, ResourceFileName) ? (int)FwSuperscriptVal.kssvSub : (int)FwSuperscriptVal.kssvOff);
			}
			...
			foreach (XmlNode child in fontNode.ChildNodes)
			{
				...
				{
					var wsId = GetWs(child.Attributes);
					if (wsId == 0)
					{
		...
		private int ColorVal(string val, string styleName)
		{
			...
			if (col.ToArgb() == 0)
			{
				ReportInvalidInstallation(string.Format(FrameworkStrings.ksUnknownUnderlineColor, styleName, ResourceFileName));
			}
		...
		/// <param name="setStrProp">the delegate to set each string property</param>
		/// <param name="options">Indicates which properties to overwrite.</param>
		private void SetParagraphProperties(string styleName, XmlNode styleTag, Action<int, int, int> setIntProp, Action<int, string> setStrProp, OverwriteOptions options)
		{
			var node = styleTag.SelectSingleNode("paragraph");
			if (node == null)
			{
				ReportInvalidInstallation(string.Format(FrameworkStrings.ksMissingParagraphNode, styleName, ResourceFileName));
			}
			var bulletFontInfoNode = node.SelectSingleNode("BulNumFontInfo");
			var paraAttributes = node.Attributes;
			...
			if (node != null)
			{
				...
				switch (sAlign)
				{
					...
					case "full":
						...
						break;
					default:
						ReportInvalidInstallation(string.Format(FrameworkStrings.ksUnknownAlignmentValue, styleName, ResourceFileName));
						break;
				}
			...
			if (node != null)
			{
				var nLeftIndent = InterpretMeasurementAttribute(node.Value, "indentLeft", styleName, ResourceFileName);
			...
			if (node != null)
			{
				var nRightIndent = InterpretMeasurementAttribute(node.Value, "indentRight", styleName, ResourceFileName);
			...
			if (node != null)
			{
				nFirstIndent = InterpretMeasurementAttribute(node.Value, "firstLine", styleName, ResourceFileName);
			...
			if (node != null)
			{
				nHangingIndent = InterpretMeasurementAttribute(node.Value, "hanging", styleName, ResourceFileName);
				fFirstLineOrHangingIndentSpecified = true;
			}

			if (nFirstIndent != 0 && nHangingIndent != 0)
			{
				ReportInvalidInstallation(string.Format(FrameworkStrings.ksInvalidFirstLineHanging, styleName, ResourceFileName));
			}
			...
			if (node != null)
			{
				var nSpaceBefore = InterpretMeasurementAttribute(node.Value, "spaceBefore", styleName, ResourceFileName);
			...
			if (node != null)
			{
				var nSpaceAfter = InterpretMeasurementAttribute(node.Value, "spaceAfter", styleName, ResourceFileName);
			...
			if (node != null)
			{
				var lineSpacing = InterpretMeasurementAttribute(node.Value, "lineSpacing", styleName, ResourceFileName);
				if (lineSpacing < 0)
				{
					ReportInvalidInstallation(string.Format(FrameworkStrings.ksNegativeLineSpacing, styleName, ResourceFileName));
				}
				...
				if (node != null)
				{
					var sLineSpacingType = node.Value;
					switch (sLineSpacingType)
					{
						...
						case "rel":
							...
							break;
						default:
							ReportInvalidInstallation(string.Format(FrameworkStrings.ksUnknownLineSpacingValue, styleName, ResourceFileName));
							break;
					}
				}
			}
			...
			if (node != null)
			{
				...
				switch (node.Value)
				{
					...
					case "trailing":
						...
						break;
					default:
						ReportInvalidInstallation(string.Format(FrameworkStrings.ksUnknownBorderValue, styleName, ResourceFileName));
						break;
				}
			...
			if (node != null)
			{
				setIntProp((int)FwTextPropType.ktptBulNumScheme, (int)FwTextPropVar.ktpvEnum, InterpretBulNumSchemeAttribute(node.Value, styleName, ResourceFileName));
			}
			node = paraAttributes.GetNamedItem("bulNumStartAt");
			if (node != null)
			{
				int nVal;
				if (!int.TryParse(node.Value, out nVal))
				{
					ReportInvalidInstallation(String.Format(FrameworkStrings.ksUnknownBulNumStartAtValue, styleName, ResourceFileName));
					nVal = 0;
				}
		...
		private void SetBulNumFontInfoProperties(string styleName, Action<int, string> setStrProp, XmlNode bulletFontInfoNode)
		{
			var propsBldr = TsStringUtils.MakePropsBldr();
			int type;
			var bulNumFontAttributes = bulletFontInfoNode.Attributes;
			if (bulNumFontAttributes == null)
			{
				return;
			}
			var attr = bulNumFontAttributes.GetNamedItem("italic");
			if (attr != null)
			{
		...
		{
			var wsId = attribs.GetNamedItem("wsId").Value;
		...
		private void SetBasedOnAndNextProps(XmlNodeList tagList)
		{
			foreach (XmlNode styleTag in tagList)
			{
				...
				m_progressDlg.Step(0);
				m_progressDlg.Message = string.Format(ResourceHelper.GetResourceString("kstidUpdatingStylesStatusMsg"), styleName);
				var attributesWithBasedOn = styleTag.Attributes;
				if (style.Type == StyleType.kstParagraph)
				{
				...
				{
					var sBasedOnStyleName = GetBasedOn(attributesWithBasedOn, styleName);
					if (string.IsNullOrEmpty(sBasedOnStyleName))
					{
						ReportInvalidInstallation(string.Format(FrameworkStrings.ksMissingBasedOnStyle, styleName, ResourceFileName));
					}
					if (!m_htUpdatedStyles.ContainsKey(sBasedOnStyleName))
					{
						ReportInvalidInstallation(string.Format(FrameworkStrings.ksUnknownBasedOnStyle, styleName, sBasedOnStyleName));
					}
					var basedOnStyle = m_htUpdatedStyles[sBasedOnStyleName];
					if (basedOnStyle.Hvo == style.Hvo)
					{
						ReportInvalidInstallation(string.Format(FrameworkStrings.ksNoBasedOnSelf, styleName, ResourceFileName));
					}
				...
				if (m_htReservedStyles.ContainsKey(styleName))
				{
					sNextStyleName = m_htReservedStyles[styleName].nextStyle;
				}
				else
				{
					var next = attributesWithBasedOn.GetNamedItem("next");
					if (next != null)
					{
				...
				if (!string.IsNullOrEmpty(sNextStyleName))
				{
					...
					{
						ReportInvalidInstallation(string.Format(FrameworkStrings.ksUnknownNextStyle, styleName, sNextStyleName, ResourceFileName));
					}
					style.NextRA = m_htUpdatedStyles.ContainsKey(sNextStyleName) ? m_htUpdatedStyles[sNextStyleName] : null;
				}
			}
		...
		{
			foreach (var styleName in m_htReservedStyles.Keys)
			{
				var info = m_htReservedStyles[styleName];
				if (!info.created)
				{
		...
		public static bool GetBoolAttribute(XmlAttributeCollection attributes, string sAttrib, string styleName, string fileName)
		{
			var sVal = attributes.GetNamedItem(sAttrib).Value;
			if (sVal == "yes" || sVal == "true")
			{
				return true;
			}
			if (sVal == "no" || sVal == "false" || sVal == string.Empty)
			{
				return false;
			}
			ReportInvalidInstallation(string.Format(FrameworkStrings.ksUnknownStyleAttribute, sAttrib, styleName, fileName));
		...
		public static int InterpretMeasurementAttribute(string sSize, string sAttrib, string styleName, string fileName)
		{
			sSize = sSize.Trim();
			if (sSize.Length >= 4)
			{
				...
				ReportInvalidInstallation(string.Format(FrameworkStrings.ksUnknownAttrUnits, sAttrib, styleName, fileName));
			}
			return 0;
		}
		...
		public static int InterpretBulNumSchemeAttribute(string sScheme, string styleName, string fileName)
		{
			...
			ReportInvalidInstallation(string.Format(FrameworkStrings.ksUnknownBulNumSchemeValue, styleName, fileName));
