...
{
#if RANDYTODO
	// TODO: Way too much class structure for only one real styles class these days.
	// ExternalSettingsAccessorBase<T> (abstract: Framework)
	//	SettingsXmlAccessorBase (abstract: Framework)
	//		StylesXmlAccessor (abstract: Framework)
	//			FlexStylesXmlAccessor (concrete: LanguageExplorer)
	//			TestAccessorForFindOrCreateStyle (concrete: Framework tests)
	// TODO: Move all of the styles classes into LE. The styles test class then moves into LE test assembly
#endif
	/// <summary>
	...
	public abstract class ExternalSettingsAccessorBase<T>
	{
		...
		/// other than ".xml".
		/// </summary>
		protected virtual string ResourceFileName => ResourceName + ".xml";
		#endregion
		...
		protected static void ReportInvalidInstallation(string message)
		{
		...
		protected static void ReportInvalidInstallation(string message, Exception e)
		{
		...
		public void EnsureCurrentResource(IThreadedProgress progressDlg)
		{
			...
			{
				ReportInvalidInstallation(string.Format(FrameworkStrings.ksInvalidResourceFileVersion, ResourceFileName), e);
		...
		{
			return ResourceList.ToArray().FirstOrDefault(res => res.Name.Equals(resourceName));
		}
