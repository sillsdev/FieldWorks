...
namespace SIL.FieldWorks.XWorks
{
	...
	public class ExportDialog : Form, IFWDisposable
	{
		...
		private void ExportWsAsLDML(string sDirectory)
		{
			...
			{
				using (XmlWriter writer = XmlWriter.Create(Path.Combine(sDirectory, ws.Id + ".ldml"), writerSettings))
				{
					ws.WriteLdml(writer);
					writer.Close();
				}
			...
			using (new WaitCursor(this))
			{
				using (var progressDlg = new ProgressDialogWithTask(this))
				{
					...
					catch (WorkerThreadException e)
					{
						...
						else if (e.InnerException is LiftIO.LiftFormatException)
						{
		...
		/// ------------------------------------------------------------------------------------
		private object ExportFxt(IProgress progressDialog, object[] parameters)
		{
		...
		/// <returns>Always null.</returns>
		/// ------------------------------------------------------------------------------------
		protected object ExportConfiguredDocView(IProgress progressDlg, object[] parameters)
		{
			...
			using (TextWriter w = new StreamWriter(outPath))
			{
				...
				m_ce.Initialize(m_cache, w, ft.m_sDataType, ft.m_sFormat, outPath, sBodyClass);
		...
		protected object ExportTranslatedLists(IProgress progressDlg, object[] parameters)
		{
			string outPath = (string)parameters[0];
			m_progressDlg = progressDlg;
