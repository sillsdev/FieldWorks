...
namespace SIL.FieldWorks.XWorks
{
	public partial class XmlDocConfigureDlg : Form, IFWDisposable
	{
		...
		List<StyleComboItem> m_rgStyles;
		...
		private void CreateComboAndTreeItems(string sLayoutType)
		{
			...
			foreach (XmlNode xnLayoutType in configureLayouts.ChildNodes)
			{
				...
				if (sLabel == "$wsName")
				{
					...
					if (!fReversalIndex)
						continue;
					int flid = BaseVirtualHandler.GetInstalledHandlerTag(m_cache, "LexDb", "CurrentReversalIndices");
					int[] rghvoIndexes = m_cache.GetVectorProperty(m_cache.LangProject.LexDbOA.Hvo, flid, false);
					foreach (int hvoIndex in rghvoIndexes)
					{
						int ws = m_cache.GetObjProperty(hvoIndex, (int)ReversalIndex.ReversalIndexTags.kflidWritingSystem);
						ILgWritingSystem lgws = LgWritingSystem.CreateFromDBObject(m_cache, ws);
						string sWsTag = lgws.ICULocale;
						...
						XmlNode xnRealLayout = CreateWsSpecficLayoutType(xnLayoutType,
							lgws.Name.UserDefaultWritingSystem, sLayout.Replace("$ws", sWsTag), sWsTag);
		...
		{
			if (m_rgStyles == null)
				m_rgStyles = new List<StyleComboItem>();
			m_rgStyles.Clear();
			m_rgStyles.Add(new StyleComboItem(null));
			foreach (BaseStyleInfo sty in m_styleSheet.Styles)
			{
				if (sty.IsCharacterStyle)
					m_rgStyles.Add(new StyleComboItem(sty));
			}
			m_rgStyles.Sort();
		}
		...
		private void m_btnRestoreDefaults_Click(object sender, EventArgs e)
		{
			...
			{
				LayoutCache.InitializePartInventories(null);
			...
			foreach (ListViewItem lvi in m_lvWritingSystems.CheckedItems)
			{
				...
				if (lvi.Tag is int)
				{
					...
					switch (ws)
					{
						...
						case LangProject.kwsPronunciation:		sWs = "pronunciation";	break;
						case LangProject.kwsReversalIndex:		sWs = "reversal";		break;
					}
				}
				else if (lvi.Tag is ILgWritingSystem)
				{
					sWs = (lvi.Tag as ILgWritingSystem).ICULocale;
				}
				if (sbLabel.Length > 0)
					sbLabel.Append(",");
				sbLabel.Append(sWs);
		...
		private void DisplayDetailsForTopLevelNode()
		{
			...
			//m_chkDisplayData.Checked = true;	// now layout attribute dependant
			m_chkDisplayData.Enabled = false;
		...
		private void InitializeWsListView()
		{
			...
			switch (m_current.WsType)
			{
				case "analysis":
					...
					for (int i = 0; i < m_cache.LangProject.CurAnalysisWssRS.Count; ++i)
					{
						ILgWritingSystem lws = m_cache.LangProject.CurAnalysisWssRS[i];
						lvi = new ListViewItem(lws.Name.UserDefaultWritingSystem);
						lvi.Tag = lws;
						m_lvWritingSystems.Items.Add(lvi);
					}
					break;
				case "vernacular":
					...
					lvi.Tag = wsDefault;
					m_lvWritingSystems.Items.Add(lvi);
					for (int i = 0; i < m_cache.LangProject.CurVernWssRS.Count; ++i)
					{
						ILgWritingSystem lws = m_cache.LangProject.CurVernWssRS[i];
						lvi = new ListViewItem(lws.Name.UserDefaultWritingSystem);
						lvi.Tag = lws;
						m_lvWritingSystems.Items.Add(lvi);
					}
				...
				case "pronunciation":
					...
					wsDefault = LangProject.kwsPronunciation;
					lvi.Tag = wsDefault;
					m_lvWritingSystems.Items.Add(lvi);
					ILgWritingSystem[] allPronunciations = LabeledMultiStringView.PronunciationWritingSystems(m_cache);
					foreach (ILgWritingSystem lws in allPronunciations)
					{
						lvi = new ListViewItem(lws.Name.UserDefaultWritingSystem);
						lvi.Tag = lws;
						m_lvWritingSystems.Items.Add(lvi);
				...
				case "reversal":
					...
					wsDefault = LangProject.kwsReversalIndex;
					lvi.Tag = wsDefault;
					m_lvWritingSystems.Items.Add(lvi);
					for (int i = 0; i < m_cache.LangProject.CurAnalysisWssRS.Count; ++i)
					{
			...
			switch (m_current.WsLabel)
			{
				...
				case "vernacular":
					...
					break;
				case "pronunciation":
					SetDefaultWritingSystem(LangProject.kwsPronunciation);
					break;
				case "reversal":
					SetDefaultWritingSystem(LangProject.kwsReversalIndex);
					break;
				case "all reversal":
				case "all analysis":
				case "all vernacular":
					...
					{
						ILgWritingSystem lws = m_lvWritingSystems.Items[i].Tag as ILgWritingSystem;
						if (lws != null)
							m_lvWritingSystems.Items[i].Checked = true;
					}
					break;
				default:	// must be explicit ws tags.
					string[] rgws = m_current.WsLabel.Split(new char[] { ',' });
					int indexTarget = 0;
					for (int i = 0; i < m_lvWritingSystems.Items.Count; ++i)
					{
					...
					for (int i = 0; i < rgws.Length; ++i)
					{
						...
						{
							ILgWritingSystem lws = m_lvWritingSystems.Items[iws].Tag as ILgWritingSystem;
							if (lws != null && lws.ICULocale == sLabel)
							{
						...
						if (!fChecked)
						{
							...
							int ws = m_cache.LanguageWritingSystemFactoryAccessor.GetWsFromStr(sLabel);
							if (ws != 0)
							{
								IWritingSystem xws = m_cache.LanguageWritingSystemFactoryAccessor.get_EngineOrNull(ws);
								if (xws != null)
								{
									ILgWritingSystem lws = LgWritingSystem.CreateFromDBObject(m_cache, ws);
									lvi = new ListViewItem(lws.Name.UserDefaultWritingSystem);
									lvi.Tag = lws;
									// Go ahead and select it, since the display is configured to show it.
									lvi.Checked = true;
									fChecked = true;
									m_lvWritingSystems.Items.Insert(indexTarget++, lvi);
								}
							}
						}
					}
		...
		private void DisplayStyleControls(bool fEnabled)
		{
			if (m_current.AllowCharStyle)
			{
				...
				m_cbCharStyle.Items.AddRange(m_rgStyles.ToArray());
				for (int i = 0; i < m_rgStyles.Count; ++i)
				{
					if (m_rgStyles[i].Style != null &&
						m_rgStyles[i].Style.Name == m_current.StyleName)
					{
		...
		private void SaveModifiedLayouts()
		{
			...
			if (m_fDeleteCustomFiles)
			{
				...
				m_layouts.DeleteUserOverrides(m_cache.DatabaseName);
				m_layouts.DeleteFlexUserOverrides(m_cache.DatabaseName);
				m_parts.DeleteUserOverrides(m_cache.DatabaseName);
				m_parts.DeleteFlexUserOverrides(m_cache.DatabaseName);
				Inventory.SetInventory("layouts", m_cache.DatabaseName, m_layouts);
				Inventory.SetInventory("parts", m_cache.DatabaseName, m_parts);
		...
		public class LayoutTreeNode : TreeNode
		{
			...
			internal bool IsNodeDirty()
			{
				...
				if (Level > 0)
				{
					...
					{
						string sVisibility = XmlUtils.GetOptionalAttributeValue(m_xnConfig, "visibility", "always");
						...
						if (sWsLabel != null)
						{
							if (sWsLabel.StartsWith(LangProject.WsParamLabel))
								sWsLabel = sWsLabel.Substring(LangProject.WsParamLabel.Length);
						}
