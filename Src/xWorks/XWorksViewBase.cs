...
namespace SIL.FieldWorks.XWorks
{
	...
	public abstract class XWorksViewBase : XCoreUserControl, IxCoreContentControl, IPaneBarUser
	{
		...
		{
			CheckDisposed();
			ConfigureCustomFields(new Tuple<PropertyTable, string>(m_propertyTable, m_propertyTable.GetStringProperty("areaChoice", string.Empty)));
			return true; // handled
		}
		...
		{
			if (!(propTableAndArea is Tuple<PropertyTable, string> args))
			{
				throw new ArgumentException(nameof(propTableAndArea));
			}

			var propertyTable = args.Item1;
			var areaChoice = args.Item2;
			var cache = propertyTable.GetValue<LcmCache>("cache");

			if (SharedBackendServices.AreMultipleApplicationsConnected(cache))
			{
				MessageBoxUtils.Show(Form.ActiveForm, xWorksStrings.ksCustomFieldsCanNotBeAddedDueToOtherAppsText,
			...
			using (var dlg = new AddCustomFieldDlg(propertyTable, locationType))
			{
