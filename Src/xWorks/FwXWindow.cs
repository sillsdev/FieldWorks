...
namespace SIL.FieldWorks.XWorks
{
		...
		IMainWindowDelegatedFunctions, IMainWindowDelegateCallbacks, IFindAndReplaceContext
	{
		...
		private void ReconstructViews(Control control)
		{
			...
			var rootSite = control as IVwRootSite;
		...
		private void ImportTranslatedLists(string filename)
		{
			...
			if (form == null)
				form = this;
			using (var dlg = new ProgressDialogWithTask(form))
			{
		...
		/// </summary>
		public object ImportTranslatedLists(IProgress dlg, object[] parameters)
		{
			...
			foreach (IVwRootBox rootb in FindAllRootBoxes(this))
			{
				...
				if (fss != null && fss.Cache != null)
				{
					...
					if (fss != fssPrev)
					{
						...
						Debug.Assert(fss.StyleListTag == LangProjectTags.kflidStyles);
						Debug.Assert(Cache.ServiceLocator.GetInstance<ICmObjectRepository>().GetClsid(fss.RootObjectHvo) == LangProjectTags.kClassId);
#endif
	...
	/// </summary>
	internal interface IRefreshCache
	{
		void Refresh();
	}
}
