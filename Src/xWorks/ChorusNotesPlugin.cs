...
namespace SIL.FieldWorks.XWorks
{
	...
	///
	/// PHASE-1 FOLLOW-UP SURFACE: per the base-PR scoping decision, this plugin is a deliberately
	/// INERT follow-up — the same status as the Avalonia browse table
	/// (<see cref="SIL.FieldWorks.Common.FwAvalonia.LexicalEditSurfaceResolver"/>'s
	/// <c>Phase1FollowUpBrowseTools</c>) and the interlinear/rule-formula plugins named in
	/// <see cref="RegionEditorPluginRegistry.RegisterBuiltins"/>. <see cref="IsPhase1FollowUp"/> is
	/// the discoverable marker for that status; <see cref="RegionEditorPluginRegistry.RegisterBuiltins"/>
	/// does NOT register this plugin in the base PR, so the composer never resolves it for the
	/// base surfaces (lexiconEdit/lexiconEditPopup/notebookEdit/posEdit) — the Messages node falls
	/// back to the companion-designated set (empty) and then the read-only placeholder row (never a
	/// crash — see FullEntryRegionMessagesCompanionTests). The phase1-followup-table branch/PR
	/// (bundled with the browse table it ships alongside) activates this plugin by restoring the
	/// registration in RegisterBuiltins, at which point this marker should flip to false (or be removed).
	/// </summary>
	public sealed class ChorusNotesPlugin : IRegionEditorPlugin
	{
		/// <summary>
		/// True while this plugin is a Phase-1 follow-up surface (not registered in the base PR).
		/// Mirrors the naming/intent of <c>LexicalEditSurfaceRegistry.Phase1FollowUpSurfaceTools</c>
		/// so the "inert until a follow-up flips it on" status is discoverable directly on the type,
		/// not just inferred from the absence of a registration call.
		/// </summary>
		public static readonly bool IsPhase1FollowUp = true;
		...
		{
			// Belt-and-suspenders: even if something registers this plugin ahead of the follow-up PR
			// flipping IsPhase1FollowUp, BuildControl still refuses to render — the gate lives on the
			// plugin itself, not only at the registration call site. The composer's null-factory-result
			// guard degrades this to the same read-only placeholder row as an unclaimed class.
			if (IsPhase1FollowUp)
				return null;
