...
namespace SIL.FieldWorks.XWorks
{
	...
	public partial class RecordEditView
	{
		...
		private void OnDetailMenuRequested(DetailMenuRequest request)
		{
			try
			{
				...
				try
				{
					...
					if (items.Count > 0)
					{
						// A keyboard gesture anchors the menu under the row it came from; a
						// pointer gesture opens it at the pointer.
						m_avaloniaEntryForm.ShowContextMenu(items, request.AnchorControl,
							request.IsPointerGesture);
						return;
					}
				}
		...
		// The adapter fallback needs a raw screen point: cursor position for a pointer
		// gesture, the anchor's bottom-left otherwise. Both corners are mapped since
		...
		private static System.Drawing.Point AdapterMenuScreenPoint(DetailMenuRequest request)
		{
			...
			if (request.IsPointerGesture || anchor == null)
