...
namespace SIL.FieldWorks.XWorks
{
	...
	/// implementation: owns the lazily opened fenced <see cref="LcmDetailEditSession"/> (opened
	...
	public abstract class DetailEditContextBase : IDetailEditContext
	{
		...
		/// <summary>The record this context edits -- a LexEntry for the lexicon tool, but typed
		/// as any
		...
		/// <inheritdoc />
		public abstract bool TrySetText(DetailField detailField, string ws, string value);

		/// <inheritdoc />
		public virtual bool TrySetRichText(DetailField detailField, string ws,
		...
		public virtual IReadOnlyList<string> Validate()
		{
			...
			// rule
			// applies ONLY when the root is a LexEntry; other record classes (RnGenericRec,
			// CmPossibility, ...)
		...
		/// "Undo/Redo Edit Entry" label is used -- the batch/bulk path that stages several fields
		/// with no
		...
		protected void EnsureOpen(string fieldLabel)
		{
			...
			_session = new LcmDetailEditSession(Cache, undoLabel, redoLabel);
		}

		/// <summary>
		/// Stages an arbitrary domain write inside THIS context's fenced session (opening it on the first
		/// edit), so a write routed from outside the registered setter dictionaries -- e.g. a
		/// plugin
		/// editor's own field (the Reversal Entries plugin) -- still rides the SAME undoable step
		/// as every
