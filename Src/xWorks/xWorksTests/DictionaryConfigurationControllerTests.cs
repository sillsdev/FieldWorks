...
namespace SIL.FieldWorks.XWorks
{
	...
	public class DictionaryConfigurationControllerTests : MemoryOnlyBackendProviderRestoredForEachTestTestBase
	{
		...
		{
			CollectionAssert.That(DictionaryConfigurationController.GetCustomFieldsForType(Cache, "LexEntry"), Is.Empty);
		}
		...
		public void GetCustomFieldsForType_EntryCustomFieldIsRepresented()
		{
				...
				CellarPropertyType.MultiString, Guid.Empty))
			{
				...
				CollectionAssert.That(customFieldNodes, Is.Not.Empty);
		...
		public void GetCustomFieldsForType_PossibilityListFieldGetsChildren()
		{
				...
				CellarPropertyType.OwningAtomic, Cache.LanguageProject.LocationsOA.Guid))
			{
				var customFieldNodes = DictionaryConfigurationController.GetCustomFieldsForType(Cache,
					"LexEntry");
				CollectionAssert.That(customFieldNodes, Is.Not.Empty, "The custom field configuration node was not inserted for a PossibilityListReference");
				Assert.That("CustomListItem", Is.EqualTo(customFieldNodes[0].Label), "Custom field did not get inserted correctly.");
				var cfChildren = customFieldNodes[0].Children;
				CollectionAssert.That(cfChildren, Is.Not.Empty, "ListItem Child nodes not created");
				...
				CollectionAssert.That(cfChildren.Where(t => t.Label == "Name" && !t.IsCustomField), Is.Not.Empty, "No standard Name node found on custom possibility list reference");
				CollectionAssert.That(cfChildren.Where(t => t.Label == "Abbreviation" && !t.IsCustomField), Is.Not.Empty, "No standard Abbreviation node found on custom possibility list reference");
				...
				CollectionAssert.That(wsOptions.Options.Where(o => o.IsEnabled), Is.Not.Empty, "No default writing system added.");
			}
		}
		...
		public void GetThePublicationsForTheCurrentConfiguration()
		{
			...
			Assert.That("A", Is.EqualTo(controller.AffectedPublications));

			m_model.Publications = new List<string> { "A", "B" };
			Assert.That("A, B", Is.EqualTo(controller.AffectedPublications));
		}
		...
		public void MergeCustomFieldsIntoDictionaryModel_NewFieldsAreAdded()
		{
				...
				WritingSystemServices.GetMagicWsIdFromName("analysis vernacular"), CellarPropertyType.MultiString, Guid.Empty))
			{
				...
				CollectionAssert.That(children, Is.Not.Empty, "Custom Field did not add to children");
				var cfNode = children[0];
				Assert.That("CustomString", Is.EqualTo(cfNode.Label));
				Assert.That("CustomString", Is.EqualTo(cfNode.FieldDescription));
				Assert.That(true, Is.EqualTo(cfNode.IsCustomField));
				...
				CollectionAssert.That(wsOptions.Options.Where(o => o.IsEnabled), Is.Not.Empty, "WsOptions not populated with any choices");
			}
		}
		...
		public void MergeCustomFieldsIntoDictionaryModel_NewFieldsOnSharedNodesAreAddedToSharedItemsExclusively()
		{
				...
				CellarPropertyType.ReferenceCollection, Guid.Empty))
			{
				...
				Assert.That("CustomString", Is.EqualTo(customNode.Label));
				Assert.That("CustomString", Is.EqualTo(customNode.FieldDescription));
				Assert.That(true, Is.EqualTo(customNode.IsCustomField));
