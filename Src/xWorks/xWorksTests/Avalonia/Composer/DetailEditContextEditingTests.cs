...
namespace SIL.FieldWorks.XWorks
{
	...
	public class DetailEditContextEditingTests : MemoryOnlyBackendProviderTestBase
	{
		...
		{
			var refreshes = 0;
			var controller = new AvaloniaDetailRefreshController(
				Cache, () => m_entry, () => false, () => refreshes++, new RefreshCoordinator());
			controller.Dispose();
	...
	public class DetailComposerTests : MemoryOnlyBackendProviderTestBase
	{
		...
		// One compose compiles each distinct (class, layout, choice, callerXml) once, so more
		// senses/examples/allomorphs must not build and fingerprint more snapshots.
		[Test]
		public void Compose_MoreItemsOfAlreadyCompiledClasses_AddNoCompileOrFingerprintWork()
		{
			NonUndoableUnitOfWorkHelper.Do(Cache.ActionHandlerAccessor, () =>
			{
				AddExample(m_entry.SensesOS[0], "una casa");
			...
			Assert.That(DetailComposer.Compose(m_entry, Cache), Is.Not.Null,
				"the priming compose fills the compiler cache");

			var baseline = MeasureCompose();
			Assert.That(baseline.Fingerprints, Is.GreaterThan(0),
				"a compose keys every compiled layout by its source fingerprint");
			Assert.That(baseline.Compiles, Is.Zero,
				"a repeat compose over unchanged sources must hit the compiler cache");
			...
			NonUndoableUnitOfWorkHelper.Do(Cache.ActionHandlerAccessor, () =>
			{
				...
				for (var i = 0; i < 3; i++)
				{
					...
					m_entry.SensesOS.Add(sense);
					sense.Gloss.set_String(Cache.DefaultAnalWs,
						TsStringUtils.MakeString("extra gloss " + (i + 1), Cache.DefaultAnalWs));
					AddExample(sense, "extra example " + (i + 1));
				}
				m_entry.AlternateFormsOS.Add(
					Cache.ServiceLocator.GetInstance<IMoStemAllomorphFactory>().Create());
			});

			var grown = MeasureCompose();
			Assert.That(grown.Compiles, Is.Zero,
				"more items of already-compiled classes must not recompile any layout");
			Assert.That(grown.Fingerprints, Is.EqualTo(baseline.Fingerprints),
				$"more items must not fingerprint more snapshots (baseline {baseline.Fingerprints}, "
				+ $"grown {grown.Fingerprints})");
			Assert.That(grown.PartsHashes, Is.Zero,
				"the shipped parts source is one string instance, hashed once per process");
		}

		private void AddExample(ILexSense sense, string text)
		{
		...
		private (int Compiles, int Fingerprints, int PartsHashes) MeasureCompose()
		{
			var compilesBefore = ViewDefinitionCompiler.CompileCount;
			var fingerprintsBefore = ViewDefinitionSourceSnapshot.FingerprintComputeCount;
			var partsHashesBefore = ViewDefinitionSourceSnapshot.PartsHashComputeCount;
			var composed = DetailComposer.Compose(m_entry, Cache);
			Assert.That(composed, Is.Not.Null, "the shipped layouts must compose");
			Assert.That(composed.Model.Fields, Is.Not.Empty, "the measured compose still composes fully");
			return (ViewDefinitionCompiler.CompileCount - compilesBefore,
				ViewDefinitionSourceSnapshot.FingerprintComputeCount - fingerprintsBefore,
				ViewDefinitionSourceSnapshot.PartsHashComputeCount - partsHashesBefore);
		}
		...
		{
			var before = DetailComposer.Compose(m_entry, Cache).Model.Fields;
		...
		public void Compose_RichAlternative_StaysEditable_AndAKeystrokePreservesUntouchedRuns()
		{
			...
			NonUndoableUnitOfWorkHelper.Do(Cache.ActionHandlerAccessor, () =>
				m_entry.Bibliography.set_String(Cache.DefaultAnalWs,
					TsStringUtils.MakeString("Smith 1999", Cache.DefaultAnalWs)));
			var plain = DetailComposer.Compose(m_entry, Cache).Model.Fields
		...
		public void Build_RichLexemeForm_StaysEditable_AndRoundTripsRunMetadata()
		{
			...
			var roundTripped = DetailRichTextAdapter.ToTsString(form.Values.Single().RichText,
		...
		public void Workflow_StyleSpan_RetagSpan_InsertLink_RoundTripsThroughTheComposedContext()
		{
			...
			var detail = DetailComposer.Compose(m_entry, Cache);
			var field = detail.Model.Fields.Single(f => f.Field == "Bibliography");
			...
			var reopened = DetailComposer.Compose(m_entry, Cache).Model.Fields
			...
			Cache.ActionHandlerAccessor.Undo();
			var afterUndo = DetailComposer.Compose(m_entry, Cache).Model.Fields
				.Single(f => f.Field == "Bibliography").Values.Single().RichText;
		...
		public void Compose_RunWithUnsupportedProperty_ComposesReadOnly_SoAnEditCannotDropIt()
		{
			...
			var rich = DetailComposer.Compose(m_entry, Cache).Model.Fields
				.Single(f => f.Field == "Bibliography");
			...
			var roundTripped = DetailRichTextAdapter.ToTsString(rich.Values.Single().RichText,
		...
		public void Edit_ApplySpanFormatting_BoldOverSpan_RoundTripsThroughToTsString()
		{
			...
			var composedBefore = DetailComposer.Compose(m_entry, Cache);
		...
		public void ReversalPlugin_EditingAForm_StagesAndCommitsThroughTheEditContext()
		{
			...
			var buildContext = new SlicePluginBuildContext(sense, null, () => composed.EditContext, Cache);
			...
			// plugin's ReversalDetailEditContext. Address it directly to assert the data path.
		...
		public void RoundTrip_UneditedValue_ReproducesOriginalTsString()
		{
			...
			var value = DetailComposer.Compose(m_entry, Cache).Model.Fields
				.Single(f => f.Field == "Bibliography").Values.Single();

			var roundTripped = DetailRichTextAdapter.ToTsString(value.RichText,
				Cache.WritingSystemFactory, Cache.DefaultAnalWs);
		...
		{
			var normal = DetailComposer.Compose(m_entry, Cache).Model.Fields;
			var hidden = DetailComposer.Compose(m_entry, Cache, showHiddenFields: true).Model.Fields;
		...
		public void ResolveWritingSystems_MatchesLegacyWritingSystemServices(string spec)
		{
			...
			var actual = DetailComposer.ResolveWritingSystems(Cache, spec);
		...
		public void ResolveWritingSystems_Pronunciation_UsesTheProjectPronunciationList()
		{
			...
			try
			{
				var actual = DetailComposer.ResolveWritingSystems(Cache, "pronunciation");
		...
		public void Compose_GhostLexemeForm_CreatesTheAllomorph_OnFirstEdit()
		{
			...
			var composed = DetailComposer.Compose(bare, Cache);
		...
		public void Compose_GhostSenses_CreateASense_WithTheTypedGloss()
		{
			...
			var composed = DetailComposer.Compose(bare, Cache, showHiddenFields: true);
			var ghost = composed.Model.Fields.FirstOrDefault(f =>
		...
		public void Compose_EntryRefConditionals_VariantAndComplexForm_ComposeDifferently()
		{
			...
			var variantFields = DetailComposer.Compose(m_entry, Cache).Model.Fields;
			...
			var complexFields = DetailComposer.Compose(m_entry, Cache).Model.Fields;
		...
		public void Compose_InfixPosition_ChoiceWhereGuidEquals_FollowsTheMorphType()
		{
			...
			var infixFields = DetailComposer.Compose(affixEntry, Cache).Model.Fields;
			...
			var prefixFields = DetailComposer.Compose(affixEntry, Cache).Model.Fields;
		...
		public void Compose_MsEnvFeatures_TargetOwnerCondition_ReadsTheOwningEntry()
		{
			...
			var withoutMsa = DetailComposer.Compose(affixEntry, Cache).Model.Fields;
			...
			var withMsa = DetailComposer.Compose(affixEntry, Cache).Model.Fields;
	...
	public class DetailComposerCustomFieldTests : MemoryOnlyBackendProviderTestBase
	{
		...
		public void Compose_CustomFields_ExpandAtThePlaceholder_WithLegacyLabelsAndValues()
		{
			...
			var expectedWs = DetailComposer.ResolveWritingSystems(Cache, "analysis vernacular");
		...
		public void Compose_CustomFields_ExpandInNestedAllomorphAndExampleLayouts()
		{
			...
			var affixFields = DetailComposer.Compose(affixEntry, Cache).Model.Fields;
		...
		public void Compose_EmptyCustomFields_StayVisible_LikeLegacyAlwaysVisibility()
		{
			...
			{
				var fields = DetailComposer.Compose(bare, Cache, showHidden).Model.Fields;
