...
namespace SIL.FieldWorks.XWorks
{
	...
	public class BulkEditBarTests : BulkEditBarTestsBase
	{
		...
		public void Allomorphs_IsAbstractForm()
		{
			...
			Assert.That(allomorphs.Count + entriesWithoutAllomorphs.Count + 1, Is.EqualTo(clerk.ListSize));

			// make sure we're on the first allomorph (e.g. the LexemeForm) of the entry we changed from
			Assert.That(clerk.CurrentObject.Hvo, Is.EqualTo(firstEntryWithAllomorph.LexemeFormOA.Hvo));
			// change the first allomorphs's IsAbstract to something else
			Assert.That(firstEntryWithAllomorph.LexemeFormOA.IsAbstract, Is.EqualTo(false));
			m_bv.OnUncheckAll();
			...
			Assert.That(firstEntryWithAllomorph.LexemeFormOA.IsAbstract, Is.EqualTo(Convert.ToBoolean(item.Value)));
