...
#pragma warning restore 67
		...
		public void GetListItems()
		{
			...
			StringAssert.That(listItems[0].Text, Does.Contain(xWorksStrings.ksNoComplexFormType));
			...
			StringAssert.That(listItems[0].Text, Does.Contain(xWorksStrings.ksNoVariantType));
			...
			StringAssert.That(listItems[0].Text, Does.Contain(xWorksStrings.ksNoVariantType));
			Assert.That(listItems[0].Tag, Is.EqualTo(XmlViewsUtils.GetGuidForUnspecifiedVariantType().ToString()));
			StringAssert.That(listItems[variantCount + 1].Text, Does.Contain(xWorksStrings.ksNoComplexFormType), "<No Complex Form Type> should immediately follow the Variant Types");
		...
		private List<ListViewItem> VerifyGetListItems(DictionaryNodeListOptions.ListIds listId, int expectedCount)
		{
			...
			StringAssert.That(label, Does.Contain(listId.ToString()));
			return result;
		}
		...
		public void UsersAreNotifiedOfSharedItems()
		{
			...
			using (var view = new TestDictionaryDetailsView())
			{
				...
				StringAssert.That(tooltip, Does.Contain("LexEntry > SensesOS > Subentries"));
				StringAssert.That(tooltip, Does.Contain("LexEntry > Subentries > Subentries"));
				...
				StringAssert.That(tooltip, Does.Contain("LexEntry > Subentries."), "Tooltip should indicate the Master Parent's location");
				StringAssert.That(tooltip, Does.Contain("LexEntry > Subentries > Subentries"), "Tooltip should indicate the node's full path");
