...
//		}
		...
		public 	void DoInsertAndDeletionTest(string insertCmdId)
		{
			...
			for(int i=0; i<m_howManyRecords; i++)
			{
				...
				SIL.FieldWorks.FDO.Ling.PartOfSpeech p = (SIL.FieldWorks.FDO.Ling.PartOfSpeech)CorrespondingRecordClerk.CurrentObject;
				p.Name.AnalysisDefaultWritingSystem = labels[i % labels.Length] + i.ToString()+" delete";
