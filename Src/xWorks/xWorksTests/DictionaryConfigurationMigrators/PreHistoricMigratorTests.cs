...
namespace SIL.FieldWorks.XWorks.DictionaryConfigurationMigrators
{
	public class PreHistoricMigratorTests : MemoryOnlyBackendProviderRestoredForEachTestTestBase
	{
		...
		public void ConvertLayoutTreeNodeToConfigNode_SubsensesBeforeAfterAndBetweenWork()
		{
			...
			Assert.That(",", Is.EqualTo(oldSubsensesNode.Children[0].Children[0].Between), "Between not migrated");
			Assert.That("@", Is.EqualTo(oldSubsensesNode.Children[0].Children[0].Before), "Before not migrated");
			Assert.That("@", Is.EqualTo(oldSubsensesNode.Children[0].Children[0].After), "After not migrated");
		}
		...
		public void ConvertLayoutTreeNodeToConfigNode_SubsensesGetsConvertedSenseChildren()
		{
			...
			Assert.That("ExampleSentences", Is.EqualTo(oldSubsensesNode.Children[0].Children[0].FieldDescription), "Defaults not copied in for fields before Subsenses");
			Assert.That("PostSubsenses", Is.EqualTo(oldSubsensesNode.Children[2].FieldDescription), "Defaults not copied into fields following Subsenses");
		}
		...
		public void ConvertLayoutTreeNodeToConfigNode_ListOptionsEnabledLexRelationWorks()
		{
			...
			Assert.That(1, Is.EqualTo(lexRelationOptions.Options.Count));
		...
		public void ConvertLayoutTreeNodeToConfigNode_ListOptionsMultipleItemsWorks()
		{
			...
			Assert.That(configNode.DictionaryNodeOptions is DictionaryNodeListOptions, Is.True, "List system options node not created");
			var lexRelationOptions = configNode.DictionaryNodeOptions as DictionaryNodeListOptions;
			Assert.That(2, Is.EqualTo(lexRelationOptions.Options.Count));
		...
		public void ConvertLayoutTreeNodeToConfigNode_SenseNumberStyleIsAddedAndUsed()
		{
			...
			Assert.That("arial", Is.EqualTo(usefulStyle.DefaultCharacterStyleInfo.FontName.Value), "arial font not used");
		...
		public void ConvertLayoutTreeNodeToConfigNode_SenseConfigsWithDifferentFontsMakeTwoStyles()
		{
			...
			Assert.That("arial", Is.EqualTo(usefulStyle.DefaultCharacterStyleInfo.FontName.Value), "arial font not used");
			usefulStyle = m_styleSheet.Styles[styleName2];
			Assert.That("notarial", Is.EqualTo(usefulStyle.DefaultCharacterStyleInfo.FontName.Value), "notarial font not used in second style");
			DeleteStyleSheet(styleName);
			DeleteStyleSheet(styleName2);
		}
		...
		public void CopyNewDefaultsIntoConvertedModel_NewNodeFromBaseIsMerged()
		{
			...
			Assert.That("Child2", Is.EqualTo(convertedModel.Parts[0].Children[1].Label), "New node from base was not merged properly");
		}
