...
namespace SIL.FieldWorks.XWorks
{
	...
	/// <summary>
	/// Base-PR scoping decision: <see cref="ChorusNotesPlugin"/> is a PHASE-1 FOLLOW-UP surface, the
	/// same status as the Avalonia browse table and the interlinear/rule-formula plugins — it is not
	/// active under the base PR's UIMode=New surfaces (lexiconEdit/lexiconEditPopup/notebookEdit/posEdit).
	/// These tests pin the gate from both sides: the plugin is not registered in the default registry
	/// the base surfaces resolve against, and even a directly-constructed instance refuses to build the
	/// live <see cref="ChorusNotesBarControl"/> while <see cref="ChorusNotesPlugin.IsPhase1FollowUp"/> is
	/// true — so a stray future registration cannot silently reactivate it.
	/// </summary>
	[TestFixture]
	public class ChorusNotesPhase1FollowUpGateTests : MemoryOnlyBackendProviderTestBase
	{
		private ILexEntry m_entry;
		...
		[Test]
		public void IsPhase1FollowUp_IsTrue()
		{
			// The discoverable marker: flipped to false (or removed) only when a follow-up PR
			// restores the registration in RegionEditorPluginRegistry.RegisterBuiltins.
			Assert.That(ChorusNotesPlugin.IsPhase1FollowUp, Is.True);
		}

		[Test]
		public void DefaultRegistry_DoesNotClaimTheMessageSliceClass()
		{
			// The base surfaces (lexiconEdit, lexiconEditPopup, notebookEdit, posEdit) all resolve
			// plugins through RegionEditorPluginRegistry.Default, so an unregistered ChorusNotesPlugin
			// here means the composer never builds the live notes bar for any of them.
			Assert.That(RegionEditorPluginRegistry.Default.Resolve(AvaloniaCompanionSlices.MessageSliceClassName),
				Is.Null, "ChorusNotesPlugin must not be registered while it is a Phase-1 follow-up surface");
		}

		[Test]
		public void BuildControl_ReturnsNull_EvenWhenInvokedDirectly()
		{
			// Belt-and-suspenders: the gate lives on the plugin itself, not only at the registration
			// call site, so even a directly constructed instance (e.g. a stray future registration)
			// cannot render the live ChorusNotesBarControl while the follow-up marker is true.
			var plugin = new ChorusNotesPlugin();
			var context = new RegionEditorBuildContext(m_entry, node: null,
				editContextAccessor: () => null, cache: Cache);
