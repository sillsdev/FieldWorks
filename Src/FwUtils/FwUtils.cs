	...
	public static partial class FwUtils
	{
		...
		public static void InitializeIcu()
		{
			if (MiscUtils.IsWindows)
			{
				...
				// ReSharper disable once AssignNullToNotNullAttribute -- If FieldWorksExe returns null we have bigger problems
				var icuPath = Path.Combine(Path.GetDirectoryName(FwDirectoryFinder.FieldWorksExe), "lib", arch);
				// Append icu dll location to PATH, such as .../lib/x64, to help C# and C++ code find icu.
				Environment.SetEnvironmentVariable("PATH", Environment.GetEnvironmentVariable("PATH") + Path.PathSeparator + icuPath);
			}
