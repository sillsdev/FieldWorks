...
protected:
	...
	static bool Contains(const T* pobjHead, const T* pobjCheck)
	{
		for (const T* pobj = pobjHead; pobj; pobj = pobj->m_pobjNext)
			if (pobjCheck == pobj)
				return true;
		return false;
	}
	...
	void Link(T ** ppobjPrev)
	{
		...
		if (ppobjPrev)
		{
			...
			AssertPtrN(*ppobjPrev);
			// Check that we aren't already in this list.
			// Sometimes lists have become corrupted due to incorrect symbol resolution.
			Assert(!Contains(*ppobjPrev, static_cast<const T*>(this)));
