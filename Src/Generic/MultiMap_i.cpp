...
Description:
	This file provides the implementations of methods for the MultiMap template collection
	classes.  It is used as an #include file in any file which explicitly instantiates any
	particular type of MultiMap<K,T>.
----------------------------------------------------------------------------------------------*/
#pragma once
#ifndef MULTIMAP_I_C_INCLUDED
#define MULTIMAP_I_C_INCLUDED
...
/*----------------------------------------------------------------------------------------------
	Constructor.
----------------------------------------------------------------------------------------------*/
template<class K, class T, class H, class EqK, class EqT>
	MultiMap<K,T,H,EqK,EqT>::MultiMap()
{
...
/*----------------------------------------------------------------------------------------------
	Return an iterator that references the first key and value stored in the MultiMap.
	If the MultiMap is empty, Begin returns the same value as End.
----------------------------------------------------------------------------------------------*/
template<class K, class T, class H, class EqK, class EqT>
	typename MultiMap<K,T,H,EqK,EqT>::iterator MultiMap<K,T,H,EqK,EqT>::Begin()
{
	AssertObj(this);
...
/*----------------------------------------------------------------------------------------------
	Return an iterator that marks the last key and value stored in the MultiMap.
	If the MultiMap is empty, End returns the same value as Begin.
----------------------------------------------------------------------------------------------*/
template<class K, class T, class H, class EqK, class EqT>
	typename MultiMap<K,T,H,EqK,EqT>::iterator MultiMap<K,T,H,EqK,EqT>::End()
{
	AssertObj(this);
...
/*----------------------------------------------------------------------------------------------
	Add one key and value to the MultiMap.  Insert potentially invalidates existing iterators
	for this MultiMap.  An exception is thrown if there are any errors.
	...
	@param value Reference to the object associated with the key.  An internal copy is made of
				this object.
	@param pihsndOut Optional pointer to an integer for returning the internal index where the
				key-value pair is stored.
----------------------------------------------------------------------------------------------*/
template<class K, class T, class H, class EqK, class EqT>
	void MultiMap<K,T,H,EqK,EqT>::Insert(K & key, T & value, int * pihsndOut)
{
	AssertObj(this);
	...
	int chsndUnique = CountUniqueKeys();
	if (chsndAvgDepth > 2 && chsndUnique > m_cBuckets)
	{
	...
	{
		// The caller's key was not in the MultiMap; it becomes first in the bucket's list
	...
	else
	{
		...
		new((void *)&m_prghsnd[ihsnd])
				HashNode(key, value, nHash, m_prghsnd[ihsndInsertAfter].GetNext());
...
/*----------------------------------------------------------------------------------------------
	Search the MultiMap for the given key, and set beginning and ending iterators
	appropriately.
...
----------------------------------------------------------------------------------------------*/
template<class K, class T, class H, class EqK, class EqT>
	bool MultiMap<K,T,H,EqK,EqT>::Retrieve(K & key, iterator * pitMin, iterator * pitLim)
{
	AssertObj(this);
	if (!m_prgihsndBuckets)
		return false;
...
/*----------------------------------------------------------------------------------------------
	Remove the element(s) with the given key from the stored MultiMap.  This potentially
	invalidates existing iterators for this MultiMap.  If the key is not found in the MultiMap,
	then nothing is deleted.  If the key is stored multiple times, all entries with the key are
	deleted.
...
----------------------------------------------------------------------------------------------*/
template<class K, class T, class H, class EqK, class EqT>
	bool MultiMap<K,T,H,EqK,EqT>::Delete(K & key)
{
	AssertObj(this);
	if (!m_prgihsndBuckets)
		return false;
...
/*----------------------------------------------------------------------------------------------
	Remove the element with the given key and value from the stored MultiMap.  This potentially
	invalidates existing iterators for this MultiMap.  If the key-value pair is not found in the
	MultiMap, then nothing is deleted.  At most one key-value pair is deleted from the MultiMap.

	@param key Reference to a copy of the key object.
	@param value Reference to a copy of the value object.
...
----------------------------------------------------------------------------------------------*/
template<class K, class T, class H, class EqK, class EqT>
	bool MultiMap<K,T,H,EqK,EqT>::Delete(K & key, T & value)
{
	AssertObj(this);
	if (!m_prgihsndBuckets)
		return false;
		 ...
		 ihsnd = m_prghsnd[ihsnd].GetNext())
	{
			...
			equal(&key, &m_prghsnd[ihsnd].GetKey(), isizeof(K)))
		{
			...
			do
			{
				...
				if (equalval(&value, &m_prghsnd[ihsnd].GetValue(), isizeof(T)))
				{
...
/*----------------------------------------------------------------------------------------------
	Free all the memory used by the MultiMap.  When done, only the minimum amount of
	...
	in the MultiMap before the memory space is freed.
----------------------------------------------------------------------------------------------*/
template<class K, class T, class H, class EqK, class EqT>
	void MultiMap<K,T,H,EqK,EqT>::Clear()
{
	AssertObj(this);
	if (!m_prgihsndBuckets)
		return;
...
/*----------------------------------------------------------------------------------------------
	Copy the content of this MultiMap to another MultiMap.  An exception is thrown if there are
	any errors.

	@param mmKT Reference to the other MultiMap.
----------------------------------------------------------------------------------------------*/
template<class K, class T, class H, class EqK, class EqT>
	void MultiMap<K,T,H,EqK,EqT>::CopyTo(MultiMap<K,T,H,EqK,EqT> & mmKT)
{
	AssertObj(this);
	AssertObj(&mmKT);
	mmKT.Clear();
	...
	for (itmm = Begin(); itmm != End(); ++itmm)
		mmKT.Insert(itmm->GetKey(), itmm->GetValue());
}
...
/*----------------------------------------------------------------------------------------------
	...
	@param pmmKT Pointer to the other MultiMap.
----------------------------------------------------------------------------------------------*/
template<class K, class T, class H, class EqK, class EqT>
	void MultiMap<K,T,H,EqK,EqT>::CopyTo(MultiMap<K,T,H,EqK,EqT> * pmmKT)
{
	if (!pmmKT)
...
/*----------------------------------------------------------------------------------------------
	If the given key is found in the MultiMap, return true, and if the provided index pointer
	is not NULL, also store an internal index value in the indicated memory location.
	If the given key is NOT found in the MultiMap, return false and ignore the provided index
...
----------------------------------------------------------------------------------------------*/
template<class K, class T, class H, class EqK, class EqT>
	bool MultiMap<K,T,H,EqK,EqT>::GetIndex(K & key, int * pihsndRet)
{
	AssertObj(this);
	if (!m_prgihsndBuckets)
		return false;
...
/*----------------------------------------------------------------------------------------------
	If the given internal MultiMap index is valid, return true, and if the provided pointer to a
...
----------------------------------------------------------------------------------------------*/
template<class K, class T, class H, class EqK, class EqT>
	bool MultiMap<K,T,H,EqK,EqT>::IndexKey(int ihsnd, K * pkeyRet)
{
	AssertObj(this);
...
/*----------------------------------------------------------------------------------------------
	If the given internal MultiMap index is valid, return true, and if the provided pointer to
	an object is not NULL, also copy the indexed value to the indicated memory location.
	If the given internal index is NOT valid, return false, and ignore the provided object
	pointer.
...
----------------------------------------------------------------------------------------------*/
template<class K, class T, class H, class EqK, class EqT>
	bool MultiMap<K,T,H,EqK,EqT>::IndexValue(int ihsnd, T * pvalueRet)
{
	AssertObj(this);
	if (ihsnd < 0 || ihsnd >= m_ihsndLim)
		return false;
...
----------------------------------------------------------------------------------------------*/
template<class K, class T, class H, class EqK, class EqT>
	int MultiMap<K,T,H,EqK,EqT>::KeyCount(int ihsndIn)
{
	AssertObj(this);
...
/*----------------------------------------------------------------------------------------------
	Return the number of unique keys stored in the MultiMap.
----------------------------------------------------------------------------------------------*/
template<class K, class T, class H, class EqK, class EqT>
	int MultiMap<K,T,H,EqK,EqT>::CountUniqueKeys()
{
	AssertObj(this);
	int ckey = 0;
	int ie;
...
/*----------------------------------------------------------------------------------------------
	Return the number of items (key-value pairs) stored in the MultiMap.
----------------------------------------------------------------------------------------------*/
template<class K, class T, class H, class EqK, class EqT>
	int MultiMap<K,T,H,EqK,EqT>::Size()
{
	AssertObj(this);
	if (!m_prgihsndBuckets)
		return 0;
...
----------------------------------------------------------------------------------------------*/
template<class K, class T, class H, class EqK, class EqT>
	int MultiMap<K,T,H,EqK,EqT>::_EmptyBuckets()
{
	AssertObj(this);
	int ceUnused = 0;
	int ie;
...
/*----------------------------------------------------------------------------------------------
	...
	objects in the MultiMap.  This is useful only for debugging the MultiMap mechanism itself.
----------------------------------------------------------------------------------------------*/
template<class K, class T, class H, class EqK, class EqT>
	int MultiMap<K,T,H,EqK,EqT>::_BucketsUsed()
{
	AssertObj(this);
	int ceUsed = 0;
	int ie;
...
/*----------------------------------------------------------------------------------------------
	...
	of the MultiMap.  This is useful only for debugging the MultiMap mechanism itself.
----------------------------------------------------------------------------------------------*/
template<class K, class T, class H, class EqK, class EqT>
	int MultiMap<K,T,H,EqK,EqT>::_FullestBucket()
{
	AssertObj(this);
	int chsndMax = 0;
	int chsnd;
...
/*----------------------------------------------------------------------------------------------
	Add one key and value to the MultiMap.  _Insert() is called only by Insert().
	...
	@param ihsndIn Index of the hash node to be inserted into a MultiMap bucket.
----------------------------------------------------------------------------------------------*/
template<class K, class T, class H, class EqK, class EqT>
	void MultiMap<K,T,H,EqK,EqT>::_Insert(int ihsndIn, int nHash, K & key)
{
	AssertObj(this);
	Assert(m_cBuckets);
...
/*----------------------------------------------------------------------------------------------
	...
	@param ihsndFirst Index of a hash node stored in the MultiMap.
----------------------------------------------------------------------------------------------*/
template<class K, class T, class H, class EqK, class EqT>
	int MultiMap<K,T,H,EqK,EqT>::_GetLastDupKey(int ihsndFirst)
{
	AssertObj(this);
	Assert((0 <= ihsndFirst) && (ihsndFirst < m_ihsndLim));
	// The rest of the HashNodes with this key follow immediately.
	int ihsnd;
