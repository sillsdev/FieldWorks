...
/*----------------------------------------------------------------------------------------------
	...
	to a character type. CharDefns<wchar> also defines OtherChar to mean schar.
----------------------------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------------------------------
	...
	to a character type. CharDefns<schar> also defines OtherChar to mean wchar.
----------------------------------------------------------------------------------------------*/
template<> class CharDefns<schar>
{
public:
	typedef wchar OtherChar;
	...
	inline int CychFitConvertedText(const YChar * prgych, int cych, int cxchMax)
{
	...
	if (cych > cxchMax &&
		ConvertText(prgych, cxchMax, (CharDefns<YChar>::OtherChar *)NULL, 0) <= cxchMax &&
		ConvertText(prgych, cxchMax + 1, (CharDefns<YChar>::OtherChar *)NULL, 0) > cxchMax)
	{
		return cxchMax;
	}
	...
	for (cychMin = 0, cychLim = cych; cychMin < cychLim; )
	{
		...
		int cxchT = ConvertText(prgych, cychT, (CharDefns<YChar>::OtherChar *)NULL, 0);
		if (cxchT > cxchMax)
			cychLim = cychT - 1;
		else
			cychMin = cychT;
...
public:
	...
	/*------------------------------------------------------------------------------------------
		...
		zero-terminated string template pszFmt of the same character type. See FormatText.
	------------------------------------------------------------------------------------------*/
	void FormatAppend(const XChar * pszFmt, ...)
	{
		AssertObj(this);
		AssertPsz(pszFmt);
		va_list argList;
	...
	/*------------------------------------------------------------------------------------------
		...
		string template (prgchFmt, cchFmt) of the same character type. See FormatText.
	------------------------------------------------------------------------------------------*/
	void FormatAppendRgch(const XChar * prgchFmt, int cchFmt, ...)
	{
		AssertObj(this);
		AssertArray(prgchFmt, cchFmt);
		va_list argList;
...
protected:
	...
	public:
		...
		void Release(void)
		{
			AssertObj(this);
			Assert(0 <= m_crefMinusOne);
			if (0 > InterlockedDecrement(&m_crefMinusOne))
				free(this);
		}
...
public:
	...
	------------------------------------------------------------------------------------------*/
	template<typename ZChar>
		StrBaseBuf<XChar, kcchMax>(const StrBaseBufCore<ZChar> & stbbc)
	{
		Assign(stbbc);
	}
	...
	------------------------------------------------------------------------------------------*/
	template<typename ZChar>
		StrBaseBuf<XChar, kcchMax>(const ZChar * psz)
	{
		Assign(psz);
	}
	...
	------------------------------------------------------------------------------------------*/
	template<typename ZChar>
		StrBaseBuf<XChar, kcchMax>(const ZChar * prgch, int cch)
	{
		Assign(prgch, cch);
	}
	...
	------------------------------------------------------------------------------------------*/
	template<typename ZChar>
		bool Assign(const StrBaseBufCore<ZChar> & stbbc)
	{
		AssertObj(this);
		AssertObj(&stbbc);
	...
	------------------------------------------------------------------------------------------*/
	template<typename ZChar>
		StrBaseBuf<XChar, kcchMax> & operator = (const StrBaseBufCore<ZChar> & stbbc)
	{
		AssertObj(this);
		AssertObj(&stbbc);
		Assign(stbbc);
	...
	------------------------------------------------------------------------------------------*/
	template<typename ZChar>
		StrBaseBuf<XChar, kcchMax> & operator = (const ZChar * psz)
	{
		AssertObj(this);
		AssertPszN(psz);
	...
	------------------------------------------------------------------------------------------*/
	template<typename ZChar>
		bool Append(const StrBaseBufCore<ZChar> & stbbc)
	{
		AssertObj(this);
		AssertObj(&stbbc);
	...
	------------------------------------------------------------------------------------------*/
	template<typename ZChar>
		StrBaseBuf<XChar, kcchMax> operator + (const StrBaseBufCore<ZChar> & stbbc) const
	{
		AssertObj(this);
		AssertObj(&stbbc);
	...
	------------------------------------------------------------------------------------------*/
	template<typename ZChar>
		StrBaseBuf<XChar, kcchMax> operator + (const ZChar * psz) const
	{
		AssertObj(this);
		AssertPszN(psz);
		StrBaseBuf<XChar, kcchMax> stbbRet(this);
	...
	------------------------------------------------------------------------------------------*/
	template<typename ZChar>
		bool Replace(int ichMin, int ichLim, StrBaseBufCore<ZChar> & stbbc)
	{
		AssertObj(this);
		AssertObj(&stbbc);
	...
	------------------------------------------------------------------------------------------*/
	template<typename ZChar>
		bool Replace(int ichMin, int ichLim, const ZChar * psz)
	{
		AssertObj(this);
		AssertPszN(psz);
	...
	/*------------------------------------------------------------------------------------------
		...
		the zero-terminated string template pszFmt of the same character type. See FormatText.
	------------------------------------------------------------------------------------------*/
	bool Format(const XChar * pszFmt, ...)
	{
		AssertObj(this);
		AssertPsz(pszFmt);
	...
	/*------------------------------------------------------------------------------------------
		...
		string template (prgchFmt, cchFmt) of the same character type. See FormatText.
	------------------------------------------------------------------------------------------*/
	bool FormatRgch(const XChar * prgchFmt, int cchFmt, ...)
	{
		AssertObj(this);
		AssertArray(prgchFmt, cchFmt);
	...
	/*------------------------------------------------------------------------------------------
		...
		zero-terminated string template pszFmt of the same character type. See FormatText.
	------------------------------------------------------------------------------------------*/
	bool FormatAppend(const XChar * pszFmt, ...)
	{
		AssertObj(this);
		AssertPsz(pszFmt);
	...
	/*------------------------------------------------------------------------------------------
		...
		string template (prgchFmt, cchFmt) of the same character type. See FormatText.
	------------------------------------------------------------------------------------------*/
	bool FormatAppendRgch(const XChar * prgchFmt, int cchFmt, ...)
	{
		AssertObj(this);
		AssertArray(prgchFmt, cchFmt);
...
namespace StrUtil
{
	...
	void InitIcuDataDir();
	void IcuCleanup();		// icu u_cleanup method, wrapped for sync
	void RestartIcu();
	...
	void ToLower(StrUni & stu, const char * pszLocale = NULL);
