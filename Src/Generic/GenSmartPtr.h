...
public:
	...
	virtual UCOMINT32 STDMETHODCALLTYPE AddRef(void)
	{
		InterlockedIncrement(&m_cref);
		return m_cref;
	}

	virtual UCOMINT32 STDMETHODCALLTYPE Release(void)
	{
...
/*----------------------------------------------------------------------------------------------
	...
	pointers.
	REVIEW: What's the difference between a GenSmartPtr and a ComSmartPtr?
----------------------------------------------------------------------------------------------*/
...
public:
	...
	GenSmartPtr(const GenSmartPtr & qobj)
	{
		m_pobj = qobj.m_pobj;
		if (m_pobj)
			m_pobj->AddRef();
	...
	GenSmartPtr & operator=(const GenSmartPtr & qobj)
	{
	...
	bool operator==(const GenSmartPtr & qobj) const
	{
...
/*----------------------------------------------------------------------------------------------
	ValidReadPtr for a smart pointer (GenSmartPtr). This is so AssertPtr works on a smart
	pointer.
----------------------------------------------------------------------------------------------*/
