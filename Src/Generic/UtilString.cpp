...
static const wchar szSpace[5] = {' ','\t','\r','\n', '\0'}; // whitespace characters
...
protected:
	...
	Vector<intptr_t> m_vArgList; // Argument list
	int m_cchMin;
	XChar m_chSign;
	...
	void FormatCore();
	// Get a vector of the arguments
	void GetArgVector(va_list vaArgList);
	// Convert a GUID to a string
	void GuidToString(GUID* pguid, XChar * prgchTerm, int & cch)
	{
		AssertPtr(pguid);

		XChar* pchT = prgchTerm;
		int iv;
		for (iv = 28; iv >= 0; iv -= 4)
	...
	void TimeToString(SilTime* pstim, bool fUTC, XChar* prgchTerm, int lenTerm, int & cch)
	{
		...
		SilTimeInfo sti;
		pstim->GetTimeInfo(&sti, fUTC);

		// Year.
		uint uT = sti.year < 0 ? -sti.year : sti.year;
		XChar* pchT = prgchTerm + lenTerm;
		cch = 0;
		do
		{
		...
		{
			prgchTerm[0] = '-';
			MoveItems(pchT, prgchTerm + 1, cch);
			cch++;
		}
		else
			MoveItems(pchT, prgchTerm, cch);

		// Month.
		prgchTerm[cch++] = '-';
		prgchTerm[cch++] = g_rgchDigits[sti.ymon / 10];
		prgchTerm[cch++] = g_rgchDigits[sti.ymon % 10];

		// Day.
		prgchTerm[cch++] = '-';
		prgchTerm[cch++] = g_rgchDigits[sti.mday / 10];
		prgchTerm[cch++] = g_rgchDigits[sti.mday % 10];

		// Hour.
		prgchTerm[cch++] = ':';
		prgchTerm[cch++] = g_rgchDigits[sti.hour / 10];
		prgchTerm[cch++] = g_rgchDigits[sti.hour % 10];

		// Minute.
		prgchTerm[cch++] = ':';
		prgchTerm[cch++] = g_rgchDigits[sti.min / 10];
		prgchTerm[cch++] = g_rgchDigits[sti.min % 10];

		// Second.
		prgchTerm[cch++] = ':';
		prgchTerm[cch++] = g_rgchDigits[sti.sec / 10];
		prgchTerm[cch++] = g_rgchDigits[sti.sec % 10];

		// MilliSecond.
		prgchTerm[cch++] = '.';
		prgchTerm[cch++] = g_rgchDigits[sti.msec / 100];
		prgchTerm[cch++] = g_rgchDigits[(sti.msec / 10) % 10];
		prgchTerm[cch++] = g_rgchDigits[sti.msec % 10];
	}

	void IntToRoman(int uT, bool fUpperCase, XChar* prgchTerm, int & cch)
	{
		if (uT <= 0)
		{
			prgchTerm[0] = '?';
		...
		if (fUpperCase)
			for (ich = 0; ich < cch; ++ich)
				prgchTerm[ich] = (XChar)toupper(prgchTerm[ich]);
	}
	void IntToAlphaOutline(int uT, bool fLowerCase, XChar* prgchTerm, int& cch)
	{
		if (uT <= 0)
		{
			prgchTerm[0] = '?';
		...
		cch = ((uT - 1) / 26) + 1;
		XChar ch = (XChar)(((uT - 1) % 26) + (fLowerCase ? 'a' : 'A'));
	...
	const XChar* ConvertNumber(XChar* pchTerm, int uRad, uint uT, int& cch)
	{
		cch = 0;
		do
		{
			*--pchTerm = g_rgchDigits[uT % uRad];
			cch++;
			uT /= uRad;
		} while (uT);
	...
	const XChar* FormatNextItem(int& ivArg, XChar* rgchTerm, const int& arraySize, int& cch)
	{
		const XChar* prgchTerm = rgchTerm;
		switch (m_chCur)
		{
		case 'c':
			// (int)XChar.
			rgchTerm[0] = static_cast<XChar>(m_vArgList[ivArg]);
			cch = 1;
			break;

		case 's':
			// XChar * psz. (Strings are the same size, schar or wchar).
			prgchTerm = reinterpret_cast<XChar*>(m_vArgList[ivArg]);
			AssertPsz(prgchTerm);
		...
		case 'r':
			...
			GetResourceString(&prgchTerm, &cch, static_cast<int>(m_vArgList[ivArg]));
			AssertPsz(prgchTerm);
			break;
		...
		case 'S':
			...
			{ // Block
				YChar * pszy = reinterpret_cast<YChar*>(m_vArgList[ivArg]);
				AssertPsz(pszy);
				m_stbT.Assign(pszy);
			}
		...
		case 'b':
			...
			{
				AssertPtr(((INT32 *)prgchTerm) - 1);
				cch = ((INT32 *)prgchTerm)[-1];
		...
		case 'B':
			...
			{ // Block
				YChar * prgchy = reinterpret_cast<YChar*>(m_vArgList[ivArg]);
				if (!prgchy)
					cch = 0;
				else
				{
					AssertPtr(((INT32 *)prgchy) - 1);
					cch = ((INT32 *)prgchy)[-1];
		...
		case 'a':
			...
			prgchTerm = reinterpret_cast<XChar*>(m_vArgList[ivArg]);
			++ivArg;
			cch = static_cast<int>(m_vArgList[ivArg]);
		...
		case 'h':
			...
			{ // Block
				uint uT = static_cast<uint>(m_vArgList[ivArg]);
		...
		case 'x':
			// Hex. If m_cchMin is not 0, don't make it longer than m_cchMin.
			{ // Block
				uint uT = static_cast<uint>(m_vArgList[ivArg]);
				if (m_cchMin > 0 && m_cchMin < 8)
					uT &= (1L << (m_cchMin * 4)) - 1;
				prgchTerm = ConvertNumber(rgchTerm + arraySize, 16, uT, cch);
			}
			break;
		...
		case 'd':
			...
			{ // Block
				...
				prgchTerm = ConvertNumber(rgchTerm + arraySize, 10, uT, cch);
			}
			break;
		...
		case 'g':
			...
			GuidToString(reinterpret_cast<GUID*>(m_vArgList[ivArg]), rgchTerm, cch);
			break;

		case 'G':
			// GUID * pguid displayed as a string of 26 digits and uppercase letters.
			GuidToString(reinterpret_cast<uint *>(m_vArgList[ivArg]), rgchTerm, cch);
			break;
		...
		case 'T':
			...
			// A year of zero is 1 BC, -1 is 2 BC, etc.
			TimeToString(reinterpret_cast<SilTime*>(m_vArgList[ivArg]), m_chCur == 'T',
				rgchTerm, arraySize, cch);
			break;
		...
		case 'm': // Int as a lowercase Roman numeral string: mcmix.
			...
			IntToRoman(static_cast<uint>(m_vArgList[ivArg]), m_chCur == 'M', rgchTerm, cch);
			break;
		...
		case 'o': // Int as lowercase alpha outline string: a, ... z, aa, bb, ... zz, aaa, ...
			// Anything illegal returns a ?.
			IntToAlphaOutline(static_cast<uint>(m_vArgList[ivArg]), m_chCur == 'o', rgchTerm, cch);
			break;

		case 'D': // Int as generic date string
			cch = 0;
			FormatDate (rgchTerm, cch, static_cast<int>(m_vArgList[ivArg]), m_cchMin > 0);
			m_cchMin = 0; // so we don't get a space for blank long date
			break;
...
----------------------------------------------------------------------------------------------*/
template<typename XChar, typename Pfn>
	void TextFormatter<XChar, Pfn>::GetArgVector(va_list vaArgList)
{
	while (FFetchChar())
	{
		// preprocess the format string and copy all input to the vector.
		if (m_chCur != '%')
			continue;
		FetchChar();	// skip %
		...
		case '<':
			...
			FetchInt(100); // Parse a non-negative integer. Update m_chCur and m_pchOut.
			Ensure(m_chCur == '>');
			FetchChar(); // m_chCur = *m_pchIn++;
			break;
		}
		...
		if (m_chCur == '^')
		{
			...
			m_vArgList.Push(static_cast<intptr_t>(va_arg(vaArgList, int)));
			FetchChar();
		}

		if (m_chCur == '-' || m_chCur == '+' || m_chCur == '0')
		{
			FetchChar();
		}
		...
		case 'D': // Int as generic date string
			...
			break;
		case 'c':
			// XChar, but we have to use an int to prevent warnings
			m_vArgList.Push(static_cast<intptr_t>(va_arg(vaArgList, int)));
			break;
		case 's':
		case 'b':
			m_vArgList.Push(reinterpret_cast<intptr_t>(va_arg(vaArgList, XChar*)));
			break;
		case 'S':
		case 'B':
			m_vArgList.Push(reinterpret_cast<intptr_t>(va_arg(vaArgList, YChar*)));
			break;
		case 'a':
			m_vArgList.Push(reinterpret_cast<intptr_t>(va_arg(vaArgList, XChar*)));
			m_vArgList.Push(static_cast<intptr_t>(va_arg(vaArgList, int)));
		...
		case 'O': // Int as uppercase alpha outline string: A, ... Z, AA, BB, ... ZZ, AAA, ...
		case 'o': // Int as lowercase alpha outline string: a, ... z, aa, bb, ... zz, aaa, ...
			m_vArgList.Push(static_cast<intptr_t>(va_arg(vaArgList, uint)));
			break;
		case 'g':
			m_vArgList.Push(reinterpret_cast<intptr_t>(va_arg(vaArgList, GUID*)));
			break;
		case 'G':
			m_vArgList.Push(reinterpret_cast<intptr_t>(va_arg(vaArgList, uint*)));
			break;
		case 't':
		case 'T':
			m_vArgList.Push(reinterpret_cast<intptr_t>(va_arg(vaArgList, SilTime*)));
			break;
		default:
			Error(); // Unexpected character.
		}
		...
		const XChar * prgchFmt, int cchFmt, va_list vaArgList)
{
	...
	GetArgVector(vaArgList);
	m_pchIn = prgchFmt; // reset input
		...
		XChar rgchTerm[], int & Xcch, int nGenDate, bool fLongDate)
{
	...
	if(fLongDate)
	{
		...
		tm * s1tm = localtime(&t);
		s1tm->tm_mday = nDay;
		s1tm->tm_mon = nMonth - 1;
		s1tm->tm_year = nYear - 1900;
		mktime(s1tm);
		strftime(rgchTemp+cch, sizeof(rgchTemp)-cch, stabFmt.Chars(), s1tm);
	}
	else
	{
		tm s2tm;
		s2tm.tm_mday = nDay;
		s2tm.tm_mon = nMonth - 1;
		s2tm.tm_year = nYear - 1900;
		strftime(rgchTemp+cch, sizeof(rgchTemp)-cch, stabFmt.Chars(), &s2tm);
	}
	cch += strlen(rgchTemp+cch);
#endif //WIN32
...
----------------------------------------------------------------------------------------------*/
template<typename XChar, typename Pfn>
	void TextFormatter<XChar, Pfn>::FormatCore()
{
	XChar rgchTerm[256];
	...
	while (FFetchChar()) // Set m_chCur to the new character and increment m_pchIn.
	{
		...
		m_chSign = 0; // Default sign, no sign.
		chPad = ' '; // Default padding character, a space.

		// Get qualifiers (count, left justify, sign, chPad, m_cchMin).
		if (m_chCur == '^')
		{
			// Repeat. This consumes an extra parameter, a count.
			cactRepeat = static_cast<int>(m_vArgList[ivArg]);
			++ivArg;
		...
		if (m_chCur >= '0' && m_chCur <= '9')
			m_cchMin = FetchInt(kcchBuf); // Parse non-negative integer. Update m_chCur, m_pchOut.
		else if (m_chCur == '*')
		{
			// Minimum field width. '*' consumes an extra parameter.
			m_cchMin = static_cast<int>(m_vArgList[ivArg]);
			++ivArg;
			Ensure((uint)m_cchMin < (uint)kcchBuf);
		}
		else
			m_cchMin = 0;

		// If we're not printing a signed decimal number, m_chSign is illegal.
		Ensure(!m_chSign || m_chCur == 'd');
		...
		prgchTerm = FormatNextItem(ivArg, rgchTerm, SizeOfArray(rgchTerm), cch);
		ivArg++;

		// Set cchPad to the number of characters to pad.
		cchPad = Max(0, m_cchMin - cch - (m_chSign != 0));
...
/*----------------------------------------------------------------------------------------------
	Format the template string pszFmt to an IStream.
----------------------------------------------------------------------------------------------*/
template<typename XChar>
...
/*----------------------------------------------------------------------------------------------
	Format the template string (prgchFmt, cchFmt) to an IStream.
----------------------------------------------------------------------------------------------*/
template<typename XChar>
...
template<typename XChar>
	typename StrBase<XChar>::StrBuffer * StrBase<XChar>::StrBuffer::Create(int cch)
{
	Assert(0 <= cch);

	int cb = isizeof(StrBuffer) + cch * isizeof(XChar);
	Assert(cb >= isizeof(StrBuffer));

	StrBuffer * pbuf = (StrBuffer *)malloc(cb);
	if (!pbuf)
		ThrowHr(WarnHr(E_OUTOFMEMORY));
	pbuf->m_crefMinusOne = 0;
	pbuf->m_cb = cch * isizeof(XChar);
	pbuf->m_rgch[cch] = 0;
...
/*----------------------------------------------------------------------------------------------
	...
	Note: I wanted to do this as a template, since it is the same as the YChar version above,
	but this seemed to add a lot of template complexity.
----------------------------------------------------------------------------------------------*/
template<typename XChar>
	void StrBase<XChar>::_Replace(int ichMin, int ichLim,
		const ZChar * prgchIns, ZChar chIns, int cchIns, int nCodePage)
{
	AssertObj(this);
	Assert(cchIns >= 0);
	AssertArray(prgchIns, cchIns);
...
----------------------------------------------------------------------------------------------*/
template<typename XChar>
	void StrBase<XChar>::FormatCore(const ZChar * prgchFmt, int cchFmt, va_list vaArgList)
{
	AssertObj(this);
	AssertArray(prgchFmt, cchFmt);
...
----------------------------------------------------------------------------------------------*/
template<typename XChar>
	void StrBase<XChar>::FormatAppendCore(const ZChar * prgchFmt, int cchFmt,
		va_list vaArgList)
{
	AssertObj(this);
	AssertArray(prgchFmt, cchFmt);
...
template<> StrBase<wchar>::StrBuffer StrBase<wchar>::s_bufEmpty = {0,0,0}; // Instantiation of empty wchar buffer.

//:Associate with StrBase<schar>.
template class StrBase<schar>; // Instantiation for XChar = schar.
template<> StrBase<schar>::StrBuffer StrBase<schar>::s_bufEmpty = {0,0,0}; // Instantiation of empty schar buffer.
	...
	bool StrBaseBuf<XChar, kcchMax>::Assign(const YChar * prgych, int cychSrc)
{
	...
	if (cxchDst == 0 && cychSrc > 0)
	{
		...
		cychSrc = CychFitConvertedText<XChar>(prgych, cychSrc, kcchMax);
...
----------------------------------------------------------------------------------------------*/
template<typename XChar, int kcchMax>
	bool StrBaseBuf<XChar, kcchMax>::Assign(const ZChar * prgych, int cychSrc)
{
	AssertObj(this);
	AssertArray(prgych, cychSrc);
	...
	bool StrBaseBuf<XChar, kcchMax>::Append(const YChar * prgych, int cychSrc)
{
	...
	if (cxchDst == 0)
	{
		m_fOverflow = true;
		// Determine the characters that fit.
		cychSrc = CychFitConvertedText<XChar>(prgych, cychSrc, kcchMax - cchCur);
...
----------------------------------------------------------------------------------------------*/
template<typename XChar, int kcchMax>
	bool StrBaseBuf<XChar, kcchMax>::Append(const ZChar * prgych, int cychSrc)
{
	AssertObj(this);
	AssertArray(prgych, cychSrc);
		...
		const YChar * prgych, int cychSrc)
{
	...
	else // The text to be inserted will itself overflow the buffer.
	{
		...
		cychSrc = CychFitConvertedText<XChar>(prgych, cychSrc, kcchMax - ichMin);
		...
		va_list vaArgList)
{
	...
	try
	{
		FormatText(FormatCallback, this,
			prgchFmt, cchFmt, vaArgList);
	}
	catch (...)
	{
...
template<typename XChar, int kcchMax>
	...
	bool StrBaseBuf<XChar, kcchMax>::FormatCore(const YChar * prgchFmt, int cchFmt,
		va_list vaArgList)
#else
	bool StrBaseBuf<XChar, kcchMax>::FormatCore(const ZChar * prgchFmt, int cchFmt,
		va_list vaArgList)
#endif
{
	AssertArray(prgchFmt, cchFmt);
...
template<typename XChar, int kcchMax>
	...
	bool StrBaseBuf<XChar, kcchMax>::FormatAppendCore(const ZChar * prgchFmt, int cchFmt,
		va_list vaArgList)
#endif
{
	AssertArray(prgchFmt, cchFmt);
...
void GetResourceString(const wchar ** pprgch, int * pcch, int stid)
{
	...
	*pprgch = pch;
#else
	StringTable::GetString(stid, *pprgch, *pcch);
	if (*pprgch == NULL)
		return;
#endif
...
namespace StrUtil
{
	...
	/**
	* @brief Map a unicode character to its lowercase equivalent using ICU.
	* @param uch32Character unicode character to make lowercase
	* @return lowercase version of uch32Character, or the same character if there was no equivalent lowercase character
	* Wraps ICU's u_tolower (/usr/share/doc/icu-doc/html/uchar_8h.html).
	*/
	UChar32 ToLower(UChar32 uch32Character)
	{
		return u_tolower(uch32Character);
	}
...
/*----------------------------------------------------------------------------------------------
	...
	@param prgch Pointer to an array of character data; not necessarily NUL-terminated.
	@param cch Number of characters (OLECHAR) in prgch; -1 for NUL-terminated prgch.
----------------------------------------------------------------------------------------------*/
...
template class Vector<intptr_t>;
