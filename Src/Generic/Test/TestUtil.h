...
namespace TestGenericLib
{
	class TestUtil : public unitpp::suite
	{
		void testGetPrimeNear()
		{
			...
			u = GetPrimeNear(4294967295);
			unitpp::assert_eq("GetPrimeNear(4294967295)", (uint)4294967291, u);
		}
		void testGetLargerPrime()
		{
			...
			u = GetLargerPrime(4294967295);
			unitpp::assert_eq("GetLargerPrime(4294967295)", (uint)4294967291, u);
		}
		void testGetSmallerPrime()
		{
			...
			u = GetSmallerPrime(4294967295);
			unitpp::assert_eq("GetSmallerPrime(4294967295)", (uint)4294967291, u);
		}
		void testGetGcdU()
		{
			uint u;
		...
		void testDirectoryFinder()
		{
			...
			unitpp::assert_true("Unexpected file allegedly found", ::GetFileAttributes(L"Some silly dummy file xhkjasoiy") == INVALID_FILE_ATTRIBUTES );
			...
			StrUni stuMig = DirectoryFinder::FwMigrationScriptDir();
			StrUni stuMigFile = stuMig;
			stuMigFile += L"\\M5toM6.sql";
			unitpp::assert_true("Expected file found in FW migration directory", ::GetFileAttributes(stuMigFile.Chars()) !=INVALID_FILE_ATTRIBUTES );

			// FwDatabaseDir();
			StrUni stuDb = DirectoryFinder::FwDatabaseDir();
			StrUni stuDbFile = stuDb;
			// At this point I don't know any predictable file I can expect to find here.
			//stuDbFile += L"\\Something.mdf";
			//unitpp::assert_true("Expected file found in FW migration directory", ::GetFileAttributes(stuDbFile.Chars()) !=INVALID_FILE_ATTRIBUTES );
			...
			stuTemplateFile += L"\\NewLangProj.xml";
			unitpp::assert_true("Expected file found in FW migration directory", ::GetFileAttributes(stuTemplateFile.Chars()) !=INVALID_FILE_ATTRIBUTES );
		}
		//ComputeHashRgb
