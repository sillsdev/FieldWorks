	...
	bool fXml)
{
	...
	for (pchw = rgchwSrc; pchw < pchwLim; )
	{
		...
		else
		{
			...
			ThrowHr(E_OUTOFMEMORY, L"BUFFER OVERFLOW in ConvertUtf16ToXmlUtf8()");
		}
		cchDst += cchChar;
	}
...
----------------------------------------------------------------------------------------------*/
STDMETHODIMP_(ULONG) XmlStringStream::AddRef()
{
	Assert(m_cref > 0);
	return ++m_cref;
}
...
----------------------------------------------------------------------------------------------*/
STDMETHODIMP_(ULONG) XmlStringStream::Release()
{
	Assert(m_cref > 0);
	if (--m_cref > 0)
		return m_cref;
...
----------------------------------------------------------------------------------------------*/
STDMETHODIMP XmlStringStream::Read(void * pv, ULONG cb, ULONG * pcbRead)
{
	BEGIN_COM_METHOD;
...
/*----------------------------------------------------------------------------------------------
	...
	@return S_OK, STG_E_INVALIDPOINTER, or E_FAIL
----------------------------------------------------------------------------------------------*/
STDMETHODIMP XmlStringStream::Write(const void * pv, ULONG cb, ULONG * pcbWritten)
{
	BEGIN_COM_METHOD;
	ChkComArrayArg((byte *)pv, cb);
