...
const wchar * g_rgchwDatePrec[] = { g_rgchwPrecFull[0], g_rgchwPrecFull[1],
										   g_rgchwPrecFull[2], g_rgchwPrecFull[3],
										   g_rgchwPrecFull[4], NULL };
const wchar * g_rgchwDateBC_AD[] = { g_rgchwBC_ADFull[0], g_rgchwBC_ADFull[1], NULL };
const wchar * g_rgchwDateBlank[] = { g_rgchwBlankFull[0], g_rgchwBlankFull[1],
											g_rgchwBlankFull[2] };

const char * g_rgchsDatePrec[] = { g_rgchPrecFull[0], g_rgchPrecFull[1],
										  g_rgchPrecFull[2], g_rgchPrecFull[3],
										  g_rgchPrecFull[4], g_rgchPrecFull[5], NULL };
const char * g_rgchsDateBC_AD[] = { g_rgchBC_ADFull[0], g_rgchBC_ADFull[1], NULL };
const char * g_rgchsDateBlank[] = { g_rgchBlankFull[0], g_rgchBlankFull[1],
										   g_rgchBlankFull[2] };
...
void LoadDateQualifiers()
{
	...
	for (i = 0; i < 5; i++)
	{
		...
		u_strcpy(reinterpret_cast<UChar*>(g_rgchwPrecFull[i]), reinterpret_cast<const UChar*>(stu.Chars()));
	...
	for (i = 0; i < 2; i++)
	{
		...
		u_strcpy(reinterpret_cast<UChar*>(g_rgchwBC_ADFull[i]), reinterpret_cast<const UChar*>(stu.Chars()));
	...
	for (i = 0; i < 3; i++)
	{
		...
		u_strcpy(reinterpret_cast<UChar*>(g_rgchwBlankFull[i]), reinterpret_cast<const UChar*>(stu.Chars()));
...
#else // WIN32
	int64 msectmp = m_msec;
	int yeartmp = (int)FloorDiv(msectmp, kmsecPerPeriod) * kyearPerPeriod + kyearBase;
	msectmp = ModPos(msectmp, kmsecPerPeriod);

	// Find the day within the 400 year period.
	day = (int)(msectmp / kmsecPerDay);

	// Get the time within the day.
	msecDay = (int)(msectmp - day * (int64)kmsecPerDay);

	yeartmp += YearFromDayInPeriod(day, &yday);

	if (!fUtc && (yeartmp > 1970))
	{
		msec *= kfttPerMsec;
		msec /= kfttPerSec;
		msec -= SECS_1601_TO_1970;
