...
----------------------------------------------------------------------------------------------*/
STDMETHODIMP_(ULONG) StringDataObject::AddRef(void)
{
	Assert(m_cref > 0);
	return ++m_cref;
}
...
/*----------------------------------------------------------------------------------------------
	...
	@return The updated reference count.
----------------------------------------------------------------------------------------------*/
STDMETHODIMP_(ULONG) StringDataObject::Release(void)
{
	Assert(m_cref > 0);
	if (--m_cref > 0)
		return m_cref;
...
/*----------------------------------------------------------------------------------------------
	...
	@return The updated reference count.
----------------------------------------------------------------------------------------------*/
STDMETHODIMP_(ULONG) StrEnumFORMATETC::AddRef(void)
{
	Assert(m_cref > 0);
	return ++m_cref;
}
...
/*----------------------------------------------------------------------------------------------
	...
	@return The updated reference count.
----------------------------------------------------------------------------------------------*/
STDMETHODIMP_(ULONG) StrEnumFORMATETC::Release(void)
{
	Assert(m_cref > 0);
	if (--m_cref > 0)
		return m_cref;
...
/*----------------------------------------------------------------------------------------------
	...
	@return S_OK (if celt items retrieved), S_FALSE (if fewer than celt items retrieved), or
					E_POINTER.
----------------------------------------------------------------------------------------------*/
STDMETHODIMP StrEnumFORMATETC::Next(ULONG celt, FORMATETC * rgelt, ULONG * pceltFetched)
{
	BEGIN_COM_METHOD;
...
/*----------------------------------------------------------------------------------------------
	...
	@return S_OK.
----------------------------------------------------------------------------------------------*/
STDMETHODIMP StrEnumFORMATETC::Skip(ULONG celt)
{
	BEGIN_COM_METHOD
...
StrUni DirectoryFinder::FwRootDataDir()
{
	RegKey rk;
	StrUni stuResult;
	if (rk.InitLm(_T("Software\\SIL\\FieldWorks")))
	{
		achar rgch[MAX_PATH];
		DWORD cb = isizeof(rgch);
		DWORD dwT;
...
/*----------------------------------------------------------------------------------------------
	looks up and returns the directory where we find data migration SQL scripts.
----------------------------------------------------------------------------------------------*/
StrUni DirectoryFinder::FwMigrationScriptDir()
{
	StrUni stuResult = FwRootCodeDir();
	stuResult += L"\\DataMigration";
	return stuResult;
}

/*----------------------------------------------------------------------------------------------
	looks up and returns the directory where we put FW database (mdf and log) files.
----------------------------------------------------------------------------------------------*/
StrUni DirectoryFinder::FwDatabaseDir()
{
