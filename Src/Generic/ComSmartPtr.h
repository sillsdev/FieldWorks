...
{
public:
	typedef typename _Interface Interface;
	...
	template<typename _Interface> ComSmartPtr(const ComSmartPtr & qobj)
	{
		m_pobj = qobj.m_pobj;
		if (m_pobj)
			m_pobj->AddRef();
	...
	template<typename _Interface> ComSmartPtr & operator=(const ComSmartPtr & qobj)
	{
	...
	template<> bool operator==(const ComSmartPtr & qobj) const
	{
	...
	void CreateInstance(const CLSID & clsid, DWORD dwClsContext = CLSCTX_ALL)
	{
		...
		try{
			...
			StrUni stu;
			stu.Format(L"Internal error: could not create object with CLSID %g (HRESULT %x). %s", &clsid, thr.Result(), thr.Message());
