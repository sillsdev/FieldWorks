...
namespace SIL.FieldWorks.Common.RootSites
{
	...
	public class EditingHelper : IDisposable, ISelectionChangeNotifier
	{
			...
			if (Callbacks?.EditedRootBox != null)
			{
				...
				if (!fWasComplex || !delOrBkspWasPressed)
				{
					...
					else
					{
						...
						{
							rootb.DataAccess.GetActionHandler().BreakUndoTask(RootSiteStrings.ksUndoTyping, RootSiteStrings.ksRedoTyping);
						}
			...
			if ((modifiers & Keys.Control) != Keys.Control || str.CompareTo(" ") != 0)
			{
				...
				catch (Exception ex)
				{
					...
					for (var ex1 = ex; ex1 != null; ex1 = ex1.InnerException)
					{
						...
						MessageBox.Show(ex1.Message, RootSiteStrings.ksWarning, MessageBoxButtons.OK, MessageBoxIcon.Warning);
		...
		public static void SetTsStringOnClipboard(ITsString tsString, bool fCopy, ILgWritingSystemFactory writingSystemFactory)
		{
			...
			{
				MessageBox.Show(RootSiteStrings.ksCopyFailed + e.Message);
			}
		}
		...
		public bool CutSelection()
		{
			try
			{
				...
				// here), now delete the range of text that has been copied to the
				// clipboard.
				DeleteSelectionTask(RootSiteStrings.ksUndoCut, RootSiteStrings.ksRedoCut);
				return true;
			}
			catch (Exception ex)
			{
				MessageBox.Show(string.Format(RootSiteStrings.ksCutFailed, ex.Message), RootSiteStrings.ksCutFailedCaption, MessageBoxButtons.OK, MessageBoxIcon.Warning);
				return false;
			}
		}
