...
*
* Revision History:
*   13 June, 2007, Yves Blouin: Created
*   7 May 2008, Steve Miller: Added this header, removed use of --
*******************************************************************************/

SET TERM !! ;
CREATE OR ALTER PROCEDURE SPX_VALID_NEW_PROC_NAME (
...
BEGIN
	 ...
	 SELECT COUNT(1) AS NameCount
		 FROM RDB$Procedures
		 WHERE RDB$Procedure_Name = CAST(:outValidName AS VARCHAR(31) CHARACTER SET UNICODE_FSS)
		 INTO :vCount;
