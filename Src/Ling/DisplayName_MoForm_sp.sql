...
declare @retval int, @fIsNocountOn int,
	...
	if @hvo is null begin
		...
		set @myCursor = CURSOR FAST_FORWARD for
			...
			from CmObject cmo
			where cmo.Class$ IN (5027, 5045, 5029) and cmo.OwnFlid$ IN (kflidLexEntry_LexemeForm, kflidLexEntry_AlternateForms)
			order by cmo.Id
		open @myCursor
	end
	else begin
		...
		set @myCursor = CURSOR FAST_FORWARD for
			...
			from CmObject cmo
			where cmo.Id = @hvo
				and cmo.Class$ IN (5027, 5045, 5029) and OwnFlid$ IN (kflidLexEntry_LexemeForm, kflidLexEntry_AlternateForms)
		open @myCursor
	end
