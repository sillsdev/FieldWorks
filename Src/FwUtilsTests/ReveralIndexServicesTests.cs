	...
	public class ReversalIndexServicesTests : MemoryOnlyBackendProviderTestBase
	{
		...
		public void CreateOrRemoveReversalIndexConfigurationFiles_DeletethNotValidConfigs()
		{
			...
			using (var tfProject = new TemporaryFolder("ProjForDontDeleteRealReversals"))
			{
				...
				ReversalIndexServices.CreateOrRemoveReversalIndexConfigurationFiles(WSMgr, Cache, FwDirectoryFinder.DefaultConfigurations, projectsDir, projectName);
		...
		private void CreateDummyConfigForWS(string filename, string ws)
		{
			...
			var xElement = new XElement(ReversalIndexServices.DictConfigElement);
			xElement.Add(new XAttribute("name", string.IsNullOrEmpty(ws) ? "All Reversal Indexes" : WSMgr.Get(ws).LanguageTag));
			xElement.Add(new XAttribute(ReversalIndexServices.WsAttribute, ws ?? string.Empty));
			xmldoc.Add(xElement);
		...
		{
			return XDocument.Load(filename).XPathSelectElement(ReversalIndexServices.DictConfigElement).Attribute(ReversalIndexServices.WsAttribute)?.Value;
		}

		private static XDocument GetLastModifiedAttributeFromFile(string filename, out XAttribute modifiedAtt)
		{
			var doc = XDocument.Load(filename);
			var config = doc.Element(ReversalIndexServices.DictConfigElement);
			if (config == null)
			{
				config = new XElement(ReversalIndexServices.DictConfigElement);
				doc.Add(config);
			}
