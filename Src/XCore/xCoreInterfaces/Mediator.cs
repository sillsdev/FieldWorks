...
//		private bool m_allowCommandsToExecute;
		...
		private Dictionary<string, MethodInfo> m_TypeMethodInfo = new Dictionary<string, MethodInfo>();
		/// <summary>
		...
		protected override void Dispose(bool disposing)
		{
			...
			if (disposing)
			{
				...
				if (m_pendingMessages != null)
				{
					...
					// Sense we have gotten rid of the pending messages, also remove the event handler.
...
//			}
		...
		{
			CheckDisposed();
			// Calling Logger.WriteEvent(message) directly requires directly referencing Reporter,
			// which hauls in COMInterfaces and a bunch of other stuff.  Decoupling by reflection
			// allows us to compile xCoreInterfaces with fewer dependencies.
			System.Type typeLogger = System.Type.GetType("SIL.Utils.Logger, Reporting");
			if (typeLogger != null)
			{
				System.Reflection.MethodInfo mi = typeLogger.GetMethod(
					"WriteEvent", BindingFlags.Public | BindingFlags.Static);
				if (mi != null)
				{
					object[] args = new Object[1];
					args[0] = message;
					mi.Invoke(null, args); // call Logger.WriteEvent(message)
				}
			}
		}
		#endregion
