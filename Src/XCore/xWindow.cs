...
namespace XCore
{
	...
	public class XWindow : Form, IFWDisposable, IxCoreColleague, IxWindow
	{
		...
		/// <summary>
		/// Virtuals are nothing in a pure XCore app.
		/// </summary>
		/// <param name="configurationNode"></param>
		protected virtual void LoadVirtuals(XmlNode configurationNode)
		{
			if (configurationNode == null)
				return;
		}
		...
		public void LoadUI(string configurationPath)
		{
			...
			XmlDocument configuration = XmlUtils.LoadConfigurationWithIncludes(configurationPath);
		...
		protected virtual void LoadUIFromXmlDocument(XmlDocument configuration, string configurationPath)
		{
			...
			string crashOnStartupDetectorPathName = CrashOnStartupDetectorPathName;
			...
			if (!wasCrashDuringPreviousStartup)
			{
				...
				System.IO.Directory.CreateDirectory(CrashOnStartupDetectorPathName.Substring(0, CrashOnStartupDetectorPathName.LastIndexOf(@"\")));
				System.IO.StreamWriter writer = System.IO.File.CreateText(CrashOnStartupDetectorPathName);
				writer.Close();
			}
			...
			if ((bool)m_mediator.PropertyTable.GetValue("DoLog", false))
				SIL.Utils.Logger.Init();
			...
			LoadVirtuals(m_windowConfigurationNode.SelectSingleNode("virtuals")); // Does nothing for pure XCore apps.
			...
			// Note: We should be able to do it directly, since everythin needed is in the default properties.
		...
		private void PrivateSuspend()
		{
			m_fullyInitialized = false; // Can't really be true, until the content control is added.
			SuspendLayout();
			//m_mainSplitContainer.Visible = false;
			//m_mainSplitContainer.Hide();
			m_mainSplitContainer.SuspendLayout();
			//Controls.Remove(m_mainSplitContainer);
		}
		...
		{
			XmlNode node = m_windowConfigurationNode.SelectSingleNode(String.Format("//menu[@id='{0}']", menuId));
			if (node == null)
		...
		protected void CreateStatusBar(XmlNode windowConfigurationNode)
		{
			...
			{
				throw new ConfigurationException("There was a problem reading the configuration for the status bar. " + error.Message, configuration);
			}
		}
		...
		public bool OnShowNotification(object notificationText)
		{
			...
			NotifyWindow nw = new NotifyWindow((string)notificationText);
		...
		protected override void OnHandleCreated(EventArgs e)
		{
			...
			// so for now we are leaving it to subclasses. For example, see LexTextApp.NewMainAppWnd().
		}
		...
		/// </summary>
		/// <param name="contentAssemblyPath"></param>
		/// <param name="contentClass"></param>
		/// <param name="contentClassNode"></param>
		protected void ChangeContentObjectIfPossible(string contentAssemblyPath, string contentClass, XmlNode contentClassNode)
		{
			...
			if (m_mainContentControl != null)
			{
				...
				// Only do the suspend, if there alrady is a control.
				// Otherwise, it was suspended way back in the LoadUIFromXmlDocument method.
				PrivateSuspend();
			...
			if (contentAssemblyPath != null)
			{
				...
				try
				{
					...
					//m_mainContentControl.Show(); // It displays with this call disabled.
					m_mainContentControl.Focus();
			...
			//m_mainSplitContainer.Visible = true;
			m_mainSplitContainer.ResumeLayout();
			ResumeLayout(); // true
			//m_mainSplitContainer.Show();
			//m_mainSplitContainer.Invalidate();
			//m_mainSplitContainer.BringToFront();
			//this.Invalidate();
		}

		private void SetToolDefaultProperties(XmlNode configurationNode)
		{
			Mediator.PropertyTable.SetProperty("AllowInsertExternalLink", true);	// default to allowing external links
		...
		protected void SetWindowLabel()
		{
			string label = m_mediator.PropertyTable.GetStringProperty("DocumentName", "");
			label += m_mediator.PropertyTable.GetStringProperty("applicationName", "application name???");
			this.Text = label;
		}
	...
	class StatusBarSizeGrip : StatusBarPanel
	{
		...
		public StatusBarSizeGrip(StatusBar bar)
		{
			...
			Width = width + SystemInformation.Border3DSize.Width * 2;
		}
