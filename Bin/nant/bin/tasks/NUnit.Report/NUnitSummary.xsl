...
<xsl:template match="testsummary">
	...
	<xsl:template name="test-results-summary">
		<xsl:variable name="totalTestCount" select="@total"/>
		...
		<xsl:variable name="totalSuccessRate" select="($totalTestCount - $totalFailureCount - $totalNotrunCount) div $totalTestCount"/>
		...
		<tr valign="top">
			...
			<td><xsl:value-of select="$totalFailureCount"/></td>
			<td>
			<xsl:call-template name="display-time">
