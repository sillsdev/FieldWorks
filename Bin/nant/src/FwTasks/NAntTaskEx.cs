...
namespace NAnt.Core.Tasks
{
	...
	public class NAntExTask : NAntTask
	{
		...
		private Project CreateProject(FileInfo buildFile)
		{
			ConstructorInfo constructorInfo = typeof(Project).GetConstructor(
				BindingFlags.NonPublic | BindingFlags.Instance, null,
				new Type[] { typeof(string), typeof(Project) }, null);
			if (constructorInfo != null)
				return (Project)constructorInfo.Invoke(new object[] { buildFile.FullName, Project});

			Log(Level.Warning, "Can't find required Project c'tor");
			return null;
		}
