...
namespace FwNantAddin2
{
	...
	public class NAntBuild: IDisposable
	{
		...
		private const string m_Nant = "nant.exe";
		...
		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Initializes a new instance of the <see cref="T:NAntBuild"/> class.
		/// </summary>
		/// ------------------------------------------------------------------------------------
		public NAntBuild()
		{
		...
		private string NAnt
		{
			get
			{
				string nant = Path.Combine(Settings.Default.NantPath, m_Nant);
				if (!File.Exists(nant))
					return m_Nant; // not at specified location - so try in path
				return nant;
			}
		}
		#endregion
		...
		/// </remarks>
		/// ------------------------------------------------------------------------------------
		private string GetTargetName(Modifiers mods, string prjPath)
		{
			if (prjPath == null || prjPath == string.Empty)
				return string.Empty;
		...
		/// <param name="buildfile">[out] name and path of the buildfile</param>
		/// <param name="fwroot">[out] fwroot path</param>
		/// ------------------------------------------------------------------------------------
		private void RetrieveBuildFile(string path, out string buildFile, out string fwroot)
		{
			...
			string configName, string unused, string[] projects)
		{
			...
			StringBuilder prjBldr = new StringBuilder();
			...
			try
			{
				InternalBuildProject(mods, prjBldr.ToString(), configName, fRebuild,
					solutionName, false);
			}
			catch(TargetException)
			{
			...
			string configName, string platformName, string[] projects, bool fWait)
		{
			...
			try
			{
				...
				{
					string target = (string)targets[projects[i]];
					if (target != null)
					{
						bool fOk = InternalBuildProject(mods, target, configName, fRebuild,
							projects[i], fWait);
						if (!fOk)
						{
							fRet = false;
