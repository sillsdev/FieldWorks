//-------------------------------------------------------------------------------------------------
// <copyright file="cpiasmsched.cpp" company="Microsoft">
...
CPI_PROPERTY_DEFINITION pdlComponentProperties[] =
{
	...
	{L"InitializesServerApplication",       cpptBoolean, 502},
	...
	)
{
	...
	for (CPI_ASSEMBLY* pItm = pList->pFirst; pItm; pItm = pItm->pNext)
	{
		...
		if (!pItm->fReferencedForUninstall && !pItm->iRoleAssignmentsUninstallCount && !WcaIsUninstalling(pItm->isInstalled, pItm->isAction))
			continue;
	...
	)
{
	...
	{
		// assemblies that are being uninstalled
		if (pItm->fIgnore || !WcaIsUninstalling(pItm->isInstalled, pItm->isAction))
			continue;
	...
	)
{
	...
	while (S_OK == (hr = WcaFetchRecord(hView, &hRec)))
	{
		...
		if (!*pItm->pwzAssemblyName && (pItm->iAttributes & aaPathFromGAC))
		{
			...
			hr = GetAssemblyName(pwzData, &pItm->pwzAssemblyName); // note: pwzData is still supposed to contain the component name
		...
		if (pItm->pApplication)
		{
			...
			if (pItm->iRoleAssignmentsUninstallCount || WcaIsUninstalling(pItm->isInstalled, pItm->isAction))
