//-------------------------------------------------------------------------------------------------
// <copyright file="cpiutilexec.cpp" company="Microsoft">
	...
	)
{
	...
	if (*ppSid)
	{
		...
		{
			*ppSid = (PSID)::HeapReAlloc(::GetProcessHeap(), HEAP_ZERO_MEMORY, *ppSid, dwLengthRequired);
			ExitOnNull1(*ppSid, hr, E_OUTOFMEMORY, "Failed to reallocate buffer for SID, len: %d", dwLengthRequired);
		}
	}
	else
	{
