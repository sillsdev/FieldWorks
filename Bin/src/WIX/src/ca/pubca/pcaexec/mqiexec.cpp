//-------------------------------------------------------------------------------------------------
// <copyright file="mqiexec.cpp" company="Microsoft">
	...
	)
{
	...
	for (int i = 0; i < iCnt; i++)
	{
		...
		// create message queue
		hr = DeleteMessageQueue(&attrs);
		ExitOnFailure1(hr, "Failed to delete message queue, key: %S", attrs.pwzKey);
	...
	)
{
	...
	do {
		...
		{
			psd = (PSECURITY_DESCRIPTOR)::HeapReAlloc(::GetProcessHeap(), HEAP_ZERO_MEMORY, psd, dw);
			ExitOnNull(psd, hr, E_OUTOFMEMORY, "Failed to reallocate buffer for security descriptor");
			hr = S_FALSE; // retry
		}
		else
		{
