...
HRESULT ScaSslNewCertficate(LPCWSTR pwzName, INT iStore,
...
HRESULT ScaSslExistingCertficateByName(LPCWSTR pwzName, INT iStore,
									   INT iStoreLocation, BSTR* pbstrCertificate,
									   DWORD* pcbCertificate, BYTE* pbaHashBuffer);

HRESULT ScaSslExistingCertficateByBinaryData(INT iStore, INT iStoreLocation,
...
LPCWSTR vcsCertQuery = L"SELECT `Certificate`, `Name`, `Component_`, `StoreLocation`, `StoreName`, `Attributes`, `CertificateData`, `PFXPassword` FROM `Certificate`";
enum eCertQuery { cqCertificate = 1, cqName, cqComponent, cqStoreLocation, cqStoreName, cqAttributes, cqCertificateData, cqPFXPassword };
	...
	)
{
	...
	while (SUCCEEDED(hr = WcaFetchRecord(hViewCertificate, &hRecCertificate)))
	{
		...
		hr = WcaGetRecordFormattedString(hRecCertificate, cqCertificateData, &pwzData);
		ExitOnFailure(hr, "failed to get Certificate.CertificateData");
		...
		hr = WcaWriteStringToCaData(pwzName, &pwzCaData);
	...
	)
{
	...
	WCHAR* pwzCA = NULL;
	DWORD cchDistinguishedName = 0, cchCA = 0;
	...
	if (dwAttributes & SCA_CERT_ATTRIBUTE_REQUEST)
	{
		...
		//        hr = ScaSslExistingCertficateByName(pwzName, iStore, iStoreLocation, pbstrCertificate, pcbCertificate, pbaHashBuffer);
	...
	if (pbData)
	{
		...
		hr = WcaAddTempRecord(&hCertificateHashView, &hCertificateHashColumns, L"CertificateHash", 0, 2, wzId, wzEncodedCertificateHash);
	...
	)
{
	...
	if (!pCertContext)
	{
		...
		if (dwContentType & CERT_QUERY_CONTENT_PFX)
		{
			hPfxCertStore = ::PFXImportCertStore((CRYPT_DATA_BLOB*)&blob, wzPFXPassword, (CERT_SYSTEM_STORE_CURRENT_USER == dwStoreLocation) ? CRYPT_USER_KEYSET : CRYPT_MACHINE_KEYSET);
	...
	)
{
	...
	hCertStore = ::CertOpenStore(CERT_STORE_PROV_SYSTEM, 0, NULL, dwStoreLocation, wzStore);
						   ...
						   BSTR *pbstrCertificate)
{
	...
	// save the certificate in place, cannot be passed to a deffered custom action
...
LExit:
	...
	if (pEnroll)
		pEnroll->Release();
	if (bstrCertificateAuthority)
		SysFreeString(bstrCertificateAuthority);
	if (bstrCertificateAttributes)
		SysFreeString(bstrCertificateAttributes);
	if (bstrDistinguishedName)
		SysFreeString(bstrDistinguishedName);
...
HRESULT ScaSslExistingCertficateByBinaryData(INT iStore, INT iStoreLocation, BYTE* pwzData, DWORD cchData)
{
	HRESULT hr = S_FALSE;
	HCERTSTORE hCertStore = NULL;
...
HRESULT ScaSslExistingCertficateByName(LPCWSTR pwzName, INT iStore, INT iStoreLocation,
...
HRESULT ScaSslNewCertficate(LPCWSTR pwzName, INT iStore, INT iStoreLocation, LPCWSTR wzComputerName, LPCWSTR wzDistinguishedName, LPCWSTR wzCertificateAuthorityOrig,
							BSTR* pbstrCertificate, DWORD* pcbCertificate, BYTE* pbaHashBuffer)
{
								   ...
								   BSTR* pbstrCertificate, DWORD* pcbCertificate, BYTE* pbaHashBuffer)
{
	...
	hr = ScaSslExistingCertficateByName(pwzName, iStore, iStoreLocation, pbstrCertificate, pcbCertificate, pbaHashBuffer);
	ExitOnFailure(hr, "Failed ScaSslExistingCertficateByName");
	if (S_OK != hr)
	{
		if (!fIsUninstalling && fIsInstalling)
		{
			...
			hr = ScaSslNewCertficate(pwzName, iStore, iStoreLocation, wzComputerName, wzDistinguishedName, wzCA,
				pbstrCertificate, pcbCertificate, pbaHashBuffer);
			ExitOnFailure(hr, "Failed ScaSslNewCertficate");
		}
		else
		{
