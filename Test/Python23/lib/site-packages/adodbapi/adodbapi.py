...
def connect(connstr):
	"Connection string as in the ADO documentation"
	try:
...
#  Set defaultIsolationLevel on module level before creating the connection. It may be one of the above
...
##    It may also be used in the implementation of executemany().
	...
	def callproc(self, procname, parameters=None):
		...
		"""
		self.messages=[]
		return self.executeHelper(procname,1,parameters)
	...
	def _makeDescriptionFromRS(self,rs):
		if (rs == None) or (rs.State == adStateClosed):
			...
			#    may be a significant drain on resources #
			self.rowcount= -1
			self.rs=rs
			...
			for i in range(nOfFields):
				...
				null_ok= f.Attributes & adFldMayBeNull
	...
	def executeHelper(self,operation,isStoredProcedureCall,parameters=None):
		...
		while tryAgain:
			try:
				...
				if parameters != None and strategy in (1,3):
				...
				if parameters != None:
					...
					for elem in parameters:
						i+=1
						if isStoredProcedureCall and i == returnValueIndex:
							i+=1
						try:
							if strategy in (1,3) :
					...
					# One workaround is to provide the date as a string in the format 'YYYY-MM-dd'
										p.Value=dateconverter.DateObjectToIsoFormatString(elem)
									else:
...
##    import locale
##    def wrappedLocaleAtoi(x): # you'll need this and a similar
##                              # for locale.atof if variants
##                              # areent converted to strings
##        return locale.atoi(str(x))
##
##    for type in int, long, float, locale.atoi, locale.atof: # or wrappedLocaleAtoi
##        try:
##            return type(variant)
##        except: #(TypeError, ValueError):
##            pass
##    raise ValueError('%s is not a NUMBER' % variant)
...
def cvtFloat(variant):
	try:
		...
		pass
	if type(variant)==types.TupleType:
		return cvtMoney(variant)
	try:
		europeVsUS=string.translate(str(variant),string.maketrans(".,",",."))
...
##    if adType == adCurrency:
##        return cvtMoney(variant)
##    if adType in (adBigInt,adUnsignedBigInt):
##        return long(variant)
##    if adType in (adInteger,adSmallInt,adTinyInt,adUnsignedInt,adUnsignedSmallInt,adUnsignedTinyInt):
##        return int(variant)
##    if adType == NUMBER:
##        try:
##            return float(variant)
...
##            return cvtMoney(variant)
##        except ValueError:
##            #A comma is a dot in (for example) Sweden, the regional settings are used in the variant
##            #TODO make this better using the specific type of variant.
##            #Can someone help?
##            strvar=str(variant)
##            i=string.find(strvar, ",")
##            if i!= -1:
##                strvar=string.replace(strvar,",",".")
##                return float(strvar)
##            raise
##    return variant
...
adoExactNumericTypes=(adDecimal,adNumeric,adVarNumeric)
adoApproximateNumericTypes=(adDouble,adSingle,adCurrency) #Currency is returned as a tuple
...
try:
	typeMap[types.BooleanType]= adBoolean
except AttributeError:
	pass # Python version less than 2.2
...
try: #If mx extensions are installed, use mxDateTime
	...
	try: #if Python 2.3 or later, use datetimelibrary by default
		import datetime
		dateconverter=pythonDateTimeConverter()
	except:
		dateconverter=pythonTimeConverter()
