...
namespace GuiTestDriver
{
	...
	public class Utilities
	{
		...
		{ // a null '' is not a number
			...
			{
				Assert.Fail("Attribute number '"+(string)arg+"' is too big (or negative) to convert" );
			}
			catch
			{
		...
		static public string evalRef(string refer, out bool found)
		{
			...
			{ // no dot, get the default data or function value
				value = Functions.identifyAndEvaluate(refer);
				found = value != null;
				if (!found) ins = ts.Instruction(refer);
			}
			else
			{ // has a dot, so split it and get the parts.
				...
				{data = null;}
				ins = ts.Instruction(refer2);
				if (data == "") data = null;
			}
			if (!found)
			{
				found = ins != null;
				if (found) value = ins.GetDataImage(data);
				else value = refer; // not a reference
			}
			return value;
		}
		...
		static public Microsoft.Win32.RegistryKey parseRegKey(string regKey, out string key)
		{
			...
			case "HKEY_USERS":
				...
				break;
			default:
				log.paragraph("Registry HKEY_ expected in " + regKey);
				break;
			}
			return regkey;
		}
