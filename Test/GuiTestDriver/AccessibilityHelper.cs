...
namespace GuiTestDriver
{
	...
	public class AccessibilityHelper : AccessibilityHelperBase
	{
		...
		public override int HWnd
		{
			get
			{
				...
				catch (System.NullReferenceException e)
				{
					...
					log.paragraph(@"HWnd could not get a window handle because of "+e.Message);
				}
				return hWind;
			}
		}
		...
		public override string Name
		{
			get
			{
				...
				catch (System.NullReferenceException e)
				{
					name = "";
					Logger log = Logger.getOnly();
					log.paragraph(@"Name could not get a name because of "+e.Message);
				}
				if (name == null) name = "";
				return name;
			}
		...
		public override AccessibleRole Role
		{
			get
			{
				...
				catch (System.NullReferenceException e)
				{
					...
					Logger log = Logger.getOnly();
					log.paragraph(@"Role could not get a role because of "+e.Message);
				}
				return role;
			}
		}
		...
		public override AccessibleStates States
		{
			get
			{
				...
				catch (System.NullReferenceException e)
				{
					...
					Logger log = Logger.getOnly();
					log.paragraph(@"States could not get a states because of "+e.Message);
				}
				return states;
			}
		}
		...
		public override string Shortcut
		{
			get
			{
				...
				catch (System.NullReferenceException e)
				{
					shortcut = "";
					Logger log = Logger.getOnly();
					log.paragraph(@"Shortcut could not get a shortcut because of "+e.Message);
				}
				return shortcut;
			}
		}
		...
		public override string DefaultAction
		{
			get
			{
				...
				catch (System.NullReferenceException e)
				{
					action = "";
					Logger log = Logger.getOnly();
					log.paragraph(@"DefaultAction could not get an action because of "+e.Message);
				}
				return action;
			}
		}
		...
		public override string Value
		{
			get
			{
				...
				catch (System.NullReferenceException e)
				{
					valu = "";
					Logger log = Logger.getOnly();
					log.paragraph(@"Value could not get a value because of "+e.Message);
				}
		...
		public override int ChildCount
		{
			get
			{
				...
				catch (System.NullReferenceException e)
				{
					Children = 0;
					Logger log = Logger.getOnly();
					log.paragraph(@"ChildCount could not get a count because of "+e.Message);
				}
				return  Children;
			}
		}
		...
		public AccessibilityHelper Parent
		{
			get
			{
				...
				catch (System.NullReferenceException e)
				{
					parent = null;
					Logger log = Logger.getOnly();
					log.paragraph(@"Parent1 could not get a paarent because of "+e.Message);
				}
				return  parent;
			}
		}
		...
		private AccessibilityHelper SearchPathByBreadth(GuiPath path, IPathVisitor visitor)
		{
			...
			int place = path.Nth;
			Logger log = Logger.getOnly();
			log.startList("SearchPathByBreadth('" + path.toString() + "') starting from ('"
				+ this.Role + "','" + this.Name + "')");
			...
			while (lists.Count > 0)
			{
				...
				while (nodes.Count > 0)
				{
					...
					string logStr = "-> Child(" + lnum + ":" + (++nnum) + ") ('" + ah.Role + "','" + ah.Name + "')";
					if (path.Next != null && path.Next.Role == AccessibleRole.Alert) logStr += "val[" + ah.Value + "]";
					log.listItem(logStr);
		...
		{
			Logger log = Logger.getOnly();
			log.startList("SearchPathByDepth('" + path.toString() + "') starting from ('"
				+ this.Role + "','" + this.Name + "')");
			if (ChildCount <= 0)
			{
				if (visitor != null) visitor.notFound(path);
			...
			foreach (AccessibilityHelper child in this)
			{
				...
				// treat name == "" and child.Name == null as a match
				// Note: a null string shows as "" in the debugger!
				log.startList("SearchPathByDepth('" + path.toString() + "') Child: ('"
		...
		public AccessibilityHelper FindClient()
		{
			...
			{
				Logger log = Logger.getOnly();
				log.paragraph("FindClient found: ('" + ah.Role + "','" + ah.Name + "')");
			}
			return ah;
		}
