...
namespace GuiTestDriver
{
	...
	public class ApplicationContext : Context
	{
		...
		public void SetSource(string name)
		{
			...
			isTrue(insSource != null,"Source is null.");
			isTrue(insSource is ApplicationContext,"Source is not an on-application instruction."); // eventually or other types.
			m_source = insSource;
		}
		/// <summary>
		/// For Testing Only:
		...
		public override void Execute()
		{
			...
			if (contextFail == "assert" && failed != null && failed != "") fail(failed);
			if (contextPass == "assert" && failed == null) fail("run='"+m_run+"' exe='"+m_exeName+"' passed");
...
////			}
		...
		private XmlElement getXmlModelRoot()
		{
			...
			if (m_GuiModelPath != null && m_GuiModelPath != "")
			{
				...
				isNotNull(root, "Application cannot find GuiModel root");
				...
				if (sections != null)
				{
					foreach (XmlNode sec in sections)
					{
						try
						{
							...
							secDoc.Load(path);
							isNotNull(secDoc, "XML Document '" + path + "' is empty");

							XmlDocumentFragment secFrag = gmDoc.CreateDocumentFragment();
							isNotNull(secFrag, "XML Fragment '" + path + "' is empty");
							...
							isNotNull(child, "XML Fragment '" + path + "' has no child");
						...
						{
							fail("Failed to read a section of the GUI Model because: " + e.Message);
						}
					}
				}
			}
		...
		private void ReadModelVars()
		{
			...
			if (vars != null)
			{
				...
				{
					Var varObj = XmlInstructionBuilder.CreateVar(varNode, this);
				}
			}
		}
