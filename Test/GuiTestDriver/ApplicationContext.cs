...
namespace GuiTestDriver
{
	...
	public class OnApplication : Context
	{
		...
		{   /*A process has a main window associated with it
			...
			m_log = Logger.getOnly();
			m_log.paragraph("Waiting hwnd = "+hwnd.ToInt32()+" for proc = "+proc.Id+" called "+proc.MainWindowTitle);
			int startTick = System.Environment.TickCount;
			// Before the main window forms, the handle can be 0,
			// or it may have the title of the splash screen
			while (!proc.HasExited && 0 == hwnd.ToInt32()
				   || (proc.MainWindowTitle != null
					   && (proc.MainWindowTitle == "RealSplashScreen"
						   || proc.MainWindowTitle == "Language Explorer" )))
			{
