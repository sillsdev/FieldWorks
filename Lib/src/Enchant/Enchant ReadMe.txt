...
To build the version we are using as of June 20 2011, you start by getting the enchant source from its SVN repository at http://svn.abisource.com/enchant/trunk.

Next apply the patches "Enchant patch.patch" and "Patch pwl.c to improve enchant performance.patch" from this directory. These patches are currently based on revision 29482; if the trunk of Enchant has moved on, you may need to get that exact revision, or you may want to try and see whether the patch works on the latest version. (TortoiseSVN has a mechanism to make and apply patches like this.) If you come up with a working patch based on a later revision of Enchant, please update the patch and this file.
