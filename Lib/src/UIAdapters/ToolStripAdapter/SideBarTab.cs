...
namespace SIL.FieldWorks.Common.UIAdapters
{
	...
	internal class SideBarTab : ButtonItem
	{
		...
		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Gets the sub menu item properties.
		/// </summary>
		/// <value>The sub menu item properties.</value>
		/// ------------------------------------------------------------------------------------
		internal TMItemProperties[] SubMenuItemProperties
		{
			get
			{
				List<TMItemProperties> props = new List<TMItemProperties>();

				foreach (SBTabItemProperties item in Properties.Items)
				{
					TMItemProperties itemProps = new TMItemProperties();
					itemProps.Message = item.Message;
					...
					if (m_itemPanel.ImageListSmall != null)
						itemProps.Image = m_itemPanel.ImageListSmall.Images[item.ImageIndex];
