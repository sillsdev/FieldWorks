		...
		void Test(string[,] result, string text)
		{
			...
			for (int i = 0; i <= result.GetUpperBound(0); ++i)
			{
				...
				if (result.GetUpperBound(1) == 2)
					Assert.That(tts[i].FirstToken.ScrRefString, Is.EqualTo(result[i, 2]), "Reference number: " + i);

				Assert.That(tts[i].Text, Is.EqualTo(result[i, 0]), "Text number: " + i.ToString());
				Assert.That(tts[i].Message, Is.EqualTo(result[i, 1]), "Message number: " + i.ToString());
			}
		}
