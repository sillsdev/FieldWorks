...
namespace SIL.FieldWorks.FDO.CoreTests.PersistingLayerTests
{
	...
	public sealed class BEPPortTests
	{
		...
		public void TestSetup()
		{
			...
			m_fdoDirectories = new TestFdoDirectories(m_projectsFolder.Path);
		}
		...
		private BackendStartupParameter GenerateBackendStartupParameters(bool isTarget, FDOBackendProviderType type)
		{
			...
			switch (type)
			{
				case FDOBackendProviderType.kXMLWithMemoryOnlyWsMgr:
					name = Path.Combine(m_projectsFolder.Path, FdoFileHelper.GetXmlDataFileName("TLP" + nameSuffix));
					break;
		...
		public void PortAllBEPsTestsUsingAnAlreadyOpenedSource(
			[Values(FDOBackendProviderType.kXMLWithMemoryOnlyWsMgr, FDOBackendProviderType.kMemoryOnly)]
			...
			FDOBackendProviderType targetType)
		{
			...
			using (var sourceCache = FdoCache.CreateCacheWithNewBlankLangProj(sourceProjectId, "en", "fr", "en", new DummyFdoUI(),
				m_fdoDirectories, new FdoSettings()))
			{
				...
				using (var targetCache = FdoCache.CreateCacheCopy(targetProjectId, "en", new DummyFdoUI(),
					m_fdoDirectories, new FdoSettings(), sourceCache))
				{
					// BEP is a singleton, so we shouldn't call Dispose on it. This will be done
					// by service locator.
		...
		public void PortAllBEPsTestsUsingAnUnopenedSource(
			[Values(FDOBackendProviderType.kXMLWithMemoryOnlyWsMgr)]
			FDOBackendProviderType sourceType,
			[Values(FDOBackendProviderType.kXMLWithMemoryOnlyWsMgr, FDOBackendProviderType.kMemoryOnly)]
			FDOBackendProviderType targetType)
		{
