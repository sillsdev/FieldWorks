...
namespace SIL.FieldWorks.FDO.DomainServices
{
	...
	public static class DataStoreInitializationServices
	{
		...
		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Ensures that all the old key terms list with its leftover key term hierarchical
		/// structures has been removed.
		/// </summary>
		/// ------------------------------------------------------------------------------------
		private static void RemoveOldKeyTermsList(FdoCache cache)
		{
			ILangProject lp = cache.LanguageProject;
			IScripture scr = lp.TranslatedScriptureOA;
			if (scr == null || scr.RemovedOldKeyTermsList)
				return; // Has already been done or not needed

			ICmPossibilityList oldList = lp.CheckListsOC.FirstOrDefault(l =>
				l.Guid != CmPossibilityListTags.kguidChkKeyTermsList && l.PossibilitiesOS.Any(p => p is IChkTerm && ((IChkTerm)p).OccurrencesOS.Count == 0));
			if (oldList != null)
				oldList.Delete();
