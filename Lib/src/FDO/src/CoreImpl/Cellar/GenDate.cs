...
namespace SIL.CoreImpl.Cellar
{
	...
	public struct GenDate : IEquatable<GenDate>, IComparable, IComparable<GenDate>
	{
		...
		public string ToLongString()
		{
			...
			switch (m_precision)
			{
				case PrecisionType.Approximate:
					sb.Append(Properties.Resources.ksGenDateApprox);
					sb.Append(" ");
					break;
				case PrecisionType.After:
					sb.Append(Properties.Resources.ksGenDateAfter);
					sb.Append(" ");
					break;
				case PrecisionType.Before:
					sb.Append(Properties.Resources.ksGenDateBefore);
					sb.Append(" ");
					break;
			}
			...
			{
				sb.Append(Properties.Resources.ksGenDateAD);
				sb.Append(" ");
			}
			...
			if (m_year != UnknownYear)
			{
				...
				{
					sb.Append(" ");
					sb.Append(Properties.Resources.ksGenDateBC);
				}
			}
			return sb.ToString();
		}
		...
		public static bool TryParse(string date, out GenDate gen)
		{
			...
			date = date.Trim();
			if (date.StartsWith(Properties.Resources.ksGenDateApprox))
			{
				precision = PrecisionType.Approximate;
				date = date.Substring(Properties.Resources.ksGenDateApprox.Length + 1);
			}
			else if (date.StartsWith(Properties.Resources.ksGenDateAfter))
			{
				precision = PrecisionType.After;
				date = date.Substring(Properties.Resources.ksGenDateAfter.Length + 1);
			}
			else if (date.StartsWith(Properties.Resources.ksGenDateBefore))
			{
				precision = PrecisionType.Before;
				date = date.Substring(Properties.Resources.ksGenDateBefore.Length + 1);
			}
			if (date.Contains(Properties.Resources.ksGenDateAD + " "))
			{
				fAD = true;
				date = date.Replace(Properties.Resources.ksGenDateAD + " ", "");
			}
			else if (date.EndsWith(Properties.Resources.ksGenDateBC))
			{
				fAD = false;
				date = date.Substring(0, date.Length - (Properties.Resources.ksGenDateBC.Length + 1));
			}
			int nDay = UnknownDay;
