...
namespace SilEncConverters40
{
	...
	public class EncConverters : Hashtable, IEncConverters
	{
		...
		public const string CstrRegKeyPluginFolder = "DeveloperPluginDir";
#else
...
#endif
#else
		public const string HKLM_CNVTRS_SUPPORTED       = @"SOFTWARE\SIL\SilEncConverters40\ConvertersSupported";
		...
		protected void GetConversionEnginesSupported()
		{
			...
			//      of a .Net assembly (e.g. SilEncConverters40, Version=4.0.0.0, Culture=neutral, PublicKeyToken=f1447bae1e63f485
			...
			foreach (string strPluginXmlFile in astrPluginXmlFiles)
			{
				...
				foreach (EncConverterPlugins.ECPluginDetailsRow aDetailsRow in aEcPluginFile.ECPluginDetails)
				{
					try
					{
						...
						//  we can't, then just ignore it.
						var rConverter = InstantiateIEncConverter(aDetailsRow.ImplementationProgId,
						...
						//      <strImplementationType> = <strProgId> e.g. "SIL.tec" = "SilEncConverters40.TecEncConverter"
						...
						if (m_mapImplTypeToProgId.ContainsKey(strImplementationType))
						{
								...
								&&  (m_mapProgIdsToAssemblyName.ContainsKey(strProgId) && !aDetailsRow.IsAssemblyReferenceNull()))
							{
								...
								Debug.Assert(!String.IsNullOrEmpty(strExistingVersion), "Not expecting there to be no version # in the existing Assembly reference!?");
								string strNewVersion = ExtractVersion(aDetailsRow.AssemblyReference);
								Debug.Assert(!String.IsNullOrEmpty(strNewVersion), "Not expecting there to be no version # in the newer Assembly reference!?");
						...
						int priorityReg = aDetailsRow.Priority;
		...
		// get the "4.0.0.0" out of "SilEncConverters40, Version=4.0.0.0, Culture=neutral, PublicKeyToken=f1447bae1e63f485"
		...
		protected void RemoveItemsRelatedTo(string strImplementationType, string strProgId)
		{
			...
			Debug.Assert(m_mapImplementTypesPriority.ContainsKey(strImplementationType), "Bad assumption about priority values");
		...
		protected void GetConversionEnginesSupported()
		{
			...
			//  app, which just called SilEncConverters40.RegisterSelf), but that didn't work on
			...
			//  SOFTWARE\SIL\SilEncConverters40\ConvertersSupported!RegisterSelf
			...
			foreach( string sImplementType in asImplementTypes )
			{
				try
				{
					...
					//  "SIL.tec" = "SilEncConverters40.TecEncConverter"
		...
		/// <param name="use">indication of the prog ID of the implementing service (e.g. SilEncConverters40.TecEncConverter)</param>
...
#if !DontCheckAssemVersion
			return InstantiateIEncConverter(strProgID, null);
#else
			Type typeEncConverter = Type.GetTypeFromProgID(strProgID);
...
//				Marshal.ReleaseComObject(ecTmp);
		...
		public static void ThrowError(ErrStatus status, string strExtra)
		{
			...
			if( strRes != null )
			{
				...
				ResourceManager LocRM = new ResourceManager("SilEncConverters40.EncCnvtrs",typeof(EncConverters).Assembly);
