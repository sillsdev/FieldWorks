...
namespace SilEncConverters31
{
	...
	/// </summary>
	[GuidAttribute("7DBE9353-03D9-4999-AEC4-6BA292AA3F5C")]
	...
	public class AdaptItEncConverter : AdaptItKBReader
	{
		...
		public void SplitAndConvert(string strInput, out List<string> lstInputTokens, out List<string> lstInBetweenTokens, out List<string> lstOutputTokens)
		{
					...
					foreach (KeyValuePair<string, string> kvp in map)
					{
						...
						else if (mapReverseLookup.TryGetValue(kvp.Value, out strKey))
						{
							...
							if (strKey[0] == '%')
							{
								...
								// add the new value
								strKey = String.Format("%{0}%{1}%{2}%", nAmbCount + 1, strKey.Substring(nAmbValueLength + 2), kvp.Key);
							}
							else
							{
			...
			if (!m_bReverseLookup & (m_caDelimitersForward != null))
				achTrimChars = m_caDelimitersForward;
			else if (m_bReverseLookup & (m_caDelimitersReverse != null))
				achTrimChars = m_caDelimitersReverse;
			else
				achTrimChars = caSplitChars;
			...
			lstInBetweenTokens = new List<string>(astrTokens.Length + 1);
			for (nWordIndex = 0; nWordIndex < astrTokens.Length; nWordIndex++)
			{
				string strAdd = astrTokens[nWordIndex].Trim(achTrimChars);
				...
				lstInBetweenTokens.Add(strInput.Substring(nStartIndex, nLength));
				nStartIndex = nIndex + lstInputTokens[nWordIndex].Length;
			}
			lstInBetweenTokens.Add(strInput.Substring(nStartIndex));
			...
			for (nWordIndex = 0; nWordIndex < lstInputTokens.Count; nWordIndex++)
				strTest += lstInBetweenTokens[nWordIndex] + lstInputTokens[nWordIndex];
			strTest += lstInBetweenTokens[nWordIndex];
			...
			for (int nMapNumber = Math.Min(lstInputTokens.Count, 10); nMapNumber > 0; nMapNumber--)
			{
				...
				for (nWordIndex = 0; nWordIndex <= (lstInputTokens.Count - nMapNumber); nWordIndex++)
				{
					...
					DoNextWord:
						if (lstInBetweenTokens[nWordIndex + nWordsInPhrase] != " ")
						{
					...
					if (mapLookup.TryGetValue(strSearchToken, out strLookedup))
					{
						...
						//  for this.
						AdjustLists(strSearchToken, strLookedup, nMapNumber, nWordIndex, ref lstInputTokens, ref lstOutputTokens, ref lstInBetweenTokens);
					}
				}
			}
		...
		protected void AdjustLists(string strSearchToken, string strReplacement, int nNumWords, int nWordIndex,
			ref List<string> lstInputTokens, ref List<string> lstOutputTokens, ref List<string> lstInBetweenTokens)
		{
			...
			while (--nNumToClear > 0)
			{
				...
				lstInBetweenTokens.RemoveAt(nWordIndex + 1);
			}
		...
		protected string CheckInHashtableValues(string strInput, Dictionary<string, string> mapLookup)
		{
			...
			{
				string strValue = (string)kvp.Value;
