...
{
	/// <summary>
	/// Unit tests for DataMigration7000071
	/// </summary>
	[TestFixture]
	public class DataMigration7000071Tests : DataMigrationTestsBase
	{
		...
		/// <summary>
		/// Test the migration from version 7000070 to 7000071.
		/// </summary>
		[Test]
		public void DataMigration7000071Test()
		{
			string projectFolder = Path.Combine(Path.GetTempPath(), "DataMigration7000071Tests");
			try
			{
				if (Directory.Exists(projectFolder))
					Directory.Delete(projectFolder, true);
				Directory.CreateDirectory(projectFolder);
				string storePath = Path.Combine(projectFolder, LcmFileHelper.ksWritingSystemsDir);
				Directory.CreateDirectory(storePath);
				string testEnglishPath = Path.Combine(storePath, "en.ldml");
				File.Copy(Path.Combine(TestDirectoryFinder.TestDataDirectory, "en_7000070.ldml"), testEnglishPath);
				...
				File.Copy(Path.Combine(TestDirectoryFinder.TestDataDirectory, "am-Ethi_7000070.ldml"), xkalPath);
				...
				string settingsFolder = Path.Combine(projectFolder, LcmFileHelper.ksConfigurationSettingsDir);
				...
				// Verification Phase
				Assert.AreEqual(7000071, dtoRepos.CurrentModelVersion, "Wrong updated version.");
