...
namespace SIL.LCModel.DomainServices
{
	...
	class ArrayBasedOwningSequence<T> : ILcmOwningSequence<T> where T : class, ICmObject
	{
		...
		public ArrayBasedOwningSequence(LcmCache cache, IScrTxtPara owner)
		{
			m_cache = cache;
			m_owner = owner;
		}
		...
		#region ILcmOwningSequence<T> Members
		public void MoveTo(int iStart, int iEnd, ILcmOwningSequence<T> seqDest, int iDestStart)
		{
			throw new NotImplementedException();
		}
		#endregion

		#region ILcmList<T> Members
	...
	class ArrayBasedOwningCollection<T> : ILcmOwningCollection<T> where T : class, ICmObject
	{
		private static int s_fakeHvo = 50000;
		...
		public ArrayBasedOwningCollection(LcmCache cache, IScrTxtPara owner)
		{
			m_cache = cache;
			m_owner = owner;
		}
	...
	/// ----------------------------------------------------------------------------------------
	[TestFixture]
	public class DataStoreInitializationServicesTests : ScrInMemoryLcmTestBase
	{
		...
		{
			base.TestSetup();
			Cache.FullyInitializedAndReadyToRock = false;
			m_para = MockRepository.GenerateStub<IScrTxtPara>();
			ILcmOwningSequence<ISegment> mockedSegments = new ArrayBasedOwningSequence<ISegment>(Cache, m_para);
			m_para.Stub(p => p.SegmentsOS).Return(mockedSegments);
			m_para.Stub(p => p.Services).Return(Cache.ServiceLocator);
			ILcmOwningCollection<ICmTranslation> mockedTranslations = new ArrayBasedOwningCollection<ICmTranslation>(Cache, m_para);
		...
		{
			Cache.ServiceLocator.GetInstance<ICmTranslationFactory>().Create(
		...
		{
			ICmPossibility btType = Cache.ServiceLocator.GetInstance<ICmPossibilityRepository>().GetObject(CmPossibilityTags.kguidTranBackTranslation);
			Cache.ServiceLocator.GetInstance<ICmTranslationFactory>().Create(m_para, btType);
			ICmTranslation existingTrans2 = MockRepository.GenerateStub<ICmTranslation>();
			existingTrans2.TypeRA = btType;
			m_para.TranslationsOC.Add(existingTrans2);
			IMultiString translation = MockRepository.GenerateStub<IMultiString>();
		...
		{
			ICmTranslation existingTrans = new CmTranslation();
		...
		{
			ICmTranslation existingTrans1 = new CmTranslation();
			m_para.TranslationsOC.Add(existingTrans1);
			ICmTranslation existingTrans2 = new CmTranslation();
			m_para.TranslationsOC.Add(existingTrans2);
			DataStoreInitializationServices.EnsureBtForPara(m_para);
		...
		public void EnsureSegmentsForPara_NoContents()
		{
			...
			DataStoreInitializationServices.EnsureSegmentsForPara(m_para);
		...
		public void ParaSegmentsValid_Valid()
		{
			...
			bool result = DataStoreInitializationServices.ParaSegmentsValid(m_para, true, true);
		...
		public void ParaSegmentsValid_ValidOrcSegmentsNoVerseNumbers()
		{
			...
			bldr.Replace(0, 0, "Short sentence. Another sentence. Hahahahaha",
				StyleUtils.CharStyleTextProps(null, Cache.DefaultVernWs));
			TsStringUtils.InsertOrcIntoPara(Guid.NewGuid(), FwObjDataTypes.kodtOwnNameGuidHot, bldr, 15, 15, Cache.DefaultVernWs);
			TsStringUtils.InsertOrcIntoPara(Guid.NewGuid(), FwObjDataTypes.kodtOwnNameGuidHot, bldr, 24, 24, Cache.DefaultVernWs);
		...
		public void ParaSegmentsValid_ValidOrcSegmentsWithVerseNumbers()
		{
			...
			TsStringUtils.InsertOrcIntoPara(Guid.NewGuid(), FwObjDataTypes.kodtOwnNameGuidHot, bldr, 18, 18, Cache.DefaultVernWs);
			TsStringUtils.InsertOrcIntoPara(Guid.NewGuid(), FwObjDataTypes.kodtOwnNameGuidHot, bldr, 37, 37, Cache.DefaultVernWs);
		...
		{
			ILcmServiceLocator sl = Cache.ServiceLocator;
		...
		public void FixSegmentsForScriptureParas_FixFourSegmentsWithOrcNotIsolated()
		{
			...
			TsStringUtils.InsertOrcIntoPara(footnote1, FwObjDataTypes.kodtOwnNameGuidHot, bldr, 20, 20, Cache.DefaultVernWs);
			para.Contents = bldr.GetString(); // automatically creates TWO segments
			para.ParseIsCurrent = false;
			DataStoreInitializationServices.FixSegmentsForScriptureParas(Cache);
			VerifyParaSegmentBreaks(para);
		...
		public void FixSegmentsForPara_ORCsInMiddleOfParaWithFullTrans()
		{
			...
			TsStringUtils.InsertOrcIntoPara(footnote1, FwObjDataTypes.kodtOwnNameGuidHot, bldr, 7, 7, Cache.DefaultVernWs);
			TsStringUtils.InsertOrcIntoPara(footnote2, FwObjDataTypes.kodtOwnNameGuidHot, bldr, 19, 19, Cache.DefaultVernWs);
			TsStringUtils.InsertOrcIntoPara(footnote3, FwObjDataTypes.kodtOwnNameGuidHot, bldr, 29, 29, Cache.DefaultVernWs);
			m_para.Contents = bldr.GetString();
			...
			TsStringUtils.InsertOrcIntoPara(footnote1, FwObjDataTypes.kodtNameGuidHot, bldr, 5, 5, Cache.DefaultAnalWs);
			TsStringUtils.InsertOrcIntoPara(footnote2, FwObjDataTypes.kodtNameGuidHot, bldr, 14, 14, Cache.DefaultAnalWs);
			TsStringUtils.InsertOrcIntoPara(footnote3, FwObjDataTypes.kodtNameGuidHot, bldr, 20, 20, Cache.DefaultAnalWs);
		...
		public void FixSegmentsForPara_OrcFollowsHardLineBreak()
		{
			...
			bldr.Replace(0, 0, strSeg1 + StringUtils.kChHardLB,
				StyleUtils.CharStyleTextProps(null, Cache.DefaultVernWs));
			Guid footnote1 = Guid.NewGuid();
			TsStringUtils.InsertOrcIntoPara(footnote1, FwObjDataTypes.kodtOwnNameGuidHot, bldr,
		...
		public void FixSegmentsForPara_OrcPrecedesHardLineBreakInMiddle()
		{
			...
			TsStringUtils.InsertOrcIntoPara(footnote1, FwObjDataTypes.kodtNameGuidHot, bldr, bldr.Length,
		...
		public void FixSegmentsForPara_OrcFollowsHardLineBreakInMiddle()
		{
			...
			TsStringUtils.InsertOrcIntoPara(footnote1, FwObjDataTypes.kodtNameGuidHot, bldr, 0, 0, Cache.DefaultAnalWs);
		...
		public void FixSegmentsForPara_EmptyTranslation()
		{
			...
			Guid footnote1 = Guid.NewGuid();
			TsStringUtils.InsertOrcIntoPara(footnote1, FwObjDataTypes.kodtOwnNameGuidHot, bldr, 6, 6, Cache.DefaultVernWs);
			m_para.Contents = bldr.GetString();
		...
		public void FixSegmentsForPara_PictureORC()
		{
			...
			TsStringUtils.InsertOrcIntoPara(picture1, FwObjDataTypes.kodtGuidMoveableObjDisp, bldr, 18, 18, Cache.DefaultVernWs);
			m_para.Contents = bldr.GetString();
			...
			// Picture goes here:            |
			bldr.Replace(0, 0, "I ate a whole cow yesterday", StyleUtils.CharStyleTextProps(null, Cache.DefaultAnalWs));
			TsStringUtils.InsertOrcIntoPara(picture1, FwObjDataTypes.kodtGuidMoveableObjDisp, bldr, 13, 13, Cache.DefaultAnalWs);
			ITsString expectedSegment0FT = bldr.GetString();
		...
		public void FixSegmentsForPara_FreeTransBeforeORCHasTrailingSpace()
		{
			...
			StyleUtils.CharStyleTextProps(null, Cache.DefaultVernWs));
			Guid picture1 = Guid.NewGuid();
			TsStringUtils.InsertOrcIntoPara(picture1, FwObjDataTypes.kodtGuidMoveableObjDisp, bldr, 19, 19, Cache.DefaultVernWs);
			m_para.Contents = bldr.GetString();
			...
			// Picture goes here:             |
			bldr.Replace(0, 0, "I ate a whole cow yesterday", StyleUtils.CharStyleTextProps(null, Cache.DefaultAnalWs));
			TsStringUtils.InsertOrcIntoPara(picture1, FwObjDataTypes.kodtGuidMoveableObjDisp, bldr, 14, 14, Cache.DefaultAnalWs);
			ITsString expectedSegment0FT = bldr.GetString();
		...
		public void FixSegmentsForPara_ORCsAtBeginningAndEndOfPara()
		{
			...
			Guid footnote2 = Guid.NewGuid();
			TsStringUtils.InsertOrcIntoPara(footnote1, FwObjDataTypes.kodtOwnNameGuidHot, bldr, 0, 0, Cache.DefaultVernWs);
			TsStringUtils.InsertOrcIntoPara(footnote2, FwObjDataTypes.kodtOwnNameGuidHot, bldr, bldr.Length, bldr.Length, Cache.DefaultVernWs);
			m_para.Contents = bldr.GetString();
			...
			TsStringUtils.InsertOrcIntoPara(footnote2, FwObjDataTypes.kodtNameGuidHot, bldr, bldr.Length, bldr.Length, Cache.DefaultAnalWs);
			ITsString expectedSegment1FT = bldr.GetString();
		...
		public void FixSegmentsForPara_OrcBetweenSentencesWithFullTrans()
		{
			...
			Guid footnote1 = Guid.NewGuid();
			Guid footnote2 = Guid.NewGuid();
			TsStringUtils.InsertOrcIntoPara(footnote1, FwObjDataTypes.kodtOwnNameGuidHot, bldr, 21, 21, Cache.DefaultVernWs);
			TsStringUtils.InsertOrcIntoPara(footnote2, FwObjDataTypes.kodtOwnNameGuidHot, bldr, 37, 37, Cache.DefaultVernWs);
			m_para.Contents = bldr.GetString();
			...
			TsStringUtils.InsertOrcIntoPara(footnote1, FwObjDataTypes.kodtNameGuidHot, bldr, bldr.Length, bldr.Length, Cache.DefaultAnalWs);
			ITsString expectedSegment0FT = bldr.GetString();

			bldr.Clear();
			// Footnotes here:  |
			bldr.Replace(0, 0, "FT of sentence 3.", StyleUtils.CharStyleTextProps(null, Cache.DefaultAnalWs));
			TsStringUtils.InsertOrcIntoPara(footnote2, FwObjDataTypes.kodtNameGuidHot, bldr, 0, 0, Cache.DefaultAnalWs);
			ITsString expectedSegment2FT = bldr.GetString();
		...
		public void FixSegmentsForPara_OrcAdjacentToVerseNumbers()
		{
			...
			Guid footnote2 = Guid.NewGuid();
			Guid footnote3 = Guid.NewGuid();
			TsStringUtils.InsertOrcIntoPara(footnote1, FwObjDataTypes.kodtOwnNameGuidHot, bldr, 2, 2, Cache.DefaultVernWs);
			TsStringUtils.InsertOrcIntoPara(footnote2, FwObjDataTypes.kodtOwnNameGuidHot, bldr, 23, 23, Cache.DefaultVernWs);
			TsStringUtils.InsertOrcIntoPara(footnote3, FwObjDataTypes.kodtOwnNameGuidHot, bldr, 25, 25, Cache.DefaultVernWs);
			m_para.Contents = bldr.GetString();
			...
			TsStringUtils.InsertOrcIntoPara(footnote3, FwObjDataTypes.kodtNameGuidHot, bldr, 0, 0, Cache.DefaultAnalWs);
		...
		public void FixSegmentsForPara_OrcBetweenChapterAndVerseNumbers()
		{
			...
			Guid footnote1 = Guid.NewGuid();
			Guid footnote2 = Guid.NewGuid();
			TsStringUtils.InsertOrcIntoPara(footnote1, FwObjDataTypes.kodtOwnNameGuidHot, bldr, 1, 1, Cache.DefaultVernWs);
			TsStringUtils.InsertOrcIntoPara(footnote2, FwObjDataTypes.kodtOwnNameGuidHot, bldr, 3, 3, Cache.DefaultVernWs);
			m_para.Contents = bldr.GetString();
		...
		public void FixSegmentsForPara_OrcBeforeSpaceBeforeVerseNumber()
		{
			...
			Guid footnote1 = Guid.NewGuid();
			TsStringUtils.InsertOrcIntoPara(footnote1, FwObjDataTypes.kodtOwnNameGuidHot, bldr, 10, 10, Cache.DefaultVernWs);
			m_para.Contents = bldr.GetString();
			...
			// Footnotes here:                |
			bldr.Replace(0, 0, "FT of verse 1.", StyleUtils.CharStyleTextProps(null, Cache.DefaultAnalWs));
			TsStringUtils.InsertOrcIntoPara(footnote1, FwObjDataTypes.kodtNameGuidHot, bldr, 14, 14, Cache.DefaultAnalWs);
			ITsString expectedSegment2FT = bldr.GetString();
		...
		public void FixSegmentsForPara_OrcsBetweenVerseNumbersWithSpaces()
		{
			...
			TsStringUtils.InsertOrcIntoPara(footnote1, FwObjDataTypes.kodtOwnNameGuidHot, bldr, 2, 2, Cache.DefaultVernWs);
			Guid footnote2 = Guid.NewGuid();
			TsStringUtils.InsertOrcIntoPara(footnote2, FwObjDataTypes.kodtOwnNameGuidHot, bldr, 5, 5, Cache.DefaultVernWs);
			m_para.Contents = bldr.GetString();
			...
			bldr.Clear();
			TsStringUtils.InsertOrcIntoPara(footnote1, FwObjDataTypes.kodtNameGuidHot, bldr, 0, 0, Cache.DefaultAnalWs);
			TsStringUtils.InsertOrcIntoPara(footnote2, FwObjDataTypes.kodtNameGuidHot, bldr, 1, 1, Cache.DefaultAnalWs);
			ITsString expectedSegment1FT = bldr.GetString();
		...
		public void FixSegmentsForPara_DualOrcsBetweenVerseNumbers()
		{
			...
			Guid footnote2 = Guid.NewGuid();
			TsStringUtils.InsertOrcIntoPara(footnote1, FwObjDataTypes.kodtOwnNameGuidHot, bldr, 1, 1, Cache.DefaultVernWs);
			TsStringUtils.InsertOrcIntoPara(footnote2, FwObjDataTypes.kodtOwnNameGuidHot, bldr, 2, 2, Cache.DefaultVernWs);
			m_para.Contents = bldr.GetString();
		...
		public void FixSegmentsForPara_DualOrcsBeforeVerseNumberAtStartOfPara()
		{
			...
			Guid footnote2 = Guid.NewGuid();
			TsStringUtils.InsertOrcIntoPara(footnote1, FwObjDataTypes.kodtOwnNameGuidHot, bldr, 0, 0, Cache.DefaultVernWs);
			TsStringUtils.InsertOrcIntoPara(footnote2, FwObjDataTypes.kodtOwnNameGuidHot, bldr, 1, 1, Cache.DefaultVernWs);
			m_para.Contents = bldr.GetString();
		...
		public void FixSegmentsForPara_DualOrcsBeforeVerseNumberMidPara()
		{
			...
			Guid footnote1 = Guid.NewGuid();
			Guid footnote2 = Guid.NewGuid();
			TsStringUtils.InsertOrcIntoPara(footnote1, FwObjDataTypes.kodtOwnNameGuidHot, bldr, ich, ich, Cache.DefaultVernWs);
			TsStringUtils.InsertOrcIntoPara(footnote2, FwObjDataTypes.kodtOwnNameGuidHot, bldr, ich + 1, ich + 1, Cache.DefaultVernWs);
			m_para.Contents = bldr.GetString();
		...
		public void FixSegmentsForPara_OrcFollowingVerseNumberAtEndOfPara()
		{
			...
			Guid footnote1 = Guid.NewGuid();
			TsStringUtils.InsertOrcIntoPara(footnote1, FwObjDataTypes.kodtOwnNameGuidHot, bldr, bldr.Length, bldr.Length, Cache.DefaultVernWs);
			m_para.Contents = bldr.GetString();
		...
		public void FixSegmentsForPara_OrcRightBeforePunctAtEndOfPara()
		{
			...
			TsStringUtils.InsertOrcIntoPara(footnote1, FwObjDataTypes.kodtNameGuidHot, bldr, 16, 16, Cache.DefaultAnalWs);
			ITsString expectedSegment0FT = bldr.GetString();
		...
		public void FixSegmentsForPara_DualOrcsWithFullTrans()
		{
			...
			TsStringUtils.InsertOrcIntoPara(footnote1, FwObjDataTypes.kodtOwnNameGuidHot, bldr, 7, 7, Cache.DefaultVernWs);
			TsStringUtils.InsertOrcIntoPara(footnote2, FwObjDataTypes.kodtOwnNameGuidHot, bldr, 8, 8, Cache.DefaultVernWs);
			TsStringUtils.InsertOrcIntoPara(footnote3, FwObjDataTypes.kodtOwnNameGuidHot, bldr, 23, 23, Cache.DefaultVernWs);
			TsStringUtils.InsertOrcIntoPara(footnote4, FwObjDataTypes.kodtOwnNameGuidHot, bldr, 24, 24, Cache.DefaultVernWs);
			TsStringUtils.InsertOrcIntoPara(footnote5, FwObjDataTypes.kodtOwnNameGuidHot, bldr, 40, 40, Cache.DefaultVernWs);
			TsStringUtils.InsertOrcIntoPara(footnote6, FwObjDataTypes.kodtOwnNameGuidHot, bldr, 41, 41, Cache.DefaultVernWs);
			m_para.Contents = bldr.GetString();
			...
			bldr.Replace(0, 0, "FT of sentence1.", StyleUtils.CharStyleTextProps(null, Cache.DefaultAnalWs));
			TsStringUtils.InsertOrcIntoPara(footnote1, FwObjDataTypes.kodtNameGuidHot, bldr, 5, 5, Cache.DefaultAnalWs);
			TsStringUtils.InsertOrcIntoPara(footnote2, FwObjDataTypes.kodtNameGuidHot, bldr, 6, 6, Cache.DefaultAnalWs);
			TsStringUtils.InsertOrcIntoPara(footnote3, FwObjDataTypes.kodtNameGuidHot, bldr, bldr.Length, bldr.Length, Cache.DefaultAnalWs);
			TsStringUtils.InsertOrcIntoPara(footnote4, FwObjDataTypes.kodtNameGuidHot, bldr, bldr.Length, bldr.Length, Cache.DefaultAnalWs);
			ITsString expectedSegment0FT = bldr.GetString();

			bldr.Clear();
			// Dual FN here:    |
			bldr.Replace(0, 0, "FT of sentence 3.", StyleUtils.CharStyleTextProps(null, Cache.DefaultAnalWs));
			TsStringUtils.InsertOrcIntoPara(footnote5, FwObjDataTypes.kodtNameGuidHot, bldr, 0, 0, Cache.DefaultAnalWs);
			TsStringUtils.InsertOrcIntoPara(footnote6, FwObjDataTypes.kodtNameGuidHot, bldr, 1, 1, Cache.DefaultAnalWs);
			ITsString expectedSegment2FT = bldr.GetString();
		...
		public void FixSegmentsForPara_OrcsBetweenSentencesWithSpaces()
		{
			...
			TsStringUtils.InsertOrcIntoPara(footnote1, FwObjDataTypes.kodtOwnNameGuidHot, bldr, ich1, ich1, Cache.DefaultVernWs);
			int ich2 = ich1 + 2; // skip past the first footnote ORC and the space
			TsStringUtils.InsertOrcIntoPara(footnote2, FwObjDataTypes.kodtOwnNameGuidHot, bldr, ich2, ich2, Cache.DefaultVernWs);
			m_para.Contents = bldr.GetString();
		...
		public void FixSegmentsForPara_OrcSurroundedBySpaces()
		{
			...
			TsStringUtils.InsertOrcIntoPara(footnote, FwObjDataTypes.kodtOwnNameGuidHot, bldr, ich, ich, Cache.DefaultVernWs);
			m_para.Contents = bldr.GetString();
			...
			// Footnote here:   |
			bldr.Replace(0, 0, " FT of sentence 2.", StyleUtils.CharStyleTextProps(null, Cache.DefaultAnalWs));
			TsStringUtils.InsertOrcIntoPara(footnote, FwObjDataTypes.kodtNameGuidHot, bldr, 0, 0, Cache.DefaultAnalWs);
			ITsString expectedSegment1FT = bldr.GetString();
		...
		{
			Assert.IsTrue(DataStoreInitializationServices.ParaSegmentsValid(m_para, false, true), "Setup of test is probably wrong.");
			DataStoreInitializationServices.FixSegmentsForPara(m_para);
		}
			...
			string freeTrans, int cNotes, bool createAnalyses)
		{
			...
			if (createAnalyses)
				LcmTestHelper.CreateAnalyses(segment, para.Contents, ichBeginOffset, ichEndOffset);
			...
			IEnumerable<int> expectedWordforms, bool verifyAnalyses)
		{
			...
			if (verifyAnalyses)
				LcmTestHelper.VerifyAnalysis(seg, iSeg, expectedFormsWithoutAnalyses, expectedWordforms);
		}
