...
namespace SIL.LCModel.Application.Impl
{
	/// ----------------------------------------------------------------------------------------
	/// <summary>
	/// Tests for the LcmCache ISilDataAccess implementation.
	/// </summary>
	/// ----------------------------------------------------------------------------------------
	[TestFixture]
	public class DomainDataByFlidTests : MemoryOnlyBackendProviderRestoredForEachTestTestBase
	{
		...
		/// <summary>
		/// Make sure the ActionHandler is the LcmCache.
		/// </summary>
		[Test]
		...
		public void set_ObjectProp_Ref_Test()
		{
			...
			// Set it to null.
			m_sda.SetObjProp(anal.Hvo, WfiAnalysisTags.kflidCategory, LcmCache.kNullHvo);
		...
		public void UnknownTests()
		{
			...
			Assert.AreSame(tpp, style.Rules, "Not the same text props via LCM.");
		...
		/// get_VecSize method. This works on all of the four main LCM vector classes,
