...
namespace SIL.LCModel.DomainServices
{
	/// ----------------------------------------------------------------------------------------
	/// <summary>
	/// A class to facilitate deep copying of generic LCM objects.
	/// </summary>
	/// ----------------------------------------------------------------------------------------
	internal class CopyObject<TTopLevel> where TTopLevel : ICmObject
	{
		...
		// Source LCM object to Copy LCM object map; Key = source HVO, Value = Copied object
		...
		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Do a deep copy of a collection of LCM objects and all owned objects.
		/// 'Outside' references are duplicated. 'Inside' ones are set to the new copies.
		/// Basic/value properties are copied unchanged.
		/// </summary>
		/// <param name="source">Collection or Sequence of LCM objects to copy</param>
		/// <param name="ownerFunct">The delegate used for adding the created object to an owner.
		/// </param>
		/// <returns>Returned object is a List of ICmObjects, internally.</returns>
		/// ------------------------------------------------------------------------------------
		internal static IEnumerable<TTopLevel> CloneLcmObjects(IEnumerable<TTopLevel> source,
		...
		private IEnumerable<TTopLevel> CloneObjectsInternal(IEnumerable<TTopLevel> source)
		{
			...
			{
				m_topLevelObj = obj;
				result.Add(CloneLcmObjectsRecursively(obj));
			}

			// Set all the reference objects in the copies of these LCM objects
		...
		private TTopLevel CloneObjectInternal(TTopLevel source)
		{
			...
			CloneLcmObjectsRecursively(source);

			// Set all the reference objects in the copy of this LCM object
		...
		/// <param name="source">Collection of LCM objects to clone</param>
		/// <returns></returns>
		/// ------------------------------------------------------------------------------------
		private IEnumerable<TObj> CloneLcmObjectsRecursively<TObj>(IEnumerable<TObj> source)
			where TObj : ICmObject
		{
			List<TObj> newHvoColl = new List<TObj>();

			foreach (TObj obj in source)
				newHvoColl.Add(CloneLcmObjectsRecursively(obj));
		...
		private void HandleBasicOrStringFlid(int thisFlid, int flidType, int hvoSrc, int hvoNew)
		{
			...
			switch (flidType)
			{
				...
				case (int)CellarPropertyType.MultiUnicode:
					...
					break;
				default:
					throw new LcmInvalidFieldTypeException(String.Format("CopyObject: Unsupported field type {0}.", flidType));
			}
		}
			...
			switch (flidType)
			{
				...
				case (int)CellarPropertyType.OwningAtomic:
					...
					if (hvoAtomic > 0)
					{
						ICmObject srcAtomic = m_servLoc.ObjectRepository.GetObject(hvoAtomic);
						if (srcAtomic != null)
							CloneLcmObjectsRecursively(srcAtomic);
					}
					break;
				case (int)CellarPropertyType.OwningCollection:
				case (int)CellarPropertyType.OwningSequence:
					...
					CloneLcmObjectsRecursively((IEnumerable<ICmObject>)srcSeq);
					break;
