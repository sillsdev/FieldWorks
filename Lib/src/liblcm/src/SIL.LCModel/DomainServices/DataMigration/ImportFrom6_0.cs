...
namespace SIL.LCModel.DomainServices.DataMigration
{
	...
	public class ImportFrom6_0
	{
		...
		public bool Import(string pathname, string projectName, string destFolder, out string projectFile)
		{
			...
			projectFile = Path.Combine(folderName, projectName + LcmFileHelper.ksFwDataXmlFileExtension);
			...
			using (var zipFile = new ZipFile(pathname))
			{
				...
				foreach (ZipEntry entry in zipFile)
				{
					...
					if (!fHasXml && entry.Name.ToLowerInvariant().EndsWith(".xml") && entry.IsFile)
					{
						...
						try
						{
							// Next step is to run the converter. It should be in the same directory as LCM.dll
				...
				if (HaveFwSqlServer && HaveOldFieldWorks)
				{
					foreach (ZipEntry entry in zipFile)
					{
						if (entry.Name.ToLowerInvariant().EndsWith(".bak") && entry.IsFile)
						{
							...
							string msg = String.Format(Strings.ksRestoringToTempProject, project);
							string errMsgFmt = String.Format(Strings.ksRestoringToTempProjectFailed,
							project, "{0}", "{1}");
							if (!CreateTempDatabase(tempPath, msg, errMsgFmt))
								return false;
							string msg2 = String.Format(Strings.ksMigratingTempCopyToFw60, proj);
							string errMsgFmt2 = String.Format(Strings.ksMigratingTempCopyToFw60Failed,
							proj, "{0}", "{1}");
							if (!MigrateTempDatabase(msg2, errMsgFmt2))
								return false;
							string tempXmlPath = Path.ChangeExtension(tempPath, "xml");
							string msg3 = String.Format(Strings.ksWritingTempCopyAsFw60XML, proj);
							string errMsgFmt3 = String.Format(Strings.ksWritingTempCopyAsFw60XMLFailed,
							proj, "{0}", "{1}");
							...
							try
							{
								// Next step is to run the converter. It should be in the same directory as SIL.LCModel.dll
		...
		public bool DeleteTempDatabase()
		{
			return CallDbProgram(String.Format("delete {0}", TempDatabaseName),
				String.Format(Strings.ksDeletingOldTempDatabase, TempDatabaseName),
				String.Format(Strings.ksDeletingOldTempDatabaseFailed,
