...
namespace SIL.LCModel.DomainServices
{
	...
	public static class ScriptureServices
	{
		...
		/// Enhance JohnT: this may need to be public beyond LCM, in which case, it needs to move.
		/// </summary>
		...
		{
			return ScrResources.ResourceManager.GetString("kstidBookName" + bookNum);
		}
		...
		internal static void AdjustObjectsInArchivedBook(IScrBook book, IScrBook archivedBook)
		{
			...
			int iFootnote = 0;
			ILcmServiceLocator serviceLocator = book.Cache.ServiceLocator;
					...
					if (guid != Guid.Empty)
					{
						switch (odt)
						{
							case FwObjDataTypes.kodtOwnNameGuidHot:
								...
								if (guid != book.FootnotesOS[iFootnote].Guid)
								{
									...
									if (iActualFootnote == -1)
									{
										...
										serviceLocator.GetInstance<ILcmUI>().ReportException(new Exception(sMsg), false);
										break;
									}
							...
							case FwObjDataTypes.kodtGuidMoveableObjDisp:
								...
								ICmPicture newPicture = CopyObject<ICmPicture>.CloneLcmObject(picture, null);
		...
		private static void AdjustBtFootnoteInArchivedBook(Guid origGuid, Guid newGuid, IStTxtPara para)
		{
			...
			foreach (CoreWritingSystemDefinition ws in trans.AvailableWritingSystems)
			{
	...
	{
		internal static IList<TObject> GetObjects(LcmCache cache, IList<int> hvos)
		{
	...
	public class SandboxGenericMSA
	{
		...
		private ILcmReferenceCollection<IPartOfSpeech> m_fromPOSes;

		/// <summary>
		/// Gets or sets the dummy MSA from parts of speech.
		/// </summary>
		public ILcmReferenceCollection<IPartOfSpeech> FromPartsOfSpeech
		{
	...
	public static class DomainObjectServices
	{
		...
		{
			var references = (from collection in incomingRefs.OfType<LcmReferenceSequence<ICmObject>>()
...
// Is this necessary:    entryRef.VariantEntryTypesRS != null && entryRef.VariantEntryTypesRS.Count > 0 &&
		...
		public static IEnumerable<IMoInflAffixSlot> GetSomeSlots(LcmCache cache, IEnumerable<IMoInflAffixSlot> allSlots, bool fLookForPrefixes)
		{
		...
		/// <returns></returns>
		public static IEnumerable<IMoInflAffixSlot> GetSlots(LcmCache cache, ILexEntry lex, IPartOfSpeech pos)
		{
		...
		/// <param name="cache"></param>
		/// <returns></returns>
		public static IEnumerable<IMoInflAffixSlot> GetAllSlots(LcmCache cache)
		{
		...
		/// ------------------------------------------------------------------------------------
		public static ICmFolder FindOrCreateFolder(LcmCache cache, int flid, string sFolder)
		{
			ILcmOwningCollection<ICmFolder> prop;
			switch (flid)
			{
