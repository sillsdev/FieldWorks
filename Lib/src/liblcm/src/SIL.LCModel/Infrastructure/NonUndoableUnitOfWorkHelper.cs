...
namespace SIL.LCModel.Infrastructure
{
	...
	public sealed class NonUndoableUnitOfWorkHelper : UnitOfWorkHelper
	{
		...
		public static void DoUsingNewOrCurrentUOW(IActionHandler actionHandler, Action task)
		{
			...
			if (uowService.CurrentProcessingState == UnitOfWorkService.BusinessTransactionState.ProcessingDataChanges)
				task();
			else
		...
		public static void DoUsingNewOrCurrentUowOrSkip(IActionHandler actionHandler, string debugMsg, Action task)
		{
			...
			if (uowService.CurrentProcessingState == UnitOfWorkService.BusinessTransactionState.ProcessingDataChanges)
				task();
			else if (uowService.CurrentProcessingState == UnitOfWorkService.BusinessTransactionState.ReadyForBeginTask)
				Do(actionHandler, task);
			else
				Debug.Fail(debugMsg);
		}
