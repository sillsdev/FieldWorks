...
{
	/// <summary>
	/// IActionHandler implementation for stateful LCM.
	///
	...
	///		This will typically be done using special LCM IUndoAction subclasses that record
	...
	{
		/// <summary>
		/// Keep track of business transaction state (Finite State Machine).
		/// </summary>
		[ComVisible(false)]
		internal enum BusinessTransactionState
		{
			ReadyForBeginTask,
		...
		private BusinessTransactionState m_currentProcessingState;
		internal BusinessTransactionState CurrentProcessingState {
			...
			set
			{
				...
				if (m_currentProcessingState == BusinessTransactionState.ReadyForBeginTask)
		...
		/// <summary>
		/// The LCM lock. This lock is used to synchronize access to the entire LCM. The UI thread is the
		/// only thread that is allowed to start UOWs and thus perform writes to LCM objects.
		/// </summary>
		...
		private void SaveInternal()
		{
			...
			try
			{
				// m_currentProcessingState *must* be BusinessTransactionState.ReadyForBeginTask.
		...
		private void RevertToSavedState()
		{
			...
			for (; ;)
			{
				UnitOfWork mostRecentUnsavedChange = null;
		...
		{
			foreach (UnitOfWork currentBundle in UnsavedUnitsOfWork)
		...
		/// </summary>
		public List<UnitOfWork> UnsavedUnitsOfWork
		{
			get
			{
				var result = new List<UnitOfWork>();
				foreach (var stack in m_undoStacks)
		...
		{
			if (CurrentProcessingState != BusinessTransactionState.ProcessingDataChanges)
		...
		void IUnitOfWorkService.RegisterObjectAsCreated(ICmObject newby)
		{
			...
			RegisterCommon(new LcmStateChangeObjectCreation(newby));
		...
		{
			RegisterCommon(new LcmStateChangeObjectDeletion(goner, xmlStateBeforeDeletion));
			...
			int owningFlidBeforeChange, int owningFlid)
		{
			...
			if (ownerBeforeChange != Guid.Empty && dirtball.Owner != null)
				RegisterCommon(new LcmOwnerChanged(dirtball, ownerBeforeChange, owningFlidBeforeChange,
		...
		{
			RegisterCommon(new LcmVectorPropertyChanged(dirtball, modifiedFlid, originalValue, newValue));
		}
		...
		{
			RegisterCommon(new LcmVectorVirtualChanged(dirtball, modifiedFlid, originalValue, newValue));
		}
		...
		{
			RegisterCommon(new LcmVirtualReferenceCollectionChangedAction<T>(dirtball, modifiedFlid, reader, added, removed));
		}
		...
		void IUnitOfWorkService.RegisterObjectAsModified(ICmObject dirtball, int modifiedFlid, Guid originalValue, Guid newValue)
		{
			RegisterCommon(new LcmGuidPropertyChanged(dirtball, modifiedFlid, originalValue, newValue));
		}
		...
		{
			RegisterCommon(new LcmAtomicRefPropertyChanged(dirtball, modifiedFlid, originalValue, newValue));
		}
		...
		{
			RegisterCommon(new LcmTsStringPropertyChanged(dirtball, modifiedFlid, originalValue, newValue));
		}
		...
		{
			RegisterCommon(new LcmTsStringVirtualChanged(dirtball, modifiedFlid, originalValue, newValue));
		}
		/// <summary>
		...
		{
			RegisterCommon(new LcmMultiTsStringPropertyChanged(dirtball, modifiedFlid, ws, originalValue, newValue));
		}
		...
		{
			RegisterCommon(new LcmMultiTsStringVirtualChanged(dirtball, modifiedFlid, ws, originalValue, newValue));
		}
		...
		{
			RegisterCommon(new LcmTsTextPropsPropertyChanged(dirtball, modifiedFlid, originalValue, newValue));
		}
		...
		{
			RegisterCommon(new LcmStringPropertyChanged(dirtball, modifiedFlid, originalValue, newValue));
		}
		...
		{
			RegisterCommon(new LcmStringVirtualChanged(dirtball, modifiedFlid, originalValue, newValue));
		}
		...
		{
			RegisterCommon(new LcmBooleanPropertyChanged(dirtball, modifiedFlid, originalValue, newValue));
		}
		...
		{
			RegisterCommon(new LcmBooleanVirtualChanged(dirtball, modifiedFlid, originalValue, newValue));
		}
		...
		{
			RegisterCommon(new LcmDateTimePropertyChanged(dirtball, modifiedFlid, originalValue, newValue));
		}
		...
		{
			RegisterCommon(new LcmBinaryPropertyChanged(dirtball, modifiedFlid, originalValue, newValue));
		}
		...
		{
			RegisterCommon(new LcmIntegerPropertyChanged(dirtball, modifiedFlid, originalValue, newValue));
		}
		...
		{
			RegisterCommon(new LcmIntegerVirtualChanged(dirtball, modifiedFlid, originalValue, newValue));
		}
		...
		{
			RegisterCommon(new LcmCustomPropertyChanged(dirtball, modifiedFlid, originalValue, newValue));
		}
		...
		{
			RegisterCommon(new LcmGenDatePropertyChanged(dirtball, modifiedFlid, originalValue, newValue));
		}
		...
		{
			RegisterCommon(new LcmStateChangeCustomFieldDefnModified(dirtball, modifiedFlid));
		}
		...
		public void SetCurrentStack(IActionHandler stack)
		{
			...
			if (CurrentProcessingState == BusinessTransactionState.ReadyForBeginTask)
			{
				m_activeUndoStack = m_currentUndoStack;
			}
		}
		...
		internal bool HasConflictingUndoChanges(UnitOfWork itemToUndo)
		{
			foreach (var stack in m_undoStacks)
		...
		internal bool HasConflictingRedoChanges(UnitOfWork itemToUndo)
		{
			foreach (var stack in m_undoStacks)
