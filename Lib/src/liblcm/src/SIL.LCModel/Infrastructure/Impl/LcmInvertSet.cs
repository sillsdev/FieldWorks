...
{
	/// <summary>
	/// LcmInvertSet wraps another LcmSet and a hashset in such a way that the items in the wrapper are the items in the hashset
	/// that are NOT in the wrapped LcmSet.
	...
	public class LcmInvertSet<T> : ILcmSet<T> where T : class, ICmObject
	{
		// The set we are inverting.
		private ILcmSet<T> m_inverse;
		...
		/// </summary>
		public LcmInvertSet(ILcmSet<T> inverse, IEnumerable<T> universe)
		{
			m_inverse = inverse;
			m_universe = universe;
		}
