...
namespace SIL.LCModel.Infrastructure.Impl
{
	...
	/// a call to the 'BeginUndoTask' method of LCM's ISILDataAccess implementation,
	/// or more directly via LCM's IActionHandler implementation.
	...
	internal abstract class UnitOfWork
	{
		...
		/// For the purposes of this dictionary, two LcmPropertyChanges (considered as keys) are equal if they modify
		...
		/// </summary>
		private readonly Dictionary<LcmPropertyChangedBase, LcmPropertyChangedBase> m_propertyChanges =
			new Dictionary<LcmPropertyChangedBase, LcmPropertyChangedBase>(new ComparePropertyChanges());
		...
		/// </summary>
		private List<LcmStateChangeCustomFieldDefnModified> m_customFieldChanges;
		...
		/// ------------------------------------------------------------------------------------
		internal UnitOfWork(UnitOfWorkService uowService)
		{
		...
		/// </summary>
		internal UnitOfWork InverseObjectChanges
		{
			get
			{
				var result = new NonUndoableUnitOfWork(m_uowService);
		...
		/// </summary>
		internal bool AffectsSameObjects(UnitOfWork other)
		{
		...
		/// </summary>
		internal bool IsAffectedByUndoing(UnitOfWork other)
		{
			...
			//    {
			//        var change = item as LcmStateChangeBase;
		...
		/// </summary>
		internal bool IsAffectedByRedoing(UnitOfWork other)
		{
			if (AffectsSameObjects(other))
				return true;
		...
		class ComparePropertyChanges : IEqualityComparer<LcmPropertyChangedBase>
		{
			public bool Equals(LcmPropertyChangedBase x, LcmPropertyChangedBase y)
			{
				return x.Object == y.Object && x.ModifiedFlid == y.ModifiedFlid && x.WS == y.WS;

			}

			public int GetHashCode(LcmPropertyChangedBase obj)
			{
		...
		internal void RecordCustomFieldDefnChanging(LcmStateChangeCustomFieldDefnModified action)
		{
			if (m_customFieldChanges == null)
				m_customFieldChanges = new List<LcmStateChangeCustomFieldDefnModified>();
		...
		/// ------------------------------------------------------------------------------------
		internal void InsertActionsFrom(UnitOfWork uow)
		{
		...
		/// <summary>
		/// Add some non-LcmStateChangeBase based IUndoAction.
		/// </summary>
		/// <param name="undoAction"></param>
		internal void AddAction(IUndoAction undoAction)
		{
			if (!(undoAction is LcmStateChangeBase))
			{
				AddVerifiedAction(undoAction);
				return;
			}

			((LcmStateChangeBase)undoAction).AddToUnitOfWork(this);
		}
		...
		internal void RegisterObjectAsDeleted(ICmObject goner)
		{
			...
			if (m_newGuids.Contains(goner.Guid))
			{
				...
				{
					if (!(change is LcmStateChangeObjectCreation)) continue;
					var newby = (LcmStateChangeObjectCreation) change;
		...
		internal IEnumerable<ChangeInformation> GetPropChangeInformation(bool fForUndo)
		{
			...
			{
				ILcmPropertyChanged propChange = change as ILcmPropertyChanged;
		...
		{
			var deletedGuids = new HashSet<Guid>(from obj in m_deletedObjects select obj.Guid);
			if (m_changes.OfType<LcmStateChangeBase>().Any(change => change.CreatesProblemReferenceOnUndo(objRepo, deletedGuids)))
				return false;
	...
	/// </summary>
	internal sealed class NonUndoableUnitOfWork : UnitOfWork
	{
		public NonUndoableUnitOfWork(UnitOfWorkService uowService) : base(uowService)
		{
		}
	...
	/// </summary>
	internal sealed class UndoableUnitOfWork : UnitOfWork
	{
		...
		/// ------------------------------------------------------------------------------------
		internal UndoableUnitOfWork(UnitOfWorkService uowService, string undoText, string redoText) : base(uowService)
		{
		...
		internal override void SetAfterXml()
		{
			...
			for (var i = m_changes.Count - 1; i >= 0; i--)
			{
				...
				if (action is LcmStateChangeObjectCreation)
					((LcmStateChangeObjectCreation)action).SetAfterXML();
			}
		}
