...
namespace SIL.LCModel.Infrastructure.Impl
{
	...
	{
		#region Data members
		private readonly Stack<UnitOfWork> m_undoBundles;
		private readonly Stack<UnitOfWork> m_redoBundles;
		...
		internal UnitOfWork m_currentBundle;
		...
		internal IEnumerable<UnitOfWork> RedoableUnitsOfWork
		{
			get { return m_redoBundles; }
		}

		internal IEnumerable<UnitOfWork> UnsavedUnitsOfWork
		{
			get
			{
		...
		public void BeginUndoTask(string bstrUndo, string bstrRedo)
		{
			...
			m_currentBundle = new UndoableUnitOfWork(m_uowService, bstrUndo, bstrRedo);
		}
		...
		{
			if (m_uowService.CurrentProcessingState == UnitOfWorkService.BusinessTransactionState.ProcessingDataChanges)
			{
				Rollback(0);
		...
		{
			if (m_uowService.CurrentProcessingState == UnitOfWorkService.BusinessTransactionState.BroadcastingPropChanges)
			{
		...
		{
			if (m_uowService.CurrentProcessingState == UnitOfWorkService.BusinessTransactionState.ReadyForBeginTask)
			{
		...
		private void EndUndoTaskCommon(bool updateDateModified)
		{
			...
			m_uowService.CurrentProcessingState = UnitOfWorkService.BusinessTransactionState.BroadcastingPropChanges;
			...
			m_uowService.CurrentProcessingState = UnitOfWorkService.BusinessTransactionState.ReadyForBeginTask;
		...
		/// ------------------------------------------------------------------------------------
		private void DoTasksForEndOfPropChanged(UnitOfWork uow, bool fromUndoRedo)
		{
		...
		internal void AddActionInternal(IUndoAction uact)
		{
			...
			// I don't need any of these for normal LCM data changes.
		...
		/// </summary>
		internal bool HasConflictingRedoChanges(UnitOfWork itemToRedo)
		{
		...
		public bool CollapseToMark(int hMark, string bstrUndo, string bstrRedo)
		{
			...
			{
				var newUow = new UndoableUnitOfWork(m_uowService, bstrUndo, bstrRedo);
		...
		internal void AddForeignBundleToUndoStack(NonUndoableUnitOfWork newUow)
		{
			...
			var unsavedBundles = new Stack<UnitOfWork>();
		...
		private void PushUowOnUndoStack(UnitOfWork newUow)
		{
			m_undoBundles.Push(newUow);
		...
		public bool IsUndoTaskActive
		{
			get
			{
				return (m_uowService.CurrentProcessingState == UnitOfWorkService.BusinessTransactionState.ProcessingDataChanges);
			}
		}
		...
		public bool CanStartUow
		{
			get
			{
				return m_uowService.CurrentProcessingState == UnitOfWorkService.BusinessTransactionState.ReadyForBeginTask
