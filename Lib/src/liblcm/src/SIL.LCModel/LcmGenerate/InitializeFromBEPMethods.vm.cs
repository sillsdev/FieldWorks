...
#foreach( $prop in $class.CollectionOwnProperties )
			...
			if (currentProperty != null)
			{
				m_$prop.NiuginianPropName = new LcmOwningCollection<I$prop.Signature>(
					uowService,
					Cache.ServiceLocator.GetInstance<I${prop.Signature}Repository>(),
					this, $prop.Number);
				((ILcmVectorInternal)m_${prop.NiuginianPropName}).LoadFromDataStoreInternal(currentProperty, idFactory);
				currentProperty.Remove();
			}
#end
#foreach( $prop in $class.SequenceOwnProperties )
			...
			if (currentProperty != null)
			{
				m_$prop.NiuginianPropName = new LcmOwningSequence<I$prop.Signature>(
					uowService,
					Cache.ServiceLocator.GetInstance<I${prop.Signature}Repository>(),
					this, $prop.Number);
				((ILcmVectorInternal)m_${prop.NiuginianPropName}).LoadFromDataStoreInternal(currentProperty, idFactory);
...
#foreach( $prop in $class.CollectionRefProperties )
			...
			if (currentProperty != null)
			{
				m_$prop.NiuginianPropName = new LcmReferenceCollection<I$prop.Signature>(
					uowService,
					Cache.ServiceLocator.GetInstance<I${prop.Signature}Repository>(),
					this, $prop.Number);
				((ILcmVectorInternal)m_${prop.NiuginianPropName}).LoadFromDataStoreInternal(currentProperty, idFactory);
...
#foreach( $prop in $class.SequenceRefProperties )
			...
			if (currentProperty != null)
			{
				...
				m_$prop.NiuginianPropName = new LcmReferenceSequence<IAnalysis>(
					...
					this, $prop.Number);
#else
				m_$prop.NiuginianPropName = new LcmReferenceSequence<I$prop.Signature>(
					uowService,
					Cache.ServiceLocator.GetInstance<I${prop.Signature}Repository>(),
					this, $prop.Number);
#end
