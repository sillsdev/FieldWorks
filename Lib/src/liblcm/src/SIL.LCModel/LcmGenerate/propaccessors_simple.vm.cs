...
$propNotes
...
#end
		{
...
#end
			{
				...
				newValue = TsStringUtils.NormalizeNfd(newValue); // Store it as NFD in LCM.
...
#end
		{
...
#end
			{
				...
				if (!string.IsNullOrEmpty(newValue))
					newValue = newValue.Normalize(NormalizationForm.FormD); // Store it as NFD in LCM.
				var originalValue = m_$prop.NiuginianPropName;
				if (newValue == originalValue)
					return;
				m_$prop.NiuginianPropName = newValue;
