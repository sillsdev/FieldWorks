...
$propNotes
		...
		{
#if( $prop.IsHandGenerated)
			var oldObjValue = $prop.NiuginianPropName$generated;
#else
			...
			else
			{
				...
				if (newObjValue.Hvo == (int)SpecialHVOValues.kHvoObjectDeleted)
					throw new LcmObjectDeletedException(String.Format("Using deleted object: {0}.", newObjValue.Guid));
				...
				if (newObjValue.Hvo == (int)SpecialHVOValues.kHvoUninitializedObject)
				{
					...
					throw new LcmObjectUninitializedException(String.Format("Using unowned object in reference property: {0}.", newObjValue.Guid));
#end
				}
#if( $prop.IsOwning)
				else
