...
## --------------------------------------------------------------------------------------------
		...
		/// </summary>
		/// <remarks>
		/// NB: This method should *never, ever* be used directly by LCM clients.
		/// It is used by LCM to handle side effects of setting an atomic owning property,
		/// or removing an object from an owning sequence/collection vector.
		/// The LCM code generator will override this method to do class-specific
...
#foreach( $prop in $class.VectorProperties)
			if (m_$prop.NiuginianPropName != null)
			{
				...
				((ILcmClearForDelete)m_${prop.NiuginianPropName}).Clear(true);
				m_$prop.NiuginianPropName = null;
			}
#end
