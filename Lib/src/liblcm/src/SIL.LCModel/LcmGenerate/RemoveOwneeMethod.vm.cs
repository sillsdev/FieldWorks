...
#foreach( $prop in $class.CollectionOwnProperties )
				case $prop.Number:
					...
					((ILcmOwningCollectionInternal<I$lcmgenerate.GetClass($prop.Signature)>)m_${prop.NiuginianPropName}).RemoveOwnee((I$lcmgenerate.GetClass($prop.Signature))removee);
					((IServiceLocatorInternal)m_cache.ServiceLocator).UnitOfWorkService.RegisterObjectAsModified(
						this,
						$prop.Number,
...
#foreach( $prop in $class.SequenceOwnProperties )
				case $prop.Number:
					var originalValue$prop.Number = m_${prop.NiuginianPropName}.ToGuidArray();
					((ILcmOwningSequenceInternal<I$lcmgenerate.GetClass($prop.Signature)>)m_${prop.NiuginianPropName}).RemoveOwnee((I$lcmgenerate.GetClass($prop.Signature))removee);
					((IServiceLocatorInternal)m_cache.ServiceLocator).UnitOfWorkService.RegisterObjectAsModified(
						this,
						$prop.Number,
						originalValue$prop.Number,
