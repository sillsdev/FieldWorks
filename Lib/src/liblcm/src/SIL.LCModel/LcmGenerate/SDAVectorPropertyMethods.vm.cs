...
#foreach( $prop in $class.CollectionProperties )
				case $prop.Number:
					lock (SyncRoot)
						return m_$prop.NiuginianPropName == null ? LcmCache.kNullHvo : m_${prop.NiuginianPropName}.ToArray()[index].Hvo;
#end
#foreach( $prop in $class.SequenceProperties )
				case $prop.Number:
					lock (SyncRoot)
						return m_$prop.NiuginianPropName == null ? LcmCache.kNullHvo : m_${prop.NiuginianPropName}[index].Hvo;
#end
			}
		}
...
#foreach( $prop in $class.VectorProperties )
				case $prop.Number:
					((ILcmVectorInternal)${prop.NiuginianPropName}).ReplaceAll(newValue, useAccessor);
					break;
#end
			}
		}
