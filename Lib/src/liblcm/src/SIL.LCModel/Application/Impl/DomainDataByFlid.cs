...
namespace SIL.LCModel.Application.Impl
{
	/// <summary>
	/// Implementation of the ISilDataAccess which works with a stateful LCM system,
	...
	internal sealed class DomainDataByFlid : ISilDataAccessManaged
	{
		...
		public long get_Int64Prop(int hvo, int tag)
		{
			...
			// then LCM needs more work, as it generates GenDates as int32 not int64.
		...
		{
			// It's all the same in a stateful LCM world.
		...
		public object get_Prop(int hvo, int tag)
		{
			...
			switch ((CellarPropertyType)m_mdc.GetFieldType(tag))
			{
				...
				case CellarPropertyType.ReferenceAtomic:
					// Will be "LcmCache.kNullHvo" for a null value
		...
		/// clients such as the Views code.
		...
		/// but is done for data integrity purposes in the DB-less LCM system.
		/// </remarks>
		...
		public int MakeNewObject(int clid, int hvoOwner, int tag, int ord)
		{
			...
			ILcmFactoryInternal factory = (ILcmFactoryInternal)owner.Cache.ServiceLocator.GetInstance(
		...
		/// the necessary class information to create, say, an LCM object. You may want to
