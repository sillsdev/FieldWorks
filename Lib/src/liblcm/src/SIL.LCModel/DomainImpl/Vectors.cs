...
// Implementation of:
//
// LcmSet<T> : ICollection<T>
//		LcmOwningCollection<T> : LcmSet<T>
//		LcmReferenceCollection<T> : LcmSet<T>
// LcmList<T> : IList<T>
//		LcmOwningSequence<T> : LcmList<T>
//		LcmReferenceSequence<T> : LcmList<T>
// </remarks>
...
{
	/// <summary>
	/// Base class for the two main types of sets (Collections) in LCM.
	/// </summary>
	/// <typeparam name="T">Some kind of CmObject class.</typeparam>
	internal abstract class LcmSet<T> : ILcmSet<T>, ILcmVectorInternal, ILcmClearForDelete
		where T : class, ICmObject
	{
		...
		protected LcmSet(IUnitOfWorkService uowService, IRepository<T> repository, ICmObject mainObject, int flid)
		{
		...
		/// </exception>
		/// <exception cref="LcmObjectDeletedException">
		...
		protected virtual void BasicValidityCheck(T obj)
		{
			...
			if (obj.Hvo == (int)SpecialHVOValues.kHvoObjectDeleted)
				throw new LcmObjectDeletedException("Object has been deleted.");
		}
		...
		void ILcmVectorInternal.ToXMLString(XmlWriter writer)
		{
			lock (SyncRoot)
			{
		...
		/// </summary>
		void ILcmVectorInternal.LoadFromDataStoreInternal(XElement reader, ICmObjectIdFactory factory)
		{
		...
		/// </summary>
		void ILcmVectorInternal.ReplaceAll(Guid[] newValue, bool useAccessor)
		{
			if (useAccessor)
			{
				Clear();
		...
		public void Clear()
		{
			((ILcmClearForDelete)this).Clear(false);
		}
		/// <summary>
		...
		/// </summary>
		void ILcmClearForDelete.Clear(bool forDeletion)
		{
			var originalValue = ToGuidArray();
		...
		#region Implementation of ILcmVector
	...
	/// </remarks>
	/// <typeparam name="T">Some kind of CmObject class.</typeparam>
	internal class LcmOwningCollection<T> : LcmSet<T>, ILcmOwningCollection<T>, ILcmOwningCollectionInternal<T>
		where T : class, ICmObject
	{
		#region Construction and Initializing
		...
		/// </summary>
		internal LcmOwningCollection(IUnitOfWorkService uowService, IRepository<T> repository, ICmObject mainObject, int flid)
		...
		void ILcmOwningCollectionInternal<T>.RemoveOwnee(T removee)
		{
	...
	/// </summary>
	/// <typeparam name="T">Some kind of CmObject class.</typeparam>
	internal class LcmReferenceCollection<T> : LcmSet<T>, ILcmReferenceCollection<T>, IReferenceSource, IVector
		where T : class, ICmObject
	{
		#region Construction and Initializing
		...
		/// </summary>
		internal LcmReferenceCollection(IUnitOfWorkService uowService, IRepository<T> repository, ICmObject mainObject, int flid)
			: base(uowService, repository, mainObject, flid)
		{ /* Nothing else to do here. */ }
		...
		protected override void BasicValidityCheck(T obj)
		{
			...
			if (obj.Hvo == (int)SpecialHVOValues.kHvoUninitializedObject)
				throw new LcmObjectUninitializedException("Object has not been initialized.");
		...
		/// <returns></returns>
		public bool IsEquivalent(ILcmReferenceCollection<T> other)
		{
		...
		/// ------------------------------------------------------------------------------------
		public void AddTo(ILcmSet<T> dest)
		{
			if (dest == null)
	...
	/// </summary>
	/// <typeparam name="T">Some kind of CmObject class.</typeparam>
	internal abstract class LcmList<T> : ILcmList<T>, ILcmVectorInternal, ILcmClearForDelete
		where T : class, ICmObject
	{
		#region Data Members
		...
		/// Construct an object which allows access to a collection attribute
		/// </summary>
		internal LcmList(IUnitOfWorkService uowService, IRepository<T> repository, ICmObject mainObject, int flid)
		{
			// JohnT: removed some validation from here; this method is called only by generated code, the risk seems
			// very low, and the validation we were doing took significant time.
			m_uowService = uowService;
		...
		private T FluffUpObjectIfNeeded(ICmObjectOrId fluffee)
		{
			...
			// See the unit test LcmMainVectorTests.FluffIngRefSeq_EstablishesBackref.
		...
		/// </summary>
		/// <param name="eventArgs">The <see cref="T:SIL.LCModel.Infrastructure.AddObjectEventArgs"/>
	...
	/// </summary>
	/// <typeparam name="T">Some kind of CmObject class.</typeparam>
	internal class LcmReferenceSequence<T> : LcmList<T>, ILcmList<T>, ILcmReferenceSequence<T>, IReferenceSource, IVector
		where T : class, ICmObject
	{
		#region Construction and Initializing
		...
		/// </summary>
		internal LcmReferenceSequence(IUnitOfWorkService uowService, IRepository<T> repository, ICmObject mainObject, int flid)
			: base(uowService, repository, mainObject, flid)
		{ /* Nothing else to do here. */ }
		...
		/// <exception cref="LcmObjectUninitializedException">Object has not been initialized.
		/// </exception>
		/// ------------------------------------------------------------------------------------
		protected override void BasicValidityCheck(T obj)
		{
		...
		/// ------------------------------------------------------------------------------------
		public void CopyTo(ILcmList<T> dest, int destIndex)
		{
			if (dest == null)
				throw new ArgumentNullException("dest");
	...
	/// <typeparam name="T">Some kind of CmObject class.</typeparam>
	/// ----------------------------------------------------------------------------------------
	internal class LcmOwningSequence<T> : LcmList<T>, ILcmList<T>, ILcmOwningSequence<T>, ILcmOwningSequenceInternal<T>
		where T : class, ICmObject
	{
		#region Construction and Initializing
		...
		/// Construct an object which allows access to a reference sequence property.
		/// </summary>
		internal LcmOwningSequence(IUnitOfWorkService uowService, IRepository<T> repository, ICmObject mainObject, int flid)
			: base(uowService, repository, mainObject, flid)
		{ /* Nothing else to do here. */ }
		...
		/// <param name="removee"></param>
		/// ------------------------------------------------------------------------------------
		void ILcmOwningSequenceInternal<T>.RemoveOwnee(T removee)
		{
		...
		public override void Replace(int start, int numberToDelete, IEnumerable<ICmObject> thingsToAdd)
		{
			...
			foreach (var obj in thingsToAdd)
			{
				...
				// ENHANCE: optimize this, by overriding this method in LcmOwningSequence,
	...
	class ObjectIdEquater : IEqualityComparer<ICmObjectOrId>
	{
		...
		public int GetHashCode(ICmObjectOrId obj)
		{
			...
			// This problem shows up when using R# test runner and running all of the LcmMainVectorTests.
