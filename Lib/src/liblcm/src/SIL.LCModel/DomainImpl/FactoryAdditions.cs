...
#pragma warning restore 219
		...
		/// <returns></returns>
		private static ICmPossibility FindMatchingPossibilityItem(LcmCache cache, string list, ITsString item)
		{
...
#pragma warning disable 168
	...
	internal partial class SegmentFactory
	{
		...
		/// <param name="para">The para.</param>
		/// <param name="beginOffset">The begin offset.</param>
		/// <param name="cache">LcmCache to use for hvo creation</param>
		/// <param name="guid">The guid to initialize the segment with.</param>
		/// ------------------------------------------------------------------------------------
		public ISegment Create(IStTxtPara para, int beginOffset, LcmCache cache, Guid guid)
		{
	...
	internal partial class VirtualOrderingFactory
	{
		public IVirtualOrdering Create(ICmObject parent, string fieldName, IEnumerable<ICmObject> desiredSequence)
		{
			...
			{
				throw new LcmInvalidFieldException("'flid' is not in the metadata cache.");
			}
			var newby = Create();
	...
	internal partial class StStyleFactory
	{
		...
		/// ------------------------------------------------------------------------------------
		public IStStyle Create(ILcmOwningCollection<IStStyle> styleList, string name,
		...
		public IStStyle Create(LcmCache cache, Guid guid)
		{
	...
	internal partial class ScrFootnoteFactory
	{
			...
			int footnoteIndex, string footnoteMarkerStyleName)
		{
			...
			using (var reader = new StringReader(sTextRepOfFootnote))
			{
				...
				// We shouldn't need to do a PropChanged in the new LCM
	...
	internal partial class ScrBookFactory
	{
		...
		/// ------------------------------------------------------------------------------------
		public IScrBook Create(ILcmOwningSequence<IScrBook> booksOS, int bookNumber,
			out IStText title)
		{
	...
	#endregion
	#region CmMediaURI
	internal partial class CmMediaURIFactory : ICmMediaURIFactory, ILcmFactoryInternal
	{
		/// <summary>
		/// Basic creation method for an CmMediaURI.
		/// </summary>
		/// <returns>A new, unowned CmMediaURI with the given guid</returns>
		public ICmMediaURI Create(LcmCache cache, Guid guid)
		{
			int hvo = ((IDataReader)m_cache.ServiceLocator.GetInstance<IDataSetup>()).GetNextRealHvo();
	...
	#endregion
	#region Lcm.Text
	internal partial class TextFactory : ITextFactory, ILcmFactoryInternal
	{
		/// <summary>
		/// Basic creation method for a Text.
		/// </summary>
		/// <returns>A new, unowned Text with the given guid</returns>
		public IText Create(LcmCache cache, Guid guid)
		{
			int hvo = ((IDataReader)m_cache.ServiceLocator.GetInstance<IDataSetup>()).GetNextRealHvo();
