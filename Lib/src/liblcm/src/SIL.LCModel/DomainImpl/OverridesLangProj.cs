...
namespace SIL.LCModel.DomainImpl
{
	...
	internal partial class LangProject
	{
		...
		public string LinkedFilesRootDir
		{
			get
			{
				return String.IsNullOrEmpty(LinkedFilesRootDir_Generated)
					...
					: LinkedFilesRelativePathHelper.GetLinkedFilesFullPathFromRelativePath(Services.GetInstance<ILcmDirectories>().ProjectsDirectory,
					LinkedFilesRootDir_Generated, m_cache.ProjectId.ProjectFolder);
			}
			set
			{
				string relativePath = LinkedFilesRelativePathHelper.GetLinkedFilesRelativePathFromFullPath(Services.GetInstance<ILcmDirectories>().ProjectsDirectory,
					value, m_cache.ProjectId.ProjectFolder, ShortName);
		...
		/// </summary>
		/// ------------------------------------------------------------------------------------
		public ILcmOwningSequence<ICmPossibility> ScriptureAnnotationDfns
		{
			get
			{
