		...
		public override bool CanDelete
		{
			get
			{
				...
				var badRefs = from item in m_incomingRefs.Items
							  where !(item is LcmReferenceCollection<IWfiWordform>)
									|| ((LcmReferenceCollection<IWfiWordform>) item).Flid != WfiWordSetTags.kflidCases
							  select item;
			...
			get
			{
				...
				if (okRefs.FirstOrDefault() != null)
				{
					...
					foreach (var item in okRefs)
					{
						if (!fFirst)
							tisb.Append(", ");
						var wordset = (IWfiWordSet) ((LcmReferenceCollection<IWfiWordform>) item).MainObject;
