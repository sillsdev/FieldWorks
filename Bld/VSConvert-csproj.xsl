				...
				<xsl:element name="{$compiler}">
					...
					<xsl:if test="string-length(ms:PropertyGroup/ms:ApplicationManifest) > 0">
						<xsl:element name="arg">
							...
							<xsl:attribute name="unless">${platform::is-unix()}</xsl:attribute>
						</xsl:element>
					</xsl:if>
					<xsl:if test="string-length(ms:PropertyGroup/ms:ApplicationManifest) &lt;= 0">
						<arg value="/nowin32manifest" unless="${{platform::is-unix()}}"/>
					</xsl:if>
...
-->
	<xsl:template name="configuration">
		...
		<target>
			...
			<!-- Handle DebugType, converted to lowercase. The only possible choices
				for DebugType are None, Enable, Full, PdbOnly -->
			<xsl:variable name="debug" select='translate(ms:DebugType, "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "abcdefghijklmnopqrstuvwxyz")'/>
			<xsl:choose>
				<xsl:when test="$debug = 'none'">
					<property name="debug" value="None"/>
				</xsl:when>
				<xsl:when test="$debug = 'enable'">
					<property name="debug" value="Enable" unless="${{platform::is-unix()}}"/>
					<property name="debug" value="Full" if="${{platform::is-unix()}}"/>
				</xsl:when>
				<xsl:when test="$debug = 'pdbonly'">
					<property name="debug" value="PdbOnly" unless="${{platform::is-unix()}}"/>
					<property name="debug" value="Full" if="${{platform::is-unix()}}"/>
				</xsl:when>
				<xsl:when test="$debug = 'full'">
