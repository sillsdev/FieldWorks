...
Tools:
    ...
    - jira_add_attachment: Upload a file to an issue
"""
...
def _comment_payload(comment: str, api_version: str) -> Dict[str, Any]:
    """Build the comment payload Jira expects for the selected API version."""
    if api_version == "3":
        return {
            'body': {
                'type': 'doc',
...
def jira_add_comment(
    ...
    try:
        ...
        payload = _comment_payload(comment, client.api_version)
...
def jira_update_comment(
    ...
    try:
        ...
        payload = _comment_payload(comment, client.api_version)
        response = client.put(client.api_path(f'issue/{issue_key}/comment/{comment_id}'), json=payload)
        return format_json_response(_simplify_comment(response))
...
def jira_add_attachment(
    ...
    """Upload one file attachment to a Jira issue."""
    try:
        client = get_jira_client(credentials)
        ...
        with path.open('rb') as file_handle:
            response = client.post_files(
                client.api_path(f'issue/{issue_key}/attachments'),
                files={'file': (path.name, file_handle)}
            )

        uploaded = response if isinstance(response, list) else [response]
        attachments = [_simplify_attachment(item) for item in uploaded]
        return format_json_response({
            'success': True,
            'issue_key': issue_key,
            'attachments': attachments,
