	...
	<xsl:template match="/">
		<PartsOfSpeech6001>
			<CmPossibilityList>
				...
				<Possibilities8>
					<xsl:for-each select="/Categories/Category[descendant-or-self::Category[@use4newlangprojs='yes']]">
...
-->
	<xsl:template name="DoPOSContent">
		<PartOfSpeech>
			...
			<xsl:for-each select="descendant::Category[@use4newLangProjects='yes']">
				<SubPossibilities7>
					<xsl:call-template name="DoPOSContent"/>
				</SubPossibilities7>
			</xsl:for-each>
