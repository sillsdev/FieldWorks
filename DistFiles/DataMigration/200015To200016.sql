-- update database from version 200015 to 200016
BEGIN TRANSACTION  --( will be rolled back if wrong version#
----------------------------------------------------------------------

--( The following are here due to FKs not being created
--( for atomic references not referencing CmObject. For example,
--( CmAnnotation.AnnotationType and CmAnnotation.InstanceOf are
--( both atomic references. AnnotationType has a FK to
--( CmAnnotationDefn, but InstanceOf does not have a FK to CmObject
...
---------------------------------------------------------------------
--( Clean data and create missing FKs per table. Some code is taken
--( from TR_Field$_UpdateModel_Ins
...
WHILE @nRowCount != 0 BEGIN
	...
	SET @sql = 'UPDATE [' + @sClass + '] WITH (SERIALIZABLE) SET [' +
		@sName + '] = NULL' + CHAR(13) +
		'FROM [' + @sClass + ']' + CHAR(13) +
		'LEFT OUTER JOIN [' + @sTargetClass + '] ON [' +
		@sTargetClass + '].[Id] = [' + @sClass + '].[' + @sName + ']' + CHAR(13) +
		'WHERE [' + @sClass + '].[' + @sName + '] IS NOT NULL AND ['
		+ @sTargetClass + '].[Id] IS NULL'
	...
	SELECT TOP 1
		...
		@DstCls = DstCls
	FROM Field$ f
	WHERE Id > @sFlid AND Type = 24 AND DstCls = 0
	ORDER BY Id
...
-------------------------------------------------------------------
--( Recreating TR_Field$_UpdateModel_Ins because of the wrong
--( variable name used in one line:
--( OLD: if @type = 24 and @sTargetClass != 'CmObject' begin
--( NEW: if @type = 24 and @sClass != 'CmObject' begin
...
if @dbVersion = 200015
begin
	update Version$ set DbVer = 200016
	COMMIT TRANSACTION
	print 'database updated to version 200016'
end
else
begin
	ROLLBACK TRANSACTION
	print 'Update aborted: this works only if DbVer = 200015 (DbVer = ' +
