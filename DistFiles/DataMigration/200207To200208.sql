-- update database FROM version 200207 to 200208
BEGIN TRANSACTION  --( will be rolled back if wrong version#)

-------------------------------------------------------------------------------
-- Fix part of LT-7240: "Find Entry" dialog won't find any entries which do not
-- have a lexeme form in the default vernacular writing system.
-------------------------------------------------------------------------------
...
IF @dbVersion = 200207
BEGIN
	UPDATE [Version$] SET [DbVer] = 200208
	COMMIT TRANSACTION
	PRINT 'database updated to version 200208'
END

ELSE
BEGIN
	ROLLBACK TRANSACTION
	PRINT 'Update aborted: this works only if DbVer = 200207 (DbVer = ' +
