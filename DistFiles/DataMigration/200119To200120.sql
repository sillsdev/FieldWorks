-- update database FROM version 200119 to 200120
...
CREATE    proc [MatchingEntries]
	...
	@wsa int
AS
	declare @AFTxt nvarchar(4000), @aftext nvarchar(4000),
	...
	declare @rowcount int, @wsFoundling int
	--( Just leave the *** for missing CF, as per LT-2545.
	--( Leave code in, for when it gets restored, however.
	--( The code will need to follow the patterns below, when it gets reinstated.
	-- if @cftext = '***'
	--	update @MatchingEntries
	--	set CFWs=@UIWs
	...
	SELECT @rowcount = count(*) FROM LexEntry_AlternateForms (readuncommitted)
	IF @rowcount > 0
	BEGIN
		INSERT INTO @wses (Ws) SELECT @wsv
		INSERT INTO @wses (Ws)
		SELECT Dst
		...
		SELECT TOP 1 @dummyId = DummyID, @entryID = EntryID, @aftext = AFTxt
		FROM @MatchingEntries
		WHERE AFTxt = '***'
		ORDER BY DummyID
		WHILE @@ROWCOUNT > 0
		BEGIN
			SELECT TOP 1 @AFTxt = af.Txt, @wsFoundling = af.Ws
			FROM LexEntry_AlternateForms leaf (readuncommitted)
			JOIN MoForm_Form af (readuncommitted) on af.Obj = leaf.Dst
			...
			SELECT TOP 1 @dummyId = DummyID, @entryID = EntryID, @aftext = AFTxt
			FROM @MatchingEntries
			WHERE AFTxt = '***' AND DummyID > @dummyId
			ORDER BY DummyID
		END
		DELETE @wses
	END
	...
	SELECT @rowcount = count(*) FROM LexSense_Gloss (readuncommitted)
	IF @rowcount > 0
	BEGIN
		INSERT INTO @wses (Ws) SELECT @wsa
		INSERT INTO @wses (Ws)
		SELECT Dst
		...
		SELECT TOP 1 @dummyId = DummyID, @entryID = EntryID, @gltext = GLTxt
		FROM @MatchingEntries
		WHERE GLTxt = '***'
		ORDER BY DummyID
		WHILE @@ROWCOUNT > 0
		BEGIN
			...
			SELECT TOP 1 @dummyId = DummyID, @entryID = EntryID, @gltext = GLTxt
			FROM @MatchingEntries
			WHERE GLTxt = '***' AND DummyID > @dummyId
			ORDER BY DummyID
		END
	END
/* This is all done by the client now.
	--( We have no way to get at the User UI writing system in the application (without it being given as a parameter, of course),
	--( so we have to just use English. This is used to set the WS for various strings, if they end up being '***',
	...
	--( Set the ws for the citation forms, if they are '***'.
	update @MatchingEntries
	set CFWs = @UIWs
	where CFTxt = '***'
	--( Set the ws for the lexeme forms, if they are '***'.
	update @MatchingEntries
	set LFWs = @UIWs
	where LFTxt = '***'
	--( Set the ws to any remaining brain-dead alternate forms, if they are '***'.
	UPDATE @MatchingEntries
	SET AFWs = @UIWs
	WHERE AFTxt = '***'
	--( Set the ws to any remaining brain-dead glosses, if they are '***'.
	UPDATE @MatchingEntries
	SET GLWs = @UIWs
	WHERE GLTxt = '***'
*/
...
if @dbVersion = 200119
begin
	UPDATE Version$ SET DbVer = 200120
	COMMIT TRANSACTION
	print 'database updated to version 200120'
end
else
begin
	ROLLBACK TRANSACTION
	print 'Update aborted: this works only if DbVer = 200119 (DbVer = ' +
