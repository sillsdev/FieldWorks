-- Update database from version 200089 to 200090
BEGIN TRANSACTION  --( will be rolled back if wrong version#)

-- This fixes the missing data part of LT-2913.
...
AS
	...
	SET @cur = CURSOR FAST_FORWARD FOR
		SELECT EntryId, AFTxt, GLTxt
		FROM @MatchingEntries
	DECLARE @hvo int, @hvoPrev int
	...
	OPEN @cur
	FETCH NEXT FROM @cur INTO @hvo, @alloTxt, @glossTxt
	...
	BEGIN
		IF @hvo = @hvoPrev BEGIN
			...
			SET AFTxt=@alloNew, GLTxt=@glossNew
				WHERE EntryId=@hvoPrev AND AFTxt=@alloPrev AND GLTxt=@glossPrev
			DELETE FROM @MatchingEntries
				WHERE EntryId=@hvo AND AFTxt=@alloTxt AND GLTxt=@glossTxt
...
if @dbVersion = 200089
begin
	update Version$ set DbVer = 200090
	COMMIT TRANSACTION
	print 'database updated to version 200090'
end
else
begin
	ROLLBACK TRANSACTION
	print 'Update aborted: this works only if DbVer = 200089 (DbVer = ' +
