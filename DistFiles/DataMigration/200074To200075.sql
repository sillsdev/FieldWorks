-- Update database from version 200074 to 200075
...
declare @retval int, @fIsNocountOn int,
	...
	begin
		...
		IF @@ROWCOUNT = 0 BEGIN
			-- No citation form, so set some defaults or the following concatenation won't work.
			SET @CfTxt='***'
			SET @CfFmt=null
			SET @CfWs=@AlloWs
		END
...
if @dbVersion = 200074
begin
	update Version$ set DbVer = 200075
	COMMIT TRANSACTION
...
begin
	...
	print 'Update aborted: this works only if DbVer = 200074 (DbVer = ' +
