-- Update database FROM version 200171 to 200172

BEGIN TRANSACTION  --( will be rolled back if wrong version#)

-----------------------------------------------------------------------
-- Fix glosses to match only on senses owned by an entry.
-----------------------------------------------------------------------
...
IF @dbVersion = 200171
BEGIN
	UPDATE [Version$] SET [DbVer] = 200172
	COMMIT TRANSACTION
	PRINT 'database updated to version 200172'
END

ELSE
BEGIN
	ROLLBACK TRANSACTION
	PRINT 'Update aborted: this works only if DbVer = 200171 (DbVer = ' +
