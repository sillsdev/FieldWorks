-- Update database from version 200050 to 200051
BEGIN TRANSACTION  --( will be rolled back if wrong version#)

---------------------------------------------------------------------
-- Fixed earlier problems with 200049To200050 migration for those who got it before it was fixed.
---------------------------------------------------------------------

-- Split PartOfSpeechOrSlot into PartOfSpeech and Slot
declare @sql VARCHAR(1000)
set @sql = 'update MoInflectionalAffixMsa
...
if exists (select * from Field$ where name = 'PartOfSpeechOrSlot') begin
	exec @sql
end
...
---------------------------------------------------------------------
if exists (select * from sysobjects where name = '_FK_MoInflectionalAffixMsa_PartOfSpeechOrSlot') begin
...
if @dbVersion = 200050
begin
	update Version$ set DbVer = 200051
	COMMIT TRANSACTION
	print 'database updated to version 200051'
end
else
begin
	ROLLBACK TRANSACTION
	print 'Update aborted: this works only if DbVer = 200050 (DbVer = ' +
