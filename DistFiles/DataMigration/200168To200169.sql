-- update database FROM version 200168 to 200169
BEGIN TRANSACTION  --( will be rolled back if wrong version#)
-----------------------------------------------------------------------
-- Add ReportServer databases to excluded list of FW database names
-----------------------------------------------------------------------
...
IF @dbVersion = 200168
BEGIN
	UPDATE [Version$] SET [DbVer] = 200169
	COMMIT TRANSACTION
	PRINT 'database updated to version 200169'
END

ELSE
BEGIN
	ROLLBACK TRANSACTION
	PRINT 'Update aborted: this works only if DbVer = 200168 (DbVer = ' +
...
--  deallocate cur_DBs
	...
	select [name]
	from master..sysdatabases
	where upper([name]) not in ('MASTER', 'MODEL', 'TEMPDB', 'MSDB', 'NORTHWIND', 'PUBS',
		'ETHNOLOGUE', 'REPORTSERVER$SILFW', 'REPORTSERVER$SILFWTEMPDB')
