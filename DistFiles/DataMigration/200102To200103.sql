-- update database from version 200102 to 200103
...
go
print 'creating function fnIsPrime$'
go
 ...
 *
 * Description:
 *		Checks to see if the input integer is a prime number.
 *		NB: "1" is not a prime number in this definition.
 *
 * Parameters:
 * 		@input			Integer to check to see if it is a prime number.
 *
 * Returns:
 *		1 if it is a prime number, otherwise 0.
 *****************************************************************************/
CREATE  FUNCTION fnIsPrime$ (@input INT)
RETURNS BIT
AS
BEGIN
	DECLARE @isPrime BIT, @root INT, @divisor INT

	SET @isPrime = 1 -- Start optimistic.
...
AS
	...
	DECLARE @isPrimeSenseComboCount BIT
	SET @curMBId = null
	EXEC @isPrimeSenseComboCount = fnIsPrime$ @senseComboCount
	...
	WHILE @rowcount > 0
	BEGIN --( Start @rowcount > 0
		IF @currentSenseCount = 1 OR @isPrimeSenseComboCount = 1
		BEGIN --( IF @currentSenseCount = 1 OR @isPrimeSenseComboCount = 1
		...
		END --( IF @currentSenseCount = 1 OR @isPrimeSenseComboCount = 1
		--( Get next pair to work on.
		SELECT TOP 1 @CurMsaId = [MsaId], @CurOrd = [Ord], @currentSenseCount = [SenseCount]
		FROM @Pair
		WHERE [Ord] > @CurOrd
...
if @dbVersion = 200102
begin
	update Version$ set DbVer = 200103
	COMMIT TRANSACTION
	print 'database updated to version 200103'
end
else
begin
	ROLLBACK TRANSACTION
	print 'Update aborted: this works only if DbVer = 200102 (DbVer = ' +
