-- Update database from version 200062 to 200063
BEGIN TRANSACTION  --( will be rolled back if wrong version#)

-- Change LexReferenceType name from 'Confer' to 'Compare'.
UPDATE CmPossibility_Name SET Txt='Compare'
	FROM LexReferenceType t
	JOIN CmPossibility_Name n on n.Obj=t.Id
	WHERE n.Txt='Confer' AND t.MappingType=5
...
BEGIN
	...
	SELECT @nvcAllo=f.Txt, @nHomograph=le.HomographNumber, @nvcPostfix=t.Postfix,
		...
		FROM LexEntry le
		LEFT OUTER JOIN LexEntry_Allomorphs a on a.Src=le.id AND
					a.Ord=(SELECT MAX(Ord) FROM LexEntry_Allomorphs WHERE Src=le.id)
...
if @dbVersion = 200062
begin
	update Version$ set DbVer = 200063
	COMMIT TRANSACTION
	print 'database updated to version 200063'
end
else
begin
	ROLLBACK TRANSACTION
	print 'Update aborted: this works only if DbVer = 200062 (DbVer = ' +
