-- update database FROM version 200148 to 200149
...
-- SetAgentEval appears not to exist anymore, so it's deleted. (This was
-- originally in the last migration script, but GetLinkedObjects$ had constants
-- in it. GetLinkedObjects$ was deleted from there to fix it.)
-------------------------------------------------------------------------------
...
IF @dbVersion = 200148
BEGIN
	UPDATE [Version$] SET [DbVer] = 200149
	COMMIT TRANSACTION
	PRINT 'database updated to version 200148'
END

ELSE
BEGIN
	ROLLBACK TRANSACTION
	PRINT 'Update aborted: this works only if DbVer = 200148 (DbVer = ' +
