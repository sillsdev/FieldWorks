	...
	public string CreateGUID()
	{
	  ...
	  <sfm:template match="Sense[not(ps)]">
		...
		<sfm:element name="Sense">
		  ...
		  <!--
			The logic is a little unique depending on if we are a Sence in an Entry or Subentry
			The common part is looking for a preceding Sense element with a ps element that is at the same level as this node.
			If the parent is a Subentry, than we will look in Subentry's and it's parent Entry element for matches.
		  -->
		  <sfm:choose>
			...
			<sfm:when test="parent::Subentry">
			  <sfm:choose>
				<sfm:when test="preceding-sibling::Sense/ps">
				  <sfm:variable name="withPS" select="preceding-sibling::Sense/ps"/>
				  <sfm:comment>** copied 'ps' **</sfm:comment>
				  <sfm:element name="seires"><sfm:attribute name="ws">en</sfm:attribute>Review the Grammatical Info of senses.</sfm:element>
				  <!--sfm:copy-of select="$withPS[count($withPS)]"/-->
				  <sfm:apply-templates select="$withPS[count($withPS)]"/>
				</sfm:when>
				<sfm:otherwise>
