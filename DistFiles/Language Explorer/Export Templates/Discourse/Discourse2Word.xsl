...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
		...
		xmlns:w="http://schemas.microsoft.com/office/word/2003/wordml" xmlns:v="urn:schemas-microsoft-com:vml">
			<w:styles>
				...
				<w:style w:type="paragraph" w:styleId="Discourse Normal">
					...
					<w:rPr>
						<w:noProof w:val="on"/> 	<!--Don't spell-check vernacular -->
						<w:sz  w:val="21"/>
					</w:rPr>
				</w:style>
				<!--word glosses -->
				<w:style w:type="paragraph" w:styleId="Discourse Gloss">
					...
					<w:rPr>
						...
						<w:noProof w:val="off"/>
						<w:rFonts>
							<xsl:attribute name="w:ascii"><xsl:value-of select="//languages/language[not(@vernacular='true')]/@font"/></xsl:attribute>
							<xsl:attribute name="w:h-ansi"><xsl:value-of select="//languages/language[not(@vernacular='true')]/@font"/></xsl:attribute>
							<xsl:attribute name="w:cs"><xsl:value-of select="//languages/language[not(@vernacular='true')]/@font"/></xsl:attribute>
							<xsl:attribute name="w:fareast"><xsl:value-of select="//languages/language[not(@vernacular='true')]/@font"/></xsl:attribute>
						</w:rFonts>
					</w:rPr>
				</w:style>
  ...
  <xsl:template match="chart">
	  <w:tbl>
		  <w:tblPr>
			  <w:tblCellMar> <!-- cell margins, twips-->
	...
	<xsl:template match="cell">
		<w:tc>
			<w:tcPr>
				<w:gridSpan>
					<xsl:attribute name="w:val" ><xsl:value-of select="@cols"/></xsl:attribute>
				</w:gridSpan>
			...
			<w:tblGrid>
				<w:gridCol w:w="200"/> <!-- minimum cell width in twips (1/20 pt)-->
			</w:tblGrid>
			<xsl:apply-templates/>
		</w:tc>
	</xsl:template>
				...
				<xsl:if test="ancestor::row/attribute::endPara='true' or ancestor::row/attribute::endSent='true'">
					<w:tcBorders>
						...
						<xsl:if test="ancestor::row/attribute::endPara='true'">
							<w:bottom w:val="single" w:sz="16" w:space="0" w:color="0"/>
						</xsl:if>
	...
	<xsl:template match="main" mode="glosses">
	</xsl:template>
