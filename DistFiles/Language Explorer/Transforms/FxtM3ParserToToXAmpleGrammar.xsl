			...
			<xsl:if test="count(PrefixSlots | SuffixSlots)>0">
			   <xsl:variable name="bAllOptional">
				  <xsl:choose><!-- use id() if it works, otherwise use another approach. -->
					 <xsl:when test="id(@slots)">
						<xsl:if test="id(@slots)/@optional='false'">false</xsl:if>
						<xsl:if test="not(id(@slots)/@optional='false')">true</xsl:if>
					 </xsl:when>
					 <xsl:otherwise>
						<xsl:variable name="idref"><xsl:value-of select="@slots"/></xsl:variable>
						<xsl:if test="//*[@id=$idref]/@optional='false'">false</xsl:if>
						<xsl:if test="not(//*[@id=$idref]/@optional='false')">true</xsl:if>
					 </xsl:otherwise>
				  </xsl:choose>
			   </xsl:variable>
...
<xsl:variable name="first-nodes">
   <xsl:choose><!-- use id() if it works, otherwise use another approach. -->
	  <xsl:when test="id($sFirst)"><xsl:value-of select="id($sFirst)"/></xsl:when>
	  <xsl:otherwise><xsl:value-of select="//*[@id=$sFirst]"/></xsl:otherwise>
   </xsl:choose>
</xsl:variable>
	  <xsl:for-each select="$first-nodes">
