...
rule {Fully analyzed stem with no inflectional template}
  Full = Stem
	  ...
	  <xsl:for-each select="$POSs/PartOfSpeech">
		 ...
		 <xsl:for-each select="AffixTemplates/MoInflAffixTemplate[@Final='1']">
...
rule {Fully analyzed stem with a final inflectional template <xsl:value-of select="@Id"/>}
  Full = <xsl:variable name="sAllSlotsOptional">
				  <xsl:for-each select="PrefixSlots | SuffixSlots">
					 <xsl:if test="key('SlotsID',@dst)/@Optional='0'">N</xsl:if>
				  </xsl:for-each>
			   </xsl:variable>
...
-->
		 ...
		 <xsl:for-each select="AffixTemplates/MoInflAffixTemplate[@Final='0']">
			<xsl:if test="count(PrefixSlots | SuffixSlots)>0">
   ...
   <xsl:template name="AffixTemplateSlotsConstraints">
	  ...
	  <xsl:for-each select="$Slots">
		 ...
		 <xsl:variable name="sItem">
		  ...
		  so are appending an underscore, and the @ord value to create a unique value.
  -->
			<xsl:text>_</xsl:text>
...
-->
 ...
 -->
			   ...
			   <xsl:if test="not(contains($sAllSlotsOptional,'N'))">0</xsl:if>
			</xsl:if>
		 </xsl:variable>
		 <xsl:if test="$bOptional='1'">(</xsl:if>
		 <xsl:value-of select="@dst"/>
		 <!-- Need to distinguish between any identical terminal symbols in PATR rule,
		  so are appending an underscore, and the @ord value to create a unique value.
  -->
