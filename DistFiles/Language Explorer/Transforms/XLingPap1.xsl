...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
   ...
   <xsl:template match="sectionRef">
	  <xsl:element name="a">
		 ...
		 <xsl:choose><!-- use id() if it works, otherwise use another approach. -->
		   <xsl:when test="id(@sec)">
			 <xsl:apply-templates select="id(@sec)" mode="number"/>
		   </xsl:when>
		   <xsl:otherwise>
			 <xsl:variable name="idref"><xsl:value-of select="@sec"/></xsl:variable>
			 <xsl:apply-templates select="//*[@id=$idref]" mode="number"/>
		   </xsl:otherwise>
		 </xsl:choose>
	  </xsl:element>
   </xsl:template>
   ...
   <xsl:template match="appendixRef">
	  <xsl:element name="a">
		 ...
		 <xsl:choose><!-- use id() if it works, otherwise use another approach. -->
		   <xsl:when test="id(@app)">
			 <xsl:apply-templates select="id(@app)" mode="numberAppendix"/>
		   </xsl:when>
		   <xsl:otherwise>
			 <xsl:variable name="idref"><xsl:value-of select="@app"/></xsl:variable>
			 <xsl:apply-templates select="//*[@id=$idref]" mode="numberAppendix"/>
		   </xsl:otherwise>
		 </xsl:choose>
	  </xsl:element>
   </xsl:template>
   ...
   <xsl:template match="line">
	  ...
	  <tr>
		 <xsl:choose>
			...
			<xsl:otherwise>
			   <xsl:choose><!-- use id() if it works, otherwise use another approach. -->
				  <xsl:when test="id(parent::lineGroup/line[1]/langData[1]/@lang)">
				  ...
				  <xsl:otherwise><!-- id() doesn't work -->
					 <xsl:variable name="idref1"><xsl:value-of select="parent::lineGroup/line[1]/langData[1]/@lang"/></xsl:variable>
					 <xsl:if test="//*[@id=$idref1]/@rtl='yes'">
						<xsl:attribute name="align">right</xsl:attribute>
					 </xsl:if>
					 <xsl:variable name="language">
						<xsl:if test="langData">
					 ...
					 <xsl:variable name="idref2"><xsl:value-of select="langData/@lang"/></xsl:variable>
					 <xsl:variable name="sOrientedContents">
						<xsl:choose>
						   <xsl:when test="langData and //*[@id=$idref2]/@rtl='yes'">
							  <!-- flip order, left to right -->
							  <xsl:call-template name="ReverseContents">
								 <xsl:with-param name="sList" select="$sContents"/>
							  </xsl:call-template>
   ...
   <xsl:template match="exampleRef">
	  <xsl:variable name="letter-not-example">
		 <xsl:if test="@letter">
			<xsl:choose><!-- use id() if it works, otherwise use another approach. -->
			   <xsl:when test="id(@letter)"><xsl:if test="name(id(@letter))!='example'">true</xsl:if></xsl:when>
			   <xsl:otherwise>
				  <xsl:variable name="idref"><xsl:value-of select="@letter"/></xsl:variable>
				  <xsl:if test="name(//*[@id=$idref])!='example'">true</xsl:if>
			   </xsl:otherwise>
			</xsl:choose>
		 </xsl:if>
	  </xsl:variable>
			...
			<xsl:element name="a">
			   ...
			   <xsl:choose><!-- use id() if it works, otherwise use another approach. -->
				  ...
				  <xsl:otherwise>
					 <xsl:variable name="idref"><xsl:value-of select="@letter"/></xsl:variable>
					 <xsl:if test="not(@letterOnly='yes')">
						<xsl:apply-templates select="//*[@id=$idref]" mode="example"/>
					 </xsl:if>
					 <xsl:apply-templates select="//*[@id=$idref]" mode="letter"/>
				  </xsl:otherwise>
			   </xsl:choose>
			   <xsl:if test="@punct">
		 ...
		 <xsl:otherwise>
			<xsl:if test="@num">
			   <xsl:element name="a">
				  ...
				  <xsl:if test="@equal='yes'">=</xsl:if>
				  <xsl:choose><!-- use id() if it works, otherwise use another approach. -->
					 <xsl:when test="id(@num)">
						<xsl:apply-templates select="id(@num)" mode="example"/>
					 </xsl:when>
					 <xsl:otherwise>
						<xsl:variable name="idref"><xsl:value-of select="@num"/></xsl:variable>
						<xsl:apply-templates select="//*[@id=$idref]" mode="example"/>
					 </xsl:otherwise>
				  </xsl:choose>
				  <xsl:if test="@punct">
   ...
   <xsl:template match="endnoteRef">
	  <xsl:call-template name="OutputEndnoteNumber">
		 ...
		 <xsl:with-param name="node">
		   <xsl:choose><!-- use id() if it works, otherwise use another approach. -->
			 <xsl:when test="id(@note)">
			   <xsl:value-of select="id(@note)"/>
			 </xsl:when>
			 <xsl:otherwise>
			   <xsl:variable name="idref"><xsl:value-of select="@note"/></xsl:variable>
