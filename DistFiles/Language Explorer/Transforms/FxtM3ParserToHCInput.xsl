...
<!-- just use the order they are in; it is correct					<xsl:sort select="@ord" data-type="number" order="ascending"/>-->
				</xsl:apply-templates>
				<xsl:apply-templates select="PrefixSlots">
<!-- just use the order they are in; it is correct					<xsl:sort select="@ord" data-type="number" order="descending"/>-->
				</xsl:apply-templates>
			</AffixTemplate>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template match="MoInflClass | ProdRestrict/CmPossibility">
