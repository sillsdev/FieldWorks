...
- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	...
	<xsl:variable name="bVernRightToLeft" select="//VernWss/LgWritingSystem/@RightToLeft"/>
	<xsl:variable name="bGlossRightToLeft" select="//AnalysisWss/LgWritingSystem/@RightToLeft"/>
	...
	<xsl:variable name="FsFeatStrucTypes" select="//FsFeatStrucType"/>
	...
	<xsl:variable name="UnderspecifiedInflectionalAffixes" select="$LexEntries[key('MsaID',MorphoSyntaxAnalysis/@dst)[name()='MoInflAffMsa' and not(PartOfSpeechRef) or name()='MoInflAffMsa' and not(Slots)]]"/>
...
-->
	<xsl:template match="/M3Dump">
		...
		<!-- output dtd path -->
		<xsl:text disable-output-escaping="yes">&#xa;&#x3c;!DOCTYPE lingPaper SYSTEM "</xsl:text>
		<xsl:value-of select="$sWordWorksTransformPath"/>
		...
		<lingPaper version="1.12.0">
			...
			<section1 id="sIntro" type="tH1">
				...
				<p>
					...
					<xsl:choose>
						<xsl:when test="string-length(LangProject/MainCountry) > 0">
			...
			<section1 id="sInflection" type="tH1">
				...
				<xsl:for-each select="$PartsOfSpeech/PartOfSpeech[descendant-or-self::MoInflAffixTemplate]">
					<xsl:sort select="Name"/>
					<!--					<xsl:if test="count($POSes/SubPossibilities[@dst=$idPOS])=0"> -->
					<section2 type="tH2">
						...
						<section2 id="sResidueUnderspecifiedInflectionalAffixes" type="tH2">
							...
							<table type="tLeftOffset" border="1">
								...
								<xsl:for-each select="$UnderspecifiedInflectionalAffixes">
									...
									<xsl:sort select="LexemeForm"/>
									<xsl:variable name="lexEntry" select="."/>
									<xsl:variable name="problemMsas" select="key('MsaID',MorphoSyntaxAnalysis/@dst)[name()='MoInflAffMsa' and not(PartOfSpeechRef) or name()='MoInflAffMsa' and not(Slots)]"/>
									<xsl:for-each select="$problemMsas">
										<tr>
											<xsl:if test="position()=1">
												<xsl:call-template name="GetCitationForm">
											...
											<td>
												<xsl:call-template name="OutputPOSAsNameWithRef">
													<xsl:with-param name="sPOSId" select="PartOfSpeechRef/@dst"/>
												</xsl:call-template>
											</td>
											<td>
					...
					<xsl:if test="$MoMorphAdhocProhibs | $MoAlloAdhocProhibs | $MoAdhocProhibGrs">
						<section2 id="sResidueAdHocs" type="tH2">
							...
							<ul>
								...
								<xsl:if test="$MoMorphAdhocProhibs[not(key('AdhocGroup',@Id))]">
									<li>
								...
								<xsl:if test="$MoAlloAdhocProhibs[not(key('AdhocGroup',@Id))]">
									<li>
			...
			<languages>
				<language id="lVern" name="vernacular">
					...
					<xsl:attribute name="font-family">
						<xsl:value-of select="//VernWss/LgWritingSystem/DefaultSerif"/>
					</xsl:attribute>
				...
				<language id="lGloss" name="gloss">
					...
					<xsl:attribute name="font-family">
						<xsl:value-of select="//AnalysisWss/LgWritingSystem/DefaultSerif"/>
					</xsl:attribute>
				</language>
	...
	<xsl:template name="OutputPhonemeTable">
		...
		<xsl:variable name="bShowBasicIPASymbol">
			<xsl:choose>
				<xsl:when test="$PhPhonemes[string-length(normalize-space(BasicIPASymbol)) &gt; 0]">Y</xsl:when>
				<xsl:otherwise>N</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
					...
					<tr>
						...
						<xsl:if test="$bShowBasicIPASymbol='Y'">
							<td>
								<langData lang="lIPA">
									<xsl:choose>
										<xsl:when test="string-length(normalize-space($phoneme/BasicIPASymbol)) &gt; 0">
									...
									<xsl:choose>
										<xsl:when test="name()='PlusConstr'">
											<xsl:call-template name="OutputPhonologicalFeatureConstraint">
												<xsl:with-param name="feature" select="key('FsClosedFeaturesID', $constraint/@Feature)"/>
							...
							<table type="tLeftOffset" border="1">
								...
								<xsl:for-each select="Features/Feature">
									...
									<tr>
										...
										<td>
											<xsl:choose>
												<xsl:when test="string-length($feature/Description) = 0">
													<xsl:text>There is no description yet.</xsl:text>
												</xsl:when>
												<xsl:otherwise>
...
- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
-->
	<xsl:template name="bPOSContainsInflAffixSlot">
		<xsl:param name="pos" select="."/>
		<xsl:param name="idSlot"/>
		<xsl:if test="$pos/descendant::MoInflAffixSlot[@Id=$idSlot]">
			<xsl:text>1</xsl:text>
		</xsl:if>
		<xsl:for-each select="$pos/SubPossibilities">
			<xsl:call-template name="bPOSContainsInflAffixSlot">
								...
								<xsl:when test="ancestor::FromExceptionFeatures">
									<xsl:variable name="posToMatch">
										<xsl:choose>
											...
											<xsl:otherwise>
												<!-- MoInflAffMsa -->
												<xsl:value-of select="ancestor::PartOfSpeechRef/@dst"/>
											</xsl:otherwise>
										</xsl:choose>
									</xsl:variable>
									<xsl:call-template name="bPosMatchesOrIsSubCat">
...
-->
	...
	<xsl:template name="CalculateUnusedSlots">
		<xsl:for-each select="//MoInflAffixSlot">
			...
			<xsl:variable name="templateWithSlot" select="../../descendant-or-self::SuffixSlots[@dst=$idSlot] | ../../descendant-or-self::PrefixSlots[@dst=$idSlot]"/>
			<xsl:if test="not($templateWithSlot)">
	...
	<xsl:template name="DefinePOS">
		...
		<section2 type="tH2">
			...
			<xsl:if test="$iCountTemplates &gt; 0">
				...
				<xsl:variable name="iSubcatCount" select="count($pos/descendant::PartOfSpeech)"/>
				<xsl:if test="$iSubcatCount &gt; 0">
					<p>
						...
						<xsl:for-each select="$pos/descendant::PartOfSpeech">
							<xsl:sort select="Name"/>
							<genericRef>
								<xsl:attribute name="gref">
									<xsl:text>sCat.</xsl:text>
...
-->
	<xsl:template name="GetCitationForm">
		...
		<td>
			...
			<langData lang="lVern">
				<xsl:choose>
					<xsl:when test="$LexEntry/CitationForm">
						<xsl:value-of select="$LexEntry/CitationForm"/>
					</xsl:when>
					<xsl:when test="$LexEntry/LexemeForm">
	...
	<xsl:template name="GetDefinition">
		...
		<td>
			...
			<xsl:choose>
				<xsl:when test="string-length(normalize-space($definition)) &gt; 0">
					<!--                    <xsl:text>  '</xsl:text>-->
					<gloss lang="lGloss">
	...
	<xsl:template name="GetMSAInfo">
		...
		<xsl:choose>
			...
			<xsl:when test="name($msa)='MoInflAffMsa'">
				<xsl:call-template name="GetPOSAbbr">
					<xsl:with-param name="posid" select="$msa/PartOfSpeechRef/@dst"/>
				</xsl:call-template>
				<xsl:text>:</xsl:text>
				<xsl:choose>
	...
	<xsl:template name="OutputRegularPhonologicalRule">
		<table>
			<tr>
				...
				<td valign="middle">---&gt;</td>
				<td valign="middle">
					<xsl:variable name="structuralChange" select="RightHandSides/StrucChange/*"/>
					<xsl:choose>
				...
				<td valign="middle">/</td>
				<td valign="middle">
					<xsl:for-each select="RightHandSides/LeftContext/*">
						<xsl:call-template name="OutputPhonologicalRuleItem">
							<xsl:with-param name="item" select="."/>
						</xsl:call-template>
					</xsl:for-each>
				...
				<td valign="middle">
					<xsl:for-each select="RightHandSides/RightContext/*">
						<xsl:call-template name="OutputPhonologicalRuleItem">
							<xsl:with-param name="item" select="."/>
						</xsl:call-template>
					</xsl:for-each>
			...
			<xsl:when test="not($slots)">
				<p>
					...
					<xsl:if test="$pos/AffixTemplates/MoInflAffixTemplate">
						...
						<xsl:choose>
							<xsl:when test="count($higherSlots) &gt; 1">
						...
						<xsl:for-each select="$higherSlots">
							<xsl:sort select="Name"/>
							<genericRef>
								<xsl:attribute name="gref">
			...
			<xsl:otherwise>
				<p>
					...
					<xsl:if test="$subTemplates">
						...
						<xsl:if test="count($subTemplates) &gt; 1">s</xsl:if>
						...
						<xsl:choose>
							<xsl:when test="count($subcats) &gt; 1">these subcategories: </xsl:when>
							<xsl:otherwise>the subcategory </xsl:otherwise>
						</xsl:choose>
						<xsl:for-each select="$subcats">
							<xsl:sort select="Name"/>
							<genericRef>
								<xsl:attribute name="gref">
									<xsl:text>sInfl.</xsl:text>
									<xsl:value-of select="$subcats/@Id"/>
								</xsl:attribute>
								<xsl:value-of select="Name"/>
							</genericRef>
							<xsl:call-template name="OutputListPunctuation"/>
		...
		<xsl:if test="string-length($sUnusedSlots) &gt; 0">
			...
			<xsl:for-each select="$slot">
				<tr>
					...
					<td>
						<xsl:if test="not(../../descendant-or-self::AffixTemplates/MoInflAffixTemplate)">
						<genericTarget id="sInflSlotFillers.{$slot/@Id}"/>
						</xsl:if>
						<xsl:value-of select="$slot/Name"/>
					</td>
				</tr>
		...
		<xsl:choose>
			<xsl:when test="not($pos/AffixTemplates/MoInflAffixTemplate)">
				...
				<p>
					...
					<xsl:value-of select="$sPOSName"/>
					<xsl:choose>
						<xsl:when test="$subcats">
							<xsl:text>, but there are for its subcategor</xsl:text>
							<xsl:choose>
								<xsl:when test="count($subcats)&gt;1">
			...
			<xsl:otherwise>
				<p>
					The category <xsl:value-of select="$sPOSName"/> has the following template<xsl:if test="count($pos/AffixTemplates/MoInflAffixTemplate)!='1'">s</xsl:if>.
				</p>
				<xsl:for-each select="$pos/AffixTemplates/MoInflAffixTemplate">
					<xsl:sort select="Name"/>
					<xsl:choose>
...
-->
	<xsl:template name="ProcessInflectionForPOS">
		...
		<p>
			This section lists all inflectional templates and slots for the <xsl:value-of select="$sPOSName"/> category<xsl:if test="PartOfSpeech[descendant::MoInflAffixTemplate]"> and its subcategories</xsl:if>.
		</p>
