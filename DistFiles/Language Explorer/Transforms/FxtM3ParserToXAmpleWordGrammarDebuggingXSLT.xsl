   ...
   <xsl:variable name="sAllStemNamesUsedInLexicalEntries">
	  <xsl:for-each select="//LexEntry">
		 ...
		 <xsl:variable name="sStemNamesUsed">
			<xsl:for-each select="//MoStemAllomorph[@Id=$allos/@dst and @IsAbstract='0' and @MorphType!=$sPhrase and @MorphType!=$sDiscontiguousPhrase and @StemName!='0']">
...
- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
		 ...
		 <xsl:element name="xsl:template">
			...
			<xsl:if test="not(//CompoundRules[MoExoCompound | MoEndoCompound])">
			   <xsl:element name="xsl:call-template">
...
- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
		 ...
		 <xsl:element name="xsl:template">
			...
			<seq>
			   <xsl:attribute name="step">
				  ...
				  <xsl:choose>
					 <xsl:when test="@Final=0">Stem</xsl:when>
   ...
   <xsl:template name="CompoundRuleNamedTemplates">
	  <xsl:if test="//CompoundRules[MoExoCompound | MoEndoCompound]">
		 <xsl:for-each select="//MoExoCompound | //MoEndoCompound">
   ...
   <xsl:template name="StemStemProductionsNamedTemplate">
	  ...
	  <xsl:element name="xsl:template">
		 <xsl:attribute name="name">StemStemProductions</xsl:attribute>
		 <xsl:choose>
			<xsl:when test="//CompoundRules[MoExoCompound | MoEndoCompound]">
			   <xsl:for-each select="//MoExoCompound | //MoEndoCompound">
				  <xsl:element name="xsl:call-template">
					 <xsl:attribute name="name">
						<xsl:text>CompoundRule</xsl:text>
...
- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	  ...
	  <xsl:element name="xsl:template">
		 ...
		 <xsl:choose>
			<xsl:when test="key('SlotID',@dst)/@Optional='0'">
			   <!-- slot is required -->
			   <xsl:element name="xsl:choose">
				  <xsl:element name="xsl:when">
...
-->
   <xsl:template name="CreateMainInflectionTemplateCore">
	  ...
	  <xsl:param name="pos"/>
	  <xsl:choose>
		 <xsl:when test="$template/@Final='1'">
			<xsl:comment>have a full</xsl:comment>
			<full>
			   <xsl:call-template name="ProcessInflectionalTemplatePercolationAndConstraints">
				  <xsl:with-param name="template" select="$template"/>
...
-->
   <xsl:template name="CreateStemNameCheckForSuperPOS">
	  ...
	  <xsl:variable name="superPOSes" select="//PartOfSpeech/SubPossibilities[@dst=$pos]"/>
	  <xsl:choose>
   ...
   <xsl:template name="StemNameRelevantToPOS">
	  ...
	  <xsl:choose>
		 ...
		 <xsl:otherwise>
			...
			<xsl:for-each select="//PartOfSpeech/SubPossibilities[@dst=$pos/@Id]">
			   <xsl:call-template name="StemNameRelevantToPOS">
...
-->
   <xsl:template name="SuperPOSHasTemplate">
	  ...
	  <xsl:if test="not($CurrentPOS/AffixTemplates/MoInflAffixTemplate)">
		 ...
		 <xsl:for-each select="//PartOfSpeech/SubPossibilities[@dst=$pos]">
			<xsl:call-template name="SuperPOSHasTemplate">
			   <xsl:with-param name="pos" select="../@Id"/>
			</xsl:call-template>
		 </xsl:for-each>
