	...
	<GAFAWSData>
	  <WordRecords>
		<xsl:for-each select="descendant-or-self::MoInflAffixTemplate[PrefixSlots or SuffixSlots]">
		  <WordRecord>
			<xsl:attribute name="WRID"><xsl:value-of select="@Id"/></xsl:attribute>
			<xsl:if test="count(PrefixSlots) > 0">
			  <Prefixes>
				<xsl:for-each select="PrefixSlots">
				  <Affix>
					<xsl:attribute name="MIDREF"><xsl:value-of select="@dst"/></xsl:attribute>
				  </Affix>
				</xsl:for-each>
			  </Prefixes>
			</xsl:if>
	  ...
	  <Morphemes>
		...
		<xsl:for-each select="descendant-or-self::MoInflAffixSlot">
		  <Morpheme>
			<xsl:attribute name="MID"><xsl:value-of select="@Id"/></xsl:attribute>
			<xsl:attribute name="type"><xsl:choose><xsl:when test="key('PrefixSlots',@Id)"><xsl:text>pfx</xsl:text></xsl:when><xsl:otherwise><xsl:text>sfx</xsl:text></xsl:otherwise></xsl:choose></xsl:attribute>
		  </Morpheme>
		</xsl:for-each>
	  </Morphemes>
	  <Classes>
...
Revision History
- - - - - - - - - - - - - - - - - - -
19-Jun-2006      Andy Black     Only operate on templates with slots.
17-Mar-2004      Andy Black     Initial draft
================================================================
 -->
