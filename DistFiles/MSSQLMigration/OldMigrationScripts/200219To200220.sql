-- Update database from version 200219 to 200220
BEGIN TRANSACTION  --( will be rolled back if wrong version#)

-------------------------------------------------------------------------------
-- FWM-144: Add LgWritingSystem.ValidChars
-------------------------------------------------------------------------------

insert into [Field$]
	([Id], [Type], [Class], [DstCls], [Name], [Custom], [CustomId], [Min], [Max], [Big])
	values(24028, 19, 24,
		null, 'ValidChars',0,Null, null, null, null)
go

-------------------------------------------------------------------------------
-- FWD-201: Change NVARCHAR(4000) to NVARCHAR(MAX)
-------------------------------------------------------------------------------
...
DECLARE	@ClassId INT;
DECLARE Classes CURSOR FOR SELECT c.Id FROM Class$ c WHERE c.Abstract = 0
OPEN Classes;
...
IF @dbVersion = 200219
BEGIN
	UPDATE Version$ SET DbVer = 200220
	COMMIT TRANSACTION
	PRINT 'database updated to version 200220'
END
ELSE
BEGIN
	ROLLBACK TRANSACTION
	PRINT 'Update aborted: this works only if DbVer = 200219 (DbVer = ' +
