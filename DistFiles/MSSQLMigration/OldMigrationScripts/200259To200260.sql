-- Update database from version 200259 to 200260
BEGIN TRANSACTION  --( will be rolled back if wrong version#)

-------------------------------------------------------------------------------
-- LT-9523, LTB-197, LTB-394, LTB-384, LTB-358, LTB-779, LT-361:
-- The parser (ParserFiler.cs) is removing data but then asking
-- UpdWifAnalysisAndEval$ to add them. The data is not valid anymore,
-- and it is okay to discard it without raising an error.
-------------------------------------------------------------------------------
...
IF @dbVersion = 200259
BEGIN
	UPDATE Version$ SET DbVer = 200260
	COMMIT TRANSACTION
	PRINT 'database updated to version 200260'
END
ELSE
BEGIN
	ROLLBACK TRANSACTION
	PRINT 'Update aborted: this works only if DbVer = 200259 (DbVer = ' +
