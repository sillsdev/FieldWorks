-- Update database from version 200230 to 200230
BEGIN TRANSACTION  --( will be rolled back if wrong version#)

-------------------------------------------------------------------------------
-- Fix the CopyObj$ stored procedure to prevent OwnOrd == NULL for objects
-- being copied into an owning sequence and to make it possible to indicate
-- where in a sequence copied objects should be inserted
-------------------------------------------------------------------------------
...
AS
	...
	IF (@nTopDestType != @kcptOwningSequence AND @hvoDstStart IS NOT NULL) BEGIN
		...
		GOTO LFail
	END
	IF (@nTopDestType = @kcptOwningSequence) BEGIN
...
-------------------------------------------------------------------------------
-- Fix any existing objects that are part of an owning sequence but have NULL
-- OwnOrd$ values
-------------------------------------------------------------------------------
-- This first update handles ScrBooks (the most common culprit) in a special
-- way to guarantee that they are ordered by their canonical number.
UPDATE scrbook_
SET ownord$ = CanonicalNum
WHERE ownord$ is null
GO

UPDATE cmobject
SET ownord$ = cmobject.id - (
	SELECT min(id)
	FROM CmObject o2
	WHERE o2.ownord$ is null
		AND o2.owner$ = cmobject.owner$
		AND o2.ownflid$ = cmobject.ownflid$
	GROUP BY owner$, ownflid$) + 1
FROM CmObject
join Field$ f on f.Id = cmobject.OwnFlid$ and f.type = 27
WHERE cmobject.ownord$ is null
GO

-- If the above updates are successful, this query should return no rows
--select *
--from cmobject o
--join field$ f on f.id = o.OwnFlid$ and f.type = 27
--where ownord$ is null
...
IF @dbVersion = 200229
BEGIN
	UPDATE Version$ SET DbVer = 200230
	COMMIT TRANSACTION
	PRINT 'database updated to version 200230'
END
ELSE
BEGIN
	ROLLBACK TRANSACTION
	PRINT 'Update aborted: this works only if DbVer = 200229 (DbVer = ' +
