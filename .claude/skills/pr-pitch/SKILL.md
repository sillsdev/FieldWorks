...
**3. Where to look** (at most five bullets) -- open with one
`**Start here:**` line naming the first file to read and why. That is the
*entry point*, not the riskiest thing; a reviewer not told where to begin
reads the diff alphabetically, which is nobody's reading order. Then the
failure points a domain expert would anticipate, ordered by what would sink
the PR -- one line each: the risk, and the thing that pins it. **The proof goes
in an accordion**; inlining it is what blows the budget.
