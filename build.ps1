...
# -TokenHygiene blocks the build, and CI passes it, so CI fails rather than
# annotating. Without the flag but with CI set, the run is advisory and
# annotates instead. An ordinary developer build is silent.
...
try {
	...
	Invoke-WithFileLockRetry -Context "FieldWorks build" -IncludeOmniSharp -RepoRoot $PSScriptRoot -Action {
		...
		$localPropsPath = Get-FieldWorksLocalLibraryPropsPath -RepositoryRoot $PSScriptRoot

		# Only when this build will restore. Removing packages that nothing is
		# going to put back would leave the tree unbuildable.
		if (-not $SkipRestore) {
			Clear-FieldWorksLocalLibraries -PackagesDirectory $packagesDir `
				-LocalRepository $localFeed

			# An earlier selection goes first, so a build naming no library
			# resolves published versions even in a restore it does not launch.
			Remove-FieldWorksLocalLibraryProps -Path $localPropsPath
		}
		...
		if ($LocalLibraries.Count -gt 0) {
			...
			try {
				# No $LASTEXITCODE check: that script reports failure by throwing,
				# so the variable would hold the last native command's result.
		...
		if ($LocalLibraries.Count -gt 0) {
			# Say how to undo it now, while the person who chose it is here.
			Write-Host "To go back to published packages, build without -LocalLibraries." `
