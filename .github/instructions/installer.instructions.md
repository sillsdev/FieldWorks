...
---
# Installer development guidelines (WiX 3 default, WiX 6 opt-in)

## Purpose & Scope
Guidance for building, validating, and debugging the FieldWorks installer for both toolsets: MSI (`FieldWorks.msi`) and bootstrapper bundle (`FieldWorksBundle.exe`).

## Key facts (FieldWorks repo)
- WiX 3 inputs live in `FLExInstaller/` (legacy batch pipeline; default).
- WiX 6 projects live in `FLExInstaller/wix6/`:
...
- Default installer artifacts (Debug) land under:
	...
	- WiX 6: `FLExInstaller\wix6\bin\x64\Debug\en-US\FieldWorks.msi`
	- WiX 6: `FLExInstaller\wix6\bin\x64\Debug\FieldWorksBundle.exe`

## Build commands
```powershell
# Preferred: traversal build wrapper (Debug, WiX 3 default)
...
# Bundle (preferred for end-user scenario, WiX 3 default)
.\FLExInstaller\bin\x64\Debug\FieldWorksBundle.exe /log "$e\bundle.log"

# Bundle (WiX 6 opt-in)
.\FLExInstaller\wix6\bin\x64\Debug\FieldWorksBundle.exe /log "$e\bundle.log"

# MSI (direct Windows Installer engine, WiX 3 default)
msiexec /i .\FLExInstaller\bin\x64\Debug\en-US\FieldWorks.msi /l*v "$e\msi-install.log"

# MSI (WiX 6 opt-in)
msiexec /i .\FLExInstaller\wix6\bin\x64\Debug\en-US\FieldWorks.msi /l*v "$e\msi-install.log"
```
