...
- FieldWorks is in a transition period: the repo currently defaults installer builds to WiX 3, but the migration target is WiX 6-first and ultimately WiX 6 as default.
- WiX 3 authoring lives in `FLExInstaller/` and legacy jobs may still checkout `sillsdev/genericinstaller` as `PatchableInstaller/`; `PatchableInstaller/` is not restored in this worktree.
