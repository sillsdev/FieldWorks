...
> **Scope clarification**: This document conflates two distinct workstreams:
> 1. **Build infrastructure fixes** (issues #1–6, #8): SDK migration, NuGet alignment, project file hygiene
> 2. **Test code fixes** (issue #7 only): Actual test implementation corrections
>
> Consider splitting into:
> - `BUILD_MIGRATION_FIXES.md` — project file, package reference, and SDK issues
> - `TEST_CODE_FIXES.md` — test implementation changes
>
> Note: Issue #5 (test code in production assemblies) reveals pre-existing build debt, not a migration-induced problem. This should be tracked separately as technical debt that was *discovered* during migration, not *caused* by it.
