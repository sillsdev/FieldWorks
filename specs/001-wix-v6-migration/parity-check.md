...
- MSI: `FLExInstaller/FieldWorks.Installer.wixproj` + `FLExInstaller/Shared/Base/*.wxs` + `FLExInstaller/Shared/Common/*.wxi`
- Bundle: `FLExInstaller/FieldWorks.Bundle.wixproj` + `FLExInstaller/Shared/Base/Bundle.wxs` + theme files
...
Hyper-V deterministic lane (script-driven):

- Setup/usage: [HYPERV_INSTALLER_TESTING.md](HYPERV_INSTALLER_TESTING.md)
- Parity driver:
...
- Evidence is written on the host to `Output\InstallerEvidence\HyperV\` and can be published to `specs/001-wix-v6-migration/evidence/`.
...
- Build: `./build.ps1 -BuildInstaller -Configuration Release`
...
If you need **deterministic clean-machine parity** (recommended for WiX3 vs WiX6):

- Use the Hyper-V lane described in [HYPERV_INSTALLER_TESTING.md](HYPERV_INSTALLER_TESTING.md).

This produces two evidence folders (WiX3 baseline and WiX6 candidate) and a generated diff report.
...
| Example: L10n payload inclusion | Legacy pipeline staged + harvested l10n implicitly | Heat harvest + feature reference | `FLExInstaller/FieldWorks.Installer.wixproj` (`HarvestedL10nFiles`) + `Framework.wxs` feature refs | ☐ | Install file listing |
...
| MSI ICE validation | Typically implicit (or run manually) | Explicit `wix.exe msi validate -sice ICE60 -wx` in build | `FLExInstaller/FieldWorks.Installer.wixproj` (`WindowsInstallerValidation`) | ☑ | Present in Release build output |
| | | | | | |
| | | | | | |
| | | | | | |
| | | | | | |
...
- Target build artifacts (Release):
  - `FLExInstaller\bin\x64\Release\en-US\FieldWorks.msi` (SHA256 `7AED3302AD3A273DAAA29D97B5829E91E684699C9150803AA26A35E29B415C09`)
  - `FLExInstaller\bin\x64\Release\en-US\FieldWorks.wixpdb`
  - `FLExInstaller\bin\x64\Release\FieldWorksBundle.exe` (SHA256 `365901C88C211C1A7019CBCB8EFB54523D212393976DA518A388E42CF1CC7F62`)
  - `FLExInstaller\bin\x64\Release\FieldWorksBundle.wixpdb`
