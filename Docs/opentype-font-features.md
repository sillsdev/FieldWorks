...
Feature discovery is script-blind, and for these features that is what the specification describes. `OpenTypeFontFeatureInfoReader` walks the feature list flat and deduplicates by tag, so a font that gave the same tag different labels or options under different scripts would show one script's strings to another. The OpenType feature registry lists Script/language sensitivity: None for both `cv01`-`cv99` and `ss01`-`ss20`, so one label set per tag is the documented behaviour for them. Making it script-aware requires passing the writing system's script into the reader; do that if a font appears that needs it.
